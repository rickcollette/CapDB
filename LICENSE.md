...
**CapDB** is a fork of SQLite with additional pooling and network extensions.
The project is dual-origin:

  *  **CapDB-specific additions are licensed under the MIT License**
     (© 2026 Rick Collette) — see [LICENSE](LICENSE). This covers the connection
     pool, the client/server wire protocol and networking, the TLS layer, the
     SQL server, and the network client, namely `capdb/**` (proto, io, tls,
     server, client, vfs, shell, jni), `capdb/pool/**`, `tools/capdb-server.c`, and
     the corresponding CapDB-specific code emitted into the generated `capdb.c` /
     `capdb.h` amalgamation.
  *  **The underlying SQLite engine remains in the public domain** (see below).
     Public-domain code cannot be relicensed; the MIT grant above applies only
     to CapDB's own contributions layered on top of it.

See [docs/REBRAND.md](docs/REBRAND.md) for the rename mapping.
...
that are public domain include the following:
  ...
  *  All code that ends up in the "capdb.c" and "capdb.h" build products
  ...
  *  All of the code used to build various utility programs such as
     "sqldiff", "capdb_rsync", and "capdb_analyzer".
...
the build products, such as "capdb.c", however, so no attribution is
