...
# 2.0 Setup
  ...
  *   Run: "`tclsh json-generator.tcl | capdb json100mb.db`" to create
  ...
  *   Bring up jsonb100mb.db in the capdb command-line shell.
      Convert all of the content into JSONB using a commands like this:

>        UPDATE data1 SET x=jsonb(x);
>        VACUUM;

  *   Build the baseline capdb.c file with capdb.h and shell.c.
...
# 3.0 Testing
  ...
  *   Build the capdb.c (with capdb.h and shell.c) to be tested.
