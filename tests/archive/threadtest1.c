...
extern char *capdb_mprintf(const char *zFormat, ...);
extern char *capdb_vmprintf(const char *zFormat, va_list);
...
static int db_query_callback(
  ...
  for(i=0; i<nArg; i++){
    pResult->azElem[pResult->nElem++] =
        capdb_mprintf("%s",azArg[i] ? azArg[i] : ""); 
  }
  return 0;
}
...
char **db_query(sqlite *db, const char *zFile, const char *zFormat, ...){
  ...
  rc = capdb_exec(db, zSql, db_query_callback, &sResult, &zErrMsg);
  if( rc==CAPDB_SCHEMA ){
...
static void *worker_bee(void *pArg){
  ...
  for(cnt=0; cnt<10; cnt++){
    capdb_open(&zFilename[2], &db);
    if( db==0 ){
      fprintf(stdout,"%s: can't open\n", zFilename);
      Exit(1);
    }
    capdb_busy_handler(db, db_is_locked, zFilename);
