...
** Compile as follows:
**
**    gcc -I. wordcount.c capdb.c -ldl -lpthreads
**
** Or:
**
**    gcc -I. -DCAPDB_THREADSAFE=0 -DCAPDB_OMIT_LOAD_EXTENSION \
**        wordcount.c capdb.c
*/
#include <stdio.h>
#include <string.h>
...
" --stats              Show capdb_status() results at the end.\n"
...
" --trace              Enable capdb_trace() output.\n"
...
static capdb_int64 realTime(void){
...
/* An capdb_exec() callback that prints results on standard output,
...
static void finalHash(unsigned int *a, char *z){
  a[3] += a[1] + a[4] + a[0];
  a[4] += a[2] + a[3];
  capdb_snprintf(17, z, "%08x%08x", a[3], a[4]);
}
...
static void checksumStep(
  ...
  int nVal, i, j;
  unsigned int *a;
  a = (unsigned*)capdb_aggregate_context(context, sizeof(unsigned int)*5);

  if( a ){
    for(i=0; i<argc; i++){
      nVal = capdb_value_bytes(argv[i]);
      zVal = (const unsigned char*)capdb_value_text(argv[i]);
...
static void checksumFinalize(capdb_context *context){
  unsigned int *a;
  char zResult[24];
  a = capdb_aggregate_context(context, 0);
  if( a ){
    finalHash(a, zResult);
    capdb_result_text(context, zResult, -1, CAPDB_TRANSIENT);
  }
}
...
int main(int argc, char **argv){
  ...
  capdb_stmt *pInsert = 0;    /* The INSERT statement */
  capdb_stmt *pUpdate = 0;    /* The UPDATE statement */
  capdb_stmt *pSelect = 0;    /* The SELECT statement */
  capdb_stmt *pDelete = 0;    /* The DELETE statement */
  ...
  capdb_int64 sumCnt = 0;     /* Sum in QUERY mode */
  capdb_int64 startTime;      /* Time of start */
  capdb_int64 totalTime = 0;  /* Total time */
  ...
  if( doTrace ) capdb_trace(db, traceCallback, 0);
  if( pageSize ){
    zSql = capdb_mprintf("PRAGMA page_size=%d", pageSize);
    capdb_exec(db, zSql, 0, 0, 0);
    capdb_free(zSql);
  }
  if( cacheSize ){
    zSql = capdb_mprintf("PRAGMA cache_size=%d", cacheSize);
    capdb_exec(db, zSql, 0, 0, 0);
    capdb_free(zSql);
  }
  if( noSync ) capdb_exec(db, "PRAGMA synchronous=OFF", 0, 0, 0);
  if( zJMode ){
    zSql = capdb_mprintf("PRAGMA journal_mode=%s", zJMode);
    capdb_exec(db, zSql, 0, 0, 0);
    capdb_free(zSql);
  }
  ...
  while( allLoop(iMode, &iLoopCnt, &iMode2, &useWithoutRowid) ){
    ...
    if( iMode==MODE_ALL ){
      if( capdb_exec(db, "DROP TABLE IF EXISTS wordcount; VACUUM;",0,0,0) ){
    ...
    if( capdb_exec(db, "BEGIN IMMEDIATE", 0, 0, 0) ){
    ...
    while( fgets(zInput, sizeof(zInput), in) ){
      for(i=0; zInput[i]; i++){
        ...
        ** Process it into the wordcount table.  */
        if( iMode2==MODE_DELETE ){
          capdb_bind_text(pDelete, 1, zInput+i, j-i, CAPDB_STATIC);
          if( capdb_step(pDelete)!=CAPDB_DONE ){
            fatal_error("DELETE failed: %s\n", capdb_errmsg(db));
          }
          capdb_reset(pDelete);
        }else if( iMode2==MODE_SELECT ){
          capdb_bind_text(pSelect, 1, zInput+i, j-i, CAPDB_STATIC);
          rc = capdb_step(pSelect);
          capdb_reset(pSelect);
          if( rc==CAPDB_ROW ){
            capdb_bind_text(pUpdate, 1, zInput+i, j-i, CAPDB_STATIC);
            if( capdb_step(pUpdate)!=CAPDB_DONE ){
              fatal_error("UPDATE failed: %s\n", capdb_errmsg(db));
            }
            capdb_reset(pUpdate);
          }else if( rc==CAPDB_DONE ){
            capdb_bind_text(pInsert, 1, zInput+i, j-i, CAPDB_STATIC);
            if( capdb_step(pInsert)!=CAPDB_DONE ){
          ...
          if( capdb_step(pSelect)==CAPDB_ROW ){
            sumCnt += capdb_column_int64(pSelect, 0);
          }
          capdb_reset(pSelect);
        }else{
          capdb_bind_text(pInsert, 1, zInput+i, j-i, CAPDB_STATIC);
          ...
          capdb_reset(pInsert);
          if( iMode2==MODE_UPDATE
           || (iMode2==MODE_INSERT && capdb_changes(db)==0)
          ){
            capdb_bind_text(pUpdate, 1, zInput+i, j-i, CAPDB_STATIC);
            if( capdb_step(pUpdate)!=CAPDB_DONE ){
              fatal_error("UPDATE failed: %s\n", capdb_errmsg(db));
        ...
        if( commitInterval>0 && (nOp%commitInterval)==0 ){
          capdb_exec(db, "COMMIT; BEGIN IMMEDIATE", 0, 0, 0);
        }
      }
    }
    capdb_exec(db, "COMMIT", 0, 0, 0);
    capdb_finalize(pInsert);  pInsert = 0;
    capdb_finalize(pUpdate);  pUpdate = 0;
    capdb_finalize(pSelect);  pSelect = 0;
    capdb_finalize(pDelete);  pDelete = 0;
  
    if( iMode2==MODE_QUERY && iMode!=MODE_ALL ){
      printf("%s sum of cnt: %lld\n", zTag, sumCnt);
      rc = capdb_prepare_v2(db,"SELECT sum(cnt*cnt) FROM wordcount", -1,
                              &pSelect, 0);
      if( rc==CAPDB_OK && capdb_step(pSelect)==CAPDB_ROW ){
        printf("%s double-check: %lld\n", zTag,capdb_column_int64(pSelect,0));
      }
      capdb_finalize(pSelect);
    }
  
  
    if( showTimer ){
      capdb_int64 elapseTime = realTime() - startTime;
    ...
    if( showSummary ){
      capdb_create_function(db, "checksum", -1, CAPDB_UTF8, 0,
  ...
  ** have been finalized */
  if( showStats ){
    capdb_db_status(db, CAPDB_DBSTATUS_LOOKASIDE_USED, &iCur, &iHiwtr, 0);
    printf("%s Lookaside Slots Used:        %d (max %d)\n", zTag, iCur,iHiwtr);
    capdb_db_status(db, CAPDB_DBSTATUS_LOOKASIDE_HIT, &iCur, &iHiwtr, 0);
    printf("%s Successful lookasides:       %d\n", zTag, iHiwtr);
    capdb_db_status(db, CAPDB_DBSTATUS_LOOKASIDE_MISS_SIZE, &iCur,&iHiwtr,0);
    printf("%s Lookaside size faults:       %d\n", zTag, iHiwtr);
    capdb_db_status(db, CAPDB_DBSTATUS_LOOKASIDE_MISS_FULL, &iCur,&iHiwtr,0);
    printf("%s Lookaside OOM faults:        %d\n", zTag, iHiwtr);
    capdb_db_status(db, CAPDB_DBSTATUS_CACHE_USED, &iCur, &iHiwtr, 0);
    ...
    capdb_db_status(db, CAPDB_DBSTATUS_SCHEMA_USED, &iCur, &iHiwtr, 0);
    printf("%s Schema Heap Usage:           %d bytes\n", zTag, iCur); 
    capdb_db_status(db, CAPDB_DBSTATUS_STMT_USED, &iCur, &iHiwtr, 0);
  ...
  ** has closed.  Memory usage should be zero at this point. */
  if( showStats ){
    capdb_status(CAPDB_STATUS_MEMORY_USED, &iCur, &iHiwtr, 0);
    printf("%s Memory Used (bytes):         %d (max %d)\n", zTag,iCur,iHiwtr);
    capdb_status(CAPDB_STATUS_MALLOC_COUNT, &iCur, &iHiwtr, 0);
    printf("%s Outstanding Allocations:     %d (max %d)\n",zTag,iCur,iHiwtr);
    capdb_status(CAPDB_STATUS_PAGECACHE_OVERFLOW, &iCur, &iHiwtr, 0);
    printf("%s Pcache Overflow Bytes:       %d (max %d)\n",zTag,iCur,iHiwtr);
    capdb_status(CAPDB_STATUS_MALLOC_SIZE, &iCur, &iHiwtr, 0);
    printf("%s Largest Allocation:          %d bytes\n",zTag,iHiwtr);
    capdb_status(CAPDB_STATUS_PAGECACHE_SIZE, &iCur, &iHiwtr, 0);
