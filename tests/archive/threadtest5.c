...
/* If rc is not CAPDB_OK, then print an error message and stop
** the test.
*/
...
*/
static capdb_int64 gettime(void){
  capdb_int64 tm;
...
*/
static void waitOnTable(capdb *db, const char *zWorker, const char *zTable){
  while(1){
    int eFound = 0;
    capdb_stmt *q = prepare(db, zWorker, __LINE__,
             "SELECT 1 FROM sqlite_schema WHERE name=%Q", zTable);
    if( capdb_step(q)==CAPDB_ROW && capdb_column_int(q,0)!=0 ){
      eFound = 1;
    }
    capdb_finalize(q);
    if( eFound ) return;
...
static void *worker(void *pArg){
  ...
  error_out(rc, "capdb_open", __LINE__);
  capdb_busy_timeout(db, 2000);
  ...
  while( 1 ){
    ...
    if( capdb_step(q1)==CAPDB_ROW ){
      tid = capdb_column_int(q1,0);
    }
    capdb_finalize(q1);
    if( tid<0 ) break;
    if( eVerbose ){
...
int main(int argc, char **argv){
  ...
  capdb_config(CAPDB_CONFIG_URI, (int)1);
  rc = capdb_open(zDbName, &db);
  error_out(rc, "capdb_open", __LINE__);
  ...
  error_out(rc, "capdb_exec", __LINE__);

  for(i=0; i<nWorker; i++){
    capdb_snprintf(sizeof(aWorkerName[i]), aWorkerName[i],
             "W%02d", i);
  ...
  for(i=0; i<nWorker; i++){
    q = prepare(db, "MAIN", __LINE__,
          ...
          aWorkerName[i]);
    if( capdb_step(q)==CAPDB_ROW ){
      printf("%s: %s\n", aWorkerName[i], capdb_column_text(q,0));
    }
    capdb_finalize(q);
  }
  q = prepare(db, "MAIN", __LINE__, "SELECT count(*) FROM p2");
  if( capdb_step(q)!=CAPDB_ROW || capdb_column_int(q,0)<10 ){
