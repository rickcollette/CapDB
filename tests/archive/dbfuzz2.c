...
**       -DCAPDB_ENABLE_DBSTAT_VTAB dbfuzz2.c capdb.c -ldl
**     ./a.out dir
*/
#include <assert.h>
#include <stdio.h>
...
static capdb_int64 szMax = 104857600;
...
static capdb_mem_methods memtraceBase;
...
static capdb_mem_methods ersaztMethods = {
  memtraceMalloc,
  memtraceFree,
  memtraceRealloc,
  memtraceSize,
...
int capdbMemTraceActivate(FILE *out){
  int rc = CAPDB_OK;
  if( memtraceBase.xMalloc==0 ){
    rc = capdb_config(CAPDB_CONFIG_GETMALLOC, &memtraceBase);
    if( rc==CAPDB_OK ){
      rc = capdb_config(CAPDB_CONFIG_MALLOC, &ersaztMethods);
    }
  }
  memtraceOut = out;
  return rc;
...
int capdbMemTraceDeactivate(void){
  int rc = CAPDB_OK;
  if( memtraceBase.xMalloc!=0 ){
    rc = capdb_config(CAPDB_CONFIG_MALLOC, &memtraceBase);
    if( rc==CAPDB_OK ){
...
int LLVMFuzzerTestOneInput(const uint8_t *aData, size_t nByte){
  ...
  rc = capdb_open(0, &db);
  if( rc ) return 1;
  a = capdb_malloc64(nByte+1);
  if( a==0 ) return 1;
  memcpy(a, aData, nByte);
  capdb_deserialize(db, "main", a, nByte, nByte,
        CAPDB_DESERIALIZE_RESIZEABLE |
        CAPDB_DESERIALIZE_FREEONCLOSE);
  x = szMax;
#ifdef CAPDB_FCNTL_SIZE_LIMIT
  capdb_file_control(db, "main", CAPDB_FCNTL_SIZE_LIMIT, &x);
#endif
  if( bVdbeDebug ){
    capdb_exec(db, "PRAGMA vdbe_debug=ON", 0, 0, 0);
  }
  if( mxCb>0 ){
    capdb_progress_handler(db, 10, progress_handler, 0);
  }
#ifdef CAPDB_TESTCTRL_PRNG_SEED
  capdb_test_control(CAPDB_TESTCTRL_PRNG_SEED, 1, db);
#endif
  for(i=0; i<sizeof(azSql)/sizeof(azSql[0]); i++){
    ...
    zErr = 0;
    nCb = 0;
    rc = capdb_exec(db, azSql[i], 0, 0, &zErr);
    if( rc && eVerbosity>=1 ){
      printf("-- rc=%d zErr=%s\n", rc, zErr);
    }
    capdb_free(zErr);
  ...
  rc = capdb_close(db);
  if( rc!=CAPDB_OK ){
    fprintf(stdout, "capdb_close() returns %d\n", rc);
  }
  if( capdb_memory_used()!=0 ){
    int nAlloc = 0;
    int nNotUsed = 0;
    capdb_status(CAPDB_STATUS_MALLOC_COUNT, &nAlloc, &nNotUsed, 0);
    fprintf(stderr,"Memory leak: %lld bytes in %d allocations\n",
            capdb_memory_used(), nAlloc);
    exit(1);
  }
  return 0;
}
  ...
  for(i=j=1; i<argc; i++){
    ...
    if( z[0]=='-' ){
      ...
      if( strcmp(z, "lookaside")==0 ){
        ...
        sz = atoi(argv[++i]);
        nSlot = atoi(argv[++i]);
        capdb_config(CAPDB_CONFIG_LOOKASIDE, sz, nSlot);
        continue;
      }
#ifndef _WIN32
