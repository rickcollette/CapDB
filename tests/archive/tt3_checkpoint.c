...
** registered with capdb_wal_hook() to try to keep the WAL-size limited to 
** around 50 pages.
**
** In test case checkpoint_starvation_1, the auto-checkpoint uses 
** CAPDB_CHECKPOINT_PASSIVE. In checkpoint_starvation_2, it uses RESTART.
...
static int checkpoint_starvation_walhook(
  ...
  if( nFrame>=CHECKPOINT_STARVATION_FRAMELIMIT ){
    capdb_wal_checkpoint_v2(db, zDb, p->eMode, 0, 0);
  }
  return CAPDB_OK;
}
...
static char *checkpoint_starvation_reader(int iTid, void *pArg){
  ...
  while( !timetostop(&err) ){
    ...
    capdb_sleep(CHECKPOINT_STARVATION_READMS);
...
static void checkpoint_starvation_main(int nMs, CheckpointStarvationCtx *p){
  ...
  for(i=0; i<4; i++){
    ...
    capdb_sleep(CHECKPOINT_STARVATION_READMS/4);
  }

  capdb_wal_hook(db.db, checkpoint_starvation_walhook, (void *)p);
  while( !timetostop(&err) ){
  ...
  printf(" Checkpoint mode  : %s\n",
      p->eMode==CAPDB_CHECKPOINT_PASSIVE ? "PASSIVE" : "RESTART"
  );
...
static void checkpoint_starvation_1(int nMs){
  Error err = {0};
  CheckpointStarvationCtx ctx = { CAPDB_CHECKPOINT_PASSIVE, 0 };
...
static void checkpoint_starvation_2(int nMs){
  Error err = {0};
  CheckpointStarvationCtx ctx = { CAPDB_CHECKPOINT_RESTART, 0 };
  checkpoint_starvation_main(nMs, &ctx);
  if( ctx.nMaxFrame>CHECKPOINT_STARVATION_FRAMELIMIT+10 ){
