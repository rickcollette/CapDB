...
** following command:
**
**   gcc -O2 threadtest3.c capdb.c -ldl -lpthread -lm
**
...
/* Report/clear errors. */
#define test_error(z, ...)      test_error_x(z, capdb_mprintf(__VA_ARGS__))
...
*/
static void md5step(capdb_context *context, int argc, capdb_value **argv){
  MD5Context *p;
  int i;
  if( argc<1 ) return;
  ...
  for(i=0; i<argc; i++){
    const char *zData = (char*)capdb_value_text(argv[i]);
    if( zData ){
      MD5Update(p, (unsigned char*)zData, strlen(zData));
    }
  }
}
static void md5finalize(capdb_context *context){
  MD5Context *p;
...
struct Statement {
  capdb_stmt *pStmt;            /* Pre-compiled statement handle */
...
static void print_err(Error *p){
  if( p->rc!=CAPDB_OK ){
    int isWarn = 0;
    if( p->rc==CAPDB_SCHEMA ) isWarn = 1;
    if( capdb_strglob("* - no such table: *",p->zErr)==0 ) isWarn = 1;
...
static void system_error(Error *pErr, int iSys){
  pErr->rc = iSys;
#if _WIN32
  pErr->zErr = capdb_mprintf("%s", strerror(iSys));
#else
  pErr->zErr = (char *)capdb_malloc(512);
...
static void sqlite_error(
  ...
  pErr->rc = capdb_errcode(pDb->db);
  pErr->zErr = capdb_mprintf(
      "capdb_%s() - %s (%d)", zFunc, capdb_errmsg(pDb->db),
      capdb_extended_errcode(pDb->db)
  );
}
...
static void opendb_x(
  ...
  if( pErr->rc==CAPDB_OK ){
    int rc;
    int flags = CAPDB_OPEN_CREATE | CAPDB_OPEN_READWRITE | CAPDB_OPEN_URI;
    if( bDelete ) unlink(zFile);
    rc = capdb_open_v2(zFile, &pDb->db, flags, 0);
    if( rc ){
      ...
      capdb_create_function(
          pDb->db, "md5sum", -1, CAPDB_UTF8, 0, 0, md5step, md5finalize
      );
      capdb_busy_handler(pDb->db, busyhandler, 0);
      capdb_exec(pDb->db, "PRAGMA synchronous=OFF", 0, 0, 0);
    }
  }
}
...
static void closedb_x(
  ...
  for(pIter=pDb->pCache; pIter; pIter=pNext){
    pNext = pIter->pNext;
    capdb_finalize(pIter->pStmt);
    capdb_free(pIter);
  }
  for(i=0; i<pDb->nText; i++){
    capdb_free(pDb->aText[i]);
  }
  capdb_free(pDb->aText);
  rc = capdb_close(pDb->db);
  if( rc && pErr->rc==CAPDB_OK ){
    pErr->zErr = capdb_mprintf("%s", capdb_errmsg(pDb->db));
  }
...
static void sql_script_x(
  ...
  if( pErr->rc==CAPDB_OK ){
    pErr->rc = capdb_exec(pDb->db, zSql, 0, 0, &pErr->zErr);
  }
}
...
static void sql_script_printf_x(
  ...
  va_start(ap, zFormat);
  if( pErr->rc==CAPDB_OK ){
    char *zSql = capdb_vmprintf(zFormat, ap);
    pErr->rc = capdb_exec(pDb->db, zSql, 0, 0, &pErr->zErr);
    capdb_free(zSql);
  }
  va_end(ap);
...
static Statement *getSqlStatement(
  ...
  for(pRet=pDb->pCache; pRet; pRet=pRet->pNext){
    if( 0==strcmp(capdb_sql(pRet->pStmt), zSql) ){
      return pRet;
    }
  }

  pRet = capdb_malloc(sizeof(Statement));
  rc = capdb_prepare_v2(pDb->db, zSql, -1, &pRet->pStmt, 0);
  if( rc!=CAPDB_OK ){
    sqlite_error(pErr, pDb, "prepare_v2");
    return 0;
  }
  assert( 0==strcmp(capdb_sql(pRet->pStmt), zSql) );
...
static capdb_stmt *getAndBindSqlStatement(
  Error *pErr,                    /* IN/OUT: Error code */
  Sqlite *pDb,                    /* Database handle */
  va_list ap                      /* SQL followed by parameters */
){
  Statement *pStatement;          /* The SQLite statement wrapper */
  capdb_stmt *pStmt;            /* The SQLite statement to return */
  ...
  for(i=1; i<=capdb_bind_parameter_count(pStmt); i++){
    const char *zName = capdb_bind_parameter_name(pStmt, i);
    void * pArg = va_arg(ap, void*);

    switch( zName[1] ){
      case 'i':
        capdb_bind_int64(pStmt, i, *(i64 *)pArg);
        break;

      default:
        pErr->rc = 1;
        pErr->zErr = capdb_mprintf("Cannot discern type: \"%s\"", zName);
        pStmt = 0;
        break;
    }
  }
...
static i64 execsql_i64_x(
  ...
  i64 iRet = 0;
  if( pErr->rc==CAPDB_OK ){
    capdb_stmt *pStmt;          /* SQL statement to execute */
    ...
    if( pStmt ){
      ...
      while( CAPDB_ROW==capdb_step(pStmt) ){
        if( first && capdb_column_count(pStmt)>0 ){
          iRet = capdb_column_int64(pStmt, 0);
        }
        first = 0;
      }
      if( CAPDB_OK!=capdb_reset(pStmt) ){
...
static char * execsql_text_x(
  ...
  if( iSlot>=pDb->nText ){
    ...
    pDb->aText = (char **)capdb_realloc(pDb->aText, nByte);
  ...
  if( pErr->rc==CAPDB_OK ){
    ...
    if( pStmt ){
      ...
      while( CAPDB_ROW==capdb_step(pStmt) ){
        if( first && capdb_column_count(pStmt)>0 ){
          zRet = capdb_mprintf("%s", capdb_column_text(pStmt, 0));
          capdb_free(pDb->aText[iSlot]);
...
static void integrity_check_x(
  ...
  if( pErr->rc==CAPDB_OK ){
    ...
    if( pStatement ){
      capdb_stmt *pStmt = pStatement->pStmt;
      while( CAPDB_ROW==capdb_step(pStmt) ){
        const char *z = (const char*)capdb_column_text(pStmt, 0);
        if( strcmp(z, "ok") ){
          if( zErr==0 ){
            zErr = capdb_mprintf("%s", z);
          }else{
            zErr = capdb_mprintf("%z\n%s", zErr, z);
          }
        }
      }
      capdb_reset(pStmt);
...
static void launch_thread_x(
  ...
  if( pErr->rc==CAPDB_OK ){
    ...
    p = (Thread *)capdb_malloc(sizeof(Thread));
...
static void join_all_threads_x(
  ...
  for(p=pThreads->pThread; p; p=pNext){
    ...
    if( rc!=0 ){
      if( pErr->rc==CAPDB_OK ) system_error(pErr, rc);
    }else{
...
static void filecopy_x(
  ...
  if( pErr->rc==CAPDB_OK ){
    ...
    if( nByte<0 ){
      test_error_x(pErr, capdb_mprintf("no such file: %s", zFrom));
    }else{
      i64 iOff;
      char aBuf[1024];
      int fd1;
...
static double currentTime(void){
  double t;
  static capdb_vfs *pTimelimitVfs = 0;
  if( pTimelimitVfs==0 ) pTimelimitVfs = capdb_vfs_find(0);
...
static char *walthread1_thread(int iTid, void *pArg){
  ...
  return capdb_mprintf("%d iterations", nIter);
}
...
static char *walthread1_ckpt_thread(int iTid, void *pArg){
  ...
  while( !timetostop(&err) ){
    ...
    if( err.rc==CAPDB_OK ) nCkpt++;
    clear_error(&err, CAPDB_BUSY);
  }
  closedb(&err, &db);

  print_and_free_err(&err);
  return capdb_mprintf("%d checkpoints", nCkpt);
}
...
static char *walthread2_thread(int iTid, void *pArg){
  ...
  print_and_free_err(&err);
  return capdb_mprintf("W %d R %d", anTrans[0], anTrans[1]);
}
...
static char *dynamic_triggers_1(int iTid, void *pArg){
  ...
  while( !timetostop(&err) ){
    ...
    for(i=1; i<9; i++){
      char *zSql = capdb_mprintf("DROP TRIGGER itr%d", i);
      execsql(&err, &db, zSql);
      capdb_free(zSql);
      nDrop++;
    }
    ...
    for(i=1; i<9; i++){
      char *zSql = capdb_mprintf("DROP TRIGGER dtr%d", i);
      execsql(&err, &db, zSql);
      capdb_free(zSql);
      nDrop++;
    }
  ...
  print_and_free_err(&err);
  return capdb_mprintf("%d created, %d dropped", nCreate, nDrop);
}
...
static char *dynamic_triggers_2(int iTid, void *pArg){
  ...
  print_and_free_err(&err);
  return capdb_mprintf("%d inserts, %d deletes", nInsert, nDelete);
}
...
static void dynamic_triggers(int nMs){
  ...
  capdb_enable_shared_cache(1);
  launch_thread(&err, &threads, dynamic_triggers_2, 0);
  launch_thread(&err, &threads, dynamic_triggers_2, 0);

  capdb_sleep(2*1000);
  capdb_enable_shared_cache(0);
...
int main(int argc, char **argv){
  ...
  ** command-line and bail out with the usage message.  */
  for(iArg=1; iArg<argc; iArg++){
    ...
    if( zArg[0]=='-' ){
      if( capdb_stricmp(zArg, "-multiplexor")==0 ){
        /* Install the multiplexor VFS as the default */
        int rc = capdb_multiplex_initialize(0, 1);
        if( rc!=CAPDB_OK ){
    ...
    for(i=0; i<sizeof(aTest)/sizeof(aTest[0]); i++){
      if( capdb_strglob(zArg, aTest[i].zTest)==0 ) break;
    }
  ...
  for(iArg=1; iArg<argc; iArg++){
    ...
    for(i=0; i<sizeof(aTest)/sizeof(aTest[0]); i++){
      ...
      if( capdb_strglob(argv[iArg],z)==0 ){
