...
*/
static int inmemClose(capdb_file *pFile){
...
static int inmemRead(
  capdb_file *pFile,   /* Read from this open file */
  void *pData,           /* Store content in this buffer */
  int iAmt,              /* Bytes of content */
  capdb_int64 iOfst    /* Start reading here */
){
...
static int inmemWrite(
  capdb_file *pFile,   /* Write to this file */
  const void *pData,     /* Content to write */
  int iAmt,              /* bytes to write */
  capdb_int64 iOfst    /* Start writing here */
){
  VHandle *pHandle = (VHandle*)pFile;
  VFile *pVFile = pHandle->pVFile;
  if( iOfst+iAmt > pVFile->sz ){
...
static int inmemTruncate(capdb_file *pFile, capdb_int64 iSize){
  VHandle *pHandle = (VHandle*)pFile;
  VFile *pVFile = pHandle->pVFile;
  if( pVFile->sz>iSize && iSize>=0 ) pVFile->sz = (int)iSize;
  return CAPDB_OK;
}
static int inmemSync(capdb_file *pFile, int flags){
  return CAPDB_OK;
}
static int inmemFileSize(capdb_file *pFile, capdb_int64 *pSize){
  *pSize = ((VHandle*)pFile)->pVFile->sz;
  return CAPDB_OK;
}
static int inmemLock(capdb_file *pFile, int type){
  return CAPDB_OK;
}
static int inmemUnlock(capdb_file *pFile, int type){
  return CAPDB_OK;
}
static int inmemCheckReservedLock(capdb_file *pFile, int *pOut){
  *pOut = 0;
  return CAPDB_OK;
}
static int inmemFileControl(capdb_file *pFile, int op, void *pArg){
  return CAPDB_NOTFOUND;
}
static int inmemSectorSize(capdb_file *pFile){
  return 512;
}
static int inmemDeviceCharacteristics(capdb_file *pFile){
  return
...
*/
static capdb_io_methods VHandleMethods = {
  /* iVersion    */  1,
  /* xClose      */  inmemClose,
  /* xRead       */  inmemRead,
  /* xWrite      */  inmemWrite,
...
static int inmemFullPathname(
  ...
  capdb_snprintf(nOut, zOut, "%s", zFilename);
  return CAPDB_OK;
}
...
static void inmemVfsRegister(void){
  static capdb_vfs inmemVfs;
  capdb_vfs *pDefault = capdb_vfs_find(0);
  inmemVfs.iVersion = 3;
...
/*
** Run multiple commands of SQL.  Similar to capdb_exec(), but does not
** stop if an error is encountered.
*/
static void runSql(capdb *db, const char *zSql, unsigned  runFlags){
  const char *zMore;
  ...
  while( zSql && zSql[0] ){
    zMore = 0;
    pStmt = 0;
    capdb_prepare_v2(db, zSql, -1, &pStmt, &zMore);
    assert( zMore<=zEnd );
    if( zMore==zSql ) break;
    ...
    if( pStmt ){
      if( (runFlags & SQL_OUTPUT)==0 ){
        ...
        int nCol = -1;
        int nRow;
        for(nRow=0; CAPDB_ROW==capdb_step(pStmt); nRow++){
          int i;
          if( nCol<0 ){
          ...
          for(i=0; i<nCol; i++){
            ...
            printf("ROW[%d].%s = ", nRow, capdb_column_name(pStmt,i));
            switch( eType ){
              case CAPDB_NULL: {
                printf("NULL\n");
                break;
...
int main(int argc, char **argv){
  ...
  capdb_stmt *pStmt;   /* A single SQL statement */
  Str sql;               /* SQL to run */
  ...
  if( verboseFlag ){
    capdb_config(CAPDB_CONFIG_LOG, sqlLog);
  }
  if( nHeap>0 ){
    void *pHeap = malloc( nHeap );
    if( pHeap==0 ) fatalError("cannot allocate %d-byte heap\n", nHeap);
    rc = capdb_config(CAPDB_CONFIG_HEAP, pHeap, nHeap, 32);
  ...
  for(i=0; i<nDb; i++){
    ...
    rc = capdb_open_v2("test.db", &db, CAPDB_OPEN_READWRITE, "inmem");
    if( rc ){
...
#ifndef CAPDB_OMIT_PROGRESS_CALLBACK
    if( vdbeLimitFlag ){
      capdb_progress_handler(db, 100000, progressHandler, &vdbeLimitFlag);
    }
#endif
    rc = capdb_prepare_v2(db, "SELECT sql FROM autoexec", -1, &pStmt, 0);
    if( rc==CAPDB_OK ){
      while( CAPDB_ROW==capdb_step(pStmt) ){
        StrAppend(&sql, (const char*)capdb_column_text(pStmt, 0));
        StrAppend(&sql, "\n");
      }
    }
    capdb_finalize(pStmt);
    ...
    reformatVfs();
    StrFree(&sql);
    if( capdb_memory_used()>0 ){
      free(azDb);
      reformatVfs();
      fatalError("memory leak of %lld bytes", capdb_memory_used());
    }
  }
  StrFree(&sql);
  reformatVfs();
