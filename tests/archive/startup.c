...
int main(int argc, char **argv){
  ...
  if( strcmp(zCmd, "run")==0 ){
    ...
    if( rc ){
      ...
      capdb_exec(db, "PRAGMA synchronous", 0, 0, &zErr);
    }
    if( zErr ){
      printf("ERROR: %s\n", zErr);
      capdb_free(zErr);
    ...
    if( showStats ){
      int iCur, iHi;
      capdb_db_status(db, CAPDB_DBSTATUS_LOOKASIDE_USED, &iCur, &iHi, 0);
      printf("-- Lookaside Slots Used:        %d (max %d)\n", iCur,iHi);
      capdb_db_status(db, CAPDB_DBSTATUS_LOOKASIDE_HIT, &iCur, &iHi, 0);
      printf("-- Successful lookasides:       %d\n", iHi);
      capdb_db_status(db, CAPDB_DBSTATUS_LOOKASIDE_MISS_SIZE, &iCur,&iHi,0);
      printf("-- Lookaside size faults:       %d\n", iHi);
      capdb_db_status(db, CAPDB_DBSTATUS_LOOKASIDE_MISS_FULL, &iCur,&iHi,0);
      printf("-- Lookaside OOM faults:        %d\n", iHi);
      capdb_db_status(db, CAPDB_DBSTATUS_CACHE_USED, &iCur, &iHi, 0);
      printf("-- Pager Heap Usage:            %d bytes\n", iCur);
      capdb_db_status(db, CAPDB_DBSTATUS_CACHE_HIT, &iCur, &iHi, 1);
      printf("-- Page cache hits:             %d\n", iCur);
      capdb_db_status(db, CAPDB_DBSTATUS_CACHE_MISS, &iCur, &iHi, 1);
      printf("-- Page cache misses:           %d\n", iCur);
      capdb_db_status(db, CAPDB_DBSTATUS_CACHE_WRITE, &iCur, &iHi, 1);
      printf("-- Page cache writes:           %d\n", iCur); 
      capdb_db_status(db, CAPDB_DBSTATUS_SCHEMA_USED, &iCur, &iHi, 0);
      printf("-- Schema Heap Usage:           %d bytes\n", iCur); 
      capdb_db_status(db, CAPDB_DBSTATUS_STMT_USED, &iCur, &iHi, 0);
  ...
  if( strcmp(zCmd, "init")==0 ){
    ...
    int rc;
    unlink(zDbName);
    zAux = capdb_mprintf("%s-journal", zDbName);
    unlink(zAux);
    capdb_free(zAux);
    zAux = capdb_mprintf("%s-wal", zDbName);
    unlink(zAux);
    capdb_free(zAux);
    rc = capdb_open(zDbName, &db);
    if( rc ){
      ...
      capdb_exec(db, zTestSchema, 0, 0, &zErr);
