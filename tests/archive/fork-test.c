...
**         f1.c $(SQLITESRC)/ext/misc/vfstrace.c $(SQLITESRC/capdb.c \
**         -ldl -lpthread -lm
**
** Test procedure:
**
...
**
**   *   If a process has open SQLite database connections when it fork()s,
**       the child can call capdb_close() on those database connections
...
/*
** This is an capdb_exec() callback routine that prints all results.
*/
...
*/
static void sqlExec(capdb *db, const char *zSql, int bCallback){
  int rc;
  char *zErr = 0;
  printf("%s: %s\n", whoAmI(), zSql);
  fflush(stdout);
  rc = capdb_exec(db, zSql, bCallback ? execCallback : 0, 0, &zErr);
  if( rc || zErr!=0 ){
    printf("%s: %s: rc=%d: %s\n", 
      whoAmI(), zSql, rc, zErr);
    exit(1);
...
int main(int argc, char **argv){
  ...
  parentPid = getpid();
  unlink(zFilename);
  rc = capdb_open(zFilename, &db);
  if( rc ){
    printf("capdb_open() returns %d\n", rc);
    exit(1);
  }
  if( useWal ){
  ...
  if( child==0 ){
    ...
    */
    for(k=0; 1/*exit-by-break*/; k++){
      const char *zDbName = capdb_db_name(db, k);
      capdb_file *pJrnl = 0;
      if( k==1 ) continue;
      if( zDbName==0 ) break;
      capdb_file_control(db, zDbName, CAPDB_FCNTL_NULL_IO, 0);
      capdb_file_control(db, zDbName, CAPDB_FCNTL_JOURNAL_POINTER, &pJrnl);
      if( pJrnl && pJrnl->pMethods && pJrnl->pMethods->xFileControl ){
        pJrnl->pMethods->xFileControl(pJrnl, CAPDB_FCNTL_NULL_IO, 0);
      }
    }    
    capdb_close(db);
    /*
