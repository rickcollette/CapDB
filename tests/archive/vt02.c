...
** capdb_vtab_in_first() and capdb_vtab_in_next() interfaces:
**
**      a=... AND d IN (...)
**      a=... AND b=... AND d IN (...)
...
**
** The capdb_vtab_distinct() interface is used to recognize DISTINCT
...
**
**       0x08           Use capdb_mprintf() to allocate an idxStr string.
...
**
**       0x80           If capdb_vtab_distinct() says that duplicate rows
...
**                      capdb_vtab_distinct() says that is ok to do.
**
...
** rows collectively show the content of the capdb_index_info object and
...
*/
struct vt02_vtab {
  capdb_vtab parent;        /* Base clase.  Must be first. */
...
*/
struct vt02_cur {
  capdb_vtab_cursor parent; /* Base class.  Must be first */
  capdb_int64 i;            /* Current entry */
  capdb_int64 iEof;         /* Indicate EOF when reaching this value */
  capdb_int64 iMin;         /* EOF if dropping below this value */
...
int vt02Connect(
  ...
  capdb_vtab **ppVTab,      /* Write the new vtab here */
...
#define VT02_COL_NONE    7
  ...
  pVtab = capdb_malloc( sizeof(*pVtab) );
  if( pVtab==0 ){
  ...
  pVtab->db = db;
  rc = capdb_declare_vtab(db, zSchema ? zSchema : zDefaultSchema);
  if( rc ){
    capdb_free(pVtab);
  }else{
    *ppVTab = &pVtab->parent;
...
*/
int vt02Disconnect(capdb_vtab *pVTab){
  capdb_free(pVTab);
  return CAPDB_OK;
}
...
*/
static void vt02ErrMsg(capdb_vtab *pVtab, const char *zFormat, ...){
  va_list ap;
  capdb_free(pVtab->zErrMsg);
  va_start(ap, zFormat);
  pVtab->zErrMsg = capdb_vmprintf(zFormat, ap);
  va_end(ap);
}
...
*/
static int vt02Open(capdb_vtab *pVTab, capdb_vtab_cursor **ppCursor){
  vt02_cur *pCur;
...
/* Close a cursor
*/
static int vt02Close(capdb_vtab_cursor *pCursor){
...
** no more entries.
*/
static int vt02Eof(capdb_vtab_cursor *pCursor){
  vt02_cur *pCur = (vt02_cur*)pCursor;
...
*/
static int vt02Next(capdb_vtab_cursor *pCursor){
  vt02_cur *pCur = (vt02_cur*)pCursor;
  do{
    pCur->i += pCur->iIncr;
...
static int vt02Filter(
  capdb_vtab_cursor *pCursor, /* The cursor to rewind */
  int idxNum,                   /* Search strategy */
  const char *idxStr,           /* Not used */
  int argc,                     /* Not used */
  capdb_value **argv          /* Not used */
){
  ...
  if( idxNum==0 ){
    ...
    pCur->i = capdb_value_int64(argv[0]);
    ...
    for(m=1000, i=0; i<=e; i++, m /= 10){
      capdb_int64 v = capdb_value_int64(argv[iArg++]);
      if( v<0 ) v = 0;
      if( v>9 ) v = 9;
      pCur->i += m*v;
      pCur->iEof = pCur->i+m;
    ...
    for(m=1000, i=0; i<=e; i++, m /= 10){
      capdb_int64 v;
      pVal = 0;
      if( capdb_vtab_in_first(0, &pVal)!=CAPDB_MISUSE
       || capdb_vtab_in_first(argv[iArg], &pVal)!=CAPDB_ERROR
      ){
        vt02ErrMsg(pCursor->pVtab, 
                "unexpected success from capdb_vtab_in_first()");
        return CAPDB_ERROR;
      }
      v = capdb_value_int64(argv[iArg++]);
      if( v<0 ) v = 0;
      if( v>9 ) v = 9;
      pCur->i += m*v;
      pCur->iEof = pCur->i+m;
    ...
    pCur->mD = 0;
    pIn = argv[iArg++];
    assert( capdb_value_type(pIn)==CAPDB_NULL );
    for( rc = capdb_vtab_in_first(pIn, &pVal);
         rc==CAPDB_OK && pVal!=0;
         rc = capdb_vtab_in_next(pIn, &pVal)
    ){
      ...
      i = capdb_value_int(pVal);
      if( i<0 || i>9 ) continue;
      pCur->mD |= 1<<i;
    }
    if( rc!=CAPDB_OK && rc!=CAPDB_DONE ){
      vt02ErrMsg(pCursor->pVtab, "Error from capdb_vtab_in_first/next()");
      return rc;
    }
  }else{
  ...
  if( bUseOffset ){
    int nSkip = capdb_value_int(argv[iArg]);
...
*/
static int vt02Rowid(capdb_vtab_cursor *pCursor, capdb_int64 *pRowid){
  vt02_cur *pCur = (vt02_cur*)pCursor;
  *pRowid = pCur->i+1;
  return CAPDB_OK;
}
...
** Logging Subsystem
**
** The capdbBestIndexLog() routine implements a logging system for
** xBestIndex calls.  This code is portable to any virtual table.
**
** capdbBestIndexLog() is the main routine,  capdbRunSql() is a
...
** extract this code and call capdbBestIndexLog() near the end of the
** xBestIndex method in cases where logging is desired.
*/
/*
** Run SQL on behalf of capdbBestIndexLog.
**
...
*/
static void capdbRunSql(
  capdb *db,               /* Run the SQL on this database connection */
  capdb_vtab *pVTab,       /* Report errors to this virtual table */
  ...
  if( zSql ){
    char *zErrMsg = 0;
    (void)capdb_exec(db, zSql, 0, 0, &zErrMsg);
    if( zErrMsg ){
      if( pVTab->zErrMsg==0 ){
        pVTab->zErrMsg = capdb_mprintf("%s in [%s]", zErrMsg, zSql);
      }
      capdb_free(zErrMsg);
    }
    capdb_free(zSql);
...
*/
static void capdbBestIndexLog(
  capdb_index_info *pInfo,  /* The capdb_index_info object */
  const char *zLogTab,        /* Log into this table */
  capdb *db,                /* Database connection containing zLogTab */
  const char **azColname,     /* Names of columns in the virtual table */
  capdb_vtab *pVTab         /* Record errors into this object */
){
  int i, rc;
  capdb_str *pStr;
  int iBI;

  if( capdb_table_column_metadata(db,0,zLogTab,0,0,0,0,0,0) ){
    /* The log table does not previously exist.  Create it. */
    capdbRunSql(db,pVTab, 
    ...
    ** to deal with this. */
    capdb_stmt *pStmt;
    char *zSql;
    zSql = capdb_mprintf("SELECT max(bi) FROM temp.\"%w\"",zLogTab);
    if( zSql==0 ){
      capdb_free(pVTab->zErrMsg);
      pVTab->zErrMsg = capdb_mprintf("out of memory");
      return;
    }
    ...
    if( rc ){
      capdb_free(pVTab->zErrMsg);
      pVTab->zErrMsg = capdb_mprintf("%s", capdb_errmsg(db));
      iBI = 0;
    }else if( capdb_step(pStmt)==CAPDB_ROW ){
      iBI = capdb_column_int(pStmt, 0)+1;
    }else{
      iBI = 1;
    }
  ...
  for(i=0; i<pInfo->nConstraint; i++){
    ...
    pVal = 0;
    rc = capdb_vtab_rhs_value(pInfo, i, &pVal);
    assert( pVal!=0 || rc!=CAPDB_OK );
    if( rc==CAPDB_OK ){
      capdb_str_appendf(pStr,",1,?1");
    }else{
      capdb_str_appendf(pStr,",0,NULL");
    }
    capdb_str_appendf(pStr,",%Q,%d)",
         capdb_vtab_collation(pInfo,i),
         capdb_vtab_in(pInfo,i,-1));
    zSql = capdb_str_finish(pStr);
    if( zSql==0 ){
      if( pVTab->zErrMsg==0 ) pVTab->zErrMsg = capdb_mprintf("out of memory");
    }else{
      capdb_stmt *pStmt = 0;
      rc = capdb_prepare_v2(db, zSql, -1, &pStmt, 0);
      if( rc ){
        ...
        if( pVal ) capdb_bind_value(pStmt, 1, pVal);
        capdb_step(pStmt);
  ...
  capdbRunSql(db,pVTab,
    "INSERT INTO temp.\"%w\"(bi,vn,ix) VALUES(%d,'capdb_vtab_distinct',%d)",
    zLogTab, iBI, capdb_vtab_distinct(pInfo)
  );
  capdbRunSql(db,pVTab,
...
*/
static int vt02BestIndex(capdb_vtab *pVTab, capdb_index_info *pInfo){
  int i;                      /* Loop counter */
  ...
  ** usable RHS values */
  for(i=0; i<pInfo->nConstraint; i++){
    ...
    if( pInfo->aConstraint[i].op!=CAPDB_INDEX_CONSTRAINT_EQ ) continue;
    switch( pInfo->aConstraint[i].iColumn ){
      case VT02_COL_FLAGS:
        if( capdb_vtab_rhs_value(pInfo, i, &pVal)==CAPDB_OK
         && capdb_value_type(pVal)==CAPDB_INTEGER
        ){
          flags = capdb_value_int(pVal);
        }
        iFlagTerm = i;
        break;
      case VT02_COL_LOGTAB:
        if( capdb_vtab_rhs_value(pInfo, i, &pVal)==CAPDB_OK
         && capdb_value_type(pVal)==CAPDB_TEXT
        ){
          zLogTab = (const char*)capdb_value_text(pVal);
        }
        iLogTerm = i;
        break;
    }
  ...
  for(i=0; i<pInfo->nConstraint; i++){
    ...
    switch( pInfo->aConstraint[i].op ){
      case CAPDB_INDEX_CONSTRAINT_FUNCTION:
  ...
  pInfo->estimatedRows = (capdb_int64)pInfo->estimatedCost;
  ...
  */
  if( pInfo->nOrderBy>0 && (flags & VT02_NO_SORT_OPT)==0 ){
    int eDistinct = capdb_vtab_distinct(pInfo);
    if( pInfo->idxNum==1 ){
  ...
  if( flags & VT02_ALLOC_IDXSTR ){
    pInfo->idxStr = capdb_mprintf("test");
  ...
  */
  ...
  if( zLogTab ){
    ...
    capdb *db = ((vt02_vtab*)pVTab)->db;
    capdbBestIndexLog(pInfo, zLogTab, db, azColname, pVTab);
  }
  pSelf->busy--;
  ...
  ** an error under OOM testing loops */
  pX = capdb_malloc(800);
  if( pX==0 ) return CAPDB_NOMEM;
  capdb_free(pX);

  return pVTab->zErrMsg!=0 ? CAPDB_ERROR : CAPDB_OK;
}

/* This is the capdb_module definition for the the virtual table defined
** by this include file.
*/
const capdb_module vt02Module = {
  /* iVersion      */  2,
...
static void vt02CoreInit(capdb *db){
  ...
  static const char zPkABCDSchema[] = 
    ...
    "PRIMARY KEY(a,b,c,d));";
  capdb_create_module(db, "vt02", &vt02Module, 0);
  capdb_create_module(db, "vt02pkx", &vt02Module, (void*)zPkXSchema);
  capdb_create_module(db, "vt02pkabcd", &vt02Module, (void*)zPkABCDSchema);
}
