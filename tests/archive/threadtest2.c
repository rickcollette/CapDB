...
int integrity_check(sqlite *db, int id){
  ...
  rc = capdb_exec(db, "pragma integrity_check", check_callback, 0, 0);
  if( rc!=CAPDB_OK && rc!=CAPDB_BUSY ){
    fprintf(stderr,"%d, Integrity check returns %d\n", id, rc);
  }
  if( all_stop ){
    capdb_exec(db, "pragma integrity_check", check_callback, 0, 0);
  }
  return 0;
}
...
void *worker(void *workerArg){
  ...
  while( !all_stop && cnt++<10000 ){
    ...
    while( (capdb_open(DB_FILE, &db))!=CAPDB_OK ) sched_yield();
    capdb_exec(db, "PRAGMA synchronous=OFF", 0, 0, 0);
    /* integrity_check(db, id); */
    if( all_stop ){ capdb_close(db); break; }
    /* fprintf(stderr, "%d: BEGIN\n", id); */
    rc = capdb_exec(db, "INSERT INTO t1 VALUES('bogus data')", 0, 0, 0);
...
int main(int argc, char **argv){
  ...
  if( strcmp(DB_FILE,":memory:") ){
    char *zJournal = capdb_mprintf("%s-journal", DB_FILE);
    unlink(DB_FILE);
    unlink(zJournal);
    capdb_free(zJournal);
  }  
  capdb_open(DB_FILE, &db);
  if( db==0 ){
    fprintf(stderr,"unable to initialize database\n");
    exit(1);
  }
  rc = capdb_exec(db, "CREATE TABLE t1(x);", 0,0,0);
  if( rc ){
