...
**
** This module compares library sprintf() against SQLite's capdb_snprintf().
...
**    make capdb.c
**    gcc -Os -c capdb.c
**    gcc -I. -Os test/fp-speed-1.c capdb.o -ldl -lm -lpthread
**
** To run the test:
**
**    ./a.out 10000000
...
static capdb_int64 timeOfDay(void){
  ...
  t =  ((capdb_uint64)tm.dwHighDateTime<<32) |
          (capdb_uint64)tm.dwLowDateTime;
  ...
  return ((capdb_int64)sNow.tv_sec)*1000000 + sNow.tv_usec;
#endif
}
...
int main(int argc, char **argv){
  ...
  printf("capdb_snprintf(\"%s\"):  ", zFmt);
  tm2 = timeOfDay();
  for(i=0; i<cnt; i++){
    capdb_snprintf(sizeof(zBuf), zBuf, zFmt, aVal[i%NN]);
  }
  tm2 = timeOfDay() - tm2;
  printf("%6.1f ns/call, %9.6f sec total\n", tm2*1.0e+3/(double)cnt,tm2*1.0e-6);

  if( tm1 < tm2 ){
    printf("sprintf() is about %g times faster than capdb_snprintf()\n",
           (double)tm2/(double)tm1);
  }else{
    printf("capdb_snprintf() is about %g times faster than sprintf()\n",
