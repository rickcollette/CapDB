...
static char *stress_thread_1(int iTid, void *pArg){
  ...
  while( !timetostop(&err) ){
    ...
    clear_error(&err, CAPDB_LOCKED);
    sql_script(&err, &db, "DROP TABLE IF EXISTS t1");
    clear_error(&err, CAPDB_LOCKED);
  }
  closedb(&err, &db);
  print_and_free_err(&err);
  return capdb_mprintf("ok");
}
...
static char *stress_thread_3(int iTid, void *pArg){
  ...
  while( !timetostop(&err) ){
    ...
    if( err.rc ) i2++;
    clear_error(&err, CAPDB_LOCKED);
    clear_error(&err, CAPDB_ERROR);
  }
  closedb(&err, &db);
  print_and_free_err(&err);
  return capdb_mprintf("read t1 %d/%d attempts", i2, i1);
}
...
static char *stress_thread_4(int iTid, void *pArg){
  ...
  closedb(&err, &db);
  print_and_free_err(&err);
  return capdb_mprintf("wrote t1 %d/%d attempts", i2, i1);
}
...
static char *stress_thread_5(int iTid, void *pArg){
  ...
  closedb(&err, &db);
  print_and_free_err(&err);
  return capdb_mprintf("deleted from t1 %d/%d attempts", i2, i1);
}
...
static char *stress2_thread_wrapper(int iTid, void *pArg){
  ...
  while( !timetostop(&err) ){
    ...
    for(cnt=0; err.rc==CAPDB_OK && cnt<STRESS2_TABCNT; cnt++){
  ...
  print_and_free_err(&err);
  return capdb_mprintf("ok %d/%d", i2, i1);
}
...
static void stress2_launch_thread_loop(
  ...
  Stress2Ctx *pCtx = capdb_malloc(sizeof(Stress2Ctx));
  pCtx->zDb = zDb;
  pCtx->xProc = x;
