...
**
** This module compares library atof() against SQLite's capdbAtoF().
...
**    make capdb.c
**    gcc -Os -c capdb.c
**    gcc -I. -Os test/fp-speed-2.c capdb.o -ldl -lm -lpthread
**
** To run the test:
**
**    ./a.out 10000000
...
int main(int argc, char **argv){
  ...
  for(i=0; i<cnt; i++){
    double r = 0.0;
    int ex = fpLiteral(i,z);
    capdb_test_control(CAPDB_TESTCTRL_ATOF, z, &r);
    arSum[ex] += r;
  }
  ...
  if( tm[0] < tm[1] ){
    printf("atof() is about %g times faster than capdbAtoF()\n",
           (double)tm[1]/(double)tm[0]);
  }else{
    printf("capdbAtoF() is about %g times faster than atof()\n",
