...
> **CapDB fork:** Tcl-based testing is **not used**. Run `capsuite`, `ctest`, and
> `capdbtest` instead — see [BUILD.md](../BUILD.md) and [docs/CAPDB.md](../docs/CAPDB.md).
> This document describes upstream SQLite's Tcl testrunner.
...
`capdb`, `fuzzcheck`, and C programs found in test/c.
...
Sometimes testrunner.tcl uses the `testfixture` and `capdb` binaries that
...
test scripts using whatever `testfixture` binary (and maybe also the `capdb`
...
like "`make testfixture capdb`" before launching binary tests.  In other
...
the testfixture and/or capdb binaries be built ahead of time.  Those
