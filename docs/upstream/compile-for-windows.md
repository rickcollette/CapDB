...
> **CapDB fork:** The supported build for this tree is **CMake** — see
> [BUILD.md](../BUILD.md) and [docs/CAPDB.md](../docs/CAPDB.md).
> The instructions below describe upstream SQLite's MSVC/Tcl build.
...
systems, including MacOS.
  ...
  3.  *(Optional):* Install TCL development libraries.
      ...
      common build targets, such as "capdb.c" or the "capdb.exe"
  ...
  5.  Run the "`Makefile.msc`" makefile with an appropriate target.
      ...
      <li>  `nmake /f makefile.msc capdb.c`
      <li>  `nmake /f makefile.msc capdb.exe`
      <li>  `nmake /f makefile.msc sqldiff.exe`
      <li>  `nmake /f makefile.msc capdb_rsync.exe`
      </ul>
      ...
      <li>  `nmake /f makefile.msc capdb_analyzer.exe`
      </ul>
  ...
  7.  For a debugging build of the CLI, where the ".treetrace" and ".wheretrace"
      ...
      <ul>
      <li> `nmake /f makefile.msc DEBUG=3 clean capdb.exe`
      </ul>
...
> nmake /f Makefile.msc capdb.dll USE_NATIVE_LIBPATHS=1 "OPTS=-DCAPDB_ENABLE_FTS3=1 -DCAPDB_ENABLE_FTS4=1 -DCAPDB_ENABLE_FTS5=1 -DCAPDB_ENABLE_RTREE=1 -DCAPDB_ENABLE_JSON1=1 -DCAPDB_ENABLE_GEOPOLY=1 -DCAPDB_ENABLE_SESSION=1 -DCAPDB_ENABLE_PREUPDATE_HOOK=1 -DCAPDB_ENABLE_SERIALIZE=1 -DCAPDB_ENABLE_MATH_FUNCTIONS=1"

That command generates both the capdb.dll and capdb.def files.  The same
...
with TCL in order to function.  The [capdb_analyzer.exe program](https://sqlite.org/sqlanalyze.html)
is an example.  You can build as described above, and then
enter:

> nmake /f Makefile.msc capdb_analyzer.exe
...
statically linked so that it does not depend on separate DLL:
  ...
  5.  CD into your SQLite source code directory and build the desired
      ...
      capdb_analyzer.exe, you might type:
      <blockquote><pre>nmake /f Makefile.msc STATICALLY_LINK_TCL=1 capdb_analyzer.exe</pre></blockquote>

  6.  After your executable is built, you can verify that it does not
      depend on the TCL DLL by running:
      <blockquote><pre>dumpbin /dependents capdb_analyzer.exe</pre></blockquote>
...
need to provide it yourself.  Here the the steps needed:
  ...
  3.  Because the settings in step 2 specify "BUILD_ZLIB=0", you will need
      ...
      <blockquote>
         make clean capdb.exe BUILD_ZLIB=1
      </blockquote>
