...
Memory allocation is slightly different depending on:
  ...
  * whether or not CAPDB_CONFIG_SMALL_MALLOC is set, and
  * whether or not worker threads are enabled.

## CAPDB_CONFIG_SMALL_MALLOC=0

Assuming CAPDB_CONFIG_SMALL_MALLOC is not set, keys passed to the sorter are
added to an in-memory buffer. This buffer is grown using capdbRealloc() as
...
either by mapping the file (via capdb_file.xFetch()) or else read back
...
## CAPDB_CONFIG_SMALL_MALLOC=1

This case is similar to the above, except that instead of accumulating
multiple keys in a single large buffer, capdbVdbeSorterWrite() stores
...
capdbHeapNearlyFull() is called. If it returns true, the current
