...
> **CapDB fork:** Tcl bindings and testfixture are **not built** in this tree.
> Use CMake and `capsuite` for regression testing. This document is retained for
...
an ordinary tclsh can subsequently run "package require capdb".
...
<li> `make tclextension-install` <br>
     ...
     `$TCLHOME/lib/tcl${TCLVERSION}/sqlite<CAPDB_VERSION>`.
...
<li> `nmake /f Makefile.msc tclextension-install` <br>
     &uarr; Verify extension installed at %TCLBUILD%\\tcl86\\lib\\tcl8.6\\capdb.*
...
<li> `nmake /f Makefile.msc tclextension-install` <br>
     &uarr; Verify extension installed at %TCLBUILD%\\tcl90\\lib\\capdb.*
<li> `nmake /f Makefile.msc tclextension-verify`
