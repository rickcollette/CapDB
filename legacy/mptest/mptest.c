...
**
**    gcc -g -c -Wall capdb.c $(OPTS)
**    gcc -g -o mptest mptest.c capdb.o $(LIBS)
**
** Recommended options:
**
**    -DHAVE_USLEEP
...
static struct Global {
  ...
  capdb *db;           /* Open connection to database */
...
static void errorMessage(const char *zFormat, ...){
  ...
  va_end(ap);
  capdb_snprintf(sizeof(zPrefix), zPrefix, "%s:ERROR: ", g.zName);
  if( g.pLog ){
...
static void fatalError(const char *zFormat, ...){
  ...
  zMsg = capdb_vmprintf(zFormat, ap);
  va_end(ap);
  capdb_snprintf(sizeof(zPrefix), zPrefix, "%s:FATAL: ", g.zName);
  if( g.pLog ){
    printWithPrefix(g.pLog, zPrefix, zMsg);
    fflush(g.pLog);
    maybeClose(g.pLog);
  ...
  if( g.db ){
    int nTry = 0;
    g.iTimeout = 0;
    while( trySql("UPDATE client SET wantHalt=1;")==CAPDB_BUSY
           && (nTry++)<100 ){
      capdb_sleep(10);
    }
  }
...
static void logMessage(const char *zFormat, ...){
  ...
  zMsg = capdb_vmprintf(zFormat, ap);
  va_end(ap);
  capdb_snprintf(sizeof(zPrefix), zPrefix, "%s: ", g.zName);
  if( g.pLog ){
    printWithPrefix(g.pLog, zPrefix, zMsg);
    fflush(g.pLog);
  }
...
static void vfsNameFunc(
  ...
  capdb_file_control(db, "main", CAPDB_FCNTL_VFSNAME, &zVfs);
  if( zVfs ){
    capdb_result_text(context, zVfs, -1, capdb_free);
  }
}
...
static void sqlErrorCallback(void *pArg, int iErrCode, const char *zMsg){
  ...
  if( iErrCode==CAPDB_ERROR && g.bIgnoreSqlErrors ) return;
  if( (iErrCode&0xff)==CAPDB_SCHEMA && g.iTrace<3 ) return;
  if( g.iTimeout==0 && (iErrCode&0xff)==CAPDB_BUSY && g.iTrace<3 ) return;
  if( (iErrCode&0xff)==CAPDB_NOTICE ){
...
*/
static capdb_stmt *prepareSql(const char *zFormat, ...){
  va_list ap;
  char *zSql;
  int rc;
  capdb_stmt *pStmt = 0;
...
static void stringAppend(String *p, const char *z, int n){
  ...
  if( p->n+n>=p->nAlloc ){
    ...
    char *zNew = capdb_realloc(p->z, nAlloc);
...
static int evalSql(String *p, const char *zFormat, ...){
  ...
  va_end(ap);
  assert( g.iTimeout>0 );
  rc = capdb_exec(g.db, zSql, evalCallback, p, &zErrMsg);
  capdb_free(zSql);
  if( rc ){
    char zErr[30];
    capdb_snprintf(sizeof(zErr), zErr, "error(%d)", rc);
...
static void evalFunc(
  ...
  rc = capdb_exec(db, zSql, evalCallback, &res, &zErrMsg);
  if( zErrMsg ){
    ...
    capdb_result_text(context, res.z, -1, CAPDB_TRANSIENT);
  }
  stringFree(&res);
}
...
static int startScript(
  ...
  *pzScript = 0;
  ...
  while(1){
    ...
    if( rc!=CAPDB_OK ){
      fatalError("in startScript: %s", capdb_errmsg(g.db));
    }
    if( g.nError || g.nTest ){
    ...
    rc = capdb_step(pStmt);
    if( rc==CAPDB_ROW ){
      int n = capdb_column_bytes(pStmt, 0);
      *pzScript = capdb_malloc(n+1);
      strcpy(*pzScript, (const char*)capdb_column_text(pStmt, 0));
      *pTaskId = taskId = capdb_column_int(pStmt, 1);
      *pzTaskName = capdb_mprintf("%s", capdb_column_text(pStmt, 2));
      capdb_finalize(pStmt);
      runSql("UPDATE task"
    ...
    if( rc==CAPDB_DONE ){
      ...
      while( trySql("COMMIT")==CAPDB_BUSY ){
        capdb_sleep(10);
        totalTime += 10;
      }
      capdb_sleep(100);
    ...
    fatalError("%s", capdb_errmsg(g.db));
  }
  g.iTimeout = DEFAULT_TIMEOUT;
}
...
static void startClient(int iClient){
  ...
  if( capdb_changes(g.db) ){
    char *zSys;
    int rc;
    zSys = capdb_mprintf("%s \"%s\" --client %d --trace %d",
                 g.argv0, g.zDbFile, iClient, g.iTrace);
    if( g.bSqlTrace ){
      zSys = capdb_mprintf("%z --sqltrace", zSys);
    }
    if( g.bSync ){
      zSys = capdb_mprintf("%z --sync", zSys);
    }
    if( g.zVfs ){
      zSys = capdb_mprintf("%z --vfs \"%s\"", zSys, g.zVfs);
    }
    if( g.iTrace>=2 ) logMessage("system('%q')", zSys);
#if !defined(_WIN32)
    zSys = capdb_mprintf("%z &", zSys);
    rc = system(zSys);
...
static void waitForClient(int iClient, int iTimeout, char *zErrPrefix){
  ...
  g.iTimeout = 0;
  while( ((rc = capdb_step(pStmt))==CAPDB_BUSY || rc==CAPDB_ROW)
    && iTimeout>0
  ){
    capdb_reset(pStmt);
    capdb_sleep(50);
...
static int booleanValue(char *zArg){
  ...
  if( capdb_stricmp(zArg, "on")==0 || capdb_stricmp(zArg,"yes")==0 ){
    return 1;
  }
  if( capdb_stricmp(zArg, "off")==0 || capdb_stricmp(zArg,"no")==0 ){
    return 0;
  }
...
static void runScript(
  ...
  while( (c = zScript[ii])!=0 ){
    ...
    if( ii>iBegin ){
      char *zSql = capdb_mprintf("%.*s", ii-iBegin, zScript+iBegin);
    ...
    */
    if( strcmp(zCmd, "glob")==0 || strcmp(zCmd, "notglob")==0 ){
      ...
      zAns += jj;
      zCopy = capdb_mprintf("%.*s", len-jj-1, zAns);
      if( (capdb_strglob(zCopy, sResult.z)==0)^isGlob ){
        errorMessage("line %d of %s:\nExpected [%s]\n     Got [%s]",
    ...
    */
    if( strcmp(zCmd, "source")==0 ){
      ...
      if( !isDirSep(zNewFile[0]) ){
        ...
        if( k>0 ){
          zNewFile = zToDel = capdb_mprintf("%.*s/%s", k,zFilename,zNewFile);
          if( zNewFile==0 ){
    ...
    */
    if( strcmp(zCmd, "if")==0 ){
      ...
      rc = capdb_step(pStmt);
      if( rc!=CAPDB_ROW || capdb_column_int(pStmt, 0)==0 ){
    ...
    */
    if( strcmp(zCmd, "wait")==0 && iClient==0 ){
      ...
      capdb_snprintf(sizeof(zError),zError,"line %d of %s\n",
                       prevLine, zFilename);
    ...
    */
    if( strcmp(zCmd, "task")==0 && iClient==0 ){
      ...
      if( iTarget<0 ){
        ...
        zTask = capdb_mprintf("%.*s", iEnd, zScript+ii+len);
        if( nArg>1 ){
          zTName = capdb_mprintf("%s", azArg[1]);
        }else{
          zTName = capdb_mprintf("%s:%d", filenameTail(zFilename), prevLine);
        }
        startClient(iTarget);
...
int CAPDB_CDECL main(int argc, char **argv){
  const char *zClient;
  int iClient;
  int n, i;
  ...
  if( strcmp(capdb_sourceid(), CAPDB_SOURCE_ID)!=0 ){
    fprintf(stderr, "SQLite library and header mismatch\n"
                    "Library: %s\n"
                    "Header:  %s\n",
                    capdb_sourceid(), CAPDB_SOURCE_ID);
    exit(1);
  }
  n = argc-2;
  capdb_snprintf(sizeof(g.zName), g.zName, "%05d.mptest", GETPID());
  ...
  capdb_config(CAPDB_CONFIG_LOG, sqlErrorCallback, 0);
  if( zClient ){
    iClient = atoi(zClient);
    if( iClient<1 ) fatalError("illegal client number: %d\n", iClient);
    capdb_snprintf(sizeof(g.zName), g.zName, "%05d.client%02d",
                     GETPID(), iClient);
  }else{
    int nTry = 0;
    if( g.iTrace>0 ){
      ...
      printf("\n");
      printf("With SQLite " CAPDB_VERSION " " CAPDB_SOURCE_ID "\n" );
      for(i=0; (zCOption = capdb_compileoption_get(i))!=0; i++){
        printf("-DCAPDB_%s\n", zCOption);
      }
      fflush(stdout);
    }
    ...
    }while( rc!=0 && (++nTry)<60 && capdb_sleep(1000)>0 );
    if( rc!=0 ){
  ...
  rc = capdb_open_v2(g.zDbFile, &g.db, openFlags, g.zVfs);
  if( rc ) fatalError("cannot open [%s]", g.zDbFile);
  if( iTmout>0 ) capdb_busy_timeout(g.db, iTmout);
  
  if( zJMode ){
#if defined(_WIN32)
    if( capdb_stricmp(zJMode,"persist")==0
     || capdb_stricmp(zJMode,"truncate")==0
    ){
  ...
  capdb_busy_handler(g.db, busyHandler, 0);
  capdb_create_function(g.db, "vfsname", 0, CAPDB_UTF8, 0,
                          vfsNameFunc, 0, 0);
  capdb_create_function(g.db, "eval", 1, CAPDB_UTF8, 0,
                          evalFunc, 0, 0);
  g.iTimeout = DEFAULT_TIMEOUT;
  if( g.bSqlTrace ) capdb_trace(g.db, sqlTraceCallback, 0);
  if( iClient>0 ){
    ...
    g.iTimeout = 0;
    iTimeout = 1000;
    while( ((rc = trySql("SELECT 1 FROM client"))==CAPDB_BUSY
        || rc==CAPDB_ROW) && iTimeout>0 ){
      capdb_sleep(10);
      iTimeout -= 10;
    }
    capdb_sleep(100);
    ...
    iTimeout = 1000;
    while( (rc = capdb_step(pStmt))==CAPDB_BUSY && iTimeout>0 ){
      capdb_sleep(10);
      iTimeout -= 10;
    }
    if( rc==CAPDB_ROW ){
      g.nError += capdb_column_int(pStmt, 0);
      g.nTest += capdb_column_int(pStmt, 1);
    }
    capdb_finalize(pStmt);
  }
  capdb_close(g.db);
