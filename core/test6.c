...
#ifndef CAPDB_OMIT_DISKIO  /* This file is a no-op if disk I/O is disabled */
...
**   capdb_file object for the host system. Each time data is
...
struct CrashFile {
  ...
  capdb_file *pRealFile;             /* Underlying "real" file handle */
  char *zName;
...
static CrashGlobal g = {0, 0, CAPDB_DEFAULT_SECTOR_SIZE, 0, 0};

/*
** Set this global variable to 1 to enable crash testing.
*/
static int capdbCrashTestEnable = 0;
...
/*
** Wrapper around the capdbOsWrite() function that avoids writing to the
...
static int writeDbFile(CrashFile *p, u8 *z, i64 iAmt, i64 iOff){
  ...
  int iSkip = 0;
  if( (iAmt-iSkip)>0 ){
    rc = capdbOsWrite(p->pRealFile, &z[iSkip], (int)(iAmt-iSkip), iOff+iSkip);
  }
  return rc;
}
...
static int writeListSync(CrashFile *pFile, int isCrash){
  ...
  */
  ...
  }else if( iDc&(CAPDB_IOCAP_SEQUENTIAL|CAPDB_IOCAP_SAFE_APPEND) ){
    int nWrite = 0;
    int iFinal;
    for(pWrite=g.pWriteList; pWrite; pWrite=pWrite->pNext) nWrite++;
    capdb_randomness(sizeof(int), &iFinal);
  ...
  ppPtr = &g.pWriteList;
  for(pWrite=*ppPtr; rc==CAPDB_OK && pWrite; pWrite=*ppPtr){
    capdb_file *pRealFile = pWrite->pFile->pRealFile;
    ...
    */
    ...
    if( !isCrash ){
      eAction = 2;
      if( (pWrite->pFile==pFile || iDc&CAPDB_IOCAP_SEQUENTIAL) ){
        eAction = 1;
      }
    }else{
      char random;
      capdb_randomness(1, &random);

      /* Do not select option 3 (sector trashing) if the IOCAP_ATOMIC flag
      ** is set or this is an OsTruncate(), not an Oswrite().
      */
      if( (iDc&CAPDB_IOCAP_ATOMIC) || (pWrite->zBuf==0) ){
        random &= 0x01;
      }
      ...
      ** out correctly).
      */
      if( (iDc&CAPDB_IOCAP_SEQUENTIAL && pWrite!=pFinal) ){
        random = 0;
      }
      ...
      */
      if( iDc&CAPDB_IOCAP_SAFE_APPEND && pWrite->zBuf ){
        i64 iSize;
        capdbOsFileSize(pRealFile, &iSize);
    ...
    switch( eAction ){
      case 1: {               /* Write out correctly */
        if( pWrite->zBuf ){
          ...
          rc = capdbOsTruncate(pRealFile, pWrite->iOffset);
        }
        *ppPtr = pWrite->pNext;
#ifdef TRACE_CRASHTEST
        if( isCrash ){
      ...
      case 3: {               /* Trash sectors */
        ...
        if( zGarbage ){
          capdb_int64 i;
          for(i=iFirst; rc==CAPDB_OK && i<=iLast; i++){
            capdb_randomness(g.iSectorSize, zGarbage);
            rc = writeDbFile(
...
** Close a crash-file.
*/
static int cfClose(capdb_file *pFile){
  CrashFile *pCrash = (CrashFile *)pFile;
  writeListSync(pCrash, 0);
  capdbOsClose(pCrash->pRealFile);
  return CAPDB_OK;
}
...
*/
static int cfTruncate(capdb_file *pFile, sqlite_int64 size){
  CrashFile *pCrash = (CrashFile *)pFile;
  assert(size>=0);
...
** Sync a crash-file.
*/
static int cfSync(capdb_file *pFile, int flags){
  CrashFile *pCrash = (CrashFile *)pFile;
  int isCrash = 0;
...
*/
static int cfFileSize(capdb_file *pFile, sqlite_int64 *pSize){
  CrashFile *pCrash = (CrashFile *)pFile;
...
*/
static int cfLock(capdb_file *pFile, int eLock){
  return capdbOsLock(((CrashFile *)pFile)->pRealFile, eLock);
}
static int cfUnlock(capdb_file *pFile, int eLock){
  return capdbOsUnlock(((CrashFile *)pFile)->pRealFile, eLock);
}
static int cfCheckReservedLock(capdb_file *pFile, int *pResOut){
  return capdbOsCheckReservedLock(((CrashFile *)pFile)->pRealFile, pResOut);
}
static int cfFileControl(capdb_file *pFile, int op, void *pArg){
  if( op==CAPDB_FCNTL_SIZE_HINT ){
    CrashFile *pCrash = (CrashFile *)pFile;
    i64 nByte = *(i64 *)pArg;
    if( nByte>pCrash->iSize ){
      if( CAPDB_OK==writeListAppend(pFile, nByte, 0, 0) ){
        pCrash->iSize = (int)nByte;
      }
    }
    return CAPDB_OK;
  }
  return capdbOsFileControl(((CrashFile *)pFile)->pRealFile, op, pArg);
}
...
*  interface.
*/
static int cfSectorSize(capdb_file *pFile){
  return g.iSectorSize;
}
static int cfDeviceCharacteristics(capdb_file *pFile){
...
*/
static int cfShmLock(capdb_file *pFile, int ofst, int n, int flags){
  capdb_file *pReal = ((CrashFile*)pFile)->pRealFile;
  return pReal->pMethods->xShmLock(pReal, ofst, n, flags);
}
static void cfShmBarrier(capdb_file *pFile){
  capdb_file *pReal = ((CrashFile*)pFile)->pRealFile;
  pReal->pMethods->xShmBarrier(pReal);
}
static int cfShmUnmap(capdb_file *pFile, int delFlag){
  capdb_file *pReal = ((CrashFile*)pFile)->pRealFile;
...
static const capdb_io_methods CrashFileVtab = {
  2,                            /* iVersion */
  cfClose,                      /* xClose */
  cfRead,                       /* xRead */
  cfWrite,                      /* xWrite */
...
*/
struct crashAppData {
  capdb_vfs *pOrig;                   /* Wrapped vfs structure */
};
...
** capdb_malloc(). The assumption here is (pVfs->szOsFile) is
...
static int cfOpen(
  ...
  capdb_vfs *pVfs = (capdb_vfs *)pCfVfs->pAppData;
  int rc;
  CrashFile *pWrapper = (CrashFile *)pFile;
  capdb_file *pReal = (capdb_file*)&pWrapper[1];

  memset(pWrapper, 0, sizeof(CrashFile));
  rc = capdbOsOpen(pVfs, zName, pReal, flags, pOutFlags);
  ...
  if( rc==CAPDB_OK ){
    ...
    rc = capdbOsFileSize(pReal, &iSize);
  ...
  if( rc==CAPDB_OK ){
    ...
    if( pWrapper->zData ){
      ...
      ** with the CAPDB_OPEN_MAIN_DB flag. This region of a database file
      ...
      */
      ...
      for(iOff=0; iOff<pWrapper->iSize; iOff += 512){
        ...
        rc = capdbOsRead(pReal, &pWrapper->zData[iOff], nRead, iOff);
      }
    }else{
      rc = CAPDB_NOMEM;
    }
  }
  if( rc!=CAPDB_OK && pWrapper->pMethod ){
    capdbOsClose(pFile);
  }
  return rc;
}

static int cfDelete(capdb_vfs *pCfVfs, const char *zPath, int dirSync){
  capdb_vfs *pVfs = (capdb_vfs *)pCfVfs->pAppData;
...
static void *cfDlOpen(capdb_vfs *pCfVfs, const char *zPath){
  capdb_vfs *pVfs = (capdb_vfs *)pCfVfs->pAppData;
  return pVfs->xDlOpen(pVfs, zPath);
}
static void cfDlError(capdb_vfs *pCfVfs, int nByte, char *zErrMsg){
  capdb_vfs *pVfs = (capdb_vfs *)pCfVfs->pAppData;
  pVfs->xDlError(pVfs, nByte, zErrMsg);
}
static void (*cfDlSym(capdb_vfs *pCfVfs, void *pH, const char *zSym))(void){
  capdb_vfs *pVfs = (capdb_vfs *)pCfVfs->pAppData;
  return pVfs->xDlSym(pVfs, pH, zSym);
}
static void cfDlClose(capdb_vfs *pCfVfs, void *pHandle){
  capdb_vfs *pVfs = (capdb_vfs *)pCfVfs->pAppData;
  pVfs->xDlClose(pVfs, pHandle);
}
static int cfRandomness(capdb_vfs *pCfVfs, int nByte, char *zBufOut){
  capdb_vfs *pVfs = (capdb_vfs *)pCfVfs->pAppData;
  return pVfs->xRandomness(pVfs, nByte, zBufOut);
}
static int cfSleep(capdb_vfs *pCfVfs, int nMicro){
  capdb_vfs *pVfs = (capdb_vfs *)pCfVfs->pAppData;
  return pVfs->xSleep(pVfs, nMicro);
}
static int cfCurrentTime(capdb_vfs *pCfVfs, double *pTimeOut){
  capdb_vfs *pVfs = (capdb_vfs *)pCfVfs->pAppData;
  return pVfs->xCurrentTime(pVfs, pTimeOut);
}
static int cfGetLastError(capdb_vfs *pCfVfs, int n, char *z){
  capdb_vfs *pVfs = (capdb_vfs *)pCfVfs->pAppData;
...
static int processDevSymArgs(
  ...
  struct DeviceFlag {
    ...
    { "batch-atomic",        CAPDB_IOCAP_BATCH_ATOMIC          },
    { 0, 0 }
  };
...
*/
static int CAPDB_TCLAPI crashEnableCmd(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  if( crashVfs.pAppData==0 ){
    capdb_vfs *pOriginalVfs = capdb_vfs_find(0);
    crashVfs.mxPathname = pOriginalVfs->mxPathname;
    crashVfs.pAppData = (void *)pOriginalVfs;
    crashVfs.szOsFile = sizeof(CrashFile) + pOriginalVfs->szOsFile;
    capdb_vfs_register(&crashVfs, isDefault);
  }else{
    crashVfs.pAppData = 0;
    capdb_vfs_unregister(&crashVfs);
  }
...
**
*/
static int CAPDB_TCLAPI crashParamsObjCmd(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
static int CAPDB_TCLAPI devSymObjCmd(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
/*
** tclcmd: capdb_crash_on_write N
*/
static int CAPDB_TCLAPI writeCrashObjCmd(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
*/
static int CAPDB_TCLAPI dsUnregisterObjCmd(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
*/
static int CAPDB_TCLAPI jtObjCmd(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
*/
static int CAPDB_TCLAPI jtUnregisterObjCmd(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
int Sqlitetest6_Init(Tcl_Interp *interp){
  ...
  Tcl_CreateObjCommand(interp, "capdb_crash_enable", crashEnableCmd, 0, 0);
  Tcl_CreateObjCommand(interp, "capdb_crashparams", crashParamsObjCmd, 0, 0);
  Tcl_CreateObjCommand(interp, "capdb_crash_now", crashNowCmd, 0, 0);
  Tcl_CreateObjCommand(interp, "capdb_simulate_device", devSymObjCmd, 0, 0);
  Tcl_CreateObjCommand(interp, "capdb_crash_on_write", writeCrashObjCmd,0,0);
