...
*/
size_t capdb_quota_fread(void*, size_t, size_t, quota_FILE*);
size_t capdb_quota_fwrite(const void*, size_t, size_t, quota_FILE*);
...
*/
int capdb_quota_ferror(quota_FILE*);

/*
** Truncate a file previously opened by capdb_quota_fopen().  Return
...
** undefined behavior.
*/
int capdb_quota_ftruncate(quota_FILE*, capdb_int64 newSize);

/*
** Return the last modification time of the opened file, in seconds
** since 1970.
*/
int capdb_quota_file_mtime(quota_FILE*, time_t *pTime);
...
** quota mechanism, or if calls to capdb_quota_fwrite() have occurred
** which have increased the file size, but those writes have not yet been
** forced to disk using capdb_quota_fflush().
**
** Return -1 if the file is not participating in quota management.
*/
capdb_int64 capdb_quota_file_size(quota_FILE*);
...
*/
capdb_int64 capdb_quota_file_truesize(quota_FILE*);
...
*/
long capdb_quota_file_available(quota_FILE*);
...
**
** A standard SQLite result code is returned (CAPDB_OK, CAPDB_NOMEM, etc.)
