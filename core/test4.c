...
#include "capdbInt.h"
#include "tclsqlite.h"
#if CAPDB_OS_UNIX && CAPDB_THREADSAFE
#include <stdlib.h>
#include <string.h>
#include <pthread.h>
#include <sched.h>
...
struct Thread {
  ...
  ** leader. */
  ...
  capdb_stmt *pStmt;     /* Pending operation */
...
static void test_barrier(){
  capdb_mutex *pMutex = capdb_mutex_alloc(CAPDB_MUTEX_STATIC_APP1);
...
static void *test_thread_main(void *pArg){
  ...
  capdb_open(p->zFilename, &p->db);
  if( CAPDB_OK!=capdb_errcode(p->db) ){
    p->zErr = strdup(capdb_errmsg(p->db));
    capdb_close(p->db);
    p->db = 0;
  }
  p->pStmt = 0;
...
*/
static int CAPDB_TCLAPI tcl_thread_create(
  void *NotUsed,
  ...
  threadset[i].busy = 1;
  capdb_free(threadset[i].zFilename);
  threadset[i].zFilename = capdb_mprintf("%s", argv[2]);
  threadset[i].opnum = 1;
...
*/
static int CAPDB_TCLAPI tcl_thread_wait(
  void *NotUsed,
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int argc,              /* Number of arguments */
  const char **argv      /* Text of each argument */
...
*/
static int CAPDB_TCLAPI tcl_thread_halt(
  void *NotUsed,
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int argc,              /* Number of arguments */
  const char **argv      /* Text of each argument */
...
*/
static int CAPDB_TCLAPI tcl_thread_argc(
  void *NotUsed,
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int argc,              /* Number of arguments */
  const char **argv      /* Text of each argument */
  ...
  test_thread_wait(&threadset[i]);
  capdb_snprintf(sizeof(zBuf), zBuf, "%d", threadset[i].argc);
...
*/
static int CAPDB_TCLAPI tcl_thread_argv(
  void *NotUsed,
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int argc,              /* Number of arguments */
  const char **argv      /* Text of each argument */
...
*/
static int CAPDB_TCLAPI tcl_thread_colname(
  void *NotUsed,
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int argc,              /* Number of arguments */
  const char **argv      /* Text of each argument */
...
*/
static int CAPDB_TCLAPI tcl_thread_result(
  void *NotUsed,
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int argc,              /* Number of arguments */
  const char **argv      /* Text of each argument */
  ...
  test_thread_wait(&threadset[i]);
  zName = capdbErrName(threadset[i].rc);
...
** error string.
*/
static int CAPDB_TCLAPI tcl_thread_error(
  void *NotUsed,
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int argc,              /* Number of arguments */
  const char **argv      /* Text of each argument */
...
static void do_compile(Thread *p){
  ...
  p->rc = capdb_prepare(p->db, p->zArg, -1, &p->pStmt, 0);
}
...
*/
static int CAPDB_TCLAPI tcl_thread_compile(
  void *NotUsed,
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int argc,              /* Number of arguments */
  const char **argv      /* Text of each argument */
  ...
  capdb_free(threadset[i].zArg);
  threadset[i].zArg = capdb_mprintf("%s", argv[2]);
  test_barrier();
  threadset[i].opnum++;
  return TCL_OK;
}
...
static void do_step(Thread *p){
  ...
  p->rc = capdb_step(p->pStmt);
  if( p->rc==CAPDB_ROW ){
    p->argc = capdb_column_count(p->pStmt);
    for(i=0; i<capdb_data_count(p->pStmt); i++){
      p->argv[i] = (char*)capdb_column_text(p->pStmt, i);
    }
    for(i=0; i<p->argc; i++){
      p->colv[i] = capdb_column_name(p->pStmt, i);
    }
  }
}
...
*/
static int CAPDB_TCLAPI tcl_thread_step(
  void *NotUsed,
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int argc,              /* Number of arguments */
  const char **argv      /* Text of each argument */
...
*/
static int CAPDB_TCLAPI tcl_thread_finalize(
  void *NotUsed,
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int argc,              /* Number of arguments */
  const char **argv      /* Text of each argument */
...
*/
static int CAPDB_TCLAPI tcl_thread_swap(
  void *NotUsed,
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int argc,              /* Number of arguments */
  const char **argv      /* Text of each argument */
...
*/
static int CAPDB_TCLAPI tcl_thread_db_get(
  void *NotUsed,
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int argc,              /* Number of arguments */
  const char **argv      /* Text of each argument */
  ...
  test_thread_wait(&threadset[i]);
  capdbTestMakePointerStr(interp, zBuf, threadset[i].db);
  threadset[i].db = 0;
...
**
*/
static int CAPDB_TCLAPI tcl_thread_db_put(
  void *NotUsed,
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int argc,              /* Number of arguments */
  const char **argv      /* Text of each argument */
  ...
  threadset[i].db = (capdb*)capdbTestTextToPtr(argv[2]);
  return TCL_OK;
}
...
*/
static int CAPDB_TCLAPI tcl_thread_stmt_get(
  void *NotUsed,
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int argc,              /* Number of arguments */
  const char **argv      /* Text of each argument */
  ...
  test_thread_wait(&threadset[i]);
  capdbTestMakePointerStr(interp, zBuf, threadset[i].pStmt);
  threadset[i].pStmt = 0;
  Tcl_AppendResult(interp, zBuf, (char*)0);
  return TCL_OK;
}
