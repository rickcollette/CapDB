...
** malloc fails.
*/
Bitvec *capdbBitvecCreate(u32 iSize){
  Bitvec *p;
...
*/
int capdbBitvecTestNotNull(Bitvec *p, u32 i){
  assert( p!=0 );
  i--;
...
int capdbBitvecTest(Bitvec *p, u32 i){
  return p!=0 && capdbBitvecTestNotNull(p,i);
}
...
*/
int capdbBitvecSet(Bitvec *p, u32 i){
  u32 h;
  ...
  while((p->iSize > BITVEC_NBIT) && p->iDivisor) {
    ...
    i = i%p->iDivisor;
    if( p->u.apSub[bin]==0 ){
      p->u.apSub[bin] = capdbBitvecCreate( p->iDivisor );
      if( p->u.apSub[bin]==0 ) return CAPDB_NOMEM_BKPT;
    }
    p = p->u.apSub[bin];
  }
  if( p->iSize<=BITVEC_NBIT ){
  ...
  do {
    if( p->u.aHash[h]==i ) return CAPDB_OK;
    h++;
...
bitvec_set_rehash:
  if( p->nSet>=BITVEC_MXHASH ){
    unsigned int j;
    int rc;
    u32 *aiValues = capdbStackAllocRaw(0, sizeof(p->u.aHash));
    if( aiValues==0 ){
      ...
      rc = capdbBitvecSet(p, i);
      for(j=0; j<BITVEC_NINT; j++){
        if( aiValues[j] ) rc |= capdbBitvecSet(p, aiValues[j]);
      }
      capdbStackFree(0, aiValues);
      return rc;
    }
  }
bitvec_set_end:
...
*/
void capdbBitvecClear(Bitvec *p, u32 i, void *pBuf){
  if( p==0 ) return;
  assert( i>0 );
  i--;
  while( p->iDivisor ){
...
*/
void capdbBitvecDestroy(Bitvec *p){
  if( p==0 ) return;
  if( p->iDivisor ){
    unsigned int i;
    for(i=0; i<BITVEC_NPTR; i++){
      capdbBitvecDestroy(p->u.apSub[i]);
    }
  }
  capdb_free(p);
}
...
static void showBitvec(Bitvec *p, int n, unsigned x){
  ...
  if( p->iSize<=BITVEC_NBIT ){
    ...
    for(i=1; i<=BITVEC_NBIT; i++){
      if( capdbBitvecTest(p,i) ) printf(" %u", x+(unsigned)i);
    }
    printf("\n");
...
**    6          Invoice capdbShowBitvec() on the Bitvec object so far
...
** SQLite has been compiled with CAPDB_DEBUG.
**
...
*/
int capdbBitvecBuiltinTest(int sz, int *aOp){
  Bitvec *pBitvec = 0;
  unsigned char *pV = 0;
  int rc = -1;
  int i, nx, pc, op;
  ...
  ** bits to act as the reference */
  if( sz<=0 ){
    pBitvec = capdbBitvecCreate( 2*(unsigned)(-sz) );
    pV = 0;
  }else{
    pBitvec = capdbBitvecCreate( sz );
    pV = capdbMallocZero( (7+(i64)sz)/8 + 1 );
  }
  pTmpSpace = capdb_malloc64(BITVEC_SZ);
  if( pBitvec==0 || pTmpSpace==0 || (pV==0 && sz>0) ) goto bitvec_end;

  /* NULL pBitvec tests */
  capdbBitvecSet(0, 1);
  capdbBitvecClear(0, 1, pTmpSpace);
  ...
  while( (op = aOp[pc])!=0 ){
    ...
    if( (op & 1)!=0 ){
      if( pV ) SETBIT(pV, (i+1));
      if( op!=5 ){
        if( capdbBitvecSet(pBitvec, i+1) ) goto bitvec_end;
      }
    }else{
      if( pV ) CLEARBIT(pV, (i+1));
      capdbBitvecClear(pBitvec, i+1, pTmpSpace);
    }
  }
  ...
  */
  if( pV ){
    rc = capdbBitvecTest(0,0) + capdbBitvecTest(pBitvec, sz+1)
            + capdbBitvecTest(pBitvec, 0)
            + (capdbBitvecSize(pBitvec) - sz);
    for(i=1; i<=sz; i++){
      if( (TESTBIT(pV,i))!=capdbBitvecTest(pBitvec,i) ){
        rc = i;
        break;
      }
    }
...
bitvec_end:
  capdb_free(pTmpSpace);
  capdb_free(pV);
  capdbBitvecDestroy(pBitvec);
  return rc;
}
