...
/*
** Extra information appended to the end of capdb_index_info but not
** visible to the xBestIndex function, at least not directly.  The
** capdb_vtab_collation() interface knows how to reach it, however.
**
** This object is not an API and can be changed from one release to the
** next.  As long as allocateIndexInfo() and capdb_vtab_collation()
...
struct HiddenIndexInfo {
  ...
  int eDistinct;           /* Value to return from capdb_vtab_distinct() */
  u32 mIn;                 /* Mask of terms that are <col> IN (...) */
  u32 mHandleIn;           /* Terms that vtab will handle as <col> IN (...) */
  capdb_value *aRhs[FLEXARRAY];  /* RHS values for constraints. MUST BE LAST
...
#define SZ_HIDDENINDEXINFO(N) \
                  (offsetof(HiddenIndexInfo,aRhs) + (N)*sizeof(capdb_value*))

/* Forward declaration of methods */
static int whereLoopResize(capdb*, WhereLoop*, int);

/*
** Return the estimated number of output rows from a WHERE clause
*/
LogEst capdbWhereOutputRowCount(WhereInfo *pWInfo){
...
*/
int capdbWhereIsDistinct(WhereInfo *pWInfo){
...
*/
int capdbWhereIsOrdered(WhereInfo *pWInfo){
...
** the final answer.
*/
int capdbWhereOrderByLimitOptLabel(WhereInfo *pWInfo){
  WhereLevel *pInner;
...
*/
void capdbWhereMinMaxOptEarlyOut(Vdbe *v, WhereInfo *pWInfo){
  WhereLevel *pInner;
  int i;
  ...
  for(i=pWInfo->nLevel-1; i>=0; i--){
    ...
    if( (pInner->pWLoop->wsFlags & WHERE_COLUMN_IN)!=0 ){
      capdbVdbeGoto(v, pInner->addrNxt);
      return;
    }
  }
  capdbVdbeGoto(v, pWInfo->iBreak);
}
...
*/
int capdbWhereContinueLabel(WhereInfo *pWInfo){
...
** out of a WHERE loop.
*/
int capdbWhereBreakLabel(WhereInfo *pWInfo){
...
*/
int capdbWhereOkOnePass(WhereInfo *pWInfo, int *aiCur){
  memcpy(aiCur, pWInfo->aiCurOnePass, sizeof(int)*2);
#ifdef WHERETRACE_ENABLED
  if( capdbWhereTrace && pWInfo->eOnePass!=ONEPASS_OFF ){
    capdbDebugPrintf("%s cursors: %d %d\n",
...
*/
int capdbWhereUsesDeferredSeek(WhereInfo *pWInfo){
...
*/
Bitmask capdbWhereGetMask(WhereMaskSet *pMaskSet, int iCursor){
  int i;
...
*/
void *capdbWhereMalloc(WhereInfo *pWInfo, u64 nByte){
  WhereMemBlock *pBlock;
  pBlock = capdbDbMallocRawNN(pWInfo->pParse->db, nByte+sizeof(*pBlock));
  if( pBlock ){
...
void *capdbWhereRealloc(WhereInfo *pWInfo, void *pOld, u64 nByte){
  void *pNew = capdbWhereMalloc(pWInfo, nByte);
  if( pNew && pOld ){
...
** capdbWhereBegin() routine.  So we know that the pMaskSet->ix[]
...
static Expr *whereRightSubexprIsColumn(Expr *p){
  p = capdbExprSkipCollateAndLikely(p->pRight);
...
** column with affinity idxaff (one of the CAPDB_AFF_XYZ values). If so,
...
*/
static CAPDB_NOINLINE const char *indexInAffinityOk(
  Parse *pParse, 
  WhereTerm *pTerm, 
  u8 idxaff
){
  ...
  if( capdbIndexAffinityOk(pX, idxaff) ){
    CollSeq *pRet = capdbExprCompareCollSeq(pParse, pX);
    return pRet ? pRet->zName : capdbStrBINARY;
  }
  return 0;
}
    ...
    do{
      for(pTerm=pWC->a+k; k<pWC->nTerm; k++, pTerm++){
        ...
        if( pTerm->leftCursor==iCur
         ...
         && (iColumn!=XN_EXPR
             || capdbExprCompareSkip(pTerm->pExpr->pLeft,
          ...
          if( (pTerm->eOperator & pScan->opMask)!=0 ){
            ...
            if( pScan->zCollName && (pTerm->eOperator & WO_ISNULL)==0 ){
              ...
              if( (pTerm->eOperator & WO_IN) ){
                ...
                CollSeq *pColl;
                if( !capdbIndexAffinityOk(pX, pScan->idxaff) ){
                  continue;
                }
                assert(pX->pLeft);
                pColl = capdbExprCompareCollSeq(pParse, pX);
                zCollName = pColl ? pColl->zName : capdbStrBINARY;
              }

              if( capdbStrICmp(zCollName, pScan->zCollName) ){
                continue;
              }
            }
            ...
            pScan->k = k+1;
#ifdef WHERETRACE_ENABLED
            if( (capdbWhereTrace & 0x20000)!=0 && pScan->nEquiv>1 ){
              int ii;
              capdbDebugPrintf("EQUIVALENT TO {%d:%d} (due to TERM-%d):",
                 pScan->aiCur[0], pScan->aiColumn[0], pTerm->iTerm);
              for(ii=1; ii<pScan->nEquiv; ii++){
                capdbDebugPrintf(" {%d:%d}",
...
*/
static CAPDB_NOINLINE WhereTerm *whereScanInitIndexExpr(WhereScan *pScan){
  pScan->idxaff = capdbExprAffinity(pScan->pIdxExpr);
...
static int findIndexCol(
  ...
  for(i=0; i<pList->nExpr; i++){
    Expr *p = capdbExprSkipCollateAndLikely(pList->a[i].pExpr);
    if( ALWAYS(p!=0)
     && (p->op==TK_COLUMN || p->op==TK_AGG_COLUMN)
     && p->iColumn==pIdx->aiColumn[iCol]
     && p->iTable==iBase
    ){
      CollSeq *pColl = capdbExprNNCollSeq(pParse, pList->a[i].pExpr);
      if( 0==capdbStrICmp(pColl->zName, zColl) ){
        return i;
      }
    }
  }
...
static int isDistinctRedundant(
  ...
  */
  for(i=0; i<pDistinct->nExpr; i++){
    Expr *p = capdbExprSkipCollateAndLikely(pDistinct->a[i].pExpr);
  ...
  */
  for(pIdx=pTab->pIndex; pIdx; pIdx=pIdx->pNext){
    ...
    for(i=0; i<pIdx->nKeyCol; i++){
      if( 0==capdbWhereFindTerm(pWC, iBase, i, ~(Bitmask)0, WO_EQ, pIdx) ){
...
static LogEst estLog(LogEst N){
  return N<=10 ? 0 : capdbLogEst(N) - 33;
}
...
static void translateColumnToCopy(
  ...
  Vdbe *v = pParse->pVdbe;
  VdbeOp *pOp = capdbVdbeGetOp(v, iStart);
  int iEnd = capdbVdbeCurrentAddr(v);
  if( pParse->db->mallocFailed ) return;
#ifdef CAPDB_DEBUG
  if( pParse->db->flags & CAPDB_VdbeAddopTrace ){
...
/*
** Two routines for printing the content of an capdb_index_info
** structure.  Used for testing and debugging only.  If neither
** CAPDB_TEST or CAPDB_DEBUG are defined, then these routines
** are no-ops.
*/
#if !defined(CAPDB_OMIT_VIRTUALTABLE) && defined(WHERETRACE_ENABLED)
static void whereTraceIndexInfoInputs(
  capdb_index_info *p,   /* The IndexInfo object */
  Table *pTab              /* The TABLE that is the virtual table */
){
  int i;
  if( (capdbWhereTrace & 0x10)==0 ) return;
  capdbDebugPrintf("capdb_index_info inputs for %s:\n", pTab->zName);
  ...
  for(i=0; i<p->nOrderBy; i++){
    capdbDebugPrintf("  orderby[%d]: col=%d desc=%d\n",
       i,
...
static void whereTraceIndexInfoOutputs(
  ...
  int i;
  if( (capdbWhereTrace & 0x10)==0 ) return;
  capdbDebugPrintf("capdb_index_info outputs for %s:\n", pTab->zName);
  for(i=0; i<p->nConstraint; i++){
    capdbDebugPrintf("  usage[%d]: argvIdx=%d omit=%d\n",
       i,
       p->aConstraintUsage[i].argvIndex,
       p->aConstraintUsage[i].omit);
  }
  capdbDebugPrintf("  idxNum=%d\n", p->idxNum);
  capdbDebugPrintf("  idxStr=%s\n", p->idxStr);
  capdbDebugPrintf("  orderByConsumed=%d\n", p->orderByConsumed);
  capdbDebugPrintf("  estimatedCost=%g\n", p->estimatedCost);
  capdbDebugPrintf("  estimatedRows=%lld\n", p->estimatedRows);
}
#else
...
#ifndef CAPDB_OMIT_AUTOMATIC_INDEX
/*
...
*/
static CAPDB_NOINLINE int columnIsGoodIndexCandidate(
  const Table *pTab,
  int iCol
){
  const Index *pIdx;
...
#endif /* CAPDB_OMIT_AUTOMATIC_INDEX */
...
static int termCanDriveIndex(
  ...
  if( !capdbIndexAffinityOk(pTerm->pExpr, aff) ) return 0;
...
**
** This is only required if capdb_stmt_scanstatus() is enabled, to
** associate an CAPDB_SCANSTAT_NCYCLE and CAPDB_SCANSTAT_NLOOP
...
static void explainAutomaticIndex(
  ...
  if( IS_STMT_SCANSTATUS(pParse->db) && pParse->explain!=2 ){
    ...
    char *zText = 0;
    int ii = 0;
    capdb_str *pStr = capdb_str_new(pParse->db);
    capdb_str_appendf(pStr,"CREATE AUTOMATIC INDEX ON %s(", pTab->zName);
    ...
    for(ii=0; ii<(pIdx->nColumn-1); ii++){
      ...
      capdb_str_appendf(pStr, "%s%s", zSep, zName);
      zSep = ", ";
    }
    zText = capdb_str_finish(pStr);
    if( zText==0 ){
      capdbOomFault(pParse->db);
    }else{
      *pAddrExplain = capdbVdbeExplain(
...
*/
static CAPDB_NOINLINE void constructAutomaticIndex(
  Parse *pParse,              /* The parsing context */
  WhereClause *pWC,           /* The WHERE clause */
  ...
  ** transient index on 2nd and subsequent iterations of the loop. */
  v = pParse->pVdbe;
  assert( v!=0 );
  addrInit = capdbVdbeAddOp0(v, OP_Once); VdbeCoverage(v);
  ...
  for(pTerm=pWC->a; pTerm<pWCEnd; pTerm++){
    ...
    ** rows of the target table (pSrc) that can be used. */
    if( (pTerm->wtFlags & TERM_VIRTUAL)==0
     && capdbExprIsSingleTableConstraint(pExpr, pTabList, pLevel->iFrom, 0)
    ){
      pPartial = capdbExprAnd(pParse, pPartial,
                                capdbExprDup(pParse->db, pExpr, 0));
    }
    if( termCanDriveIndex(pTerm, pSrc, notReady) ){
      ...
      testcase( iCol==BMS-1 );
      if( !sentWarning ){
        capdb_log(CAPDB_WARNING_AUTOINDEX,
  ...
  */
  ...
  pIdx = capdbAllocateIndexObject(pParse->db, nKeyCol+HasRowid(pTable),
                                    0, &zNotUsed);
  ...
  for(pTerm=pWC->a; pTerm<pWCEnd; pTerm++){
    if( termCanDriveIndex(pTerm, pSrc, notReady) ){
      ...
      if( (idxCols & cMask)==0 ){
        ...
        pIdx->azColl[n] = pColl ? pColl->zName : capdbStrBINARY;
        n++;
        if( ALWAYS(pX->pLeft!=0)
         && capdbExprAffinity(pX->pLeft)!=CAPDB_AFF_TEXT
        ){
  ...
  ** a covering index */
  ...
  capdbVdbeAddOp2(v, OP_OpenAutoindex, pLevel->iIdxCur, nKeyCol+1);
  capdbVdbeSetP4KeyInfo(pParse, pIdx);
  VdbeComment((v, "for %s", pTable->zName));
  if( OptimizationEnabled(pParse->db, CAPDB_BloomFilter) && useBloomFilter ){
    capdbWhereExplainBloomFilter(pParse, pWC->pWInfo, pLevel);
    pLevel->regFilter = ++pParse->nMem;
    capdbVdbeAddOp2(v, OP_Blob, 10000, pLevel->regFilter);
  }
  ...
  if( pSrc->fg.viaCoroutine ){
    ...
    addrCounter = capdbVdbeAddOp2(v, OP_Integer, 0, 0);
    capdbVdbeAddOp3(v, OP_InitCoroutine, regYield, 0, pSubq->addrFillSub);
    addrTop =  capdbVdbeAddOp1(v, OP_Yield, regYield);
    VdbeCoverage(v);
    VdbeComment((v, "next row of %s", pSrc->pSTab->zName));
  }else{
    assert( pLevel->addrHalt );
    addrTop = capdbVdbeAddOp2(v, OP_Rewind,pLevel->iTabCur,pLevel->addrHalt);
    VdbeCoverage(v);
  }
  if( pPartial ){
    iContinue = capdbVdbeMakeLabel(pParse);
    capdbExprIfFalse(pParse, pPartial, iContinue, CAPDB_JUMPIFNULL);
    pLoop->wsFlags |= WHERE_PARTIALIDX;
  }
  regRecord = capdbGetTempReg(pParse);
  regBase = capdbGenerateIndexKey(
      pParse, pIdx, pLevel->iTabCur, regRecord, 0, 0, 0, 0
  );
  if( pLevel->regFilter ){
    capdbVdbeAddOp4Int(v, OP_FilterAdd, pLevel->regFilter, 0,
                         regBase, pLoop->u.btree.nEq);
  }
  capdbVdbeScanStatusCounters(v, addrExp, addrExp, capdbVdbeCurrentAddr(v));
  capdbVdbeAddOp2(v, OP_IdxInsert, pLevel->iIdxCur, regRecord);
  capdbVdbeChangeP5(v, OPFLAG_USESEEKRESULT);
  if( pPartial ) capdbVdbeResolveLabel(v, iContinue);
  if( pSrc->fg.viaCoroutine ){
    assert( pSrc->fg.isSubquery && pSrc->u4.pSubq!=0 );
    capdbVdbeChangeP2(v, addrCounter, regBase+n);
    ...
    capdbVdbeAddOp2(v, OP_Next, pLevel->iTabCur, addrTop+1); VdbeCoverage(v);
    capdbVdbeChangeP5(v, CAPDB_STMTSTATUS_AUTOINDEX);
  ...
  capdbVdbeJumpHere(v, addrInit);
  capdbVdbeScanStatusRange(v, addrExp, addrExp, -1);

end_auto_index_create:
  capdbExprDelete(pParse->db, pPartial);
}
#endif /* CAPDB_OMIT_AUTOMATIC_INDEX */
...
** this recursive initialization if the CAPDB_BloomPulldown optimization has
** been turned off.
**
...
** capdbWhereCodeOneLoopStart() will generate code to test the Bloom filter
...
** is set.
*/
static CAPDB_NOINLINE void capdbConstructBloomFilter(
  ...
  do{
    ...
    u64 sz;
    int iSrc;
    capdbWhereExplainBloomFilter(pParse, pWInfo, pLevel);
    ...
    */
    ...
    pTab = pItem->pSTab;
    assert( pTab!=0 );
    sz = capdbLogEstToInt(pTab->nRowLogEst);
    if( sz<10000 ){
      sz = 10000;
    }else if( sz>10000000 ){
      sz = 10000000;
    }
    capdbVdbeAddOp2(v, OP_Blob, (int)sz, pLevel->regFilter);

    addrTop = capdbVdbeAddOp1(v, OP_Rewind, iCur); VdbeCoverage(v);
    pWCEnd = &pWInfo->sWC.a[pWInfo->sWC.nTerm];
    for(pTerm=pWInfo->sWC.a; pTerm<pWCEnd; pTerm++){
      Expr *pExpr = pTerm->pExpr;
      if( (pTerm->wtFlags & TERM_VIRTUAL)==0
       && capdbExprIsSingleTableConstraint(pExpr, pTabList, iSrc, 0)
      ){
        capdbExprIfFalse(pParse, pTerm->pExpr, addrCont, CAPDB_JUMPIFNULL);
      }
    }
    if( pLoop->wsFlags & WHERE_IPK ){
      int r1 = capdbGetTempReg(pParse);
      capdbVdbeAddOp2(v, OP_Rowid, iCur, r1);
      capdbVdbeAddOp4Int(v, OP_FilterAdd, pLevel->regFilter, 0, r1, 1);
      capdbReleaseTempReg(pParse, r1);
    }else{
      Index *pIdx = pLoop->u.btree.pIndex;
      int n = pLoop->u.btree.nEq;
      int r1 = capdbGetTempRange(pParse, n);
      int jj;
      for(jj=0; jj<n; jj++){
        assert( pIdx->pTable==pItem->pSTab );
        capdbExprCodeLoadIndexColumn(pParse, pIdx, iCur, jj, r1+jj);
      }
      capdbVdbeAddOp4Int(v, OP_FilterAdd, pLevel->regFilter, 0, r1, n);
      capdbReleaseTempRange(pParse, r1, n);
    }
    capdbVdbeResolveLabel(v, addrCont);
    capdbVdbeAddOp2(v, OP_Next, pLevel->iTabCur, addrTop+1);
    VdbeCoverage(v);
    capdbVdbeJumpHere(v, addrTop);
    pLoop->wsFlags &= ~WHERE_BLOOMFILTER;
    if( OptimizationDisabled(pParse->db, CAPDB_BloomPulldown) ) break;
...
/*
** Allocate and populate an capdb_index_info structure. It is the
** responsibility of the caller to eventually release the structure
** by passing the pointer returned by this function to freeIndexInfo().
*/
static capdb_index_info *allocateIndexInfo(
  WhereInfo *pWInfo,              /* The WHERE clause */
  WhereClause *pWC,               /* The WHERE clause being analyzed */
  ...
  struct capdb_index_constraint *pIdxCons;
  struct capdb_index_orderby *pIdxOrderBy;
  ...
  ** the capdb_index_info structure.
  */
  nOrderBy = 0;
  if( pOrderBy ){
    ...
    for(i=0; i<n; i++){
      ...
      if( capdbExprIsConstant(0, pExpr) ){
        continue;
      }
      ...
      ** of the COLLATE operator matches the collation of the column. */
      if( pExpr->op==TK_COLLATE
       ...
       && pE2->iTable==pSrc->iCursor
        ...
        zColl = capdbColumnColl(&pTab->aCol[pE2->iColumn]);
        if( zColl==0 ) zColl = capdbStrBINARY;
        if( capdb_stricmp(pExpr->u.zToken, zColl)==0 ) continue;
      }
  ...
  /* Allocate the capdb_index_info structure
  */
  pIdxInfo = capdbDbMallocZero(pParse->db, sizeof(*pIdxInfo)
                           + (sizeof(*pIdxCons) + sizeof(*pUsage))*nTerm
                           + sizeof(*pIdxOrderBy)*nOrderBy
                           + SZ_HIDDENINDEXINFO(nTerm) );
  if( pIdxInfo==0 ){
    capdbErrorMsg(pParse, "out of memory");
    return 0;
  }
  pHidden = (struct HiddenIndexInfo*)&pIdxInfo[1];
  pIdxCons = (struct capdb_index_constraint*)&pHidden->aRhs[nTerm];
  pIdxOrderBy = (struct capdb_index_orderby*)&pIdxCons[nTerm];
  pUsage = (struct capdb_index_constraint_usage*)&pIdxOrderBy[nOrderBy];
  pIdxInfo->aConstraint = pIdxCons;
  pIdxInfo->aOrderBy = pIdxOrderBy;
  pIdxInfo->aConstraintUsage = pUsage;
  pIdxInfo->colUsed = (capdb_int64)pSrc->colUsed;
  if( HasRowid(pTab)==0 ){
    ...
    ** ensure they are available for cases like RIGHT joins or OR loops. */
    Index *pPk = capdbPrimaryKeyIndex((Table*)pTab);
    assert( pPk!=0 );
  ...
  for(p=pWC, i=j=0; p; p=p->pOuter){
    ...
    for(pTerm=p->a; i<nLast; i++, pTerm++){
      ...
      if( op==WO_AUX ){
        ...
        if( op==WO_ISNULL ){
          pIdxCons[j].op = CAPDB_INDEX_CONSTRAINT_ISNULL;
        }else{
          pIdxCons[j].op = CAPDB_INDEX_CONSTRAINT_IS;
        }
      }else{
        pIdxCons[j].op = (u8)op;
        /* The direct assignment in the previous line is possible only because
        ** the WO_ and CAPDB_INDEX_CONSTRAINT_ codes are identical.  The
        ** following asserts verify this fact. */
        assert( WO_EQ==CAPDB_INDEX_CONSTRAINT_EQ );
        assert( WO_LT==CAPDB_INDEX_CONSTRAINT_LT );
        assert( WO_LE==CAPDB_INDEX_CONSTRAINT_LE );
        assert( WO_GT==CAPDB_INDEX_CONSTRAINT_GT );
        assert( WO_GE==CAPDB_INDEX_CONSTRAINT_GE );
        assert( pTerm->eOperator&(WO_IN|WO_EQ|WO_LT|WO_LE|WO_GT|WO_GE|WO_AUX) );

        if( op & (WO_LT|WO_LE|WO_GT|WO_GE)
            && capdbExprIsVector(pTerm->pExpr->pRight)
          ){
          testcase( j!=i );
          if( j<16 ) mNoOmit |= (1 << j);
  ...
  for(i=j=0; i<nOrderBy; i++){
    Expr *pExpr = pOrderBy->a[i].pExpr;
    if( capdbExprIsConstant(0, pExpr) ) continue;
...
/*
** Free and zero the capdb_index_info.idxStr value if needed.
*/
static void freeIdxStr(capdb_index_info *pIdxInfo){
...
/*
** Free an capdb_index_info structure allocated by allocateIndexInfo()
** and possibly modified by xBestIndex methods.
*/
static void freeIndexInfo(capdb *db, capdb_index_info *pIdxInfo){
  ...
  for(i=0; i<pIdxInfo->nConstraint; i++){
    capdbValueFree(pHidden->aRhs[i]); /* IMP: R-14553-25174 */
    pHidden->aRhs[i] = 0;
  }
  freeIdxStr(pIdxInfo);
  capdbDbFree(db, pIdxInfo);
}

/*
** The table object reference passed as the second argument to this function
** must represent a virtual table. This function invokes the xBestIndex()
** method of the virtual table with the capdb_index_info object that
** comes in as the 3rd argument to this function.
**
** If an error occurs, pParse is populated with an error message and an
** appropriate error code is returned.  A return of CAPDB_CONSTRAINT from
** xBestIndex is not considered an error.  CAPDB_CONSTRAINT indicates that
** the current configuration of "unusable" flags in capdb_index_info can
...
*/
static int vtabBestIndex(Parse *pParse, Table *pTab, capdb_index_info *p){
  int rc;
  capdb_vtab *pVtab;

  assert( IsVirtual(pTab) );
  pVtab = capdbGetVTable(pParse->db, pTab)->pVtab;
  ...
  if( rc!=CAPDB_OK && rc!=CAPDB_CONSTRAINT ){
    if( rc==CAPDB_NOMEM ){
      capdbOomFault(pParse->db);
    }else if( !pVtab->zErrMsg ){
      capdbErrorMsg(pParse, "%s", capdbErrStr(rc));
    }else{
      capdbErrorMsg(pParse, "%s", pVtab->zErrMsg);
    }
  }
  if( pTab->u.vtab.p->bAllSchemas ){
    capdbVtabUsesAllSchemas(pParse);
  }
...
static int whereKeyStats(
  ...
  */
  ...
  do{
    ...
    pRec->nField = n;
    res = capdbVdbeRecordCompare(aSample[iSamp].n, aSample[iSamp].p, pRec);
    if( res<0 ){
  ...
  ** than to invoke the asserts.  */
  if( pParse->db->mallocFailed==0 ){
    if( res==0 ){
      ...
      pRec->nField = nField;
      assert( 0==capdbVdbeRecordCompare(aSample[i].n, aSample[i].p, pRec)
      ...
      ** (iCol+1) field prefix of sample i.  */
      ...
      pRec->nField = iCol+1;
      assert( i==pIdx->nSample
           || capdbVdbeRecordCompare(aSample[i].n, aSample[i].p, pRec)>0
           || pParse->db->mallocFailed );
      ...
      ** If (i>0), then pRec must also be greater than sample (i-1).  */
      if( iCol>0 ){
        pRec->nField = iCol;
        assert( capdbVdbeRecordCompare(aSample[i].n, aSample[i].p, pRec)<=0
             || pParse->db->mallocFailed || CORRUPT_DB );
      }
      if( i>0 ){
        pRec->nField = nField;
        assert( capdbVdbeRecordCompare(aSample[i-1].n, aSample[i-1].p, pRec)<0
             || pParse->db->mallocFailed || CORRUPT_DB );
      }
    }
  }
...
static LogEst whereRangeAdjust(WhereTerm *pTerm, LogEst nNew){
  ...
  if( pTerm ){
    if( pTerm->truthProb<=0 ){
      ...
      nRet -= 20;        assert( 20==capdbLogEst(4) );
    }
  }
  return nRet;
}
...
*/
char capdbIndexColumnAffinity(capdb *db, Index *pIdx, int iCol){
  assert( iCol>=0 && iCol<pIdx->nColumn );
  if( !pIdx->zColAff ){
    if( capdbIndexAffinityStr(db, pIdx)==0 ) return CAPDB_AFF_BLOB;
  }
...
static int whereRangeSkipScanEst(
  ...
  int rc = CAPDB_OK;
  u8 aff = capdbIndexColumnAffinity(db, p, nEq);
  CollSeq *pColl;
 
  capdb_value *p1 = 0;          /* Value extracted from pLower */
  capdb_value *p2 = 0;          /* Value extracted from pUpper */
  capdb_value *pVal = 0;        /* Value extracted from record */

  pColl = capdbLocateCollSeq(pParse, p->azColl[nEq]);
  if( pLower ){
    rc = capdbStat4ValueFromExpr(pParse, pLower->pExpr->pRight, aff, &p1);
    nLower = 0;
  }
  if( pUpper && rc==CAPDB_OK ){
    rc = capdbStat4ValueFromExpr(pParse, pUpper->pExpr->pRight, aff, &p2);
  ...
  if( p1 || p2 ){
    int i;
    int nDiff;
    for(i=0; rc==CAPDB_OK && i<p->nSample; i++){
      rc = capdbStat4Column(db, p->aSample[i].p, p->aSample[i].n, nEq, &pVal);
      if( rc==CAPDB_OK && p1 ){
        int res = capdbMemCompare(p1, pVal, pColl);
        if( res>=0 ) nLower++;
      }
      if( rc==CAPDB_OK && p2 ){
        int res = capdbMemCompare(p2, pVal, pColl);
        if( res>=0 ) nUpper++;
      }
    }
    nDiff = (nUpper - nLower);
    ...
    ** using the method described in the header comment for this function. */
    if( nDiff!=1 || pUpper==0 || pLower==0 ){
      int nAdjust = (capdbLogEst(p->nSample) - capdbLogEst(nDiff));
      pLoop->nOut -= nAdjust;
      *pbDone = 1;
...
**
** When this function is called, *pnOut is set to the capdbLogEst() of the
...
static int whereRangeScanEst(
  ...
  if( p->nSample>0 && ALWAYS(nEq<p->nSampleCol)
   && OptimizationEnabled(pParse->db, CAPDB_Stat4)
  ){
    if( nEq==pBuilder->nRecValid ){
      ...
      */
      ...
      if( pLower ){
        ...
        rc = capdbStat4ProbeSetValue(pParse, p, &pRec, pExpr, nBtm, nEq, &n);
        if( rc==CAPDB_OK && n ){
          tRowcnt iNew;
          u16 mask = WO_GT|WO_LE;
          if( capdbExprVectorSize(pExpr)>n ) mask = (WO_LE|WO_LT);
      ...
      if( pUpper ){
        ...
        rc = capdbStat4ProbeSetValue(pParse, p, &pRec, pExpr, nTop, nEq, &n);
        if( rc==CAPDB_OK && n ){
          tRowcnt iNew;
          u16 mask = WO_GT|WO_LE;
          if( capdbExprVectorSize(pExpr)>n ) mask = (WO_LE|WO_LT);
      ...
      if( rc==CAPDB_OK ){
        if( iUpper>iLower ){
          nNew = capdbLogEst(iUpper - iLower);
          /* TUNING:  If both iUpper and iLower are derived from the same
          ** sample, then assume they are 4x more selective.  This brings
          ** the estimated selectivity more in line with what it would be
          ** if estimated without the use of STAT4 tables. */
          if( iLwrIdx==iUprIdx ){ nNew -= 20; }
          assert( 20==capdbLogEst(4) );
        }else{
          nNew = 10;        assert( 10==capdbLogEst(2) );
        }
        if( nNew<nOut ){
          nOut = nNew;
        }
...
**
** Write the estimated row count into *pnRow and return CAPDB_OK.
...
static int whereEqualScanEst(
  ...
  ** of this one, no estimate can be made. Return CAPDB_NOTFOUND. */
  if( pBuilder->nRecValid<(nEq-1) ){
    return CAPDB_NOTFOUND;
  }

  /* This is an optimization only. The call to capdbStat4ProbeSetValue()
  ** below would return the same value.  */
  ...
  rc = capdbStat4ProbeSetValue(pParse, p, &pRec, pExpr, 1, nEq-1, &bOk);
  pBuilder->pRec = pRec;
  if( rc!=CAPDB_OK ) return rc;
  if( bOk==0 ) return CAPDB_NOTFOUND;
...
static int whereInScanEst(
  ...
  Index *p = pBuilder->pNew->u.btree.pIndex;
  i64 nRow0 = capdbLogEstToInt(p->aiRowLogEst[0]);
  int nRecValid = pBuilder->nRecValid;
  int rc = CAPDB_OK;     /* Subfunction return code */
  tRowcnt nEst;           /* Number of rows for a single term */
  tRowcnt nRowEst = 0;    /* New estimate of the number of rows */
  int i;                  /* Loop counter */

  assert( p->aSample!=0 );
  for(i=0; rc==CAPDB_OK && i<pList->nExpr; i++){
    nEst = nRow0;
...
#if defined(WHERETRACE_ENABLED) || defined(CAPDB_DEBUG)
/*
** Print the content of a WhereTerm object
*/
void capdbWhereTermPrint(WhereTerm *pTerm, int iTerm){
  if( pTerm==0 ){
    capdbDebugPrintf("TERM-%-3d NULL\n", iTerm);
  }else{
    char zType[8];
    char zLeft[50];
    memcpy(zType, "....", 5);
    ...
    if( pTerm->eOperator & WO_SINGLE ){
      assert( (pTerm->eOperator & (WO_OR|WO_AND))==0 );
      capdb_snprintf(sizeof(zLeft),zLeft,"left={%d:%d}",
                       pTerm->leftCursor, pTerm->u.x.leftColumn);
    }else if( (pTerm->eOperator & WO_OR)!=0 && pTerm->u.pOrInfo!=0 ){
      capdb_snprintf(sizeof(zLeft),zLeft,"indexable=0x%llx",
                       pTerm->u.pOrInfo->indexable);
    }else{
      capdb_snprintf(sizeof(zLeft),zLeft,"left=%d", pTerm->leftCursor);
    }
    ...
    ** shown about each Term */
    if( capdbWhereTrace & 0x10000 ){
      capdbDebugPrintf(" prob=%-3d prereq=%llx,%llx",
        pTerm->truthProb, (u64)pTerm->prereqAll, (u64)pTerm->prereqRight);
    }
    if( (pTerm->eOperator & (WO_OR|WO_AND))==0 && pTerm->u.x.iField ){
      capdbDebugPrintf(" iField=%d", pTerm->u.x.iField);
    }
    if( pTerm->iParent>=0 ){
      capdbDebugPrintf(" iParent=%d", pTerm->iParent);
    }
    capdbDebugPrintf("\n");
    capdbTreeViewExpr(0, pTerm->pExpr, 0);
  }
}
void capdbShowWhereTerm(WhereTerm *pTerm){
  capdbWhereTermPrint(pTerm, 0);
}
#endif
...
*/
void capdbWhereClausePrint(WhereClause *pWC){
  int i;
  for(i=0; i<pWC->nTerm; i++){
    capdbWhereTermPrint(&pWC->a[i], i);
  }
}
#endif
...
*/
void capdbWhereLoopPrint(const WhereLoop *p, const WhereClause *pWC){
  WhereInfo *pWInfo;
  if( pWC ){
    int nb;
    SrcItem *pItem;
    ...
    capdbDebugPrintf("%c%2d.%0*llx.%0*llx", p->cId,
                       p->iTab, nb, p->maskSelf, nb, p->prereq & mAll);
    capdbDebugPrintf(" %12s",
                       pItem->zAlias ? pItem->zAlias : pTab->zName);
  }else{
    pWInfo = 0;
    capdbDebugPrintf("%c%2d.%03llx.%03llx %c%d",
  ...
  if( (p->wsFlags & WHERE_VIRTUALTABLE)==0 ){
    ...
    if( p->u.btree.pIndex && (zName = p->u.btree.pIndex->zName)!=0 ){
      if( strncmp(zName, "sqlite_autoindex_", 17)==0 ){
        int i = capdbStrlen30(zName) - 1;
        while( zName[i]!='_' ) i--;
        zName += i;
      }
      capdbDebugPrintf(".%-16s %2d", zName, p->u.btree.nEq);
    }else{
      capdbDebugPrintf("%20s","");
    }
  }else{
    char *z;
    if( p->u.vtab.idxStr ){
      z = capdb_mprintf("(%d,\"%s\",%#x)",
                p->u.vtab.idxNum, p->u.vtab.idxStr, p->u.vtab.omitMask);
    }else{
      z = capdb_mprintf("(%d,%x)", p->u.vtab.idxNum, p->u.vtab.omitMask);
    }
    capdbDebugPrintf(" %-19s", z);
    capdb_free(z);
  }
  if( p->wsFlags & WHERE_SKIPSCAN ){
    capdbDebugPrintf(" f %06x %d-%d", p->wsFlags, p->nLTerm,p->nSkip);
  }else{
    capdbDebugPrintf(" f %06x N %d", p->wsFlags, p->nLTerm);
  }
  if( pWInfo && pWInfo->bStarUsed && p->rStarDelta!=0 ){
    capdbDebugPrintf(" cost %d,%d,%d delta=%d\n",
                       p->rSetup, p->rRun, p->nOut, p->rStarDelta);
  }else{
    capdbDebugPrintf(" cost %d,%d,%d\n", p->rSetup, p->rRun, p->nOut);
  }
  if( p->nLTerm && (capdbWhereTrace & 0x4000)!=0 ){
    int i;
    for(i=0; i<p->nLTerm; i++){
      capdbWhereTermPrint(p->aLTerm[i], i);
    }
  }
}
void capdbShowWhereLoop(const WhereLoop *p){
  if( p ) capdbWhereLoopPrint(p, 0);
}
void capdbShowWhereLoopList(const WhereLoop *p){
  while( p ){
    capdbShowWhereLoop(p);
    p = p->pNextLoop;
  }
}
#endif
...
*/
static void whereLoopClearUnion(capdb *db, WhereLoop *p){
  if( p->wsFlags & (WHERE_VIRTUALTABLE|WHERE_AUTO_INDEX) ){
    if( (p->wsFlags & WHERE_VIRTUALTABLE)!=0 && p->u.vtab.needFree ){
      capdb_free(p->u.vtab.idxStr);
      p->u.vtab.needFree = 0;
      p->u.vtab.idxStr = 0;
    }else if( (p->wsFlags & WHERE_AUTO_INDEX)!=0 && p->u.btree.pIndex!=0 ){
      capdbDbFree(db, p->u.btree.pIndex->zColAff);
      capdbDbFreeNN(db, p->u.btree.pIndex);
      p->u.btree.pIndex = 0;
    }
  }
}
...
*/
static void whereLoopClear(capdb *db, WhereLoop *p){
  if( p->aLTerm!=p->aLTermSpace ){
    capdbDbFreeNN(db, p->aLTerm);
    p->aLTerm = p->aLTermSpace;
    p->nLSlot = ArraySize(p->aLTermSpace);
  }
...
*/
static int whereLoopResize(capdb *db, WhereLoop *p, int n){
  WhereTerm **paNew;
  if( p->nLSlot>=n ) return CAPDB_OK;
  n = (n+7)&~7;
  paNew = capdbDbMallocRawNN(db, sizeof(p->aLTerm[0])*n);
  if( paNew==0 ) return CAPDB_NOMEM_BKPT;
  memcpy(paNew, p->aLTerm, sizeof(p->aLTerm[0])*p->nLSlot);
  if( p->aLTerm!=p->aLTermSpace ) capdbDbFreeNN(db, p->aLTerm);
  p->aLTerm = paNew;
  p->nLSlot = n;
  return CAPDB_OK;
}
...
*/
static int whereLoopXfer(capdb *db, WhereLoop *pTo, WhereLoop *pFrom){
...
*/
static void whereLoopDelete(capdb *db, WhereLoop *p){
  assert( db!=0 );
  whereLoopClear(db, p);
...
*/
static void whereInfoFree(capdb *db, WhereInfo *pWInfo){
  assert( pWInfo!=0 );
  assert( db!=0 );
  capdbWhereClauseClear(&pWInfo->sWC);
  while( pWInfo->pLoops ){
  ...
  while( pWInfo->pMemToFree ){
    ...
    capdbDbNNFreeNN(db, pWInfo->pMemToFree);
    pWInfo->pMemToFree = pNext;
  }
  capdbDbNNFreeNN(db, pWInfo);
}
...
static int whereLoopInsert(WhereLoopBuilder *pBuilder, WhereLoop *pTemplate){
  ...
  */
  if( pBuilder->pOrSet!=0 ){
    if( pTemplate->nLTerm ){
      ...
      if( capdbWhereTrace & 0x8 ){
        capdbDebugPrintf(x?"   or-%d:  ":"   or-X:  ", n);
        capdbWhereLoopPrint(pTemplate, pBuilder->pWC);
      }
#endif
    }
    return CAPDB_OK;
  ...
  */
  ...
  if( capdbWhereTrace & 0x8 ){
    if( p!=0 ){
      capdbDebugPrintf("replace: ");
      capdbWhereLoopPrint(p, pBuilder->pWC);
      capdbDebugPrintf("   with: ");
    }else{
      capdbDebugPrintf("    add: ");
    }
    capdbWhereLoopPrint(pTemplate, pBuilder->pWC);
  }
#endif
  if( p==0 ){
    ...
    *ppPrev = p = capdbDbMallocRawNN(db, sizeof(WhereLoop));
...
#if WHERETRACE_ENABLED /* 0x8 */
      if( capdbWhereTrace & 0x8 ){
        capdbDebugPrintf(" delete: ");
        capdbWhereLoopPrint(pToDel, pBuilder->pWC);
      }
#endif
  ...
  if( (p->wsFlags & WHERE_VIRTUALTABLE)==0 ){
    ...
    if( pIndex && pIndex->idxType==CAPDB_IDXTYPE_IPK ){
      p->u.btree.pIndex = 0;
    }
  }
  return rc;
...
static void whereLoopOutputAdjust(
  ...
  for(i=pWC->nBase, pTerm=pWC->a; i>0; i--, pTerm++){
    ...
    if( j<0 ){
      capdbProgressCheck(pWC->pWInfo->pParse);
        ...
        ** guess a reasonable truth probability. */
        ...
        if( (pTerm->eOperator&(WO_EQ|WO_IS))!=0
         && (pTerm->wtFlags & TERM_HIGHTRUTH)==0  /* tag-20200224-1 */
          ...
          if( capdbExprIsInteger(pRight, &k, pParse) && k>=(-1) && k<=1 ){
            k = 10;
          }else{
            k = 20;
          }
        ...
        if( ExprHasProperty(pOpExpr, EP_InfixFunc) 
         && pOpExpr->op==TK_FUNCTION
          ...
          eOp = capdbExprIsLikeOperator(pOpExpr);
          if( ALWAYS(eOp>0) ){
            int szPattern;
            Expr *pRHS = pOpExpr->x.pList->a[0].pExpr;
            eOp = eOp==CAPDB_INDEX_CONSTRAINT_LIKE;
...
static int whereRangeVectorLen(
  ...
  int nCmp = capdbExprVectorSize(pTerm->pExpr->pLeft);
  int i;
  ...
  for(i=1; i<nCmp; i++){
    ...
    ** leftmost index column.  */
    ...
    aff = capdbCompareAffinity(pRhs, capdbExprAffinity(pLhs));
    idxaff = capdbTableColumnAffinity(pIdx->pTable, pLhs->iColumn);
    if( aff!=idxaff ) break;

    if( ExprHasProperty(pTerm->pExpr, EP_Commuted) ) SWAP(Expr*, pRhs, pLhs);
    pColl = capdbBinaryCompareCollSeq(pParse, pLhs, pRhs);
    if( pColl==0 ) break;
    if( capdbStrICmp(pColl->zName, pIdx->azColl[i+nEq]) ) break;
  }
  return i;
}
...
** compiled with -DCAPDB_ENABLE_COSTMULT
*/
...
** function returns.
**
** If pProbe->idxType==CAPDB_IDXTYPE_IPK, that means pIndex is
...
static int whereLoopAddBtreeIndex(
  ...
  Parse *pParse = pWInfo->pParse;        /* Parsing context */
  capdb *db = pParse->db;       /* Database connection malloc context */
  ...
  assert( pNew->u.btree.nEq<pProbe->nKeyCol
       || pProbe->idxType!=CAPDB_IDXTYPE_PRIMARYKEY );
  ...
  for(; rc==CAPDB_OK && pTerm!=0; pTerm = whereScanNext(&scan)){
    ...
    ** to mix with a lower range bound from some other source */
    ...
    if( IsUniqueIndex(pProbe) && saved_nEq==pProbe->nKeyCol-1 ){
      pBuilder->bldFlags1 |= CAPDB_BLDF1_UNIQUE;
    }else{
      pBuilder->bldFlags1 |= CAPDB_BLDF1_INDEXED;
    }
    ...
    if( eOp & WO_IN ){
      ...
      if( ExprUseXSelect(pExpr) ){
        ...
        int i;
        int bRedundant = 0;
        nIn = 46;  assert( 46==capdbLogEst(25) );
        ...
        ** first such term in use, and sets nIn back to 0 if it is not. */
        ...
        /* "x IN (value, value, ...)" */
        nIn = capdbLogEst(pExpr->x.pList->nExpr);
      }
      if( pProbe->hasStat1 && rLogSize>=10 ){
        ...
        */
        ...
        }else if( nInMul<2 && OptimizationEnabled(db, CAPDB_SeekScan) ){
          WHERETRACE(0x40,
            ("IN operator (N=%d M=%d logK=%d nIn=%d rLogSize=%d x=%d"
      ...
      ** data, using some other estimate.  */
      ...
      if( pTerm->truthProb<=0 && pProbe->aiColumn[saved_nEq]>=0 ){
        ...
        if( nInMul==0
         ...
         && OptimizationEnabled(db, CAPDB_Stat4)
        ){
          Expr *pExpr = pTerm->pExpr;
          ...
          if( rc!=CAPDB_OK ) break;          /* Jump out of the pTerm loop */
          if( nOut ){
            pNew->nOut = capdbLogEst(nOut);
            if( nEq==1
             ...
             ** See tag-202002240-1 */
             && pNew->nOut+10 > pProbe->aiRowLogEst[0]
              ...
              if( capdbWhereTrace & 0x20 ){
                capdbDebugPrintf(
                   "STAT4 determines term has low selectivity:\n");
                capdbWhereTermPrint(pTerm, 999);
              }
#endif
              ...
              if( pTerm->wtFlags & TERM_HEURTRUTH ){
                ...
                ** loop computations. */
                pBuilder->bldFlags2 |= CAPDB_BLDF2_2NDPASS;
              }
            }
    ...
    */
    ...
    if( pProbe->idxType==CAPDB_IDXTYPE_IPK ){
    ...
    rCostIdx = capdbLogEstAdd(rLogSize, rCostIdx);
    ...
    */
    ...
    if( (pNew->wsFlags & (WHERE_IDX_ONLY|WHERE_IPK|WHERE_EXPRIDX))==0 ){
      pNew->rRun = capdbLogEstAdd(pNew->rRun, pNew->nOut + 16);
    }
  ...
  ** more expensive.  */
  assert( 42==capdbLogEst(18) );
  if( saved_nEq==saved_nSkip
   && saved_nEq+1<pProbe->nKeyCol
   && saved_nEq==pNew->nLTerm
   && pProbe->noSkipScan==0
   && pProbe->hasStat1!=0
   && OptimizationEnabled(db, CAPDB_SkipScan)
   && pProbe->aiRowLogEst[saved_nEq+1]>=42  /* TUNING: Minimum for skip-scan */
   && pSrc->fg.fromExists==0
   && (rc = whereLoopResize(db, pNew, pNew->nLTerm+1))==CAPDB_OK
  ){
    LogEst nIter;
    pNew->u.btree.nEq++;
    pNew->nSkip++;
...
static int indexMightHelpWithOrderBy(
  ...
  for(ii=0; ii<pOB->nExpr; ii++){
    Expr *pExpr = capdbExprSkipCollateAndLikely(pOB->a[ii].pExpr);
    ...
    if( (pExpr->op==TK_COLUMN || pExpr->op==TK_AGG_COLUMN) 
     && pExpr->iTable==iCursor 
      ...
      for(jj=0; jj<pIndex->nKeyCol; jj++){
        ...
        if( capdbExprCompareSkip(pExpr,aColExpr->a[jj].pExpr,iCursor)==0 ){
          return 1;
        }
      }
    }
...
static int whereUsablePartialIndex(
  ...
  for(i=0, pTerm=pWC->a; i<pWC->nTerm; i++, pTerm++){
    ...
    if( (!ExprHasProperty(pExpr, EP_OuterON) || pExpr->w.iJoin==iTab)
     ...
     && capdbExprImpliesExpr(pParse, pExpr, pWhere, iTab)
     && !capdbExprImpliesExpr(pParse, pExpr, pWhere, -1)
...
static int exprIsCoveredByIndex(
  ...
  for(i=0; i<pIdx->nColumn; i++){
    if( pIdx->aiColumn[i]==XN_EXPR
     && capdbExprCompare(0, pExpr, pIdx->aColExpr->a[i].pExpr, iTabCur)==0
    ){
      return 1;
    }
  }
...
*/
static CAPDB_NOINLINE u32 whereIsCoveringIndex(
  ...
  w.u.pCovIdxCk = &ck;
  capdbWalkSelect(&w, pWInfo->pSelect);
  if( ck.bUnidx ){
    rc = 0;
  }else if( ck.bExpr ){
    rc = WHERE_EXPRIDX;
...
/*
** This is an capdbParserAddCleanup() callback that is invoked to
** free the Parse->pIdxEpr list when the Parse object is destroyed.
*/
static void whereIndexedExprCleanup(capdb *db, void *pObject){
  IndexedExpr **pp = (IndexedExpr**)pObject;
  while( *pp!=0 ){
    IndexedExpr *p = *pp;
    *pp = p->pIENext;
    capdbExprDelete(db, p->pExpr);
    capdbDbFreeNN(db, p);
  }
}
...
static void wherePartIdxExpr(
  ...
  if( (pPart->op==TK_EQ || pPart->op==TK_IS) ){
    ...
    if( !capdbExprIsConstant(0, pRight) ) return;
    if( !capdbIsBinary(capdbExprCompareCollSeq(pParse, pPart)) ) return;
    if( pLeft->iColumn<0 ) return;
    aff = pIdx->pTable->aCol[pLeft->iColumn].affinity;
    if( aff>=CAPDB_AFF_TEXT ){
      if( pItem ){
        capdb *db = pParse->db;
        IndexedExpr *p = (IndexedExpr*)capdbDbMallocRaw(db, sizeof(*p));
        if( p ){
          int bNullRow = (pItem->fg.jointype&(JT_LEFT|JT_LTORJ))!=0;
          p->pExpr = capdbExprDup(db, pRight, 0);
          ...
          if( p->pIENext==0 ){
            ...
            capdbParserAddCleanup(pParse, whereIndexedExprCleanup, pArg);
          }
        }
...
static int whereLoopAddBtree(
  ...
  if( !pBuilder->pOrSet      /* Not part of an OR optimization */
   ...
   && (pWInfo->pParse->db->flags & CAPDB_AutoIndex)!=0
   ...
   && (pSrc->fg.jointype & JT_RIGHT)==0 /* Not the right tab of a RIGHT JOIN */
    ...
    rLogSize = estLog(rSize);
    for(pTerm=pWC->a; rc==CAPDB_OK && pTerm<pWCEnd; pTerm++){
      if( pTerm->prereqRight & pNew->maskSelf ) continue;
      if( termCanDriveIndex(pTerm, pSrc, 0) ){
        ...
        ** not be unreasonable to make this value much larger. */
        pNew->nOut = 43;  assert( 43==capdbLogEst(20) );
        pNew->rRun = capdbLogEstAdd(rLogSize,pNew->nOut);
  ...
  ** consider index pProbe.  */
  for(; rc==CAPDB_OK && pProbe;
      ...
      */
      ...
      if( pProbe->isCovering ){
        ...
        if( m==TOPBIT || (pProbe->bHasExpr && !pProbe->bHasVCol && m!=0) ){
          ...
          if( isCov==0 ){
           ...
           && (HasRowid(pTab) || pWInfo->pSelect!=0 || capdbFaultSim(700))
        ){
          WHERETRACE(0x200,
      ...
      if( b
       ...
       || ( m==0
         ...
         && capdbGlobalConfig.bUseCis
         && OptimizationEnabled(pWInfo->pParse->db, CAPDB_CoverIdxScan)
          )
      ){
        pNew->iSortIdx = b ? iSortIdx : 0;
        ...
        if( m!=0 ){
          ...
          ** table lookup. */
          ...
          for(ii=0; ii<pWC2->nTerm; ii++){
            ...
            if( !capdbExprCoveredByIndex(pTerm->pExpr, iCur, pProbe) ){
              break;
            }
          ...
          pNew->rRun = capdbLogEstAdd(pNew->rRun, nLookup);
        }
        ApplyCostMultiplier(pNew->rRun, pTab->costMult);
        whereLoopOutputAdjust(pWC, pNew, rSize);
    ...
    if( pBuilder->bldFlags1==CAPDB_BLDF1_INDEXED ){
...
#ifdef CAPDB_ENABLE_STAT4
    capdbStat4ProbeFree(pBuilder->pRec);
...
static int isLimitTerm(WhereTerm *pTerm){
  ...
  return pTerm->eMatchOp>=CAPDB_INDEX_CONSTRAINT_LIMIT
      && pTerm->eMatchOp<=CAPDB_INDEX_CONSTRAINT_OFFSET;
}
...
static int allConstraintsUsed(
  struct capdb_index_constraint_usage *aUsage, 
  int nCons
){
  int ii;
...
static int whereLoopAddVirtualOne(
  ...
  capdb_index_info *pIdxInfo,   /* Populated object for xBestIndex */
  ...
  struct capdb_index_constraint *pIdxCons;
  struct capdb_index_constraint_usage *pUsage = pIdxInfo->aConstraintUsage;
  int i;
  int mxTerm;
  int rc = CAPDB_OK;
  ...
  ** arguments mUsable and mExclude. */
  pIdxCons = *(struct capdb_index_constraint**)&pIdxInfo->aConstraint;
  ...
  /* Initialize the output fields of the capdb_index_info structure */
  memset(pUsage, 0, sizeof(pUsage[0])*nConstraint);
  assert( pIdxInfo->needToFreeIdxStr==0 );
  pIdxInfo->idxStr = 0;
  pIdxInfo->idxNum = 0;
  pIdxInfo->orderByConsumed = 0;
  pIdxInfo->estimatedCost = CAPDB_BIG_DBL / (double)2;
  ...
  if( rc ){
    if( rc==CAPDB_CONSTRAINT ){
      /* If the xBestIndex method returns CAPDB_CONSTRAINT, that means
    ...
    if( (iTerm = pUsage[i].argvIndex - 1)>=0 ){
      ...
      if( iTerm>=nConstraint
       ...
       || pIdxCons->usable==0
        ...
        capdbErrorMsg(pParse,"%s.xBestIndex malfunction",pSrc->pSTab->zName);
        freeIdxStr(pIdxInfo);
        return CAPDB_ERROR;
      }
      ...
      if( pUsage[i].omit ){
        ...
        if( pTerm->eMatchOp==CAPDB_INDEX_CONSTRAINT_OFFSET ){
  ...
  pNew->u.vtab.isOrdered = (i8)(pIdxInfo->orderByConsumed ?
      pIdxInfo->nOrderBy : 0);
  pNew->u.vtab.bIdxNumHex = (pIdxInfo->idxFlags&CAPDB_INDEX_SCAN_HEX)!=0;
  pNew->rSetup = 0;
  pNew->rRun = capdbLogEstFromDouble(pIdxInfo->estimatedCost);
  pNew->nOut = capdbLogEst(pIdxInfo->estimatedRows);

  /* Set the WHERE_ONEROW flag if the xBestIndex() method indicated
  ** that the scan will visit at most one row. Clear it otherwise. */
  if( pIdxInfo->idxFlags & CAPDB_INDEX_SCAN_UNIQUE ){
    pNew->wsFlags |= WHERE_ONEROW;
  }else{
    pNew->wsFlags &= ~WHERE_ONEROW;
  }
  ...
  if( pNew->u.vtab.needFree ){
    capdb_free(pNew->u.vtab.idxStr);
    pNew->u.vtab.needFree = 0;
  }
  WHERETRACE(0xffffffff, ("  bIn=%d prereqIn=%04llx prereqOut=%04llx\n",
                      *pbIn, (capdb_uint64)mPrereq,
                      (capdb_uint64)(pNew->prereq & ~mPrereq)));
...
**
** pIdxInfo must be an capdb_index_info structure passed into xBestIndex.
** This routine depends on there being a HiddenIndexInfo structure immediately
** following the capdb_index_info structure.
**
...
*/
const char *capdb_vtab_collation(capdb_index_info *pIdxInfo, int iCons){
  HiddenIndexInfo *pHidden = (HiddenIndexInfo*)&pIdxInfo[1];
  const char *zRet = 0;
  if( iCons>=0 && iCons<pIdxInfo->nConstraint ){
    ...
    if( pX->pLeft ){
      pC = capdbExprCompareCollSeq(pHidden->pParse, pX);
    }
    zRet = (pC ? pC->zName : capdbStrBINARY);
  }
  return zRet;
}
...
*/
int capdb_vtab_in(capdb_index_info *pIdxInfo, int iCons, int bHandle){
  HiddenIndexInfo *pHidden = (HiddenIndexInfo*)&pIdxInfo[1];
...
*/
int capdb_vtab_rhs_value(
  capdb_index_info *pIdxInfo,   /* Copy of first argument to xBestIndex */
  int iCons,                      /* Constraint for which RHS is wanted */
  capdb_value **ppVal           /* Write value extracted here */
){
  HiddenIndexInfo *pH = (HiddenIndexInfo*)&pIdxInfo[1];
  capdb_value *pVal = 0;
  int rc = CAPDB_OK;
  if( iCons<0 || iCons>=pIdxInfo->nConstraint ){
    rc = CAPDB_MISUSE_BKPT; /* EV: R-30545-25046 */
  }else{
    if( pH->aRhs[iCons]==0 ){
      WhereTerm *pTerm = termFromWhereClause(
          pH->pWC, pIdxInfo->aConstraint[iCons].iTermOffset
      );
      rc = capdbValueFromExpr(
          pH->pParse->db, pTerm->pExpr->pRight, ENC(pH->pParse->db),
          CAPDB_AFF_BLOB, &pH->aRhs[iCons]
      );
  ...
  *ppVal = pVal;
  ...
  if( rc==CAPDB_OK && pVal==0 ){  /* IMP: R-19933-32160 */
    rc = CAPDB_NOTFOUND;          /* IMP: R-36424-56542 */
  }
...
*/
int capdb_vtab_distinct(capdb_index_info *pIdxInfo){
  HiddenIndexInfo *pHidden = (HiddenIndexInfo*)&pIdxInfo[1];
...
*/
void capdbVtabUsesAllSchemas(Parse *pParse){
  ...
  if( DbMaskNonZero(pParse->writeMask) ){
    for(i=0; i<nDb; i++){
      capdbBeginWriteOperation(pParse, 0, i);
    }
  }
}
...
static int whereLoopAddVirtual(
  ...
  capdb_index_info *p;       /* Object to pass to xBestIndex() */
  ...
  ** result (if the xBestIndex() implementation is sane). */
  if( rc==CAPDB_OK && ((mBest = (pNew->prereq & ~mPrereq))!=0 || bIn) ){
    ...
    ** in the set of terms that apply to the current virtual table.  */
    while( rc==CAPDB_OK ){
      ...
      WHERETRACE(0x800, ("  VirtualOne: mPrev=%04llx mNext=%04llx\n",
                       (capdb_uint64)mPrev, (capdb_uint64)mNext));
      rc = whereLoopAddVirtualOne(
...
static int whereLoopAddOr(
  ...
  if( pItem->fg.jointype & JT_RIGHT ) return CAPDB_OK;

  for(pTerm=pWC->a; pTerm<pWCEnd && rc==CAPDB_OK; pTerm++){
    if( (pTerm->eOperator & WO_OR)!=0
     && (pTerm->u.pOrInfo->indexable & pNew->maskSelf)!=0
      ...
      for(pOrTerm=pOrWC->a; pOrTerm<pOrWCEnd; pOrTerm++){
        ...
        if( capdbWhereTrace & 0x20000 ){
          capdbWhereClausePrint(sSubBuild.pWC);
        }
#endif
#ifndef CAPDB_OMIT_VIRTUALTABLE
        ...
        testcase( rc==CAPDB_NOMEM && sCur.n>0 );
        testcase( rc==CAPDB_DONE );
        if( sCur.n==0 ){
          sSum.n = 0;
          break;
        }else if( once ){
          ...
          for(i=0; i<sPrev.n; i++){
            for(j=0; j<sCur.n; j++){
              whereOrInsert(&sSum, sPrev.a[i].prereq | sCur.a[j].prereq,
                            capdbLogEstAdd(sPrev.a[i].rRun, sCur.a[j].rRun),
                            capdbLogEstAdd(sPrev.a[i].nOut, sCur.a[j].nOut));
            }
          }
        }
      }
      ...
      for(i=0; rc==CAPDB_OK && i<sSum.n; i++){
...
static int whereLoopAddAll(WhereLoopBuilder *pBuilder){
  ...
  pBuilder->iPlanLimit = CAPDB_QUERY_PLANNER_LIMIT;
  for(iTab=0, pItem=pTabList->a; pItem<pEnd; iTab++, pItem++){
    Bitmask mUnusable = 0;
    pNew->iTab = iTab;
    pBuilder->iPlanLimit += CAPDB_QUERY_PLANNER_LIMIT_INCR;
    pNew->maskSelf = capdbWhereGetMask(&pWInfo->sMaskSet, pItem->iCursor);
    if( bFirstPastRJ
    ...
    if( IsVirtual(pItem->pSTab) ){
      ...
      for(p=&pItem[1]; p<pEnd; p++){
        if( mUnusable || (p->fg.jointype & (JT_OUTER|JT_CROSS)) ){
          mUnusable |= capdbWhereGetMask(&pWInfo->sMaskSet, p->iCursor);
        }
      }
    ...
    if( rc==CAPDB_OK && pBuilder->pWC->hasOr ){
    ...
    if( rc || db->mallocFailed ){
      if( rc==CAPDB_DONE ){
        ...
        capdb_log(CAPDB_WARNING, "abbreviated query algorithm search");
        rc = CAPDB_OK;
      }else{
        break;
      }
...
*/
static CAPDB_NOINLINE int wherePathMatchSubqueryOB(
  WhereInfo *pWInfo,      /* The WHERE clause */
...
static i8 wherePathSatisfiesOrderBy(
  ...
  capdb *db = pWInfo->pParse->db;  /* Database connection */
  ...
  */
  ...
  assert( pOrderBy!=0 );
  if( nLoop && OptimizationDisabled(db, CAPDB_OrderByIdxJoin) ) return 0;
  ...
  for(iLoop=0; isOrderDistinct && obSat<obDone && iLoop<=nLoop; iLoop++){
    ...
    */
    for(i=0; i<nOrderBy; i++){
      ...
      pOBExpr = capdbExprSkipCollateAndLikely(pOrderBy->a[i].pExpr);
      if( NEVER(pOBExpr==0) ) continue;
      if( pOBExpr->op!=TK_COLUMN && pOBExpr->op!=TK_AGG_COLUMN ) continue;
      if( pOBExpr->iTable!=iCur ) continue;
      pTerm = capdbWhereFindTerm(&pWInfo->sWC, iCur, pOBExpr->iColumn,
                       ~ready, eqOpMask, 0);
      if( pTerm==0 ) continue;
      if( pTerm->eOperator==WO_IN ){
      ...
      if( (pTerm->eOperator&(WO_EQ|WO_IS))!=0 && pOBExpr->iColumn>=0 ){
        Parse *pParse = pWInfo->pParse;
        CollSeq *pColl1 = capdbExprNNCollSeq(pParse, pOrderBy->a[i].pExpr);
        CollSeq *pColl2 = capdbExprCompareCollSeq(pParse, pTerm->pExpr);
        assert( pColl1 );
        if( pColl2==0 || capdbStrICmp(pColl1->zName, pColl2->zName) ){
          continue;
        }
        testcase( pTerm->pExpr->op==TK_IS );
      }
    ...
    if( (pLoop->wsFlags & WHERE_ONEROW)==0 ){
      if( pLoop->wsFlags & WHERE_IPK ){
        if( pLoop->u.btree.pOrderBy
         && OptimizationEnabled(db, CAPDB_OrderBySubq)
        ...
        */
        ...
        for(i=0; bOnce && i<nOrderBy; i++){
          ...
          if( iColumn>=XN_ROWID ){
            ...
            if( capdbExprCompareSkip(pOBExpr, pIxExpr, iCur) ){
              continue;
            }
          }
          if( iColumn!=XN_ROWID ){
            pColl = capdbExprNNCollSeq(pWInfo->pParse, pOrderBy->a[i].pExpr);
            if( capdbStrICmp(pColl->zName, pIndex->azColl[j])!=0 ) continue;
          }
    ...
    if( isOrderDistinct ){
      ...
      for(i=0; i<nOrderBy; i++){
        ...
        mTerm = capdbWhereExprUsage(&pWInfo->sMaskSet,p);
        if( mTerm==0 && !capdbExprIsConstant(0,p) ) continue;
...
/*
** If the WHERE_GROUPBY flag is set in the mask passed to capdbWhereBegin(),
...
** if the WHERE_SORTBYGROUP flag is also passed to capdbWhereBegin(), then
...
*/
int capdbWhereIsSorted(WhereInfo *pWInfo){
...
static LogEst whereSortingCost(
  ...
  */
  ...
  nCol = capdbLogEst((pWInfo->pSelect->pEList->nExpr+59)/30);
  rSortCost = nRow + nCol;
  if( nSorted>0 ){
    /* Scale the result by (Y/X) */
    rSortCost += capdbLogEst((nOrderBy-nSorted)*100/nOrderBy) - 66;
  }
  ...
  */
  if( (pWInfo->wctrlFlags & WHERE_USE_LIMIT)!=0 ){
    ...
    ** reduces the number of output rows by a factor of 2 */
    if( nRow>10 ){ nRow -= 10;  assert( 10==capdbLogEst(2) ); }
  }
...
** considered a star-query, using CAPDB_TESTCTRL_OPTIMIZATION to
** disable the CAPDB_StarQuery optimization.  In the CLI, the command
...
static int computeMxChoice(WhereInfo *pWInfo){
  ...
  */
  ...
  if( nLoop>=4  /* Constraint (aa) */
   && !pWInfo->bStarDone
   && OptimizationEnabled(pWInfo->pParse->db, CAPDB_StarQuery)
  ){
    ...
    */
    ...
    for(iFromIdx=0, m=1; iFromIdx<nLoop; iFromIdx++, m<<=1){
      ...
      ** to make cost adjustments. */
      ...
      if( capdbWhereTrace & 0x80000 ){
        Bitmask mShow = mSeen;
        capdbDebugPrintf("Fact table %s(%d), dimensions:",
...
#ifdef WHERETRACE_ENABLED /* 0x80000 */
    if( (capdbWhereTrace & 0x80000)!=0 && pWInfo->bStarUsed ){
      capdbDebugPrintf("WhereLoops changed by star-query heuristic:\n");
      for(pWLoop=pWInfo->pLoops; pWLoop; pWLoop=pWLoop->pNextLoop){
        if( pWLoop->rStarDelta ){
          capdbWhereLoopPrint(pWLoop, &pWInfo->sWC);
        }
      }
    }
#endif
...
*/
static CAPDB_NOINLINE int whereLoopIsNoBetter(
...
** costs if nRowEst==0.
**
** Return CAPDB_OK on success or CAPDB_NOMEM of a memory allocation
** error occurs.
*/
static int wherePathSolver(WhereInfo *pWInfo, LogEst nRowEst){
  ...
  ** nRowEst parameter.  */
  ...
  pSpace = capdbStackAllocRawNN(pParse->db, nSpace);
  if( pSpace==0 ) return CAPDB_NOMEM_BKPT;
  ...
  ** rows, then do not use the automatic index. */
  aFrom[0].nRow = MIN(pParse->nQueryLoop, 48);  assert( 48==capdbLogEst(28) );
  nFrom = 1;
  ...
  ** best paths at each generation */
  for(iLoop=0; iLoop<nLoop; iLoop++){
    ...
    for(ii=0, pFrom=aFrom; ii<nFrom; ii++, pFrom++){
      for(pWLoop=pWInfo->pLoops; pWLoop; pWLoop=pWLoop->pNextLoop){
        ...
        ** Compute its cost */
        ...
        if( pWLoop->rSetup ){
          rUnsort = capdbLogEstAdd(pWLoop->rSetup, rUnsort);
        }
        rUnsort = capdbLogEstAdd(rUnsort, pFrom->rUnsort);
        ...
        if( isOrdered>=0 && isOrdered<nOrderBy ){
          ...
          ** required. */
          rCost = capdbLogEstAdd(rUnsort, aSortCost[isOrdered]) + 3;
        ...
        if( jj>=nTo ){
          ...
          if( nTo>=mxChoice
            ...
            ** this candidate as not viable. */
            ...
            if( capdbWhereTrace&0x4 ){
              capdbDebugPrintf("Skip   %s cost=%-3d,%3d,%3d order=%c\n",
...
#ifdef WHERETRACE_ENABLED /* 0x4 */
          if( capdbWhereTrace&0x4 ){
            capdbDebugPrintf("New    %s cost=%-3d,%3d,%3d order=%c\n",
                wherePathName(pFrom, iLoop, pWLoop), rCost, nOut, rUnsort,
                isOrdered>=0 ? isOrdered+'0' : '?');
          }
#endif
           ...
           || (pTo->rCost==rCost && pTo->nRow==nOut && pTo->rUnsort==rUnsort
            ...
            if( capdbWhereTrace&0x4 ){
              capdbDebugPrintf(
                  ...
                  wherePathName(pFrom, iLoop, pWLoop), rCost, nOut, rUnsort,
                  isOrdered>=0 ? isOrdered+'0' : '?');
              capdbDebugPrintf("   vs %s cost=%-3d,%3d,%3d order=%c\n",
          ...
          ** pTo path.  Replace pTo with the candidate. */
          ...
          if( capdbWhereTrace&0x4 ){
            capdbDebugPrintf(
                ...
                wherePathName(pFrom, iLoop, pWLoop), rCost, nOut, rUnsort,
                isOrdered>=0 ? isOrdered+'0' : '?');
            capdbDebugPrintf("  was %s cost=%-3d,%3d,%3d order=%c\n",
                wherePathName(pTo, iLoop+1, 0), pTo->rCost, pTo->nRow,
                pTo->rUnsort, pTo->isOrdered>=0 ? pTo->isOrdered+'0' : '?');
          }
#endif
    ...
    if( capdbWhereTrace & 0x02 ){
      LogEst rMin, rFloor = 0;
      int nDone = 0;
      int nProgress;
      capdbDebugPrintf("---- after round %d ----\n", iLoop);
      do{
        nProgress = 0;
        rMin = 0x7fff;
        ...
        for(ii=0, pTo=aTo; ii<nTo; ii++, pTo++){
          if( pTo->rCost==rMin ){
            capdbDebugPrintf(" %s cost=%-3d nrow=%-3d order=%c",
               wherePathName(pTo, iLoop+1, 0), pTo->rCost, pTo->nRow,
               pTo->isOrdered>=0 ? (pTo->isOrdered+'0') : '?');
            if( pTo->isOrdered>0 ){
              capdbDebugPrintf(" rev=0x%llx\n", pTo->revLoop);
            }else{
              capdbDebugPrintf("\n");
            }
            nDone++;
  ...
  if( nFrom==0 ){
    capdbErrorMsg(pParse, "no query solution");
    capdbStackFreeNN(pParse->db, pSpace);
    return CAPDB_ERROR;
  }
...
*/
static CAPDB_NOINLINE void whereInterstageHeuristic(WhereInfo *pWInfo){
  int i;
#ifdef WHERETRACE_ENABLED
  int once = 0;
#endif
...
#ifdef WHERETRACE_ENABLED
        if( capdbWhereTrace & 0x80 ){
          if( once==0 ){
            capdbDebugPrintf("Loops disabled by interstage heuristic:\n");
            once = 1;
          }
          capdbWhereLoopPrint(pLoop, &pWInfo->sWC);
        }
#endif /* WHERETRACE_ENABLED */
...
** general-purpose query planner, and thereby yield faster capdb_prepare()
...
static int whereShortCut(WhereLoopBuilder *pBuilder){
  ...
  if( pTerm ){
    ...
    pLoop->rRun = 33;  /* 33==capdbLogEst(10) */
  }else{
    for(pIdx=pTab->pIndex; pIdx; pIdx=pIdx->pNext){
      int opMask;
      ...
      pLoop->rRun = 39;  /* 39==capdbLogEst(15) */
      break;
    }
  }
  if( pLoop->wsFlags ){
    ...
    pLoop->maskSelf = 1; /* capdbWhereGetMask(&pWInfo->sMaskSet, iCur); */
...
#ifdef WHERETRACE_ENABLED
    if( capdbWhereTrace & 0x02 ){
      capdbDebugPrintf("whereShortCut() used to compute solution\n");
    }
#endif
    return 1;
  }
...
static void showAllWhereLoops(WhereInfo *pWInfo, WhereClause *pWC){
  if( capdbWhereTrace ){    /* Display all of the WhereLoop objects */
    WhereLoop *p;
    int i;
...
**       LEFT JOIN t3 ON (t1.ipk=t3.ipk)
*/
static CAPDB_NOINLINE Bitmask whereOmitNoopJoin(
  WhereInfo *pWInfo,
  Bitmask notReady
){
  int i;
  ...
  assert( OptimizationEnabled(pWInfo->pParse->db, CAPDB_OmitNoopJoin) );
  ...
  ** condition (1) of the header comment */
  ...
  tabUsed = capdbWhereExprListUsage(&pWInfo->sMaskSet, pWInfo->pResultSet);
  if( pWInfo->pOrderBy ){
    tabUsed |= capdbWhereExprListUsage(&pWInfo->sMaskSet, pWInfo->pOrderBy);
  }
...
*/
static CAPDB_NOINLINE void whereCheckIfBloomFilterIsUseful(
  ...
  assert( pWInfo->nLevel>=2 );
  assert( OptimizationEnabled(pWInfo->pParse->db, CAPDB_BloomFilter) );
  for(i=0; i<pWInfo->nLevel; i++){
     ...
     && ALWAYS((pLoop->wsFlags & (WHERE_IPK|WHERE_INDEXED))!=0)
      ...
      if( nSearch > pTab->nRowLogEst ){
        ...
        WHERETRACE(0xffffffff, (
           ...
           pLoop->cId, (double)capdbLogEstToInt(nSearch), pTab->zName,
           (double)capdbLogEstToInt(pTab->nRowLogEst)));
      }
    }
    nSearch += pLoop->nOut;
  }
...
*/
static CAPDB_NOINLINE void whereAddIndexedExpr(
  ...
  for(i=0; i<pIdx->nColumn; i++){
    ...
    if( j==XN_EXPR ){
      ...
      pExpr = capdbColumnExpr(pTab, &pTab->aCol[j]);
    }else{
      continue;
    }
    if( capdbExprIsConstant(0,pExpr) ) continue;
    p = capdbDbMallocRaw(pParse->db,  sizeof(IndexedExpr));
    if( p==0 ) break;
    p->pIENext = pParse->pIdxEpr;
#ifdef WHERETRACE_ENABLED
    if( capdbWhereTrace & 0x200 ){
      capdbDebugPrintf("New pParse->pIdxEpr term {%d,%d}\n", iIdxCur, i);
      if( capdbWhereTrace & 0x5000 ) capdbShowExpr(pExpr);
    }
#endif
    p->pExpr = capdbExprDup(pParse->db, pExpr, 0);
    p->iDataCur = pTabItem->iCursor;
    p->iIdxCur = iIdxCur;
    p->iIdxCol = i;
    p->bMaybeNullRow = (pTabItem->fg.jointype & (JT_LEFT|JT_LTORJ|JT_RIGHT))!=0;
    if( capdbIndexAffinityStr(pParse->db, pIdx) ){
...
** (Also CAPDB_DBCONFIG_REVERSE_SCANORDER).
*/
static CAPDB_NOINLINE void whereReverseScanOrder(WhereInfo *pWInfo){
  int ii;
...
** should invoke capdbWhereEnd() with the return value of this function
...
**        foreach row2 in t2 do      |-- by capdbWhereBegin()
**          foreach row3 in t3 do   /
**            ...
**          end                     \    Code generated
**        end                        |-- by capdbWhereEnd()
**      end                         /
**
...
** and capdbWhereEnd() generates the code to close them.
**
** The code that capdbWhereBegin() generates leaves the cursors named
...
WhereInfo *capdbWhereBegin(
  ...
  u16 wctrlFlags,         /* The WHERE_* flags defined in capdbInt.h */
  ...
  */
  ...
  if( pTabList->nSrc>BMS ){
    capdbErrorMsg(pParse, "at most %d tables in a join", BMS);
    return 0;
  }
  ...
  */
  ...
  pWInfo = capdbDbMallocRawNN(db, nByteWInfo + sizeof(WhereLoop));
  if( db->mallocFailed ){
    capdbDbFree(db, pWInfo);
    pWInfo = 0;
  ...
  pWInfo->iBreak = pWInfo->iContinue = capdbVdbeMakeLabel(pParse);
  ...
  pMaskSet->ix[0] = -99; /* Initialize ix[0] to a value that can never be
                         ...
                         ** test for pMaskSet->n==0 in capdbWhereGetMask() */
  sWLB.pWInfo = pWInfo;
  sWLB.pWC = &pWInfo->sWC;
  ...
  */
  capdbWhereClauseInit(&pWInfo->sWC, pWInfo);
  capdbWhereSplit(&pWInfo->sWC, pWhere, TK_AND);
  ...
  */
  if( nTabList==0 ){
    ...
    if( (wctrlFlags & WHERE_WANT_DISTINCT)!=0
     && OptimizationEnabled(db, CAPDB_DistinctOpt)
    ){
      pWInfo->eDistinct = WHERE_DISTINCT_UNIQUE;
    }
    ...
    */
    ...
    do{
      ...
      capdbWhereTabFuncArgs(pParse, &pTabList->a[ii], &pWInfo->sWC);
    }while( (++ii)<pTabList->nSrc );
  #ifdef CAPDB_DEBUG
    {
      Bitmask mx = 0;
      for(ii=0; ii<pTabList->nSrc; ii++){
        Bitmask m = capdbWhereGetMask(pMaskSet, pTabList->a[ii].iCursor);
        assert( m>=mx );
        mx = m;
      }
    }
  ...
  capdbWhereExprAnalyze(pTabList, &pWInfo->sWC);
  if( pSelect && pSelect->pLimit ){
    capdbWhereAddLimit(&pWInfo->sWC, pSelect);
  }
  ...
  */
  for(ii=0; ii<sWLB.pWC->nBase; ii++){
    ...
    if( pT->prereqAll==0                           /* Conditions (1) and (2) */
     ...
     && !(ExprHasProperty(pX, EP_InnerON)          /* Condition (3) */
      ...
      capdbExprIfFalse(pParse, pX, pWInfo->iBreak, CAPDB_JUMPIFNULL);
      pT->wtFlags |= TERM_CODED;
    }
  }

  if( wctrlFlags & WHERE_WANT_DISTINCT ){
    if( OptimizationDisabled(db, CAPDB_DistinctOpt) ){
      /* Disable the DISTINCT optimization if CAPDB_DistinctOpt is set via
      ** capdb_test_ctrl(CAPDB_TESTCTRL_OPTIMIZATIONS,...) */
      wctrlFlags &= ~WHERE_WANT_DISTINCT;
...
#if defined(WHERETRACE_ENABLED)
  if( capdbWhereTrace & 0xffffffff ){
    capdbDebugPrintf("*** Optimizer Start *** (wctrlFlags: 0x%x",wctrlFlags);
    if( wctrlFlags & WHERE_USE_LIMIT ){
      capdbDebugPrintf(", limit: %d", iAuxArg);
    }
    capdbDebugPrintf(")\n");
    if( capdbWhereTrace & 0x8000 ){
      Select sSelect;
      ...
      capdbTreeViewSelect(0, &sSelect, 0);
    }
    if( capdbWhereTrace & 0x4000 ){ /* Display all WHERE clause terms */
      capdbDebugPrintf("---- WHERE clause at start of analysis:\n");
      capdbWhereClausePrint(sWLB.pWC);
    }
  }
#endif
  ...
  if( nTabList!=1 || whereShortCut(&sWLB)==0 ){
    ...
    ** loops will be built using the revised truthProb values. */
    if( sWLB.bldFlags2 & CAPDB_BLDF2_2NDPASS ){
  ...
  if( pWInfo->pOrderBy==0 && (db->flags & CAPDB_ReverseOrder)!=0 ){
...
#ifdef WHERETRACE_ENABLED
  if( capdbWhereTrace ){
    capdbDebugPrintf("---- Solution cost=%d, nRow=%d",
                       pWInfo->rTotalCost, pWInfo->nRowOut);
    if( pWInfo->nOBSat>0 ){
      capdbDebugPrintf(" ORDERBY=%d,0x%llx", pWInfo->nOBSat, pWInfo->revMask);
    }
    switch( pWInfo->eDistinct ){
      case WHERE_DISTINCT_UNIQUE: {
        capdbDebugPrintf("  DISTINCT=unique");
        break;
      }
      case WHERE_DISTINCT_ORDERED: {
        capdbDebugPrintf("  DISTINCT=ordered");
        break;
      }
      case WHERE_DISTINCT_UNORDERED: {
        capdbDebugPrintf("  DISTINCT=unordered");
        break;
      }
    }
    capdbDebugPrintf("\n");
    for(ii=0; ii<pWInfo->nLevel; ii++){
      capdbWhereLoopPrint(pWInfo->a[ii].pWLoop, sWLB.pWC);
    }
  }
#endif
  ...
  ** procedure to keep the capdbWhereBegin() procedure from becoming
  ** too large.  If capdbWhereBegin() becomes too large, that prevents
  ** some C-compiler optimizers from in-lining the
  ** capdbWhereCodeOneLoopStart() procedure, and it is important to
  ** in-line capdbWhereCodeOneLoopStart() for performance reasons.
  */
  notReady = ~(Bitmask)0;
  if( pWInfo->nLevel>=2       /* Must be a join, or this opt8n is pointless */
   ...
   && OptimizationEnabled(db, CAPDB_OmitNoopJoin)                /* (7) */
  ){
  ...
  */
  if( pWInfo->nLevel>=2
   && OptimizationEnabled(db, CAPDB_BloomFilter)
  ){
    whereCheckIfBloomFilterIsUseful(pWInfo);
  }

#if defined(WHERETRACE_ENABLED)
  if( capdbWhereTrace & 0x4000 ){ /* Display all terms of the WHERE clause */
    capdbDebugPrintf("---- WHERE clause at end of analysis:\n");
    capdbWhereClausePrint(sWLB.pWC);
  }
  ...
  */
  ...
  if( (wctrlFlags & WHERE_ONEPASS_DESIRED)!=0 ){
    ...
    if( bOnerow || (
     ...
     && OptimizationEnabled(db, CAPDB_OnePass)
    )){
  ...
  */
  for(ii=0, pLevel=pWInfo->a; ii<nTabList; ii++, pLevel++){
    ...
    pLoop = pLevel->pWLoop;
    pLevel->addrBrk = capdbVdbeMakeLabel(pParse);
    ...
    if( (pLoop->wsFlags & WHERE_VIRTUALTABLE)!=0 ){
      const char *pVTab = (const char *)capdbGetVTable(db, pTab);
      int iCur = pTabItem->iCursor;
      capdbVdbeAddOp4(v, OP_VOpen, iCur, 0, 0, pVTab, P4_VTAB);
    ...
    if( ((pLoop->wsFlags & WHERE_IDX_ONLY)==0
     ...
     || (pTabItem->fg.jointype & (JT_LTORJ|JT_RIGHT))!=0
      ...
      capdbOpenTable(pParse, pTabItem->iCursor, iDb, pTab, op);
      ...
      if( pWInfo->eOnePass==ONEPASS_OFF
        ...
        ** the P4 operand of the OP_OpenRead/Write opcode. */
        ...
        int n = 0;
        for(; b; b=b>>1, n++){}
        capdbVdbeChangeP4(v, -1, CAPDB_INT_TO_PTR(n), P4_INT32);
        assert( n<=pTab->nCol );
      }
#ifdef CAPDB_ENABLE_CURSOR_HINTS
      if( pLoop->u.btree.pIndex!=0 && (pTab->tabFlags & TF_WithoutRowid)==0 ){
        capdbVdbeChangeP5(v, OPFLAG_SEEKEQ|bFordelete);
      }else
#endif
      {
        capdbVdbeChangeP5(v, bFordelete);
      }
#ifdef CAPDB_ENABLE_COLUMN_USED_MASK
      capdbVdbeAddOp4Dup8(v, OP_ColumnsUsed, pTabItem->iCursor, 0, 0,
      ...
      if( ii>=2
       ...
       && pLevel->addrHalt==pWInfo->a[0].addrHalt
        ...
        capdbVdbeAddOp2(v, OP_IfEmpty, pTabItem->iCursor, pWInfo->iBreak);
        VdbeCoverage(v);
      }
    }else{
    ...
    if( pLoop->wsFlags & WHERE_INDEXED ){
      ...
      if( !HasRowid(pTab) && IsPrimaryKeyIndex(pIx)
        ...
        ** WITHOUT ROWID table.  No need for a separate index */
        ...
        if( pIx->bHasExpr && OptimizationEnabled(db, CAPDB_IndexedExpr) ){
      ...
      assert( iIndexCur>=0 );
      if( op ){
        capdbVdbeAddOp3(v, op, iIndexCur, pIx->tnum, iDb);
        capdbVdbeSetP4KeyInfo(pParse, pIx);
        if( (pLoop->wsFlags & WHERE_CONSTRAINT)!=0
         ...
         && pWInfo->eDistinct!=WHERE_DISTINCT_ORDERED
          ...
          capdbVdbeChangeP5(v, OPFLAG_SEEKEQ);
        }
...
#ifdef CAPDB_ENABLE_COLUMN_USED_MASK
        {
          ...
          capdbVdbeAddOp4Dup8(v, OP_ColumnsUsed, iIndexCur, 0, 0,
                                (u8*)&colUsed, P4_INT64);
        }
#endif /* CAPDB_ENABLE_COLUMN_USED_MASK */
      }
    }
    if( iDb>=0 ) capdbCodeVerifySchema(pParse, iDb);
    if( (pTabItem->fg.jointype & JT_RIGHT)!=0
     && (pLevel->pRJ = capdbWhereMalloc(pWInfo, sizeof(WhereRightJoin)))!=0
    ){
      WhereRightJoin *pRJ = pLevel->pRJ;
      pRJ->iMatch = pParse->nTab++;
      pRJ->regBloom = ++pParse->nMem;
      capdbVdbeAddOp2(v, OP_Blob, 65536, pRJ->regBloom);
      pRJ->regReturn = ++pParse->nMem;
      capdbVdbeAddOp2(v, OP_Null, 0, pRJ->regReturn);
      assert( pTab==pTabItem->pSTab );
      if( HasRowid(pTab) ){
        KeyInfo *pInfo;
        capdbVdbeAddOp2(v, OP_OpenEphemeral, pRJ->iMatch, 1);
        pInfo = capdbKeyInfoAlloc(pParse->db, 1, 0);
        if( pInfo ){
          pInfo->aColl[0] = 0;
          pInfo->aSortFlags[0] = 0;
          capdbVdbeAppendP4(v, pInfo, P4_KEYINFO);
        }
      }else{
        Index *pPk = capdbPrimaryKeyIndex(pTab);
        capdbVdbeAddOp2(v, OP_OpenEphemeral, pRJ->iMatch, pPk->nKeyCol);
        capdbVdbeSetP4KeyInfo(pParse, pPk);
      }
      pLoop->wsFlags &= ~WHERE_IDX_ONLY;
  ...
  pWInfo->iTop = capdbVdbeCurrentAddr(v);
  if( db->mallocFailed ) goto whereBeginError;
  ...
  for(ii=0; ii<nTabList; ii++){
    ...
    if( pSrc->fg.isMaterialized ){
      ...
      if( pSrc->fg.isCorrelated==0 ){
        iOnce = capdbVdbeAddOp0(v, OP_Once); VdbeCoverage(v);
      }else{
        iOnce = 0;
      }
      capdbVdbeAddOp2(v, OP_Gosub, pSubq->regReturn, pSubq->addrFillSub);
      VdbeComment((v, "materialize %!S", pSrc));
      if( iOnce )  capdbVdbeJumpHere(v, iOnce);
    }
    ...
    if( (wsFlags & (WHERE_AUTO_INDEX|WHERE_BLOOMFILTER))!=0 ){
      if( (wsFlags & WHERE_AUTO_INDEX)!=0 ){
        ...
        capdbConstructBloomFilter(pWInfo, ii, pLevel, notReady);
      }
      if( db->mallocFailed ) goto whereBeginError;
    }
    addrExplain = capdbWhereExplainOneScan(
        pParse, pTabList, pLevel, wctrlFlags
    );
    pLevel->addrBody = capdbVdbeCurrentAddr(v);
    notReady = capdbWhereCodeOneLoopStart(pParse,v,pWInfo,ii,pLevel,notReady);
    pWInfo->iContinue = pLevel->addrCont;
    if( (wsFlags&WHERE_MULTI_OR)==0 && (wctrlFlags&WHERE_OR_SUBCLAUSE)==0 ){
      capdbWhereAddScanStatus(v, pTabList, pLevel, addrExplain);
    }
  }

  /* Done. */
  VdbeModuleComment((v, "Begin WHERE-core"));
  pWInfo->iEndWhere = capdbVdbeCurrentAddr(v);
  return pWInfo;
...
whereBeginError:
  ...
  ** being declared but never used */
  capdbShowWhereLoopList(0);
#endif /* WHERETRACE_ENABLED */
  return 0;
}

/*
** Part of capdbWhereEnd() will rewrite opcodes to reference the
** index rather than the main table.  In CAPDB_DEBUG mode, we want
...
#else
# define OpcodeRewriteTrace(D,K,P) capdbWhereOpcodeRewriteTrace(D,K,P)
  static void capdbWhereOpcodeRewriteTrace(
    capdb *db,
    int pc,
    VdbeOp *pOp
  ){
    if( (db->flags & CAPDB_VdbeAddopTrace)==0 ) return;
    capdbVdbePrintOp(0, pc, pOp);
    capdbShowWhereTerm(0); /* So compiler won't complain about unused func */
  }
#endif

/*
** Generate the end of the WHERE loop.  See comments on
** capdbWhereBegin() for additional information.
*/
void capdbWhereEnd(WhereInfo *pWInfo){
  Parse *pParse = pWInfo->pParse;
  Vdbe *v = pParse->pVdbe;
  int i;
  WhereLevel *pLevel;
  ...
  int iEnd = capdbVdbeCurrentAddr(v);
  int nRJ = 0;
#ifndef CAPDB_DISABLE_SKIPAHEAD_DISTINCT
  int addrSeek = 0;
#endif
  ...
  */
  ...
  for(i=pWInfo->nLevel-1; i>=0; i--){
    ...
    if( pLevel->pRJ ){
      ...
      ** the RIGHT JOIN table */
      WhereRightJoin *pRJ = pLevel->pRJ;
      capdbVdbeResolveLabel(v, pLevel->addrCont);
      ...
      ** to resolve. */
      pLevel->addrCont = capdbVdbeMakeLabel(pParse);
      pRJ->endSubrtn = capdbVdbeCurrentAddr(v);
      capdbVdbeAddOp3(v, OP_Return, pRJ->regReturn, pRJ->addrSubrtn, 1);
      VdbeCoverage(v);
      nRJ++;
    }
    pLoop = pLevel->pWLoop;
    if( pLevel->op!=OP_Noop ){
      ...
      if( pWInfo->eDistinct==WHERE_DISTINCT_ORDERED
       ...
       && pIdx->aiRowLogEst[n]>=36
        ...
        if( pLevel->iLeftJoin ){
          addrIfNull = capdbVdbeAddOp2(v, OP_IfNullRow, pLevel->iIdxCur, r1);
        }
        for(j=0; j<n; j++){
          capdbVdbeAddOp3(v, OP_Column, pLevel->iIdxCur, j, r1+j);
        }
        pParse->nMem += n+1;
        op = pLevel->op==OP_Prev ? OP_SeekLT : OP_SeekGT;
        addrSeek = capdbVdbeAddOp4Int(v, op, pLevel->iIdxCur, 0, r1, n);
        VdbeCoverageIf(v, op==OP_SeekLT);
        VdbeCoverageIf(v, op==OP_SeekGT);
        capdbVdbeAddOp2(v, OP_Goto, 1, pLevel->p2);
        if( pLevel->iLeftJoin ){
          capdbVdbeJumpHere(v, addrIfNull);
        }
      }
#endif /* CAPDB_DISABLE_SKIPAHEAD_DISTINCT */
    }
    if( pTabList->a[pLevel->iFrom].fg.fromExists ){
      /* This is an EXISTS-to-JOIN optimization loop. If this loop sees a 
      ** successful row, it should break out of itself. */
      capdbVdbeAddOp2(v, OP_Goto, 0, pLevel->addrBrk);
      VdbeComment((v, "EXISTS break %d", i));
    }
    capdbVdbeResolveLabel(v, pLevel->addrCont);
    if( pLevel->op!=OP_Noop ){
      capdbVdbeAddOp3(v, pLevel->op, pLevel->p1, pLevel->p2, pLevel->p3);
      capdbVdbeChangeP5(v, pLevel->p5);
      VdbeCoverage(v);
      VdbeCoverageIf(v, pLevel->op==OP_Next);
      VdbeCoverageIf(v, pLevel->op==OP_Prev);
      VdbeCoverageIf(v, pLevel->op==OP_VNext);
      if( pLevel->regBignull ){
        capdbVdbeResolveLabel(v, pLevel->addrBignull);
        capdbVdbeAddOp2(v, OP_DecrJumpZero, pLevel->regBignull, pLevel->p2-1);
        VdbeCoverage(v);
      }
#ifndef CAPDB_DISABLE_SKIPAHEAD_DISTINCT
      if( addrSeek ){
        capdbVdbeJumpHere(v, addrSeek);
        addrSeek = 0;
      }
#endif
    }
    if( (pLoop->wsFlags & WHERE_IN_ABLE)!=0 && pLevel->u.in.nIn>0 ){
      struct InLoop *pIn;
      int j;
      capdbVdbeResolveLabel(v, pLevel->addrNxt);
      for(j=pLevel->u.in.nIn, pIn=&pLevel->u.in.aInLoop[j-1]; j>0; j--, pIn--){
        assert( capdbVdbeGetOp(v, pIn->addrInTop+1)->opcode==OP_IsNull
                 || pParse->db->mallocFailed );
        capdbVdbeJumpHere(v, pIn->addrInTop+1);
        if( pIn->eEndLoopOp!=OP_Noop ){
          if( pIn->nPrefix ){
            ...
            if( pLevel->iLeftJoin ){
              ...
              ** be coded.  */
              capdbVdbeAddOp2(v, OP_IfNotOpen, pIn->iCur,
                  capdbVdbeCurrentAddr(v) + 2 + bEarlyOut);
              VdbeCoverage(v);
            }
            if( bEarlyOut ){
              capdbVdbeAddOp4Int(v, OP_IfNoHope, pLevel->iIdxCur,
                  capdbVdbeCurrentAddr(v)+2,
          ...
          capdbVdbeAddOp2(v, pIn->eEndLoopOp, pIn->iCur, pIn->addrInTop);
          VdbeCoverage(v);
    ...
    capdbVdbeResolveLabel(v, pLevel->addrBrk);
    if( pLevel->pRJ ){
      capdbVdbeAddOp3(v, OP_Return, pLevel->pRJ->regReturn, 0, 1);
      VdbeCoverage(v);
    }
    if( pLevel->addrSkip ){
      capdbVdbeGoto(v, pLevel->addrSkip);
      VdbeComment((v, "next skip-scan on %s", pLoop->u.btree.pIndex->zName));
      capdbVdbeJumpHere(v, pLevel->addrSkip);
      capdbVdbeJumpHere(v, pLevel->addrSkip-2);
    }
#ifndef CAPDB_LIKE_DOESNT_MATCH_BLOBS
    if( pLevel->addrLikeRep ){
      capdbVdbeAddOp2(v, OP_DecrJumpZero, (int)(pLevel->iLikeRepCntr>>1),
                        pLevel->addrLikeRep);
      VdbeCoverage(v);
    }
#endif
    if( pLevel->iLeftJoin ){
      ...
      addr = capdbVdbeAddOp1(v, OP_IfPos, pLevel->iLeftJoin); VdbeCoverage(v);
      ...
      if( (ws & WHERE_IDX_ONLY)==0 ){
        ...
        if( pSrc->fg.viaCoroutine ){
          ...
          assert( pSrc->pSTab!=0 );
          m = pSrc->pSTab->nCol;
          capdbVdbeAddOp3(v, OP_Null, 0, n, n+m-1);
        }
        capdbVdbeAddOp1(v, OP_NullRow, pLevel->iTabCur);
      }
      if( (ws & WHERE_INDEXED)
       || ((ws & WHERE_MULTI_OR) && pLevel->u.pCoveringIdx)
        ...
        if( ws & WHERE_MULTI_OR ){
          ...
          int iDb = capdbSchemaToIndex(db, pIx->pSchema);
          capdbVdbeAddOp3(v, OP_ReopenIdx, pLevel->iIdxCur, pIx->tnum, iDb);
          capdbVdbeSetP4KeyInfo(pParse, pIx);
        }
        capdbVdbeAddOp1(v, OP_NullRow, pLevel->iIdxCur);
      }
      if( pLevel->op==OP_Return ){
        capdbVdbeAddOp2(v, OP_Gosub, pLevel->p1, pLevel->addrFirst);
      }else{
        capdbVdbeGoto(v, pLevel->addrFirst);
      }
  ...
  for(i=0, pLevel=pWInfo->a; i<pWInfo->nLevel; i++, pLevel++){
    ...
    */
    if( pLevel->pRJ ){
      capdbWhereRightJoinLoop(pWInfo, i, pLevel);
      continue;
    }
    ...
    **
    ** Calls to the code generator in between capdbWhereBegin and
    ** capdbWhereEnd will have created code that references the table
...
#ifdef WHERETRACE_ENABLED
            if( capdbWhereTrace & 0x200 ){
              capdbDebugPrintf("Disable pParse->pIdxEpr term {%d,%d}\n",
                                  p->iIdxCur, p->iIdxCol);
              if( capdbWhereTrace & 0x5000 ) capdbShowExpr(p->pExpr);
            }
#endif
            p->iDataCur = -1;
            p->iIdxCur = -1;
        ...
        if( pOp->p1!=pLevel->iTabCur ){
         ...
         || pOp->opcode==OP_Offset
          ...
          if( !HasRowid(pTab) ){
            ...
            testcase( x!=capdbStorageColumnToTable(pTab,x) );
            x = capdbStorageColumnToTable(pTab,x);
          }
          x = capdbTableColumnToIndex(pIdx, x);
          if( x>=0 ){
            pOp->p2 = x;
            ...
            if( pLoop->wsFlags & WHERE_IDX_ONLY ){
              ...
              ** is not part of the index.  */
              capdbErrorMsg(pParse, "internal query planner error");
              pParse->rc = CAPDB_INTERNAL;
            }else{
              ...
              ** text that accompanies the OP_Explain opcode, if any.  */
              ...
              capdbWhereAddExplainText(pParse,
                  pLevel->addrBody-1,
                  pTabList,
                  pLevel,
                  pWInfo->wctrlFlags
...
#ifdef CAPDB_DEBUG
      if( db->flags & CAPDB_VdbeAddopTrace ) printf("TRANSLATE complete\n");
#endif
    }
  }
  ...
  ** Set it.
  */
  capdbVdbeResolveLabel(v, pWInfo->iBreak);
