...
#endif /* CAPDB_OMIT_FLOATING_POINT */

/*
** Calls to capdbFaultSim() are used to simulate a failure during testing,
** or to bypass normal error detection during testing in order to let
** execute proceed further downstream.
**
** In deployment, capdbFaultSim() *always* return CAPDB_OK (0).  The
** capdbFaultSim() function only returns non-zero during testing.
**
** During testing, if the test harness has set a fault-sim callback using
** a call to capdb_test_control(CAPDB_TESTCTRL_FAULT_INSTALL), then
** each call to capdbFaultSim() is relayed to that application-supplied
** callback and the integer return value form the application-supplied
** callback is returned by capdbFaultSim().
**
** The integer argument to capdbFaultSim() is a code to identify which
** capdbFaultSim() instance is being invoked. Each call to capdbFaultSim()
** should have a unique code.  To prevent legacy testing applications from
** breaking, the codes should not be changed or reused.
*/
#ifndef CAPDB_UNTESTABLE
int capdbFaultSim(int iTest){
  int (*xCallback)(int) = capdbGlobalConfig.xTestCallback;
  return xCallback ? xCallback(iTest) : CAPDB_OK;
}
#endif
...
**
** Use the math library isnan() function if compiled with CAPDB_HAVE_ISNAN.
** Otherwise, we have our own implementation that works on most systems.
*/
int capdbIsNaN(double x){
  int rc;   /* The value return */
#if !CAPDB_HAVE_ISNAN && !HAVE_ISNAN
  u64 y;
  memcpy(&y,&x,sizeof(y));
  rc = IsNaN(y);
#else
...
*/
int capdbStrlen30(const char *z){
  if( z==0 ) return 0;
...
*/
char *capdbColumnType(Column *pCol, char *zDflt){
  if( pCol->colFlags & COLFLAG_HASTYPE ){
    return pCol->zCnName + strlen(pCol->zCnName) + 1;
  }else if( pCol->eCType ){
    assert( pCol->eCType<=CAPDB_N_STDTYPE );
    return (char*)capdbStdType[pCol->eCType-1];
  }else{
    return zDflt;
  }
}
...
/*
** Helper function for capdbError() - called rarely.  Broken out into
** a separate routine to avoid unnecessary register saves on entry to
** capdbError().
*/
static CAPDB_NOINLINE void  capdbErrorFinish(capdb *db, int err_code){
  if( db->pErr ) capdbValueSetNull(db->pErr);
  capdbSystemError(db, err_code);
}

/*
** Set the current error code to err_code and clear any prior error message.
** Also set iSysErrno (by calling capdbSystem) if the err_code indicates
** that would be appropriate.
*/
void capdbError(capdb *db, int err_code){
  assert( db!=0 );
  db->errCode = err_code;
  if( err_code || db->pErr ){
    capdbErrorFinish(db, err_code);
  }else{
    db->errByteOffset = -1;
  }
}
...
/*
** The equivalent of capdbError(db, CAPDB_OK).  Clear the error state
** and error message.
*/
void capdbErrorClear(capdb *db){
  assert( db!=0 );
  db->errCode = CAPDB_OK;
  db->errByteOffset = -1;
  if( db->pErr ) capdbValueSetNull(db->pErr);
...
/*
** Load the capdb.iSysErrno field if that is an appropriate thing
** to do based on the SQLite error code in rc.
*/
void capdbSystemError(capdb *db, int rc){
  if( rc==CAPDB_IOERR_NOMEM ) return;
#if defined(CAPDB_USE_SEH) && !defined(CAPDB_OMIT_WAL)
  if( rc==CAPDB_IOERR_IN_PAGE ){
    int ii;
    int iErr;
    capdbBtreeEnterAll(db);
    for(ii=0; ii<db->nDb; ii++){
      if( db->aDb[ii].pBt ){
        iErr = capdbPagerWalSystemErrno(capdbBtreePager(db->aDb[ii].pBt));
        if( iErr ){
          db->iSysErrno = iErr;
        }
      }
...
#endif
  rc &= 0xff;
  if( rc==CAPDB_CANTOPEN || rc==CAPDB_IOERR ){
    db->iSysErrno = capdbOsGetLastError(db->pVfs);
  }
}
...
**
** To clear the most recent error for sqlite handle "db", capdbError
** should be called with err_code set to CAPDB_OK and zFormat set
** to NULL.
*/
void capdbErrorWithMsg(capdb *db, int err_code, const char *zFormat, ...){
  assert( db!=0 );
  db->errCode = err_code;
  capdbSystemError(db, err_code);
  if( zFormat==0 ){
    capdbError(db, err_code);
  }else if( db->pErr || (db->pErr = capdbValueNew(db))!=0 ){
    char *z;
    va_list ap;
    va_start(ap, zFormat);
    z = capdbVMPrintf(db, zFormat, ap);
    va_end(ap);
    capdbValueSetStr(db->pErr, -1, z, CAPDB_UTF8, CAPDB_DYNAMIC);
  }
}
...
*/
void capdbProgressCheck(Parse *p){
  capdb *db = p->db;
...
** compiling an SQL statement (i.e. within capdb_prepare()). The
** last thing the capdb_prepare() function does is copy the error
** stored by this function into the database handle using capdbError().
** Functions capdbError() or capdbErrorWithMsg() should be used
** during statement execution (capdb_step() etc.).
*/
void capdbErrorMsg(Parse *pParse, const char *zFormat, ...){
  char *zMsg;
  va_list ap;
  capdb *db = pParse->db;
  assert( db!=0 );
  ...
  db->errByteOffset = -2;
  va_start(ap, zFormat);
  zMsg = capdbVMPrintf(db, zFormat, ap);
  va_end(ap);
...
*/
int capdbErrorToParser(capdb *db, int errCode){
  Parse *pParse;
...
** "a-b-c".
*/
void capdbDequote(char *z){
  char quote;
  int i, j;
  if( z==0 ) return;
  quote = z[0];
  if( !capdbIsquote(quote) ) return;
  if( quote=='[' ) quote = ']';
  for(i=1, j=0;; i++){
    assert( z[i] );
    if( z[i]==quote ){
...
void capdbDequoteExpr(Expr *p){
  assert( !ExprHasProperty(p, EP_IntValue) );
  assert( capdbIsquote(p->u.zToken[0]) );
  p->flags |= p->u.zToken[0]=='"' ? EP_Quoted|EP_DblQuoted : EP_Quoted;
  capdbDequote(p->u.zToken);
}
...
*/
void capdbDequoteNumber(Parse *pParse, Expr *p){
  ...
  if( p ){
    ...
    p->op = TK_INTEGER;
    do {
      if( *pIn!=CAPDB_DIGIT_SEPARATOR ){
        *pOut++ = *pIn;
        if( *pIn=='e' || *pIn=='E' || *pIn=='.' ) p->op = TK_FLOAT;
      }else{
        if( (bHex==0 && (!capdbIsdigit(pIn[-1]) || !capdbIsdigit(pIn[1])))
         || (bHex==1 && (!capdbIsxdigit(pIn[-1]) || !capdbIsxdigit(pIn[1])))
        ){
          capdbErrorMsg(pParse, "unrecognized token: \"%s\"", p->u.zToken);
        }
      }
    }while( *pIn++ );
    ...
    ** parts of the code expect this.  See also tag-20240227-b. */
    if( p->op==TK_INTEGER && capdbGetInt32(p->u.zToken, &iValue) ){
      p->u.iValue = iValue;
      p->flags |= EP_IntValue;
    }
  }
...
** is always a no-op.
*/
void capdbDequoteToken(Token *p){
  unsigned int i;
  if( p->n<2 ) return;
  if( !capdbIsquote(p->z[0]) ) return;
  for(i=1; i<p->n-1; i++){
    if( capdbIsquote(p->z[i]) ) return;
  }
  p->n -= 2;
  p->z++;
}
...
*/
void capdbTokenInit(Token *p, char *z){
  p->z = z;
  p->n = capdbStrlen30(z);
}

/* Convenient short-hand */
#define UpperToLower capdbUpperToLower

/*
** Some systems have stricmp().  Others have strcasecmp().  Because
** there is no consistency, we will define our own.
**
** IMPLEMENTATION-OF: R-30243-02494 The capdb_stricmp() and
** capdb_strnicmp() APIs allow applications and extensions to compare
** the contents of two buffers containing UTF-8 strings in a
** case-independent fashion, using the same definition of "case
** independence" that SQLite uses internally when comparing identifiers.
*/
int capdb_stricmp(const char *zLeft, const char *zRight){
  if( zLeft==0 ){
    return zRight ? -1 : 0;
  }else if( zRight==0 ){
    return 1;
  }
  return capdbStrICmp(zLeft, zRight);
}
int capdbStrICmp(const char *zLeft, const char *zRight){
  unsigned char *a, *b;
  int c, x;
...
int capdb_strnicmp(const char *zLeft, const char *zRight, int N){
...
** 64 bits.
*/
static u64 capdbMultiply128(u64 a, u64 b, u64 *pLo){
#if defined(CAPDB_USE_UINT128)
  __uint128_t r = (__uint128_t)a * b;
  *pLo = (u64)r;
  return (u64)(r>>64);
#elif defined(_WIN64) && !defined(CAPDB_DISABLE_INTRINSIC)
  *pLo = a*b;
  return __umulh(a, b);
#else
  u64 a0 = (u32)a;
...
*/
static u64 capdbMultiply160(u64 a, u32 aLo, u64 b, u32 *pLo){
#if defined(CAPDB_USE_UINT128)
  __uint128_t r = (__uint128_t)a * b;
...
static u64 powerOfTen(int p, u32 *pLo){
  ...
  x = capdbMultiply160(s,aScaleLo[g+13],aBase[n],&lo);
  if( (U64_BIT(63) & x)==0 ){
    x  = x<<1 | ((lo>>31)&1);
    lo = (lo<<1) | 1;
  }
...
static int countLeadingZeros(u64 m){
    ...
    && !defined(CAPDB_DISABLE_INTRINSIC)
...
*/
static void capdbFp2Convert10(u64 m, int e, int n, u64 *pD, int *pP){
  int p;
  u64 h, d1;
  u32 d2;
  assert( n>=1 && n<=18 );
  p = n - 1 - pwr2to10(e+63);
  h = capdbMultiply128(m, powerOfTen(p,&d2), &d1);
...
*/
static double capdbFp10Convert2(u64 d, int p){
  int b, lp, e, adj, s;
  u32 pwr10l, mid1;
  ...
  x = d<<(64-b);
  hi = capdbMultiply128(x,pwr10h,&lo);
  mid1 = lo>>32;
  sticky = 1;
  if( (hi & (U64_BIT(s)-1))==0 ) {
    u32 mid2 = capdbMultiply128(x,((u64)pwr10l)<<32,&lo)>>32;
    sticky = (mid1-mid2 > 1);
    hi -= mid1 < mid2;
  }
  u = (hi>>s) | sticky;
...
** Then invoke capdbFp10Convert2() to calculated the closest possible
...
*/
int capdbAtoF(const char *zIn, double *pResult){
#ifndef CAPDB_OMIT_FLOATING_POINT
  ...
  if( (v = (unsigned)z[0] - '0')<10 ){
    ...
    z++;
    if( (v = (unsigned)z[0] - '0')<10 ) goto parse_integer_part;
  }else if( capdbIsspace(z[0]) ){
    do{ z++; }while( capdbIsspace(z[0]) );
    goto start_of_text;
  }else{
    s = 0;
  }
  ...
  if( s==0 ){
    ...
    *pResult = capdbFp10Convert2(s,d);
  }
  if( neg ) *pResult = -*pResult;
  assert( !capdbIsNaN(*pResult) );
...
#else
  return capdbAtoi64(z, pResult, strlen(z), CAPDB_UTF8)==0;
#endif /* CAPDB_OMIT_FLOATING_POINT */
}
...
# define CAPDB_AVOID_U64_DIVIDE 1
#endif

#ifdef CAPDB_AVOID_U64_DIVIDE
/*
** Render an unsigned 64-bit integer as text onto the end of a 2-byte
** aligned buffer that is CAPDB_U64_DIGIT+1 bytes long.  The last byte
...
*/
static int capdbUInt64ToText(u64 v, char *zOut){
  u32 x32, kk;
  int i;
  zOut[CAPDB_U64_DIGITS] = 0;
  i = CAPDB_U64_DIGITS;
  assert( TWO_BYTE_ALIGNMENT(&capdbDigitPairs.a[0]) );
  ...
  while( (v>>32)!=0 ){
    ...
    assert( i>=8 );
    i -= 8;
    *(u16*)(&zOut[i]) = *(u16*)&capdbDigitPairs.a[x1*2];
    *(u16*)(&zOut[i+2]) = *(u16*)&capdbDigitPairs.a[x0*2];
    *(u16*)(&zOut[i+4]) = *(u16*)&capdbDigitPairs.a[y1*2];
    *(u16*)(&zOut[i+6]) = *(u16*)&capdbDigitPairs.a[y0*2];
  }
  x32 = v;
  while( x32>=10 ){
    kk  = x32 % 100;
    x32 = x32 / 100;
    assert( TWO_BYTE_ALIGNMENT(&capdbDigitPairs.a[kk*2]) );
    assert( i>=2 );
    i -= 2;
    assert( TWO_BYTE_ALIGNMENT(&zOut[i]) );
    *(u16*)(&zOut[i]) = *(u16*)&capdbDigitPairs.a[kk*2];
  }
  if( x32 ){
    assert( i>0 );
    zOut[--i] = x32 + '0';
...
#endif /* defined(CAPDB_AVOID_U64_DIVIDE) */
...
*/
int capdbInt64ToText(i64 v, char *zOut){
  int i;
  u64 x;
  union {
    char a[CAPDB_U64_DIGITS+1];
    u16 forceAlignment;
  } u;
  if( v>0 ){
    x = v;
...
#ifdef CAPDB_AVOID_U64_DIVIDE
  i = capdbUInt64ToText(x, u.a);
#else
  i = sizeof(u.a)-1;
  u.a[i] = 0;
  while( x>=10 ){
    int kk = (x%100)*2;
    assert( TWO_BYTE_ALIGNMENT(&capdbDigitPairs.a[kk]) );
    assert( TWO_BYTE_ALIGNMENT(&u.a[i-2]) );
    *(u16*)(&u.a[i-2]) = *(u16*)&capdbDigitPairs.a[kk];
    i -= 2;
    x /= 100;
  }
  if( x ){
...
** given by enc.
*/
int capdbAtoi64(const char *zNum, i64 *pNum, int length, u8 enc){
  int incr;
  u64 u = 0;
  ...
  assert( enc==CAPDB_UTF8 || enc==CAPDB_UTF16LE || enc==CAPDB_UTF16BE );
  if( enc==CAPDB_UTF8 ){
    incr = 1;
  }else{
    incr = 2;
    length &= ~1;
    assert( CAPDB_UTF16LE==2 && CAPDB_UTF16BE==3 );
    for(i=3-enc; i<length && zNum[i]==0; i+=2){}
    nonNum = i<length;
    zEnd = &zNum[i^1];
    zNum += (enc&1);
  }
  while( zNum<zEnd && capdbIsspace(*zNum) ) zNum+=incr;
  if( zNum<zEnd ){
    if( *zNum=='-' ){
      neg = 1;
      zNum+=incr;
...
**     3    Special case of 9223372036854775808
*/
int capdbDecOrHexToI64(const char *z, i64 *pOut){
  ...
  if( z[0]=='0'
   && (z[1]=='x' || z[1]=='X')
    ...
    int i, k;
    for(i=2; z[i]=='0'; i++){}
    for(k=i; capdbIsxdigit(z[k]); k++){
      u = u*16 + capdbHexToInt(z[k]);
    }
    memcpy(pOut, &u, 8);
    if( k-i>16 ) return 2;
    if( z[k]!=0 ) return 1;
...
#endif /* CAPDB_OMIT_HEX_INTEGER */
  {
    int n = (int)(0x3fffffff&strspn(z,"+- \n\t0123456789"));
    if( z[n] ) n++;
    return capdbAtoi64(z, pOut, n, CAPDB_UTF8);
  }
}
...
** This is different from capdbAtoi64() which requires the
** input number to be zero-terminated.
*/
int capdbGetInt32(const char *zNum, int *pValue){
  sqlite_int64 v = 0;
  int i, c;
  int neg = 0;
  if( zNum[0]=='-' ){
    ...
    zNum++;
        ...
        && (zNum[1]=='x' || zNum[1]=='X')
        && capdbIsxdigit(zNum[2])
  ){
    u32 u = 0;
    zNum += 2;
    while( zNum[0]=='0' ) zNum++;
    for(i=0; i<8 && capdbIsxdigit(zNum[i]); i++){
      u = u*16 + capdbHexToInt(zNum[i]);
    }
    if( (u&0x80000000)==0 && capdbIsxdigit(zNum[i])==0 ){
      memcpy(pValue, &u, 4);
      return 1;
    }else{
      return 0;
...
#endif
  if( !capdbIsdigit(zNum[0]) ) return 0;
  while( zNum[0]=='0' ) zNum++;
...
*/
void capdbFpDecode(FpDecode *p, double r, int iRound, int mxRound){
  ...
  ** NaN. */
  ...
  capdbFp2Convert10(v, e, (iRound<=0||iRound>=18)?18:iRound+1, &v, &exp);  
  ...
  ** which to store a digit. */
  assert( sizeof(p->zBuf)==CAPDB_U64_DIGITS+1 );
  assert( v>0 );
  zBuf = p->zBuf;
#ifdef CAPDB_AVOID_U64_DIVIDE
  i = capdbUInt64ToText(v, zBuf);
#else
  i = CAPDB_U64_DIGITS;
  while( v>=10 ){
    int kk = (v%100)*2;
    ...
    assert( i-2>=0 );
    *(u16*)(&zBuf[i-2]) = *(u16*)&capdbDigitPairs.a[kk];
    i -= 2;
    v /= 100;
  }
  if( v ){
...
#endif /* CAPDB_AVOID_U64_DIVIDE */
  assert( i>=0 && i<CAPDB_U64_DIGITS );
  n = CAPDB_U64_DIGITS - i;  /* Total number of digits extracted */
  assert( n>0 );
  assert( n<=CAPDB_U64_DIGITS );
  p->iDP = n + exp;
  if( iRound<=0 ){
    iRound = p->iDP - iRound;
    ...
    if( iRound==17 ){
      ...
      ** as 49.469999999999999. */
      if( z[15]=='9' && z[14]=='9' ){
        ...
        if( r==capdbFp10Convert2(v2, exp + n - jj) ){
          iRound = jj+1;
        }
...
*/
int capdbGetUInt32(const char *z, u32 *pI){
  u64 v = 0;
  int i;
  for(i=0; capdbIsdigit(z[i]); i++){
    v = v*10 + z[i] - '0';
...
*/
static int CAPDB_NOINLINE putVarint64(unsigned char *p, u64 v){
  int i, j, n;
  u8 buf[10];
...
int capdbPutVarint(unsigned char *p, u64 v){
  if( v<=0x7f ){
    p[0] = v&0x7f;
    return 1;
  }
...
/*
** Bitmasks used by capdbGetVarint().  These precomputed constants
...
*/
u8 capdbGetVarint(const unsigned char *p, u64 *v){
  u32 a,b,s;
...
*/
u8 capdbGetVarint32(const unsigned char *p, u32 *v){
  u64 v64;
  u8 n;
  ...
  ** the getVarint32() macro */
  ...
  /* four or more bytes */
  n = capdbGetVarint(p, &v64);
  assert( n>3 && n<=9 );
  if( (v64 & CAPDB_MAX_U32)!=v64 ){
    *v = 0xffffffff;
  }else{
    *v = (u32)v64;
  }
...
void capdbPut4byte(unsigned char *p, u32 v){
#if CAPDB_BYTEORDER==4321
  memcpy(p,&v,4);
#elif CAPDB_BYTEORDER==1234 && GCC_VERSION>=4003000
...
** the calling routine.
*/
void *capdbHexToBlob(capdb *db, const char *z, int n){
  char *zBlob;
  int i;

  zBlob = (char *)capdbDbMallocRawNN(db, n/2 + 1);
  n--;
  if( zBlob ){
    for(i=0; i<n; i+=2){
      zBlob[i/2] = (capdbHexToInt(z[i])<<4) | capdbHexToInt(z[i+1]);
    }
    zBlob[i/2] = 0;
  }
  return zBlob;
}
#endif /* !CAPDB_OMIT_BLOB_LITERAL */
...
** CAPDB_MISUSE immediately.
**
** capdbSafetyCheckOk() requires that the db pointer be valid for
** use.  capdbSafetyCheckSickOrOk() allows a db pointer that failed to
** open properly and is not fit for general use but which can be
** used as an argument to capdb_errmsg() or capdb_close().
*/
int capdbSafetyCheckOk(capdb *db){
  u8 eOpenState;
  if( db==0 ){
  ...
  if( eOpenState!=CAPDB_STATE_OPEN ){
...
int capdbSafetyCheckSickOrOk(capdb *db){
  u8 eOpenState;
  eOpenState = db->eOpenState;
  if( eOpenState!=CAPDB_STATE_SICK &&
      eOpenState!=CAPDB_STATE_OPEN &&
      eOpenState!=CAPDB_STATE_BUSY ){
    testcase( capdbGlobalConfig.xLog!=0 );
...
*/
int capdbAddInt64(i64 *pA, i64 iB){
...
int capdbSubInt64(i64 *pA, i64 iB){
#if GCC_VERSION>=5004000 && !defined(__INTEL_COMPILER)
  ...
  if( iB==SMALLEST_INT64 ){
    ...
    *pA -= iB;
    ...
    return capdbAddInt64(pA, -iB);
  }
#endif
}
int capdbMulInt64(i64 *pA, i64 iB){
#if GCC_VERSION>=5004000 && !defined(__INTEL_COMPILER)
...
*/
void capdbFileSuffix3(const char *zBaseFilename, char *z){
#if CAPDB_ENABLE_8_3_NAMES<2
  if( capdb_uri_boolean(zBaseFilename, "8_3_names", 0) )
#endif
  {
    int i, sz;
    sz = capdbStrlen30(z);
...
**
*/
LogEst capdbLogEstAdd(LogEst a, LogEst b){
...
*/
LogEst capdbLogEstFromDouble(double x){
  u64 a;
  LogEst e;
  assert( sizeof(x)==8 && sizeof(a)==8 );
  if( x<=1 ) return 0;
  if( x<=2000000000 ) return capdbLogEst((u64)x);
  memcpy(&a, &x, 8);
  e = (a>>52) - 1022;
  return e*10;
}
...
*/
VList *capdbVListAdd(
  capdb *db,           /* The database connection used for malloc() */
  ...
  if( pIn==0 || pIn[1]+nInt > pIn[0] ){
    ...
    capdb_int64 nAlloc = (pIn ? 2*(capdb_int64)pIn[0] : 10) + nInt;
    VList *pOut = capdbDbRealloc(db, pIn, nAlloc*sizeof(int));
...
** the list
*/
const char *capdbVListNumToName(VList *pIn, int iVal){
  int i, mx;
  if( pIn==0 ) return 0;
  mx = pIn[1];
  i = 2;
...
*/
int capdbVListNameToNum(VList *pIn, const char *zName, int nName){
  int i, mx;
  if( pIn==0 ) return 0;
  mx = pIn[1];
  i = 2;
