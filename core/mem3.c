...
** in the build only if CAPDB_ENABLE_MEMSYS3 is defined.
*/
#include "capdbInt.h"

/*
** This version of the memory allocator is only built into the library
** CAPDB_ENABLE_MEMSYS3 is defined. Defining this symbol does not
...
*/
static CAPDB_WSD struct Mem3Global {
  /*
...
static void memsys3UnlinkFromList(u32 i, u32 *pRoot){
  ...
  assert( capdb_mutex_held(mem3.mutex) );
  if( prev==0 ){
    *pRoot = next;
  }else{
...
** capdbGlobalConfig.bMemStat is true.
*/
static void memsys3Enter(void){
  if( capdbGlobalConfig.bMemstat==0 && mem3.mutex==0 ){
    mem3.mutex = capdbMutexAlloc(CAPDB_MUTEX_STATIC_MEM);
  }
  capdb_mutex_enter(mem3.mutex);
}
static void memsys3Leave(void){
  capdb_mutex_leave(mem3.mutex);
}
...
static int memsys3Init(void *NotUsed){
  ...
  if( !capdbGlobalConfig.pHeap ){
    return CAPDB_ERROR;
  }

  /* Store a pointer to the memory block in global structure mem3. */
  assert( sizeof(Mem3Block)==8 );
  mem3.aPool = (Mem3Block *)capdbGlobalConfig.pHeap;
  mem3.nPool = (capdbGlobalConfig.nHeap / sizeof(Mem3Block)) - 2;
...
*/
void capdbMemsys3Dump(const char *zFilename){
#ifdef CAPDB_DEBUG
  FILE *out;
  u32 i, j;
  u32 size;
...
const capdb_mem_methods *capdbMemGetMemsys3(void){
  static const capdb_mem_methods mempoolMethods = {
     memsys3Malloc,
     memsys3Free,
     memsys3Realloc,
     memsys3Size,
