...
*/
static int winMutexHeld(capdb_mutex *p){
  return p->nRef!=0 && p->owner==GetCurrentThreadId();
}

static int winMutexNotheld(capdb_mutex *p){
  return p->nRef==0 || p->owner!=GetCurrentThreadId();
}
#endif
...
** compiled without mutexes (CAPDB_THREADSAFE=0).
*/
...
** of the capdb_initialize() and capdb_shutdown() processing, the
** "interlocked" magic used here is probably not strictly necessary.
*/
static LONG volatile winMutex_lock = 0;

void capdb_win32_sleep(DWORD milliseconds); /* os_win.c */
...
static int winMutexInit(void){
  ...
  if( InterlockedCompareExchange(&winMutex_lock, 1, 0)==0 ){
    ...
    for(i=0; i<ArraySize(aWindowsMutex); i++){
      capdb_mutex *p = &aWindowsMutex[i].m;
      p->id = i+2;
...
*/
static capdb_mutex *winMutexAlloc(int iType){
  capdb_mutex *p;
  ...
  switch( iType ){
    ...
    case CAPDB_MUTEX_RECURSIVE: {
      ...
      if( p ){
        p->id = iType;
#ifdef CAPDB_DEBUG
#ifdef CAPDB_WIN32_MUTEX_TRACE_DYNAMIC
        p->trace = 1;
#endif
#endif
        if( iType==CAPDB_MUTEX_RECURSIVE ){
...
#ifdef CAPDB_DEBUG
#ifdef CAPDB_WIN32_MUTEX_TRACE_STATIC
...
*/
static void winMutexFree(capdb_mutex *p){
  assert( p );
  assert( p->nRef==0 && p->owner==0 );
  if( p->id==CAPDB_MUTEX_FAST ){
    capdb_free(p);
  }else if( p->id==CAPDB_MUTEX_RECURSIVE ){
...
*/
static void winMutexEnter(capdb_mutex *p){
  assert( p );
#ifdef CAPDB_DEBUG
  assert( p->id==CAPDB_MUTEX_RECURSIVE || winMutexNotheld(p) );
#endif
  assert( winMutex_isInit==1 );
  if( p->id==CAPDB_MUTEX_RECURSIVE ){
...
static int winMutexTry(capdb_mutex *p){
  int rc = CAPDB_BUSY;
  assert( p );
  assert( p->id==CAPDB_MUTEX_RECURSIVE || winMutexNotheld(p) );
  /*
  ** The capdb_mutex_try() routine is seldom used, and when it is
...
*/
static void winMutexLeave(capdb_mutex *p){
  assert( p );
#ifdef CAPDB_DEBUG
  assert( p->nRef>0 );
