...
*/
void capdbTestBinToHex(unsigned char *zBuf, int N){
...
*/
int capdbTestHexToBin(const unsigned char *zIn, int N, unsigned char *aOut){
...
*/
static int CAPDB_TCLAPI hexio_read(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
*/
static int CAPDB_TCLAPI hexio_write(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  aOut = capdb_malloc64( 1 + nIn/2 );
  if( aOut==0 ){
    return TCL_ERROR;
  }
  nOut = capdbTestHexToBin(zIn, (int)nIn, aOut);
  out = fopen(zFile, "r+b");
  if( out==0 ){
    out = fopen(zFile, "r+");
  }
...
** hexadecimal digits.
*/
static int CAPDB_TCLAPI hexio_get_int(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
*/
static int CAPDB_TCLAPI hexio_render_int16(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
*/
static int CAPDB_TCLAPI hexio_render_int32(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
** capdbUtf8to8() convert it back to hex and return the result.
*/
static int CAPDB_TCLAPI utf8_to_utf8(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  z = capdb_malloc64( n+4 );
  n = capdbTestHexToBin(zOrig, (int)n, z);
  z[n] = 0;
  nOut = capdbUtf8To8(z);
  capdbTestBinToHex(z,nOut);
  Tcl_AppendResult(interp, (char*)z, NULL);
  capdb_free(z);
  return TCL_OK;
#else
  Tcl_AppendResult(interp, 
      "[utf8_to_utf8] unavailable - CAPDB_DEBUG not defined", NULL
  );
  return TCL_ERROR;
#endif
}
...
*/
static int CAPDB_TCLAPI read_fts3varint(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  nVal = getFts3Varint((char*)zBlob, (capdb_int64 *)(&iVal));
...
*/
static int CAPDB_TCLAPI make_fts3record(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  for(i=0; i<(int)nArg; i++){
    ...
    if( TCL_OK==Tcl_GetWideIntFromObj(0, aArg[i], &iVal) ){
      if( nOut+10>nAlloc ){
        ...
        unsigned char *aNew = capdb_realloc(aOut, nNew);
        if( aNew==0 ){
          capdb_free(aOut);
          return TCL_ERROR;
        }
      ...
      while( (nOut + nVal)>nAlloc ){
        capdb_int64 nNew = nAlloc?nAlloc*2:128;
        unsigned char *aNew = capdb_realloc64(aOut, nNew);
        if( aNew==0 ){
          capdb_free(aOut);
          return TCL_ERROR;
        }
