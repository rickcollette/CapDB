...
static void MD5DigestToBase10x8(unsigned char digest[16], char zDigest[50]){
  ...
  for(i=j=0; i<16; i+=2){
    ...
    if( i>0 ) zDigest[j++] = '-';
    capdb_snprintf(50-j, &zDigest[j], "%05u", x);
    j += 5;
  }
  zDigest[j] = 0;
}
...
*/
static int CAPDB_TCLAPI md5_cmd(
  void*cd,
  Tcl_Interp *interp,
  int argc,
  const char **argv
...
** name of the file.
*/
static int CAPDB_TCLAPI md5file_cmd(
  void*cd,
  Tcl_Interp *interp,
  int argc,
  const char **argv
...
int Md5_Register(
  ...
  int rc = capdb_create_function(db, "md5sum", -1, CAPDB_UTF8, 0, 0,
                                 md5step, md5finalize);
  capdb_overload_function(db, "md5sum", -1);  /* To exercise this API */
  return rc;
}
