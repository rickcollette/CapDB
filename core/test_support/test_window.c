...
static void doTestWindowStep(
  ...
  int i;
  TestWindow *p = (TestWindow*)capdb_user_data(ctx);
  Tcl_Obj *pEval = Tcl_DuplicateObj(bInverse ? p->xInverse : p->xStep);
  TestWindowCtx *pCtx = capdb_aggregate_context(ctx, sizeof(TestWindowCtx));
  ...
  if( pCtx ){
    ...
    for(i=0; i<nArg; i++){
      Tcl_Obj *pArg;
      pArg = Tcl_NewStringObj((const char*)capdb_value_text(apArg[i]), -1);
      Tcl_ListObjAppendElement(p->interp, pEval, pArg);
    }
    rc = Tcl_EvalObjEx(p->interp, pEval, TCL_EVAL_GLOBAL);
    if( rc!=TCL_OK ){
      zResult = Tcl_GetStringResult(p->interp);
      capdb_result_error(ctx, zResult, -1);
    }else{
...
static void doTestWindowFinalize(int bValue, capdb_context *ctx){
  TestWindow *p = (TestWindow*)capdb_user_data(ctx);
  ...
  if( pCtx ){
    ...
    if( rc!=TCL_OK ){
      ...
      capdb_result_text(ctx, zResult, -1, CAPDB_TRANSIENT);
    }
...
static void testWindowFinal(capdb_context *ctx){
  doTestWindowFinalize(0, ctx);
}
static void testWindowValue(capdb_context *ctx){
...
/*
** Usage: capdb_create_window_function DB NAME XSTEP XFINAL XVALUE XINVERSE
*/
static int CAPDB_TCLAPI test_create_window(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  rc = capdb_create_window_function(db, zName, -1, CAPDB_UTF8, (void*)pNew,
...
static int CAPDB_TCLAPI test_create_window_misuse(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  rc = capdb_create_window_function(db, "fff", -1, CAPDB_UTF8, 0,
      0, testWindowFinal, testWindowValue, testWindowInverse,
      0
  );
  if( rc!=CAPDB_MISUSE ) goto error;
  rc = capdb_create_window_function(db, "fff", -1, CAPDB_UTF8, 0,
...
static void sumintStep(
  ...
  assert( nArg==1 );
  if( capdb_value_type(apArg[0])!=CAPDB_INTEGER ){
    capdb_result_error(ctx, "invalid argument", -1);
    return;
  }
  pInt = (capdb_int64*)capdb_aggregate_context(ctx, sizeof(capdb_int64));
  if( pInt ){
    *pInt += capdb_value_int64(apArg[0]);
  }
}
...
** xFinal for sumint().
*/
static void sumintFinal(capdb_context *ctx){
  capdb_int64 res = 0;
  capdb_int64 *pInt;
  pInt = (capdb_int64*)capdb_aggregate_context(ctx, 0);
  if( pInt ) res = *pInt;
  capdb_result_int64(ctx, res);
}

/*
** xValue for sumint().
*/
static void sumintValue(capdb_context *ctx){
  capdb_int64 res = 0;
  capdb_int64 *pInt;
  pInt = (capdb_int64*)capdb_aggregate_context(ctx, 0);
  if( pInt ) res = *pInt;
...
static int CAPDB_TCLAPI test_create_sumint(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  rc = capdb_create_window_function(db, "sumint", 1, CAPDB_UTF8, 0,
...
static int CAPDB_TCLAPI test_override_sum(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  rc = capdb_create_function(db, "sum", -1, CAPDB_UTF8, 0,
...
int Sqlitetest_window_Init(Tcl_Interp *interp){
  static struct {
     ...
     { "capdb_create_window_function", test_create_window, 0 },
