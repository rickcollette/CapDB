...
** macro. i.e. so that STRINGVALUE(CAPDB_NOMEM) -> "7".
*/
...
static void set_options(Tcl_Interp *interp){
...
#if defined(CAPDB_ENABLE_CARRAY)
...
#ifdef CAPDB_DEFAULT_CKPTFULLFSYNC
  Tcl_SetVar2(interp, "sqlite_options", "default_ckptfullfsync", 
              CAPDB_DEFAULT_CKPTFULLFSYNC ? "1" : "0", TCL_GLOBAL_ONLY);
#else
...
#ifdef CAPDB_DISABLE_PAGECACHE_OVERFLOW_STATS
  ...
  Tcl_SetVar2(interp, "sqlite_options", "worker_threads", 
      STRINGVALUE(CAPDB_MAX_WORKER_THREADS), TCL_GLOBAL_ONLY
  );
...
#endif /* CAPDB_OMIT_AUTOVACUUM */
#if !defined(CAPDB_DEFAULT_AUTOVACUUM)
  ...
  Tcl_SetVar2(interp, "sqlite_options", "default_autovacuum", 
      STRINGVALUE(CAPDB_DEFAULT_AUTOVACUUM), TCL_GLOBAL_ONLY);
#endif

#ifdef CAPDB_OMIT_BETWEEN_OPTIMIZATION
...
#ifdef CAPDB_ENABLE_ORDEREDSETFUNC
...
#ifdef CAPDB_ENABLE_OVERSIZE_CELL_CHECK
...
#ifdef CAPDB_OMIT_COMPILEOPTION_DIAGS
...
#if defined(CAPDB_ENABLE_FTS3) && !defined(CAPDB_DISABLE_FTS3_UNICODE)
...
#ifdef CAPDB_DISABLE_FTS4_DEFERRED
...
#ifdef CAPDB_ENABLE_ICU_COLLATIONS
...
#ifdef CAPDB_OMIT_INTEGRITY_CHECK
...
#if defined(CAPDB_DEFAULT_FILE_FORMAT) && CAPDB_DEFAULT_FILE_FORMAT==1
...
#ifdef CAPDB_OMIT_LIKE_OPTIMIZATION
...
#if defined(CAPDB_OMIT_PRAGMA) || defined(CAPDB_OMIT_FLAG_PRAGMAS)
...
#ifdef CAPDB_OMIT_SCHEMA_PRAGMAS
...
#ifdef CAPDB_OMIT_SCHEMA_VERSION_PRAGMAS
...
#ifdef CAPDB_ENABLE_POOL
  Tcl_SetVar2(interp, "sqlite_options", "connection_pool", "1", TCL_GLOBAL_ONLY);
#else
  Tcl_SetVar2(interp, "sqlite_options", "connection_pool", "0", TCL_GLOBAL_ONLY);
#endif

#ifdef CAPDB_ENABLE_NETWORK
  Tcl_SetVar2(interp, "sqlite_options", "capdb_network", "1", TCL_GLOBAL_ONLY);
#else
  Tcl_SetVar2(interp, "sqlite_options", "capdb_network", "0", TCL_GLOBAL_ONLY);
#endif
...
#endif
#if defined(CAPDB_ENABLE_STMTVTAB) && !defined(CAPDB_OMIT_VIRTUALTABLE)
...
#  endif
...
#endif
#if defined(CAPDB_PREFER_PROXY_LOCKING) && defined(__APPLE__)
  ...
  Tcl_SetVar2(interp, "sqlite_options", "threadsafe", 
      CAPDB_THREADSAFE ? "1" : "0", TCL_GLOBAL_ONLY);
  Tcl_SetVar2(interp, "sqlite_options", "threadsafe1", 
      CAPDB_THREADSAFE==1 ? "1" : "0", TCL_GLOBAL_ONLY);
  Tcl_SetVar2(interp, "sqlite_options", "threadsafe2", 
      CAPDB_THREADSAFE==2 ? "1" : "0", TCL_GLOBAL_ONLY);
  assert( capdb_threadsafe()==CAPDB_THREADSAFE );
...
#if defined(CAPDB_ENABLE_UNLOCK_NOTIFY)
...
#ifdef CAPDB_FAST_SECURE_DELETE
...
#ifdef CAPDB_MULTIPLEX_EXT_OVWR
...
#ifdef CAPDB_ENABLE_URI_00_ERROR
...
#if defined(CAPDB_ENABLE_NORMALIZE)
  ...
  Tcl_SetVar2(interp, "sqlite_options", "setlk_timeout", 
      STRINGVALUE(CAPDB_ENABLE_SETLK_TIMEOUT), TCL_GLOBAL_ONLY);
#endif

#define LINKVAR(x) { \
    static const int cv_ ## x = CAPDB_ ## x; \
    Tcl_LinkVar(interp, "CAPDB_" #x, (char *)&(cv_ ## x), \
  ...
  {
    static const int cv_TEMP_STORE = CAPDB_TEMP_STORE;
