...
static struct TestSyscallGlobal {
  ...
  int pgsz;
  capdb_syscall_ptr orig_getpagesize;
} gSyscall = { 0, 0, 0, 0, 0 };
...
** (ex: Mac, OpenBSD).  So substitute a likely equivalent: capdb_uint64 */
static ssize_t ts_pread64(int fd, void *aBuf, size_t nBuf, capdb_uint64 off);
...
static ssize_t ts_pwrite64(int fd, const void *aBuf, size_t nBuf, capdb_uint64 off);
...
struct TestSyscallArray {
  const char *zName;
  capdb_syscall_ptr xTest;
  capdb_syscall_ptr xOrig;
  int default_errno;              /* Default value for errno following errors */
  int custom_errno;               /* Current value for errno if error */
} aSyscall[] = {
  /*  0 */ { "open",      (capdb_syscall_ptr)ts_open,      0, EACCES, 0 },
  /*  1 */ { "close",     (capdb_syscall_ptr)ts_close,     0, 0, 0 },
  /*  2 */ { "access",    (capdb_syscall_ptr)ts_access,    0, 0, 0 },
  /*  3 */ { "getcwd",    (capdb_syscall_ptr)ts_getcwd,    0, 0, 0 },
  /*  4 */ { "stat",      (capdb_syscall_ptr)ts_stat,      0, 0, 0 },
  /*  5 */ { "fstat",     (capdb_syscall_ptr)ts_fstat,     0, 0, 0 },
  /*  6 */ { "ftruncate", (capdb_syscall_ptr)ts_ftruncate, 0, EIO, 0 },
  /*  7 */ { "fcntl",     (capdb_syscall_ptr)ts_fcntl,     0, EACCES, 0 },
  /*  8 */ { "read",      (capdb_syscall_ptr)ts_read,      0, 0, 0 },
  /*  9 */ { "pread",     (capdb_syscall_ptr)ts_pread,     0, 0, 0 },
  /* 10 */ { "pread64",   (capdb_syscall_ptr)ts_pread64,   0, 0, 0 },
  /* 11 */ { "write",     (capdb_syscall_ptr)ts_write,     0, 0, 0 },
  /* 12 */ { "pwrite",    (capdb_syscall_ptr)ts_pwrite,    0, 0, 0 },
  /* 13 */ { "pwrite64",  (capdb_syscall_ptr)ts_pwrite64,  0, 0, 0 },
  /* 14 */ { "fchmod",    (capdb_syscall_ptr)ts_fchmod,    0, 0, 0 },
  /* 15 */ { "fallocate", (capdb_syscall_ptr)ts_fallocate, 0, 0, 0 },
  /* 16 */ { "mmap",      (capdb_syscall_ptr)ts_mmap,      0, 0, 0 },
  /* 17 */ { "mremap",    (capdb_syscall_ptr)ts_mremap,    0, 0, 0 },
           { 0, 0, 0, 0, 0 }
};
...
#define orig_pread64   ((ssize_t(*)(int,void*,size_t,capdb_uint64))aSyscall[10].xOrig)
#define orig_write     ((ssize_t(*)(int,const void*,size_t))aSyscall[11].xOrig)
#define orig_pwrite    ((ssize_t(*)(int,const void*,size_t,off_t))\
                       aSyscall[12].xOrig)
#define orig_pwrite64  ((ssize_t(*)(int,const void*,size_t,capdb_uint64))\
                       aSyscall[13].xOrig)
...
static int CAPDB_TCLAPI test_syscall_install(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
static int CAPDB_TCLAPI test_syscall_uninstall(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
static int CAPDB_TCLAPI test_syscall_reset(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  if( objc==2 ){
    ...
    for(i=0; rc==CAPDB_OK && aSyscall[i].zName; i++){
      if( strlen(aSyscall[i].zName)!=nFunc ) continue;
      if( memcmp(aSyscall[i].zName, zFunc, nFunc) ) continue;
      aSyscall[i].xOrig = 0;
    }
...
static int CAPDB_TCLAPI test_syscall_exists(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
static int CAPDB_TCLAPI test_syscall_fault(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
static int CAPDB_TCLAPI test_syscall_errno(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
static int CAPDB_TCLAPI test_syscall_list(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
static int CAPDB_TCLAPI test_syscall_defaultvfs(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
static int CAPDB_TCLAPI test_syscall_pagesize(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  if( pgsz<0 ){
    ...
    gSyscall.pgsz = pgsz;
    pVfs->xSetSystemCall(
        pVfs, "getpagesize", (capdb_syscall_ptr)ts_getpagesize
    );
  }
...
static int CAPDB_TCLAPI test_syscall(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
