...
**
*************************************************************************
** This file contains test logic for the capdb_backup() interface.
**
*/
...
static int CAPDB_TCLAPI backupTestCmd(
  ...
  capdb_backup *p = (capdb_backup *)clientData;
  int iCmd;
  int rc;
  ...
  switch( aSub[iCmd].eCmd ){
    ...
    case BACKUP_STEP: {
      ...
      rc = capdb_backup_step(p, nPage);
      Tcl_SetResult(interp, (char *)capdbErrName(rc), TCL_STATIC);
      break;
    }

    case BACKUP_REMAINING:
      Tcl_SetObjResult(interp, Tcl_NewIntObj(capdb_backup_remaining(p)));
      break;

    case BACKUP_PAGECOUNT:
      Tcl_SetObjResult(interp, Tcl_NewIntObj(capdb_backup_pagecount(p)));
      break;
  }
...
static void CAPDB_TCLAPI backupTestFinish(ClientData clientData){
  capdb_backup *pBackup = (capdb_backup *)clientData;
  capdb_backup_finish(pBackup);
}

/*
**     capdb_backup CMDNAME DESTHANDLE DESTNAME SRCHANDLE SRCNAME
**
*/
static int CAPDB_TCLAPI backupTestInit(
  ClientData clientData, 
  Tcl_Interp *interp, 
  int objc,
  Tcl_Obj *const*objv
  ...
  pBackup = capdb_backup_init(pDestDb, zDestName, pSrcDb, zSrcName);
  if( !pBackup ){
    Tcl_AppendResult(interp, "capdb_backup_init() failed", NULL);
    return TCL_ERROR;
  }
...
int Sqlitetestbackup_Init(Tcl_Interp *interp){
  Tcl_CreateObjCommand(interp, "capdb_backup", backupTestInit, 0, 0);
  return TCL_OK;
}
