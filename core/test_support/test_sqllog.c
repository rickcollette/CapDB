...
** USAGE
**
**   To use this module, SQLite must be compiled with the CAPDB_ENABLE_SQLLOG
...
**   of this file onto the end of the "capdb.c" amalgamation and then 
**   recompile the application as normal except with the addition  of the
**   -DCAPDB_ENABLE_SQLLOG option.
**
**   At runtime, logging is enabled by setting environment variable
**   CAPDB_SQLLOG_DIR to the name of a directory in which to store logged 
...
**   by setting the environment variable CAPDB_SQLLOG_REUSE_FILES to 0.
**
**   If the environment variable CAPDB_SQLLOG_CONDITIONAL is defined, then
...
** OUTPUT:
**
**   The CAPDB_SQLLOG_DIR is populated with three types of files:
**
...
**   logging proceeds as expected. Errors are logged by calling capdb_log().
*/
...
#define ENVIRONMENT_VARIABLE1_NAME "CAPDB_SQLLOG_DIR"
#define ENVIRONMENT_VARIABLE2_NAME "CAPDB_SQLLOG_REUSE_FILES"
#define ENVIRONMENT_VARIABLE3_NAME "CAPDB_SQLLOG_CONDITIONAL"
...
** open (if any more are opened an error is logged using capdb_log()
...
static struct SLGlobal {
  ...
  capdb_mutex *mutex;           /* Recursive mutex */
...
** eventually free it using capdb_free().
*/
static char *sqllogFindFile(const char *zFile){
  ...
  if( fd==0 ){
    capdb_log(CAPDB_IOERR, "sqllogFindFile(): error in fopen()");
    return 0;
  }
  ...
  ** copy and break out of the loop.  */
  while( feof(fd)==0 ){
    ...
    if( fgets(zLine, sizeof(zLine), fd) ){
      ...
      if( n==strlen(zFile) && 0==memcmp(zFile, z, n) ){
        ...
        zRet = capdb_mprintf("%s_%s.db", sqllogglobal.zPrefix, zBuf);
        break;
      }
    }
  }
  ...
  if( ferror(fd) ){
    capdb_log(CAPDB_IOERR, "sqllogFindFile(): error reading index file");
  }
...
static int sqllogFindAttached(
  ...
  ** described by the last row returned.  */
  ...
  sqllogglobal.bRec = 1;
  rc = capdb_prepare_v2(db, "PRAGMA database_list", -1, &pStmt, 0);
  if( rc==CAPDB_OK ){
    while( CAPDB_ROW==capdb_step(pStmt) ){
      const char *zVal1; int nVal1;
      const char *zVal2; int nVal2;

      zVal1 = (const char*)capdb_column_text(pStmt, 1);
      nVal1 = capdb_column_bytes(pStmt, 1);
      if( zName ){
        memcpy(zName, zVal1, nVal1+1);
      }

      zVal2 = (const char*)capdb_column_text(pStmt, 2);
      nVal2 = capdb_column_bytes(pStmt, 2);
      memcpy(zFile, zVal2, nVal2+1);

      if( zSearch && strlen(zSearch)==nVal1 
       && 0==capdb_strnicmp(zSearch, zVal1, nVal1)
      ){
        break;
      }
    }
  ...
  if( rc!=CAPDB_OK ){
    capdb_log(rc, "sqllogFindAttached(): error in \"PRAGMA database_list\"");
  }
  return rc;
}
...
static void sqllogCopydb(struct SLConn *p, const char *zSearch, int bLog){
  ...
  if( zFile[0]=='\0' ){
    ...
    if( zInit==0 ){
      ...
      zInit = capdb_mprintf("%s_%02d.db", sqllogglobal.zPrefix, iDb);

      /* Create the backup */
      assert( sqllogglobal.bRec==0 );
      sqllogglobal.bRec = 1;
      rc = capdb_open(zInit, &copy);
      if( rc==CAPDB_OK ){
        capdb_backup *pBak;
        capdb_exec(copy, "PRAGMA synchronous = 0", 0, 0, 0);
        pBak = capdb_backup_init(copy, "main", p->db, zName);
        if( pBak ){
          capdb_backup_step(pBak, -1);
          rc = capdb_backup_finish(pBak);
        }else{
          rc = capdb_errcode(copy);
        }
        capdb_close(copy);
      }
      sqllogglobal.bRec = 0;
      ...
      if( rc==CAPDB_OK ){
        ...
        capdb_log(rc, "sqllogCopydb(): error backing up database");
      }
    }
  }

  if( bLog ){
    zFree = capdb_mprintf("ATTACH '%q' AS '%q'; -- clock=%d\n", 
        zInit, zName, sqllogglobal.iClock++
    );
  }else{
    zFree = capdb_mprintf("-- Main database is '%q'\n", zInit);
  }
  ...
  if( p->fd==0 ){
    ...
    ** environment variable $ENVIRONMENT_VARIABLE1_NAME.  */
    if( sqllogglobal.zPrefix[0]==0 ){
      ...
      capdb_snprintf(sizeof(sqllogglobal.zPrefix), sqllogglobal.zPrefix,
                        "%s/sqllog_%05d", zVar, getProcessId());
      capdb_snprintf(sizeof(sqllogglobal.zIdx), sqllogglobal.zIdx,
    ...
    /* Open the log file */
    zLog = capdb_mprintf("%s_%05d.sql", sqllogglobal.zPrefix, p->iLog);
    p->fd = fopen(zLog, "w");
    capdb_free(zLog);
    if( p->fd==0 ){
      capdb_log(CAPDB_IOERR, "sqllogOpenlog(): Failed to open log file");
    }
  }
}
...
static void testSqllogStmt(struct SLConn *p, const char *zSql){
  ...
  if( nFirst!=6 || 0!=capdb_strnicmp("ATTACH", zFirst, 6) ){
...
**
** If an error occurs, capdb_log() is invoked to report it to the user
** and zero returned.
*/
static int sqllogTraceDb(capdb *db){
  int bRet = 1;
...
/*
** The CAPDB_CONFIG_SQLLOG callback registered by capdb_init_sqllog().
**
...
** into the capdb_config(CAPDB_CONFIG_SQLLOG) statement.  In this
** particular implementation, pCtx is always a pointer to the 
** sqllogglobal global variable define above.
*/
static void testSqllog(void *pCtx, capdb *db, const char *zSql, int eType){
  struct SLConn *p = 0;
  capdb_mutex *mainmtx = capdb_mutex_alloc(CAPDB_MUTEX_STATIC_MAIN);
  ...
  if( eType==0 ){
    capdb_mutex_enter(mainmtx);
    if( sqllogglobal.mutex==0 ){
      sqllogglobal.mutex = capdb_mutex_alloc(CAPDB_MUTEX_RECURSIVE);
    }
    capdb_mutex_leave(mainmtx);

    capdb_mutex_enter(sqllogglobal.mutex);
    ...
    capdb_mutex_leave(sqllogglobal.mutex);
  }
    ...
    if( eType==2 ){
      ...
      if( sqllogglobal.nConn==0 ){
        capdb_mutex_free(sqllogglobal.mutex);
...
/*
** This function is called either before capdb_initialized() or by it.
** It checks if the CAPDB_SQLLOG_DIR variable is defined, and if so 
** registers an CAPDB_CONFIG_SQLLOG callback to record the applications
** database activity.
*/
void capdb_init_sqllog(void){
  if( getenv(ENVIRONMENT_VARIABLE1_NAME) ){
    if( CAPDB_OK==capdb_config(CAPDB_CONFIG_SQLLOG, testSqllog, 0) ){
