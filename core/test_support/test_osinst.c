...
**
** All calls on capdb_file objects except xFileControl() are logged.
** Additionally, calls to the xAccess(), xOpen(), and xDelete()
** methods are logged. The other capdb_vfs object methods (xDlXXX,
...
**       );
**
**       int capdb_vfslog_finalize(const char *zVfs);
**
** ANNOTATING LOG FILES:
**
**   To write an arbitrary message into a log file:
**
**       int capdb_vfslog_annotate(const char *zVfs, const char *zMsg);
**
** READING LOG FILES:
**
...
**
**       int capdb_vfslog_register(capdb *db);
**
...
struct VfslogVfs {
  ...
  capdb_file *pLog;             /* Log file handle */
  capdb_int64 iOffset;          /* Log file offset of start of write buffer */
...
struct VfslogFile {
  ...
  capdb_vfs *pVfslog;           /* Associated VsflogVfs object */
...
*/
static int vfslogClose(capdb_file*);
static int vfslogRead(capdb_file*, void*, int iAmt, capdb_int64 iOfst);
static int vfslogWrite(capdb_file*,const void*,int iAmt, capdb_int64 iOfst);
static int vfslogTruncate(capdb_file*, capdb_int64 size);
static int vfslogSync(capdb_file*, int flags);
static int vfslogFileSize(capdb_file*, capdb_int64 *pSize);
static int vfslogLock(capdb_file*, int);
static int vfslogUnlock(capdb_file*, int);
static int vfslogCheckReservedLock(capdb_file*, int *pResOut);
static int vfslogFileControl(capdb_file*, int op, void *pArg);
static int vfslogSectorSize(capdb_file*);
static int vfslogDeviceCharacteristics(capdb_file*);

static int vfslogShmLock(capdb_file *pFile, int ofst, int n, int flags);
static int vfslogShmMap(capdb_file *pFile,int,int,int,volatile void **);
static void vfslogShmBarrier(capdb_file*);
static int vfslogShmUnmap(capdb_file *pFile, int deleteFlag);

/*
** Method declarations for vfslog_vfs.
*/
static int vfslogOpen(capdb_vfs*, const char *, capdb_file*, int , int *);
static int vfslogDelete(capdb_vfs*, const char *zName, int syncDir);
static int vfslogAccess(capdb_vfs*, const char *zName, int flags, int *);
static int vfslogFullPathname(capdb_vfs*, const char *zName, int, char *zOut);
static void *vfslogDlOpen(capdb_vfs*, const char *zFilename);
static void vfslogDlError(capdb_vfs*, int nByte, char *zErrMsg);
static void (*vfslogDlSym(capdb_vfs *pVfs, void *p, const char*zSym))(void);
static void vfslogDlClose(capdb_vfs*, void*);
static int vfslogRandomness(capdb_vfs*, int nByte, char *zOut);
static int vfslogSleep(capdb_vfs*, int microseconds);
static int vfslogCurrentTime(capdb_vfs*, double*);

static int vfslogGetLastError(capdb_vfs*, int, char *);
static int vfslogCurrentTimeInt64(capdb_vfs*, capdb_int64*);

static capdb_vfs vfslog_vfs = {
  1,                              /* iVersion */
...
static capdb_io_methods vfslog_io_methods = {
  2,                              /* iVersion */
  vfslogClose,                    /* xClose */
  vfslogRead,                     /* xRead */
  vfslogWrite,                    /* xWrite */
...
#ifdef _WIN32
#include <time.h>
static capdb_uint64 vfslog_time(){
  FILETIME ft;
...
static void vfslog_call(capdb_vfs *, int, int, capdb_int64, int, int, int);
static void vfslog_string(capdb_vfs *, const char *);

/*
** Close an vfslog-file.
*/
static int vfslogClose(capdb_file *pFile){
  capdb_uint64 t;
  int rc = CAPDB_OK;
...
*/
static int vfslogTruncate(capdb_file *pFile, sqlite_int64 size){
  int rc;
  capdb_uint64 t;
  VfslogFile *p = (VfslogFile *)pFile;
  t = vfslog_time();
...
** Sync an vfslog-file.
*/
static int vfslogSync(capdb_file *pFile, int flags){
  int rc;
  capdb_uint64 t;
  VfslogFile *p = (VfslogFile *)pFile;
  t = vfslog_time();
...
*/
static int vfslogFileSize(capdb_file *pFile, sqlite_int64 *pSize){
  int rc;
  capdb_uint64 t;
  VfslogFile *p = (VfslogFile *)pFile;
  t = vfslog_time();
...
** Lock an vfslog-file.
*/
static int vfslogLock(capdb_file *pFile, int eLock){
  int rc;
  capdb_uint64 t;
  VfslogFile *p = (VfslogFile *)pFile;
  t = vfslog_time();
...
*/
static int vfslogUnlock(capdb_file *pFile, int eLock){
  int rc;
  capdb_uint64 t;
  VfslogFile *p = (VfslogFile *)pFile;
  t = vfslog_time();
...
*/
static int vfslogCheckReservedLock(capdb_file *pFile, int *pResOut){
  int rc;
  capdb_uint64 t;
  VfslogFile *p = (VfslogFile *)pFile;
  t = vfslog_time();
...
*/
static int vfslogFileControl(capdb_file *pFile, int op, void *pArg){
  VfslogFile *p = (VfslogFile *)pFile;
  int rc = p->pReal->pMethods->xFileControl(p->pReal, op, pArg);
  if( op==CAPDB_FCNTL_VFSNAME && rc==CAPDB_OK ){
    *(char**)pArg = capdb_mprintf("vfslog/%z", *(char**)pArg);
  }
  return rc;
}
...
*/
static int vfslogSectorSize(capdb_file *pFile){
  int rc;
  capdb_uint64 t;
  VfslogFile *p = (VfslogFile *)pFile;
  t = vfslog_time();
...
*/
static int vfslogDeviceCharacteristics(capdb_file *pFile){
  int rc;
  capdb_uint64 t;
  VfslogFile *p = (VfslogFile *)pFile;
  t = vfslog_time();
...
static void vfslogShmBarrier(capdb_file *pFile){
  capdb_uint64 t;
  VfslogFile *p = (VfslogFile *)pFile;
  t = vfslog_time();
  p->pReal->pMethods->xShmBarrier(p->pReal);
  t = vfslog_time() - t;
  vfslog_call(p->pVfslog, OS_SHMBARRIER, p->iFileId, t, CAPDB_OK, 0, 0);
}
static int vfslogShmUnmap(capdb_file *pFile, int deleteFlag){
  int rc;
  capdb_uint64 t;
...
** returning.
*/
static int vfslogDelete(capdb_vfs *pVfs, const char *zPath, int dirSync){
  int rc;
  capdb_uint64 t;
  t = vfslog_time();
...
*/
static void *vfslogDlOpen(capdb_vfs *pVfs, const char *zPath){
...
*/
static void vfslogDlError(capdb_vfs *pVfs, int nByte, char *zErrMsg){
...
*/
static void vfslogDlClose(capdb_vfs *pVfs, void *pHandle){
...
** random data.
*/
static int vfslogRandomness(capdb_vfs *pVfs, int nByte, char *zBufOut){
...
** actually slept.
*/
static int vfslogSleep(capdb_vfs *pVfs, int nMicro){
...
*/
static int vfslogCurrentTime(capdb_vfs *pVfs, double *pTimeOut){
  return REALVFS(pVfs)->xCurrentTime(REALVFS(pVfs), pTimeOut);
}

static int vfslogGetLastError(capdb_vfs *pVfs, int a, char *b){
  return REALVFS(pVfs)->xGetLastError(REALVFS(pVfs), a, b);
}
static int vfslogCurrentTimeInt64(capdb_vfs *pVfs, capdb_int64 *p){
...
static void vfslog_flush(VfslogVfs *p){
  ...
  int pending = capdb_io_error_pending;
  int persist = capdb_io_error_persist;
  int diskfull = capdb_diskfull_pending;

  capdb_io_error_pending = 0;
  capdb_io_error_persist = 0;
  capdb_diskfull_pending = 0;
#endif
...
#ifdef CAPDB_TEST
  capdb_io_error_pending = pending;
  capdb_io_error_persist = persist;
  capdb_diskfull_pending = diskfull;
#endif
}
...
static void vfslog_string(capdb_vfs *pVfs, const char *zStr){
  VfslogVfs *p = (VfslogVfs *)pVfs;
  unsigned char *zRec;
...
int capdb_vfslog_new(
  ...
  pParent = capdb_vfs_find(zParentVfs);
  if( !pParent ){
    return CAPDB_ERROR;
  }
  ...
  p = (VfslogVfs *)capdb_malloc(nByte);
  memset(p, 0, nByte);

  p->pVfs = pParent;
  p->pLog = (capdb_file *)&p[1];
  memcpy(&p->base, &vfslog_vfs, sizeof(capdb_vfs));
  ...
  flags = CAPDB_OPEN_READWRITE|CAPDB_OPEN_CREATE|CAPDB_OPEN_SUPER_JOURNAL;
  ...
  if( rc==CAPDB_OK ){
    ...
    p->iOffset = 0;
    p->nBuf = 20;
    rc = capdb_vfs_register((capdb_vfs *)p, 1);
  }
  if( rc ){
    vfslog_finalize(p);
  }
...
struct VfslogVtab {
  ...
  capdb_file *pFd;              /* File descriptor open on vfslog file */
  capdb_int64 nByte;            /* Size of file in bytes */
...
struct VfslogCsr {
  ...
  capdb_int64 iRowid;           /* Current rowid. */
  capdb_int64 iOffset;          /* Offset of next record in file */
...
static int vlogConnect(
  ...
  capdb_vfs *pVfs;              /* VFS used to read log file */
  ...
  *ppVtab = 0;
  ...
  p->pFd = (capdb_file *)&p[1];
  p->zFile = &((char *)p->pFd)[pVfs->szOsFile];

  zFile = capdb_mprintf("%s", argv[3]);
  if( !zFile ){
    capdb_free(p);
    return CAPDB_NOMEM;
  }
...
*/
static int vlogBestIndex(capdb_vtab *tab, capdb_index_info *pIdxInfo){
...
*/
static int vlogDisconnect(capdb_vtab *pVtab){
...
*/
static int vlogOpen(capdb_vtab *pVTab, capdb_vtab_cursor **ppCursor){
  VfslogCsr *pCsr;                /* Newly allocated cursor object */

  pCsr = capdb_malloc(sizeof(VfslogCsr));
  if( !pCsr ) return CAPDB_NOMEM;
...
*/
static int vlogClose(capdb_vtab_cursor *pCursor){
  VfslogCsr *p = (VfslogCsr *)pCursor;
  int i;
  for(i=0; i<p->nFile; i++){
    capdb_free(p->azFile[i]);
  }
  capdb_free(p->azFile);
  capdb_free(p->zTransient);
  capdb_free(p);
  return CAPDB_OK;
}
...
*/
static int vlogNext(capdb_vtab_cursor *pCursor){
  VfslogCsr *pCsr = (VfslogCsr *)pCursor;
  VfslogVtab *p = (VfslogVtab *)pCursor->pVtab;
  int rc = CAPDB_OK;
  int nRead;

  capdb_free(pCsr->zTransient);
  pCsr->zTransient = 0;
...
static int vlogEof(capdb_vtab_cursor *pCursor){
  VfslogCsr *pCsr = (VfslogCsr *)pCursor;
  VfslogVtab *p = (VfslogVtab *)pCursor->pVtab;
...
static int vlogColumn(
  ...
  switch( i ){
    case 0: {
      capdb_result_text(ctx, vfslog_eventname(val), -1, CAPDB_STATIC);
      break;
    }
    case 1: {
      ...
      capdb_result_text(ctx, zStr, -1, CAPDB_TRANSIENT);
      break;
    }
    default:
      capdb_result_int(ctx, val);
      break;
  }
...
static int vlogRowid(capdb_vtab_cursor *pCursor, sqlite_int64 *pRowid){
  VfslogCsr *pCsr = (VfslogCsr *)pCursor;
  *pRowid = pCsr->iRowid;
  return CAPDB_OK;
}
...
int capdb_vfslog_register(capdb *db){
  static capdb_module vfslog_module = {
    0,                            /* iVersion */
    vlogConnect,                /* xCreate */
  ...
  capdb_create_module(db, "vfslog", &vfslog_module, 0);
  return CAPDB_OK;
}
...
#if defined(CAPDB_TEST) || defined(TCLSH)

#include "tclsqlite.h"

static int CAPDB_TCLAPI test_vfslog(
  void *clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  switch( (enum VL_enum)iSub ){
    case VL_ANNOTATE: {
      ...
      rc = capdb_vfslog_annotate(zVfs, zMsg);
      if( rc!=CAPDB_OK ){
    ...
    case VL_NEW: {
      ...
      rc = capdb_vfslog_new(zVfs, zParent, zLog);
      if( rc!=CAPDB_OK ){
        Tcl_AppendResult(interp, "failed", (char*)0);
        return TCL_ERROR;
      }
    ...
    case VL_REGISTER: {
      ...
      if( Tcl_GetCommandInfo(interp, zDb, &cmdInfo) ){
        ...
        rc = capdb_vfslog_register(db);
      }
      if( rc!=CAPDB_OK ){
        Tcl_AppendResult(interp, "bad capdb handle: ", zDb, (void*)0);
        return TCL_ERROR;
      }
      break;
#endif
