...
** write requests that attempt to enlarge a file fail with CAPDB_FULL.
**
** However, before returning CAPDB_FULL, the write requests invoke
...
**
** A quota group must be established (using capdb_quota_set(...))
...
struct quotaGroup {
  ...
  capdb_int64 iLimit;          /* Upper bound on total file size */
  capdb_int64 iSize;           /* Current size of all files */
  void (*xCallback)(             /* Callback invoked when going over quota */
     const char *zFilename,         /* Name of file whose size increases */
     capdb_int64 *piLimit,        /* IN/OUT: The current limit */
     capdb_int64 iSize,           /* Total size of all files in the group */
...
struct quotaFile {
  ...
  capdb_int64 iSize;            /* Current size of this file */
...
struct quotaConn {
  ...
  /* The underlying VFS capdb_file is appended to this object */
};
...
struct quota_FILE {
  ...
  capdb_int64 iOfst;    /* Current offset into the file */
...
** list of quotaGroups.
*/
static void quotaEnter(void){ capdb_mutex_enter(gQuota.pMutex); }
static void quotaLeave(void){ capdb_mutex_leave(gQuota.pMutex); }
...
/* Translate an capdb_file* that is really a quotaConn* into
** the capdb_file* for the underlying original VFS.
*/
static capdb_file *quotaSubOpen(capdb_file *pConn){
  quotaConn *p = (quotaConn*)pConn;
  return (capdb_file*)&p[1];
}
...
static quotaFile *quotaFindFile(
  ...
  if( pFile==0 && createFlag ){
    ...
    pFile = (quotaFile *)capdb_malloc( sizeof(*pFile) + nName + 1 );
    if( pFile ){
...
static char *quota_utf8_to_mbcs(const char *zUtf8){
  ...
  if( nWide==0 ) return 0;
  zTmpWide = (LPWSTR)capdb_malloc( (nWide+1)*sizeof(zTmpWide[0]) );
  ...
  zMbcs = nMbcs ? (char*)capdb_malloc( nMbcs+1 ) : 0;
  if( zMbcs ){
...
static int quotaOpen(
  ...
  capdb_file *pSubOpen;                    /* Real file descriptor */
  capdb_vfs *pOrigVfs = gQuota.pOrigVfs;   /* Real VFS */

  /* If the file is not a main database file or a WAL, then use the
  ** normal xOpen method.
  */
  if( (flags & (CAPDB_OPEN_MAIN_DB|CAPDB_OPEN_WAL))==0 ){
  ...
  if( pGroup==0 ){
    ...
    */
    ...
    if( rc==CAPDB_OK ){
      ...
      pFile->deleteOnClose = (flags & CAPDB_OPEN_DELETEONCLOSE)!=0;
      pFile->nRef++;
...
*/
static int quotaClose(capdb_file *pConn){
  quotaConn *p = (quotaConn*)pConn;
  quotaFile *pFile = p->pFile;
  capdb_file *pSubOpen = quotaSubOpen(pConn);
  int rc;
...
*/
static int quotaTruncate(capdb_file *pConn, capdb_int64 size){
  quotaConn *p = (quotaConn*)pConn;
  capdb_file *pSubOpen = quotaSubOpen(pConn);
...
*/
static int quotaSync(capdb_file *pConn, int flags){
  capdb_file *pSubOpen = quotaSubOpen(pConn);
...
*/
static int quotaFileSize(capdb_file *pConn, capdb_int64 *pSize){
  quotaConn *p = (quotaConn*)pConn;
  capdb_file *pSubOpen = quotaSubOpen(pConn);
  quotaFile *pFile = p->pFile;
  quotaGroup *pGroup;
...
*/
static int quotaLock(capdb_file *pConn, int lock){
  capdb_file *pSubOpen = quotaSubOpen(pConn);
...
*/
static int quotaUnlock(capdb_file *pConn, int lock){
  capdb_file *pSubOpen = quotaSubOpen(pConn);
...
*/
static int quotaCheckReservedLock(capdb_file *pConn, int *pResOut){
  capdb_file *pSubOpen = quotaSubOpen(pConn);
...
*/
static int quotaFileControl(capdb_file *pConn, int op, void *pArg){
  capdb_file *pSubOpen = quotaSubOpen(pConn);
  int rc = pSubOpen->pMethods->xFileControl(pSubOpen, op, pArg);
#if defined(CAPDB_FCNTL_VFSNAME)
  if( op==CAPDB_FCNTL_VFSNAME && rc==CAPDB_OK ){
    *(char**)pArg = capdb_mprintf("quota/%z", *(char**)pArg);
  }
#endif
  return rc;
}
...
*/
static int quotaSectorSize(capdb_file *pConn){
  capdb_file *pSubOpen = quotaSubOpen(pConn);
...
*/
static int quotaDeviceCharacteristics(capdb_file *pConn){
  capdb_file *pSubOpen = quotaSubOpen(pConn);
...
*/
static void quotaShmBarrier(capdb_file *pConn){
  capdb_file *pSubOpen = quotaSubOpen(pConn);
...
*/
static int quotaShmUnmap(capdb_file *pConn, int deleteFlag){
  capdb_file *pSubOpen = quotaSubOpen(pConn);
...
** during start-up.
*/
int capdb_quota_initialize(const char *zOrigVfsName, int makeDefault){
  capdb_vfs *pOrigVfs;
  if( gQuota.isInitialized ) return CAPDB_MISUSE;
  pOrigVfs = capdb_vfs_find(zOrigVfsName);
  if( pOrigVfs==0 ) return CAPDB_ERROR;
  assert( pOrigVfs!=&gQuota.sThisVfs );
  gQuota.pMutex = capdb_mutex_alloc(CAPDB_MUTEX_FAST);
  if( !gQuota.pMutex ){
    return CAPDB_NOMEM;
  }
  ...
  capdb_vfs_register(&gQuota.sThisVfs, makeDefault);
  return CAPDB_OK;
}
...
*/
int capdb_quota_shutdown(void){
  quotaGroup *pGroup;
  if( gQuota.isInitialized==0 ) return CAPDB_MISUSE;
  for(pGroup=gQuota.pGroup; pGroup; pGroup=pGroup->pNext){
    if( quotaGroupOpenFileCount(pGroup)>0 ) return CAPDB_MISUSE;
  }
  while( gQuota.pGroup ){
  ...
  capdb_mutex_free(gQuota.pMutex);
  capdb_vfs_unregister(&gQuota.sThisVfs);
...
int capdb_quota_set(
  ...
  capdb_int64 iLimit,           /* New quota to set for this quota group */
  void (*xCallback)(              /* Callback invoked when going over quota */
     const char *zFilename,         /* Name of file whose size increases */
     capdb_int64 *piLimit,        /* IN/OUT: The current limit */
     capdb_int64 iSize,           /* Total size of all files in the group */
  ...
  if( pGroup==0 ){
    ...
    pGroup = (quotaGroup *)capdb_malloc( sizeof(*pGroup) + nPattern + 1 );
    if( pGroup==0 ){
      quotaLeave();
      return CAPDB_NOMEM;
    }
...
*/
int capdb_quota_file(const char *zFilename){
  char *zFull = 0;
  capdb_file *fd;
  int rc;
  int outFlags = 0;
  ...
  fd = (capdb_file *)capdb_malloc(nAlloc);
  if( fd==0 ){
    rc = CAPDB_NOMEM;
  }else{
  ...
  if( rc==CAPDB_OK ){
    ...
    rc = quotaOpen(&gQuota.sThisVfs, zFull, fd,
                   CAPDB_OPEN_READONLY | CAPDB_OPEN_MAIN_DB, &outFlags);
    if( rc==CAPDB_OK ){
...
*/
quota_FILE *capdb_quota_fopen(const char *zFilename, const char *zMode){
  quota_FILE *p = 0;
  char *zFull = 0;
  ...
  zFull = (char*)capdb_malloc(gQuota.sThisVfs.mxPathname + 1);
  if( zFull==0 ) return 0;
  rc = gQuota.pOrigVfs->xFullPathname(gQuota.pOrigVfs, zFilename,
                                      gQuota.sThisVfs.mxPathname+1, zFull);
  if( rc ) goto quota_fopen_error;
  p = (quota_FILE*)capdb_malloc(sizeof(*p));
...
size_t capdb_quota_fwrite(
  ...
  ** downward */
  if( rc<nmemb && pFile ){
    size_t nWritten = rc;
    capdb_int64 iNewEnd = iOfst + size*nWritten;
...
*/
int capdb_quota_fclose(quota_FILE *p){
  int rc;
  quotaFile *pFile;
  rc = fclose(p->f);
  pFile = p->pFile;
...
*/
int capdb_quota_fflush(quota_FILE *p, int doFsync){
  int rc;
  rc = fflush(p->f);
  if( rc==0 && doFsync ){
#ifdef _WIN32
...
*/
int capdb_quota_fseek(quota_FILE *p, long offset, int whence){
...
*/
void capdb_quota_rewind(quota_FILE *p){
  rewind(p->f);
}
...
*/
long capdb_quota_ftell(quota_FILE *p){
  return ftell(p->f);
}
...
*/
int capdb_quota_ferror(quota_FILE *p){
  return ferror(p->f);
}
...
*/
int capdb_quota_ftruncate(quota_FILE *p, capdb_int64 szNew){
  quotaFile *pFile = p->pFile;
  int rc;
...
#ifdef _WIN32
#  if defined(__MSVCRT__) && defined(CAPDB_TEST)
...
*/
int capdb_quota_file_mtime(quota_FILE *p, time_t *pTime){
  int rc;
#ifdef _WIN32
  struct _stati64 buf;
...
** system.
*/
capdb_int64 capdb_quota_file_truesize(quota_FILE *p){
  int rc;
#ifdef _WIN32
  struct _stati64 buf;
  rc = _stati64(p->zMbcsName, &buf);
...
*/
capdb_int64 capdb_quota_file_size(quota_FILE *p){
...
** in the given file.
*/
long capdb_quota_file_available(quota_FILE *p){
  FILE* f = p->f;
  long pos1, pos2;
  int rc;
  pos1 = ftell(f);
...
*/
int capdb_quota_remove(const char *zFilename){
  ...
  */
  ...
  pGroup = quotaGroupFind(zFull);
  if( pGroup ){
    for(pFile=pGroup->pFiles; pFile && rc==CAPDB_OK; pFile=pNextFile){
...
static void tclQuotaCallback(
  ...
  capdb_randomness(sizeof(rnd), (void *)&rnd);
...
/*
** tclcmd: capdb_quota_initialize NAME MAKEDEFAULT
*/
static int CAPDB_TCLAPI test_quota_initialize(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  /* Call capdb_quota_initialize() */
  rc = capdb_quota_initialize(zName, makeDefault);
...
/*
** tclcmd: capdb_quota_shutdown
*/
static int CAPDB_TCLAPI test_quota_shutdown(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  /* Call capdb_quota_shutdown() */
  rc = capdb_quota_shutdown();
  Tcl_SetResult(interp, (char *)capdbErrName(rc), TCL_STATIC);
...
/*
** tclcmd: capdb_quota_set PATTERN LIMIT SCRIPT
*/
static int CAPDB_TCLAPI test_quota_set(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  void (*xDestroy)(void*);        /* Optional destructor for pArg */
  void (*xCallback)(const char *, capdb_int64 *, capdb_int64, void *);
  ...
  if( nScript>0 ){
    ...
    p = (TclQuotaCallback *)capdb_malloc(sizeof(TclQuotaCallback));
    if( !p ){
      Tcl_SetResult(interp, (char *)"CAPDB_NOMEM", TCL_STATIC);
      return TCL_OK;
    }
  ...
  /* Invoke capdb_quota_set() */
  rc = capdb_quota_set(zPattern, iLimit, xCallback, (void*)p, xDestroy);
...
/*
** tclcmd: capdb_quota_file FILENAME
*/
static int CAPDB_TCLAPI test_quota_file(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  /* Invoke capdb_quota_file() */
  rc = capdb_quota_file(zFilename);
...
/*
** tclcmd:  capdb_quota_dump
*/
static int CAPDB_TCLAPI test_quota_dump(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  for(pGroup=gQuota.pGroup; pGroup; pGroup=pGroup->pNext){
    ...
    for(pFile=pGroup->pFiles; pFile; pFile=pFile->pNext){
      ...
      capdb_snprintf(sizeof(zTemp), zTemp, "%s", pFile->zFilename);
...
/*
** tclcmd: capdb_quota_fopen FILENAME MODE
*/
static int CAPDB_TCLAPI test_quota_fopen(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  p = capdb_quota_fopen(zFilename, zMode);
  capdb_snprintf(sizeof(zReturn), zReturn, "%p", p);
...
/*
** tclcmd: capdb_quota_fread HANDLE SIZE NELEM
*/
static int CAPDB_TCLAPI test_quota_fread(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  p = capdbTestTextToPtr(Tcl_GetString(objv[1]));
  if( Tcl_GetIntFromObj(interp, objv[2], &sz) ) return TCL_ERROR;
  if( Tcl_GetIntFromObj(interp, objv[3], &nElem) ) return TCL_ERROR;
  zBuf = (char*)capdb_malloc( sz*nElem + 1 );
  if( zBuf==0 ){
    Tcl_SetResult(interp, "out of memory", TCL_STATIC);
    return TCL_ERROR;
  }
  got = capdb_quota_fread(zBuf, sz, nElem, p);
  zBuf[got*sz] = 0;
...
/*
** tclcmd: capdb_quota_fwrite HANDLE SIZE NELEM CONTENT
*/
static int CAPDB_TCLAPI test_quota_fwrite(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  got = capdb_quota_fwrite(zBuf, sz, nElem, p);
  Tcl_SetObjResult(interp, Tcl_NewWideIntObj(got));
  return TCL_OK;
}

/*
** tclcmd: capdb_quota_fclose HANDLE
*/
static int CAPDB_TCLAPI test_quota_fclose(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  p = capdbTestTextToPtr(Tcl_GetString(objv[1]));
  rc = capdb_quota_fclose(p);
  Tcl_SetObjResult(interp, Tcl_NewIntObj(rc));
  return TCL_OK;
}

/*
** tclcmd: capdb_quota_fflush HANDLE ?HARDSYNC?
*/
static int CAPDB_TCLAPI test_quota_fflush(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  rc = capdb_quota_fflush(p, doSync);
  Tcl_SetObjResult(interp, Tcl_NewIntObj(rc));
  return TCL_OK;
}

/*
** tclcmd: capdb_quota_fseek HANDLE OFFSET WHENCE
*/
static int CAPDB_TCLAPI test_quota_fseek(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  rc = capdb_quota_fseek(p, ofst, whence);
  Tcl_SetObjResult(interp, Tcl_NewIntObj(rc));
  return TCL_OK;
}

/*
** tclcmd: capdb_quota_rewind HANDLE
*/
static int CAPDB_TCLAPI test_quota_rewind(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  p = capdbTestTextToPtr(Tcl_GetString(objv[1]));
  capdb_quota_rewind(p);
  return TCL_OK;
}

/*
** tclcmd: capdb_quota_ftell HANDLE
*/
static int CAPDB_TCLAPI test_quota_ftell(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  p = capdbTestTextToPtr(Tcl_GetString(objv[1]));
  x = capdb_quota_ftell(p);
  Tcl_SetObjResult(interp, Tcl_NewWideIntObj(x));
  return TCL_OK;
}

/*
** tclcmd: capdb_quota_ftruncate HANDLE SIZE
*/
static int CAPDB_TCLAPI test_quota_ftruncate(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  x = (capdb_int64)w;
  rc = capdb_quota_ftruncate(p, x);
  Tcl_SetObjResult(interp, Tcl_NewIntObj(rc));
  return TCL_OK;
}

/*
** tclcmd: capdb_quota_file_size HANDLE
*/
static int CAPDB_TCLAPI test_quota_file_size(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  p = capdbTestTextToPtr(Tcl_GetString(objv[1]));
  x = capdb_quota_file_size(p);
  Tcl_SetObjResult(interp, Tcl_NewWideIntObj(x));
  return TCL_OK;
}

/*
** tclcmd: capdb_quota_file_truesize HANDLE
*/
static int CAPDB_TCLAPI test_quota_file_truesize(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  p = capdbTestTextToPtr(Tcl_GetString(objv[1]));
  x = capdb_quota_file_truesize(p);
  Tcl_SetObjResult(interp, Tcl_NewWideIntObj(x));
  return TCL_OK;
}

/*
** tclcmd: capdb_quota_file_mtime HANDLE
*/
static int CAPDB_TCLAPI test_quota_file_mtime(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
/*
** tclcmd: capdb_quota_remove FILENAME
*/
static int CAPDB_TCLAPI test_quota_remove(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  zFilename = Tcl_GetString(objv[1]);
  rc = capdb_quota_remove(zFilename);
  Tcl_SetObjResult(interp, Tcl_NewIntObj(rc));
  return TCL_OK;
}

/*
** tclcmd: capdb_quota_glob PATTERN TEXT
**
** Test the glob pattern matching.  Return 1 if TEXT matches PATTERN
** and return 0 if it does not.
*/
static int CAPDB_TCLAPI test_quota_glob(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
/*
** tclcmd: capdb_quota_file_available HANDLE
**
** Return the number of bytes from the current file point to the end of
** the file.
*/
static int CAPDB_TCLAPI test_quota_file_available(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  p = capdbTestTextToPtr(Tcl_GetString(objv[1]));
  x = capdb_quota_file_available(p);
  Tcl_SetObjResult(interp, Tcl_NewWideIntObj(x));
  return TCL_OK;
}

/*
** tclcmd: capdb_quota_ferror HANDLE
**
** Return true if the file handle is in the error state.
*/
static int CAPDB_TCLAPI test_quota_ferror(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
int Sqlitequota_Init(Tcl_Interp *interp){
  static struct {
    ...
    { "capdb_quota_initialize",    test_quota_initialize },
    { "capdb_quota_shutdown",      test_quota_shutdown },
    { "capdb_quota_set",           test_quota_set },
    { "capdb_quota_file",          test_quota_file },
    { "capdb_quota_dump",          test_quota_dump },
    { "capdb_quota_fopen",         test_quota_fopen },
    { "capdb_quota_fread",         test_quota_fread },
    { "capdb_quota_fwrite",        test_quota_fwrite },
    { "capdb_quota_fclose",        test_quota_fclose },
    { "capdb_quota_fflush",        test_quota_fflush },
    { "capdb_quota_fseek",         test_quota_fseek },
    { "capdb_quota_rewind",        test_quota_rewind },
    { "capdb_quota_ftell",         test_quota_ftell },
    { "capdb_quota_ftruncate",     test_quota_ftruncate },
    { "capdb_quota_file_size",     test_quota_file_size },
    { "capdb_quota_file_truesize", test_quota_file_truesize },
    { "capdb_quota_file_mtime",    test_quota_file_mtime },
    { "capdb_quota_remove",        test_quota_remove },
    { "capdb_quota_glob",          test_quota_glob },
    { "capdb_quota_file_available",test_quota_file_available },
    { "capdb_quota_ferror",        test_quota_ferror },
  };
  int i;
