...
** capdb_wsd_init() and capdb_wsd_find() functions required if the
** CAPDB_OMIT_WSD symbol is defined at build time.
*/

#if defined(CAPDB_OMIT_WSD) && defined(CAPDB_TEST)
...
int capdb_wsd_init(int N, int J){
  ...
  return pGlobal ? CAPDB_OK : CAPDB_NOMEM;
}
