...
**
** The code in this file used to be found in tclcapdb.c, contained within
** #if CAPDB_TEST ... #endif.  It is factored out into this separate module
...
/* Forward declaration */
static int CAPDB_TCLAPI load_testfixture_extensions(
  ClientData cd,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
**
**   * the [capdb] extension itself,
**
**   * If CAPDB_TCLMD5 or CAPDB_TEST is defined, the Md5 commands, and
**
**   * If CAPDB_TEST is set, the various test interfaces used by the Tcl
**     test suite.
*/
const char *capdbTestInit(Tcl_Interp *interp){
  ...
  /* pool Tcl hooks removed; use capsuite / threadtest_pool */
#endif
#if defined(CAPDB_ENABLE_NETWORK)
  /* capdb Tcl hooks removed; use capsuite / capdbtest */
#endif
  ...
  ** be sure to generate core files if we crash */
  ...
  if( Tcl_GetCommandInfo(interp, "capdb", &cmdInfo)==0 ){
    Sqlite3_Init(interp);
  }
#ifdef CAPDB_ENABLE_ZIPVFS
  Zipvfs_Init(interp);
...
#endif
#if defined(CAPDB_ENABLE_POOL)
  /* pool: no Tcl test hooks */
#endif
#if defined(CAPDB_ENABLE_NETWORK)
  /* capdb: no Tcl test hooks */
#endif
  Fts5tcl_Init(interp);
