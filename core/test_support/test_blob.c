...
extern void *capdbTestTextToPtr(const char *z);

/*
** Return a pointer to a buffer containing a text representation of the
** pointer passed as the only argument. The original pointer may be extracted
** from the text using capdbTestTextToPtr().
*/
static char *ptrToText(void *p){
  static char buf[100];
  capdb_snprintf(sizeof(buf)-1, buf, "%p", p);
  return buf;
}

/*
** Attempt to extract a blob handle (type capdb_blob*) from the Tcl
...
static int blobHandleFromObj(
  ...
  if( n==0 ){
    ...
    *ppBlob = (capdb_blob*)capdbTestTextToPtr(z);
  }
...
/*
** capdb_blob_open DB DATABASE TABLE COLUMN ROWID FLAGS VARNAME
**
** Tcl test harness for the capdb_blob_open() function.
*/
static int CAPDB_TCLAPI test_blob_open(
  ...
  capdb_blob *pBlob = (capdb_blob*)&flags;   /* Non-zero initialization */
  int rc;
  ...
  if( nVarname>0 ){
    rc = capdb_blob_open(db, zDb, zTable, zColumn, iRowid, flags, &pBlob);
    Tcl_SetVar(interp, zVarname, ptrToText(pBlob), 0);
  }else{
    rc = capdb_blob_open(db, zDb, zTable, zColumn, iRowid, flags, 0);
  }
...
/*
** capdb_blob_close  HANDLE
*/
static int CAPDB_TCLAPI test_blob_close(
  ClientData clientData, /* Not used */
...
/*
** capdb_blob_bytes  HANDLE
*/
static int CAPDB_TCLAPI test_blob_bytes(
  ClientData clientData, /* Not used */
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int objc,              /* Number of arguments */
  Tcl_Obj *CONST objv[]  /* Command arguments */
...
/*
** capdb_blob_read  CHANNEL OFFSET N
**
**   This command is used to test the capdb_blob_read() in ways that
**   the Tcl channel interface does not. The first argument should
**   be the name of a valid channel created by the [incrblob] method
**   of a database handle. This function calls capdb_blob_read()
**   to read N bytes from offset OFFSET from the underlying SQLite
**   blob handle.
**
**   On success, a byte-array object containing the read data is 
**   returned. On failure, the interpreter result is set to the
**   text representation of the returned error code (i.e. "CAPDB_NOMEM")
**   and a Tcl exception is thrown.
*/
static int CAPDB_TCLAPI test_blob_read(
  ClientData clientData, /* Not used */
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int objc,              /* Number of arguments */
  Tcl_Obj *CONST objv[]  /* Command arguments */
  ...
  rc = capdb_blob_read(pBlob, zBuf, nByte, iOffset);
  if( rc==CAPDB_OK ){
...
/*
** capdb_blob_write HANDLE OFFSET DATA ?NDATA?
**
**   This command is used to test the capdb_blob_write() in ways that
**   the Tcl channel interface does not. The first argument should
**   be the name of a valid channel created by the [incrblob] method
**   of a database handle. This function calls capdb_blob_write()
**   to write the DATA byte-array to the underlying SQLite blob handle.
**   at offset OFFSET.
**
**   On success, an empty string is returned. On failure, the interpreter
**   result is set to the text representation of the returned error code 
**   (i.e. "CAPDB_NOMEM") and a Tcl exception is thrown.
*/
static int CAPDB_TCLAPI test_blob_write(
  ClientData clientData, /* Not used */
  Tcl_Interp *interp,    /* The TCL interpreter that invoked this command */
  int objc,              /* Number of arguments */
  Tcl_Obj *CONST objv[]  /* Command arguments */
  ...
  rc = capdb_blob_write(pBlob, zBuf, n, iOffset);
  if( rc!=CAPDB_OK ){
    Tcl_SetResult(interp, (char *)capdbErrName(rc), TCL_VOLATILE);
  }
...
int Sqlitetest_blob_Init(Tcl_Interp *interp){
  ...
  static struct {
     ...
     { "capdb_blob_open",            test_blob_open        },
     { "capdb_blob_close",           test_blob_close       },
     { "capdb_blob_bytes",           test_blob_bytes       },
     { "capdb_blob_read",            test_blob_read        },
     { "capdb_blob_write",           test_blob_write       },
  };
  int i;
