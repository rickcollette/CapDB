...
struct jt_file {
  ...
  capdb_int64 iMaxOff;   /* Maximum offset written to this transaction */

  jt_file *pNext;          /* All files are stored in a linked list */
  capdb_file *pReal;     /* The file handle for the underlying vfs */
};

/*
** Method declarations for jt_file.
*/
static int jtClose(capdb_file*);
static int jtRead(capdb_file*, void*, int iAmt, capdb_int64 iOfst);
static int jtWrite(capdb_file*,const void*,int iAmt, capdb_int64 iOfst);
static int jtTruncate(capdb_file*, capdb_int64 size);
static int jtSync(capdb_file*, int flags);
static int jtFileSize(capdb_file*, capdb_int64 *pSize);
static int jtLock(capdb_file*, int);
static int jtUnlock(capdb_file*, int);
static int jtCheckReservedLock(capdb_file*, int *);
static int jtFileControl(capdb_file*, int op, void *pArg);
static int jtSectorSize(capdb_file*);
static int jtDeviceCharacteristics(capdb_file*);

/*
** Method declarations for jt_vfs.
*/
static int jtOpen(capdb_vfs*, const char *, capdb_file*, int , int *);
static int jtDelete(capdb_vfs*, const char *zName, int syncDir);
static int jtAccess(capdb_vfs*, const char *zName, int flags, int *);
static int jtFullPathname(capdb_vfs*, const char *zName, int, char *zOut);
static void *jtDlOpen(capdb_vfs*, const char *zFilename);
static void jtDlError(capdb_vfs*, int nByte, char *zErrMsg);
static void (*jtDlSym(capdb_vfs*,void*, const char *zSymbol))(void);
static void jtDlClose(capdb_vfs*, void*);
static int jtRandomness(capdb_vfs*, int nByte, char *zOut);
static int jtSleep(capdb_vfs*, int microseconds);
static int jtCurrentTime(capdb_vfs*, double*);
static int jtCurrentTimeInt64(capdb_vfs*, capdb_int64*);
static int jtGetLastError(capdb_vfs*, int, char*);

static capdb_vfs jt_vfs = {
  2,                             /* iVersion */
...
static capdb_io_methods jt_io_methods = {
  1,                             /* iVersion */
  jtClose,                       /* xClose */
  jtRead,                        /* xRead */
  jtWrite,                       /* xWrite */
...
static void enterJtMutex(void){
  capdb_mutex_enter(capdb_mutex_alloc(CAPDB_MUTEX_STATIC_PRNG));
}
static void leaveJtMutex(void){
  capdb_mutex_leave(capdb_mutex_alloc(CAPDB_MUTEX_STATIC_PRNG));
}

extern int capdb_io_error_pending;
extern int capdb_io_error_hit;
static void stop_ioerr_simulation(int *piSave, int *piSave2){
  *piSave = capdb_io_error_pending;
  *piSave2 = capdb_io_error_hit;
  capdb_io_error_pending = -1;
  capdb_io_error_hit = 0;
}
static void start_ioerr_simulation(int iSave, int iSave2){
  capdb_io_error_pending = iSave;
  capdb_io_error_hit = iSave2;
}
...
static void closeTransaction(jt_file *p){
  capdbBitvecDestroy(p->pWritable);
  capdb_free(p->aCksum);
  p->pWritable = 0;
  p->aCksum = 0;
  p->nSync = 0;
...
** Close an jt-file.
*/
static int jtClose(capdb_file *pFile){
  jt_file **pp;
...
**
**   a) CAPDB_OPEN_MAIN_DB was specified when the file was opened.
**
...
static jt_file *locateDatabaseHandle(const char *zJournal, int noLock){
  ...
  for(pMain=g.pList; pMain; pMain=pMain->pNext){
    ...
    if( (pMain->flags&CAPDB_OPEN_MAIN_DB)
     && ((int)strlen(pMain->zName)==nName)
     && 0==memcmp(pMain->zName, zJournal, nName)
     && ((pMain->eLock>=CAPDB_LOCK_RESERVED) || noLock)
    ){
      break;
    }
  }
...
**
** CAPDB_OK is returned if the journal-header is successfully 
** decoded. Otherwise, CAPDB_ERROR.
*/
static int decodeJournalHdr(
  ...
  if( memcmp(aMagic, zBuf, 8) ) return CAPDB_ERROR;
...
static int openTransaction(jt_file *pMain, jt_file *pJournal){
  unsigned char *aData;
  capdb_file *p = pMain->pReal;
  int rc = CAPDB_OK;

  closeTransaction(pMain);
  aData = capdb_malloc(pMain->nPagesize);
  pMain->pWritable = capdbBitvecCreate(pMain->nPage);
  pMain->aCksum = capdb_malloc(sizeof(u32) * (pMain->nPage + 1));
  pJournal->iMaxOff = 0;
  ...
  if( !pMain->pWritable || !pMain->aCksum || !aData ){
    ...
    */
    rc = capdbOsRead(p, aData, pMain->nPagesize, 0);
    if( rc==CAPDB_OK ){
      u32 nDbsize = decodeUint32(&aData[28]);
      if( nDbsize>0 && memcmp(&aData[24], &aData[92], 4)==0 ){
        u32 iPg;
        for(iPg=nDbsize+1; iPg<=pMain->nPage; iPg++){
          capdbBitvecSet(pMain->pWritable, iPg);
        }
      }
    }
    iTrunk = decodeUint32(&aData[32]);
    while( rc==CAPDB_OK && iTrunk>0 ){
      u32 nLeaf;
      u32 iLeaf;
      capdb_int64 iOff = (i64)(iTrunk-1)*pMain->nPagesize;
      rc = capdbOsRead(p, aData, pMain->nPagesize, iOff);
      nLeaf = decodeUint32(&aData[4]);
      for(iLeaf=0; rc==CAPDB_OK && iLeaf<nLeaf; iLeaf++){
        u32 pgno = decodeUint32(&aData[8+4*iLeaf]);
        capdbBitvecSet(pMain->pWritable, pgno);
      }
    ...
    if( rc==CAPDB_OK ){
      int ii;
      for(ii=0; rc==CAPDB_OK && ii<(int)pMain->nPage; ii++){
        i64 iOff = (i64)(pMain->nPagesize) * (i64)ii;
        if( iOff==PENDING_BYTE ) continue;
        rc = capdbOsRead(pMain->pReal, aData, pMain->nPagesize, iOff);
        pMain->aCksum[ii] = genCksum(aData, pMain->nPagesize);
        if( ii+1==(int)pMain->nPage && rc==CAPDB_IOERR_SHORT_READ ){
          rc = CAPDB_OK;
        }
      }
    }
...
static int readJournalFile(jt_file *p, jt_file *pMain){
  ...
  capdb_file *pReal = p->pReal;
  capdb_int64 iOff = 0;
  capdb_int64 iSize = p->iMaxOff;
  unsigned char *aPage;
  int iSave;
  int iSave2;

  aPage = capdb_malloc(pMain->nPagesize);
  if( !aPage ){
  ...
  while( rc==CAPDB_OK && iOff<iSize ){
    ...
    if( nRec==0 ){
      ...
      */
      if( iSize>=(iOff+nSector) ){
        rc = capdbOsRead(pReal, zBuf, 28, iOff);
        if( rc!=CAPDB_OK || 0==decodeJournalHdr(zBuf, 0, 0, 0, 0) ){
          continue;
        }
      }
    ...
    for(ii=0; rc==CAPDB_OK && ii<nRec && iOff<iSize; ii++){
      u32 pgno;
      rc = capdbOsRead(pReal, zBuf, 4, iOff);
      if( rc==CAPDB_OK ){
        pgno = decodeUint32(zBuf);
        if( pgno>0 && pgno<=pMain->nPage ){
          if( 0==capdbBitvecTest(pMain->pWritable, pgno) ){
            rc = capdbOsRead(pReal, aPage, pMain->nPagesize, iOff+4);
            if( rc==CAPDB_OK ){
...
static int jtWrite(
  ...
  int rc;
  jt_file *p = (jt_file *)pFile;
  if( p->flags&CAPDB_OPEN_MAIN_JOURNAL ){
    if( iOfst==0 ){
      ...
      if( iAmt==28 ){
        ...
        ** when a transaction is first started.  */
        ...
        if( CAPDB_OK!=(rc=openTransaction(pMain, p)) ){
          return rc;
        }
      }
    }
  ...
  if( p->flags&CAPDB_OPEN_MAIN_DB && p->pWritable ){
    if( iAmt<(int)p->nPagesize 
      ...
      ** fail with sync=normal or sync=full, this may indicate problem.  */
      ...
      assert( pgno>p->nPage || capdbBitvecTest(p->pWritable, pgno) );
    }
  }

  rc = capdbOsWrite(p->pReal, zBuf, iAmt, iOfst);
  if( (p->flags&CAPDB_OPEN_MAIN_JOURNAL) && iAmt==12 ){
    jt_file *pMain = locateDatabaseHandle(p->zName, 0);
...
** Truncate an jt-file.
*/
static int jtTruncate(capdb_file *pFile, sqlite_int64 size){
  jt_file *p = (jt_file *)pFile;
  if( p->flags&CAPDB_OPEN_MAIN_JOURNAL && size==0 ){
  ...
  if( p->flags&CAPDB_OPEN_MAIN_DB && p->pWritable ){
    ...
    for(pgno=(u32)(size/p->nPagesize+1); pgno<=p->nPage; pgno++){
      assert( pgno==locking_page || capdbBitvecTest(p->pWritable, pgno) );
    }
  }
...
** Sync an jt-file.
*/
static int jtSync(capdb_file *pFile, int flags){
  jt_file *p = (jt_file *)pFile;
...
*/
static int jtFileSize(capdb_file *pFile, sqlite_int64 *pSize){
  jt_file *p = (jt_file *)pFile;
...
** Lock an jt-file.
*/
static int jtLock(capdb_file *pFile, int eLock){
  int rc;
  jt_file *p = (jt_file *)pFile;
  rc = capdbOsLock(p->pReal, eLock);
  if( rc==CAPDB_OK && eLock>p->eLock ){
    p->eLock = eLock;
  }
  return rc;
}
...
** Unlock an jt-file.
*/
static int jtUnlock(capdb_file *pFile, int eLock){
  int rc;
  jt_file *p = (jt_file *)pFile;
  rc = capdbOsUnlock(p->pReal, eLock);
  if( rc==CAPDB_OK && eLock<p->eLock ){
    p->eLock = eLock;
  }
  return rc;
...
*/
static int jtCheckReservedLock(capdb_file *pFile, int *pResOut){
  jt_file *p = (jt_file *)pFile;
...
*/
static int jtFileControl(capdb_file *pFile, int op, void *pArg){
  jt_file *p = (jt_file *)pFile;
...
*/
static int jtSectorSize(capdb_file *pFile){
  jt_file *p = (jt_file *)pFile;
...
*/
static int jtDeviceCharacteristics(capdb_file *pFile){
  jt_file *p = (jt_file *)pFile;
...
static int jtOpen(
  ...
  assert( rc==CAPDB_OK || p->pReal->pMethods==0 );
  if( rc==CAPDB_OK ){
...
** returning.
*/
static int jtDelete(capdb_vfs *pVfs, const char *zPath, int dirSync){
...
*/
static void *jtDlOpen(capdb_vfs *pVfs, const char *zPath){
...
*/
static void jtDlError(capdb_vfs *pVfs, int nByte, char *zErrMsg){
...
*/
static void (*jtDlSym(capdb_vfs *pVfs, void *p, const char *zSym))(void){
...
*/
static void jtDlClose(capdb_vfs *pVfs, void *pHandle){
...
** random data.
*/
static int jtRandomness(capdb_vfs *pVfs, int nByte, char *zBufOut){
...
** actually slept.
*/
static int jtSleep(capdb_vfs *pVfs, int nMicro){
...
*/
static int jtCurrentTime(capdb_vfs *pVfs, double *pTimeOut){
  return g.pVfs->xCurrentTime(g.pVfs, pTimeOut);
}
/*
** Return the current time as a Julian Day number in *pTimeOut.
*/
static int jtCurrentTimeInt64(capdb_vfs *pVfs, capdb_int64 *pTimeOut){
  return g.pVfs->xCurrentTimeInt64(g.pVfs, pTimeOut);
}

static int jtGetLastError(capdb_vfs *pVfs, int n, char *z){
...
int jt_register(char *zWrap, int isDefault){
  ...
  capdb_vfs_register(&jt_vfs, isDefault);
  return CAPDB_OK;
}
