...
static int tclvarConnect(
  ...
  if( pVtab==0 ) return CAPDB_NOMEM;
  *ppVtab = &pVtab->base;
  pVtab->interp = (Tcl_Interp *)pAux;
  capdb_declare_vtab(db, zSchema);
  return CAPDB_OK;
}
/* Note that for this virtual table, the xCreate and xConnect
** methods are identical. */

static int tclvarDisconnect(capdb_vtab *pVtab){
  capdb_free(pVtab);
  return CAPDB_OK;
}
...
*/
static int tclvarOpen(capdb_vtab *pVTab, capdb_vtab_cursor **ppCursor){
  tclvar_cursor *pCur;
  pCur = capdbMallocZero(sizeof(tclvar_cursor));
  *ppCursor = &pCur->base;
  return CAPDB_OK;
}
...
*/
static int tclvarClose(capdb_vtab_cursor *cur){
...
static int tclvarNext(capdb_vtab_cursor *cur){
  Tcl_Obj *pObj;
  Tcl_Size n = 0;
  int ok = 0;
...
static int tclvarFilter(
  ...
  for(i=0; idxStr[i]; i++){
    switch( idxStr[i] ){
      case TCLVAR_NAME_EQ:
        zEq = (const char*)capdb_value_text(argv[i]);
        break;
      case TCLVAR_NAME_MATCH:
        zMatch = (const char*)capdb_value_text(argv[i]);
        break;
      case TCLVAR_VALUE_GLOB:
        zGlob = (const char*)capdb_value_text(argv[i]);
        break;
      case TCLVAR_VALUE_REGEXP:
        zRegexp = (const char*)capdb_value_text(argv[i]);
        break;
      case TCLVAR_VALUE_LIKE:
        zLike = (const char*)capdb_value_text(argv[i]);
        break;
      default:
        assert( 0 );
    }
...
static int tclvarColumn(capdb_vtab_cursor *cur, capdb_context *ctx, int i){
  Tcl_Obj *p1;
  Tcl_Obj *p2;
  const char *z1; 
  const char *z2 = "";
  ...
  switch (i) {
    case 0: {
      capdb_result_text(ctx, z1, -1, CAPDB_TRANSIENT);
      break;
    }
    case 1: {
      capdb_result_text(ctx, z2, -1, CAPDB_TRANSIENT);
      break;
    }
    case 2: {
      Tcl_Obj *pVal = Tcl_GetVar2Ex(interp, z1, *z2?z2:0, TCL_GLOBAL_ONLY);
      capdb_result_text(ctx, Tcl_GetString(pVal), -1, CAPDB_TRANSIENT);
      break;
    }
    case 3: {
      char *z3;
      if( p2 ){
        z3 = capdb_mprintf("%s(%s)", z1, z2);
        capdb_result_text(ctx, z3, -1, capdb_free);
      }else{
        capdb_result_text(ctx, z1, -1, CAPDB_TRANSIENT);
      }
      break;
...
static int tclvarRowid(capdb_vtab_cursor *cur, sqlite_int64 *pRowid){
  *pRowid = 0;
  return CAPDB_OK;
}

static int tclvarEof(capdb_vtab_cursor *cur){
  tclvar_cursor *pCur = (tclvar_cursor*)cur;
...
*/
static int tclvarBestIndex(capdb_vtab *tab, capdb_index_info *pIdxInfo){
  tclvar_vtab *pTab = (tclvar_vtab*)tab;
  int ii;
  char *zStr = capdb_malloc(32);
  int iStr = 0;

  if( zStr==0 ) return CAPDB_NOMEM;
  zStr[0] = '\0';
  ...
  for(ii=0; ii<pIdxInfo->nConstraint; ii++){
    ...
    if( pCons->usable ){
      /* name = ? */
      if( pCons->op==CAPDB_INDEX_CONSTRAINT_EQ && pCons->iColumn==0 ){
      ...
      /* name MATCH ? */
      if( pCons->op==CAPDB_INDEX_CONSTRAINT_MATCH && pCons->iColumn==0 ){
      ...
      /* value GLOB ? */
      if( pCons->op==CAPDB_INDEX_CONSTRAINT_GLOB && pCons->iColumn==2 ){
      ...
      /* value REGEXP ? */
      if( pCons->op==CAPDB_INDEX_CONSTRAINT_REGEXP && pCons->iColumn==2 ){
      ...
      /* value LIKE ? */
      if( pCons->op==CAPDB_INDEX_CONSTRAINT_LIKE && pCons->iColumn==2 ){
...
static int tclvarUpdate(
  ...
  if( argc==1 ){
    ...
    const char *zVar = (const char*)capdb_value_text(argv[0]);
    Tcl_UnsetVar(pTab->interp, zVar, TCL_GLOBAL_ONLY);
    return CAPDB_OK;
  }
  if( capdb_value_type(argv[0])==CAPDB_NULL ){
    /* An INSERT operation */
    const char *zValue = (const char*)capdb_value_text(argv[4]);
    const char *zName;
    if( capdb_value_type(argv[5])!=CAPDB_TEXT ){
      tab->zErrMsg = capdb_mprintf("the 'fullname' column must be TEXT");
      return CAPDB_ERROR;
    }
    zName = (const char*)capdb_value_text(argv[5]);
    if( zValue ){
  ...
  if( capdb_value_type(argv[0])==CAPDB_TEXT
   && capdb_value_type(argv[1])==CAPDB_TEXT
  ){
    /* An UPDATE operation */
    const char *zOldName = (const char*)capdb_value_text(argv[0]);
    const char *zNewName = (const char*)capdb_value_text(argv[1]);
    const char *zValue = (const char*)capdb_value_text(argv[4]);
  ...
  tab->zErrMsg = capdb_mprintf("prohibited TCL variable change");
  return CAPDB_ERROR;
}
...
*/
static capdb_module tclvarModule = {
  0,                         /* iVersion */
  tclvarConnect,
  tclvarConnect,
  tclvarBestIndex,
...
*/
static int CAPDB_TCLAPI register_tclvar_module(
...
#ifndef CAPDB_OMIT_VIRTUALTABLE
  capdb_create_module(db, "tclvar", &tclvarModule, (void*)interp);
  rc = Tcl_Eval(interp, 
