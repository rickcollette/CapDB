...
static int circle_geom(
  ...
  if( pCircle==0 ){
    ...
    ** Return an error if the table does not have exactly 2 dimensions. */
    if( nCoord!=4 ) return CAPDB_ERROR;

    /* Test that the correct number of parameters (3) have been supplied,
    ** and that the parameters are in range (that the radius of the circle 
    ** radius is greater than zero). */
    if( p->nParam!=3 || p->aParam[2]<0.0 ) return CAPDB_ERROR;

    /* Allocate a structure to cache parameter data in. Return CAPDB_NOMEM
    ** if the allocation fails. */
    pCircle = (Circle *)(p->pUser = capdb_malloc(sizeof(Circle)));
    if( !pCircle ) return CAPDB_NOMEM;
  ...
  ** the output variable to zero and return CAPDB_OK. */
  *pRes = 0;
  return CAPDB_OK;
}
...
*/
static int circle_query_func(capdb_rtree_query_info *p){
  int i;                          /* Iterator variable */
  Circle *pCircle;                /* Structure defining circular region */
  double xmin, xmax;              /* X dimensions of box being tested */
  double ymin, ymax;              /* X dimensions of box being tested */
  ...
  if( pCircle==0 ){
    ...
    ** for correctness or stored into a Circle structure yet. Do this now. */
    ...
    /* This geometry callback is for use with a 2-dimensional r-tree table.
    ** Return an error if the table does not have exactly 2 dimensions. */
    if( p->nCoord!=4 ) return CAPDB_ERROR;

    /* Test that the correct number of parameters (1 or 4) have been supplied.
    */
    if( p->nParam!=4 && p->nParam!=1 ) return CAPDB_ERROR;
    ...
    ** units of the center of the circle. */
    if( p->nParam==4 ){
      ...
      const char *z = (const char*)capdb_value_text(p->apSqlParam[0]);
      pCircle->centerx = 0.0;
      pCircle->centery = 0.0;
      pCircle->radius = 0.0;
...
static int cube_geom(
  ...
  if( pCube==0 ){
    ...
    pCube = (Cube *)capdb_malloc(sizeof(Cube));
    if( !pCube ){
      return CAPDB_NOMEM;
    }
    pCube->x = p->aParam[0];
...
static int CAPDB_TCLAPI register_cube_geom(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  rc = capdb_rtree_geometry_callback(db, "cube", cube_geom, (void *)&gHere);
  Tcl_SetResult(interp, (char *)capdbErrName(rc), TCL_STATIC);
#endif
  return TCL_OK;
}

static int CAPDB_TCLAPI register_circle_geom(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  if( getDbPointer(interp, Tcl_GetString(objv[1]), &db) ) return TCL_ERROR;
  rc = capdb_rtree_geometry_callback(db, "circle", circle_geom, 0);
  if( rc==CAPDB_OK ){
    rc = capdb_rtree_query_callback(db, "Qcircle",
                                      circle_query_func, 0, 0);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_rtree_query_callback(db, "breadthfirstsearch",
                                      bfs_query_func, 0, 0);
  }
  Tcl_SetResult(interp, (char *)capdbErrName(rc), TCL_STATIC);
#endif
