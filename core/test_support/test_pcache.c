...
** uninitialized.  Allocate some memory and report CAPDB_NOMEM if
...
static int testpcacheInit(void *pArg){
  ...
  testpcacheGlobal.pDummy = capdb_malloc(10);
  return testpcacheGlobal.pDummy==0 ? CAPDB_NOMEM : CAPDB_OK;
}
...
static void testpcacheShutdown(void *pArg){
  ...
  assert( testpcacheGlobal.pDummy!=0 );
  assert( testpcacheGlobal.nInstance==0 );
  capdb_free( testpcacheGlobal.pDummy );
...
** If more pages are requested, capdbPcacheFetch() returns NULL.
**
** If testing with in-memory temp tables, provide a larger pcache.
** Some of the test cases need this.
*/
#if defined(CAPDB_TEMP_STORE) && CAPDB_TEMP_STORE>=2
...
struct testpcache {
  capdb_int64 szPage;     /* Size of each page.  Multiple of 8. */
  ...
  struct testpcachePage {
    capdb_pcache_page page;  /* Base class */
...
*/
static capdb_pcache *testpcacheCreate(
  int szPage,
  int szExtra,
  int bPurgeable
){
  ...
  return (capdb_pcache*)p;
}

/*
** Set the cache size
*/
static void testpcacheCachesize(capdb_pcache *pCache, int newSize){
...
*/
static int testpcachePagecount(capdb_pcache *pCache){
  testpcache *p = (testpcache*)pCache;
  assert( p->iMagic==TESTPCACHE_VALID );
  assert( testpcacheGlobal.pDummy!=0 );
  assert( testpcacheGlobal.nInstance>0 );
...
** Fetch a page.
*/
static capdb_pcache_page *testpcacheFetch(
  capdb_pcache *pCache,
  unsigned key,
  int createFlag
){
...
** is discarded.
*/
static void testpcacheTruncate(capdb_pcache *pCache, unsigned iLimit){
  testpcache *p = (testpcache*)pCache;
  unsigned int i;
  assert( p->iMagic==TESTPCACHE_VALID );
  assert( testpcacheGlobal.pDummy!=0 );
...
*/
static void testpcacheDestroy(capdb_pcache *pCache){
  testpcache *p = (testpcache*)pCache;
  assert( p->iMagic==TESTPCACHE_VALID );
  assert( testpcacheGlobal.pDummy!=0 );
  assert( testpcacheGlobal.nInstance>0 );
...
void installTestPCache(
  ...
  static const capdb_pcache_methods2 testPcache = {
    1,
  ...
  static capdb_pcache_methods2 defaultPcache;
  ...
  if( installFlag!=isInstalled ){
    if( installFlag ){
      capdb_config(CAPDB_CONFIG_GETPCACHE2, &defaultPcache);
      assert( defaultPcache.xCreate!=testpcacheCreate );
      capdb_config(CAPDB_CONFIG_PCACHE2, &testPcache);
    }else{
      assert( defaultPcache.xCreate!=0 );
      capdb_config(CAPDB_CONFIG_PCACHE2, &defaultPcache);
    }
