...
static int getDbPointer(Tcl_Interp *interp, Tcl_Obj *pObj, capdb **pDb){
  Tcl_CmdInfo info;
...
/*
** Implementation of the command created by [capdb_recover_init]:
**
**     $cmd config OP ARG
**     $cmd run
**     $cmd errmsg
...
static int testRecoverCmd(
  ...
  switch( iSub ){
    case 0:  assert( capdb_stricmp("config", aSub[iSub].zSub)==0 ); {
      const char *aOp[] = {
        "testdb",          /* 0 */
        "lostandfound",    /* 1 */
        "freelistcorrupt", /* 2 */
      ...
      switch( iOp ){
        case 0:
          res = capdb_recover_config(pTest->p, 
        ...
        case 1: {
          ...
          res = capdb_recover_config(pTest->p, 
              CAPDB_RECOVER_LOST_AND_FOUND, (void*)(zStr[0] ? zStr : 0)
          );
          break;
        }
        case 2: {
          ...
          res = capdb_recover_config(pTest->p, 
              CAPDB_RECOVER_FREELIST_CORRUPT, (void*)&iVal
          );
          break;
        }
        case 3: {
          ...
          if( Tcl_GetBooleanFromObj(interp, objv[3], &iVal) ) return TCL_ERROR;
          res = capdb_recover_config(pTest->p, 
              CAPDB_RECOVER_ROWIDS, (void*)&iVal
          );
          break;
        }
        case 4: {
          ...
          if( Tcl_GetBooleanFromObj(interp, objv[3], &iVal) ) return TCL_ERROR;
          res = capdb_recover_config(pTest->p, 
              CAPDB_RECOVER_SLOWINDEXES, (void*)&iVal
          );
          break;
        }
        case 5: {
          res = capdb_recover_config(pTest->p, 12345, 0);
          break;
        }
      }
    ...
    case 1:  assert( capdb_stricmp("run", aSub[iSub].zSub)==0 ); {
      int res = capdb_recover_run(pTest->p);
      Tcl_SetObjResult(interp, Tcl_NewIntObj(res));
      break;
    }
    case 2:  assert( capdb_stricmp("errmsg", aSub[iSub].zSub)==0 ); {
      const char *zErr = capdb_recover_errmsg(pTest->p);
      Tcl_SetObjResult(interp, Tcl_NewStringObj(zErr, -1));
      break;
    }
    case 3:  assert( capdb_stricmp("errcode", aSub[iSub].zSub)==0 ); {
      int errCode = capdb_recover_errcode(pTest->p);
      Tcl_SetObjResult(interp, Tcl_NewIntObj(errCode));
      break;
    }
    case 4:  assert( capdb_stricmp("finish", aSub[iSub].zSub)==0 ); {
      int res = capdb_recover_errcode(pTest->p);
      int res2;
      if( res!=CAPDB_OK ){
        const char *zErr = capdb_recover_errmsg(pTest->p);
        Tcl_SetObjResult(interp, Tcl_NewStringObj(zErr, -1));
      }
      res2 = capdb_recover_finish(pTest->p);
      assert( res2==res );
      if( res ) return TCL_ERROR;
      break;
    }
    case 5:  assert( capdb_stricmp("step", aSub[iSub].zSub)==0 ); {
      int res = capdb_recover_step(pTest->p);
      Tcl_SetObjResult(interp, Tcl_NewIntObj(res));
      break;
    }
  }
...
/*
** capdb_recover_init DB DBNAME URI
*/
static int test_capdb_recover_init(
  void *clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  if( bSql==0 ){
    ...
    pNew->p = capdb_recover_init(db, zDb, zUri);
  }else{
    pNew->interp = interp;
    pNew->pScript = objv[3];
    Tcl_IncrRefCount(pNew->pScript);
    pNew->p = capdb_recover_init_sql(db, zDb, xSqlCallback, (void*)pNew);
  }
...
** capdb_recover_init DB DBNAME URI
*/
static int test_capdb_dbdata_init(
  void *clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
int TestRecover_Init(Tcl_Interp *interp){
  ...
  struct Cmd {
    ...
    { "capdb_recover_init", test_capdb_recover_init, 0 },
    { "capdb_recover_init_sql", test_capdb_recover_init, (void*)1 },
    { "capdb_dbdata_init", test_capdb_dbdata_init, (void*)1 },
  };
  int i;
