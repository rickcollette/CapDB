...
static int invokeTclGeomCb(
  ...
  if( p->pContext ){
    ...
    capdb_snprintf(sizeof(aPtr)-1, aPtr, "%p", (void*)p->pContext);
    ...
    capdb_snprintf(sizeof(aPtr)-1, aPtr, "%p", (void*)p);
    Tcl_ListObjAppendElement(interp, pScript, Tcl_NewStringObj(aPtr,-1));
    ...
    if( rc!=TCL_OK ){
      ...
      if( nObj>0 ){
        ...
        if( 0==capdb_stricmp(zCmd, "zero") ){
          p->aParam[0] = 0.0;
          p->nParam = 1;
        }
        else if( 0==capdb_stricmp(zCmd, "user") ){
          if( p->pUser || p->xDelUser ){
            rc = CAPDB_ERROR;
          }else{
            BoxGeomCtx *pBGCtx = capdb_malloc(sizeof(BoxGeomCtx));
            if( pBGCtx==0 ){
              rc = CAPDB_NOMEM;
            }else{
        ...
        else if( 0==capdb_stricmp(zCmd, "user_is_zero") ){
          if( p->pUser || p->xDelUser ) rc = CAPDB_ERROR;
        }
      }
    }
  }
...
# capdb_rtree_geometry structure which provides information about how
...
*/
static int box_geom(
  capdb_rtree_geometry *p,      /* R-50437-53270 */
  int nCoord,                     /* R-02424-24769 */
  capdb_rtree_dbl *aCoord,      /* R-00090-24248 */
  ...
  if( invokeTclGeomCb("box", p, nCoord, aCoord) ) return CAPDB_ERROR;
...
static int CAPDB_TCLAPI register_box_geom(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  capdb_rtree_geometry_callback(db, "box", box_geom, (void*)pCtx);

  capdb_snprintf(64, aPtr, "%p", (void*)pCtx);
  Tcl_SetObjResult(interp, Tcl_NewStringObj(aPtr, -1));
  return TCL_OK;
}

static int box_query(capdb_rtree_query_info *pInfo){
  ...
  rc = Tcl_EvalObjEx(interp, pEval, 0) ? CAPDB_ERROR : CAPDB_OK;
...
static int CAPDB_TCLAPI register_box_query(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
