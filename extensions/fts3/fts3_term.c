...
struct Fts3termCursor {
  ...
  capdb_int64 iRowid;           /* Current 'rowid' value */
  capdb_int64 iDocid;           /* Current 'docid' value */
...
static int fts3termConnectMethod(
  ...
  *ppVtab = 0;
  ...
  rc = capdb_declare_vtab(db, FTS3_TERMS_SCHEMA);
  if( rc!=CAPDB_OK ) return rc;

  nByte = sizeof(Fts3termTable);
  p = (Fts3termTable *)capdbFts3MallocZero(nByte);
  if( !p ) return CAPDB_NOMEM;

  p->pFts3Tab = (Fts3Table*)capdbFts3MallocZero(
...
*/
static int fts3termDisconnectMethod(capdb_vtab *pVtab){
...
** xOpen - Open a cursor.
*/
static int fts3termOpenMethod(capdb_vtab *pVTab, capdb_vtab_cursor **ppCsr){
  Fts3termCursor *pCsr;            /* Pointer to cursor object to return */

  UNUSED_PARAMETER(pVTab);

  pCsr = (Fts3termCursor *)capdb_malloc(sizeof(Fts3termCursor));
...
*/
static int fts3termCloseMethod(capdb_vtab_cursor *pCursor){
...
*/
static int fts3termNextMethod(capdb_vtab_cursor *pCursor){
  Fts3termCursor *pCsr = (Fts3termCursor *)pCursor;
  Fts3Table *pFts3 = ((Fts3termTable *)pCursor->pVtab)->pFts3Tab;
  int rc;
  capdb_int64 v;
  ...
  if( pCsr->csr.aDoclist==0 
   || pCsr->pNext>=&pCsr->csr.aDoclist[pCsr->csr.nDoclist-1]
    ...
    /* Read docid */
    pCsr->pNext += capdbFts3GetVarint(pCsr->pNext, &pCsr->iDocid);
  }

  pCsr->pNext += capdbFts3GetVarint(pCsr->pNext, &v);
  if( v==0 ){
    pCsr->pNext += capdbFts3GetVarint(pCsr->pNext, &v);
    pCsr->iDocid += v;
    pCsr->pNext += capdbFts3GetVarint(pCsr->pNext, &v);
...
static int fts3termFilterMethod(
  ...
  rc = capdbFts3SegReaderCursor(pFts3, 0, p->iIndex, FTS3_SEGCURSOR_ALL,
...
*/
static int fts3termEofMethod(capdb_vtab_cursor *pCursor){
  Fts3termCursor *pCsr = (Fts3termCursor *)pCursor;
  return pCsr->isEof;
}
...
static int fts3termColumnMethod(
  ...
  switch( iCol ){
    case 0:
      ...
      break;
    case 1:
      capdb_result_int64(pCtx, p->iDocid);
      break;
    case 2:
      capdb_result_int64(pCtx, p->iCol);
      break;
    default:
      capdb_result_int64(pCtx, p->iPos);
      break;
  }
...
/*
** Register the fts3term module with database connection db. Return CAPDB_OK
** if successful or an error code if capdb_create_module() fails.
*/
int capdbFts3InitTerm(capdb *db){
  static const capdb_module fts3term_module = {
     0,                           /* iVersion      */
  ...
  rc = capdb_create_module(db, "fts4term", &fts3term_module, 0);
  return rc;
}
