...
** that the capdb_tokenizer_module.xLanguage() method is invoked correctly.
*/
...
#if defined(CAPDB_ENABLE_FTS3) || defined(CAPDB_ENABLE_FTS4)

/* Required so that the "ifdef CAPDB_ENABLE_FTS3" below works */
#include "fts3Int.h"
...
*/
static int CAPDB_TCLAPI fts3_near_match_cmd(
...
*/
static int CAPDB_TCLAPI fts3_configure_incr_load_cmd(
  ClientData clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
static int testTokenizerCreate(
  ...
  pNew = capdb_malloc(sizeof(test_tokenizer));
  if( !pNew ) return CAPDB_NOMEM;
  memset(pNew, 0, sizeof(test_tokenizer));

  *ppTokenizer = (capdb_tokenizer *)pNew;
  return CAPDB_OK;
}

static int testTokenizerDestroy(capdb_tokenizer *pTokenizer){
...
static int testTokenizerOpen(
  ...
  pCsr = (test_tokenizer_cursor *)capdb_malloc(sizeof(test_tokenizer_cursor));
  if( pCsr==0 ){
    rc = CAPDB_NOMEM;
  }else{
...
static int testTokenizerClose(capdb_tokenizer_cursor *pCursor){
...
static int testTokenizerNext(
  capdb_tokenizer_cursor *pCursor,  /* Cursor returned by testTokenizerOpen */
  ...
  if( p==pEnd ){
    ...
    nToken = (capdb_int64)(p-pToken);

    /* Copy the token into the buffer */
    if( nToken>pCsr->nBuffer ){
      capdb_free(pCsr->aBuffer);
      pCsr->aBuffer = capdb_malloc64(nToken);
    }
    if( pCsr->aBuffer==0 ){
      rc = CAPDB_NOMEM;
    }else{
...
static int CAPDB_TCLAPI fts3_test_tokenizer_cmd(
  ClientData clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
#ifdef CAPDB_ENABLE_FTS3
  static const capdb_tokenizer_module testTokenizerModule = {
    1,
    testTokenizerCreate,
  ...
  const capdb_tokenizer_module *pPtr = &testTokenizerModule;
  if( objc!=1 ){
    Tcl_WrongNumArgs(interp, 1, objv, "");
    return TCL_ERROR;
  }
  Tcl_SetObjResult(interp, Tcl_NewByteArrayObj(
    (const unsigned char *)&pPtr, sizeof(capdb_tokenizer_module *)
  ));
#endif
  UNUSED_PARAMETER(clientData);
  return TCL_OK;
...
static int CAPDB_TCLAPI fts3_test_varint_cmd(
  ClientData clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  nByte = capdbFts3PutVarint(aBuf, w);
  nByte2 = capdbFts3GetVarint(aBuf, &w2);
  if( w!=w2 || nByte!=nByte2 ){
    char *zErr = capdb_mprintf("error testing %lld", w);
  ...
  if( w<=2147483647 && w>=0 ){
    ...
    if( (int)w!=i || nByte!=nByte2 ){
      char *zErr = capdb_mprintf("error testing %lld (32-bit)", w);
      Tcl_ResetResult(interp);
      Tcl_AppendResult(interp, zErr, NULL);
      return TCL_ERROR;
    }
...
/*
**      capdb_fts3_may_be_corrupt BOOLEAN
**
** Set or clear the global "may-be-corrupt" flag. Return the old value.
*/
static int CAPDB_TCLAPI fts3_may_be_corrupt(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
#ifdef CAPDB_DEBUG
  int bOld = capdb_fts3_may_be_corrupt;
  ...
  if( objc==2 ){
    ...
    capdb_fts3_may_be_corrupt = bNew;
  }
...
int Sqlitetestfts3_Init(Tcl_Interp *interp){
  ...
  Tcl_CreateObjCommand(
      interp, "capdb_fts3_may_be_corrupt", fts3_may_be_corrupt, 0, 0
  );
  return TCL_OK;
}
#endif                  /* CAPDB_ENABLE_FTS3 || CAPDB_ENABLE_FTS4 */
#endif                  /* ifdef CAPDB_TEST */
