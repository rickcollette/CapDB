...
Suppose variable `capdb_stmt *pStmt` is a pointer to an SQLite
...
> ~~~
capdb_qrf_spec spec;  /* Format specification */
...
rc = capdb_format_query_result(pStmt, &spec, &zErrMsg); /* Format results */
if( rc ){
  printf("Error (%d): %s\n", rc, zErrMsg);  /* Report an error */
  capdb_free(zErrMsg);              /* Free the error message text */
}else{
  printf("%s", zResult);              /* Report the results */
}
capdb_free(zResult);              /* Free memory used to hold results */
~~~

The `capdb_qrf_spec` object describes the desired output format
and where to send the generated output. Most of the work in using
the QRF involves filling out the capdb_qrf_spec.

### 1.1 Using QRF with SQL text

If you start with SQL text instead of an capdb_stmt pointer, and
especially if the SQL text might comprise two or more statements, then
the SQL text needs to be converted into capdb_stmt objects separately.
If the original SQL text is in a variable `const char *zSql` and the
database connection is in variable `capdb *db`, then code
...
char *zErrMsg;          /* Text error message (optional) */
char *zResult = 0;      /* Formatted output written here */
capdb_stmt *pStmt;    /* Next prepared statement */
int rc;                 /* Result code */
...
while( zSql && zSql[0] ){
  ...
  rc = capdb_prepare_v2(db, zSql, -1, &pStmt, &zSql);
  if( rc!=CAPDB_OK ){
    printf("Error: %s\n", capdb_errmsg(db));
  }else{
    rc = capdb_format_query_result(pStmt, &spec, &zErrMsg); /* Get results */
    if( rc ){
      printf("Error (%d): %s\n", rc, zErrMsg);  /* Report an error */
      capdb_free(zErrMsg);              /* Free the error message text */
    }else{
...
<a id="spec"></a>
## 2.0 The `capdb_qrf_spec` object

The `capdb_qrf_spec` looks like this:
...
`capdb_qrf_spec` object is in the subsequent sections.

### 2.1 Structure Version Number

The capdb_qrf_spec.iVersion field must be 1.  Future enhancements to 
the QRF might add new fields to the bottom of the capdb_qrf_spec
...
from capdb_malloc().  If the capdb_qrf_spec.xWrite column is not NULL,
then that function is invoked (using capdb_qrf_spec.xWriteArg as its
first argument) to transmit the formatted output.  Or, if
capdb_qrf_spec.pzOutput points to a pointer to a character, then that
pointer is made to point to memory obtained from capdb_malloc() that
...
from capdb_malloc().  In that case, the buffer is resized using
capdb_realloc() and the new text is appended.

One of either capdb_qrf_spec.xWrite and capdb_qrf_spec.pzOutput must be
non-NULL and the other must be NULL.

The return value from xWrite is an SQLITE result code.  The usual return
should be CAPDB_OK.  But if for some reason the write fails, a different
value might be returned.

### 2.3 Output Format

The capdb_qrf_spec.eStyle field is an integer code that defines the
...
Other fields in capdb_qrf_spec might be used or might be
ignored, depending on the value of eStyle.

### 2.4 Show Column Names (bTitles)

The capdb_qrf_spec.bTitles field can be either QRF_SW_Auto,
...
The capdb_qrf_spec.eEsc determines how ASCII control characters are
...
The capdb_qrf_spec.eText controls how text values are rendered in the
display.  capdb_qrf_spec.eTitle controls how column names are rendered.
...
If the capdb_qrf_spec.bTextJsonb flag is QRF_SW_On and if the value to be
displayed is JSONB, then the JSONB is translated into text JSON and the
text is shown according to the capdb_qrf_spec.eText setting as
described in the previous section.

If the bTextJsonb flag is QRF_SW_Off (the usual case) or if the BLOB value to
be displayed is not JSONB, then the capdb_qrf_spec.eBlob field determines
...
by capdb_format_query_result() based on eStyle and eText.
...
If the capdb_qrf_spec.nCharLimit setting is non-zero, then the formatter
...
If the capdb_qrf_spec.nLineLimit setting is non-zero, then the
...
If the capdb_qrf_spec.nTitleLimit is non-zero, then the formatter
...
If the capdb_qrf_spec.nMultiInsert value is positive, then the
...
the width of any individual column to capdb_qrf_spec.nWrap characters
...
The capdb_qrf_spec.nScreenWidth field can be set the number of
...
The capdb_qrf_spec.aWidth field is a pointer to an array of
...
QRF_STYLE_Markdown, or QRF_STYLE_Table).  The capdb_qrf_spec.nWidth
...
The capdb_qrf_spec.aAlign field points to an array of unsigned characters
that specifies alignment (both vertical and horizontal) of individual
columns within the table.  The capdb_qrf_spec.nAlign fields holds
the number of entries in the aAlign\[\] array.

If capdb_qrf_spec.aAlign is a NULL pointer or if capdb_qrf_spec.nAlign
is zero, or for columns to the right of what are specified by
capdb_qrf_spec.nAlign, the capdb_qrf_spec.eDfltAlign value is used
for the alignment.  Column names can be (and often are) aligned
differently, as specified by capdb_qrf_spec.eTitleAlign.
...
The capdb_qrf_spec.zColumnSep and capdb_qrf_spec.zRowSep strings
...
The capdb_qrf_spec.zTableName value is the name of the output table
when eStyle is QRF_STYLE_Insert.

### 2.16 The Rendering Of NULL (zNull)

If a value is NULL then show the NULL using the string
found in capdb_qrf_spec.zNull.  If zNull is itself a NULL pointer
then NULL values are rendered as an empty string.

### 2.17 Optional Value Rendering Callback

If the capdb_qrf_spec.xRender field is not NULL, then each
capdb_value coming out of the query is first passed to the
...
into memory obtained from capdb_malloc() and return a pointer
to that memory.  The xRender function can decline
to render (for example, based on the capdb_value_type() or other
...
The capdb_format_query_result() function (which calls xRender)
...
capdb_column_name() interface not from capdb_column_value(),
and so that names of columns are never processed by xRender.

## 3.0 The `capdb_format_query_result()` Interface

Invoke the `capdb_format_query_result(P,S,E)` interface to run
the prepared statement P and format its results according to the
specification found in S.  The capdb_format_query_result() function
will return an SQLite result code, usually CAPDB_OK, but perhaps
CAPDB_NOMEM or CAPDB_ERROR or similar.  If an error occurs and if
...
from capdb_malloc() and it is the responsibility of the caller to
free that memory by a subsequent call to capdb_free().
...
the eStyle field of the capdb_qrf_spec object. The set of
...
**Explain** or **Eqp** if the `capdb_stmt_explain()` function
...
The nWidth, aWidth, and mxWidth fields of the `capdb_qrf_spec` object
...
the entire result.  However, if capdb_qrf_spec.bBorder is QRF_No, then
...
and zRowSep fields of the `capdb_qrf_spec` structure.  The text
...
with hard-coded values for some of the capdb_qrf_spec settings:
...
by the zTableName field of `capdb_qrf_spec`.  If zTableName is NULL,
...
is converted back before `capdb_format_query_result()` returns.
...
-DCAPDB_ENABLE_STMT_SCANSTATUS and if the CAPDB_DBCONFIG_STMT_SCANSTATUS
is enabled for the database connection.  The **StatsVm** style
also requires the bytecode() virtual table which is enabled using
the -DCAPDB_ENABLE_BYTECODE_VTAB compile-time option.
