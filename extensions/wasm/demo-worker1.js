/*
  ...
  ***********************************************************************
  ...
  A basic test script for capdb-worker1.js.
...
(function(){
  ...
  const SW = new Worker("jswasm/capdb-worker1.js");
  const DbState = {
    id: undefined
  };
  ...
  */
  ...
  const runTests = function(){
    ...
    */
    ...
    runOneTest('open', {
      filename:'testing2.capdb',
      simulateError: simulateOpenError
    }, function(ev){
      log("open result",ev);
      T.assert('testing2.capdb'===ev.result.filename)
        .assert(ev.dbId)
        .assert(ev.messageId)
  ...
  SW.onmessage = function(ev){
    if(!ev.data || 'object'!==typeof ev.data){
      warn("Unknown capdb-worker message type:",ev);
      return;
    }
    ...
    switch(ev.type){
        case 'capdb-api':
          switch(ev.result){
              case 'worker1-ready':
                log("Message:",ev);
                globalThis.capdbTestModule.setStatus(null);
                runTests();
                return;
              default:
                warn("Unknown capdb-api message type:",ev);
                return;
          }
        default:
