/*
  ...
  ***********************************************************************
  ...
  Demonstration of the capdb Worker API #1 Promiser: a Promise-based
  proxy for for the capdb Worker #1 API.
*/
//#if target:es6-module
import {default as promiserFactory} from "./jswasm/capdb-worker1-promiser.mjs";
//#else
"use strict";
const promiserFactory = globalThis.capdbWorker1Promiser.v2;
...
//#/if
  ...
  const workerPromise = await promiserFactory(promiserConfig)
        .then((func)=>{
          ...
          globalThis.capdbTestModule.setStatus(null)/*hide the HTML-side is-loading spinner*/;
          return func;
        });
  ...
  const runTests = async function(){
    const dbFilename = '/testing2.capdb';
    startTime = performance.now();

    await wtest('config-get', (ev)=>{
      const r = ev.result;
      log('capdb.config subset:', r);
  ...
  const runTests2 = async function(){
    ...
    await wtest('export', function(ev){
      ...
      T.assert('string' === typeof ev.filename)
        ...
        .assert('application/x-capdb' === ev.mimetype);
    });
