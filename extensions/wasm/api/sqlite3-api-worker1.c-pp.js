...
/**
  ...
  ***********************************************************************
  ...
  This file requires that the core C-style capdb API and OO API #1
  have been loaded.
*/

/**
  capdb.initWorker1API() implements a Worker-based wrapper around
  ...
  Promise-based wrapper for this API (`capdb-worker1-promiser.js`)
  ...
  This operation fetches the serializable parts of the capdb API
  configuration.
  ...
  ```
  {
    ...
    result: {
      ...
      version: capdb.version object

      bigIntEnabled: bool. True if BigInt support is enabled.

      vfsList: result of capdb.capi.capdb_js_vfs_list()
   }
  }
  ```
  ...
  ```
  {
    ...
    args:{
      ...
      See the capdb.oo1.DB constructor for peculiarities and
      transformations,

      vfs: capdb_vfs name. Ignored if filename is ":memory:" or "".
  ...
  `capdb_total_changes()` before and after the SQL is evaluated. If
  ...
  calling `capdb_total_changes64()` before and after the SQL is
  evaluated.
  ...
  to the result of capdb_last_insert_rowid(). This value is only
  ...
  ```
  {
    ...
    dbId: ...as above...
    result: {
      byteArray: Uint8Array (as per capdb_js_db_export()),
      filename: the db filename,
      mimetype: "application/x-capdb"
    }
  }
  ```
...
capdb.initWorker1API = function(){
  'use strict';
  const toss = (...args)=>{throw new Error(args.join(' '))};
  if(!(globalThis.WorkerGlobalScope instanceof Function)){
    toss("initWorker1API() must be run from a Worker thread.");
  }
  const capdb = this.capdb || toss("Missing this.capdb object.");
    ...
    */
    ...
    close: function(db,alsoUnlink){
      if(db){
        ...
        const pVfs = util.capdb__wasm_db_vfs(db.pointer, 0);
        db.close();
        const ddNdx = this.dbList.indexOf(db);
        if(ddNdx>=0) this.dbList.splice(ddNdx, 1);
        if(alsoUnlink && filename && pVfs){
          util.capdb__wasm_vfs_unlink(pVfs, filename);
        }
      }
    },
    /**
  ...
  */
  const wMsgHandler = {
    open: function(ev){
      ...
      rc.persistent = !!capdb.capi.capdb_js_db_uses_vfs(db.pointer, "opfs");
      rc.dbId = getDbId(db);
      rc.vfs = db.dbVfsName();
      return rc;
    },
    ...
    exec: function(ev){
      ...
      try {
        ...
        const lastInsertRowId = !!rc.lastInsertRowId
              ? capdb.capi.capdb_last_insert_rowid(db)
              : undefined;
    ...
    'config-get': function(){
      const rc = Object.create(null), src = capdb.config;
      [
        'bigIntEnabled'
      ].forEach(function(k){
        if(Object.getOwnPropertyDescriptor(src, k)) rc[k] = src[k];
      });
      rc.version = capdb.version;
      rc.vfsList = capdb.capi.capdb_js_vfs_list();
      return rc;
    },

    /**
       Exports the database to a byte array, as per
       capdb_serialize(). Response is an object:
    ...
    */
    export: function(ev){
      const db = getMsgDb(ev);
      const response = {
        byteArray: capdb.capi.capdb_js_db_export(db.pointer),
  ...
  globalThis.onmessage = async function(ev){
    ...
    try {
      ...
      if(0) capdb.config.warn("Worker is propagating an exception to main thread.",
  ...
  globalThis.postMessage({type:'capdb-api',result:'worker1-ready'});
}.bind({capdb});
});
//#else
