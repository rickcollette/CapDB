/* @preserve
  ...
  ***********************************************************************
  ...
  counterpart of the API defined in capdb-vfs-opfs.js.
  ...
  access to the capdb JS/WASM bits, so any bits which it needs (most
  notably CAPDB_xxx integer codes) have to be imported into it via an
...
//#undef opfs-async-proxy
  ...
  */
  ...
  /**
     __openFiles is a map of capdb_file pointers (integers) to
  ...
  */
  ...
  /**
     __implicitLocks is a Set of capdb_file pointers (integers)
  ...
  */
  ...
  /**
     Attempts to find a suitable CAPDB_xyz result code for Error
  ...
  */
  GetSyncHandleError.convertRc = (e,rc)=>{
    if( e instanceof GetSyncHandleError ){
      if( e.cause.name==='NoModificationAllowedError'
        ...
        return state.sq3Codes.CAPDB_BUSY;
      }else if( 'NotFoundError'===e.cause.name ){
        /**
           Maintenance reminder: CAPDB_NOTFOUND, though it looks like
           a good match, has different semantics than NotFoundError
           and is not suitable here.
        */
        return state.sq3Codes.CAPDB_CANTOPEN;
      }
  ...
  /**
     ...
     as an I/O error or CAPDB_BUSY, depending on the exception and
     the context.
  ...
  */
  ...
  /**
     Asynchronous wrappers for capdb_vfs and capdb_io_methods
  ...
  */
  const vfsAsyncImpls = {
    ...
    xAccess: async (filename)=>{
      /* OPFS cannot support the full range of xAccess() queries
         capdb calls for. We can essentially just tell if the file
         ...
         The return semantics of this function differ from capdb's
    ...
    xClose: async function(fid/*capdb_file pointer*/){
      const opName = 'xClose';
      ...
      if(fh){
        ...
        rc = state.sq3Codes.CAPDB_NOTFOUND;
      }
    ...
    xDeleteNoWait: async function(filename, syncDir = 0, recursive = false){
      ...
      */
      ...
      try {
        ...
        state.s11n.storeException(2,e);
        rc = state.sq3Codes.CAPDB_IOERR_DELETE;
      }
      return rc;
    },
    xFileSize: async function(fid/*capdb_file pointer*/){
      const fh = __openFiles[fid];
      let rc = 0;
      try{
        ...
        rc = GetSyncHandleError.convertRc(e,state.sq3Codes.CAPDB_IOERR);
      }
    ...
    /**
       ...
       i.e. a lookup key.
    */
    xOpen: async function(fid/*capdb_file pointer*/, filename,
                          flags/*CAPDB_OPEN_...*/,
                          opfsFlags/*OPFS_...*/){
      const opName = 'xOpen';
      const create = (state.sq3Codes.CAPDB_OPEN_CREATE & flags);
      try{
        ...
        try {
          ...
          state.s11n.storeException(1,e);
          storeAndNotify(opName, state.sq3Codes.CAPDB_NOTFOUND);
          return;
        }
        ...
        const fh = Object.assign(Object.create(null),{
          ...
          readOnly: !create && !!(state.sq3Codes.CAPDB_OPEN_READONLY & flags),
          deleteOnClose: !!(state.sq3Codes.CAPDB_OPEN_DELETEONCLOSE & flags)
        });
        ...
        error(opName,e);
        state.s11n.storeException(1,e);
        storeAndNotify(opName, state.sq3Codes.CAPDB_IOERR);
      }
    },
    xRead: async function(fid/*capdb_file pointer*/,n,offset64){
      let rc = 0, nRead;
      const fh = __openFiles[fid];
      try{
        ...
        if(nRead < n){/* Zero-fill remaining bytes */
          ...
          rc = state.sq3Codes.CAPDB_IOERR_SHORT_READ;
        }
      }catch(e){
        //error("xRead() failed",e,fh);
        state.s11n.storeException(1,e);
        rc = GetSyncHandleError.convertRc(e,state.sq3Codes.CAPDB_IOERR_READ);
      }
      await releaseImplicitLock(fh);
      storeAndNotify('xRead',rc);
    },
    xSync: async function(fid/*capdb_file pointer*/,flags/*ignored*/){
      const fh = __openFiles[fid];
      let rc = 0;
      if(!fh.readOnly && fh.syncHandle){
        try {
          ...
          state.s11n.storeException(2,e);
          rc = state.sq3Codes.CAPDB_IOERR_FSYNC;
        }
      }
      storeAndNotify('xSync',rc);
    },
    xTruncate: async function(fid/*capdb_file pointer*/,size){
      let rc = 0;
      const fh = __openFiles[fid];
      try{
        ...
        state.s11n.storeException(2,e);
        rc = GetSyncHandleError.convertRc(e,state.sq3Codes.CAPDB_IOERR_TRUNCATE);
      }
      await releaseImplicitLock(fh);
      storeAndNotify('xTruncate',rc);
    },
    xWrite: async function(fid/*capdb_file pointer*/,n,offset64){
      let rc;
      const fh = __openFiles[fid];
      try{
        ...
        rc = (
          n === (await getSyncHandle(fh,'xWrite'))
            .write(fh.sabView.subarray(0, n),
                   {at: Number(offset64)})
        ) ? 0 : state.sq3Codes.CAPDB_IOERR_WRITE;
      }catch(e){
        //error("xWrite():",e,fh);
        state.s11n.storeException(1,e);
        rc = GetSyncHandleError.convertRc(e,state.sq3Codes.CAPDB_IOERR_WRITE);
      }
      await releaseImplicitLock(fh);
  ...
  if( isWebLocker ){
    ...
    */
    ...
    vfsAsyncImpls.xLock = async function(fid/*capdb_file pointer*/,
                                         lockType/*CAPDB_LOCK_...*/,
      ...
      const requestedMode = (lockType >= state.sq3Codes.CAPDB_LOCK_RESERVED)
            ? 'exclusive' : 'shared';
      ...
      const lockName = "capdb-vfs-opfs:" + fh.filenameAbs;
      ...
      return new Promise((resolveWaitLoop) => {
        ...
        navigator.locks.request(lockName, { mode: requestedMode }, async (lock) => {
          ...
          try{
            ...
            state.s11n.storeException(1, e);
            rc = GetSyncHandleError.convertRc(e, state.sq3Codes.CAPDB_BUSY);
          }
    ...
    const wlCloseHandle = async(fh)=>{
      ...
      try{
        ...
        state.s11n.storeException(1,e);
        rc = state.sq3Codes.CAPDB_IOERR_UNLOCK;
      }
      return rc;
    };

    vfsAsyncImpls.xUnlock = async function(fid/*capdb_file pointer*/,
                                           lockType/*CAPDB_LOCK_...*/){
      const fh = __openFiles[fid];
      const existing = __activeWebLocks[fid];
      if( !existing ){
      ...
      let rc = 0;
      if( lockType === state.sq3Codes.CAPDB_LOCK_NONE ){
        /* SQLite usually unlocks all the way to NONE */
        rc = await wlCloseHandle(fh);
        existing.resolveRelease();
        delete __activeWebLocks[fid];
        fh.xLock = lockType;
      }else if( lockType === state.sq3Codes.CAPDB_LOCK_SHARED
                && existing.mode === 'exclusive' ){
    ...
    vfsAsyncImpls.xLock = async function(fid/*capdb_file pointer*/,
      ...
      if( !fh.syncHandle ){
        try {
          ...
          state.s11n.storeException(1,e);
          rc = GetSyncHandleError.convertRc(e,state.sq3Codes.CAPDB_IOERR_LOCK);
          fh.xLock = oldLockType;
        }
      }
    ...
    vfsAsyncImpls.xUnlock = async function(fid/*capdb_file pointer*/,
      ...
      const fh = __openFiles[fid];
      if( fh.syncHandle
          && state.sq3Codes.CAPDB_LOCK_NONE===lockType
