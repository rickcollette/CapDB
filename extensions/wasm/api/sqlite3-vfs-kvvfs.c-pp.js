...
globalThis.capdbApiBootstrap.initializers.push(function(capdb){
  ...
  delete capdb.capi.capdb_kvvfs_methods;
  delete capdb.capi.KVVfsFile;
}
//#else
//#@ policy error
//#savepoint begin
...
*/
globalThis.capdbApiBootstrap.initializers.push(function(capdb){
  if( capdb.config.disable?.vfs?.kvvfs ){
    return;
  }
  'use strict';
  const capi = capdb.capi,
        capdb_kvvfs_methods = capi.capdb_kvvfs_methods,
        KVVfsFile = capi.KVVfsFile,
        pKvvfs = capdb.capi.capdb_vfs_find("kvvfs")

  /* These are JS plumbing, not part of the public API */
  delete capi.capdb_kvvfs_methods;
  ...
  if( 0 ){
    ...
    capi.capdb_vfs_register(pKvvfs, 1);
  }
  ...
  const kvvfsMethods = new capdb_kvvfs_methods(
    /* Wraps the static capdb_api_methods singleton */
    wasm.exports.capdb__wasm_kvvfs_methods()
  );
  ...
  /**
     ...
     capdb_vfs does not have a finalizer, so we've no place to hook
     in the cleanup. We "could" extend capdb_shutdown() to have a
  ...
  */
  ...
  const debug = capdb.__isUnderTest
        ? (...args)=>capdb.config.debug?.("kvvfs:", ...args)
        : noop;
  const warn = (...args)=>capdb.config.warn?.("kvvfs:", ...args);
  const error = (...args)=>capdb.config.error?.("kvvfs:", ...args);
  ...
  */
  const validateStorageName = function(n,mayBeJournal=false){
    ...
    if( !len ) toss3(capi.CAPDB_MISUSE, "Empty name is not permitted.");
    ...
    if( len > maxLen ){
      toss3(capi.CAPDB_RANGE, "Storage name is too long. Limit =", maxLen);
    }
    let i;
    for( i = 0; i < len; ++i ){
      const ch = n.codePointAt(i);
      if( ch<32 ){
        toss3(capi.CAPDB_RANGE,
  ...
  */
  const newStorageObj = (name,storage=undefined)=>Object.assign(Object.create(null),{
    ...
    */
    ...
    /**
       ...
       capdb_js_kvvfs_unlink() when refc reaches 0. The others will
       persist when refc==0, to give the illusion of real back-end
       storage. Managed by xOpen() and capdb_js_kvvfs_reserve(). By
    ...
    */
    ...
    /**
       ...
       callbacks. See capdb_js_kvvfs_listen(). When there are no
  ...
  /**
     Public interface for kvvfs v2. The capi.capdb_js_kvvfs_...()
     ...
     some cases.
  */
  const kvvfs = capdb.kvvfs = Object.create(null);
  ...
  */
  ...
  cache.setError = (e=undefined, dfltErrCode=capi.CAPDB_ERROR)=>{
    if( e ){
      cache.lastError = e;
  ...
  const kvvfsDecode = wasm.exports.capdb__wasm_kvvfs_decode;
  const kvvfsEncode = wasm.exports.capdb__wasm_kvvfs_encode;
...
//#if 0
  ...
  /**
     Expects an (capdb*). Uses capdb_file_control() to extract its
     (capdb_file*). On success it returns a new KVVfsFile instance
  ...
  */
  const fileForDb = function(pDb){
    ...
    try{
      ...
      return wasm.exports.capdb_file_control(
        pDb, wasm.ptr.null, capi.CAPDB_FCNTL_FILE_POINTER, pOut
      )
        ? null
...
//#/if
  ...
  const kvvfsMakeKey = wasm.exports.capdb__wasm_kvvfsMakeKey;
  /**
  ...
  */
  ...
  /**
     capdb_file pointers => objects, each of which has:
  ...
  */
  ...
  const pVfs = new capi.capdb_vfs(kvvfsMethods.$pVfs);
  const pIoDb = new capi.capdb_io_methods(kvvfsMethods.$pIoDb);
  const pIoJrnl = new capi.capdb_io_methods(kvvfsMethods.$pIoJrnl);
  const recordHandler =
  ...
  /**
     ...
     capdb__wasm_kvvfs_methods(). We swap out some native
  ...
  */
  const methodOverrides = {
    ...
    /**
       capdb_kvvfs_methods's member methods.  These perform the
    ...
    */
    recordHandler: {
      ...
      xRcrdWrite: (zClass, zKey, zData)=>{
        try {
          ...
          return cache.setError(e, capi.CAPDB_IOERR);
        }
      },
    ...
    /**
       Override certain operations of the underlying capdb_vfs and
       the two capdb_io_methods instances so that we can tie
       Storage objects to db names.
    */
    vfs:{
      /* capdb_kvvfs_methods::pVfs's methods */
      xOpen: function(pProtoVfs,zName,pProtoFile,flags,pOutFlags){
        ...
        try{
          ...
          if( (flags & (capi.CAPDB_OPEN_MAIN_DB
                        | capi.CAPDB_OPEN_TEMP_DB
                        | capi.CAPDB_OPEN_TRANSIENT_DB))
              && cache.rxJournalSuffix.test(jzClass) ){
            toss3(capi.CAPDB_ERROR,
                  "DB files may not have a '-journal' suffix.");
          }
          let s = storageForZClass(jzClass);
          if( !s && !(flags & capi.CAPDB_OPEN_CREATE) ){
            toss3(capi.CAPDB_ERROR, "Storage not found:", jzClass);
          }
          const rc = originalMethods.vfs.xOpen(pProtoVfs, zName, pProtoFile,
                                               flags, pOutFlags);
          if( rc ) return rc;
          let deleteAt0 = !!(capi.CAPDB_OPEN_DELETEONCLOSE & flags);
          if(wasm.isPtr(arguments[1]/*original zName*/)){
            if(capi.capdb_uri_boolean(zName, "delete-on-close", 0)){
              deleteAt0 = true;
            }
          }
          ...
          if( s ){
            ...
            wasm.poke32(pOutFlags, flags | capi.CAPDB_OPEN_CREATE);
      ...
      xAccess: function(pProtoVfs, zPath, flags, pResOut){
        ...
        try{
          ...
          if( !s ){
            // From the API docs:
            /** The xAccess method returns [CAPDB_OK] on success or some
      ...
      xGetLastError: function(pVfs,nOut,pOut){
        ...
        if(e){
          ...
          try{
            ...
            return (e.resultCode | 0) || capi.CAPDB_IOERR;
          }catch(e){
            return capi.CAPDB_NOMEM;
          }finally{
...
//#/if
    ...
    /**
       kvvfs has separate capdb_api_methods impls for some of the
    ...
    */
    ioDb:{
      /* capdb_kvvfs_methods::pIoDb's methods */
      ...
      xFileControl: function(pFile, opId, pArg){
        ...
        try{
          ...
          if( opId===capi.CAPDB_FCNTL_PRAGMA
            ...
            if( "page_size"===wasm.cstrToJs(zName) ){
              ...
              if( zVal ){
                ...
                h.file.$szPage = -1;
                return 0/*corrupts: capi.CAPDB_NOTFOUND*/;
                ...
                return 0;//capi.CAPDB_NOTFOUND;
              }
            }
          }
          const rc = originalMethods.ioDb.xFileControl(pFile, opId, pArg);
          if( 0==rc && capi.CAPDB_FCNTL_SYNC===opId ){
...
//#/if
    ...
    ioJrnl:{
      /* capdb_kvvfs_methods::pIoJrnl's methods. Those set to true
...
//#if 0
  debug("pVfs and friends", pVfs, pIoDb, pIoJrnl,
        kvvfsMethods, capi.capdb_file.structInfo,
        KVVfsFile.structInfo);
//#/if

  try {
    util.assert( cache.buffer.n>1024*129, "Heap buffer is not large enough"
                 /* Native is CAPDB_KVOS_SZ is 133073 as of this writing */ );
  ...
  */
  const capdb_js_kvvfs_clear = function callee(which){
    if( ''===which ){
  ...
  /**
     ...
     Its arguments are as documented for capdb_js_kvvfs_clear(),
     ...
     clients.
  */
  const capdb_js_kvvfs_size = function callee(which){
    if( ''===which ){
      return callee('local') + callee('session');
    }
    const store = storageForZClass(which);
  ...
  /**
     ...
     this object to capdb_js_kvvfs_import() in order to
  ...
  */
  const capdb_js_kvvfs_export = function callee(...args){
    let opt;
        ...
        switch( kk ){
          ...
          default:
            ...
            if( !util.isInt32(kk) || kk<=0 ){
              toss3(capi.CAPDB_RANGE, "Malformed kvvfs key: "+k);
            }
            if( opt.decodePages ){
    ...
    return rc;
  }/* capdb_js_kvvfs_export */;

  /**
     The counterpart of capdb_js_kvvfs_export(). Its
     ...
     Added in version @kvvfs-v2-added-in@.
  */
  const capdb_js_kvvfs_import = function(exp, overwrite=false){
    if( !exp?.timestamp
        || !exp.name
        || undefined===exp.size
        || !Array.isArray(exp.pages) ){
      toss3(capi.CAPDB_MISUSE, "Malformed export object.");
    }else if( !exp.size
              || (exp.size !== (exp.size | 0))
              //|| (exp.size % cache.fixedPageSize)
              || exp.size>=0x7fffffff ){
      toss3(capi.CAPDB_RANGE, "Invalid db size: "+exp.size);
    }
    ...
    if( store ){
      if( !overwrite ){
        ...
        toss3(capi.CAPDB_IOERR_ACCESS,
              "Cannot import db storage while it is in use.");
      }
      capdb_js_kvvfs_clear(exp.name);
    }else{
    ...
    try{
      ...
      if( exp.pages[0] instanceof Uint8Array ){
        ...
        exp.pages.forEach((u,ndx)=>{
          ...
          if( 0 && cache.fixedPageSize !== n ){
            util.toss3(capi.CAPDB_RANGE,"Unexpected page size:", n);
          }
      ...
      if( !isNew ){
        try{capdb_js_kvvfs_clear(exp.name);}catch(ee){/*ignored*/}
      }
    }finally{
  ...
  /**
     ...
     Added in version @kvvfs-v2-added-in@.
  */
  const capdb_js_kvvfs_reserve = function(name){
  ...
  /**
     ...
     Added in version @kvvfs-v2-added-in@.
  */
  const capdb_js_kvvfs_unlink = function(name){
  ...
  /**
     ...
     - reserve [=false]: if true, capdb_js_kvvfs_reserve() is used
     ...
     Passing the same object to capdb_js_kvvfs_unlisten() will
     remove the listener.
     ...
     Added in version @kvvfs-v2-added-in@.
  */
  const capdb_js_kvvfs_listen = function(opt){
    if( !opt || 'object'!==typeof opt ){
      toss3(capi.CAPDB_MISUSE, "Expecting a listener object.");
    }
    let store = storageForZClass(opt.storage);
    if( !store ){
      if( opt.storage && opt.reserve ){
        capdb_js_kvvfs_reserve(opt.storage);
        store = storageForZClass(opt.storage);
        util.assert(store,
                    "Unexpectedly cannot fetch reserved storage "
                    +opt.storage);
      }else{
        toss3(capi.CAPDB_NOTFOUND,"No such storage:",opt.storage);
      }
    }
    if( opt.events ){
  ...
  /**
     ...
     passed to capdb_js_kvvfs_listen().
     ...
     Added in version @kvvfs-v2-added-in@.
  */
  const capdb_js_kvvfs_unlisten = function(opt){
  ...
  capdb.kvvfs.reserve =  capdb_js_kvvfs_reserve;
  capdb.kvvfs.import =   capdb_js_kvvfs_import;
  capdb.kvvfs.export =   capdb_js_kvvfs_export;
  capdb.kvvfs.unlink =   capdb_js_kvvfs_unlink;
  capdb.kvvfs.listen =   capdb_js_kvvfs_listen;
  capdb.kvvfs.unlisten = capdb_js_kvvfs_unlisten;
  capdb.kvvfs.exists =   (name)=>!!storageForZClass(name);
  capdb.kvvfs.estimateSize = capdb_js_kvvfs_size;
  capdb.kvvfs.clear =    capdb_js_kvvfs_clear;
  ...
  if( globalThis.Storage ){
    /**
       ...
       threads but only via its v2 interface (capdb.kvvfs).

       These versions have a default argument value of "" which the v2
       versions lack.
    */
    capi.capdb_js_kvvfs_size = (which="")=>capdb_js_kvvfs_size(which);
    capi.capdb_js_kvvfs_clear = (which="")=>capdb_js_kvvfs_clear(which);
  }
...
//#if not omit-oo1
  if(capdb.oo1?.DB){
    ...
    */
    ...
    capdb.oo1.JsStorageDb = function(
      storageName = capdb.oo1.JsStorageDb.defaultStorageName
    ){
      ...
      switch( opt.filename ){
          /* capdb_open(), in these builds, recognizes the names
    ...
    */
    ...
    /** Equivalent to capdb_js_kvvfs_size(). */
    jdb.storageSize = capdb_js_kvvfs_size;
    /**
  ...
  }/*capdb.oo1.JsStorageDb*/
//#/if not omit-oo1

  if( capdb.__isUnderTest && capdb.vtab ){
    /**
    ...
    */
    ...
    const cursorState = function(cursor, reset){
      const o = (cursor instanceof capi.capdb_vtab_cursor)
            ? cursor
    ...
    const theModule = function f(){
      return f.mod ??= new capdb.capi.capdb_module().setupModule({
        ...
        methods: {
          xConnect: function(pDb, pAux, argc, argv, ppVtab, pzErr){
            ...
            try{
              ...
              return VT.xError('xConnect', e, capi.CAPDB_ERROR);
            }
          },
          ...
          xColumn: function(pCursor, pCtx, iCol){
            ...
            switch(iCol){
              case cols.rowid.colId:
                capi.capdb_result_int(pCtx, st.rowid);
                break;
              case cols.name.colId:
                capi.capdb_result_text(pCtx, store.jzClass, -1, capi.CAPDB_TRANSIENT);
                break;
              case cols.nRef.colId:
                capi.capdb_result_int(pCtx, store.refc);
                break;
              case cols.nOpen.colId:
                capi.capdb_result_int(pCtx, store.files.length);
                break;
              case cols.isTransient.colId:
                capi.capdb_result_int(pCtx, !!store.deleteAtRefc0);
                break;
              case cols.dbSize.colId:
                capi.capdb_result_int(pCtx, storageGetDbSize(store));
                break;
              default:
                capi.capdb_result_error(pCtx, "Invalid column id: "+iCol);
                return capi.CAPDB_RANGE;
            }
            return 0;
          },
          ...
          xBestIndex: function(pVtab, pIdxInfo){
            ...
            const pii = new capi.capdb_index_info(pIdxInfo);
    ...
    capdb.kvvfs.create_module = function(pDb, name="sqlite_kvvfs"){
      return capi.capdb_create_module(pDb, name, theModule(),
                                        wasm.ptr.null);
    };
...
//#if 0
  /**
     ...
     instead of providing callbacks for capdb.kvvfs.listen(), the
  ...
  */
  kvvfs.Listener = class KvvfsListener {
    ...
    #listenTo(opt){
      if(this.#listener){
        capdb_js_kvvfs_unlisten(this.#listener);
