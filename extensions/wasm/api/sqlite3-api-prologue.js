/*
  ...
  ***********************************************************************
  ...
  loads the corresponding WASM module. The capdb JS API has no hard
...
/**
   capdbApiBootstrap() is the only global symbol persistently
   ...
   environments. That said, the "capdb-api.js" intermediary build
   ...
   - `allocExportName`: the name of the function, in `exports`, of the
     ...
     to `"capdb_malloc"`. Beware that using any allocator other than
     capdb_malloc() may require care in certain client-side code
     regarding which allocator is uses. Notably, capdb_deserialize()
     and capdb_serialize() can only safely use memory from different
     allocators under very specific conditions. The canonical builds
     of this API guaranty that `capdb_malloc()` is the JS-side
     allocator implementation.

   - `deallocExportName`: the name of the function, in `exports`, of
     the `free(3)`-compatible routine for the WASM
     environment. Defaults to `"capdb_free"`.

   - `reallocExportName`: the name of the function, in `exports`, of
     the `realloc(3)`-compatible routine for the WASM
     environment. Defaults to `"capdb_realloc"`.
   ...
   The returned object is the top-level capdb namespace object.


   Client code may optionally assign capdbApiBootstrap.defaultConfig
   an object-type value before calling capdbApiBootstrap() (without
   ...
   capdbApiBootstrap() themselves, nor to access it before it is
   called, an alternative option for setting the configuration is to
   define globalThis.capdbApiConfig to an object. If it is set, it
   is used instead of capdbApiBootstrap.defaultConfig if
   capdbApiBootstrap() is called without arguments. Setting the
   `exports` and `memory` parts require already having loaded the WASM
   module, though.

   Both capdbApiBootstrap.defaultConfig and
   globalThis.capdbApiConfig get deleted by capdbApiBootstrap()
   ...
   This function returns a Promise to the capdb namespace object,
   which resolves after the async pieces of the library init are
   complete.
*/
'use strict';
globalThis.capdbApiBootstrap = async function capdbApiBootstrap(
  apiConfig = (globalThis.capdbApiConfig || capdbApiBootstrap.defaultConfig)
){
  if(capdbApiBootstrap.capdb){ /* already initialized */
    (capdbApiBootstrap.capdb.config || console).warn(
      "capdbApiBootstrap() called multiple times.",
      "Config and external initializers are ignored on calls after the first."
    );
    return capdbApiBootstrap.capdb;
  }
  ...
  const config = nu({
    ...
    /**
       ...
       it becomes significant with, e.g., capdb_deserialize() and
  ...
  Object.assign(config, {
    allocExportName: config.useStdAlloc ? 'malloc' : 'capdb_malloc',
    deallocExportName: config.useStdAlloc ? 'free' : 'capdb_free',
    reallocExportName: config.useStdAlloc ? 'realloc' : 'capdb_realloc'
  });
  ...
  /**
      The main capdb binding API gets installed into this object,
      mimicking the C API as closely as we can. The numerous members
      names with prefixes 'capdb_' and 'CAPDB_' behave, insofar as
      ...
      in this file or in capdb-api-glue.js. capi members which are
  ...
  */
  ...
  const __rcStr = (rc)=>{
    return (capi.capdb_js_rc_str && capi.capdb_js_rc_str(rc))
  ...
  */
  class SQLite3Error extends Error {
    /**
       ...
       an CAPDB_... result code and it is passed to
       capdb.capi.capdb_js_rc_str() to stringify it.
       ...
       else it will be set to capi.CAPDB_ERROR.
    */
    constructor(...args){
      let rc;
      if(args.length){
      ...
      this.resultCode = rc || capi.CAPDB_ERROR;
  ...
  class WasmAllocError extends Error {
    ...
    */
    constructor(...args){
      ...
      this.resultCode = capi.CAPDB_NOMEM;
  ...
  */
  ...
  Object.assign(capi, {
    /**
       capdb_bind_blob() works exactly like its C counterpart unless
       ...
       ignored and capi.CAPDB_WASM_DEALLOC is assumed.
       ...
       above-described types, capi.CAPDB_MISUSE is returned.

       The first argument may be either an `capdb_stmt*` WASM
       pointer or an capdb.oo1.Stmt instance.

       For consistency with the C API, it requires the same number of
       arguments. It returns capi.CAPDB_MISUSE if passed any other
       argument count.
    */
    capdb_bind_blob: undefined/*installed later*/,

    /**
       capdb_bind_text() works exactly like its C counterpart unless
       its 3rd argument is one of:
    ...
    */
    capdb_bind_text: undefined/*installed later*/,

    /**
       capdb_create_function_v2() differs from its native
       counterpart only in the following ways:

       1) The fourth argument (`eTextRep`) argument must not specify
       any encoding other than capdb.CAPDB_UTF8. The JS API does not
       ...
       the bottom 4 bits of that argument are 0, CAPDB_UTF8 is
       assumed.
       ...
       capdb.capi.wasm.installFunction() and that wrapper is passed
       ...
       - `pCtx` in the above descriptions is a `capdb_context*`. At
       ...
       - If a JS wrapper throws, it gets translated to
         capdb_result_error() or capdb_result_error_nomem(),
         depending on whether the exception is an
         capdb.WasmAllocError object or not.
       ...
       - When passing on WASM function pointers, arguments are _not_
         ...
         exposed for client-side use as capdb_values_to_js(),
         capdb_result_js(), and capdb_result_error_js().
       ...
       or (B) when pDb is passed to capdb_close_v2(). If this factor
    ...
    */
    ...
    /**
       ...
       capdb_create_function_v2(), with 0 as the final argument.
    */
    ...
    /**
       The capdb_create_window_function() JS wrapper differs from
       its native implementation in the exact same way that
       capdb_create_function_v2() does. The additional function,
    ...
    */
    ...
    /**
       The capdb_prepare_v3() binding handles two different uses
       with differing JS/WASM semantics:

       1) capdb_prepare_v3(pDb, sqlString, -1, prepFlags, ppStmt , null)

       2) capdb_prepare_v3(pDb, sqlPointer, sqlByteLen, prepFlags, ppStmt, sqlPointerToPointer)
       ...
       documented for the C-side capdb_prepare_v3().
       ...
       return CAPDB_MISUSE and capdb_errmsg() will contain a string
    ...
    */
    capdb_prepare_v3: (dbPtr, sql, sqlByteLen, prepFlags,
                         stmtPtrPtr, strPtrPtr)=>{}/*installed later*/,

    /**
       Equivalent to calling capdb_prapare_v3() with 0 as its 4th argument.
    */
    capdb_prepare_v2: (dbPtr, sql, sqlByteLen,
                         stmtPtrPtr,strPtrPtr)=>{}/*installed later*/,
    ...
    /**
       ...
       capdb_exec() call, it installs a WASM-bound function which
    ...
    */
    capdb_exec: (pDb, sql, callback, pVoid, pErrMsg)=>{}/*installed later*/,
    ...
    /**
       ...
       values.
     */
    capdb_randomness: (n, outPtr)=>{/*installed later*/},
  }/*capi*/);
...
//#/if
  ...
  /**
     ...
     a number of WASM-specific utilities, in capdb-api-glue.c-pp.js.
  */
  Object.assign(wasm, {
    ...
    */
    ...
    /**
       ...
       capdb.wasm (whwasmutil.js) can figure this out _if_ they can
       ...
       (here) instead.
    */
    pointerSize: ('number'===typeof config.exports.capdb_libversion()) ? 4 : 8,

    /**
       True if BigInt support was enabled via (e.g.) the
       Emscripten -sWASM_BIGINT flag, else false. When
       enabled, certain 64-bit capdb APIs are enabled which
    ...
    */
    ...
    /**
       ...
       allocator.  Works like capdb_malloc() but throws a
       WasmAllocError if allocation fails. It is important that any
       code which might pass through the capdb C API NOT throw and
       must instead return CAPDB_NOMEM (or equivalent, depending on
       the context).

       Very few cases in the capdb JS APIs can result in
       ...
       capdb_create_function_v2(). Such bindings created
    ...
    */
    ...
    /**
       ...
       capdb_realloc(M,N), where M is either NULL or a pointer
       ...
       Results are undefined if N is negative (capdb_realloc()
    ...
    */
    ...
    /**
       The API's primary point of access to the WASM-side memory
       deallocator. Works like capdb_free().
  ...
  */
  ...
  /**
     ...
     capdb_compileoption_get() and capdb_compileoption_used(). It
     ...
     active when when compiling the capdb module, else false.

     Compile-time option names may optionally include their
     "CAPDB_" prefix. When it returns an object of all options,
  ...
  */
  wasm.compileOptionUsed = function f(optName){
    if(!arguments.length){
      ...
      const rc = nu(), ov = [0,0];
      let i = 0, k;
      while((k = capi.capdb_compileoption_get(i++))){
        f._opt(k,ov);
        rc[ov[0]] = ov[1];
      }
      return f._result = rc;
      ...
      const rc = nu();
      optName.forEach((v)=>{
        rc[v] = capi.capdb_compileoption_used(v);
      });
      return rc;
    }else if('object' === typeof optName){
      Object.keys(optName).forEach((k)=> {
        optName[k] = capi.capdb_compileoption_used(k);
      });
      return optName;
    }
    return (
      'string'===typeof optName
    ) ? !!capi.capdb_compileoption_used(optName) : false;
  }/*compileOptionUsed()*/;

  /**
     capdb.wasm.pstack (pseudo-stack) holds a special-case allocator
     ...
     pretty common when using capdb.capi).
  ...
  */
  wasm.pstack = nu({
    ...
    */
    restore: wasm.exports.capdb__wasm_pstack_restore,
    ...
    */
    alloc: function(n){
      ...
      return wasm.exports.capdb__wasm_pstack_alloc(n)
    ...
    */
    ...
    /**
       ...
       passing it the capdb object, then restores the pstack
  ...
  Object.defineProperties(wasm.pstack, {
    ...
    */
    quota: {
      configurable: false, iterable: true, writeable: false,
      get: wasm.exports.capdb__wasm_pstack_quota
    },
    ...
    */
    remaining: {
      configurable: false, iterable: true, writeable: false,
      get: wasm.exports.capdb__wasm_pstack_remaining
    }
  })/*wasm.pstack properties*/;

  /**
     Docs: https://sqlite.org/wasm/doc/trunk/api-c-style.md#capdb_randomness
  */
  capi.capdb_randomness = (...args)=>{
    if(1===args.length
       && util.isTypedArray(args[0])
       && 1===args[0].BYTES_PER_ELEMENT){
      const ta = args[0];
      if(0===ta.byteLength){
        wasm.exports.capdb_randomness(0,wasm.ptr.null);
        return ta;
      }
      const stack = wasm.pstack.pointer;
      try {
        let n = ta.byteLength, offset = 0;
        const r = wasm.exports.capdb_randomness;
        ...
        config.error("Highly unexpected (and ignored!) "+
                     "exception in capdb_randomness():",e);
      }finally{
        wasm.pstack.restore(stack);
      }
      return ta;
    }
    wasm.exports.capdb_randomness(...args);
  };
  ...
  */
  capi.capdb_wasmfs_opfs_dir = function(){
    ...
    if(!pdir
       ...
       || !wasm.exports.capdb__wasm_init_wasmfs){
      return this.dir = "";
    }
    try{
      if(pdir && 0===wasm.xCallWrapped(
        'capdb__wasm_init_wasmfs', 'i32', ['string'], pdir
      )){
      ...
      // capdb__wasm_init_wasmfs() is not available
      return this.dir = "";
    }
  }.bind(nu());

  /**
     Returns true if capdb.capi.capdb_wasmfs_opfs_dir() is a
     non-empty string and the given name starts with (that string +
     '/'), else returns false.
  */
  capi.capdb_wasmfs_filename_is_persistent = function(name){
    const p = capi.capdb_wasmfs_opfs_dir();
    return (p && name) ? name.startsWith(p+'/') : false;
  };

  /**
     Given an `capdb*`, an capdb_vfs name, and an optional db name
     ...
     The truthy value it returns is a pointer to the `capdb_vfs`
     object.
     ...
     false is returned.
  */
  capi.capdb_js_db_uses_vfs = function(pDb,vfsName,dbName=0){
    try{
      const pK = capi.capdb_vfs_find(vfsName);
      if(!pK) return false;
      else if(!pDb){
        return pK===capi.capdb_vfs_find(0) ? pK : false;
      }else{
        return pK===capi.capdb_js_db_vfs(pDb,dbName) ? pK : false;
      }
    }catch(e){
  ...
  /**
     Returns an array of the names of all currently-registered capdb
     VFSes.
  */
  capi.capdb_js_vfs_list = function(){
    const rc = [];
    let pVfs = capi.capdb_vfs_find(wasm.ptr.null);
    while(pVfs){
      const oVfs = new capi.capdb_vfs(pVfs);
  ...
  /**
     A convenience wrapper around capdb_serialize() which serializes
     the given `capdb*` pointer to a Uint8Array. The first argument
     may be either an `capdb*` or an capdb.oo1.DB instance.
  ...
  */
  capi.capdb_js_db_export = function(pDb, schema=0){
    pDb = wasm.xWrap.testConvertArg('capdb*', pDb);
    if(!pDb) toss3('Invalid capdb* argument.');
    ...
    try{
      ...
      */
      const zSchema = schema
            ...
            : wasm.ptr.null;
      let rc = wasm.exports.capdb__wasm_db_serialize(
        pDb, zSchema, ppOut, pSize, 0
      );
      if(rc){
        toss3("Database serialization failed with code",
             capdb.capi.capdb_js_rc_str(rc));
      }
      ...
      if(pOut) wasm.exports.capdb_free(pOut);
      wasm.scopedAllocPop(scope);
    }
  };

  /**
     Given a `capdb*` and a database name (JS string or WASM
     C-string pointer, which may be 0), returns a pointer to the
     capdb_vfs responsible for it. If the given db name is null/0,
     or not provided, then "main" is assumed.
  */
  capi.capdb_js_db_vfs =
    (dbPointer, dbName=wasm.ptr.null)=>util.capdb__wasm_db_vfs(dbPointer, dbName);

  /**
     A thin wrapper around capi.capdb_aggregate_context() which
  ...
  */
  capi.capdb_js_aggregate_context = (pCtx, n)=>{
    return capi.capdb_aggregate_context(pCtx, n)
      || (n ? WasmAllocError.toss("Cannot allocate",n,
                                  "bytes for capdb_aggregate_context()")
          : 0);
  };
  ...
  /**
     ...
     capdb_js_vfs_create_file().
  */
  capi.capdb_js_posix_create_file = function(filename, data, dataLen){
    let pData;
    if(data && wasm.isPtr(data)){
      ...
      SQLite3Error.toss("Invalid 2nd argument for capdb_js_posix_create_file().");
    }
    try{
      if(!util.isInt32(dataLen) || dataLen<0){
        SQLite3Error.toss("Invalid 3rd argument for capdb_js_posix_create_file().");
      }
      const rc = util.capdb__wasm_posix_create_file(filename, pData, dataLen);
      if(rc) SQLite3Error.toss("Creation of file failed with capdb result code",
                               capi.capdb_js_rc_str(rc));
    }finally{
  ...
  /**
     ...
     debug builds of capdb because its out-of-scope use of the
     capdb_vfs API triggers assertions in the core library.  That
     ...
     - The "unix" VFS and its variants can get equivalent
       functionality with capdb_js_posix_create_file().

     - OPFS: use either capdb.oo1.OpfsDb.importDb(), for the "opfs"
     ...
     capdb_vfs.  The first argument may be a VFS name (JS string
     only, NOT a WASM C-string), WASM-managed `capdb_vfs*`, or
     a capi.capdb_vfs instance. Pass 0 (a NULL pointer) to use the
     default VFS. If passed a string which does not resolve using
     capdb_vfs_find(), an exception is thrown. (Note that a WASM
     C-string is not accepted because it is impossible to
     distinguish from a C-level `capdb_vfs*`.)
  ...
  */
  capi.capdb_js_vfs_create_file = function(vfs, filename, data, dataLen){
    config.warn("capdb_js_vfs_create_file() is deprecated and",
    ...
    if(data){
      if( wasm.isPtr(data) ){
        ...
        if( data instanceof Uint8Array ){
          ...
          SQLite3Error.toss("Invalid 3rd argument type for capdb_js_vfs_create_file().");
        }
      }
    }else{
       pData = 0;
    ...
    if(!util.isInt32(dataLen) || dataLen<0){
      if( pData && pData!==data ) wasm.dealloc(pData);
      SQLite3Error.toss("Invalid 4th argument for capdb_js_vfs_create_file().");
    }
    try{
      const rc = util.capdb__wasm_vfs_create_file(vfs, filename, pData, dataLen);
      if(rc) SQLite3Error.toss("Creation of file failed with capdb result code",
                               capi.capdb_js_rc_str(rc));
    }finally{
       if( pData && pData!==data ) wasm.dealloc(pData);
  ...
  /**
     ...
     Added in 3.44
  */
  capi.capdb_js_sql_to_string = (sql)=>{
  ...
  /**
     ...
     Returns capi.CAPDB_MISUSE if op is not a valid operation ID.

     The variants which take `(int, int*)` arguments treat a
     missing or falsy pointer argument as 0.
  */
  capi.capdb_db_config = function(pDb, op, ...args){
    switch(op){
      case capi.CAPDB_DBCONFIG_ENABLE_FKEY:
      case capi.CAPDB_DBCONFIG_ENABLE_TRIGGER:
      case capi.CAPDB_DBCONFIG_ENABLE_FTS3_TOKENIZER:
      case capi.CAPDB_DBCONFIG_ENABLE_LOAD_EXTENSION:
      case capi.CAPDB_DBCONFIG_NO_CKPT_ON_CLOSE:
      case capi.CAPDB_DBCONFIG_ENABLE_QPSG:
      case capi.CAPDB_DBCONFIG_TRIGGER_EQP:
      case capi.CAPDB_DBCONFIG_RESET_DATABASE:
      case capi.CAPDB_DBCONFIG_DEFENSIVE:
      case capi.CAPDB_DBCONFIG_WRITABLE_SCHEMA:
      case capi.CAPDB_DBCONFIG_LEGACY_ALTER_TABLE:
      case capi.CAPDB_DBCONFIG_DQS_DML:
      case capi.CAPDB_DBCONFIG_DQS_DDL:
      case capi.CAPDB_DBCONFIG_ENABLE_VIEW:
      case capi.CAPDB_DBCONFIG_LEGACY_FILE_FORMAT:
      case capi.CAPDB_DBCONFIG_TRUSTED_SCHEMA:
      case capi.CAPDB_DBCONFIG_STMT_SCANSTATUS:
      case capi.CAPDB_DBCONFIG_REVERSE_SCANORDER:
      case capi.CAPDB_DBCONFIG_ENABLE_ATTACH_CREATE:
      case capi.CAPDB_DBCONFIG_ENABLE_ATTACH_WRITE:
      case capi.CAPDB_DBCONFIG_ENABLE_COMMENTS:
      case capi.CAPDB_DBCONFIG_FP_DIGITS:
        if( !this.ip ){
          this.ip = wasm.xWrap('capdb__wasm_db_config_ip','int',
                               ['capdb*', 'int', 'int', '*']);
        }
        return this.ip(pDb, op, args[0], args[1] || 0);
      case capi.CAPDB_DBCONFIG_LOOKASIDE:
        if( !this.pii ){
          this.pii = wasm.xWrap('capdb__wasm_db_config_pii', 'int',
                                ['capdb*', 'int', '*', 'int', 'int']);
        }
        return this.pii(pDb, op, args[0], args[1], args[2]);
      case capi.CAPDB_DBCONFIG_MAINDBNAME:
        if(!this.s){
          this.s = wasm.xWrap('capdb__wasm_db_config_s','int',
                              ['capdb*', 'int', 'string:static']
  ...
  /**
     Given a (capdb_value*), this function attempts to convert it
     ...
     - It does not support capdb_value_to_pointer() conversions
       ...
       arguments). Clients using capdb_value_to_pointer(), and its
       related APIs, will need to manage those themselves.
  */
  capi.capdb_value_to_js = function(pVal,throwIfCannotConvert=true){
    let arg;
    const valType = capi.capdb_value_type(pVal);
    switch(valType){
        case capi.CAPDB_INTEGER:
          if(wasm.bigIntEnabled){
            arg = capi.capdb_value_int64(pVal);
            if(util.bigIntFitsDouble(arg)) arg = Number(arg);
          }
          else arg = capi.capdb_value_double(pVal)/*yes, double, for larger integers*/;
          break;
        case capi.CAPDB_FLOAT:
          arg = capi.capdb_value_double(pVal);
          break;
        case capi.CAPDB_TEXT:
          arg = capi.capdb_value_text(pVal);
          break;
        case capi.CAPDB_BLOB:{
          const n = capi.capdb_value_bytes(pVal);
          const pBlob = capi.capdb_value_blob(pVal);
          if(n && !pBlob) capdb.WasmAllocError.toss(
        ...
        case capi.CAPDB_NULL:
          arg = null; break;
        default:
          if(throwIfCannotConvert){
            toss3(capi.CAPDB_MISMATCH,
                  "Unhandled capdb_value_type():",valType);
          }
          arg = undefined;
    }
    return arg;
  ...
  /**
     Requires a C-style array of `capdb_value*` objects and the
     number of entries in that array. Returns a JS array containing
     the results of passing each C array entry to
     capdb_value_to_js(). The 3rd argument to this function is
     passed on as the 2nd argument to that one.
  */
  capi.capdb_values_to_js = function(argc,pArgv,throwIfCannotConvert=true){
    let i;
    const tgt = [];
    for(i = 0; i < argc; ++i){
      /**
         ...
         do not.
      */
      tgt.push(capi.capdb_value_to_js(
  ...
  /**
     Calls either capdb_result_error_nomem(), if e is-a
     WasmAllocError, or capdb_result_error(). In the latter case,
     the second argument is coerced to a string to create the error
     message.

     The first argument is a (capdb_context*). Returns void.
     Does not throw.
  */
  capi.capdb_result_error_js = function(pCtx,e){
    if(e instanceof WasmAllocError){
      capi.capdb_result_error_nomem(pCtx);
    }else{
      ...
      capi.capdb_result_error(pCtx, ''+e, -1);
    }
  };

  /**
     This function passes its 2nd argument to one of the
     capdb_result_xyz() routines, depending on the type of that
     argument:

     - If (val instanceof Error), this function passes it to
       capdb_result_error_js().
     - `null`: `capdb_result_null()`
     - `boolean`: `capdb_result_int()` with a value of 0 or 1.
     - `number`: `capdb_result_int()`, `capdb_result_int64()`, or
       `capdb_result_double()`, depending on the range of the number
     ...
     - `string`: `capdb_result_text()`
     - Uint8Array or Int8Array or ArrayBuffer: `capdb_result_blob()`
     - `undefined`: is a no-op provided to simplify certain use cases.

     Anything else triggers `capdb_result_error()` with a
     description of the problem.

     The first argument to this function is a `(capdb_context*)`.
     Returns void. Does not throw.
  */
  capi.capdb_result_js = function(pCtx,val){
    if(val instanceof Error){
      capi.capdb_result_error_js(pCtx, val);
      return;
    }
    try{
      switch(typeof val) {
          case 'undefined':
            /* This is a no-op. This routine originated in the create_function()
               ...
               that the caller was responsible for calling capdb_result_xxx()
               (if needed). */
            break;
          case 'boolean':
            capi.capdb_result_int(pCtx, val ? 1 : 0);
            break;
          case 'bigint':
            if(util.bigIntFits32(val)){
              capi.capdb_result_int(pCtx, Number(val));
            }else if(util.bigIntFitsDouble(val)){
              capi.capdb_result_double(pCtx, Number(val));
            }else if(wasm.bigIntEnabled){
              if(util.bigIntFits64(val)) capi.capdb_result_int64(pCtx, val);
          ...
          case 'number': {
            let f;
            if(util.isInt32(val)){
              f = capi.capdb_result_int;
            }else if(wasm.bigIntEnabled
                     && Number.isInteger(val)
                     && util.bigIntFits64(BigInt(val))){
              f = capi.capdb_result_int64;
            }else{
              f = capi.capdb_result_double;
            }
            f(pCtx, val);
            break;
          }
          case 'string': {
            ...
            capi.capdb_result_text(pCtx, p, n, capi.CAPDB_WASM_DEALLOC);
            break;
          }
          case 'object':
            if(null===val/*yes, typeof null === 'object'*/) {
              capi.capdb_result_null(pCtx);
              break;
            }else if(util.isBindableTypedArray(val)){
              const pBlob = wasm.allocFromTypedArray(val);
              capi.capdb_result_blob(
                pCtx, pBlob, val.byteLength,
                capi.CAPDB_WASM_DEALLOC
              );
              break;
            }
            // else fall through
  ...
  /**
     Returns the result capdb_column_value(pStmt,iCol) passed to
     capdb_value_to_js(). The 3rd argument of this function is
     ignored by this function except to pass it on as the second
     argument of capdb_value_to_js(). If the capdb_column_value()
     ...
     Note that capdb_column_value() returns an "unprotected" value
  ...
  */
  capi.capdb_column_js = function(pStmt, iCol, throwIfCannotConvert=true){
    const v = capi.capdb_column_value(pStmt, iCol);
    return (0===v) ? undefined : capi.capdb_value_to_js(v, throwIfCannotConvert);
  };

  if( true ){ /* changeset/preupdate additions... */
    /**
       Internal impl of capdb_preupdate_new/old_js() and
       capdb_changeset_new/old_js().
    */
    const __newOldValue = function(pObj, iCol, impl){
      ...
      return pv ? capi.capdb_value_to_js( pv, true ) : undefined;
    }.bind(nu());

    /**
       A wrapper around capdb_preupdate_new() which fetches the
       capdb_value at the given index and returns the result of
       passing it to capdb_value_to_js(). Throws on error.
    */
    capi.capdb_preupdate_new_js =
      (pDb, iCol)=>__newOldValue(pDb, iCol, 'capdb_preupdate_new');

    /**
       The capdb_preupdate_old() counterpart of
       capdb_preupdate_new_js(), with an identical interface.
    */
    capi.capdb_preupdate_old_js =
      (pDb, iCol)=>__newOldValue(pDb, iCol, 'capdb_preupdate_old');

    /**
       A wrapper around capdb_changeset_new() which fetches the
       capdb_value at the given index and returns the result of
       passing it to capdb_value_to_js(). Throws on error.

       If capdb_changeset_new() succeeds but has no value to report,
       this function returns the undefined value, noting that
       undefined is not a valid conversion from an `capdb_value`, so
       is unambiguous.
    */
    capi.capdb_changeset_new_js =
      (pChangesetIter, iCol) => __newOldValue(pChangesetIter, iCol,
                                              'capdb_changeset_new');

    /**
       The capdb_changeset_old() counterpart of
       capdb_changeset_new_js(), with an identical interface.
    */
    capi.capdb_changeset_old_js =
      (pChangesetIter, iCol)=>__newOldValue(pChangesetIter, iCol,
                                            'capdb_changeset_old');
  }/*changeset/preupdate additions*/

  /**
     capdb_js_retry_busy(maxTimes,callback[,beforeRetry])

     Calls the given _synchronous_ callback function. If that function
     returns capdb.capi.CAPDB_BUSY _or_ throws an SQLite3Error
     ...
     capdb.capi.CAPDB_BUSY is thrown. If the callback throws any
  ...
  */
  capi.capdb_js_retry_busy = function(maxTimes, callback, beforeRetry){
    for(let n = 1; n <= maxTimes; ++n){
      try{
        if( beforeRetry && n>1 ) beforeRetry(n);
        const rc = callback();
        if( capi.CAPDB_BUSY===rc ){
          if( n===maxTimes ){
            throw new SQLite3Error(rc, [
              "capdb_js_retry_busy() max retry attempts (",
              maxTimes,
              ") reached."
            ].join(''));
          }
  ...
  const capdb = {
    ...
    config,
    /**
       Holds the version info of the capdb source tree from which
       the generated capdb-api.js gets built. Its version may well
       differ from that reported by capdb_libversion(), but that
    ...
    */
    ...
    /**
       ...
       resolves to the capdb namespace object. Any error in the
    ...
    */
    asyncPostInit: async function ff(){
      ...
      const postInit = async ()=>{
        if(!capdb.__isUnderTest){
          /* Delete references to internal-only APIs which are used by
             some initializers. Retain them when running in test mode
             so that we can add tests for them. */
          delete capdb.util;
          delete capdb.StructBinder;
          delete capdb.opfs;
        }
        return capdb;
      };
      const catcher = (e)=>{
        config.error("an async capdb initializer failed:",e);
        throw e;
      };
      if(!lia || !lia.length){
      ...
      lia = lia.map((f)=>{
        return (f instanceof Function) ? async x=>f(capdb) : f;
      });
      lia.push(postInit);
      let p = Promise.resolve(capdb);
      while(lia.length) p = p.then(lia.shift());
      return ff.isReady = p.catch(catcher);
    }.bind(capdbApiBootstrap),
    /**
       ...
       before capdbApiBootstrap() can be declared. It is not
       necessarily available to any capdbApiBootstrap.initializers
       but "should" be in place (if it's added at all) by the time
       that capdbApiBootstrap.initializersAsync is processed.

       This state is not part of the public API, only intended for use
       with the capdb API bootstrapping and wasm-loading process.
    */
    scriptInfo: undefined
  };
  if( 'undefined'!==typeof capdbIsUnderTest/* from post-js-header.js */ ){
    capdb.__isUnderTest = !!capdbIsUnderTest;
  }
  try{
    capdbApiBootstrap.initializers.forEach((f)=>{
      f(capdb);
    });
  }catch(e){
    /* If we don't report this here, it can get completely swallowed
       up and disappear into the abyss of Promises and Workers. */
    console.error("capdb bootstrap initializer threw:",e);
    throw e;
  }
  delete capdbApiBootstrap.initializers;
  capdbApiBootstrap.capdb = capdb;
  if( 'undefined'!==typeof capdbInitScriptInfo/* from post-js-header.js */ ){
    capdbInitScriptInfo.debugModule(
      "capdbApiBootstrap() complete", capdb
    );
    capdb.scriptInfo
    /* Used by some async init code. As of 2025-11-15 this is still
       ...
       that VFS loading. */ = capdbInitScriptInfo;
  }
  if( capdb.__isUnderTest ){
    if( 'undefined'!==typeof EmscriptenModule ){
      capdb.config.emscripten = EmscriptenModule;
    }
    /*
      The problem with exposing these pieces (in non-testing runs) via
      capdb.wasm is that it exposes non-SQLite pieces to the
    ...
    */
    const iw = capdb.scriptInfo?.instantiateWasm;
    if( iw ){
      /* Metadata injected by the custom Module.instantiateWasm()
         in pre-js.c-pp.js. */
      capdb.wasm.module = iw.module;
      capdb.wasm.instance = iw.instance;
      capdb.wasm.imports = iw.imports;
    }
  }
  ...
  /**
     ...
     objects...
  */
  delete globalThis.capdbApiConfig;
  delete globalThis.capdbApiBootstrap;
  delete capdbApiBootstrap.defaultConfig;
  return capdb.asyncPostInit().then((s)=>{
    if( 'undefined'!==typeof capdbInitScriptInfo/* from post-js-header.js */ ){
      capdbInitScriptInfo.debugModule(
        "capdb.asyncPostInit() complete", s
      );
    }
...
/**
  globalThis.capdbApiBootstrap.initializers is an internal detail
  used by the various pieces of the capdb API's amalgamation
  ...
  to this array. When capdbApiBootstrap() is called for the first
  time, each such function will be called (in their appended order)
  and passed the capdb namespace object, into which they can install
  their features. At the end of that process, this array is deleted.

  The order of insertion into this array is significant for
  some pieces. e.g. capdb.capi and capdb.wasm cannot be fully
  utilized until the whwasmutil.js part is plugged in via
  capdb-api-glue.js.
*/
globalThis.capdbApiBootstrap.initializers = [];

/**
  globalThis.capdbApiBootstrap.initializersAsync is an internal detail
  used by the capdb API's amalgamation process. It must not be
  modified by client code except when plugging such code into the
  amalgamation process.

  The counterpart of globalThis.capdbApiBootstrap.initializers,
  ...
  with the capdb object as its only argument.
  ...
  capdb.asyncPostInit(). This means, for example, that intializers
  added to globalThis.capdbApiBootstrap.initializers may push entries to
  this list.
*/
globalThis.capdbApiBootstrap.initializersAsync = [];

/**
   Client code may assign capdbApiBootstrap.defaultConfig an
   object-type value before calling capdbApiBootstrap() (without
   arguments) in order to tell that call to use this object as its
   default config value. The intention of this is to provide
   ...
   global-scope symbol.
*/
globalThis.capdbApiBootstrap.defaultConfig = Object.create(null);

/**
   Placeholder: gets installed by the first call to
   globalThis.capdbApiBootstrap(). However, it is recommended that the
   caller of capdbApiBootstrap() capture its return value and delete
   globalThis.capdbApiBootstrap after calling it. It returns the same
   value which will be stored here.
*/
globalThis.capdbApiBootstrap.capdb = undefined;
