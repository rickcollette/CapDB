...
/*
  ...
  ***********************************************************************
  ...
  This file holds code shared by capdb-vfs-opfs{,-wl}.c-pp.js. It
  creates a private/internal capdb.opfs namespace common to the two
  and used (only) by them and the test framework. It is not part of
  the public API. The library deletes capdb.opfs in its final
...
globalThis.capdbApiBootstrap.initializers.push(function(capdb){
  'use strict';
  if( capdb.config.disable?.vfs?.opfs &&
      capdb.config.disable.vfs['opfs-vfs'] ){
    return;
  }
  const toss = capdb.util.toss,
        capi = capdb.capi,
        util = capdb.util,
        wasm = capdb.wasm;
  ...
  /**
     ...
     by the Promise setup and, on success, installed as capdb.opfs.
  ...
  */
  const opfsUtil = capdb.opfs = Object.create(null);
  ...
  */
  opfsUtil.mkdir = async function(absDirName){
    try {
      ...
      //capdb.config.warn("mkdir(",absDirName,") failed:",e);
      return false;
    }
  };
  ...
  */
  ...
  /**
     ...
     Traversal starts at capdb.opfs.rootDirectory.
  ...
  */
  ...
  /**
     ...
     - `directory` [FileSystemDirectoryEntry=capdb.opfs.rootDirectory]
  ...
  */
  opfsUtil.vfsInstallationFeatureCheck = function(vfsName){
    if( !globalThis.SharedArrayBuffer || !globalThis.Atomics ){
      ...
      toss("The OPFS capdb_vfs cannot run in the main thread",
  ...
  */
  opfsUtil.initOptions = function callee(vfsName, options){
    ...
    if( urlParams.has(vfsName+'-disable') ){
      //capdb.config.warn('Explicitly not installing "opfs" VFS due to opfs-disable flag.');
      return;
    }
    try{
    ...
    if( !opfsUtil.proxyUri ){
      opfsUtil.proxyUri = "capdb-opfs-async-proxy.js";
      if( capdb.scriptInfo?.capdbDir ){
        /* Doing this from one scope up, outside of this function, does
           not work. */
        opfsUtil.proxyUri = (
          capdb.scriptInfo.capdbDir + opfsUtil.proxyUri
        );
      }
    }
    ...
    //capdb.config.warn("opfsUtil options =",JSON.stringify(options), 'urlParams =', urlParams);
  ...
  /**
     ...
     capi.capdb_vfs instance, tagged with lots of extra state which
     ...
     capdb_io_methods to override and Y is a callback which gets
  ...
  */
  opfsUtil.createVfsState = function(){
    ...
    const opfsVfs = state.vfs = new capi.capdb_vfs();
    const opfsIoMethods = opfsVfs.ioMethods = new capi.capdb_io_methods();

    opfsIoMethods.$iVersion = 1;
    opfsVfs.$iVersion = 2/*yes, two*/;
    opfsVfs.$szOsFile = capi.capdb_file.structInfo.sizeof;
    ...
    opfsVfs.addOnDispose(
      ...
      /**
         ...
         capdb_os_end() impl into the wasm build, but the shutdown
         ...
         capdb_os_end() is called (_if_ it gets called at all in a
    ...
    opfsVfs.metrics = util.nu({
      ...
      dump: function(){
        ...
        capdb.config.log(globalThis.location.href,
                    "metrics for",globalThis.location.href,":",metrics,
                    "\nTotal of",n,"op(s) for",t,
                    "ms (incl. "+w+" ms of waiting on the async side)");
        capdb.config.log("Serialization metrics:",opfsVfs.metrics.counters.s11n);
    ...
    */
    ...
    /* Size of file I/O buffer block. 64k = max capdb page size, and
    ...
    */
    ...
    /**
       CAPDB_xxx constants to export to the async worker
       counterpart...
    */
    state.sq3Codes = Object.create(null);
    for(const k of [
      'CAPDB_ACCESS_EXISTS',
      'CAPDB_ACCESS_READWRITE',
      'CAPDB_BUSY',
      'CAPDB_CANTOPEN',
      'CAPDB_ERROR',
      'CAPDB_IOERR',
      ...
      'CAPDB_IOERR_LOCK',
      'CAPDB_IOERR_READ',
      'CAPDB_IOERR_SHORT_READ',
      'CAPDB_IOERR_TRUNCATE',
      'CAPDB_IOERR_UNLOCK',
      'CAPDB_IOERR_WRITE',
      'CAPDB_LOCK_EXCLUSIVE',
      'CAPDB_LOCK_NONE',
      'CAPDB_LOCK_PENDING',
      'CAPDB_LOCK_RESERVED',
      'CAPDB_LOCK_SHARED',
    ...
    /**
       ...
       (see capdb-opfs-async-proxy.c-pp.js).
    */
    ...
    /**
       Map of capdb_file pointers to objects constructed by xOpen().
    */
    const __openFiles = opfsVfs.__openFiles = Object.create(null);

    /**
       Impls for the capdb_io_methods methods. Maintenance reminder:
    ...
    */
    const ioSyncWrappers = opfsVfs.ioSyncWrappers = util.nu({
      ...
      xDeviceCharacteristics: function(pFile){
        return capi.CAPDB_IOCAP_UNDELETABLE_WHEN_OPEN;
      },
      ...
      xRead: function(pFile,pDest,n,offset64){
       mTimeStart('xRead');
        ...
        try {
          ...
          if(0===rc || capi.CAPDB_IOERR_SHORT_READ===rc){
            /**
    ...
    /**
       Impls for the capdb_vfs methods. Maintenance reminder: members
    ...
    */
    const vfsSyncWrappers = opfsVfs.vfsSyncWrappers = {
      ...
      xFullPathname: function(pVfs,zName,nOut,pOut){
        ...
        return i<nOut ? 0 : capi.CAPDB_CANTOPEN
        /*CANTOPEN is required by the docs but CAPDB_RANGE would be a closer match*/;
      },
      xGetLastError: function(pVfs,nOut,pOut){
        /* Mutex use in the overlying APIs cause xGetLastError() to
           ...
           race condition between capdb_open()'s call to xOpen() and
           this function. */
        capdb.config.warn("OPFS xGetLastError() has nothing sensible to return.");
        return 0;
      },
      ...
      xOpen: function f(pVfs, zName, pFile, flags, pOutFlags){
        ...
        if( !zName ){
          ...
          if(capi.capdb_uri_boolean(zName, "opfs-unlock-asap", 0)){
            /* -----------------------^^^^^ MUST pass the untranslated
               C-string here. */
            opfsFlags |= state.opfsFlags.OPFS_UNLOCK_ASAP;
          }
          if(capi.capdb_uri_boolean(zName, "delete-before-open", 0)){
            opfsFlags |= state.opfsFlags.OPFS_UNLINK_BEFORE_OPEN;
          }
          jzName = wasm.cstrToJs(zName);
          //capdb.config.warn("xOpen zName =",zName, "opfsFlags =",opfsFlags);
        }else{
          capdb.config.error("Impossible zName value in xOpen?", zName);
          return capi.CAPDB_CANTOPEN;
        }
        const fh = util.nu({
          fid: pFile,
          filename: jzName,
          ...
          flags: flags,
          readOnly: !(capi.CAPDB_OPEN_CREATE & flags)
            && !!(flags & capi.CAPDB_OPEN_READONLY)
        });
        const rc = opRun('xOpen', pFile, jzName, flags, opfsFlags);
        if(rc){
          if( zToFree ) wasm.dealloc(zToFree);
        }else{
          /* Recall that capdb_vfs::xClose() will be called, even on
             error, unless pFile->pMethods is NULL. */
          if(fh.readOnly){
            wasm.poke(pOutFlags, capi.CAPDB_OPEN_READONLY, 'i32');
          }
          __openFiles[pFile] = fh;
          fh.sabView = state.sabFileBufView;
          fh.sq3File = new capi.capdb_file(pFile);
          if( zToFree ) fh.sq3File.addOnDispose(zToFree);
          fh.sq3File.$pMethods = opfsIoMethods.pointer;
          fh.lockType = capi.CAPDB_LOCK_NONE;
        }
        mTimeEnd();
        return rc;
      }/*xOpen()*/
    ...
    const pDVfs = capi.capdb_vfs_find(null)/*pointer to default VFS*/;
    if(pDVfs){
      const dVfs = new capi.capdb_vfs(pDVfs);
...
//#undef vfs.metrics.enable
    ...
    /**
       ...
       plugging in a capdb.oo1 wrapper. It is passed (capdb, opfsVfs),
       where opfsVfs is the capdb_vfs object which was set up by
    ...
    opfsVfs.bindVfs = function(ioMethods, callback){
      ...
      const thePromise = new Promise(function(promiseResolve_, promiseReject_){
        ...
        const promiseResolve = ()=>{
          try{
            callback(capdb, opfsVfs);
          }catch(e){
            return promiseReject(e);
          }
          promiseWasRejected = false;
          return promiseResolve_(capdb);
        };
        const options = opfsUtil.options;
//#if not target:es6-bundler-friendly
        ...
        //capdb.config.error("proxyUri",options.proxyUri, (new Error()));
//#/if
...
//#if target:es6-bundler-friendly
              (()=>{
                ...
                const url = new URL('capdb-opfs-async-proxy.js', import.meta.url);
...
//#/if
        ...
        const sanityCheck = function(){
          ...
          const sq3File = new capi.capdb_file();
          try{
            const fid = sq3File.pointer;
            const openFlags = capi.CAPDB_OPEN_CREATE
                  | capi.CAPDB_OPEN_READWRITE
            //| capi.CAPDB_OPEN_DELETEONCLOSE
                  | capi.CAPDB_OPEN_MAIN_DB;
        ...
        W.onmessage = function({data}){
          //capdb.config.warn(vfsName,"Worker.onmessage:",data);
          switch(data.type){
              ...
              try {
                ...
                if(opfsUtil.thisThreadHasOPFS()){
                  opfsUtil.getRootDir().then((d)=>{
                    ...
                    log("End of OPFS capdb_vfs setup.", opfsVfs);
                    promiseResolve();
                  }).catch(promiseReject);
                }else{
                  promiseResolve();
...
}/*capdbApiBootstrap.initializers*/);
//#/if target:node
