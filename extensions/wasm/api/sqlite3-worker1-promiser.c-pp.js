...
/*
  ...
  ***********************************************************************
  ...
  This file implements a Promise-based proxy for the capdb Worker
  ...
  and (B) it's _not_ a Worker which loads the capdb WASM/JS
...
//#/if
/**
   Configures an capdb Worker API #1 Worker such that it can be
   ...
   `capdb-worker1.js` or a functional equivalent. Note that the
   ...
   'capdb-api'/'worker1-ready' message, which it does when
   capdb.initWorker1API() completes its initialization. This is the
   ...
   of returning a promise from capdbWorker1Promiser() is not lost on
   the developers: see capdbWorker1Promiser.v2() which uses a
   Promise instead.
   ...
   ```
   const config = {...};
   const sq3Promiser = capdbWorker1Promiser(config);
   ...
   - "v1" of this this API is not suitable for use as an ESM module
     ...
     on in this file.
*/
globalThis.capdbWorker1Promiser = function callee(config = callee.defaultConfig){
  ...
  config.worker.onmessage = function(ev){
    ...
    if(!msgHandler){
      if(ev && 'capdb-api'===ev.type && 'worker1-ready'===ev.result) {
      ...
      else err("capdbWorker1Promiser() unhandled worker message:",ev);
      return;
    }
  ...
  return promiserFunc = function(/*(msgType, msgArgs) || (msgEnvelope)*/){
    ...
    if(1===arguments.length){
      ...
      toss("Invalid arguments for capdbWorker1Promiser()-created factory.");
    }
...
}/*capdbWorker1Promiser()*/;

globalThis.capdbWorker1Promiser.defaultConfig = {
  worker: function(){
//#if target:es6-bundler-friendly
    return new Worker(new URL("capdb-worker1-bundler-friendly.mjs", import.meta.url),{
      type: 'module'
    });
//#elif target:es6-module
    return new Worker(new URL("capdb-worker1.mjs", import.meta.url),{
      type: 'module'
    });
//#else
    let theJs = "capdb-worker1.js";
    if(this.currentScript){
      const src = this.currentScript.src.split('/');
      src.pop();
      theJs = src.join('/')+'/' + theJs;
      //capdb.config.warn("promiser currentScript, theJs =",this.currentScript,theJs);
    }else if(globalThis.location){
      //capdb.config.warn("promiser globalThis.location =",globalThis.location);
...
//#/if
  ,
  onerror: (...args)=>console.error('capdbWorker1Promiser():',...args)
}/*defaultConfig*/;

/**
   capdbWorker1Promiser.v2(), added in 3.46, works identically to
   capdbWorker1Promiser() except that it returns a Promise instead
   of relying an an onready callback in the config object. The Promise
   resolves to the same factory function which
   capdbWorker1Promiser() returns.
   ...
   function throws.
*/
globalThis.capdbWorker1Promiser.v2 = function callee(config = callee.defaultConfig){
  let oldFunc;
  ...
  return p;
   ...
   /* We do this because clients are recommended to delete
      globalThis.capdbWorker1Promiser. */
  original: capdbWorker1Promiser
});

globalThis.capdbWorker1Promiser.v2.defaultConfig =
  globalThis.capdbWorker1Promiser.defaultConfig;

//#if target:es6-module
/**
  When built as a module, we export capdbWorker1Promiser.v2()
  instead of capdbWorker1Promise() because (A) its interface is more
  ...
  incompatibility.
*/
export default capdbWorker1Promiser.v2;
delete globalThis.capdbWorker1Promiser;
//#/if /* target:es6-module */
//#else
