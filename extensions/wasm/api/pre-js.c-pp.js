/**
   ...
   This file is intended to be prepended to the capdb.js build using
   Emscripten's --pre-js=THIS_FILE flag (or equivalent). It is run
   from inside of capdbInitModule(), after Emscripten's Module is
...
(function(Module){
  const sIMS =
        globalThis.capdbInitModuleState/*from extern-post-js.c-pp.js*/
        || Object.assign(Object.create(null),{
          /* In WASMFS builds this file gets loaded once per thread,
             but capdbInitModuleState is not getting set for the
             worker threads? That those workers seem to function fine
             despite that is curious. */
          debugModule: function(){
            console.warn("globalThis.capdbInitModuleState is missing",arguments);
          }
        });
  delete globalThis.capdbInitModuleState;
  sIMS.debugModule('pre-js.js capdbInitModuleState =',sIMS);
  ...
  /**
     ...
     2) If capdbInitModuleState.capdbDir is set, then (thatName + path)
...
//#else
    ...
    if(up.has(path)){
      theFile = up.get(path);
    }else if(this.capdbDir){
      theFile = this.capdbDir + path;
    ...
    this.debugModule(
      ...
      'capdbInitModuleState.scriptDir =',this.scriptDir,
...
//#if Module.instantiateWasm and not wasmfs and not target:node
  ...
  */
  Module['instantiateWasm'] = function callee(imports,onSuccess){
    ...
    const finalThen = (arg)=>{
      arg.imports = imports;
      sims.instantiateWasm = arg /* used by capdb-api-prologue.c-pp.js */;
