...
/*
  ...
  ***********************************************************************
  ...
  This file holds a capdb_vfs backed by OPFS storage which uses a
  ...
  each, and manipulating those handles via the synchronous capdb_vfs
...
globalThis.capdbApiBootstrap.initializers.push(function(capdb){
  ...
  /* Bitmask of file types which may persist across sessions.
     CAPDB_OPEN_xyz types not listed here may be inadvertently
  ...
  const PERSISTENT_FILE_TYPES =
        ...
        capi.CAPDB_OPEN_MAIN_JOURNAL |
        capi.CAPDB_OPEN_SUPER_JOURNAL |
        capi.CAPDB_OPEN_WAL;
  const FLAG_COMPUTE_DIGEST_V2 = capi.CAPDB_OPEN_MEMORY
  /* Part of the fix for
     ...
     CAPDB_OPEN_... flags and we must be careful here to not use a
     flag bit which is otherwise relevant for the VFS.
     CAPDB_OPEN_MEMORY is handled by capdb_open_v2() and friends,
     ...
     move to another bit.  Similarly, if the CAPDB_OPEN_MEMORY bit
  ...
  */
  ...
  const log = capdb.config.log;
  const warn = capdb.config.warn;
  const error = capdb.config.error;

  /* Maps (capdb_vfs*) to OpfsSAHPool instances */
  ...
  /* Maps (capdb_file*) to OpfsSAHPool instances */
  ...
  */
    ...
    xClose: function(pFile){
      ...
      if(file) {
        try{
          ...
          file.sah.flush();
          if(file.flags & capi.CAPDB_OPEN_DELETEONCLOSE){
            pool.deletePath(file.path);
          }
        }catch(e){
          return pool.storeErr(e, capi.CAPDB_IOERR);
        }
      }
      return 0;
    },
    ...
    xRead: function(pFile,pDest,n,offset64){
      ...
      try {
        ...
        if(nRead < n){
          ...
          return capi.CAPDB_IOERR_SHORT_READ;
        }
        return 0;
      }catch(e){
        return pool.storeErr(e, capi.CAPDB_IOERR);
  ...
  const opfsIoMethods = new capi.capdb_io_methods();
  ...
  const vfsMethods = {
    ...
    xDelete: function(pVfs, zName, doSyncDir){
      ...
      try{
        ...
        pool.storeErr(e);
        return capi.CAPDB_IOERR_DELETE;
      }
    },
    ...
    xGetLastError: function(pVfs,nOut,pOut){
      ...
      return e ? (e.capdbRc || capi.CAPDB_IOERR) : 0;
    },
    ...
    xOpen: function f(pVfs, zName, pFile, flags, pOutFlags){
      ...
      try{
        ...
        if(!sah && (flags & capi.CAPDB_OPEN_CREATE)) {
        ...
        // Subsequent I/O methods are only passed the capdb_file
        // pointer, so map the relevant info we need to that pointer.
        const file = {path, flags, sah};
        pool.mapS3FileToOFile(pFile, file);
        file.lockType = capi.CAPDB_LOCK_NONE;
        const sq3File = new capi.capdb_file(pFile);
  ...
  */
  const createOpfsVfs = function(vfsName){
    if( capdb.capi.capdb_vfs_find(vfsName)){
      toss3("VFS name is already registered:", vfsName);
    }
    const opfsVfs = new capi.capdb_vfs();
    /* We fetch the default VFS so that we can inherit some
       methods from it. */
    const pDVfs = capi.capdb_vfs_find(null);
    const dVfs = pDVfs
          ? new capi.capdb_vfs(pDVfs)
          : null /* dVfs will be null when capdb is built with
                    CAPDB_OS_OTHER. */;
  ...
  /**
     ...
     OPFS SharedAccessHandle Pool capdb_vfs.
  */
  class OpfsSAHPool {
    ...
    /* Maps (capdb_file*) to xOpen's file objects. */
    ...
    */
    getAssociatedPath(sah){
      ...
      if(this.#apBody[0] &&
         ((flags & capi.CAPDB_OPEN_DELETEONCLOSE) ||
    ...
    /**
       Stores the given client-defined path and CAPDB_OPEN_xyz flags
    ...
    */
    ...
    /**
       ...
       assumed to be an CAPDB_xxx result code, defaulting to
       CAPDB_IOERR if code is falsy.

       Returns the 2nd argument.
    */
    storeErr(e,code){
      if(e){
        e.capdbRc = code || capi.CAPDB_IOERR;
        this.error(e);
      }
      this.$error = e;
      return code;
    ...
    */
    ...
    /**
       Given an (capdb_file*), returns the mapped
       xOpen file object.
    */
    ...
    /**
       Maps or unmaps (if file is falsy) the given (capdb_file*)
    ...
    */
    ...
    /**
       Removes this object's capdb_vfs registration and shuts down
    ...
    */
    async removeVfs(){
      ...
      capi.capdb_vfs_unregister(this.#cVfs.pointer);
      this.#cVfs.dispose();
      ...
      try{
        ...
        capdb.config.error(this.vfsName,"removeVfs() failed with no recovery strategy:",e);
    ...
    pauseVfs(){
      if(this.#mapS3FileToOFile_.size>0){
        capdb.SQLite3Error.toss(
          capi.CAPDB_MISUSE, "Cannot pause VFS",
          this.vfsName,"because it has opened files."
        );
      }
      if(this.#mapSAHToName.size>0){
        capi.capdb_vfs_unregister(this.vfsName);
        this.releaseAccessHandles();
      }
      return this;
    }
    ...
    */
    async unpauseVfs(){
      if(0===this.#mapSAHToName.size){
        return this.acquireAccessHandles(false).
          then(()=>capi.capdb_vfs_register(this.#cVfs, 0),this);
      }
      return this;
    }
    ...
    async importDbChunked(name, callback){
      ...
      this.setAssociatedPath(sah, name, capi.CAPDB_OPEN_MAIN_DB);
      return nWrote;
    }
  ...
  */
  ...
  /**
     ...
     capdb_vfs.  Only one instance of this VFS can be installed per
     ...
     compatible with `capdb_vfs_register()` and friends and suitable
     ...
     handles are currently in use, e.g. by an capdb db.
  ...
  */
  capdb.installOpfsSAHPoolVfs = async function(options=Object.create(null)){
