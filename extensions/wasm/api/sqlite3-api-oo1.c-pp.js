...
/*
  ...
  ***********************************************************************
  ...
  This file contains the so-called OO #1 API wrapper for the capdb
  WASM build. It requires that capdb-api-glue.js has already run
  and it installs its deliverable as capdb.oo1.
*/
globalThis.capdbApiBootstrap.initializers.push(function(capdb){
  const toss3 = (...args)=>{throw new capdb.SQLite3Error(...args)};

  const capi = capdb.capi, wasm = capdb.wasm, util = capdb.util;
  /* What follows is colloquially known as "OO API #1". It is a
     binding of the capdb API which is designed to be run within
     the same thread (main or worker) as the one in which the
     capdb WASM binding was initialized. This wrapper cannot use
     the capdb binding if, e.g., the wrapper is in the main thread
     and the capdb API is in a worker. */

  const outWrapper = function(f){
    return (...args)=>f("capdb.oo1:",...args);
  };

  const debug = capdb.__isUnderTest
        ? outWrapper(console.debug.bind(console))
        : outWrapper(capdb.config.debug);
  const warn = capdb.__isUnderTest
        ? outWrapper(console.warn.bind(console))
        : outWrapper(capdb.config.warn);
  const error = capdb.__isUnderTest
        ? outWrapper(console.error.bind(console))
        : outWrapper(capdb.config.error);
  ...
  */
  ...
  /**
     ...
     (capdb*) resp. (capdb_stmt*) pointers which themselves are
     ...
     wrap an (capdb[_stmt]*) object as an oo1.DB/Stmt without taking
  ...
  */
  ...
  const checkSqlite3Rc = function(dbPtr, sqliteResultCode){
    if(sqliteResultCode){
      ...
      toss3(
        sqliteResultCode,
        "capdb result code",sqliteResultCode+":",
        (dbPtr
         ? capi.capdb_errmsg(dbPtr)
         : capi.capdb_errstr(sqliteResultCode))
      );
    }
    return arguments[0];
  };
  ...
  /**
     capdb_trace_v2() callback which gets installed by the DB ctor
     if its open-flags contain "t".
  */
  const __dbTraceToConsole =
        wasm.installFunction('i(ippp)', function(t,c,p,x){
          if(capi.CAPDB_TRACE_STMT===t){
            // x == SQL, p == capdb_stmt*
            console.log("SQL TRACE #"+(++this.counter),
                        'via capdb@'+c+'['+capi.capdb_db_filename(c,null)+']',
                        wasm.cstrToJs(x));
          }
        }.bind({counter: 0}));

  /**
     A map of capdb_vfs pointers to SQL code or a callback function
     to run when the DB constructor opens a database with the given
     VFS. In the latter case, the call signature is
     (theDbObject,capdbNamespace) and the callback is expected to
     throw on error.
  */
...
//#if enable-see
  ...
  */
  const dbCtorApplySEEKey = function(db,opt){
    ...
    let stmt;
    try{
      stmt = db.prepare("PRAGMA "+keytype+"="+util.capdb__wasm_qfmt_token(key, 1));
      stmt.step();
      return true;
    }finally{
...
//#/if enable-see
  ...
  */
  const dbCtorHelper = function ctor(...args){
    ...
    //capdb.config.debug("DB ctor",opt);
    let pDb;
    if( (pDb = opt['capdb*']) ){
      ...
      //capdb.config.debug("creating proxy db from",opt);
      if( !opt['capdb*:takeOwnership'] ){
        /* This is object does not own its handle. */
        __doesNotOwnHandle.add(this);
      }
      this.filename = capi.capdb_db_filename(pDb,'main');
    }else{
      let fn = opt.filename, vfsName = opt.vfs, flagsStr = opt.flags;
      if( ('string'!==typeof fn && !wasm.isPtr(fn))
          || 'string'!==typeof flagsStr
          || (vfsName && ('string'!==typeof vfsName && !wasm.isPtr(vfsName))) ){
        capdb.config.error("Invalid DB ctor args",opt,arguments);
        toss3("Invalid arguments for DB constructor:", arguments, "opts:", opt);
      }
      let oflags = 0;
      if( flagsStr.indexOf('c')>=0 ){
        oflags |= capi.CAPDB_OPEN_CREATE | capi.CAPDB_OPEN_READWRITE;
      }
      if( flagsStr.indexOf('w')>=0 ) oflags |= capi.CAPDB_OPEN_READWRITE;
      if( 0===oflags ) oflags |= capi.CAPDB_OPEN_READONLY;
      oflags |= capi.CAPDB_OPEN_EXRESCODE;
      const stack = wasm.pstack.pointer;
      try {
        const pPtr = wasm.pstack.allocPtr() /* output (capdb**) arg */;
        let rc = capi.capdb_open_v2(fn, pPtr, oflags, vfsName || wasm.ptr.null);
        pDb = wasm.peekPtr(pPtr);
        checkSqlite3Rc(pDb, rc);
        capi.capdb_extended_result_codes(pDb, 1);
        if(flagsStr.indexOf('t')>=0){
          capi.capdb_trace_v2(pDb, capi.CAPDB_TRACE_STMT,
      ...
      this.filename =
        /* A poor design choice we have to keep: this.filename may be
           ...
           capdb_db_filename(pDb) but that discrepancy went too long
...
//#/if
        ...
        const pVfs = capi.capdb_js_db_vfs(pDb)
        ...
        if(postInitSql){
          ...
          */
          if(postInitSql instanceof Function){
            postInitSql(this, capdb);
          }else{
            checkSqlite3Rc(
              pDb, capi.capdb_exec(pDb, postInitSql, 0, 0, 0)
            );
          }
        }
      }catch(e){
  ...
  /**
     ...
     the given capdb_vfs pointer. The 2nd argument must be a
     function, which gets called with
     (theOo1DbObject,capdbNamespace) at the end of the DB()
     ...
     intended only for use by DB subclasses or capdb_vfs
     implementations.
  ...
  */
  ...
  /**
     The DB class provides a high-level OO wrapper around an capdb
     db handle.

     The given db filename must be resolvable using whatever
     filesystem layer (virtual or otherwise) is set up for the default
     capdb VFS or a VFS which can resolve it must be specified.

     The special capdb db names ":memory:" and "" (temporary db)
     ...
     - "t": enable tracing of SQL executed on this database handle,
       ...
       `capdb.capi.capdb_trace_v2(thisDb.pointer, 0, 0, 0)`.
     ...
     capdb_open_v2(): the name of an capdb VFS. Pass a falsy value,
...
//#/if enable-see
     ...
     For purposes of passing a DB instance to C-style capdb
     functions, the DB object's read-only `pointer` property holds its
     `capdb*` pointer value. That property can also be used to check
  ...
  */
  ...
  /**
     ...
     These do not (and need not) line up with the CAPDB_type
  ...
  */
  ...
  /**
     This class wraps capdb_stmt. Calling this constructor
     directly will trigger an exception. Use DB.prepare() to create
     new instances.

     For purposes of passing a Stmt instance to C-style capdb
     functions, its read-only `pointer` property holds its `capdb_stmt*`
     pointer value.
     ...
     read-only proxy for capdb_column_count() and its use in loops
  ...
  */
  const Stmt = function(/*oo1db, stmtPtr, BindTypes [,takeOwnership=true] */){
    if(BindTypes!==arguments[2]){
      toss3(capi.CAPDB_MISUSE, "Do not call the Stmt constructor directly. Use DB.prepare().");
    }
    this.db = arguments[0];
  ...
  */
  ...
  /**
     Expects to be given a DB instance or an `capdb*` pointer (may
     be null) and an capdb API result code. If the result code is
     not falsy, this function throws an SQLite3Error with an error
     message from capdb_errmsg(), using db (or, if db is-a DB,
     db.pointer) as the db handle, or capdb_errstr() if db is
     falsy. Note that if it's passed a non-error code like CAPDB_ROW
     or CAPDB_DONE, it will still throw but the error string might be
  ...
  */
  ...
  DB.prototype = {
    ...
    */
    ...
    /**
       Returns the number of changes, as per capdb_changes()
       (if the first argument is false) or capdb_total_changes()
       (if it's true). If the 2nd argument is true, it uses
       capdb_changes64() or capdb_total_changes64(), which
    ...
    */
    changes: function(total=false,sixtyFour=false){
      ...
      if(total){
        return sixtyFour
          ? capi.capdb_total_changes64(p)
          : capi.capdb_total_changes(p);
      }else{
        return sixtyFour
          ? capi.capdb_changes64(p)
          : capi.capdb_changes(p);
      }
    },
    /**
       Similar to the this.filename but returns the
       capdb_db_filename() value for the given database name,
    ...
    */
    dbFilename: function(dbName='main'){
      return capi.capdb_db_filename(affirmDbOpen(this).pointer, dbName);
    },
    /**
    ...
    */
    dbName: function(dbNumber=0){
      return capi.capdb_db_name(affirmDbOpen(this).pointer, dbNumber);
    },
    /**
       Returns the name of the capdb_vfs used by the given database
    ...
    */
    dbVfsName: function(dbName=0){
      let rc;
      const pVfs = capi.capdb_js_db_vfs(
        affirmDbOpen(this).pointer, dbName
      );
      if(pVfs){
        const v = new capi.capdb_vfs(pVfs);
    ...
    /**
       ...
       Long-time practice (with other capdb script bindings)
    ...
    */
    prepare: function(sql){
      ...
      let ppStmt, pStmt;
      try{
        ppStmt = wasm.pstack.alloc(8)/* output (capdb_stmt**) arg */;
        DB.checkRc(this, capi.capdb_prepare_v2(this.pointer, sql, -1, ppStmt, null));
    ...
    /**
       ...
       capdb records the SQL for the 2nd statement.
    ...
    exec: function(/*(sql [,obj]) || (obj)*/){
      ...
      try{
        ...
        const ppStmt  = wasm.scopedAlloc(
          /* output (capdb_stmt**) arg and pzTail */
          (2 * wasm.ptr.size) + (sqlByteLen + 1/* SQL + NUL */)
        );
        const pzTail = wasm.ptr.add(ppStmt, wasm.ptr.size) /* final arg to capdb_prepare_v2() */;
        ...
        while(pSql && wasm.peek8(pSql)
          ...
          DB.checkRc(this, capi.capdb_prepare_v3(
          ...
          if(!pStmt) continue;
          //capdb.config.debug("exec() pSql =",capi.capdb_sql(pStmt));
          if(saveSql) saveSql.push(capi.capdb_sql(pStmt).trim());
        ...
        capdb.config.warn("DB.exec() is propagating exception",opt,e);
        throw e;
      }*/finally{
        if(stmt){
          __execLock.delete(stmt);
...
//#if 0
    /**
       ...
       WASM (capdb*) pointer instead of a Stmt object.
       ...
       statements (for for which capdb_prepare() returns a NULL
       capdb_stmt, i.e. spaces and comments) are not added to this
       list unless...
       ...
       a Stmt object (by default) or an (capdb*) WASM pointer (if
    ...
    forEachStmt: function(opt){
      ...
      try{
        ...
        while( pSql && wasm.peek8(pSql) ){
          ...
          if(opt.saveSql){
            if( pStmt ) opt.saveSql.push(capi.capdb_sql(pStmt).trim());
          ...
          if(!pStmt) continue;
          //capdb.config.debug("forEachStmt() pSql =",capi.capdb_sql(pStmt));
          if( !opt.callback ){
            capi.capdb_finalize(pStmt);
            pStmt = null;
            continue;
          }
          ...
          if( true===callaback(stmt || pStmt) ){
            ...
            const rx = capi.capdb_reset(pStmt/*[tag:insert-returning-reset]*/);
            capi.capdb_finalize(pStmt);
            pStmt = null;
            DB.checkRc(this, rx);
          }
        ...
        if(stmt) stmt.finalize();
        else if(pStmt) capi.capdb_finalize(pStmt);
        wasm.scopedAllocPop(stack);
      }
      return this;
    }/*forEachStmt()*/,
//#/if nope
    ...
    /**
       ...
       capdb_create_window_function(), but doing so will "leak" the
       ...
       cannot be determined. The docs for capdb_create_function_v2()
       ...
       function-type property, as per capdb_create_function_v2().
       ...
       any number of arguments, up to capdb's compile-time
       limits. capdb will enforce the argument count if is zero or
       greater. The callback always receives a pointer to an
       `capdb_context` object as its first argument. Any arguments
       after that are from SQL code. The leading context argument does
       _not_ count towards the function's arity. See the docs for
       capdb.capi.capdb_create_function_v2() for why that argument
       ...
       - `deterministic` = capdb.capi.CAPDB_DETERMINISTIC
       - `directOnly` = capdb.capi.CAPDB_DIRECTONLY
       - `innocuous` = capdb.capi.CAPDB_INNOCUOUS
    ...
    */
    createFunction: function f(name, xFunc, opt){
      ...
      let fFlags = 0 /*flags for capdb_create_function_v2()*/;
      if(getOwnOption(opt, 'deterministic')) fFlags |= capi.CAPDB_DETERMINISTIC;
      if(getOwnOption(opt, 'directOnly')) fFlags |= capi.CAPDB_DIRECTONLY;
      if(getOwnOption(opt, 'innocuous')) fFlags |= capi.CAPDB_INNOCUOUS;
      ...
      let rc;
      if( isWindow ){
        rc = capi.capdb_create_window_function(
          this.pointer, name, arityArg,
          capi.CAPDB_UTF8 | fFlags, pApp || 0,
    ...
    /**
       ...
       CAPDB_{typename} constants. Passing the undefined value is
    ...
    */
    ...
    /**
       ...
       handles prepared using capi.capdb_prepare_v3() (or
       equivalent).
    */
    ...
    */
    transaction: function(/* [beginQualifier,] */callback){
      ...
      if(arguments.length>1){
        if(/[^a-zA-Z]/.test(arguments[0])){
          toss3(capi.CAPDB_MISUSE, "Invalid argument for BEGIN qualifier.");
        }
        opener += ' '+arguments[0];
    ...
    /**
       This works similarly to transaction() but uses capdb's SAVEPOINT
  ...
  /**
     Returns a new oo1.DB instance which wraps the given (capdb*)
     WASM pointer, optionally with or without taking over ownership of
     that pointer.

     The first argument must be either a non-NULL (capdb*) WASM
     pointer.
     ...
     pass that pointer to capdb_close() when its close() method is
     ...
     - DO NOT call capdb_close() (or similar) on the being-proxied
  ...
  */
  DB.wrapHandle = function(pDb, takeOwnership=false){
    if( !pDb || !wasm.isPtr(pDb) ){
      throw new capdb.SQLite3Error(capi.CAPDB_MISUSE,
                                     "Argument must be a WASM capdb pointer");
    }
    return new DB({
      /* This ctor call style is very specifically internal-use-only.
         It is not documented and may change at any time. */
      "capdb*": pDb,
      "capdb*:takeOwnership": !!takeOwnership
    });
  };
  ...
  */
  const affirmSupportedBindType = function(v){
    //capdb.config.log('affirmSupportedBindType',v);
  ...
  */
  const affirmParamIndex = function(stmt,key){
    const n = ('number'===typeof key)
          ? key : capi.capdb_bind_parameter_index(stmt.pointer, key);
  ...
  */
  const bindOne = function f(stmt,ndx,bindType,val){
    ...
    if(!f._){
      ...
      f._ = {
        string: function(stmt, ndx, val, asBlob){
          ...
          const f = asBlob ? capi.capdb_bind_blob : capi.capdb_bind_text;
          return f(stmt.pointer, ndx, pStr, n, capi.CAPDB_WASM_DEALLOC);
        }
      };
    }/* static init */
    ...
    switch((null===val || undefined===val) ? BindTypes.null : bindType){
        case BindTypes.null:
          rc = capi.capdb_bind_null(stmt.pointer, ndx);
          break;
        case BindTypes.string:
        ...
        case BindTypes.number: {
          let m;
          if(util.isInt32(val)) m = capi.capdb_bind_int;
          else if('bigint'===typeof val){
            if(!util.bigIntFits64(val)){
              f._tooBigInt(val);
            }else if(wasm.bigIntEnabled){
              m = capi.capdb_bind_int64;
            }else if(util.bigIntFitsDouble(val)){
              val = Number(val);
              m = capi.capdb_bind_double;
            }else{
              f._tooBigInt(val);
            }
          }else{ // !int32, !bigint
        ...
        case BindTypes.boolean:
          rc = capi.capdb_bind_int(stmt.pointer, ndx, val ? 1 : 0);
          break;
        case BindTypes.blob: {
          ...
          rc = capi.capdb_bind_blob(stmt.pointer, ndx, pBlob, val.byteLength,
                                      capi.CAPDB_WASM_DEALLOC);
          break;
        }
        default:
          capdb.config.warn("Unsupported bind() argument type:",val);
  ...
  Stmt.prototype = {
    /**
       ...
       capdb_finalize() (0 on success, non-0 on error), or the
       ...
       If Stmt does not own its underlying (capdb_stmt*) (see
    ...
    */
    clearBindings: function(){
      ...
      capi.capdb_clear_bindings(this.pointer);
        __stmtMayGet.delete(this);
      return this;
    },
    /**
       ...
       underlying call to capdb_reset() returns non-0.
       ...
       capdb_reset() returns non-0, but it was discovered that
    ...
    */
    reset: function(alsoClearBinds){
      ...
      const rc = capi.capdb_reset(affirmStmtOpen(this).pointer);
      __stmtMayGet.delete(this);
    ...
    /**
       ...
       - Numbers are bound as either doubles or integers: doubles if
         ...
         significant for the majority of clients due to capdb's data
    ...
    */
    step: function(){
      ...
      const rc = capi.capdb_step(affirmStmtOpen(this).pointer);
      switch(rc){
        case capi.CAPDB_DONE:
          __stmtMayGet.delete(this);
          return false;
        case capi.CAPDB_ROW:
        ...
        default:
          __stmtMayGet.delete(this);
          capdb.config.warn("capdb_step() rc=",rc,
                              capi.capdb_js_rc_str(rc),
                              "SQL =", capi.capdb_sql(this.pointer));
    ...
    */
    ...
    /**
       ...
       of the enumeration values for capdb types, which are
       defined as members of the capdb module: CAPDB_INTEGER,
       CAPDB_FLOAT, CAPDB_TEXT, CAPDB_BLOB. Any other value,
       ...
       capdb will convert the value to a string.
       ...
       Potential TODO: add type ID CAPDB_JSON, which fetches the
    ...
    */
    get: function(ndx,asType){
      ...
      if(Array.isArray(ndx)){
        ...
        const n = this.columnCount;
        while(i<n){
          ndx[capi.capdb_column_name(this.pointer,i)] = this.get(i++);
        }
        return ndx;
      }
      affirmColIndex(this, ndx);
      switch(undefined===asType
             ? capi.capdb_column_type(this.pointer, ndx)
             : asType){
          case capi.CAPDB_NULL: return null;
          case capi.CAPDB_INTEGER:{
            if(wasm.bigIntEnabled){
              const rc = capi.capdb_column_int64(this.pointer, ndx);
              ...
              const rc = capi.capdb_column_double(this.pointer, ndx);
              if(rc>Number.MAX_SAFE_INTEGER || rc<Number.MIN_SAFE_INTEGER){
                /* Throwing here is arguable but, since we're explicitly
                   extracting an CAPDB_INTEGER-type value, it seems fair to throw
              ...
              //capdb.config.log("get integer rc=",rc,isInt32(rc));
              return util.isInt32(rc) ? (rc | 0) : rc;
            }
          }
          case capi.CAPDB_FLOAT:
            return capi.capdb_column_double(this.pointer, ndx);
          case capi.CAPDB_TEXT:
            return capi.capdb_column_text(this.pointer, ndx);
          case capi.CAPDB_BLOB: {
            const n = capi.capdb_column_bytes(this.pointer, ndx),
                  ptr = capi.capdb_column_blob(this.pointer, ndx),
                  rc = new Uint8Array(n);
            if(n){
    ...
    /** Equivalent to get(ndx) but coerces the result to an
        integer. */
    getInt: function(ndx){return this.get(ndx,capi.CAPDB_INTEGER)},
    /** Equivalent to get(ndx) but coerces the result to a
        float. */
    getFloat: function(ndx){return this.get(ndx,capi.CAPDB_FLOAT)},
    /** Equivalent to get(ndx) but coerces the result to a
        string. */
    getString: function(ndx){return this.get(ndx,capi.CAPDB_TEXT)},
    /** Equivalent to get(ndx) but coerces the result to a
        Uint8Array. */
    getBlob: function(ndx){return this.get(ndx,capi.CAPDB_BLOB)},
    /**
    ...
    */
    getJSON: function(ndx){
      const s = this.get(ndx, capi.CAPDB_STRING);
    ...
    */
    getColumnNames: function(tgt=[]){
      ...
      const n = this.columnCount;
      for(let i = 0; i < n; ++i){
        tgt.push(capi.capdb_column_name(this.pointer, i));
      }
      return tgt;
    },
    /**
    ...
    */
    getParamIndex: function(name){
      return (affirmStmtOpen(this).parameterCount
              ? capi.capdb_bind_parameter_index(this.pointer, name)
              : undefined);
    },
    /**
    ...
    */
    getParamName: function(ndx){
      return (affirmStmtOpen(this).parameterCount
              ? capi.capdb_bind_parameter_name(this.pointer, ndx)
              : undefined);
    },

    /**
       Behaves like capdb_stmt_busy() but throws if this statement
       is closed and returns a value of type boolean instead of integer.

       Added in 3.47.
    */
    isBusy: function(){
      return 0!==capi.capdb_stmt_busy(affirmStmtOpen(this));
    },

    /**
       Behaves like capdb_stmt_readonly() but throws if this statement
       is closed and returns a value of type boolean instead of integer.

       Added in 3.47.
    */
    isReadOnly: function(){
      return 0!==capi.capdb_stmt_readonly(affirmStmtOpen(this));
    }
  }/*Stmt.prototype*/;
  ...
  /**
     Stmt.columnCount is an interceptor for capdb_column_count().
  ...
  */
  ...
  /**
     ...
     [^1]: The first argument cannot be a (capdb*) because the
     ...
     (capdb*) rather than a DB object. If so, we could laxen the
     first argument's requirement and allow an (capdb*). Because
     DB.wrapHandle() enables multiple DB objects to proxy the same
     (capdb*), we cannot unambiguously translate the first arugment
     from (capdb*) to DB instances for us with this function's first
     argument.
  */
  Stmt.wrapHandle = function(oo1db, pStmt, takeOwnership=false){
    let ctor = Stmt;
    if( !(oo1db instanceof DB) || !oo1db.pointer ){
      throw new capdb.SQLite3Error(capdb.CAPDB_MISUSE,
                                     "First argument must be an opened "+
                                     "capdb.oo1.DB instance");
    }
