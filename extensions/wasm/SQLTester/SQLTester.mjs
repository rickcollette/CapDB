...
import capdbApiInit from '/jswasm/capdb.mjs';

const capdb = await capdbApiInit();
...
const tryInstallVfs = function f(vfsName){
  ...
  const pVfs = capdb.capi.capdb_vfs_find(vfsName);
  if(pVfs){
    log("Installing",'"'+vfsName+'"',"as default VFS.");
    const rc = capdb.capi.capdb_vfs_register(pVfs, 1);
    if(rc){
      capdb.SQLite3Error.toss(rc,"While trying to register",vfsName,"vfs.");
    }
    f.vfsName = vfsName;
  }
  return !!pVfs;
...
if( 0 && globalThis.WorkerGlobalScope ){
  ...
  if( 1 && capdb.oo1.OpfsDb ){
    /* Really slow with these tests */
    tryInstallVfs("opfs");
  }else if( capdb.installOpfsSAHPoolVfs ){
    await capdb.installOpfsSAHPoolVfs({
      clearOnInit: true,
      initialCapacity: 15,
      name: 'opfs-SQLTester'
    }).then(pool=>{
...
class DbException extends SQLTesterException {
  constructor(testScript, pDb, rc, closeDb=false){
    super(testScript, "DB error #"+rc+": "+capdb.capi.capdb_errmsg(pDb));
    this.name = 'DbException';
    if( closeDb ) capdb.capi.capdb_close_v2(pDb);
  }
  isFatal() { return true; }
}
...
const Util = newObj({
  ...
  unlink: function f(fn){
    if(!f.unlink){
      f.unlink = capdb.wasm.xWrap('capdb__wasm_vfs_unlink','int',
                                    ['*','string']);
    }
  ...
  strglob: capdb.wasm.xWrap('capdb__wasm_SQLTester_strglob','int',
                              ['string','string'])
})/*Util*/;
...
#clearBuffer(buffer){
  ...
  closeAllDbs(){
    for(let i = 0; i<this.#db.list.length; ++i){
      if(this.#db.list[i]){
        capdb.capi.capdb_close_v2(this.#db.list[i]);
        this.#db.list[i] = null;
      }
    }
    this.#db.iCurrentDb = 0;
  ...
  openDb(name, createIfNeeded){
    ...
    this.closeDb();
    const capi = capdb.capi, wasm = capdb.wasm;
    let pDb = 0;
    let flags = capi.CAPDB_OPEN_READWRITE;
    if( createIfNeeded ) flags |= capi.CAPDB_OPEN_CREATE;
    try{
      let rc;
      wasm.pstack.call(function(){
        let ppOut = wasm.pstack.allocPtr();
        rc = capdb.capi.capdb_open_v2(name, ppOut, flags, null);
      ...
      if( 0!=rc ){
        capdb.SQLite3Error.toss(
          rc,
          "capdb result code",rc+":",
          (pDb ? capdb.capi.capdb_errmsg(pDb)
           : capdb.capi.capdb_errstr(rc))
        );
      }
  ...
  runTests(){
    this.outln("SQLite version ", capdb.capi.capdb_libversion()," with ",
               capdb.wasm.ptr.size, "-byte WASM pointers");
  ...
  #appendDbErr(pDb, sb, rc){
    sb.push(capdb.capi.capdb_js_rc_str(rc), ' ');
    const msg = this.#escapeSqlValue(capdb.capi.capdb_errmsg(pDb));
    if( '{' === msg.charAt(0) ){
      sb.push(msg);
    }else{
      sb.push('{', msg, '}');
  ...
  execSql(pDb, throwOnError, appendMode, rowMode, sql){
    ...
    const wasm = capdb.wasm, capi = capdb.capi;
    sql = (sql instanceof Uint8Array)
      ? sql
      : Util.utf8Encode(capi.capdb_js_sql_to_string(sql));
    const self = this;
    ...
    wasm.scopedAllocCall(function(){
      ...
      const ppStmt = wasm.scopedAlloc(
        (2 * wasm.ptr.size) /* output (capdb_stmt**) arg and pzTail */
      ...
      while( pSql && wasm.peek8(pSql) ){
        ...
        if( sb ){
          const nCol = capi.capdb_column_count(pStmt);
          let colName, val;
          while( capi.CAPDB_ROW === (rc = capi.capdb_step(pStmt)) ) {
            for( let i=0; i < nCol; ++i ){
              if( spacing++ > 0 ) sb.push(' ');
              if( self.#emitColNames ){
                colName = capi.capdb_column_name(pStmt, i);
                switch(appendMode){
              ...
              val = capi.capdb_column_text(pStmt, i);
              if( null===val ){
        ...
        if( capi.CAPDB_ROW===rc || capi.CAPDB_DONE===rc) rc = 0;
        else if( rc!=0 ){
          if( sb ){
            self.#appendDbErr(pDb, sb, rc);
          }
    ...
    for(const rp of props){
      ...
      switch(rp){
        case "RECURSIVE_TRIGGERS":
          ...
          break;
        case "TEMPSTORE_FILE":
          /* This _assumes_ that the lib is built with CAPDB_TEMP_STORE=1 or 2,
...
class RunCommand extends Command {
  process(t, ts, argv){
    ...
    if( 0!==rc && t.verbosity()>0 ){
      const msg = capdb.capi.capdb_errmsg(pDb);
