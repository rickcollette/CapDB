/*
  ...
  ***********************************************************************
  ...
  This is the JS Worker file for the capdb fiddle app. It loads the
  capdb wasm module and offers access to the db via the Worker
  ...
  - shellExec: data=text to execute as if it had been entered in the
    capdb CLI shell app (as opposed to capdb_exec()). This event
...
(function(){
  ...
  const fixmeOPFS = "(FIXME: won't work with OPFS-over-capdb_vfs.)";
  let capdb /* gets assigned when the wasm module is loaded */;

  self.onerror = function(/*message, source, lineno, colno, error*/) {
    const err = arguments[4];
    if(err && 'ExitStatus'==err.name){
      /* This is relevant for the capdb shell binding but not the
         lower-level binding. */
  ...
  const Sqlite3Shell = {
    ...
    dbFilename: function f(){
      if(!f._) f._ = capdb.wasm.xWrap('fiddle_db_filename', "string", ['string']);
      return f._(0);
    },
    dbHandle: function f(){
      if(!f._) f._ = capdb.wasm.xWrap("fiddle_db_handle", "capdb*");
      return f._();
    },
    dbIsOpfs: function f(){
      return capdb.opfs && capdb.capi.capdb_js_db_uses_vfs(
        this.dbHandle(), "opfs"
      );
    },
    runMain: function f(){
      if(f.argv) return 0===f.argv.rc;
      const dbName = "/fiddle.capdb";
      f.argv = [
        'capdb-fiddle.wasm',
        '-bail', '-safe',
        dbName
      ...
      /* We need to call capdb_shutdown() in order to avoid numerous
         legitimate warnings from the shell about it being initialized
         after capdb_initialize() has been called. This means,
      ...
      if(f.argv.rc){
        stderr("Fatal error initializing capdb shell.");
        fiddleModule.isDead = true;
        return false;
      }
      wMsg('sqlite-version', {
        lib: capi.capdb_libversion(),
        srcId: capi.capdb_sourceid()
      });
      stdout("WASM pointer size: "+wasm.ptr.size);
      stdout('Welcome to the "fiddle" shell. Tap the About button for more info.');
      if(capi.capdb_vfs_find("opfs")){
    ...
    */
    exec: function f(sql){
      if(!f._){
        ...
        f._ = capdb.wasm.xWrap('fiddle_exec', undefined, ['string']);
        f.getPrompt = capdb.wasm.xWrap('fiddle_get_prompt', 'string:dealloc', []);
      }
      ...
      try {
        ...
        wMsg('working','end');
        wMsg('wasm-info', {
          pointerSize: capdb.wasm.ptr.size,
          heapSize: capdb.wasm.heap8().byteLength,
          prompt: f.getPrompt()
        });
      }
    },
    resetDb: function f(){
      if(!f._) f._ = capdb.wasm.xWrap('fiddle_reset_db', undefined);
    ...
    interrupt: function f(){
      if(!f._) f._ = capdb.wasm.xWrap('fiddle_interrupt', undefined);
  ...
  self.onmessage = function f(ev){
    ...
    switch(ev.type){
      ...
      case 'db-export': {
        ...
        try{
          ...
          const buffer = capdb.capi.capdb_js_db_export(
    ...
    capdb.config.warn("Unknown fiddle-worker message type:",ev);
  };
  ...
  */
  capdbInitModule(fiddleModule).then((_capdb)=>{
    capdb = _capdb;
    capdb.config.warn("Installing capdb module globally (in Worker)",
                        "for use in the dev console.", capdb);
    globalThis.capdb = capdb;
    const dbVfs = capdb.wasm.xWrap('fiddle_db_vfs', "*", ['string']);
