...
/**
   ...
   capdb (upon which it is based) depends heavily on file
...
/**
   ...
   This API very specifically uses capdb_realloc() as its basis.
*/
...
/**
   ...
   This API very specifically uses capdb_malloc() as its basis.
*/
...
/**
   ...
   cmpp_malloc(), cmpp_realloc(), or the capdb_malloc() family of
   APIs.
...
/**
   ...
   NULL or a format string compatible with capdb_mprintf().
...
struct cmpp_b {
  /**
     ...
     uses capdb_realloc() and friends for memory management.
...
/**
   This creates a formatted string using capdb_mprintf() and emits it
...
/**
  ...
  fails.
*/
capdb_stmt * cmpp__stmt(cmpp * pp, enum CmppStmt_e which,
                          bool prepEvenIfErr);

/**
   Reminder to self: this must return an CAPDB_... code, not a
   CMPP_RC_... code.

   On success it returns 0, CAPDB_ROW, or CAPDB_DONE. On error it
   returns another non-0 CAPDB_... code and updates pp->pimpl->err.
...
*/
int cmpp__step(cmpp * const pp, capdb_stmt * const q, bool resetIt);

/** Resets and clear bindings from q (if q is not NULL). */
void cmpp__stmt_reset(capdb_stmt * const q);

/**
   Expects an SQLite result value. If it's CAPDB_OK, CAPDB_ROW, or
   CAPDB_DONE, 0 is returned without side-effects, otherwise pp->err
...
/* Proxy for capdb_bind_int64(). */
int cmpp__bind_int(cmpp *pp, capdb_stmt *pStmt, int col, int64_t val);
...
*/
int cmpp__bind_int_text(cmpp *pp, capdb_stmt *pStmt, int col, int64_t val);

/* Proxy for capdb_bind_null(). */
int cmpp__bind_null(cmpp *pp, capdb_stmt *pStmt, int col);

/* Proxy for capdb_bind_text() which updates pp->err on error. */
int cmpp__bind_text(cmpp *pp,capdb_stmt *pStmt, int col,
                    unsigned const char * zStr);

/* Proxy for capdb_bind_text() which updates pp->err on error. */
int cmpp__bind_textn(cmpp *pp,capdb_stmt *pStmt, int col,
...
#if 0
/**
   Proxy for capdb_bind_text(). It uses capdb_str_vappendf() so
   supports all of its formatting options.
*/
int cmpp__bind_textv(cmpp*pp, capdb_stmt *pStmt, int col,
                     const char * zFmt, ...);
#endif

/**
   Proxy for capdb_str_finish() which updates pp's error state if s
   ...
   the returned string.
*/
char * cmpp_str_finish(cmpp *pp, capdb_str *s, int * n);
...
/**
   ...
   Returns ppCode.
*/
int cmpp__bind_arg(cmpp_dx * const dx, capdb_stmt * q,
...
*/
int cmpp__bind_group(cmpp_dx * const dx, capdb_stmt * const q,
...
*/
int cmpp__define_from_row(cmpp * const pp, capdb_stmt * const q,
                          bool defineIfNoRow);
...
/**
   ...
   because that's how capdb_str_appendf() and friends work.
*/
...
CMPP__EXPORT(void, cmpp_mfree)(void *p){
  /* This MUST be a proxy for capdb_free() because allocate memory
     exclusively using capdb_malloc() and friends. */
  capdb_free(p);
}

CMPP__EXPORT(void *, cmpp_mrealloc)(void * p, size_t n){
  return capdb_realloc64(p, n);
}

CMPP__EXPORT(void *, cmpp_malloc)(size_t n){
#if 1
  return capdb_malloc64(n);
#else
  void * p = capdb_malloc64(n);
  if( p ) memset(p, 0, n);
  return p;
#endif
}
...
/**
   Returns an approximate cmpp_tt for the given CAPDB_... value from
   capdb_column_type() or capdb_value_type().
*/
...
CMPP__EXPORT(int, cmpp_ctor)(cmpp **pOut, cmpp_ctor_cfg const * cfg){
  ...
  if( mv ){
    ...
    *pi = cmpp_pimpl_empty;
    ...
    if( cfg->dbFile ){
      pi->db.zName = capdb_mprintf("%s", cfg->dbFile);
...
CMPP__EXPORT(void, cmpp_reset)(cmpp *pp){
  ...
  if( pi->stmt.spRelease && pi->stmt.spRollback ){
    ...
    */
    capdb_reset(pi->stmt.spRelease);
    while( CAPDB_DONE==capdb_step(pi->stmt.spRelease) ){
      capdb_reset(pi->stmt.spRollback);
      capdb_step(pi->stmt.spRollback);
      capdb_reset(pi->stmt.spRelease);
    }
  }
  cmpp__out_close(pp);
...
#define E(N,S) \
  if(pi->stmt.N) {capdb_finalize(pi->stmt.N); pi->stmt.N = 0;}
  CmppStmt_map(E) (void)0;
#undef E
  if( pi->db.dbh ){
    if( CAPDB_TXN_WRITE==capdb_txn_state(pi->db.dbh, NULL) ){
      capdb_exec(pi->db.dbh, "COMMIT;", 0, 0, NULL)
        /* ignoring error */;
    }
    capdb_close(pi->db.dbh);
    pi->db.dbh = 0;
  }
...
char * cmpp_str_finish(cmpp *pp, capdb_str *s, int * n){
  char * z = 0;
  int const rc = capdb_str_errcode(s);
  cmpp__db_rc(pp, rc, "capdb_str_errcode()");
  if(0==rc){
    int const nStr = capdb_str_length(s);
    if(n) *n = nStr;
    z = capdb_str_finish(s);
    if( !z ){
      assert( 0==nStr && "else rc!=0" );
    }
  }else{
    cmpp_mfree( capdb_str_finish(s) );
  }
  return z;
}
...
int cmpp__bind_int(cmpp *pp, capdb_stmt *pStmt, int col, int64_t val){
  return ppCode
    ? ppCode
    : cmpp__db_rc(pp, capdb_bind_int64(pStmt, col, val),
                     "from cmpp__bind_int()");
}

int cmpp__bind_int_text(cmpp *pp, capdb_stmt *pStmt, int col,
                        int64_t val){
...
int cmpp__bind_null(cmpp *pp, capdb_stmt *pStmt, int col){
  return ppCode
    ? ppCode
    : cmpp__db_rc(pp, capdb_bind_null(pStmt, col),
                     "from cmpp__bind_null()");
}

static int cmpp__bind_textx(cmpp *pp, capdb_stmt *pStmt, int col,
                            unsigned const char * zStr, cmpp_ssize_t n,
                            void (*dtor)(void *)){
  if( 0==ppCode ){
    cmpp__db_rc(
      pp, (zStr && n)
      ? capdb_bind_text(pStmt, col,
                          (char const *)zStr,
                          (int)n, dtor)
      : capdb_bind_null(pStmt, col),
      capdb_sql(pStmt)
    );
  }
  return ppCode;
...
int cmpp__bind_textv(cmpp*pp, capdb_stmt *pStmt, int col,
  ...
  if( 0==p->err.code ){
    int rc;
    capdb_str * str = capdb_str_new(pp->pimpl->db.dbh);
    int n = 0;
    char * z;
    va_list va;
    if( !str ) return ppCode;
    va_start(va,zFmt);
    capdb_str_vappendf(str, zFmt, va);
    va_end(va);
    z = cmpp_str_finish(str, &n);
    cmpp__db_rc(
      pp, z
      ? capdb_bind_text(pStmt, col, z, n, capdb_free)
      : capdb_bind_null(pStmt, col),
      capdb_sql(pStmt)
    );
    cmpp_mfree(z);
    ...
    if( !cmpp_check_oom(pp, delim) ){
      ...
      if( zDelim ){
        ...
        delim->open.z = delim->zOwns =
          (unsigned char*)capdb_mprintf("%.*s", n, zDelim);
  ...
  if( q && 0==cmpp__bind_textn(pp, q, 2, kvp.k.z, (int)kvp.k.n) ){
    ...
    if( fw.nContent ){
      cmpp__bind_textx(pp, q, 3, fw.zContent,
                       (cmpp_ssize_t)fw.nContent, capdb_free);
...
CMPP__EXPORT(int, cmpp_has)(cmpp *pp, const char * zName, cmpp_ssize_t nName){
  int rc = 0;
  capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_defHas, false);
  if( q ){
    nName = cmpp__strlen(zName, nName);
    cmpp__bind_textn(pp, q, 1, ustr_c(zName), nName);
    if(CAPDB_ROW == cmpp__step(pp, q, true)){
      rc = 1;
    }else{
      rc = 0;
    }
...
int cmpp__get_bool(cmpp *pp, unsigned const char *zName, cmpp_ssize_t nName){
  int rc = 0;
  capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_defGetBool, false);
  if( q ){
    nName = cmpp__strlenu(zName, nName);
    cmpp__bind_textn(pp, q, 1, zName, nName);
    assert(0==ppCode);
    if(CAPDB_ROW == cmpp__step(pp, q, false)){
      rc = capdb_column_int(q, 0);
    }else{
      rc = 0;
...
int cmpp__get_int(cmpp *pp, unsigned const char * zName,
                  cmpp_ssize_t nName, int *pOut ){
  capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_defGetInt, false);
  if( q ){
    nName = cmpp__strlenu(zName, nName);
    cmpp__bind_textn(pp, q, 1, zName, nName);
    assert(0==ppCode);
    if(CAPDB_ROW == cmpp__step(pp, q, false)){
      *pOut = capdb_column_int(q,0);
    }else{
      cmpp__affirm_undef_policy(pp, zName, nName);
    }
    cmpp__stmt_reset(q);
...
int cmpp__get_b(cmpp *pp, unsigned const char * zName,
  ...
  int rc = 0;
  capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_defGet, false);
  if( q ){
    nName = cmpp__strlenu(zName, nName);
    cmpp__bind_textn(pp, q, 1, zName, nName);
    int n = 0;
    if(CAPDB_ROW == cmpp__step(pp, q, false)){
      const unsigned char * z = capdb_column_text(q, 3);
      n = capdb_column_bytes(q, 3);
...
int cmpp__get(cmpp *pp, unsigned const char * zName,
  ...
  if( q ){
    ...
    if(CAPDB_ROW == cmpp__step(pp, q, false)){
      ...
      *zVal = ustr_nc(capdb_mprintf("%.*s", n, z))
...
CMPP__EXPORT(int, cmpp_undef)(cmpp *pp, const char * zKey,
                                unsigned int *nRemoved){
  capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_defDel, false);
  if( q ){
    unsigned int const n = strlen(zKey);
    cmpp__bind_textn(pp, q, 1, ustr_c(zKey), (cmpp_ssize_t)n);
    cmpp__step(pp, q, true);
    if( nRemoved ){
      *nRemoved = (unsigned)capdb_changes(pp->pimpl->db.dbh);
    }
...
int cmpp__include_dir_add(cmpp *pp, const char * zDir, int priority, int64_t * pRowid){
  if( pRowid ) *pRowid = 0;
  if( !ppCode && zDir && *zDir ){
    capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_inclPathAdd, false);
    if( q ){
...
int cmpp__include_dir_rm_id(cmpp *pp, int64_t rowid){
  capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_inclPathRmId, true);
  if( q ){
    /* Hoop-jumping to allow this to work even if pp's in an error
       state. */
    int rc = capdb_bind_int64(q, 1, rowid);
    if( 0==rc ){
      rc = capdb_step(q);
      if( CAPDB_ROW==rc ){
        --pp->pimpl->flags.nIncludeDir;
        rc = 0;
      }else if( CAPDB_DONE==rc ){
        rc = 0;
      }
    }
    if( rc && !ppCode ){
      cmpp__db_rc(pp, rc, capdb_sql(q));
    }
    cmpp__stmt_reset(q);
...
CMPP__EXPORT(int, cmpp_db_name_set)(cmpp *pp, const char * zName){
  if( 0==ppCode ){
    ...
    if( zName ){
      char * const z = capdb_mprintf("%s", zName);
...
CMPP__EXPORT(int, cmpp_d_register)(cmpp * pp, cmpp_d_reg const * r,
  ...
  e1->zName = capdb_mprintf("%s", r->name);
  if( 0==cmpp_check_oom(pp, e1->zName) ){
    ...
    if( r->closer.f
      ...
      e1->d.closer = &e2->d;
      e2->zName = capdb_mprintf("/%s", r->name);
...
CMPP__EXPORT(int, cmpp_sp_begin)(cmpp *pp){
  if( 0==ppCode ){
    capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_spBegin, true);
    assert( q || !"db init would have otherwise failed");
    if( q && CAPDB_DONE==cmpp__step(pp, q, true) ){
...
CMPP__EXPORT(int, cmpp_sp_rollback)(cmpp *const pp){
  ...
  if( !pp->pimpl->flags.nSavepoint ){
    ...
    capdb_stmt * q = cmpp__stmt(pp, CmppStmt_spRollback, true);
    assert( q || !"db init would have otherwise failed");
    if( q && CAPDB_DONE==cmpp__step(pp, q, true) ){
...
CMPP__EXPORT(int, cmpp_sp_commit)(cmpp * const pp){
  if( 0==ppCode ){
    if( !pp->pimpl->flags.nSavepoint ){
      ...
      capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_spRelease, true);
      assert( q || !"db init would have otherwise failed");
      if( q && CAPDB_DONE==cmpp__step(pp, q, true) ){
        --pp->pimpl->flags.nSavepoint;
      }
...
cmpp_tt cmpp__tt_for_sqlite(int sqType){
  cmpp_tt rv;
  switch( sqType ){
    case CAPDB_INTEGER: rv = cmpp_TT_Int;    break;
    case CAPDB_NULL:    rv = cmpp_TT_Null;   break;
...
int cmpp__define_from_row(cmpp * const pp, capdb_stmt * const q,
                          bool defineIfNoRow){
  if( 0==ppCode ){
    int const nCol = capdb_column_count(q);
    assert( capdb_data_count(q)>0 || defineIfNoRow);
    /* Create a #define for each column */
    bool const hasRow = capdb_data_count(q)>0;
    for( int i = 0; !ppCode && i < nCol; ++i ){
      char const * const zCol = capdb_column_name(q, i);
      if( hasRow ){
        unsigned char const * const zVal = capdb_column_text(q, i);
        int const nVal = capdb_column_bytes(q, i);
        cmpp_tt const ttype =
          cmpp__tt_for_sqlite(capdb_column_type(q,i));
...
cmpp_d const * cmpp__d_search3(cmpp *pp, const char *zName,
  ...
  if( !d ){
    ...
    if( !e
      ...
      char * z = capdb_mprintf("libcmpp-d-%s", zName);
      cmpp_check_oom(pp, z);
...
int cmpp__errv(cmpp *pp, int rc, char const *zFmt, va_list va){
  if( pp ){
    ...
    if( pp->pimpl->dx || (zFmt && *zFmt) ){
      capdb_str * sstr = 0;
      sstr = capdb_str_new(pp->pimpl->db.dbh);
      if( pp->pimpl->dx ){
        cmpp__dx_append_script_info(pp->pimpl->dx, sstr);
        capdb_str_append(sstr, ": ", 2);
      }
      if( zFmt && *zFmt ){
        capdb_str_vappendf(sstr, zFmt, va);
      }else{
        capdb_str_appendf(sstr, "No error info provided.");
      }
      pp->pimpl->err.zMsgC =
        pp->pimpl->err.zMsg = capdb_str_finish(sstr);
...
char * cmpp_path_search(cmpp *pp,
  ...
  if( !ppCode ){
    ...
    if( q ){
      ...
      if( CAPDB_ROW==dbrc ){
        unsigned char const * s = capdb_column_text(q, 1);
        zrc = capdb_mprintf("%s", s);
...
void cmpp__dump_defines(cmpp *pp, cmpp_FILE * fp, int bIndent){
  capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_defSelAll, false);
  if( q ){
    while( CAPDB_ROW==capdb_step(q) ){
      int const tt = capdb_column_int(q, 0);
      unsigned char const * zK = capdb_column_text(q, 1);
      unsigned char const * zV = capdb_column_text(q, 2);
      int const nK = capdb_column_bytes(q, 1);
      int const nV = capdb_column_bytes(q, 2);
...
int main(int argc, char const * const * argv){
  ...
  capdb_config(CAPDB_CONFIG_URI,1);
  {
...
/**
   A proxy for capdb_prepare() which updates pp->pimpl->err on error.
*/
static int cmpp__prepare(cmpp *pp, capdb_stmt **pStmt,
                         const char * zSql, ...){
  ...
  va_start(va, zSql);
  capdb_str_vappendf(str, zSql, va);
  va_end(va);
  z = cmpp_str_finish(pp, str, &n);
  if( z ){
    int const rc = capdb_prepare_v2(pp->pimpl->db.dbh, z, n, pStmt, 0);
    cmpp__db_rc(pp, rc, z);
    capdb_free(z);
  }
  return ppCode;
...
/**
   Expects an CAPDB_... result code and returns an approximate match
   from cmpp_rc_e. It specifically treats CAPDB_ROW and CAPDB_DONE
   as non-errors, returning 0 for those.
*/
static int cmpp__db_errcode(capdb * const db, int sqliteCode);
int cmpp__db_errcode(capdb * const db, int sqliteCode){
  (void)db;
  int rc = 0;
  switch(sqliteCode & 0xff){
    case CAPDB_ROW:
    case CAPDB_DONE:
    case CAPDB_OK: rc = 0; break;
    case CAPDB_NOMEM: rc = CMPP_RC_OOM; break;
    case CAPDB_CORRUPT: rc = CMPP_RC_CORRUPT; break;
    case CAPDB_TOOBIG:
    case CAPDB_FULL:
    case CAPDB_RANGE: rc = CMPP_RC_RANGE; break;
    case CAPDB_NOTFOUND: rc = CMPP_RC_NOT_FOUND; break;
    case CAPDB_PERM:
    case CAPDB_AUTH:
    case CAPDB_BUSY:
    case CAPDB_LOCKED:
    case CAPDB_READONLY: rc = CMPP_RC_ACCESS; break;
    case CAPDB_CANTOPEN:
    case CAPDB_IOERR: rc = CMPP_RC_IO; break;
    case CAPDB_NOLFS: rc = CMPP_RC_UNSUPPORTED; break;
    default:
      //MARKER(("capdb_errcode()=0x%04x\n", rc));
      rc = CMPP_RC_DB; break;
  }
  return rc;
}
...
int cmpp__define2(cmpp *pp,
                  ...
                  cmpp_ssize_t nVal,
                  cmpp_tt tType){
  capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_defIns, false);
  if( q ){
...
int cmpp__define_shadow(cmpp *pp, unsigned char const *zKey,
                        ...
                        int ttype,
                        int64_t * pId){
  capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_sdefIns, false);
  if( q ){
    if( 0==cmpp__define_impl(pp, q, zKey, nKey, zVal, nVal, ttype, false)
        && pId ){
      *pId = capdb_column_int64(q, 0);
      assert( *pId );
    }
    cmpp__stmt_reset(q);
  }
...
int cmpp__define_unshadow(cmpp *pp, unsigned char const *zKey,
  ...
  capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_sdefDel, false);
  if( q ){
...
/*
** This capdb_trace_v2() callback outputs tracing info using
    ...
    case CAPDB_TRACE_STMT:{
      ...
      if(pi->sqlTrace.out.out){
        char * const zExp = pi->sqlTrace.expandSql
          ? capdb_expanded_sql((capdb_stmt*)p)
          : 0;
        capdb_str * const s = capdb_str_new(pi->db.dbh);
        if( pi->dx ){
          cmpp__dx_append_script_info(pi->dx, s);
          capdb_str_appendchar(s, 1, ':');
          capdb_str_appendchar(s, 1, ' ');
        }
        capdb_str_appendall(s, zExp ? zExp : zSql);
        capdb_str_appendchar(s, 1, '\n');
        int const n = capdb_str_length(s);
        if( n ){
          char * const z = capdb_str_finish(s);
          if( z ){
...
#include <sys/stat.h>
/*
** capdb UDF which returns true if its argument refers to an
...
static void cmpp__udf_file_exists(
  ...
  if( 0!=zName ){
    struct stat sb;
    capdb_result_int(context, stat(zName, &sb)
                       ? 0
                       : S_ISREG(sb.st_mode));
  }
}
...
static void cmpp__udf_truthy(
  ...
  assert(1==argc);
  int buul = 0;
  capdb_value * const sv = argv[0];
  switch( capdb_value_type(sv) ){
    case CAPDB_NULL:
      break;
    case CAPDB_FLOAT:
      buul = 0.0!=capdb_value_double(sv);
      break;
    case CAPDB_INTEGER:
      buul = 0!=capdb_value_int(sv);
      break;
    case CAPDB_TEXT:
    case CAPDB_BLOB:{
      int const n = capdb_value_bytes(sv);
      if( n>1 ) buul = 1;
      else if( 1==n ){
        const char *z =
          (const char*)capdb_value_text(sv);
        buul = z
          ? 0!=strcmp(z,"0")
          : 0;
      }
...
static void cmpp__udf_compare(
  ...
  (void)(argc);  /* Unused parameter */
  assert(2==argc);
  capdb_value * const v1 = argv[0];
  capdb_value * const v2 = argv[1];
  unsigned char const * const z1 = capdb_value_text(v1);
  unsigned char const * const z2 = capdb_value_text(v2);
  int const n1 = capdb_value_bytes(v1);
  int const n2 = capdb_value_bytes(v2);
  int rv;
  if( !z1 ){
    rv = z2 ? -1 : 0;
  }else if( !z2 ){
...
int cmpp__db_init(cmpp *pp){
  ...
  const char * zSchema =
    ...
    */
    ...
    ;
  cmpp__err_clear(pp);
  int openFlags = CAPDB_OPEN_READWRITE;
  if( pi->db.zName ){
    openFlags |= CAPDB_OPEN_CREATE;
  }
  rc = capdb_open_v2(
  ...
  capdb_busy_timeout(pi->db.dbh, 5000);
  capdb_db_config(pi->db.dbh, CAPDB_DBCONFIG_MAINDBNAME,
                    CMPP__DB_MAIN_NAME);
  rc = capdb_trace_v2(pi->db.dbh, CAPDB_TRACE_STMT,
  ...
  struct {
    ...
    void (*xUdf)(capdb_context *,int,capdb_value **);
    int arity;
    int flags;
  } aFunc[] = {
    {
      ...
      .arity = 1,
      .flags = CAPDB_UTF8 | CAPDB_DIRECTONLY
    },
    {
      .zName = "cmpp_truthy",
      .xUdf  = cmpp__udf_truthy,
      .arity = 1,
      .flags = CAPDB_UTF8 | CAPDB_DIRECTONLY | CAPDB_DETERMINISTIC
    },
    {
      .zName = "cmpp_compare",
  ...
  if( pi->db.zName ){
    /* Drop all cmpp tables when using a persistent db so that we are
       ...
       toggle this. */
    rc = capdb_exec(pi->db.dbh, zDrops, 0, 0, &zErr);
  }
  if( !rc ){
    rc = capdb_exec(pi->db.dbh, zSchema, 0, 0, &zErr);
  }

  if( !rc ){
    extern int capdb_series_init(capdb *, char **, const capdb_api_routines *);
    rc = capdb_series_init(pi->db.dbh, &zErr, NULL);
  }
  ...
  while(0){
    ...
    capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_insTtype, false);
    if( !q ) goto end;
#define E(N,STR)                                                      \
    ...
    if( CAPDB_DONE!=cmpp__step(pp, q, true) ) return ppCode;
    cmpp_tt_map(E)
#undef E
    capdb_finalize(q);
    pi->stmt.insTtype = 0;
...
static int cmpp__including_has(cmpp *pp, unsigned const char * zName){
  int rc = 0;
  capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_inclHas, false);
...
static char * cmpp__include_search(cmpp *pp, unsigned const char * zKey,
                                   int * nVal){
  char * zName = 0;
  capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_inclSearch, false);
  if( nVal ) *nVal = 0;
  if( q && 0==cmpp__bind_text(pp, q, 1, zKey) ){
    int const rc = cmpp__step(pp, q, false);
    if(CAPDB_ROW==rc){
      const unsigned char * z = capdb_column_text(q, 0);
      int const n = capdb_column_bytes(q,0);
      zName = n ? capdb_mprintf("%.*s", n, z) : 0;
...
static int cmpp__include_rm(cmpp *pp, unsigned const char * zKey){
  capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_inclDel, false);
  if( q ){
...
static int cmpp__including_add(cmpp *pp, unsigned const char * zKey,
  ...
  capdb_stmt * const q = cmpp__stmt(pp, CmppStmt_inclIns, false);
  if( q ){
...
static int cmpp_kav_each_f_query__bind(
  ...
  if( ':'!=zKey[0] && '$'!=zKey[0] /*&& '@'!=zKey[0]*/ ){
    ...
    capdb_stmt * const q = callbackState;
    assert( q );
    int const bindNdx =
      capdb_bind_parameter_index(q, (char const*)zKey);
    if( bindNdx ){
...
static void cmpp_dx_f_query(cmpp_dx *dx){
  ...
  if( batchMode ){
    ...
    char *zErr = 0;
    cmpp__pi(dx->pp);
    int rc = capdb_exec(pi->db.dbh, (char const *)sql->z, 0, 0, &zErr);
    rc = cmpp__db_rc(dx->pp, rc, zErr);
    capdb_free(zErr);
    goto cleanup;
  }

  if( cmpp__db_rc(pp, capdb_prepare_v2(
  ...
  //g_warn("SQL via stmt: <<%s>>", capdb_sql(q));
  int const nCol = capdb_column_count(q);
  if( !nCol ){
    cmpp_err_set(pp, CMPP_RC_RANGE,
    ...
    switch( arg->ttype ){
      ...
      case cmpp_TT_OpGlob:{
        ...
        if( 0==cmpp_arg_to_b(dx, pPrev, &osR, 0) ){
          ...
          result = 0==capdb_strglob((char const *)zGlob,
                                      (char const *)osR.z);
...
int cmpp__bind_arg(cmpp_dx * const dx, capdb_stmt * const q,
...
/* series_cursor is a subclass of capdb_vtab_cursor which will
...
struct series_cursor {
  ...
  capdb_int64 iOBase;      /* Original starting value ("start") */
  capdb_int64 iOTerm;      /* Original terminal value ("stop") */
  capdb_int64 iOStep;      /* Original step value */
  capdb_int64 iBase;       /* Starting value to actually use */
  capdb_int64 iTerm;       /* Terminal value to actually use */
  capdb_uint64 iStep;      /* The step size */
...
*/
static capdb_uint64 span64(capdb_int64 a, capdb_int64 b){
  assert( a>=b );
  return (*(capdb_uint64*)&a) - (*(capdb_uint64*)&b);
}  
...
*/
static capdb_int64 add64(capdb_int64 a, capdb_uint64 b){
  capdb_uint64 x = *(capdb_uint64*)&a;
  x += b;
  return *(capdb_int64*)&x;
}
static capdb_int64 sub64(capdb_int64 a, capdb_uint64 b){
  capdb_uint64 x = *(capdb_uint64*)&a;
  x -= b;
  return *(capdb_int64*)&x;
}
...
*/
static int seriesDisconnect(capdb_vtab *pVtab){
  capdb_free(pVtab);
  return CAPDB_OK;
}
...
*/
static int seriesOpen(capdb_vtab *pUnused, capdb_vtab_cursor **ppCursor){
  series_cursor *pCur;
  (void)pUnused;
...
*/
static int seriesClose(capdb_vtab_cursor *cur){
  capdb_free(cur);
  return CAPDB_OK;
}
...
*/
static int seriesNext(capdb_vtab_cursor *cur){
...
#define LARGEST_INT64  ((capdb_int64)0x7fffffffffffffffLL)
#define LARGEST_UINT64 ((capdb_uint64)0xffffffffffffffffULL)
#define SMALLEST_INT64 ((capdb_int64)0x8000000000000000LL)
#endif

/*
** The rowid is the same as the value.
*/
static int seriesRowid(capdb_vtab_cursor *cur, sqlite_int64 *pRowid){
  series_cursor *pCur = (series_cursor*)cur;
  *pRowid = pCur->iValue;
  return CAPDB_OK;
}
...
** row of output.
*/
static int seriesEof(capdb_vtab_cursor *cur){
  series_cursor *pCur = (series_cursor*)cur;
  return pCur->bDone;
}
...
*/
#ifndef CAPDB_SERIES_CONSTRAINT_VERIFY
# define CAPDB_SERIES_CONSTRAINT_VERIFY 0
#endif
...
*/
static capdb_uint64 seriesSteps(series_cursor *pCur){
  if( pCur->bDesc ){
...
#if defined(CAPDB_ENABLE_MATH_FUNCTIONS) || defined(_WIN32)
/*
...
#elif defined(__GNUC__) && !defined(CAPDB_DISABLE_INTRINSIC)
/*
...
static int seriesFilter(
  ...
  int i;                                /* Loop counter */
  capdb_int64 iMin = SMALLEST_INT64;  /* Smallest allowed output value */
  capdb_int64 iMax = LARGEST_INT64;   /* Largest allowed output value */
  capdb_int64 iLimit = 0;             /* if >0, the value of the LIMIT */
  capdb_int64 iOffset = 0;            /* if >0, the value of the OFFSET */
  ...
  */
  if( idxNum & 0x01 ){
    pCur->iOBase = capdb_value_int64(argv[iArg++]);
  }else{
    pCur->iOBase = 0;
  }
  if( idxNum & 0x02 ){
    pCur->iOTerm = capdb_value_int64(argv[iArg++]);
  }else{
    pCur->iOTerm = 0xffffffff;
  }
  if( idxNum & 0x04 ){
    pCur->iOStep = capdb_value_int64(argv[iArg++]);
  ...
  */
  if( idxNum & 0x20 ){
    iLimit = capdb_value_int64(argv[iArg++]);
    if( idxNum & 0x40 ){
      iOffset = capdb_value_int64(argv[iArg++]);
    }
  }
  ...
  */
  if( idxNum & 0x3380 ){
    if( idxNum & 0x0080 ){    /* value=X */
      if( capdb_value_numeric_type(argv[iArg])==CAPDB_FLOAT ){
        double r = capdb_value_double(argv[iArg++]);
        if( r==seriesCeil(r)
         && r>=(double)SMALLEST_INT64
         && r<=(double)LARGEST_INT64
        ){
          iMin = iMax = (capdb_int64)r;
        }else{
          goto series_no_rows;
        }
      }else{
        iMin = iMax = capdb_value_int64(argv[iArg++]);
      }
    }else{
      if( idxNum & 0x0300 ){  /* value>X or value>=X */
        if( capdb_value_numeric_type(argv[iArg])==CAPDB_FLOAT ){
          ...
          if( r<(double)SMALLEST_INT64 ){
            ...
            iMin = (capdb_int64)seriesCeil(r+1.0);
          }else{
            iMin = (capdb_int64)seriesCeil(r);
          }
        }else{
          iMin = capdb_value_int64(argv[iArg++]);
          if( (idxNum & 0x0200)!=0 ){
      ...
      if( idxNum & 0x3000 ){   /* value<X or value<=X */
        if( capdb_value_numeric_type(argv[iArg])==CAPDB_FLOAT ){
          ...
          if( r>(double)LARGEST_INT64 ){
            ...
            iMax = (capdb_int64)(r-1.0);
          }else{
            iMax = (capdb_int64)seriesFloor(r);
          }
        }else{
          iMax = capdb_value_int64(argv[iArg++]);
          if( idxNum & 0x2000 ){
    ...
    */
    if( pCur->bDesc==0 ){
      if( pCur->iBase<iMin ){
        capdb_uint64 span = span64(iMin,pCur->iBase);
      ...
      if( pCur->iBase>iMax ){
        capdb_uint64 span = span64(pCur->iBase,iMax);
  ...
  */
  ...
  if( idxNum & 0x20 ){
    if( iOffset>0 ){
      if( seriesSteps(pCur) < (capdb_uint64)iOffset ){
        goto series_no_rows;
      }else if( pCur->bDesc ){
    ...
    if( iLimit>=0 && seriesSteps(pCur) > (capdb_uint64)iLimit ){
...
static int seriesBestIndex(
  ...
  ** are the last three columns in the virtual table. */
  ...
  for(i=0; i<pIdxInfo->nConstraint; i++, pConstraint++){
    ...
    if( op>=CAPDB_INDEX_CONSTRAINT_LIMIT
     && op<=CAPDB_INDEX_CONSTRAINT_OFFSET
      ...
      if( pConstraint->usable==0 ){
        /* do nothing */
      }else if( op==CAPDB_INDEX_CONSTRAINT_LIMIT ){
        aIdx[3] = i;
        idxNum |= 0x20;
      }else{
        assert( op==CAPDB_INDEX_CONSTRAINT_OFFSET );
        aIdx[4] = i;
        idxNum |= 0x40;
      }
      continue;
    ...
    iMask = 1 << iCol;
#ifndef ZERO_ARGUMENT_GENERATE_SERIES
    if( iCol==0 && op==CAPDB_INDEX_CONSTRAINT_EQ ){
      bStartSeen = 1;
    }
#endif
    if( pConstraint->usable==0 ){
      unusableMask |=  iMask;
      continue;
    }else if( op==CAPDB_INDEX_CONSTRAINT_EQ ){
      idxNum |= iMask;
      aIdx[iCol] = i;
    }
  }
  ...
  for(i=0; i<7; i++){
    if( (j = aIdx[i])>=0 ){
      ...
      pIdxInfo->aConstraintUsage[j].omit =
         !CAPDB_SERIES_CONSTRAINT_VERIFY || i>=3;
    }
  }
...
*/
static capdb_module seriesModule = {
  0,                         /* iVersion */
  0,                         /* xCreate */
...
int capdb_series_init(
  ...
  (void)pApi;
#ifndef CAPDB_OMIT_VIRTUALTABLE
  if( capdb_libversion_number()<3008012 && pzErrMsg!=0 ){
    *pzErrMsg = capdb_mprintf(
        "generate_series() requires SQLite 3.8.12 or later");
    return CAPDB_ERROR;
  }
  rc = capdb_create_module(db, "generate_series", &seriesModule, 0);
#endif
  return rc;
}
#define CMPP_D_DEMO
