/*
  ...
  ***********************************************************************
  ...
  A basic test script for capdb.wasm with kvvfs support. This file
  must be run in main JS thread and capdb.js must have been loaded
  before it.
*/
'use strict';
(function(){
  ...
  const runTests = function(capdb){
    const capi = capdb.capi,
          oo = capdb.oo1,
          wasm = capdb.wasm;
    log("Loaded module:",capi.capdb_libversion(), capi.capdb_sourceid());
    T.assert( 0 !== capi.capdb_vfs_find(null) );
    if(!capi.capdb_vfs_find('kvvfs')){
  ...
  capdbInitModule(globalThis.capdbTestModule).then((capdb)=>{
    runTests(capdb);
  });
})();
