...
const jsSqlite = urlArgs.get('capdb.dir')
  +'/capdb.js?opfs-async-proxy-id='
      +options.workerName;
importScripts(jsSqlite)/*Sigh - URL args are not propagated this way*/;
//const capdbInitModule = (await import(jsSqlite)).default;
globalThis.capdbInitModule.__isUnderTest =
  true /* causes capdb.opfs to be retained */;
globalThis.capdbInitModule().then(async function(capdb){
  ...
  if(!capdb.opfs){
    stderr("This code requires the (private) capdb.opfs object. Aborting.");
    return;
  }
  ...
  if(urlArgs.has('unlink-db')){
    await capdb.opfs.unlink(dbName);
  ...
  const run = async function(){
    const Ctors = Object.assign(Object.create(null),{
      opfs: capdb.oo1.OpfsDb,
      'opfs-wl': capdb.oo1.OpfsWlDb
    });
    ...
    while(true){
      try{
        ...
        if(e instanceof capdb.SQLite3Error
           && capdb.capi.CAPDB_BUSY===e.resultCode){
    ...
    capdb.capi.capdb_busy_timeout(db.pointer, 5000);
    capdb.capi.capdb_js_retry_busy(-1, ()=>{
      db.transaction((db)=>{
        db.exec([
