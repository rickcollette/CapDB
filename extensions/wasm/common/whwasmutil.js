...
function WhWasmUtilInstaller(target){
  ...
  */
  ...
  /**
     ...
     the capdb API, we need to pass them on as, variably, a C-string
     ...
     backfire? We handle that at the client level in capdb with a
  ...
  /**
     ...
     - bindScope (string): one of ('transient', 'context',
       ...
       - 'singleton': holds one function-pointer binding for this
         ...
         `capdb*`) then "context" (see below) is the proper mode.
          ...
          if(pair[1]){
            ...
            try{
              /* Because the pending native call might rely on the
                 ...
                 with capdb's xDestroy() methods, we don't
...
```
     ...
     - Figure out whether it makes sense to extend the arg adapter
       ...
       handling of the final argument to capdb_prepare_v2() depends
