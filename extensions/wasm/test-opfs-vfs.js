...
*/
'use strict';
const tryOpfsVfs = async function(capdb){
  ...
  const pVfs = capi.capdb_vfs_find("opfs") || toss("Missing 'opfs' VFS.");
  ...
  if(urlArgs.has('delete')) capdb.opfs.unlink(dbFile);

  const db = new capdb.oo1.OpfsDb(dbFile,'ct');
...
importScripts('jswasm/capdb.js');
self.capdbInitModule()
  .then((capdb)=>tryOpfsVfs(capdb))
  .catch((e)=>{
