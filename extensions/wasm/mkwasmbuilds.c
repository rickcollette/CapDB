...
** capdb's canonical WASM build. The main motivation is to generate
...
enum BuildDefFlags {
  ...
  ** but Emscripten's generated capdb.js differs between
...
** WASM file). Because we only need two capdb.wasm files (one each
...
** behavior. This code lives/runs within the generated capdbInitModule().
**
** --post-js = gets injected after Emscripten's main work, but still
** within the body of capdbInitModule().
**
** --extern-pre-js = gets injected before capdbInitModule(), in the
** global scope. We inject the license and version info here.
**
** --extern-post-js = gets injected immediately after
** capdbInitModule(), in the global scope. In this step we replace
** capdbInitModule() with a slightly customized one, the main
...
**
** Then there's capdb-api.BuildName.js, which is the entire SQLite3
** JS API (generated from the list defined in $(capdb-api.jses)). It
...
struct BuildDef {
  ...
  ** must hold the base name to rename it to.  Typically "capdb" or
  ** "capdb-64bit". This is the case for builds which are named
  ** something like capdb-foo-bar but can use the vanilla
  ** capdb.wasm file. In such cases we don't need the extra
  ** capdb-foo-bar.wasm which Emscripten (necessarily) creates when
  ** compiling the module, so we patch (at build-time) the JS file to
  ** use this name instead capdb-foo-bar.
  */
  const char *zDotWasm;
...
const BuildDefs oBuildDefs = {
 ...
 /* speedtest1, our primary benchmarking tool */
  .speedtest1 = {
    ...
    " $(cflags.common)"
    " -DCAPDB_SPEEDTEST1_WASM"
    " $(CAPDB_OPT)"
    " -UCAPDB_WASM_BARE_BONES"
    " -UCAPDB_C -DCAPDB_C=$(capdb.c)"
  ...
  */
  .bundler = {
    .zEmo        = "👛",
    .zBaseName   = "capdb-bundler-friendly",
    .zDotWasm    = "capdb",
  ...
  .bundler64 = {
    .zEmo        = "📦",
    .zBaseName   = "capdb-bundler-friendly-64bit",
    .zDotWasm    = "capdb-64bit",
...
static void mk_prologue(void){
  ...
  ** set up by this point in the build process. */
  ...
  pf(zBanner
     ...
     " $(CAPDB_OPT) "
     " $(cflags.wasm_extra_init) $(capdb-wasm.c.in)\n"
     "endef\n"
  );

  {
    ...
    */
    const char * zOptFlags =
      ...
      ** the vanilla build of capdb.wasm with -Oz (our shipping
...
static void mk_pre_post(char const *zBuildName, BuildDef const * pB){
  ...
  if( 0!=WASM_CUSTOM_INSTANTIATE && zBaseName ){
    pf("$(eval $(call b.c-pp.target,"
       ...
       "$(extern-post-js.%s.js),"
       "$(c-pp.D.%s) --@policy=error -Dcapdb.wasm=%s.wasm"
       "))",
       zBuildName, zBuildName, zBuildName,
       zBaseName);
  }else{
  ...
  pf("pre-post.%s.flags = "
     "--extern-pre-js=$(capdb-license-version.js) "
     "--pre-js=$(pre-js.%s.js) "
...
/**
   Emit rules for capdb-api.${zBuildName}.js.
*/
static void emit_api_js(char const *zBuildName){
  pf("capdb-api.%s.js = $(dir.tmp)/capdb-api.%s.js\n",
     zBuildName, zBuildName);
  pf("$(eval $(call b.c-pp.target,"
     "%s,"
     "$(capdb-api.jses),"
...
static void mk_lib_mode(const char *zBuildName, const BuildDef * pB){
  ...
  { /* build it... */
    pf(zBanner
       "$(out.%s.js): $(MAKEFILE_LIST) $(capdb-wasm.c.in)"
    ...
    { /* Post-compilation transformations and copying to
      ...
      ** capdb-bundler-friendly.mjs (which is used by the npm
  ...
  for(int i = 0; i < 2; ++i ){
    ...
    {/* emcc */
      pf("$(out.%s.js): $(MAKEFILE_LIST) "
         ...
         "$(fiddle.c.in) "
         "$(pre-post.%s.deps) "
         "$(dir.dout)/capdb-opfs-async-proxy.js",
         zBuildName, zBuildName);
      if( isDebug ){
      ...
      pf("\t@$(call b.strip-js-emcc-bindings,$(logtag.%s))\n",
         zBuildName);
      pf("\t@$(call b.cp,%s,$(dir.dout)/capdb-opfs-async-proxy.js,"
         "$(dir $@)"
         ")\n", zBuildName);
      if( isDebug ){
        pf("\t@$(call b.cp,%s,"
           ...
           "$(dir.fiddle)/capdb-opfs-async-proxy.js,"
           "$(dir $@)"
           ")\n",
           zBuildName);
      }
