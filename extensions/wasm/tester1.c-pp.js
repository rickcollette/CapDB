/*
  ...
  ***********************************************************************
  ...
  Main functional and regression tests for the capdb WASM API.
  ...
  capdb WASM/JS module is complete, that queue is processed. If any
  ...
  close it in a later test. Each test gets passed the capdb
...
import {default as capdbInitModule} from "@capdb.js@";
globalThis.capdbInitModule = capdbInitModule;
//#else
'use strict';
//#/if
(function(self){
  ...
  */
  ...
  const haveWasmCTests = ()=>{
    return !!wasm.exports.capdb__wasm_test_intptr;
  };
  ...
  /**
     Helpers for writing capdb-specific tests.
  */
  const TestUtil = {
    ...
    */
    ...
    TestGroup: (function(){
      ...
      TestGroup.prototype = {
        ...
        run: async function(capdb){
          logClass('group-start',"Group #"+this.number+':',this.name);
          if(this.predicate){
            const p = this.predicate(capdb);
          ...
          for(const t of this.tests){
            ...
            let rc = t.test.call(groupState, capdb);
      ...
      try {
        if (initDb) {
          ...
          T.assert(err, "Expecting an exception")
            .assert(capi.CAPDB_NOTADB == err.resultCode,
                    "Expecting NOTADB");
        }/*initDb*/
        db = new ctor({
          ...ctorOptFunc(false),
...
//#/if enable-see
  ...
  */
  T.opfsCommon = {
    sanityChecks: async function(vfsName, oo1Ctor, capdb){
      T.assert(capi.capdb_vfs_find(vfsName));
      const opfs = capdb.opfs;
      const filename = this.opfsDbFile = '/dir/capdb-tester1.db';
      ...
      try {
        ...
        this.opfsDbExport = capi.capdb_js_db_export(db);
    ...
    importer: async function(vfsName, oo1Ctor, capdb){
      let db;
      ...
      try {
        ...
        db = null;
        this.opfsUnlink =
          (fn=filename)=>capdb.util.capdb__wasm_vfs_unlink(vfsName, fn);
        this.opfsUnlink(filename);
        T.assert(!(await capdb.opfs.entryExists(filename)));
    ...
    opfsUtil: async function(vfsName, oo1Ctor, capdb){
      const filename = this.opfsDbFile;
      ...
      /**************************************************************
        ATTENTION CLIENT-SIDE USERS: capdb.opfs is NOT intended
        for client-side use. It is only for this project's own
        internal use. Its APIs are subject to change or removal at
        any time. The capdb.opfs namespace is REMOVED from the
        capdb namespace in non-test runs of the library.
      ***************************************************************/
      const opfs = capdb.opfs;
      ...
      const testDir = '/capdb-opfs-'+opfs.randomFilename(12);
  ...
  T.g('Basic sanity checks')
    .t({
      ...
      test:function(capdb){
        for(const k of [
          'CAPDB_CONFIG_GETMALLOC', 'CAPDB_CONFIG_URI'
        ]){
          T.assert(capi[k] > 0);
        }
        T.assert(capi.CAPDB_MISUSE===capi.capdb_config(
          capi.CAPDB_CONFIG_URI, 1
        ), "MISUSE because the library has already been initialized.");
        T.assert(capi.CAPDB_MISUSE === capi.capdb_config(
          // not enough args
          capi.CAPDB_CONFIG_GETMALLOC
        ));
        T.assert(capi.CAPDB_NOTFOUND === capi.capdb_config(
          // unhandled-in-JS config option
          capi.CAPDB_CONFIG_GETMALLOC, 1
        ));
        if(0){
          log("We cannot _fully_ test capdb_config() after the library",
    ...
    .t({
      ...
      test: function(capdb){
        if(capdb.config.useStdAlloc){
          ...
          T.assert(wasm.alloc.impl === wasm.exports.capdb_malloc)
            .assert(wasm.dealloc.impl === wasm.exports.capdb_free)
            .assert(wasm.realloc.impl === wasm.exports.capdb_realloc);
        }
      }
    })
    .t('Namespace object checks', function(capdb){
      const wasmCtypes = wasm.ctype;
      T.assert(wasmCtypes.structs[0].name==='capdb_vfs').
        assert(wasmCtypes.structs[0].members.szOsFile.sizeof>=4).
        assert(wasmCtypes.structs[1/*capdb_io_methods*/
                                 ].members.xFileSize.offset>0);
      [ /* Spot-check a handful of constants to make sure they got installed... */
        'CAPDB_SCHEMA','CAPDB_NULL','CAPDB_UTF8','CAPDB_UTF8_ZT',
        'CAPDB_STATIC', 'CAPDB_DIRECTONLY',
        'CAPDB_OPEN_CREATE', 'CAPDB_OPEN_DELETEONCLOSE'
      ...
      T.assert(capi.capdb_errstr(capi.CAPDB_IOERR_ACCESS).indexOf("I/O")>=0).
        assert(capi.capdb_errstr(capi.CAPDB_CORRUPT).indexOf('malformed')>0).
        assert(capi.capdb_errstr(capi.CAPDB_OK) === 'not an error');

      try {
        throw new capdb.WasmAllocError;
      }catch(e){
        T.assert(e instanceof Error)
          .assert(e instanceof capdb.WasmAllocError)
          .assert("Allocation failed." === e.message);
      }
      try {
        throw new capdb.WasmAllocError("test",{
          cause: 3
        });
      }catch(e){
        T.assert(3 === e.cause)
      ...
      try {throw new capdb.WasmAllocError("test","ing",".")}
      catch(e){T.assert("test ing ." === e.message)}

      try{ throw new capdb.SQLite3Error(capi.CAPDB_SCHEMA) }
      catch(e){
        T.assert('CAPDB_SCHEMA' === e.message)
          .assert(capi.CAPDB_SCHEMA === e.resultCode);
      }
      try{ capdb.SQLite3Error.toss(capi.CAPDB_CORRUPT,{cause: true}) }
      catch(e){
        T.assert('CAPDB_CORRUPT' === e.message)
          .assert(capi.CAPDB_CORRUPT === e.resultCode)
          .assert(true===e.cause);
      }
      try{ capdb.SQLite3Error.toss("resultCode check") }
      catch(e){
        T.assert(capi.CAPDB_ERROR === e.resultCode)
          .assert('resultCode check' === e.message);
      }
    })
  ////////////////////////////////////////////////////////////////////
    .t('strglob/strlike', function(capdb){
      T.assert(0===capi.capdb_strglob("*.txt", "foo.txt")).
        assert(0!==capi.capdb_strglob("*.txt", "foo.xtx")).
        assert(0===capi.capdb_strlike("%.txt", "foo.txt", 0)).
        assert(0!==capi.capdb_strlike("%.txt", "foo.xtx", 0));
    })
  ...
  T.g('C/WASM Utilities')
    .t('capdb.wasm namespace', function(capdb){
      ...
      // alloc(), realloc(), allocFromTypedArray()
      {
        ...
        T.assert('number' === typeof capdb.capi.CAPDB_MAX_ALLOCATION_SIZE);
        if(!capdb.config.useStdAlloc){
          const tooMuch = capdb.capi.CAPDB_MAX_ALLOCATION_SIZE + 1,
                isAllocErr = (e)=>e instanceof capdb.WasmAllocError;
      ...
      //log("xWrap()...");
      {
        ...
        if(haveWasmCTests()){
          if(!capdb.config.useStdAlloc){
            fw = w.xWrap('capdb__wasm_test_str_hello', 'utf8:dealloc',['i32']);
            rc = fw(0);
            T.assert('hello'===rc);
            rc = fw(1);
            T.assert(null===rc);
          ...
          if(w.bigIntEnabled){
            ...
            fw = w.xWrap('capdb__wasm_test_int64_times2','thrice','twice');
            rc = fw(1);
            T.assert(12n===rc);
            ...
            w.scopedAllocCall(function(){
              ...
              const f = w.xWrap('capdb__wasm_test_int64_minmax',undefined,['i64*','i64*']);
  ...
  ////////////////////////////////////////////////////////////////////
    .t({
      name: 'capdb.StructBinder (jaccwabyt🐇)',
      predicate: (capdb)=>!!capdb.wasm.exports.capdb__wasm_test_struct
        || "Built without CAPDB_WASM_ENABLE_C_TESTS",
      test: function(capdb){
        const S = capdb, W = S.wasm;
        const MyStructDef = {
          sizeof: 0, members: {}
        };
        ...
        try{
          ...
          const testFunc = 1
                ? W.xGet('capdb__wasm_test_struct'/*name gets mangled in -O3 builds!*/)
                : W.xWrap('capdb__wasm_test_struct', undefined, '*');
          let counter = 0;
  ...
  ////////////////////////////////////////////////////////////////////
    .t('capdb.wasm.pstack', function(capdb){
      const P = wasm.pstack;
      const isAllocErr = (e)=>e instanceof capdb.WasmAllocError;
      ...
      try{
        ...
        T.assert(P.quota >= 4096)
          ...
          .mustThrowMatching(
            ()=>P.alloc('i33'),
            (e)=>e instanceof capdb.WasmAllocError
          );
        ;
        let p1 = P.alloc(12);
  ...
  T.g('capdb_randomness()')
    .t('To memory buffer', function(capdb){
      ...
      try{
        ...
        T.assert(undefined === capi.capdb_randomness(n - 10, p));
        let j, check = 0;
    ...
    .t('To byte array', function(capdb){
  ...
  T.g('capdb.oo1')
    .t({
      ...
      test: function(capdb){
        ...
        capdb.util.capdb__wasm_vfs_unlink(0, dbFile);
        const db = this.db = new capdb.oo1.DB(dbFile, 0 ? 'ct' : 'c');
        db.onclose = {
          disposeAfter: [],
          disposeBefore: [
            (db)=>{
              //console.debug("db.onclose.before dropping modules");
              //capdb.capi.capdb_drop_modules(db.pointer, 0);
            }
          ],
          before: function(db){
            while(this.disposeBefore.length){
              ...
              else if(v instanceof capdb.StructBinder.StructType){
                v.dispose();
        ...
        T.assert(wasm.isPtr(db.pointer))
          ...
          .assert(0===capdb.capi.capdb_extended_result_codes(db.pointer,1))
          .assert('main'===db.dbName(0))
          .assert('string' === typeof db.dbVfsName())
          .assert(db.pointer === wasm.xWrap.testConvertArg('capdb*',db));
        // Custom db error message handling via capdb_prepare_v2/v3()
        let rc = capi.capdb_prepare_v3(db.pointer, {/*invalid*/}, -1, 0, null, null);
        T.assert(capi.CAPDB_MISUSE === rc)
          .assert(0 === capi.capdb_errmsg(db.pointer).indexOf("Invalid SQL"))
        ...
        T.assert(ex instanceof capdb.SQLite3Error)
          .assert(capi.CAPDB_MISUSE===ex.resultCode)
          .assert(0===ex.message.indexOf("CAPDB_MISUSE: capdb result code"))
          .assert(ex.message.indexOf("Invalid SQL")>0);
        T.assert(db === db.checkRc(0))
          .assert(db === capdb.oo1.DB.checkRc(db,0))
          .assert(null === capdb.oo1.DB.checkRc(null,0));
        this.progressHandlerCount = 0;
        if( wasm.compileOptionUsed('OMIT_PROGRESS_CALLBACK') ){
          T.assert( !capi.capdb_progress_handler );
        }else{
          T.assert( !!capi.capdb_progress_handler );
          capi.capdb_progress_handler(db, 5, (p)=>{
  ...
  ////////////////////////////////////////////////////////////////////
    .t({
      ...
      test: function(capdb){
        ...
        //capdb.config.debug("Proxying",this.db);
        const misuseMsg = "CAPDB_MISUSE: Argument must be a WASM capdb pointer";
        T.mustThrowMatching(()=>capdb.oo1.DB.wrapHandle(this.db), misuseMsg)
          .mustThrowMatching(()=>capdb.oo1.DB.wrapHandle(0), misuseMsg);
        let dw = capdb.oo1.DB.wrapHandle(this.db.pointer);
        //capdb.config.debug('dw',dw);
        T.assert( dw, '!!dw' )
          .assert( dw instanceof capdb.oo1.DB, 'dw is-a oo1.DB' )
        ...
        try {
          ...
          let rc = capi.capdb_open_v2( ":memory:", ppOut,
                                         capi.CAPDB_OPEN_CREATE
                                         | capi.CAPDB_OPEN_READWRITE,
                                         0);
          ...
          const pTmp = pDb;
          dw = capdb.oo1.DB.wrapHandle(pDb, true);
          pDb = 0;
          //capdb.config.debug("dw",dw);
          ...
          rc = capi.capdb_prepare_v2( dw, select1, -1, ppOut, 0 );
          T.assert( 0===rc, 'prepare_v2() rc='+rc );
          pStmt = wasm.peekPtr(ppOut);
          T.assert( pStmt && wasm.isPtr(pStmt), 'pStmt is valid?' );
          try {
            //log( "capi.capdb_sql() =",capi.capdb_sql(pStmt));
            T.assert( select1 === capi.capdb_sql(pStmt), 'SQL mismatch' );
            q = capdb.oo1.Stmt.wrapHandle(dw, pStmt, false);
            ...
            q = capdb.oo1.Stmt.wrapHandle(dw, pStmt, true);
            pStmt = 0;
            q.reset();
            T.assert( q.step(), "step()" )
              .assert( !q.step(), "!step()" );
          }finally{
            if( pStmt ) capi.capdb_finalize(pStmt)
            if( q ) q.finalize();
          }

        }finally{
          wasm.pstack.restore(stack);
          if( pDb ){ capi.capdb_close_v2(pDb); }
          else if( dw ){ dw.close(); }
        }
      }
  ...
  ////////////////////////////////////////////////////////////////////
    .t('capdb_db_config() and capdb_status()', function(capdb){
      let rc = capi.capdb_db_config(this.db, capi.CAPDB_DBCONFIG_LEGACY_ALTER_TABLE, 0, 0);
      T.assert(0===rc);
      rc = capi.capdb_db_config(this.db, capi.CAPDB_DBCONFIG_MAX+1, 0);
      T.assert(capi.CAPDB_MISUSE === rc);
      rc = capi.capdb_db_config(this.db, capi.CAPDB_DBCONFIG_MAINDBNAME, "main");
      T.assert(0 === rc);
      const stack = wasm.pstack.pointer;
      try {
        ...
        rc = capi.capdb_status(capi.CAPDB_STATUS_MEMORY_USED,
                                 pCur, pHi, 0);
        ...
        if(wasm.bigIntEnabled){
          ...
          rc = capi.capdb_status64(capi.CAPDB_STATUS_MEMORY_USED,
                                     pCur, pHi, 0);
      ...
      capi.capdb_db_config(this.db, capi.CAPDB_DBCONFIG_ENABLE_COMMENTS, 0, null);
      T.mustThrow(()=>this.db.exec("select 1 /* with comments */"), "SQL comments are disallowed");
      capi.capdb_db_config(this.db, capi.CAPDB_DBCONFIG_ENABLE_COMMENTS, 1, null);
      this.db.exec("select 1 /* with comments */");
      /* CAPDB_DBCONFIG_ENABLE_ATTACH_... are in the ATTACH-specific tests */
    })

  ////////////////////////////////////////////////////////////////////
    .t('DB.Stmt', function(capdb){
      ...
      try {
        T.assert(wasm.isPtr(st.pointer))
          ...
          .assert(
            capi.capdb_stmt_status(
              st, capi.CAPDB_STMTSTATUS_RUN, 0
            ) === 0)
          ...
          .assert(0===capi.capdb_bind_parameter_count(st))
          .mustThrow(()=>st.bind(1,null))
          .assert(true===st.step())
          .assert(3 === st.get(0))
          .mustThrow(()=>st.get(1))
          .mustThrow(()=>st.get(0,~capi.CAPDB_INTEGER))
          .assert(3 === st.get(0,capi.CAPDB_INTEGER))
          .assert(3 === st.getInt(0))
          .assert('3' === st.get(0,capi.CAPDB_TEXT))
          .assert('3' === st.getString(0))
          .assert(3.0 === st.get(0,capi.CAPDB_FLOAT))
          .assert(3.0 === st.getFloat(0))
          .assert(3 === st.get({}).a)
          .assert(3 === st.get([])[0])
          .assert(3 === st.getJSON(0))
          .assert(st.get(0,capi.CAPDB_BLOB) instanceof Uint8Array)
          .assert(1===st.get(0,capi.CAPDB_BLOB).length)
      ...
      T.mustThrowMatching(()=>new capdb.oo1.Stmt("hi"), function(err){
        return (err instanceof capdb.SQLite3Error)
          && capi.CAPDB_MISUSE === err.resultCode
          && 0 < err.message.indexOf("Do not call the Stmt constructor directly.")
      });
    })

  ////////////////////////////////////////////////////////////////////////
    .t('capdb_js_...()', function(){
      const db = this.db;
      if(1){
        const vfsList = capi.capdb_js_vfs_list();
        T.assert(vfsList.length>1);
        wasm.scopedAllocCall(()=>{
          const vfsArg = (v)=>wasm.xWrap.testConvertArg('capdb_vfs*',v);
          for(const v of vfsList){
            T.assert('string' === typeof v);
            const pVfs = capi.capdb_vfs_find(v);
            T.assert(wasm.isPtr(pVfs))
              .assert(pVfs===vfsArg(v));
            const vfs = new capi.capdb_vfs(pVfs);
      ...
      /**
         ...
         db.
      */
      const pVfsMem = capi.capdb_vfs_find('memdb'),
            pVfsDflt = capi.capdb_vfs_find(0),
            pVfsDb = capi.capdb_js_db_vfs(db.pointer);
      T.assert(pVfsMem > 0)
        .assert(pVfsDflt > 0)
        .assert(pVfsDb > 0)
      ...
      ;
      /*const vMem = new capi.capdb_vfs(pVfsMem),
        vDflt = new capi.capdb_vfs(pVfsDflt),
        vDb = new capi.capdb_vfs(pVfsDb);*/
      const duv = capi.capdb_js_db_uses_vfs;
  ...
  ////////////////////////////////////////////////////////////////////
    .t('Table t', function(capdb){
      ...
      if(wasm.bigIntEnabled && haveWasmCTests()){
        const mI = wasm.xCall('capdb__wasm_test_int64_max');
      ...
      try {
        T.assert(0===st.columnCount)
          ...
          .assert(1===capi.capdb_bind_parameter_count(st))
      ...
      try {
        ...
        T.assert(e instanceof capdb.SQLite3Error)
  ...
  ////////////////////////////////////////////////////////////////////
    .t({
      name: "capdb_set_authorizer()",
      predicate: ()=>!!wasm.exports.capdb_set_authorizer || "Missing capdb_set_authorizer()",
      test:function(capdb){
        T.assert(capi.CAPDB_IGNORE>0)
          .assert(capi.CAPDB_DENY>0);
        const db = this.db;
        const ssa = capi.capdb_set_authorizer;
        const n = db.selectValue('select count(*) from t');
        T.assert(n>0);
        let authCount = 0;
        let rc = ssa(db, function(pV, iCode, s0, s1, s2, s3){
          ++authCount;
          return capi.CAPDB_IGNORE;
        }, 0);
        T.assert(0===rc)
          .assert(
        ...
        /* Reminder: we don't use DELETE because, from the C API docs:
          ...
          "If the action code is [CAPDB_DELETE] and the callback
          returns [CAPDB_IGNORE] then the [DELETE] operation proceeds
  ...
  ////////////////////////////////////////////////////////////////////////
    .t("capdb_table_column_metadata()", function(capdb){
      const stack = wasm.pstack.pointer;
      try{
        const [pzDT, pzColl, pNotNull, pPK, pAuto] =
              wasm.pstack.allocPtr(5);
        const rc = capi.capdb_table_column_metadata(
  ...
  ////////////////////////////////////////////////////////////////////////
    .t('selectArray/Object()', function(capdb){
      const db = this.db;
  ...
  ////////////////////////////////////////////////////////////////////////
    .t('selectArrays/Objects()', function(capdb){
      const db = this.db;
  ...
  ////////////////////////////////////////////////////////////////////////
    .t('selectArray/Object/Values() via INSERT/UPDATE...RETURNING', function(capdb){
    ...
    ////////////////////////////////////////////////////////////////////////
    .t({
      name: 'capdb_js_db_export()',
      predicate: ()=>true,
      test: function(capdb){
        const db = this.db;
        const xp = capi.capdb_js_db_export(db.pointer);
    ...
    .t({
      name: 'capdb_js_posix_create_file()',
      predicate: ()=>true,
      test: function(capdb){
        const db = this.db;
        const filename = "capdb_js_posix_create_file.db";
        capi.capdb_js_posix_create_file(filename, this.dbExport);
        delete this.dbExport;
        const db2 = new capdb.oo1.DB(filename,'r');
        try {
          ...
          db2.close();
          capdb.util.capdb__wasm_vfs_unlink(0, filename);
        }
      }
    }/*capdb_js_posix_create_file()*/)
    ...
    .t({
      ...
      test: function(capdb){
        ...
        db.createFunction(
          ...
          1 ? (pCx,a,b)=>a+b
            : (pCx,a,b)=>{
              /*return capdb.capi.capdb_result_error_js(
                db, capdb.capi.CAPDB_ERROR, "foo???"
              );*/
        ...
        db.createFunction("bar", {
          arity: -1,
          xFunc: (pCx,...args)=>{
            T.assert(db.pointer === capi.capdb_context_db_handle(pCx));
            let rc = 0;
        ...
        let rc = capdb.capi.capdb_create_function_v2(
          this.db, "foo", 0, -1, 0, 0, 0, 0, 0
        );
        T.assert(
          capdb.capi.CAPDB_FORMAT === rc,
          "For invalid eTextRep argument."
        );
        rc = capdb.capi.capdb_create_function_v2(this.db, "foo", 0);
        T.assert(
          capdb.capi.CAPDB_MISUSE === rc,
        ...
        rc = capi.capdb_create_function_v2(
          db, "nary", 0, capi.CAPDB_UTF8, 0, fArityCheck, 0, 0, 0
        );
        T.assert( 0===rc );
        rc = capi.capdb_create_function_v2(
          db, "nary", 1, capi.CAPDB_UTF8, 0, fArityCheck, 0, 0, 0
        );
        T.assert( 0===rc );
        ...
        capi.capdb_create_function_v2(
          db, "nary", 0, capi.CAPDB_UTF8, 0, 0, 0, 0, 0
        );
        T.mustThrowMatching((()=>db.selectValue(sqlFArity0)),
                            (e)=>((e instanceof capdb.SQLite3Error)
        ...
        capi.capdb_create_function_v2(
          db, "nary", 1, capi.CAPDB_UTF8, 0, 0, 0, 0, 0
        );
  ...
  ////////////////////////////////////////////////////////////////////
    .t({
      ...
      test: function(capdb){
        const db = this.db;
        const sjac = capi.capdb_js_aggregate_context;
        db.createFunction({
          name: 'summer',
          xStep: (pCtx, n)=>{
        ...
        let v = db.selectValue([
          ...
          /* ------------------^^^^^^^^^^^ ensures that we're handling
              capdb_aggregate_context() properly. */
        ]);
        T.assert(15===v);
        ...
        db.createFunction({
          ...
          xFinal: (pCtx)=>{
            const ac = sjac(pCtx, 0);
            capi.capdb_result_int( pCtx, ac ? wasm.peek32(ac) : 0 );
            // xFinal() may either return its value directly or call
            // capdb_result_xyz() and return undefined. Both are
  ...
  ////////////////////////////////////////////////////////////////////
    .t({
      name: 'Window UDFs',
      predicate: (capdb)=>!!capdb.wasm.exports.capdb_create_window_function
      /*!capdb.wasm.compileOptionUsed('OMIT_WINDOWFUNC')*/
        || "Missing window functions",
      test: function(){
        /* Example window function, table, and results taken from:
           https://sqlite.org/windowfunctions.html#udfwinfunc */
        const db = this.db;
        const sjac = (cx,n=4)=>capi.capdb_js_aggregate_context(cx,n);
  ...
  ////////////////////////////////////////////////////////////////////
    .t("ATTACH", function(capdb){
      ...
      /** Demonstrate the JS-simplified form of the capdb_exec() callback... */
      let colCount = 0, rowCount = 0;
      let rc = capi.capdb_exec(
      ...
      // Demonstrate that an OOM result does not propagate through capdb_exec()...
      rc = capi.capdb_exec(
        db, ["select a,"," a*2 from foo.bar"], (aVals, aNames)=>{
          capdb.WasmAllocError.toss("just testing");
        }, 0, 0
      );
      T.assert(capi.CAPDB_ABORT === rc);
      ...
      /* CAPDB_DBCONFIG_ENABLE_ATTACH_CREATE/WRITE... */
      const db2 = new capdb.oo1.DB();
      try{
        capi.capdb_db_config(db2, capi.CAPDB_DBCONFIG_ENABLE_ATTACH_CREATE, 0, null);
        T.mustThrow(()=>db2.exec("attach 'attached.db' as foo"),
                    "Cannot create a new db via ATTACH");
        capi.capdb_db_config(db2, capi.CAPDB_DBCONFIG_ENABLE_ATTACH_CREATE, 1, null);
        db2.exec([
        ...
        capi.capdb_db_config(db2, capi.CAPDB_DBCONFIG_ENABLE_ATTACH_WRITE, 0, null);
        db2.exec("attach 'attached.db' as foo");
        T.mustThrow(()=>db2.exec("insert into foo.t(a) values(2)"),
                   "ATTACH_WRITE is false");
        capi.capdb_db_config(db2, capi.CAPDB_DBCONFIG_ENABLE_ATTACH_WRITE, 1, null);
        db2.exec([
          "detach foo;",
          "attach 'attached.db' as foo;",
  ...
  ////////////////////////////////////////////////////////////////////
    .t("Read-only", function(capdb){
      T.assert( 0===capi.capdb_db_readonly(this.db, "main") );
      const db = new capdb.oo1.DB('file://'+this.db.filename+'?mode=ro');
      T.assert( 1===capi.capdb_db_readonly(db, "main") );
      T.assert( -1===capi.capdb_db_readonly(db, "nope") );
      db.close();
    })
    ...
    .t({
      ...
      test: function(){
        ...
        try{
          ...
          const cf = w.xGet('capdb__wasm_test_intptr');
          ...
          if(w.bigIntEnabled){
            ...
            const cf64w = w.xGet('capdb__wasm_test_int64ptr');
            cf64w(pi64);
            T.assert(v64() == BigInt(2 * o64));
            cf64w(pi64);
            T.assert(v64() == BigInt(4 * o64));

            const biTimes2 = w.xGet('capdb__wasm_test_int64_times2');
            ...
            const minMaxI64 = [
              w.xCall('capdb__wasm_test_int64_min'),
              w.xCall('capdb__wasm_test_int64_max')
            ];
            ...
            w.xCall('capdb__wasm_test_int64_minmax', pMin, pMax);
  ...
  ////////////////////////////////////////////////////////////////////////
    .t({
      ...
      predicate: (capdb)=>(!!capdb.capi.capdb_vtab || "Missing vtab support"),
      test: function(capdb){
        const VT = capdb.vtab;
        ...
        */
        const tmplMod = new capdb.capi.capdb_module();
        T.assert(!tmplMod.$xUpdate);
        const dbg = 1 ? ()=>{} : capdb.config.debug;
          ...
          methods: {
            xConnect: function(pDb, pAux, argc, argv, ppVtab, pzErr){
              ...
              try{
                ...
                const rc = capi.capdb_declare_vtab(
            ...
            xOpen: function(pVtab, ppCursor){
              ...
              try{
                ...
                T.assert(t instanceof capi.capdb_vtab)
                  .assert(c instanceof capi.capdb_vtab_cursor);
                c._rowId = 0;
                return 0;
              }catch(e){
            ...
            xClose: function(pCursor){
              ...
              try{
                ...
                T.assert(c instanceof capi.capdb_vtab_cursor)
            ...
            xColumn: function(pCursor, pCtx, iCol){
              ...
              try{
                ...
                switch(iCol){
                    case tmplCols.A:
                      capi.capdb_result_int(pCtx, 1000 + c._rowId);
                      break;
                    case tmplCols.B:
                      capi.capdb_result_int(pCtx, 2000 + c._rowId);
                      break;
                    default: capdb.SQLite3Error.toss("Invalid column id",iCol);
                }
                return 0;
              }catch(e){
            ...
            xFilter: function(pCursor, idxNum, idxCStr,
                              argc, argv/* [capdb_value* ...] */){
              dbg("xFilter",...arguments);
              try{
                const c = VT.xCursor.get(pCursor);
                c._rowId = 0;
                const list = capi.capdb_values_to_js(argc, argv);
              ...
              try{
                ...
                if(pii.$nConstraint>0){
                  ...
                  for(let i=0; i < max; ++i ){
                    ...
                    T.assert(v instanceof sii.capdb_index_constraint)
                      .assert(v.pointer >= pii.$aConstraint);
                    v.dispose();
                    v = pii.nthConstraintUsage(i,true);
                    T.assert(wasm.isPtr(v));
                    v = pii.nthConstraintUsage(i);
                    T.assert(v instanceof sii.capdb_index_constraint_usage)
                ...
                if(pii.$nOrderBy>0){
                  ...
                  for(let i=0; i < max; ++i ){
                    ...
                    T.assert(wasm.isPtr(v));
                    v = pii.nthOrderBy(i);
                    T.assert(v instanceof sii.capdb_index_orderby)
        ...
        let rc = capi.capdb_create_module(
    ...
    .t({
      ...
      test: function(capdb){
        ...
        /**
           ...
           ext/misc/templatevtab.c.
        */
        let throwOnCreate = 1 ? 0 : capi.CAPDB_CANTOPEN
        /* ^^^ just for testing exception wrapping. Note that sqlite
           always translates errors from a vtable to a generic
           CAPDB_ERROR unless it's from xConnect()/xCreate() and that
           callback sets an error string. */;
        const vtabTrace = 1
              ? ()=>{}
              : (methodName,...args)=>console.debug('capdb_module::'+methodName+'():',...args);
        const modConfig = {
        ...
        this.db.onclose.disposeAfter.push(tmplMod);
        this.db.checkRc(capi.capdb_create_module(
  ...
  ////////////////////////////////////////////////////////////////////////
    .t('Custom collation', function(capdb){
      let collationCounter = 0;
      let myCmp = function(pArg,n1,p1,n2,p2){
        //int (*)(void*,int,const void*,int,const void*)
        ++collationCounter;
        const rc = wasm.exports.capdb_strnicmp(p1,p2,(n1<n2?n1:n2));
        return rc ? rc : (n1 - n2);
      };
      let rc = capi.capdb_create_collation_v2(this.db, "mycollation", capi.CAPDB_UTF8,
                                                0, myCmp, 0);
      this.db.checkRc(rc);
      ...
      rc = capi.capdb_create_collation(this.db,"hi",capi.CAPDB_UTF8/*not enough args*/);
      T.assert(capi.CAPDB_MISUSE === rc);
      rc = capi.capdb_create_collation_v2(this.db,"hi",capi.CAPDB_UTF8+1/*invalid encoding*/,0,0,0);
      T.assert(capi.CAPDB_FORMAT === rc)
        .mustThrowMatching(()=>this.db.checkRc(rc),
                           /CAPDB_UTF8 is the only supported encoding./);
      /*
      ...
      */
      ...
      rc = capi.capdb_create_collation_v2(this.db, "MYCOLLATION", capi.CAPDB_UTF8,
                                            0, myCmp, 0);
      this.db.checkRc(rc);
      rc = this.db.selectValue("select 'hi' = 'HI' collate mycollation");
      ...
      try {
        ...
        rc = capdb.capi.capdb_extended_errcode(this.db);
      }
      T.assert(capi.CAPDB_ERROR_MISSING_COLLSEQ === rc);
    })/*custom collation*/

  ////////////////////////////////////////////////////////////////////////
    .t('Close db', function(){
      T.assert(this.db).assert(wasm.isPtr(this.db.pointer));
      //wasm.capdb__wasm_db_reset(this.db); // will leak virtual tables!
      this.db.close();
  ...
  T.g('kvvfs')
    .t({
      ...
      test: function(capdb){
        ...
        if( isUIThread() ){
          T.assert( !!capi.capdb_js_kvvfs_size )
            .assert( !!capi.capdb_js_kvvfs_clear );
        }else{
          /* Historical behaviour retained not for compatibility but
             ...
             strings). */
          T.assert( !capi.capdb_js_kvvfs_clear )
            .assert( !capi.capdb_js_kvvfs_size );
        }
        const k = capdb.kvvfs;
        ...
        if( 0 ){
          ...
          try{
            ...
            const rc = wasm.exports.capdb__wasm_kvvfs_decode(pI, pO, nO);
    ...
    .t({
      ...
      test: function(capdb){
        const JDb = capdb.oo1.JsStorageDb;
        const pVfs = capi.capdb_vfs_find('kvvfs');
        T.assert(looksLikePtr(pVfs));
        let x = capdb.kvvfs.internal.storageForZClass('session');
        ...
        const unlink = this.kvvfsUnlink = ()=>capdb.kvvfs.clear(filename);
        unlink();
    ...
    .t({
      ...
      test: function(capdb){
        ...
        const JDb = capdb.oo1.JsStorageDb;
        const DB = capdb.oo1.DB;
        T.mustThrowMatching(()=>new JDb(""), capi.CAPDB_MISUSE);
        T.mustThrowMatching(()=>{
        ...
        }, capi.CAPDB_RANGE);
        {
          ...
          (new JDb(name)).close();
          T.assert( capdb.kvvfs.unlink(name) );
        }

        capdb.kvvfs.clear(filename);
        let db = new JDb(filename);
        const sqlSetup = [
          'create table kvvfs(a);',
          'insert into kvvfs(a) values(1),(2),(3)'
        ...
        try {
          ...
          const exportDb = capdb.kvvfs.export;
          db = new JDb(filename);
          db.exec('insert into kvvfs(a) values(4),(5),(6)');
          T.assert(6 === db.selectValue('select count(*) from kvvfs'));
          ...
          const n = capdb.kvvfs.estimateSize( dbFilename );
    ...
    .t({
      ...
      test: function(capdb){
        ...
        try {
          ...
          const importDb = capdb.kvvfs.import;
          ...
          if( 0 ){
            ...
            q1.bind('from q1');
            T.assert(capi.CAPDB_DONE===capi.capdb_step(q1),
    ...
    .t({
      name: 'kvvfs vtab',
      predicate: (capdb)=>!!capdb.kvvfs.create_module,
      test: function(capdb){
        const kvvfs = capdb.kvvfs;
        const db = new capdb.oo1.DB();
        const db2 = new capdb.oo1.DB('file:foo?vfs=kvvfs&delete-on-close=1');
        try{
...
//#/if enable-see
  ...
  T.g('Hook APIs')
    .t({
      name: "capdb_commit/rollback/update_hook()",
      predicate: ()=>wasm.bigIntEnabled || "Update hook requires int64",
      test: function(capdb){
        let countCommit = 0, countRollback = 0;;
        const db = new capdb.oo1.DB(':memory:',1 ? 'c' : 'ct');
        let rc = capi.capdb_commit_hook(db, (p)=>{
          //debug("commit hook",arguments);
          ++countCommit;
          return (17 == p) ? 0 : capi.CAPDB_ERROR;
        }, 17);
        T.assert( wasm.ptr.null === rc );

        // Commit hook...
        T.assert( 0!=capi.capdb_get_autocommit(db) );
        ...
        T.assert(17 == capi.capdb_commit_hook(db, 0, 0));

        // Rollback hook:
        rc = capi.capdb_rollback_hook(db, (p)=>{
          ++countRollback;
          T.assert( 21 == p );
        }, 21);
        T.assert( wasm.ptr.null===rc );
        T.mustThrowMatching(()=>{
          ...
          return (capi.CAPDB_MISUSE === e.resultCode)
            && ( e.message.indexOf('Invalid argument') > 0 );
        });
        T.assert(0 === countRollback, "Transaction was not started.");
        T.mustThrowMatching(()=>{
          db.transaction('immediate', ()=>{
            capdb.SQLite3Error.toss(capi.CAPDB_FULL,'testing rollback hook');
          });
        }, (e)=>{
          //console.error("transaction error:",e);
          return capi.CAPDB_FULL === e.resultCode
        });
        T.assert(1 === countRollback);
        T.assert(21 == capi.capdb_rollback_hook(db, 0, 0));

        // Update hook...
        const countUpdate = Object.create(null);
        capi.capdb_update_hook(db, (p,op,dbName,tbl,rowid)=>{
          T.assert('main' === dbName.toLowerCase())
            .assert('t' === tbl.toLowerCase())
            .assert(33==p)
            .assert('bigint' === typeof rowid);
          switch(op){
              case capi.CAPDB_INSERT:
              case capi.CAPDB_UPDATE:
              case capi.CAPDB_DELETE:
        ...
        T.assert(1 === countRollback)
          ...
          .assert(1 === countUpdate[capi.CAPDB_INSERT])
          .assert(2 === countUpdate[capi.CAPDB_UPDATE])
          .assert(1 === countUpdate[capi.CAPDB_DELETE]);
        //wasm.xWrap.FuncPtrAdapter.debugFuncInstall = true;
        T.assert(33 == capi.capdb_update_hook(db, 0, 0));
        //wasm.xWrap.FuncPtrAdapter.debugFuncInstall = false;
        db.close();
      }
    ...
    .t({
      ...
      predicate: ()=>capi.capdb_preupdate_hook || "Missing pre-update hook API",
      test: function(capdb){
        const db = new capdb.oo1.DB(':memory:', 1 ? 'c' : 'ct');
        const countHook = Object.create(null);
        let rc = capi.capdb_preupdate_hook(
          db, function(p, pDb, op, zDb, zTbl, iKey1, iKey2){
            T.assert(9 == p)
              .assert(db.pointer === pDb)
              .assert(1 === capi.capdb_preupdate_count(pDb))
              .assert( 0 > capi.capdb_preupdate_blobwrite(pDb) );
            countHook[op] = (countHook[op]||0) + 1;
            switch(op){
                case capi.CAPDB_INSERT:
                case capi.CAPDB_UPDATE:
                 T.assert('number' === typeof capi.capdb_preupdate_new_js(pDb, 0));
                  break;
                case capi.CAPDB_DELETE:
                 T.assert('number' === typeof capi.capdb_preupdate_old_js(pDb, 0));
                  break;
                default: toss("Unexpected hook operator:",op);
            }
          },
        ...
        T.assert(1 === countHook[capi.CAPDB_INSERT])
          .assert(2 === countHook[capi.CAPDB_UPDATE])
          .assert(1 === countHook[capi.CAPDB_DELETE]);
        //wasm.xWrap.FuncPtrAdapter.debugFuncInstall = true;
        T.assert( !!capi.capdb_preupdate_hook(db, 0, 0) );
        //wasm.xWrap.FuncPtrAdapter.debugFuncInstall = false;
        T.assert( !capi.capdb_preupdate_hook(db, 0, 0) );
        db.close();
      }
  ...
  T.g('Auto-extension API')
    .t({
      ...
      test: function(capdb){
        ...
        (new capdb.oo1.DB()).close();
        T.assert( 0===counter );
        capi.capdb_auto_extension(fp);
        (new capdb.oo1.DB()).close();
        T.assert( 1===counter );
        (new capdb.oo1.DB()).close();
        T.assert( 2===counter );
        capi.capdb_cancel_auto_extension(fp);
        wasm.uninstallFunction(fp);
        (new capdb.oo1.DB()).close();
        T.assert( 2===counter );
      }
  ...
  T.g('Session API')
    .t({
      ...
      predicate: ()=>!!capi.capdb_changegroup_add || "Missing session API",
      test: function(capdb){
        //warn("The session API tests could use some expansion.");
        const db1 = new capdb.oo1.DB(), db2 = new capdb.oo1.DB();
        const sqlInit = [
        ...
        try{
          ...
          let rc = capi.capdb_session_create(db1, "main", ppOut);
          T.assert(0===rc);
          let pSession = wasm.peekPtr(ppOut);
          T.assert(pSession && wasm.isPtr(pSession));
          capi.capdb_session_table_filter(pSession, (pCtx, tbl)=>{
          ...
          if(testSessionEnable){
            rc = capi.capdb_session_enable(pSession, 0);
            T.assert( 0 === rc )
              .assert( 0 === capi.capdb_session_enable(pSession, -1) );
            db1.exec("delete from t where rowid=2;");
            rc = capi.capdb_session_enable(pSession, 1);
            T.assert( rc > 0 )
              .assert( capi.capdb_session_enable(pSession, -1) > 0 )
              .assert(undefined === db1.selectValue('select a from t where rowid=2'));
          }else{
            //warn("capdb_session_enable() tests are currently disabled.");
          }
          ...
          rc = capi.capdb_session_changeset(pSession, pnChanges, ppChanges);
          T.assert( 0 === rc );
          capi.capdb_session_delete(pSession);
          pSession = 0;
          ...
          rc = capi.capdb_changeset_invert( nChanges, pChanges, pnChanges, ppChanges );
          T.assert( 0 === rc );
          rc = capi.capdb_changeset_apply(
...
//#query {
  ...
  select 'opfs-wl', 'OPFS with Web Locks', 'OpfsWlDb'
   ...
   T.g('@vfsName@: @label@',
      (capdb)=>(!!capi.capdb_vfs_find("@vfsName@") || 'requires "@vfsName@" VFS'))
    .t({
      name: '@vfsName@ db sanity checks',
      test: async (capdb)=>{
        await T.opfsCommon.sanityChecks('@vfsName@', capdb.oo1.@oo1Ctor@, capdb);
      }
    })
    .t({
      name: '@vfsName@ import/export',
      test: async (capdb)=>{
        await T.opfsCommon.importer('@vfsName@', capdb.oo1.@oo1Ctor@, capdb);
      }
    })
//#if vfsName = "opfs"
//#// This is independent of the VFS, so only test this once
    .t({
      ...
      test: async (capdb)=>{
        await T.opfsCommon.opfsUtil("@vfsName@", capdb.oo1.@oo1Ctor@, capdb);
      }
    })
//#/if
//#if enable-see
    .t({
      ...
      predicate: (capdb)=>!!capdb.oo1.@oo1Ctor@,
      test: async function(capdb){
        const ctor = capdb.oo1.@oo1Ctor@;
        const filename = 'capdb-see.edb';
...
//#/query
  ...
  T.g('OPFS SyncAccessHandle Pool VFS',
      (capdb)=>((isWorker() && !!capdb.installOpfsSAHPoolVfs) || "requires OPFS SAH Pool APIs"))
    .t({
      name: 'SAH sanity checks',
      test: async function(capdb){
        T.assert(!capdb.capi.capdb_vfs_find(sahPoolConfig.name))
          .assert(capdb.capi.capdb_js_vfs_list().indexOf(sahPoolConfig.name) < 0)
        const inst = capdb.installOpfsSAHPoolVfs,
              catcher = (e)=>{
        ...
        T.assert(u1 === u2)
          ...
          .assert(capdb.capi.capdb_vfs_find(sahPoolConfig.name))
        ...
        T.assert(db instanceof capdb.oo1.DB)
        ...
        T.assert(1 === u1.getFileCount())
          .mustThrowMatching(
            ()=>u1.pauseVfs(),
            (err)=>{
              return capi.CAPDB_MISUSE===err.resultCode
                && /^CAPDB_MISUSE: Cannot pause VFS /.test(err.message);
            },
        ...
        T.assert( u2===u2.pauseVfs() )
          .assert( u2.isPaused() )
          .assert( !capi.capdb_vfs_find(u2.vfsName) )
        ...
        if(1){ // test exportFile() and importDb()
          ...
          T.assert(db2 instanceof capdb.oo1.DB)
        ...
        T.assert(P3 === conf2.$testThrowPhase2)
          .assert(cErr === P3)
          .assert(undefined === u3)
          .assert(!capdb.capi.capdb_vfs_find(conf2.name));
...
//#/if enable-see
  ...
  ////////////////////////////////////////////////////////////////////////
  T.g('Misc. APIs')
    .t('bind_parameter_...', function(capdb){
      const db = new capdb.oo1.DB();
      db.exec("create table t(a)");
      const stmt = db.prepare("insert into t(a) values($a)");
      T.assert( 1===capi.capdb_bind_parameter_count(stmt) )
        .assert( 1===stmt.parameterCount )
        .assert( 1===capi.capdb_bind_parameter_index(stmt, "$a") )
        .assert( 0===capi.capdb_bind_parameter_index(stmt, ":a") )
        .assert( 1===stmt.getParamIndex("$a") )
        .assert( 0===stmt.getParamIndex(":a") )
        .assert( "$a"===capi.capdb_bind_parameter_name(stmt, 1) )
        .assert( null===capi.capdb_bind_parameter_name(stmt, 0) )
    ...
    /**
       ...
       via DB.exec().
    */
    .t('locked-by-exec() APIs', function(capdb){
      const db = new capdb.oo1.DB();
    ...
    .t("Misc. stmt_...", function(capdb){
      ...
      try{
        ...
        T.assert( stmt.isReadOnly() )
          .assert( 0===capi.capdb_stmt_isexplain(stmt) )
          .assert( 0===capi.capdb_stmt_explain(stmt, 1) )
          .assert( 0!==capi.capdb_stmt_isexplain(stmt) )
          .assert( 0===capi.capdb_stmt_explain(stmt, 2) )
          .assert( 0!==capi.capdb_stmt_isexplain(stmt) )
          .assert( 0===capi.capdb_stmt_explain(stmt, 0) )
          .assert( 0===capi.capdb_stmt_isexplain(stmt) );
        let n = 0;
        while( capi.CAPDB_ROW === capi.capdb_step(stmt) ){
          ++n;
          T.assert( 0!==capi.capdb_stmt_explain(stmt, 1),
                    "Because stmt is busy" )
            .assert( capi.capdb_stmt_busy(stmt) )
            .assert( stmt.isBusy() )
            .assert( 0!==capi.capdb_stmt_readonly(stmt) )
            .assert( true===stmt.isReadOnly() );
          const sv = capi.capdb_column_value(stmt, 0);
          T.assert( 123===capi.capdb_value_int(sv) )
            .assert( "doggiebiscuits"===capi.capdb_column_decltype(stmt,0) )
            .assert( null===capi.capdb_column_decltype(stmt,1) );
        }
        T.assert( 1===n )
          .assert( 0===capi.capdb_stmt_busy(stmt) )
          .assert( !stmt.isBusy() );

        if( wasm.exports.capdb_column_origin_name ){
          log("Column metadata APIs enabled");
          T.assert( "t" === capi.capdb_column_table_name(stmt, 0))
            .assert("a" === capi.capdb_column_origin_name(stmt, 0))
            .assert("main" === capi.capdb_column_database_name(stmt, 0))
        }else{
        ...
        stmt.step();
        const sv = capi.capdb_column_value(stmt,0);
        T.assert("a\0b\0c"===capi.capdb_value_text(sv),
                 "Expecting NULs to have survived.");
        stmt.finalize(); stmt = null;

        /* capdb_bind_zeroblob() (added in 3.53) */
        stmt = db.prepare("select ?1");
        T.assert( 0===capi.capdb_bind_zeroblob(stmt, 1, 53) );
        T.assert( stmt.step() );
        const b = stmt.get(0);
        stmt.finalize(); stmt = null;
  ...
  ////////////////////////////////////////////////////////////////////
    .t("interrupt", function(capdb){
      const db = new capdb.oo1.DB();
      T.assert( 0===capi.capdb_is_interrupted(db) );
      capi.capdb_interrupt(db);
      T.assert( 0!==capi.capdb_is_interrupted(db) );
      db.close();
    })

  ////////////////////////////////////////////////////////////////////
    .t("capdb_set_errmsg()", function(capdb){
      /* Added in 3.51.0 */
      const db = new capdb.oo1.DB();//(':memory:','wt');
      try{
        const capi = capdb.capi;
        const sse = capi.capdb_set_errmsg,
              sec = capi.capdb_errcode,
              sem = capi.capdb_errmsg;
        T.assert( 0===sec(db) )
          .assert( "not an error"===sem(db) );
        let rc = sse(db, capi.CAPDB_RANGE, "nope");
        T.assert( 0==rc )
          .assert( capi.CAPDB_RANGE===sec(db) )
          .assert( "nope"===sem(db) );
        rc = sse(0, 0, 0);
        T.assert( capi.CAPDB_MISUSE===rc );
        rc = sse(db, 0, 0);
  ...
  T.g('Bug Reports')
    ...
    .t({
      ...
      test: async function(capdb){
        ...
        const tryOne = function(vfsName,descr){
          ...
          let db = new capdb.oo1.DB(uri + (vfsName ? '?vfs='+vfsName : ''));
          db.exec([
            "drop table if exists t;",
            "create table t(a);",
          ...
          db.close();
          db = new capdb.oo1.DB(uri+'?mode=ro'+
                                  (vfsName ? '&vfs='+vfsName : ''));
          let err;
          try {
          ...
          T.assert(err && (err.message.indexOf('CAPDB_READONLY')===0));
          try{
        ...
        let poolUtil;
        await capdb.installOpfsSAHPoolVfs(poolConfig).then(p=>poolUtil=p);
        T.assert(!!capdb.capi.capdb_vfs_find(poolConfig.name), "Expecting to find just-registered VFS");
        try{
    ...
    .t({
      ...
      name: 'capdb_set_auxdata() binding signature',
      test: function(capdb){
        const db = new capdb.oo1.DB();
        const stack = wasm.pstack.pointer;
          ...
          db.createFunction("auxtest",{
            xFunc: function(pCx, x, y){
              ...
              if( !localAux ){
                ...
                */
                ++nAuxSet;
                capi.capdb_set_auxdata(pCx, 0, pAux, pAuxDtor);
              }else{
  ...
  ////////////////////////////////////////////////////////////////////////
  log("Loading and initializing capdb WASM module...");
  if(1){
    globalThis.capdbApiConfig = {
...
//#if not target:es6-module
  if(!globalThis.capdbInitModule && !isUIThread()){
    /* Vanilla worker, as opposed to an ES6 module worker */
    /*
      If capdb.js is in a directory other than this script, in order
      to get capdb.js to resolve capdb.wasm properly, we have to
      explicitly tell it where capdb.js is being loaded from. We do
      that by passing the `capdb.dir=theDirName` URL argument to
      _this_ script. That URL argument will be seen by the JS/WASM
      loader and it will adjust the capdb.wasm path accordingly. If
      capdb.js/.wasm are in the same directory as this script then
      that's not needed.
    ...
    */
    ...
    if(urlParams.has('capdb.dir')){
      capdbJs = urlParams.get('capdb.dir') + '/' + capdbJs;
    }
    importScripts(capdbJs);
  }
//#/if
  globalThis.capdbInitModule.__isUnderTest =
    true /* disables certain API-internal cleanup so that we can
            test internal APIs from here */;
  globalThis.capdbInitModule({
    print: log,
    printErr: error
  }).then(async function(capdb){
    TestUtil.assert(!!capdb.util);
    log("Done initializing WASM/JS bits. Running tests...");
    capdb.config.warn("Installing capdb bits as global S for local dev/test purposes.");
    globalThis.S = capdb;
    /*await capdb.installOpfsSAHPoolVfs(sahPoolConfig)
    ...
    capi = capdb.capi;
    wasm = capdb.wasm;
    log("capdb version:",capi.capdb_libversion(),
        capi.capdb_sourceid());
    if(wasm.bigIntEnabled){
    ...
    if(haveWasmCTests()){
      log("capdb__wasm_test_...() APIs are available.");
    }else{
      logClass('warning',"capdb__wasm_test_...() APIs unavailable.");
    }
    log("registered vfs list =",capi.capdb_js_vfs_list().join(', '));
    SQLite3 = capdb;
    log("WASM pointer size:",wasm.ptr.size,"bytes.");
    TestUtil.checkHeapSize();
    TestUtil.runTests(capdb);
  });
})(self);
