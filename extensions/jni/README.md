...
capdb API. If you are reading this from the distribution ZIP file,
...
final capdb db = capdb_open(":memory:");
try {
  final int rc = capdb_errcode(db);
  if( 0 != rc ){
    if( null != db ){
      System.out.print("Error opening db: "+capdb_errmsg(db));
    }else{
  ...
  // ALWAYS close databases using capdb_close() or capdb_close_v2()
  // when done with them. All of their active statement handles must
  // first have been passed to capdb_finalize().
  capdb_close_v2(db);
}
```
...
- `capdb_result_java_object()`
- `capdb_column_java_object()`
- `capdb_value_java_object()`
...
statement handles. `capdb_close()` fails if the db cannot be closed
whereas `capdb_close_v2()` recognizes that case and marks the db as
...
`CAPDB_API_ARMOR` mechanism to help product against such misuse. Some
...
```c
// C:
int capdb_create_collation(capdb * db, const char * name, int eTextRep,
                             void *pUserData,
                             int (*xCompare)(void*,int,void const *,int,void const *));

int capdb_create_collation_v2(capdb * db, const char * name, int eTextRep,
                                void *pUserData,
                                int (*xCompare)(void*,int,void const *,int,void const *),
                                void (*xDestroy)(void*));
```
...
```java
// Java:
int capdb_create_collation(capdb db, String name, int eTextRep,
                             Object pUserData, xCompareType xCompare);

int capdb_create_collation_v2(capdb db, String name, int eTextRep,
                                Object pUserData,
...
```java
int rc = capdb_create_collation(db, "mycollation", CAPDB_UTF8, new SomeCallbackType(){
...
- In the specific example above, `capdb_create_collation_v2()`
...
The [`capdb_create_function()`](https://sqlite.org/c3ref/create_function.html)
...
```java
int capdb_create_function(capdb db, String funcName, int nArgs,
...
Various APIs which accept callbacks, e.g. `capdb_trace_v2()` and
`capdb_update_hook()`, use interfaces similar to those shown above.
