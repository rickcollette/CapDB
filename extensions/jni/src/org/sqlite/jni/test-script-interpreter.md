...
appended to the result buffer according to the following rules:
  ...
  *   If capdb_column_text() returns NULL, then append "nil" - or
      some other text that is specified by the --null command - and skip
      all subsequent rules.

  *   If capdb_column_text() is an empty string, append `{}` to the
      result buffer and skip all subsequent rules.

  *   If capdb_column_text() does not contain any special
  ...
  *   If capdb_column_text() does not contains curly braces, then put
...
code (ex: "CAPDB_CONSTRAINT") as if it were a column value.  Then append
