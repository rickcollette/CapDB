...
/**
   Callback for use with {@link CApi#capdb_prepare_multi}.
*/
public interface PrepareMultiCallback extends CallbackProxy {

  /**
     Gets passed a capdb_stmt which it may handle in arbitrary ways,
     transferring ownership of it to this function.

     capdb_prepare_multi() will _not_ finalize st - it is up
     to the call() implementation how st is handled.

     Must return 0 on success or an CAPDB_... code on error. If it
     throws, capdb_prepare_multi() will transform the exception into
  ...
  */
  final class StepAll implements PrepareMultiCallback {
    public StepAll(){}
    /**
       Calls capdb_step() on st until it returns something other than
       CAPDB_ROW. If the final result is CAPDB_DONE then 0 is returned,
       else the result of the final step is returned.
    */
    @Override public int call(capdb_stmt st){
      int rc = CApi.CAPDB_DONE;
      while( CApi.CAPDB_ROW == (rc = CApi.capdb_step(st)) ){}
      return CApi.CAPDB_DONE==rc ? 0 : rc;
    }
  }
}
