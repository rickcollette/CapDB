...
class DbException extends SQLTesterException {
  DbException(capdb db, int rc, boolean closeDb){
    super("DB error #"+rc+": "+capdb_errmsg(db),true);
    if( closeDb ) capdb_close_v2(db);
  }
  DbException(capdb db, int rc){
    this(db, rc, false);
  }
}
...
public class SQLTester {
  ...
  private final capdb[] aDb = new capdb[7];
  ...
  capdb setCurrentDb(int n){
    affirmDbId(n);
    iCurrentDb = n;
    return this.aDb[n];
  }

  capdb getCurrentDb(){ return aDb[iCurrentDb]; }

  capdb getDbById(int id){
    return affirmDbId(id).aDb[id];
  }

  void closeDb(int id) {
    final capdb db = affirmDbId(id).aDb[id];
    if( null != db ){
      capdb_close_v2(db);
      aDb[id] = null;
    }
  ...
  void closeAllDbs(){
    for(int i = 0; i<aDb.length; ++i){
      capdb_close_v2(aDb[i]);
      aDb[i] = null;
    }
  }

  capdb openDb(String name, boolean createIfNeeded) {
    closeDb();
    int flags = CAPDB_OPEN_READWRITE;
    if( createIfNeeded ) flags |= CAPDB_OPEN_CREATE;
    final OutputPointer.capdb out = new OutputPointer.capdb();
    int rc = capdb_open_v2(name, out, flags, null);
    final capdb db = out.take();
  ...
  capdb openDb(int slot, String name, boolean createIfNeeded) throws DbException {
    affirmDbId(slot);
    iCurrentDb = slot;
  ...
  */
  ...
  private void appendDbErr(capdb db, StringBuilder sb, int rc){
    sb.append(org.sqlite.jni.capi.ResultCode.getEntryForInt(rc)).append(' ');
    final String msg = escapeSqlValue(capdb_errmsg(db));
    if( '{' == msg.charAt(0) ){
      sb.append(msg);
    }else{
  ...
  */
  public int execSql(capdb db, boolean throwOnError,
        ...
        if( null!=sb ){
          ...
          while( CAPDB_ROW == (rc = capdb_step(stmt)) ){
            for(int i = 0; i < nCol; ++i){
              ...
              val = capdb_column_text16(stmt, i);
              if( null==val ){
                sb.append( nullView );
                continue;
              }
  ...
  public static void main(String[] argv) throws Exception{
    ...
    capdb_auto_extension(ax);
    try {
      t.runTests();
    }finally{
      capdb_cancel_auto_extension(ax);
  ...
  */
  ...
  /**
     Works essentially the same as capdb_strglob() except that the
