...
**          $expert destroy
*/
static int CAPDB_TCLAPI testExpertCmd(
  void *clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
){
  capdbexpert *pExpert = (capdbexpert*)clientData;
  struct Subcmd {
    const char *zSub;
    int nArg;
    const char *zMsg;
  ...
  switch( iSub ){
    case 0: {      /* sql */
      ...
      rc = capdb_expert_sql(pExpert, zArg, &zErr);
      break;
    }

    case 1: {      /* analyze */
      rc = capdb_expert_analyze(pExpert, &zErr);
      break;
    }

    case 2: {      /* count */
      int n = capdb_expert_count(pExpert);
    ...
    case 3: {      /* report */
      ...
      zReport = capdb_expert_report(pExpert, iStmt, 1+iEnum);
...
static void CAPDB_TCLAPI testExpertDel(void *clientData){
  capdbexpert *pExpert = (capdbexpert*)clientData;
  capdb_expert_destroy(pExpert);
}

/*
** capdb_expert_new DB
*/
static int CAPDB_TCLAPI test_capdb_expert_new(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  zCmd = capdb_mprintf("capdbexpert%d", ++iCmd);
  if( zCmd==0 ){
    Tcl_AppendResult(interp, "out of memory", (char*)0);
    return TCL_ERROR;
  }

  pExpert = capdb_expert_new(db, &zErr);
  if( pExpert==0 ){
...
int TestExpert_Init(Tcl_Interp *interp){
  ...
  struct Cmd {
    ...
    { "capdb_expert_new", test_capdb_expert_new },
  };
  int i;
