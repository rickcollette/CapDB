...
struct amatch_vtab {
  ...
  capdb_stmt *pVCheck;     /* Query to check zVocabTab */
...
struct amatch_cursor {
  ...
  capdb_int64 iRowid;      /* The rowid of the current word */
  amatch_langid iLang;       /* Use this language ID */
  amatch_cost rLimit;        /* Maximum cost of any term */
  capdb_int64 nBuf;        /* Space allocated for zBuf */
...
static int amatchLoadOneRule(
  ...
  if( rCost<=0 || rCost>AMATCH_MX_COST ){
    *pzErr = capdb_mprintf("%s: cost must be between 1 and %d", 
        p->zClassName, AMATCH_MX_COST
    );
    rc = CAPDB_ERROR;
  }else
  if( nFrom>AMATCH_MX_LENGTH || nTo>AMATCH_MX_LENGTH ){
    *pzErr = capdb_mprintf("%s: maximum string length is %d", 
        p->zClassName, AMATCH_MX_LENGTH
    );
    rc = CAPDB_ERROR;    
  }else
  if( iLang<0 || iLang>AMATCH_MX_LANGID ){
    *pzErr = capdb_mprintf("%s: iLang must be between 0 and %d", 
  ...
  {
    pRule = capdb_malloc64( sizeof(*pRule) + nFrom + nTo );
    if( pRule==0 ){
      rc = CAPDB_NOMEM;
    }else{
...
static int amatchLoadRules(
  ...
  zSql = capdb_mprintf("SELECT * FROM %Q.%Q", p->zDb, p->zCostTab);
  if( zSql==0 ){
    rc = CAPDB_NOMEM;
  }else{
    ...
    if( rc!=CAPDB_OK ){
      *pzErr = capdb_mprintf("%s: %s", p->zClassName, capdb_errmsg(db));
    }else if( capdb_column_count(pStmt)!=4 ){
      *pzErr = capdb_mprintf("%s: %s has %d columns, expected 4",
          p->zClassName, p->zCostTab, capdb_column_count(pStmt)
      );
      rc = CAPDB_ERROR;
    }else{
...
** and returns a pointer to a buffer allocated using capdb_malloc() 
...
static char *amatchDequote(const char *zIn){
  capdb_int64 nIn;              /* Size of input string, in bytes */
...
static void amatchFree(amatch_vtab *p){
  if( p ){
    amatchFreeRules(p);
    amatchVCheckClear(p);
    capdb_free(p->zClassName);
    capdb_free(p->zDb);
    capdb_free(p->zCostTab);
    capdb_free(p->zVocabTab);
    capdb_free(p->zVocabWord);
    capdb_free(p->zVocabLang);
    capdb_free(p->zSelf);
    memset(p, 0, sizeof(*p));
    capdb_free(p);
  }
...
*/
static int amatchDisconnect(capdb_vtab *pVtab){
...
static int amatchConnect(
  ...
  *ppVtab = 0;
  ...
  rc = CAPDB_OK;
  if( pNew->zCostTab==0 ){
    *pzErr = capdb_mprintf("no edit_distances table specified");
    rc = CAPDB_ERROR;
  }else{
...
*/
static int amatchOpen(capdb_vtab *pVTab, capdb_vtab_cursor **ppCursor){
  amatch_vtab *p = (amatch_vtab*)pVTab;
  amatch_cursor *pCur;
...
*/
static int amatchClose(capdb_vtab_cursor *cur){
...
static void amatchAddWord(
  ...
  if( nBase+nTail+3>pCur->nBuf ){
    ...
    pCur->zBuf = capdb_realloc64(pCur->zBuf, pCur->nBuf);
    if( pCur->zBuf==0 ){
      pCur->nBuf = 0;
      return;
    }
  ...
  pWord = capdb_malloc64( sizeof(*pWord) + nBase + nTail - 1 );
  if( pWord==0 ) return;
...
*/
static int amatchNext(capdb_vtab_cursor *cur){
  amatch_cursor *pCur = (amatch_cursor*)cur;
  amatch_word *pWord = 0;
  amatch_avl *pNode;
  int isMatch = 0;
  ...
  if( p->pVCheck==0 ){
    ...
    rc = capdb_prepare_v2(p->db, zSql, -1, &p->pVCheck, 0);
    capdb_free(zSql);
    if( rc ) return rc;
  }
  capdb_bind_int(p->pVCheck, 2, pCur->iLang);
  ...
  do{
    ...
    if( nWord+20>nBuf ){
      nBuf = nWord+100;
      zBuf = capdb_realloc64(zBuf, nBuf);
    ...
    if( zNextIn[0] && zNextIn[0]!='*' ){
      capdb_reset(p->pVCheck);
      amatchStrcat(zBuf, zNextIn);
      capdb_bind_text(p->pVCheck, 1, zBuf, nWord+nNextIn, CAPDB_STATIC);
      rc = capdb_step(p->pVCheck);
      if( rc==CAPDB_ROW ){
        zW = (const char*)capdb_column_text(p->pVCheck, 0);
    ...
    while( 1 ){
      ...
      capdb_reset(p->pVCheck);
      capdb_bind_text(p->pVCheck, 1, zBuf, -1, CAPDB_TRANSIENT);
      rc = capdb_step(p->pVCheck);
...
static int amatchFilter(
  ...
  idx = 0;
  if( idxNum & 1 ){
    zWord = (const char*)capdb_value_text(argv[0]);
    idx++;
  }
  if( idxNum & 2 ){
    pCur->rLimit = (amatch_cost)capdb_value_int(argv[idx]);
    idx++;
  }
  if( idxNum & 4 ){
    pCur->iLang = (amatch_cost)capdb_value_int(argv[idx]);
    idx++;
  }
  pCur->zInput = capdb_mprintf("%s", zWord);
  if( pCur->zInput==0 ) return CAPDB_NOMEM;
...
** return NULL
*/
static int amatchColumn(capdb_vtab_cursor *cur, capdb_context *ctx, int i){
  amatch_cursor *pCur = (amatch_cursor*)cur;
  switch( i ){
    case AMATCH_COL_WORD: {
      capdb_result_text(ctx, pCur->pCurrent->zWord+2, -1, CAPDB_STATIC);
      break;
    }
    case AMATCH_COL_DISTANCE: {
      capdb_result_int(ctx, pCur->pCurrent->rCost);
      break;
    }
    ...
    case AMATCH_COL_NWORD: {
      capdb_result_int(ctx, pCur->nWord);
      break;
    }
    default: {
...
** The rowid.
*/
static int amatchRowid(capdb_vtab_cursor *cur, sqlite_int64 *pRowid){
  amatch_cursor *pCur = (amatch_cursor*)cur;
  *pRowid = pCur->iRowid;
  return CAPDB_OK;
}
...
** EOF indicator
*/
static int amatchEof(capdb_vtab_cursor *cur){
  amatch_cursor *pCur = (amatch_cursor*)cur;
...
static int amatchUpdate(
  ...
  (void)pRowid;
  if( argc==1 ){
    pVTab->zErrMsg = capdb_mprintf("DELETE from %s is not allowed", 
                                      p->zSelf);
    return CAPDB_ERROR;
  }
  if( capdb_value_type(argv[0])!=CAPDB_NULL ){
    pVTab->zErrMsg = capdb_mprintf("UPDATE of %s is not allowed", 
                                      p->zSelf);
    return CAPDB_ERROR;
  }
  if( capdb_value_type(argv[2+AMATCH_COL_WORD])!=CAPDB_NULL
   || capdb_value_type(argv[2+AMATCH_COL_DISTANCE])!=CAPDB_NULL
   || capdb_value_type(argv[2+AMATCH_COL_LANGUAGE])!=CAPDB_NULL
  ){
    pVTab->zErrMsg = capdb_mprintf(
            "INSERT INTO %s allowed for column [command] only", p->zSelf);
    return CAPDB_ERROR;
  }
  zCmd = capdb_value_text(argv[2+AMATCH_COL_COMMAND]);
  if( zCmd==0 ) return CAPDB_OK;
...
*/
static capdb_module amatchModule = {
  0,                      /* iVersion */
...
#ifndef CAPDB_OMIT_VIRTUALTABLE
  rc = capdb_create_module(db, "approximate_match", &amatchModule, 0);
#endif /* CAPDB_OMIT_VIRTUALTABLE */
  return rc;
}
