...
** of the output variable to $PTR using capdb_bind_pointer() with a
...
static void rememberFunc(
  ...
  ptr = capdb_value_pointer(argv[1], "carray");
  if( ptr ) *ptr = v;
...
int capdb_remember_init(
  ...
  rc = capdb_create_function(db, "remember", 2, CAPDB_UTF8, 0,
                               rememberFunc, 0, 0);
  return rc;
}
