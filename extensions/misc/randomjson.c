...
static void randJsonFunc(
  ...
  unsigned int iSeed;
  int eType = *(int*)capdb_user_data(context);
  Prng prng;
  char z1[STRSZ+1], z2[STRSZ+1];

  iSeed = (unsigned int)capdb_value_int(argv[0]);
  prngSeed(&prng, iSeed);
  ...
  capdb_result_text(context, z1, -1, CAPDB_TRANSIENT);
}

#ifdef _WIN32
  __declspec(dllexport)
#endif
int capdb_randomjson_init(
  capdb *db, 
  char **pzErrMsg, 
  ...
  rc = capdb_create_function(db, "random_json", 1,
                   CAPDB_UTF8|CAPDB_INNOCUOUS|CAPDB_DETERMINISTIC,
                   &cZero, randJsonFunc, 0, 0);
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "random_json5", 1,
                   CAPDB_UTF8|CAPDB_INNOCUOUS|CAPDB_DETERMINISTIC,
