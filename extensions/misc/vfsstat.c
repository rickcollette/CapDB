...
** global array.
*/
static capdb_uint64 aVfsCnt[VFSSTAT_MXCNT];
...
*/
static int vstatClose(capdb_file*);
static int vstatRead(capdb_file*, void*, int iAmt, capdb_int64 iOfst);
static int vstatWrite(capdb_file*,const void*,int iAmt, capdb_int64 iOfst);
static int vstatTruncate(capdb_file*, capdb_int64 size);
static int vstatSync(capdb_file*, int flags);
static int vstatFileSize(capdb_file*, capdb_int64 *pSize);
static int vstatLock(capdb_file*, int);
static int vstatUnlock(capdb_file*, int);
static int vstatCheckReservedLock(capdb_file*, int *pResOut);
static int vstatFileControl(capdb_file*, int op, void *pArg);
static int vstatSectorSize(capdb_file*);
static int vstatDeviceCharacteristics(capdb_file*);
static int vstatShmMap(capdb_file*, int iPg, int pgsz, int, void volatile**);
static int vstatShmLock(capdb_file*, int offset, int n, int flags);
static void vstatShmBarrier(capdb_file*);
static int vstatShmUnmap(capdb_file*, int deleteFlag);
static int vstatFetch(capdb_file*, capdb_int64 iOfst, int iAmt, void **pp);
static int vstatUnfetch(capdb_file*, capdb_int64 iOfst, void *p);

/*
** Methods for VStatVfs
*/
static int vstatOpen(capdb_vfs*, const char *, capdb_file*, int , int *);
static int vstatDelete(capdb_vfs*, const char *zName, int syncDir);
static int vstatAccess(capdb_vfs*, const char *zName, int flags, int *);
static int vstatFullPathname(capdb_vfs*, const char *zName, int, char *zOut);
static void *vstatDlOpen(capdb_vfs*, const char *zFilename);
static void vstatDlError(capdb_vfs*, int nByte, char *zErrMsg);
static void (*vstatDlSym(capdb_vfs *pVfs, void *p, const char*zSym))(void);
static void vstatDlClose(capdb_vfs*, void*);
static int vstatRandomness(capdb_vfs*, int nByte, char *zOut);
static int vstatSleep(capdb_vfs*, int microseconds);
static int vstatCurrentTime(capdb_vfs*, double*);
static int vstatGetLastError(capdb_vfs*, int, char *);
static int vstatCurrentTimeInt64(capdb_vfs*, capdb_int64*);
...
static const capdb_io_methods vstat_io_methods = {
  3,                              /* iVersion */
  vstatClose,                      /* xClose */
  vstatRead,                       /* xRead */
  vstatWrite,                      /* xWrite */
...
** Close an vstat-file.
*/
static int vstatClose(capdb_file *pFile){
...
*/
static int vstatTruncate(capdb_file *pFile, sqlite_int64 size){
  int rc;
  VStatFile *p = (VStatFile *)pFile;
...
** Sync an vstat-file.
*/
static int vstatSync(capdb_file *pFile, int flags){
  int rc;
  VStatFile *p = (VStatFile *)pFile;
...
*/
static int vstatFileSize(capdb_file *pFile, sqlite_int64 *pSize){
  int rc;
  VStatFile *p = (VStatFile *)pFile;
...
** Lock an vstat-file.
*/
static int vstatLock(capdb_file *pFile, int eLock){
  int rc;
  VStatFile *p = (VStatFile *)pFile;
...
*/
static int vstatUnlock(capdb_file *pFile, int eLock){
  int rc;
  VStatFile *p = (VStatFile *)pFile;
...
*/
static int vstatCheckReservedLock(capdb_file *pFile, int *pResOut){
  int rc;
  VStatFile *p = (VStatFile *)pFile;
...
*/
static int vstatFileControl(capdb_file *pFile, int op, void *pArg){
  VStatFile *p = (VStatFile *)pFile;
  int rc;
  rc = p->pReal->pMethods->xFileControl(p->pReal, op, pArg);
  if( op==CAPDB_FCNTL_VFSNAME && rc==CAPDB_OK ){
    *(char**)pArg = capdb_mprintf("vstat/%z", *(char**)pArg);
  }
  return rc;
}
...
*/
static int vstatSectorSize(capdb_file *pFile){
  int rc;
  VStatFile *p = (VStatFile *)pFile;
...
*/
static int vstatDeviceCharacteristics(capdb_file *pFile){
  int rc;
  VStatFile *p = (VStatFile *)pFile;
...
static int vstatShmLock(capdb_file *pFile, int offset, int n, int flags){
  VStatFile *p = (VStatFile *)pFile;
  return p->pReal->pMethods->xShmLock(p->pReal, offset, n, flags);
}

/* Memory barrier operation on shared memory */
static void vstatShmBarrier(capdb_file *pFile){
  VStatFile *p = (VStatFile *)pFile;
  p->pReal->pMethods->xShmBarrier(p->pReal);
}

/* Unmap a shared memory segment */
static int vstatShmUnmap(capdb_file *pFile, int deleteFlag){
  VStatFile *p = (VStatFile *)pFile;
...
static int vstatUnfetch(capdb_file *pFile, capdb_int64 iOfst, void *pPage){
  VStatFile *p = (VStatFile *)pFile;
...
static int vstatOpen(
  ...
  }else if( flags & CAPDB_OPEN_MAIN_JOURNAL ){
    p->eFiletype = VFSSTAT_JOURNAL;
  }else if( flags & CAPDB_OPEN_WAL ){
    p->eFiletype = VFSSTAT_WAL;
  }else if( flags & CAPDB_OPEN_MASTER_JOURNAL ){
    p->eFiletype = VFSSTAT_MASTERJRNL;
  }else if( flags & CAPDB_OPEN_SUBJOURNAL ){
    p->eFiletype = VFSSTAT_SUBJRNL;
  }else if( flags & CAPDB_OPEN_TEMP_DB ){
    p->eFiletype = VFSSTAT_TEMPDB;
  }else if( flags & CAPDB_OPEN_TEMP_JOURNAL ){
...
** returning.
*/
static int vstatDelete(capdb_vfs *pVfs, const char *zPath, int dirSync){
  int rc;
...
*/
static void *vstatDlOpen(capdb_vfs *pVfs, const char *zPath){
...
*/
static void vstatDlError(capdb_vfs *pVfs, int nByte, char *zErrMsg){
...
*/
static void vstatDlClose(capdb_vfs *pVfs, void *pHandle){
...
** random data.
*/
static int vstatRandomness(capdb_vfs *pVfs, int nByte, char *zBufOut){
...
** actually slept.
*/
static int vstatSleep(capdb_vfs *pVfs, int nMicro){
...
*/
static int vstatCurrentTime(capdb_vfs *pVfs, double *pTimeOut){
  STATCNT(VFSSTAT_ANY,VFSSTAT_CURTIME)++;
  return REALVFS(pVfs)->xCurrentTime(REALVFS(pVfs), pTimeOut);
}

static int vstatGetLastError(capdb_vfs *pVfs, int a, char *b){
  return REALVFS(pVfs)->xGetLastError(REALVFS(pVfs), a, b);
}
static int vstatCurrentTimeInt64(capdb_vfs *pVfs, capdb_int64 *p){
  STATCNT(VFSSTAT_ANY,VFSSTAT_CURTIME)++;
...
*/
static int vstattabConnect(capdb*, void*, int, const char*const*, 
                           capdb_vtab**,char**);
static int vstattabBestIndex(capdb_vtab*,capdb_index_info*);
static int vstattabDisconnect(capdb_vtab*);
static int vstattabOpen(capdb_vtab*, capdb_vtab_cursor**);
static int vstattabClose(capdb_vtab_cursor*);
static int vstattabFilter(capdb_vtab_cursor*, int idxNum, const char *idxStr,
                          int argc, capdb_value **argv);
static int vstattabNext(capdb_vtab_cursor*);
static int vstattabEof(capdb_vtab_cursor*);
static int vstattabColumn(capdb_vtab_cursor*,capdb_context*,int);
static int vstattabRowid(capdb_vtab_cursor*,capdb_int64*);
static int vstattabUpdate(capdb_vtab*,int,capdb_value**,capdb_int64*);
...
#define VSTAT_COLUMN_COUNT 2
  ...
  rc = capdb_declare_vtab(db,"CREATE TABLE x(file,stat,count)");
  if( rc==CAPDB_OK ){
...
*/
static int vstattabDisconnect(capdb_vtab *pVtab){
  capdb_free(pVtab);
  return CAPDB_OK;
}
...
*/
static int vstattabOpen(capdb_vtab *p, capdb_vtab_cursor **ppCursor){
  VfsStatCursor *pCur;
...
*/
static int vstattabClose(capdb_vtab_cursor *cur){
  capdb_free(cur);
  return CAPDB_OK;
}
...
*/
static int vstattabNext(capdb_vtab_cursor *cur){
  ((VfsStatCursor*)cur)->i++;
  return CAPDB_OK;
}
...
static int vstattabColumn(
  ...
  switch( i ){
    case VSTAT_COLUMN_FILE: {
      capdb_result_text(ctx, azFile[pCur->i/VFSSTAT_nStat], -1, CAPDB_STATIC);
      break;
    }
    case VSTAT_COLUMN_STAT: {
      const char **az;
      az = (pCur->i/VFSSTAT_nStat)==VFSSTAT_ANY ? azStatAny : azStat;
      capdb_result_text(ctx, az[pCur->i%VFSSTAT_nStat], -1, CAPDB_STATIC);
      break;
    }
    case VSTAT_COLUMN_COUNT: {
      capdb_result_int64(ctx, aVfsCnt[pCur->i]);
      break;
    }
  }
  return CAPDB_OK;
...
*/
static int vstattabRowid(capdb_vtab_cursor *cur, sqlite_int64 *pRowid){
  VfsStatCursor *pCur = (VfsStatCursor*)cur;
  *pRowid = pCur->i;
  return CAPDB_OK;
}
...
** row of output.
*/
static int vstattabEof(capdb_vtab_cursor *cur){
  VfsStatCursor *pCur = (VfsStatCursor*)cur;
...
static int vstattabUpdate(
  ...
  capdb_int64 iRowid, x;
  if( argc==1 ) return CAPDB_ERROR;
  if( capdb_value_type(argv[0])!=CAPDB_INTEGER ) return CAPDB_ERROR;
  iRowid = capdb_value_int64(argv[0]);
  if( iRowid!=capdb_value_int64(argv[1]) ) return CAPDB_ERROR;
  if( iRowid<0 || iRowid>=VFSSTAT_MXCNT ) return CAPDB_ERROR;
  if( capdb_value_type(argv[VSTAT_COLUMN_COUNT+2])!=CAPDB_INTEGER ){
    return CAPDB_ERROR;
  }
  x = capdb_value_int64(argv[VSTAT_COLUMN_COUNT+2]);
  if( x<0 ) return CAPDB_ERROR;
  aVfsCnt[iRowid] = x;
  return CAPDB_OK;
}

static capdb_module VfsStatModule = {
  0,                         /* iVersion */
  0,                         /* xCreate */
...
/*
** This routine is an capdb_auto_extension() callback, invoked to register
...
static int vstatRegister(
  ...
  return capdb_create_module(db, "vfsstat", &VfsStatModule, 0);
}
...
int capdb_vfsstat_init(
  ...
  vstat_vfs.pVfs = capdb_vfs_find(0);
  if( vstat_vfs.pVfs==0 ) return CAPDB_ERROR;
  vstat_vfs.base.szOsFile = sizeof(VStatFile) + vstat_vfs.pVfs->szOsFile;
  rc = capdb_vfs_register(&vstat_vfs.base, 1);
  if( rc==CAPDB_OK ){
    rc = vstatRegister(db, pzErrMsg, pApi);
    if( rc==CAPDB_OK ){
      rc = capdb_auto_extension((void(*)(void))vstatRegister);
    }
  }
