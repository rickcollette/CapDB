...
******************************************************************************
**
** This file contains code to implement the capdb_normalize() function.
**
**    char *capdb_normalize(const char *zSql);
**
** This function takes an SQL string as input and returns a "normalized"
** version of that string in memory obtained from capdb_malloc64().  The
...
** command-line program, compile this file with -DCAPDB_NORMALIZE_CLI
...
*/
static const unsigned char capdbUpperToLower[] = {
...
*/
static const unsigned char capdbCtypeMap[256] = {
...
*/
static capdb_int64 capdbGetToken(const unsigned char *z, int *tokenType){
  capdb_int64 i;
  int c;
    ...
    case CC_DIGIT: {
      *tokenType = TK_INTEGER;
      if( z[0]=='0' && (z[1]=='x' || z[1]=='X') && capdbIsxdigit(z[2]) ){
        for(i=3; capdbIsxdigit(z[i]); i++){}
        return i;
      }
      for(i=0; capdbIsdigit(z[i]); i++){}
      if( z[i]=='.' ){
        i++;
        while( capdbIsdigit(z[i]) ){ i++; }
        *tokenType = TK_FLOAT;
      }
      if( (z[i]=='e' || z[i]=='E') &&
           ( capdbIsdigit(z[i+1]) 
...
char *capdb_normalize(const char *zSql){
  ...
  capdb_int64 nZ;     /* Size of the output string in bytes */
  capdb_int64 nSql;   /* Size of the input string in bytes */
  int i;                /* Next character to read from zSql[] */
  int j;                /* Next slot to fill in on z[] */
  int tokenType;        /* Type of the next token */
  capdb_int64 n;      /* Size of the next token */
  int k;                /* Loop counter */

  nSql = strlen(zSql);
  nZ = nSql;
  z = capdb_malloc64( nZ+2 );
  if( z==0 ) return 0;
  for(i=j=0; zSql[i]; i += n){
    ...
    switch( tokenType ){
      ...
      case TK_PUNCT:
      case TK_NAME: {
        if( n==4 && capdb_strnicmp(zSql+i,"NULL",4)==0 ){
...
** compile this one source file with the -DCAPDB_NORMALIZE_CLI and link
** it against any SQLite library.  The resulting command-line program will
** run capdb_normalize() over the text of all files named on the command-
** line and show the result on standard output.
*/
#ifdef CAPDB_NORMALIZE_CLI
#include <stdio.h>
#include <stdlib.h>

/*
** Break zIn up into separate SQL statements and run capdb_normalize()
...
static void normalizeFile(char *zIn){
  ...
  for(i=0; zIn[i]; i++){
    ...
    cSaved = zIn[i+1];
    zIn[i+1] = 0;
    if( capdb_complete(zIn) ){
      char *zOut = capdb_normalize(zIn);
      if( zOut ){
        printf("%s\n", zOut);
        capdb_free(zOut);
      }else{
...
** command-line and run the text of each through capdb_normalize().
*/
int main(int argc, char **argv){
  ...
  for(i=1; i<argc; i++){
    ...
    sz = ftell(in);
    rewind(in);
    zBuf = capdb_realloc64(zBuf, sz+1);
    if( zBuf==0 ){
