...
*/
  ...
  /* no-op */
#elif defined(CAPDB_STATIC_PERCENTILE)
#  include "capdb.h"
#else
#  include "capdbext.h"
...
static void percentError(capdb_context *pCtx, const char *zFormat, ...){
  PercentileFunc *pFunc = (PercentileFunc*)capdb_user_data(pCtx);
  char *zMsg1;
  char *zMsg2;
  va_list ap;
  ...
  va_end(ap);
  zMsg2 = zMsg1 ? capdb_mprintf(zMsg1, pFunc->zName) : 0;
...
static void percentStep(capdb_context *pCtx, int argc, capdb_value **argv){
  ...
  if( argc==1 ){
    ...
    rPct = capdb_value_double(argv[1])/(double)pFunc->mxFrac;
...
#if defined(_WIN32) && !defined(SQLITE3_H) && !defined(CAPDB_STATIC_PERCENTILE)
__declspec(dllexport)
#endif
int capdb_percentile_init(
  capdb *db, 
  char **pzErrMsg, 
  ...
  for(i=0; i<sizeof(aPercentFunc)/sizeof(aPercentFunc[0]); i++){
    rc = capdb_create_window_function(db,
            aPercentFunc[i].zName,
            aPercentFunc[i].nArg,
            CAPDB_UTF8|CAPDB_INNOCUOUS|CAPDB_SELFORDER1,
            (void*)&aPercentFunc[i],
