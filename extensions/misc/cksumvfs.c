...
** capdb_database_file_object() interface which was added in
...
** "-DCAPDB_CKSUMVFS_STATIC" option so that this module knows that
...
** to the capdb_load_extension() API call.  Then you invoke the
** capdb_load_extension() API and shutdown the dummy database
...
**     capdb_load_extension(db, "./cksumvfs");
**     capdb_close(db);
**
** If this extension is compiled with -DCAPDB_CKSUMVFS_STATIC and
** statically linked against the application, initialize it using
** a single API call as follows:
**
**     capdb_register_cksumvfs();
**
...
** USING
**
** Open database connections using the capdb_open() or 
** capdb_open_v2() interfaces, as normal.  Ordinary database files
** (without a checksum) will operate normally.  Databases with 
** checksums will return an CAPDB_IOERR_DATA error if a page is
...
**
**    int n = 8;
**    capdb_file_control(db, 0, CAPDB_FCNTL_RESERVE_BYTES, &n);
**
...
** CAPDB_IOERR_DATA errors if a checksum mismatch is detected while
...
*/
#if defined(CAPDB_AMALGAMATION) && !defined(CAPDB_CKSUMVFS_STATIC)
# define CAPDB_CKSUMVFS_STATIC
#endif
...
*/
typedef struct capdb_vfs CksmVfs;
...
#define ORIGFILE(p) ((capdb_file*)(((CksmFile*)(p))+1))
...
*/
static int cksmClose(capdb_file*);
static int cksmRead(capdb_file*, void*, int iAmt, capdb_int64 iOfst);
static int cksmWrite(capdb_file*,const void*,int iAmt, capdb_int64 iOfst);
static int cksmTruncate(capdb_file*, capdb_int64 size);
static int cksmSync(capdb_file*, int flags);
static int cksmFileSize(capdb_file*, capdb_int64 *pSize);
static int cksmLock(capdb_file*, int);
static int cksmUnlock(capdb_file*, int);
static int cksmCheckReservedLock(capdb_file*, int *pResOut);
static int cksmFileControl(capdb_file*, int op, void *pArg);
static int cksmSectorSize(capdb_file*);
static int cksmDeviceCharacteristics(capdb_file*);
static int cksmShmMap(capdb_file*, int iPg, int pgsz, int, void volatile**);
static int cksmShmLock(capdb_file*, int offset, int n, int flags);
static void cksmShmBarrier(capdb_file*);
static int cksmShmUnmap(capdb_file*, int deleteFlag);
static int cksmFetch(capdb_file*, capdb_int64 iOfst, int iAmt, void **pp);
static int cksmUnfetch(capdb_file*, capdb_int64 iOfst, void *p);

/*
** Methods for CksmVfs
*/
static int cksmOpen(capdb_vfs*, const char *, capdb_file*, int , int *);
static int cksmDelete(capdb_vfs*, const char *zName, int syncDir);
static int cksmAccess(capdb_vfs*, const char *zName, int flags, int *);
static int cksmFullPathname(capdb_vfs*, const char *zName, int, char *zOut);
static void *cksmDlOpen(capdb_vfs*, const char *zFilename);
static void cksmDlError(capdb_vfs*, int nByte, char *zErrMsg);
static void (*cksmDlSym(capdb_vfs *pVfs, void *p, const char*zSym))(void);
static void cksmDlClose(capdb_vfs*, void*);
static int cksmRandomness(capdb_vfs*, int nByte, char *zOut);
static int cksmSleep(capdb_vfs*, int microseconds);
static int cksmCurrentTime(capdb_vfs*, double*);
static int cksmGetLastError(capdb_vfs*, int, char *);
static int cksmCurrentTimeInt64(capdb_vfs*, capdb_int64*);
static int cksmSetSystemCall(capdb_vfs*, const char*,capdb_syscall_ptr);
static capdb_syscall_ptr cksmGetSystemCall(capdb_vfs*, const char *z);
static const char *cksmNextSystemCall(capdb_vfs*, const char *zName);

static capdb_vfs cksm_vfs = {
...
static const capdb_io_methods cksm_io_methods = {
  3,                              /* iVersion */
  cksmClose,                      /* xClose */
  cksmRead,                       /* xRead */
  cksmWrite,                      /* xWrite */
...
static void cksmVerifyFunc(
  ...
  u8 *data;
  u8 cksum[8];
  data = (u8*)capdb_value_blob(argv[0]);
  if( data==0 ) return;
  if( capdb_value_type(argv[0])!=CAPDB_BLOB ) return;
  nByte = capdb_value_bytes(argv[0]);
  if( nByte<512 || nByte>65536 || (nByte & (nByte-1))!=0 ) return;
  cksmCompute(data, nByte-8, cksum);
  capdb_result_int(context, memcmp(data+nByte-8,cksum,8)==0);
}

#ifdef CAPDB_CKSUMVFS_INIT_FUNCNAME
/*
** SQL function:    initialize_cksumvfs(SCHEMANAME)
**
** This SQL functions (whose name is actually determined at compile-time
** by the value of the CAPDB_CKSUMVFS_INIT_FUNCNAME macro) invokes:
**
**   capdb_file_control(db, SCHEMANAME, CAPDB_FCNTL_RESERVE_BYTE, &n);
**
** In order to set the reserve bytes value to 8, so that cksumvfs will
** operation.  This feature is provided (if and only if the
** CAPDB_CKSUMVFS_INIT_FUNCNAME compile-time option is set to a string
...
** direct access to the capdb_file_control() interface (ex: Java).
**
** This interface is undocumented, apart from this comment.  Usage
** example:
**
**    1.  Compile with -DCAPDB_CKSUMVFS_INIT_FUNCNAME="ckvfs_init"
...
static void cksmInitFunc(
  ...
  int nByte = 8;
  const char *zSchemaName = (const char*)capdb_value_text(argv[0]);
  capdb *db = capdb_context_db_handle(context);
  capdb_file_control(db, zSchemaName, CAPDB_FCNTL_RESERVE_BYTES, &nByte);
  /* Return NULL */
}
#endif /* CAPDB_CKSUMBFS_INIT_FUNCNAME */

/*
** Close a cksm-file.
*/
static int cksmClose(capdb_file *pFile){
...
*/
static int cksmTruncate(capdb_file *pFile, sqlite_int64 size){
  pFile = ORIGFILE(pFile);
...
** Sync a cksm-file.
*/
static int cksmSync(capdb_file *pFile, int flags){
  pFile = ORIGFILE(pFile);
...
*/
static int cksmFileSize(capdb_file *pFile, sqlite_int64 *pSize){
  CksmFile *p = (CksmFile *)pFile;
  pFile = ORIGFILE(p);
...
** Lock a cksm-file.
*/
static int cksmLock(capdb_file *pFile, int eLock){
  pFile = ORIGFILE(pFile);
...
** Unlock a cksm-file.
*/
static int cksmUnlock(capdb_file *pFile, int eLock){
  pFile = ORIGFILE(pFile);
...
*/
static int cksmCheckReservedLock(capdb_file *pFile, int *pResOut){
  pFile = ORIGFILE(pFile);
...
*/
static int cksmFileControl(capdb_file *pFile, int op, void *pArg){
  int rc;
  CksmFile *p = (CksmFile*)pFile;
  pFile = ORIGFILE(pFile);
  if( op==CAPDB_FCNTL_PRAGMA ){
    ...
    assert( azArg[1]!=0 );
    if( capdb_stricmp(azArg[1],"checksum_verification")==0 ){
      char *zArg = azArg[2];
      if( zArg!=0 ){
        if( (zArg[0]>='1' && zArg[0]<='9')
         || capdb_strlike("enable%",zArg,0)==0
         || capdb_stricmp("yes",zArg)==0
         || capdb_stricmp("on",zArg)==0
        ){
      ...
      azArg[0] = capdb_mprintf("%d",p->verifyCksm);
      return CAPDB_OK;
    }else if( p->computeCksm && azArg[2]!=0
           && capdb_stricmp(azArg[1], "page_size")==0 ){
  ...
  if( rc==CAPDB_OK && op==CAPDB_FCNTL_VFSNAME ){
    *(char**)pArg = capdb_mprintf("cksm/%z", *(char**)pArg);
  }
  return rc;
}
...
*/
static int cksmSectorSize(capdb_file *pFile){
  pFile = ORIGFILE(pFile);
...
*/
static int cksmDeviceCharacteristics(capdb_file *pFile){
  int devchar = 0;
  pFile = ORIGFILE(pFile);
  devchar = pFile->pMethods->xDeviceCharacteristics(pFile);
  return (devchar & ~CAPDB_IOCAP_SUBPAGE_READ);
}
...
static int cksmShmLock(capdb_file *pFile, int offset, int n, int flags){
  pFile = ORIGFILE(pFile);
  return pFile->pMethods->xShmLock(pFile,offset,n,flags);
}

/* Memory barrier operation on shared memory */
static void cksmShmBarrier(capdb_file *pFile){
  pFile = ORIGFILE(pFile);
  pFile->pMethods->xShmBarrier(pFile);
}

/* Unmap a shared memory segment */
static int cksmShmUnmap(capdb_file *pFile, int deleteFlag){
  pFile = ORIGFILE(pFile);
...
static int cksmUnfetch(capdb_file *pFile, capdb_int64 iOfst, void *pPage){
  pFile = ORIGFILE(pFile);
...
*/
static int cksmDelete(capdb_vfs *pVfs, const char *zPath, int dirSync){
...
static void *cksmDlOpen(capdb_vfs *pVfs, const char *zPath){
  return ORIGVFS(pVfs)->xDlOpen(ORIGVFS(pVfs), zPath);
}
static void cksmDlError(capdb_vfs *pVfs, int nByte, char *zErrMsg){
  ORIGVFS(pVfs)->xDlError(ORIGVFS(pVfs), nByte, zErrMsg);
}
static void (*cksmDlSym(capdb_vfs *pVfs, void *p, const char *zSym))(void){
  return ORIGVFS(pVfs)->xDlSym(ORIGVFS(pVfs), p, zSym);
}
static void cksmDlClose(capdb_vfs *pVfs, void *pHandle){
  ORIGVFS(pVfs)->xDlClose(ORIGVFS(pVfs), pHandle);
}
static int cksmRandomness(capdb_vfs *pVfs, int nByte, char *zBufOut){
  return ORIGVFS(pVfs)->xRandomness(ORIGVFS(pVfs), nByte, zBufOut);
}
static int cksmSleep(capdb_vfs *pVfs, int nMicro){
  return ORIGVFS(pVfs)->xSleep(ORIGVFS(pVfs), nMicro);
}
static int cksmCurrentTime(capdb_vfs *pVfs, double *pTimeOut){
  return ORIGVFS(pVfs)->xCurrentTime(ORIGVFS(pVfs), pTimeOut);
}
static int cksmGetLastError(capdb_vfs *pVfs, int a, char *b){
  return ORIGVFS(pVfs)->xGetLastError(ORIGVFS(pVfs), a, b);
}
static int cksmCurrentTimeInt64(capdb_vfs *pVfs, capdb_int64 *p){
  capdb_vfs *pOrig = ORIGVFS(pVfs);
  int rc;
...
static int cksmSetSystemCall(
  capdb_vfs *pVfs,
  const char *zName,
  capdb_syscall_ptr pCall
){
  return ORIGVFS(pVfs)->xSetSystemCall(ORIGVFS(pVfs),zName,pCall);
}
static capdb_syscall_ptr cksmGetSystemCall(
  capdb_vfs *pVfs,
  const char *zName
){
  return ORIGVFS(pVfs)->xGetSystemCall(ORIGVFS(pVfs),zName);
}
static const char *cksmNextSystemCall(capdb_vfs *pVfs, const char *zName){
...
static int cksmRegisterFunc(
  ...
  int rc;
  if( db==0 ) return CAPDB_OK;
  rc = capdb_create_function(db, "verify_checksum", 1,
                   CAPDB_UTF8|CAPDB_INNOCUOUS|CAPDB_DETERMINISTIC,
                   0, cksmVerifyFunc, 0, 0);
#ifdef CAPDB_CKSUMVFS_INIT_FUNCNAME
  (void)capdb_create_function(db, CAPDB_CKSUMVFS_INIT_FUNCNAME, 1,
                   CAPDB_UTF8|CAPDB_DIRECTONLY,
                   0, cksmInitFunc, 0, 0);
#endif
  return rc;
}
...
static int cksmRegisterVfs(void){
  int rc = CAPDB_OK;
  capdb_vfs *pOrig = capdb_vfs_find(0);
  if( pOrig==0 ) return CAPDB_ERROR;
  cksm_vfs.iVersion = pOrig->iVersion;
  cksm_vfs.pAppData = pOrig;
  cksm_vfs.szOsFile = pOrig->szOsFile + sizeof(CksmFile);
  rc = capdb_vfs_register(&cksm_vfs, 1);
  if( rc==CAPDB_OK ){
    rc = capdb_auto_extension((void(*)(void))cksmRegisterFunc);
  }
  return rc;
}

#if defined(CAPDB_CKSUMVFS_STATIC)
/* This variant of the initializer runs when the extension is
** statically linked.
*/
int capdb_register_cksumvfs(const char *NotUsed){
  (void)NotUsed;
  return cksmRegisterVfs();
}
int capdb_unregister_cksumvfs(void){
  if( capdb_vfs_find("cksmvfs") ){
    capdb_vfs_unregister(&cksm_vfs);
    capdb_cancel_auto_extension((void(*)(void))cksmRegisterFunc);
  }
  return CAPDB_OK;
}
#endif /* defined(CAPDB_CKSUMVFS_STATIC */
...
#endif
/* 
** This routine is called by capdb_load_extension() when the
** extension is first loaded.
***/
int capdb_cksumvfs_init(
  capdb *db, 
  char **pzErrMsg, 
  const capdb_api_routines *pApi
){
  ...
  if( rc==CAPDB_OK ) rc = CAPDB_OK_LOAD_PERMANENTLY;
  return rc;
}
#endif /* !defined(CAPDB_CKSUMVFS_STATIC) */
