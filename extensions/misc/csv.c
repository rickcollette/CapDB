...
** if this module is compiled with -DCAPDB_TEST.
*/
#include <capdbext.h>
...
static void csv_errmsg(CsvReader *p, const char *zFormat, ...){
  va_list ap;
  va_start(ap, zFormat);
  capdb_vsnprintf(CSV_MXERR, p->zErr, zFormat, ap);
  va_end(ap);
}
...
static int csv_reader_open(
  ...
  if( zFilename ){
    p->zIn = capdb_malloc64( CSV_INBUFSZ );
    if( p->zIn==0 ){
...
** in this file. */
static int csvtabCreate(capdb*, void*, int, const char*const*, 
                           capdb_vtab**,char**);
static int csvtabConnect(capdb*, void*, int, const char*const*, 
                           capdb_vtab**,char**);
static int csvtabBestIndex(capdb_vtab*,capdb_index_info*);
static int csvtabDisconnect(capdb_vtab*);
static int csvtabOpen(capdb_vtab*, capdb_vtab_cursor**);
static int csvtabClose(capdb_vtab_cursor*);
static int csvtabFilter(capdb_vtab_cursor*, int idxNum, const char *idxStr,
                          int argc, capdb_value **argv);
static int csvtabNext(capdb_vtab_cursor*);
static int csvtabEof(capdb_vtab_cursor*);
static int csvtabColumn(capdb_vtab_cursor*,capdb_context*,int);
static int csvtabRowid(capdb_vtab_cursor*,capdb_int64*);
...
typedef struct CsvCursor {
  ...
  capdb_int64 iRowid;           /* The current rowid.  Negative for EOF */
} CsvCursor;

/* Transfer error message text from a reader into a CsvTable */
static void csv_xfer_error(CsvTable *pTab, CsvReader *pRdr){
  capdb_free(pTab->base.zErrMsg);
  pTab->base.zErrMsg = capdb_mprintf("%s", pRdr->zErr);
}

/*
** This method is the destructor fo a CsvTable object.
*/
static int csvtabDisconnect(capdb_vtab *pVtab){
  CsvTable *p = (CsvTable*)pVtab;
  capdb_free(p->zFilename);
  capdb_free(p->zData);
  capdb_free(p);
  return CAPDB_OK;
}
...
**
** Only available if compiled with CAPDB_TEST:
**    
...
static int csvtabConnect(
  ...
  int rc = CAPDB_OK;        /* Result code from this routine */
...
# define CSV_SCHEMA   (azPValue[2])
  ...
  *ppVtab = (capdb_vtab*)pNew;
  ...
  if( CSV_SCHEMA==0 ){
    ...
    char *zSep = "";
    int iCol = 0;
    capdb_str_appendf(pStr, "CREATE TABLE x(");
    if( nCol<0 && bHeader<1 ){
      nCol = 0;
      do{
    ...
    if( nCol>0 && bHeader<1 ){
      for(iCol=0; iCol<nCol; iCol++){
        capdb_str_appendf(pStr, "%sc%d TEXT", zSep, iCol);
        zSep = ",";
      }
    }else{
      do{
        ...
        if( (nCol>0 && iCol<nCol) || (nCol<0 && bHeader) ){
          capdb_str_appendf(pStr,"%s\"%w\" TEXT", zSep, z);
          zSep = ",";
          iCol++;
        }
      }while( sRdr.cTerm==',' );
    ...
    CSV_SCHEMA = capdb_str_finish(pStr);
  ...
  rc = capdb_declare_vtab(db, CSV_SCHEMA);
  if( rc ){
    csv_errmsg(&sRdr, "bad schema: '%s' - %s", CSV_SCHEMA, capdb_errmsg(db));
    goto csvtab_connect_error;
  }
  for(i=0; i<sizeof(azPValue)/sizeof(azPValue[0]); i++){
    capdb_free(azPValue[i]);
  }
...
csvtab_connect_error:
  ...
  if( sRdr.zErr[0] ){
    capdb_free(*pzErr);
    *pzErr = capdb_mprintf("%s", sRdr.zErr);
  }
  csv_reader_reset(&sRdr);
...
*/
static int csvtabClose(capdb_vtab_cursor *cur){
...
*/
static int csvtabOpen(capdb_vtab *p, capdb_vtab_cursor **ppCursor){
  CsvTable *pTab = (CsvTable*)p;
  CsvCursor *pCur;
  size_t nByte;
  nByte = sizeof(*pCur) + (sizeof(char*)+sizeof(i64))*pTab->nCol;
  pCur = capdb_malloc64( nByte );
...
*/
static int csvtabNext(capdb_vtab_cursor *cur){
  CsvCursor *pCur = (CsvCursor*)cur;
  ...
  do{
    ...
    if( i<pTab->nCol ){
      if( pCur->aLen[i] < pCur->rdr.n+1 ){
        char *zNew = capdb_realloc64(pCur->azVal[i], pCur->rdr.n+1);
        if( zNew==0 ){
...
static int csvtabColumn(
  ...
  if( i>=0 && i<pTab->nCol && pCur->azVal[i]!=0 ){
    capdb_result_text(ctx, pCur->azVal[i], -1, CAPDB_TRANSIENT);
  }
  return CAPDB_OK;
}
...
*/
static int csvtabRowid(capdb_vtab_cursor *cur, sqlite_int64 *pRowid){
  CsvCursor *pCur = (CsvCursor*)cur;
  *pRowid = pCur->iRowid;
  return CAPDB_OK;
}
...
** row of output.
*/
static int csvtabEof(capdb_vtab_cursor *cur){
  CsvCursor *pCur = (CsvCursor*)cur;
...
static int csvtabFilter(
  ...
  ** error in csvtabNext(). */
  if( csv_append(&pCur->rdr, 0) ) return CAPDB_NOMEM;
  ...
  if( (((CsvTable*)tab)->tstFlags & CSVTEST_FIDX)!=0 ){
    ...
    */
    ...
    for(i=0; i<pIdxInfo->nConstraint; i++){
      ...
      if( op==CAPDB_INDEX_CONSTRAINT_EQ 
       || op==CAPDB_INDEX_CONSTRAINT_LIKE
       || op==CAPDB_INDEX_CONSTRAINT_GLOB
      ){
...
** CAPDB_READONLY since the CSV file is not really writable.
*/
static int csvtabUpdate(capdb_vtab *p,int n,capdb_value**v,capdb_int64*x){
  return CAPDB_READONLY;
}
static capdb_module CsvModuleFauxWrite = {
  0,                       /* iVersion */
  csvtabCreate,            /* xCreate */
  csvtabConnect,           /* xConnect */
  csvtabBestIndex,         /* xBestIndex */
...
int capdb_csv_init(
  ...
  rc = capdb_create_module(db, "csv", &CsvModule, 0);
#ifdef CAPDB_TEST
  if( rc==CAPDB_OK ){
    rc = capdb_create_module(db, "csv_wr", &CsvModuleFauxWrite, 0);
  }
#endif
  return rc;
#else
