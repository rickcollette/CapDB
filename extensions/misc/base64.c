...
static void base64(capdb_context *context, int na, capdb_value *av[]){
  capdb_int64 nb;
  capdb_int64 nv = capdb_value_bytes(av[0]);
  capdb_int64 nc;
  ...
  case CAPDB_BLOB:
    ...
    if( nvMax < nc ){
      capdb_result_error(context, "blob expanded to base64 too big", -1);
      return;
    }
  ...
  case CAPDB_TEXT:
    ...
    if( nvMax < nb ){
      capdb_result_error(context, "blob from base64 may be too big", -1);
      return;
    }else if( nb<1 ){
      nb = 1;
    }
    ...
    break;
  default:
    capdb_result_error(context, "base64 accepts only blob or text", -1);
    return;
  }
  return;
 memFail:
  capdb_result_error(context, "base64 OOM", -1);
}
...
#endif
int capdb_base64_init
#else
static int capdb_base64_init
#endif
...
*/
#define BASE64_INIT(db) capdb_base64_init(db, 0, 0)
