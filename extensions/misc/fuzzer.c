...
/* If CAPDB_DEBUG is not defined, disable assert statements. */
...
static int fuzzerLoadOneRule(
  ...
  if( iRuleset<0 || iRuleset>FUZZER_MX_RULEID ){
    *pzErr = capdb_mprintf("%s: ruleset must be between 0 and %d", 
...
static int fuzzerLoadRules(
  ...
  zSql = capdb_mprintf("SELECT * FROM %Q.%Q", zDb, zData);
  if( zSql==0 ){
    rc = CAPDB_NOMEM;
  }else{
    ...
    if( rc!=CAPDB_OK ){
      ...
      *pzErr = capdb_mprintf("%s: %s has %d columns, expected 4",
          p->zClassName, zData, capdb_column_count(pStmt)
      );
      rc = CAPDB_ERROR;
    }else{
...
*/
static int fuzzerDisconnect(capdb_vtab *pVtab){
...
static int fuzzerConnect(
  ...
  if( argc!=4 ){
    *pzErr = capdb_mprintf(
    ...
    capdb_int64 nModule;        /* Length of zModule, in bytes */

    nModule = strlen(zModule);
    pNew = capdb_malloc64( sizeof(*pNew) + nModule + 1);
    if( pNew==0 ){
      rc = CAPDB_NOMEM;
    }else{
      ...
      if( rc==CAPDB_OK ){
        rc = capdb_declare_vtab(db, "CREATE TABLE x(word,distance,ruleset)");
      }
      if( rc!=CAPDB_OK ){
        fuzzerDisconnect((capdb_vtab *)pNew);
        pNew = 0;
      }else{
...
*/
static int fuzzerOpen(capdb_vtab *pVTab, capdb_vtab_cursor **ppCursor){
  fuzzer_vtab *p = (fuzzer_vtab*)pVTab;
  fuzzer_cursor *pCur;
...
*/
static int fuzzerClose(capdb_vtab_cursor *cur){
...
static int fuzzerRender(
  ...
  capdb_int64 n;                /* Size of output term without nul-term */
  char *z;                        /* Buffer to assemble output term in */

  n = pStem->nBasis + pRule->nTo - pRule->nFrom;
  if( (*pnBuf)<n+1 ){
    (*pzBuf) = capdb_realloc64((*pzBuf), n+100);
    if( (*pzBuf)==0 ) return CAPDB_NOMEM;
    (*pnBuf) = n+100;
  }
  n = pStem->n;
  z = *pzBuf;
...
static void fuzzerStemPrint(
  ...
  if( pStem->n<0 ){
    ...
    char *zBuf = 0;
    int nBuf = 0;
    if( fuzzerRender(pStem, &zBuf, &nBuf)!=CAPDB_OK ) return;
...
static int fuzzerSeen(fuzzer_cursor *pCur, fuzzer_stem *pStem){
  ...
  if( fuzzerRender(pStem, &pCur->zBuf, &pCur->nBuf)==CAPDB_NOMEM ){
    return -1;
  }
...
static fuzzer_stem *fuzzerNewStem(
  ...
  pNew = capdb_malloc64( sizeof(*pNew) + strlen(zWord) + 1 );
  if( pNew==0 ) return 0;
  memset(pNew, 0, sizeof(*pNew));
...
*/
static int fuzzerNext(capdb_vtab_cursor *cur){
  fuzzer_cursor *pCur = (fuzzer_cursor*)cur;
  int rc;
  ...
  while( (pStem = pCur->pStem)!=0 ){
    ...
    if( res<0 ){
      ...
      if( (rc = fuzzerSeen(pCur, pStem))!=0 ){
        if( rc<0 ) return CAPDB_NOMEM;
        continue;
      }
      return CAPDB_OK;  /* New word found */
    }
    pCur->pStem = 0;
...
static int fuzzerFilter(
  ...
  if( idxNum & 2 ){
    pCur->rLimit = (fuzzer_cost)capdb_value_int(argv[idx]);
    idx++;
  }
  if( idxNum & 4 ){
    pCur->iRuleset = (fuzzer_cost)capdb_value_int(argv[idx]);
    idx++;
  }
...
** return NULL
*/
static int fuzzerColumn(capdb_vtab_cursor *cur, capdb_context *ctx, int i){
  fuzzer_cursor *pCur = (fuzzer_cursor*)cur;
  if( i==0 ){
    /* the "word" column */
    if( fuzzerRender(pCur->pStem, &pCur->zBuf, &pCur->nBuf)==CAPDB_NOMEM ){
      return CAPDB_NOMEM;
    }
    capdb_result_text(ctx, pCur->zBuf, -1, CAPDB_TRANSIENT);
  }else if( i==1 ){
    /* the "distance" column */
    capdb_result_int(ctx, pCur->pStem->rCostX);
  }else{
...
** The rowid.
*/
static int fuzzerRowid(capdb_vtab_cursor *cur, sqlite_int64 *pRowid){
  fuzzer_cursor *pCur = (fuzzer_cursor*)cur;
  *pRowid = pCur->iRowid;
  return CAPDB_OK;
}
...
*/
static int fuzzerEof(capdb_vtab_cursor *cur){
  fuzzer_cursor *pCur = (fuzzer_cursor*)cur;
...
*/
static int fuzzerBestIndex(capdb_vtab *tab, capdb_index_info *pIdxInfo){
  int iPlan = 0;
  int iDistTerm = -1;
  int iRulesetTerm = -1;
  int i;
...
*/
static capdb_module fuzzerModule = {
  0,                           /* iVersion */
  fuzzerConnect,
  fuzzerConnect,
  fuzzerBestIndex,
...
#ifndef CAPDB_OMIT_VIRTUALTABLE
  rc = capdb_create_module(db, "fuzzer", &fuzzerModule, 0);
#endif
  return rc;
}
