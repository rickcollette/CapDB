...
*     NAME=fossildelta
**
** First run "make capdbext.h" then:
**
...
static int delta_create(
  ...
  */
  nHash = lenSrc/NHASH;
  collide = capdb_malloc64( (capdb_int64)nHash*2*sizeof(int) );
...
static void deltaCreateFunc(
  ...
  nNew = capdb_value_bytes(argv[1]);
  aNew = (const char*)capdb_value_blob(argv[1]);
  aOut = capdb_malloc64(nNew+70);
  if( aOut==0 ){
    ...
    if( nOut<0 ){
      capdb_free(aOut);
      capdb_result_error(context, "cannot create fossil delta", -1);
    }else{
...
static void deltaApplyFunc(
  ...
  aOut = capdb_malloc64((capdb_int64)nOut+1);
  if( aOut==0 ){
    capdb_result_error_nomem(context);
  }else{
...
static void deltaOutputSizeFunc(
  ...
  assert( argc==1 );
  if( capdb_value_type(argv[0])==CAPDB_NULL ) return;
  nDelta = capdb_value_bytes(argv[0]);
  aDelta = (const char*)capdb_value_blob(argv[0]);
...
*/
static int deltaparsevtabDisconnect(capdb_vtab *pVtab){
...
*/
static int deltaparsevtabOpen(capdb_vtab *p, capdb_vtab_cursor **ppCursor){
...
*/
static int deltaparsevtabClose(capdb_vtab_cursor *cur){
  deltaparsevtab_cursor *pCur = (deltaparsevtab_cursor*)cur;
  capdb_free(pCur->aDelta);
  capdb_free(pCur);
  return CAPDB_OK;
}
...
*/
static int deltaparsevtabNext(capdb_vtab_cursor *cur){
  deltaparsevtab_cursor *pCur = (deltaparsevtab_cursor*)cur;
  const char *z;
  int i = 0;
...
static int deltaparsevtabColumn(
  ...
  switch( i ){
    case DELTAPARSEVTAB_OP: {
      capdb_result_text(ctx, azOp[pCur->eOp], -1, CAPDB_STATIC);
      break;
    }
    case DELTAPARSEVTAB_A1: {
      capdb_result_int(ctx, pCur->a1);
      break;
    }
    case DELTAPARSEVTAB_A2: {
      if( pCur->eOp==DELTAPARSE_OP_COPY ){
        capdb_result_int(ctx, pCur->a2);
      }else if( pCur->eOp==DELTAPARSE_OP_INSERT ){
        if( (i64)pCur->a2 + (i64)pCur->a1 > pCur->nDelta ){
          capdb_result_zeroblob(ctx, pCur->a1);
        }else{
          capdb_result_blob(ctx, pCur->aDelta+pCur->a2, pCur->a1,
                              CAPDB_TRANSIENT);
        }
      }
      break;
    ...
    case DELTAPARSEVTAB_DELTA: {
      capdb_result_blob(ctx, pCur->aDelta, pCur->nDelta, CAPDB_TRANSIENT);
      break;
    }
  }
  return CAPDB_OK;
...
*/
static int deltaparsevtabRowid(capdb_vtab_cursor *cur, sqlite_int64 *pRowid){
  deltaparsevtab_cursor *pCur = (deltaparsevtab_cursor*)cur;
  *pRowid = pCur->iCursor;
  return CAPDB_OK;
}
...
** row of output.
*/
static int deltaparsevtabEof(capdb_vtab_cursor *cur){
  deltaparsevtab_cursor *pCur = (deltaparsevtab_cursor*)cur;
...
static int deltaparsevtabFilter(
  ...
  pCur->nDelta = capdb_value_bytes(argv[0]);
  a = (const char*)capdb_value_blob(argv[0]);
  if( pCur->nDelta==0 || a==0 ){
    return CAPDB_OK;
  }
  pCur->aDelta = capdb_malloc64( pCur->nDelta+1 );
  if( pCur->aDelta==0 ){
    pCur->nDelta = 0;
    return CAPDB_NOMEM;
  }
...
** virtual table.
*/
static capdb_module deltaparsevtabModule = {
  /* iVersion    */ 0,
  /* xCreate     */ 0,
...
#endif
int capdb_fossildelta_init(
  capdb *db,
  char **pzErrMsg,
  const capdb_api_routines *pApi
){
  static const int enc = CAPDB_UTF8|CAPDB_INNOCUOUS;
  int rc = CAPDB_OK;
  CAPDB_EXTENSION_INIT2(pApi);
  (void)pzErrMsg;  /* Unused parameter */
  rc = capdb_create_function(db, "delta_create", 2, enc, 0,
                               deltaCreateFunc, 0, 0);
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "delta_apply", 2, enc, 0,
                                 deltaApplyFunc, 0, 0);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "delta_output_size", 1, enc, 0,
                                 deltaOutputSizeFunc, 0, 0);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_create_module(db, "delta_parse", &deltaparsevtabModule, 0);
  }
  return rc;
}
