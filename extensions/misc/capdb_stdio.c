...
/*
** If the CAPDB_U8TEXT_ONLY option is defined, then use O_U8TEXT
** when appropriate on all output.  (Sometimes use O_BINARY when
** rendering ASCII text in cases where NL-to-CRLF expansion would
** not be correct.)
**
** If the CAPDB_U8TEXT_STDIO option is defined, then use O_U8TEXT
...
**
** The CAPDB_USE_W32_FOR_CONSOLE_IO macro is also available.  If
...
FILE *capdb_fopen(const char *zFilename, const char *zMode){
  ...
  b1 = capdb_malloc64( (sz1+1)*sizeof(b1[0]) );
  b2 = capdb_malloc64( (sz2+1)*sizeof(b1[0]) );
  if( b1 && b2 ){
...
*/
FILE *capdb_popen(const char *zCommand, const char *zMode){
  FILE *fp = 0;
  wchar_t *b1, *b2;
  int sz1, sz2;
...
*/
char *capdb_fgets(char *buf, int sz, FILE *in){
  if( UseWtextForInput(in) ){
    ...
    ** into '?'.
    */
    wchar_t *b1 = capdb_malloc64( sz*sizeof(wchar_t) );
    if( b1==0 ) return 0;
#ifdef CAPDB_USE_W32_FOR_CONSOLE_IO
    DWORD nRead = 0;
    if( IsConsole(in)
...
*/
int capdb_fputs(const char *z, FILE *out){
  if( !UseWtextForOutput(out) ){
    ...
    */
    int sz = (int)strlen(z);
    wchar_t *b1 = capdb_malloc64( (sz+1)*sizeof(wchar_t) );
    if( b1==0 ) return 0;
...
#endif
    {
      /* As long as CAPDB_USE_W32_FOR_CONSOLE_IO is not defined, or for
...
*/
int capdb_fprintf(FILE *out, const char *zFormat, ...){
  int rc;
  if( UseWtextForOutput(out) ){
...
int capdb_vfprintf(FILE *out, const char *zFormat, va_list ap){
  int rc;
  if( UseWtextForOutput(out) ){
    /* When writing to the command-prompt in Windows, it is necessary
    ** to use _O_WTEXT input mode and write UTF-16 characters.
...
** _O_TEXT.
*/
void capdb_fsetmode(FILE *fp, int mode){
