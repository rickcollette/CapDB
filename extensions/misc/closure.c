...
*/
struct closure_avl {
  capdb_int64 id;     /* Id of this entry in the table */
...
*/
static closure_avl *closureAvlSearch(closure_avl *p, capdb_int64 id){
  while( p && id!=p->id ){
...
static void closureFree(closure_vtab *p){
  if( p ){
    ...
    capdb_free(p->zIdColumn);
    capdb_free(p->zParentColumn);
    memset(p, 0, sizeof(*p));
    capdb_free(p);
  }
}
...
*/
static int closureDisconnect(capdb_vtab *pVtab){
...
*/
static int closureOpen(capdb_vtab *pVTab, capdb_vtab_cursor **ppCursor){
  closure_vtab *p = (closure_vtab*)pVTab;
  closure_cursor *pCur;
...
/*
** Wrapper around capdb_free
*/
static void closureMemFree(closure_avl *p){ capdb_free(p); }
...
static void closureClearCursor(closure_cursor *pCur){
  ...
  capdb_free(pCur->zTableName);
  capdb_free(pCur->zIdColumn);
  capdb_free(pCur->zParentColumn);
  pCur->zTableName = 0;
  pCur->zIdColumn = 0;
...
*/
static int closureClose(capdb_vtab_cursor *cur){
...
*/
static int closureNext(capdb_vtab_cursor *cur){
  closure_cursor *pCur = (closure_cursor*)cur;
...
static int closureInsertNode(
  ...
  capdb_int64 id,       /* The node ID */
  int iGeneration         /* The generation number for this node */
){
  closure_avl *pNew = capdb_malloc64( sizeof(*pNew) );
  if( pNew==0 ) return CAPDB_NOMEM;
  memset(pNew, 0, sizeof(*pNew));
  pNew->id = id;
...
static int closureFilter(
  ...
  iRoot = capdb_value_int64(argv[0]);
  if( (idxNum & 0x000f0)!=0 ){
    mxGen = capdb_value_int(argv[(idxNum>>4)&0x0f]);
    if( (idxNum & 0x00002)!=0 ) mxGen--;
  }
  if( (idxNum & 0x00f00)!=0 ){
    zTableName = (const char*)capdb_value_text(argv[(idxNum>>8)&0x0f]);
    pCur->zTableName = capdb_mprintf("%s", zTableName);
  }
  if( (idxNum & 0x0f000)!=0 ){
    zIdColumn = (const char*)capdb_value_text(argv[(idxNum>>12)&0x0f]);
    pCur->zIdColumn = capdb_mprintf("%s", zIdColumn);
  }
  if( (idxNum & 0x0f0000)!=0 ){
    zParentColumn = (const char*)capdb_value_text(argv[(idxNum>>16)&0x0f]);
    pCur->zParentColumn = capdb_mprintf("%s", zParentColumn);
  }
  ...
  if( zSql==0 ){
    ...
    rc = capdb_prepare_v2(pVtab->db, zSql, -1, &pStmt, 0);
    capdb_free(zSql);
    if( rc ){
      capdb_free(pVtab->base.zErrMsg);
      pVtab->base.zErrMsg = capdb_mprintf("%s", capdb_errmsg(pVtab->db));
      return rc;
    }
  }
  if( rc==CAPDB_OK ){
  ...
  while( (pAvl = queuePull(&sQueue))!=0 ){
    ...
    capdb_bind_int64(pStmt, 1, pAvl->id);
    while( rc==CAPDB_OK && capdb_step(pStmt)==CAPDB_ROW ){
      if( capdb_column_type(pStmt,0)==CAPDB_INTEGER ){
        capdb_int64 iNew = capdb_column_int64(pStmt, 0);
...
** return NULL
*/
static int closureColumn(capdb_vtab_cursor *cur, capdb_context *ctx, int i){
  closure_cursor *pCur = (closure_cursor*)cur;
  switch( i ){
    case CLOSURE_COL_ID: {
      capdb_result_int64(ctx, pCur->pCurrent->id);
      break;
    }
    case CLOSURE_COL_DEPTH: {
      capdb_result_int(ctx, pCur->pCurrent->iGeneration);
      break;
    }
...
*/
static int closureRowid(capdb_vtab_cursor *cur, sqlite_int64 *pRowid){
  closure_cursor *pCur = (closure_cursor*)cur;
...
** EOF indicator
*/
static int closureEof(capdb_vtab_cursor *cur){
  closure_cursor *pCur = (closure_cursor*)cur;
...
static int closureBestIndex(
  capdb_vtab *pTab,             /* The virtual table */
  capdb_index_info *pIdxInfo    /* Information about the query */
){
  int iPlan = 0;
  int i;
  int idx = 1;
  ...
  for(i=0; i<pIdxInfo->nConstraint; i++, pConstraint++){
    ...
    if( (iPlan & 0x0000f0)==0
     ...
     && (pConstraint->op==CAPDB_INDEX_CONSTRAINT_LT
      ...
      if( pConstraint->op==CAPDB_INDEX_CONSTRAINT_LT ) iPlan |= 0x000002;
      rCost /= 5.0;
    }
    if( (iPlan & 0x000f00)==0
...
*/
static capdb_module closureModule = {
  0,                      /* iVersion */
...
int capdb_closure_init(
  ...
  (void)pzErrMsg;
#ifndef CAPDB_OMIT_VIRTUALTABLE
  rc = capdb_create_module(db, "transitive_closure", &closureModule, 0);
#endif /* CAPDB_OMIT_VIRTUALTABLE */
  return rc;
}
