...
**
** Space to hold the result is obtained from capdb_malloc()
**
** Return NULL if memory allocation fails.  
*/
static unsigned char *phoneticHash(const unsigned char *zIn, int nIn){
  unsigned char *zOut = capdb_malloc64( (i64)nIn + 1 );
  int i;
  int nOut = 0;
  char cPrev = 0x77;
  char cPrevX = 0x77;
...
static void phoneticHashSqlFunc(
  ...
  if( zIn==0 ) return;
  zOut = phoneticHash(zIn, capdb_value_bytes(argv[0]));
  if( zOut==0 ){
    capdb_result_error_nomem(context);
  }else{
    capdb_result_text(context, (char*)zOut, -1, capdb_free);
  }
}
...
static int editdist1(const char *zA, const char *zB, int *pnMatch){
  ...
  if( nB<(sizeof(mStack)*4)/(sizeof(mStack[0])*5) ){
    ...
    m = toFree = capdb_malloc64( ((i64)nB+1)*5LL*sizeof(m[0])/4 );
    if( m==0 ) return -3;
  }
  cx = (char*)&m[nB+1];
...
static void editdistSqlFunc(
  ...
  int res = editdist1(
                    (const char*)capdb_value_text(argv[0]),
                    (const char*)capdb_value_text(argv[1]),
                    0);
  if( res<0 ){
    if( res==(-3) ){
      capdb_result_error_nomem(context);
      ...
      capdb_result_error(context, "non-ASCII input to editdist()", -1);
    }else{
      capdb_result_error(context, "NULL input to editdist()", -1);
    }
  }else{ 
    capdb_result_int(context, res);
  }
}
...
static int editDist3ConfigLoad(
  ...
  capdb *db,            /* Load from this database */
  ...
  zSql = capdb_mprintf("SELECT iLang, cFrom, cTo, iCost"
  ...
  while( capdb_step(pStmt)==CAPDB_ROW ){
    int iLang = capdb_column_int(pStmt, 0);
    const char *zFrom = (const char*)capdb_column_text(pStmt, 1);
    int nFrom = zFrom ? capdb_column_bytes(pStmt, 1) : 0;
    const char *zTo = (const char*)capdb_column_text(pStmt, 2);
    int nTo = zTo ? capdb_column_bytes(pStmt, 2) : 0;
    int iCost = capdb_column_int(pStmt, 3);
    ...
    if( pLang==0 || iLang!=iLangPrev ){
      EditDist3Lang *pNew;
      pNew = capdb_realloc64(p->a, ((i64)p->nLang+1)*sizeof(p->a[0]));
      if( pNew==0 ){ rc = CAPDB_NOMEM; break; }
      p->a = pNew;
      pLang = &p->a[p->nLang];
      p->nLang++;
      pLang->iLang = iLang;
    ...
    if( nFrom==1 && zFrom[0]=='?' && nTo==0 ){
      ...
      pCost = capdb_malloc64( sizeof(*pCost) + nExtra );
      if( pCost==0 ){ rc = CAPDB_NOMEM; break; }
...
static void editDist3FromStringDelete(EditDist3FromString *p){
  ...
  if( p ){
    for(i=0; i<p->n; i++){
      capdb_free(p->a[i].apDel);
      capdb_free(p->a[i].apSubst);
    }
    capdb_free(p);
  }
}
...
static EditDist3FromString *editDist3FromStringNew(
  ...
  pStr = capdb_malloc64( sizeof(*pStr) + sizeof(pStr->a[0])*n + (i64)n + 1 );
  if( pStr==0 ) return 0;
  ...
  for(i=0; i<n; i++){
    ...
    for(p=pLang->pCost; p; p=p->pNext){
      ...
      if( p->nTo==0 ){
        apNew = capdb_realloc64(pFrom->apDel,
                                sizeof(*apNew)*((i64)pFrom->nDel+1));
        if( apNew==0 ) break;
        pFrom->apDel = apNew;
        apNew[pFrom->nDel++] = p;
      }else{
        apNew = capdb_realloc64(pFrom->apSubst,
...
** architecture in use.
*/
#ifndef CAPDB_SPELLFIX_STACKALLOC_SZ
# define CAPDB_SPELLFIX_STACKALLOC_SZ  (1024)
#endif
...
static int editDist3Core(
  ...
  int res;
  capdb_uint64 nByte;
  unsigned int stackSpace[CAPDB_SPELLFIX_STACKALLOC_SZ/sizeof(unsigned int)];
  ...
  if( nByte<=sizeof(stackSpace) ){
    ...
    m = pToFree = capdb_malloc64( nByte );
  ...
  for(i2=0; i2<n2; i2++){
    ...
    for(p=pLang->pCost; p; p=p->pNext){
      ...
      a2[i2].nIns++;
      apNew = capdb_realloc64(a2[i2].apIns, sizeof(*apNew)*a2[i2].nIns);
      if( apNew==0 ){
        res = -1;  /* Out of memory */
        goto editDist3Abort;
      }
...
editDist3Abort:
  for(i2=0; i2<n2; i2++) capdb_free(a2[i2].apIns);
  capdb_free(pToFree);
  return res;
}
...
static void editDist3SqlFunc(
  ...
  EditDist3Config *pConfig = (EditDist3Config*)capdb_user_data(context);
  capdb *db = capdb_context_db_handle(context);
  int rc;
  if( argc==1 ){
    const char *zTable = (const char*)capdb_value_text(argv[0]);
    rc = editDist3ConfigLoad(pConfig, db, zTable);
    if( rc ) capdb_result_error_code(context, rc);
  }else{
    const char *zA = (const char*)capdb_value_text(argv[0]);
    const char *zB = (const char*)capdb_value_text(argv[1]);
    int nA = capdb_value_bytes(argv[0]);
    int nB = capdb_value_bytes(argv[1]);
    int iLang = argc==3 ? capdb_value_int(argv[2]) : 0;
...
*/
static int editDist3Install(capdb *db){
  int rc;
  EditDist3Config *pConfig = capdb_malloc64( sizeof(*pConfig) );
  if( pConfig==0 ) return CAPDB_NOMEM;
  memset(pConfig, 0, sizeof(*pConfig));
  rc = capdb_create_function_v2(db, "editdist3",
              2, CAPDB_UTF8|CAPDB_DETERMINISTIC, pConfig,
              editDist3SqlFunc, 0, 0, 0);
  if( rc==CAPDB_OK ){
    rc = capdb_create_function_v2(db, "editdist3",
                3, CAPDB_UTF8|CAPDB_DETERMINISTIC, pConfig,
                editDist3SqlFunc, 0, 0, 0);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_create_function_v2(db, "editdist3",
                1, CAPDB_UTF8|CAPDB_DETERMINISTIC, pConfig,
...
#if !CAPDB_AMALGAMATION
/*
** This lookup table is used to help decode the first byte of
** a multi-byte UTF8 character.
*/
static const unsigned char capdbUtf8Trans1[] = {
...
#ifdef CAPDB_SPELLFIX_5BYTE_MAPPINGS
 unsigned char cTo4;
#endif
};
...
**
** Space to hold the returned string comes from capdb_malloc() and
** should be freed by the caller.
*/
static unsigned char *transliterate(const unsigned char *zIn, int nIn){
#ifdef CAPDB_SPELLFIX_5BYTE_MAPPINGS
  unsigned char *zOut = capdb_malloc64( (i64)nIn*5 + 1 );
#else
  unsigned char *zOut = capdb_malloc64( (i64)nIn*4 + 1 );
#endif
  int c, sz, nOut;
  if( zOut==0 ) return 0;
  nOut = 0;
...
#endif /* CAPDB_SPELLFIX_5BYTE_MAPPINGS */
              }
            }
          }
          c = 0;
...
static void transliterateSqlFunc(
  ...
  const unsigned char *zIn = capdb_value_text(argv[0]);
  int nIn = capdb_value_bytes(argv[0]);
...
struct spellfix1_cursor {
  ...
  capdb_stmt *pFullScan;     /* Shadow query for a full table scan */
  struct spellfix1_row {       /* For each row of content */
    capdb_int64 iRowid;         /* Rowid for this row */
...
static void spellfix1DbExec(
  ...
  capdb *db,           /* Database in which to run SQL */
...
*/
static int spellfix1Uninit(int isDestroy, capdb_vtab *pVTab){
  ...
  if( rc==CAPDB_OK ){
    ...
    capdb_free(p->zCostTable);
    capdb_free(p);
  }
  return rc;
}
static int spellfix1Disconnect(capdb_vtab *pVTab){
  return spellfix1Uninit(0, pVTab);
}
static int spellfix1Destroy(capdb_vtab *pVTab){
...
static int spellfix1Init(
  ...
  pNew = capdb_malloc64( sizeof(*pNew) + (i64)nDbName + 1);
  if( pNew==0 ){
    rc = CAPDB_NOMEM;
  }else{
    ...
    pNew->zTableName = capdb_mprintf("%s", zTableName);
    pNew->db = db;
...
#define SPELLFIX_COL_COMMAND        11
    ...
    for(i=3; rc==CAPDB_OK && i<argc; i++){
      if( strncmp(argv[i],"edit_cost_table=",16)==0 && pNew->zCostTable==0 ){
        pNew->zCostTable = spellfix1Dequote(&argv[i][16]);
        if( pNew->zCostTable==0 ) rc = CAPDB_NOMEM;
        continue;
      }
      *pzErr = capdb_mprintf("bad argument to spellfix1(): \"%s\"", argv[i]);
      rc = CAPDB_ERROR; 
    }
  }
...
static void spellfix1ResetCursor(spellfix1_cursor *pCur){
  ...
  for(i=0; i<pCur->nRow; i++){
    capdb_free(pCur->a[i].zWord);
  }
  pCur->nRow = 0;
  pCur->iRow = 0;
  pCur->nSearch = 0;
  if( pCur->pFullScan ){
    capdb_finalize(pCur->pFullScan);
    pCur->pFullScan = 0;
  }
}
...
static void spellfix1ResizeCursor(spellfix1_cursor *pCur, int N){
  ...
  assert( N>=pCur->nRow );
  aNew = capdb_realloc64(pCur->a, sizeof(pCur->a[0])*N);
  if( aNew==0 && N>0 ){
...
*/
static int spellfix1Close(capdb_vtab_cursor *cur){
  spellfix1_cursor *pCur = (spellfix1_cursor *)cur;
  spellfix1ResetCursor(pCur);
  spellfix1ResizeCursor(pCur, 0);
  capdb_free(pCur->zPattern);
  capdb_free(pCur);
  return CAPDB_OK;
}
...
*/
static int spellfix1BestIndex(capdb_vtab *tab, capdb_index_info *pIdxInfo){
  int iPlan = 0;
  int iLangTerm = -1;
  int iTopTerm = -1;
  int iScopeTerm = -1;
  ...
  for(i=0; i<pIdxInfo->nConstraint; i++, pConstraint++){
    ...
    if( (iPlan & SPELLFIX_IDXNUM_MATCH)==0 
     ...
     && pConstraint->op==CAPDB_INDEX_CONSTRAINT_MATCH
    ){
    ...
    if( (iPlan & SPELLFIX_IDXNUM_LANGID)==0
     ...
     && pConstraint->op==CAPDB_INDEX_CONSTRAINT_EQ
    ){
    ...
    if( (iPlan & SPELLFIX_IDXNUM_DIST)==0
     ...
     && (pConstraint->op==CAPDB_INDEX_CONSTRAINT_LT
          || pConstraint->op==CAPDB_INDEX_CONSTRAINT_LE)
    ){
      if( pConstraint->op==CAPDB_INDEX_CONSTRAINT_LT ){
...
*/
static int spellfix1Open(capdb_vtab *pVTab, capdb_vtab_cursor **ppCursor){
  spellfix1_vtab *p = (spellfix1_vtab*)pVTab;
...
*/
static int CAPDB_CDECL spellfix1RowCompare(const void *A, const void *B){
...
typedef struct MatchQuery {
  ...
  capdb_stmt *pStmt;             /* shadow table query statment */
...
static void spellfix1RunQuery(MatchQuery *p, const char *zQuery, int nQuery){
  ...
  if( capdb_bind_text(pStmt, 1, zHash1, -1, CAPDB_STATIC)==CAPDB_NOMEM
   || capdb_bind_text(pStmt, 2, zHash2, -1, CAPDB_STATIC)==CAPDB_NOMEM
  ){
    p->rc = CAPDB_NOMEM;
    return;
  }
  ...
  while( capdb_step(pStmt)==CAPDB_ROW ){
    int iMatchlen = -1;
    iRank = capdb_column_int(pStmt, 2);
    if( p->pMatchStr3 ){
      int nWord = capdb_column_bytes(pStmt, 1);
      zWord = (const char*)capdb_column_text(pStmt, 1);
      iDist = editDist3Core(p->pMatchStr3, zWord, nWord, p->pLang, &iMatchlen);
    }else{
      zK1 = (const char*)capdb_column_text(pStmt, 3);
      if( zK1==0 ) continue;
      iDist = editdist1(p->zPattern, zK1, 0);
    }
    if( iDist<0 ){
      p->rc = iDist==(-4) ? CAPDB_TOOBIG : CAPDB_NOMEM;
      break;
    }
    pCur->nSearch++;
    ...
    ** query, grow the array to ensure there is room for the new entry. */
    ...
    if( pCur->nRow<pCur->nAlloc ){
      ...
      idx = idxWorst;
      capdb_free(pCur->a[idx].zWord);
    }else{
      continue;
    }

    pCur->a[idx].zWord = capdb_mprintf("%s", capdb_column_text(pStmt, 1));
    if( pCur->a[idx].zWord==0 ){
      p->rc = CAPDB_NOMEM;
      break;
    }
    pCur->a[idx].iRowid = capdb_column_int64(pStmt, 0);
...
static int spellfix1FilterForMatch(
  ...
  capdb_stmt *pStmt = 0;           /* Shadow table query */
  int rc;                            /* Result code */
  ...
  if( p->zCostTable!=0 && p->pConfig3==0 ){
    p->pConfig3 = capdb_malloc64( sizeof(p->pConfig3[0]) );
    if( p->pConfig3==0 ) return CAPDB_NOMEM;
  ...
  if( idxNum&2 ){
    iLang = capdb_value_int(argv[idx++]);
  }
  if( idxNum&4 ){
    iLimit = capdb_value_int(argv[idx++]);
    if( iLimit<1 ) iLimit = 1;
  }
  if( idxNum&8 ){
    x.iScope = capdb_value_int(argv[idx++]);
    if( x.iScope<1 ) x.iScope = 1;
    if( x.iScope>SPELLFIX_MX_HASH-2 ) x.iScope = SPELLFIX_MX_HASH-2;
  }
  if( idxNum&(16|32) ){
    x.iMaxDist = capdb_value_int(argv[idx++]);
    if( idxNum&16 ) x.iMaxDist--;
    if( x.iMaxDist<0 ) x.iMaxDist = 0;
  }
  spellfix1ResetCursor(pCur);
  spellfix1ResizeCursor(pCur, iLimit);
  zMatchThis = capdb_value_text(argv[0]);
  if( zMatchThis==0 ) return CAPDB_OK;
  if( p->pConfig3 ){
  ...
  zPattern = (char*)transliterate(zMatchThis, capdb_value_bytes(argv[0]));
  capdb_free(pCur->zPattern);
...
static int spellfix1FilterForFullScan(
  ...
  if( zSql==0 ) return CAPDB_NOMEM;
  rc = capdb_prepare_v2(pVTab->db, zSql, -1, &pCur->pFullScan, 0);
  capdb_free(zSql);
  if( rc==CAPDB_OK && (idxNum & 64) ){
    assert( argc==1 );
    rc = capdb_bind_value(pCur->pFullScan, 1, argv[0]);
  }
  pCur->nRow = pCur->iRow = 0;
  if( rc==CAPDB_OK ){
    rc = capdb_step(pCur->pFullScan);
    if( rc==CAPDB_ROW ){ pCur->iRow = -1; rc = CAPDB_OK; }
...
*/
static int spellfix1Next(capdb_vtab_cursor *cur){
  spellfix1_cursor *pCur = (spellfix1_cursor *)cur;
  int rc = CAPDB_OK;
  if( pCur->iRow < pCur->nRow ){
    if( pCur->pFullScan ){
      rc = capdb_step(pCur->pFullScan);
      if( rc!=CAPDB_ROW ) pCur->iRow = pCur->nRow;
...
*/
static int spellfix1Eof(capdb_vtab_cursor *cur){
  spellfix1_cursor *pCur = (spellfix1_cursor *)cur;
...
static int spellfix1Column(
  ...
  if( pCur->pFullScan ){
    if( i<=SPELLFIX_COL_LANGID ){
      capdb_result_value(ctx, capdb_column_value(pCur->pFullScan, i));
    }else{
  ...
  switch( i ){
    case SPELLFIX_COL_WORD: {
      capdb_result_text(ctx, pCur->a[pCur->iRow].zWord, -1, CAPDB_STATIC);
      break;
    }
    case SPELLFIX_COL_RANK: {
      capdb_result_int(ctx, pCur->a[pCur->iRow].iRank);
      break;
    }
    case SPELLFIX_COL_DISTANCE: {
      capdb_result_int(ctx, pCur->a[pCur->iRow].iDistance);
      break;
    }
    case SPELLFIX_COL_LANGID: {
      capdb_result_int(ctx, pCur->iLang);
      break;
    }
    case SPELLFIX_COL_SCORE: {
      capdb_result_int(ctx, pCur->a[pCur->iRow].iScore);
      break;
    }
    case SPELLFIX_COL_MATCHLEN: {
      ...
      if( iMatchlen<0 ){
        ...
        if( nPattern>0 && pCur->zPattern[nPattern-1]=='*' ){
          ...
          if( !zTranslit ) return CAPDB_NOMEM;
          res = editdist1(pCur->zPattern, zTranslit, &iMatchlen);
          capdb_free(zTranslit);
          if( res<0 ) return res==(-4) ? CAPDB_TOOBIG : CAPDB_NOMEM;
      ...
      capdb_result_int(ctx, iMatchlen);
      break;
    }
    case SPELLFIX_COL_PHONEHASH: {
      capdb_result_text(ctx, pCur->a[pCur->iRow].zHash, -1, CAPDB_STATIC);
      break;
    }
    case SPELLFIX_COL_TOP: {
      capdb_result_int(ctx, pCur->iTop);
      break;
    }
    case SPELLFIX_COL_SCOPE: {
      capdb_result_int(ctx, pCur->iScope);
      break;
    }
    case SPELLFIX_COL_SRCHCNT: {
      capdb_result_int(ctx, pCur->nSearch);
      break;
    }
    default: {
      capdb_result_null(ctx);
...
** The rowid.
*/
static int spellfix1Rowid(capdb_vtab_cursor *cur, sqlite_int64 *pRowid){
  spellfix1_cursor *pCur = (spellfix1_cursor*)cur;
  if( pCur->pFullScan ){
    *pRowid = capdb_column_int64(pCur->pFullScan, 4);
  }else{
...
*/
static const char *spellfix1GetConflict(capdb *db){
  static const char *azConflict[] = {
    /* Note: Instead of "FAIL" - "ABORT". */
    "ROLLBACK", "IGNORE", "ABORT", "ABORT", "REPLACE"
  };
  int eConflict = capdb_vtab_on_conflict(db);

  assert( eConflict==CAPDB_ROLLBACK || eConflict==CAPDB_IGNORE
       || eConflict==CAPDB_FAIL || eConflict==CAPDB_ABORT
       || eConflict==CAPDB_REPLACE
  );
  assert( CAPDB_ROLLBACK==1 );
  assert( CAPDB_IGNORE==2 );
  assert( CAPDB_FAIL==3 );
  assert( CAPDB_ABORT==4 );
  assert( CAPDB_REPLACE==5 );
...
static int spellfix1Update(
  ...
  int rc = CAPDB_OK;
  capdb_int64 rowid, newRowid;
  spellfix1_vtab *p = (spellfix1_vtab*)pVTab;
  capdb *db = p->db;

  if( argc==1 ){
    /* A delete operation on the rowid given by argv[0] */
    rowid = *pRowid = capdb_value_int64(argv[0]);
    spellfix1DbExec(&rc, db, "DELETE FROM \"%w\".\"%w_vocab\" "
                           " WHERE id=%lld",
                  p->zDbName, p->zTableName, rowid);
  }else{
    const unsigned char *zWord = capdb_value_text(argv[SPELLFIX_COL_WORD+2]);
    int nWord = capdb_value_bytes(argv[SPELLFIX_COL_WORD+2]);
    int iLang = capdb_value_int(argv[SPELLFIX_COL_LANGID+2]);
    int iRank = capdb_value_int(argv[SPELLFIX_COL_RANK+2]);
    const unsigned char *zSoundslike =
           capdb_value_text(argv[SPELLFIX_COL_SOUNDSLIKE+2]);
    int nSoundslike = capdb_value_bytes(argv[SPELLFIX_COL_SOUNDSLIKE+2]);
    char *zK1, *zK2;
    int i;
    char c;
    ...
    if( zWord==0 ){
      ...
      ** what special actions to take */
      const char *zCmd = 
         (const char*)capdb_value_text(argv[SPELLFIX_COL_COMMAND+2]);
      if( zCmd==0 ){
        pVTab->zErrMsg = capdb_mprintf("NOT NULL constraint failed: %s.word",
                                         p->zTableName);
        return CAPDB_CONSTRAINT_NOTNULL;
      }
      ...
      if( strncmp(zCmd,"edit_cost_table=",16)==0 ){
        ...
        if( p->zCostTable==0 ) return CAPDB_NOMEM;
        if( p->zCostTable[0]==0 || capdb_stricmp(p->zCostTable,"null")==0 ){
          capdb_free(p->zCostTable);
          p->zCostTable = 0;
        }
        return CAPDB_OK;
      }
      pVTab->zErrMsg = capdb_mprintf("unknown value for %s.command: \"%w\"",
                                       p->zTableName, zCmd);
      return CAPDB_ERROR;
    }
    if( iRank<1 ) iRank = 1;
    ...
    if( capdb_value_type(argv[0])==CAPDB_NULL ){
      if( capdb_value_type(argv[1])==CAPDB_NULL ){
        spellfix1DbExec(&rc, db,
        ...
        newRowid = capdb_value_int64(argv[1]);
        spellfix1DbExec(&rc, db,
      ...
      *pRowid = capdb_last_insert_rowid(db);
      ...
      rowid = capdb_value_int64(argv[0]);
      newRowid = *pRowid = capdb_value_int64(argv[1]);
      spellfix1DbExec(&rc, db,
...
*/
static int spellfix1Rename(capdb_vtab *pVTab, const char *zNew){
  spellfix1_vtab *p = (spellfix1_vtab*)pVTab;
  capdb *db = p->db;
  int rc = CAPDB_OK;
  char *zNewName = capdb_mprintf("%s", zNew);
  if( zNewName==0 ){
    return CAPDB_NOMEM;
  }
  spellfix1DbExec(&rc, db, 
...
*/
static capdb_module spellfix1Module = {
  0,                       /* iVersion */
...
*/
static int spellfix1Register(capdb *db){
  int rc = CAPDB_OK;
  unsigned int i;
  rc = capdb_create_function(db, "spellfix1_translit", 1,
                               CAPDB_UTF8|CAPDB_DETERMINISTIC, 0,
                                transliterateSqlFunc, 0, 0);
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "spellfix1_editdist", 2,
                                 CAPDB_UTF8|CAPDB_DETERMINISTIC, 0,
                                  editdistSqlFunc, 0, 0);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "spellfix1_phonehash", 1,
                                 CAPDB_UTF8|CAPDB_DETERMINISTIC, 0,
                                  phoneticHashSqlFunc, 0, 0);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "spellfix1_scriptcode", 1,
                                  CAPDB_UTF8|CAPDB_DETERMINISTIC, 0,
                                  scriptCodeSqlFunc, 0, 0);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_create_module(db, "spellfix1", &spellfix1Module, 0);
  }
  if( rc==CAPDB_OK ){
