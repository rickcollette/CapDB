...
static void rot13func(
  ...
  zIn = (const unsigned char*)capdb_value_text(argv[0]);
  nIn = capdb_value_bytes(argv[0]);
  if( nIn<sizeof(zTemp)-1 ){
    zOut = zTemp;
  }else{
    zOut = zToFree = (unsigned char*)capdb_malloc64( nIn+1 );
    if( zOut==0 ){
  ...
  zOut[i] = 0;
  capdb_result_text(context, (char*)zOut, i, CAPDB_TRANSIENT);
  capdb_free(zToFree);
}
...
int capdb_rot_init(
  ...
  rc = capdb_create_function(db, "rot13", 1,
                   CAPDB_UTF8|CAPDB_INNOCUOUS|CAPDB_DETERMINISTIC,
                   0, rot13func, 0, 0);
  if( rc==CAPDB_OK ){
    rc = capdb_create_collation(db, "rot13", CAPDB_UTF8, 0, rot13CollFunc);
  }
  return rc;
}
