...
static void ieee754func(
  ...
  if( argc==1 ){
    ...
    if( capdb_value_type(argv[0])==CAPDB_BLOB
     && capdb_value_bytes(argv[0])==sizeof(r)
    ){
      const unsigned char *x = capdb_value_blob(argv[0]);
      unsigned int i;
      capdb_uint64 v = 0;
    ...
    if( a==0 ){
      e = 0;
      m = 0;
    }else if( a==(capdb_int64)0x8000000000000000LL ){
      e = -1996;
      m = -1;
    }else{
      e = a>>52;
    ...
    switch( *(int*)capdb_user_data(context) ){
      case 0:
        capdb_snprintf(sizeof(zResult), zResult, "ieee754(%lld,%d)",
                         m, e-1075);
        capdb_result_text(context, zResult, -1, CAPDB_TRANSIENT);
        break;
      case 1:
        capdb_result_int64(context, m);
        break;
      case 2:
        capdb_result_int(context, e-1075);
        break;
    }
  }else{
    capdb_int64 m, e, a;
    double r;
    int isNeg = 0;
    m = capdb_value_int64(argv[0]);
    e = capdb_value_int64(argv[1]);
    ...
    if( m<0 ){
      ...
      capdb_result_double(context, 0.0);
      return;
    }
    ...
    a = m & ((((capdb_int64)1)<<52)-1);
    a |= e<<52;
    if( isNeg ) a |= ((capdb_uint64)1)<<63;
    memcpy(&r, &a, sizeof(r));
...
static void ieee754func_from_blob(
  ...
  if( capdb_value_type(argv[0])==CAPDB_BLOB
   && capdb_value_bytes(argv[0])==sizeof(double)
  ){
    double r;
    const unsigned char *x = capdb_value_blob(argv[0]);
    unsigned int i;
...
static void ieee754func_to_blob(
  ...
  UNUSED_PARAMETER(argc);
  if( capdb_value_type(argv[0])==CAPDB_FLOAT
   || capdb_value_type(argv[0])==CAPDB_INTEGER
  ){
    ...
    capdb_result_blob(context, a, sizeof(r), CAPDB_TRANSIENT);
  }
}
...
static void ieee754inc(
  ...
  m2 = m1 + N;
  memcpy(&r2, &m2, 8);
  capdb_result_double(context, r2);
}
...
int capdb_ieee_init(
  ...
  for(i=0; i<sizeof(aFunc)/sizeof(aFunc[0]) && rc==CAPDB_OK; i++){
    rc = capdb_create_function(db, aFunc[i].zFName, aFunc[i].nArg,
