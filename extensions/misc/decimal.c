...
#ifndef CAPDB_DECIMAL_MAX_DIGIT
# define CAPDB_DECIMAL_MAX_DIGIT 10000000
#endif
...
static Decimal *decimalNewFromText(const char *zIn, int n){
  ...
  if( iExp>0 ){
    ...
    if( iExp>0 ){   
      signed char *a = capdb_realloc64(p->a, (capdb_int64)p->nDigit
                                     + (capdb_int64)iExp + 1 );
  ...
  if( p->nDigit>CAPDB_DECIMAL_MAX_DIGIT ) goto new_from_text_failed;
  return p;

new_from_text_failed:
  if( p ){
    if( p->a ) capdb_free(p->a);
    capdb_free(p);
  }
  return 0;
}
...
/*
** Allocate a new Decimal object from an capdb_value.  Return a pointer
** to the new object, or NULL if there is an error.  If the pCtx argument
** is not NULL, then errors are reported on it as well.
**
** If the pIn argument is CAPDB_TEXT or CAPDB_INTEGER, it is converted
** directly into a Decimal.  For CAPDB_FLOAT or for CAPDB_BLOB of length
...
static Decimal *decimal_new(
  capdb_context *pCtx,       /* Report error here, if not null */
  capdb_value *pIn,          /* Construct the decimal object from this */
  int bTextOnly                /* Always interpret pIn as text if true */
){
  Decimal *p = 0;
  int eType = capdb_value_type(pIn);
  if( bTextOnly && (eType==CAPDB_FLOAT || eType==CAPDB_BLOB) ){
    eType = CAPDB_TEXT;
  }
  switch( eType ){
    case CAPDB_TEXT:
    case CAPDB_INTEGER: {
      const char *zIn = (const char*)capdb_value_text(pIn);
    ...
    case CAPDB_FLOAT: {
      p = decimalFromDouble(capdb_value_double(pIn));
      break;
    }
    ...
    case CAPDB_BLOB: {
      ...
      if( capdb_value_bytes(pIn)!=sizeof(r) ) break;
      x = capdb_value_blob(pIn);
...
*/
static void decimal_result(capdb_context *pCtx, Decimal *p){
  char *z;
  int i, j;
  int n;
  if( p==0 || p->oom ){
  ...
  z = capdb_malloc64( (capdb_int64)p->nDigit+8 );
  if( z==0 ){
    capdb_result_error_nomem(pCtx);
    return;
  }
  ...
  z[i] = 0;
  capdb_result_text(pCtx, z, i, capdb_free);
}
...
** zeros omitted.
*/
static void decimal_result_sci(capdb_context *pCtx, Decimal *p, int N){
  ...
  nDigit -= nZero;
  z = capdb_malloc64( (capdb_int64)nDigit+20 );
  if( z==0 ){
    capdb_result_error_nomem(pCtx);
    return;
  }
  ...
  exp = nDigit - nFrac - 1;
  capdb_snprintf(nDigit+20-i, &z[i], "e%+03d", exp);
  capdb_result_text(pCtx, z, -1, capdb_free);
}
...
static void decimal_expand(Decimal *p, int nDigit, int nFrac){
  ...
  if( nDigit+1>CAPDB_DECIMAL_MAX_DIGIT ){ p->oom = 1; return; }
  a = capdb_realloc64(p->a, nDigit+1);
  if( a==0 ){
    p->oom = 1;
    return;
  }
...
static void decimalMul(Decimal *pA, Decimal *pB){
  ...
  sumDigit += pB->nDigit;
  sumDigit += 2;
  if( sumDigit>CAPDB_DECIMAL_MAX_DIGIT ){ pA->oom = 1; return; }
  acc = capdb_malloc64( sumDigit );
  if( acc==0 ){
    pA->oom = 1;
    goto mul_end;
  }
...
static Decimal *decimalFromDouble(double r){
  ...
  memcpy(&a,&r,sizeof(a));
  if( a==0 || a==(capdb_int64)0x8000000000000000LL){
    e = 0;
    m = 0;
  }else{
    e = a>>52;
    m = a & ((((capdb_int64)1)<<52)-1);
    if( e==0 ){
      m <<= 1;
    }else{
      m |= ((capdb_int64)1)<<52;
  ...
  capdb_snprintf(sizeof(zNum), zNum, "%lld", m);
...
static void decimalFunc(
  ...
  int N;
  if( argc==2 ){
    N = capdb_value_int(argv[1]);
    if( N>0 ) decimal_round(p, N);
  }else{
    N = 0;
  }
  if( p ){
    if( capdb_user_data(context)!=0 ){
...
static void decimalSumValue(capdb_context *context){
  Decimal *p = capdb_aggregate_context(context, 0);
  if( p==0 ) return;
  decimal_result(context, p);
}
static void decimalSumFinalize(capdb_context *context){
  Decimal *p = capdb_aggregate_context(context, 0);
  if( p==0 ) return;
  decimal_result(context, p);
  decimal_clear(p);
...
static void decimalPow2Func(
  ...
  if( capdb_value_type(argv[0])==CAPDB_INTEGER ){
    Decimal *pA = decimalPow2(capdb_value_int(argv[0]));
...
int capdb_decimal_init(
  ...
  for(i=0; i<(int)(sizeof(aFunc)/sizeof(aFunc[0])) && rc==CAPDB_OK; i++){
  ...
  if( rc==CAPDB_OK ){
    rc = capdb_create_window_function(db, "decimal_sum", 1,
                   CAPDB_UTF8|CAPDB_INNOCUOUS|CAPDB_DETERMINISTIC, 0,
                   decimalSumStep, decimalSumFinalize,
                   decimalSumValue, decimalSumInverse, 0);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_create_collation(db, "decimal", CAPDB_UTF8,
                                  0, decimalCollFunc);
  }
  return rc;
}
