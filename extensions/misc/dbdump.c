...
**
** The capdb_db_dump() subroutine returns CAPDB_OK on success or some error
...
struct DText {
  char *z;              /* The text */
  capdb_int64 n;      /* Number of bytes of content in z[] */
  capdb_int64 nAlloc; /* Number of bytes allocated to z[] */
};
...
static void appendText(DText *p, char const *zAppend, char quote){
  ...
  if( p->n+len>=p->nAlloc ){
    ...
    zNew = capdb_realloc64(p->z, p->nAlloc);
    if( zNew==0 ){
      freeText(p);
      return;
    }
...
static char **tableColumnList(DState *p, const char *zTab){
  ...
  char *zSql;
  capdb_int64 nCol = 0;
  capdb_int64 nAlloc = 0;
  int nPK = 0;       /* Number of PRIMARY KEY columns seen */
  int isIPK = 0;     /* True if one PRIMARY KEY column of type INTEGER */
  int preserveRowid = 1;
  int rc;

  zSql = capdb_mprintf("PRAGMA table_info=%Q", zTab);
  if( zSql==0 ) return 0;
  ...
  while( capdb_step(pStmt)==CAPDB_ROW ){
    if( nCol>=nAlloc-2 ){
      ...
      azNew = capdb_realloc64(azCol, nAlloc*sizeof(azCol[0]));
      if( azNew==0 ) goto col_oom;
      azCol = azNew;
      azCol[0] = 0;
    }
    azCol[++nCol] = capdb_mprintf("%s", capdb_column_text(pStmt, 1));
    if( azCol[nCol]==0 ) goto col_oom;
    if( capdb_column_int(pStmt, 5) ){
      nPK++;
      if( nPK==1
       && capdb_stricmp((const char*)capdb_column_text(pStmt,2),
                          "INTEGER")==0 
      ){
        isIPK = 1;
      }else{
  ...
  */
  if( isIPK ){
    ...
    */
    zSql = capdb_mprintf("SELECT 1 FROM pragma_index_list(%Q)"
  ...
  if( preserveRowid ){
    ...
    ** rowid */
    ...
    for(j=0; j<3; j++){
      for(i=1; i<=nCol; i++){
        if( capdb_stricmp(azRowid[j],azCol[i])==0 ) break;
      }
      if( i>nCol ){
        ...
        ** tables will fail this last check */
        rc = capdb_table_column_metadata(p->db,0,zTab,azRowid[j],0,0,0,0,0);
        if( rc==CAPDB_OK ) azCol[0] = azRowid[j];
        break;
      }
    }
  }
...
/*
** This is an capdb_exec callback routine used for dumping the database.
...
static int dump_callback(void *pArg, int nArg, char **azArg, char **azCol){
  ...
  if( strcmp(zTable, "sqlite_sequence")==0 ){
  ...
  }else if( capdb_strglob("sqlite_stat?", zTable)==0 ){
    ...
    if( capdb_strglob("CREATE TABLE ['\"]*", zSql)==0 ){
  ...
  if( strcmp(zType, "table")==0 ){
    ...
    */
    ...
    rc = capdb_prepare_v2(p->db, sSelect.z, -1, &pStmt, 0);
    if( rc!=CAPDB_OK ){
      p->nErr++;
      ...
      while( CAPDB_ROW==capdb_step(pStmt) ){
        ...
        for(i=0; i<nCol; i++){
          ...
          switch( capdb_column_type(pStmt,i) ){
            case CAPDB_INTEGER: {
              output_formatted(p, "%lld", capdb_column_int64(pStmt,i));
              break;
            }
            case CAPDB_FLOAT: {
              double r = capdb_column_double(pStmt,i);
              capdb_uint64 ur;
            ...
            case CAPDB_TEXT: {
              output_quoted_escaped_string(p, 
                   (const char*)capdb_column_text(pStmt,i));
              break;
            }
            case CAPDB_BLOB: {
              int nByte = capdb_column_bytes(pStmt,i);
              unsigned char *a = (unsigned char*)capdb_column_blob(pStmt,i);
              int j;
...
static void output_sql_from_query(
  ...
  va_list ap;
  va_start(ap, zSelect);
  zSql = capdb_vmprintf(zSelect, ap);
  va_end(ap);
  if( zSql==0 ){
    p->rc = CAPDB_NOMEM;
    p->nErr++;
  ...
  rc = capdb_prepare_v2(p->db, zSql, -1, &pSelect, 0);
  capdb_free(zSql);
  if( rc!=CAPDB_OK || !pSelect ){
  ...
  nResult = capdb_column_count(pSelect);
  while( rc==CAPDB_ROW ){
    z = (const char*)capdb_column_text(pSelect, 0);
    p->xCallback(z, p->pArg);
    for(i=1; i<nResult; i++){
      p->xCallback(",", p->pArg);
      p->xCallback((const char*)capdb_column_text(pSelect,i), p->pArg);
    }
    if( z==0 ) z = "";
    while( z[0] && (z[0]!='-' || z[1]!='-') ) z++;
    if( z[0] ){
  ...
  rc = capdb_finalize(pSelect);
  if( rc!=CAPDB_OK ){
    output_formatted(p, "/**** ERROR: (%d) %s *****/\n", rc,
                     capdb_errmsg(p->db));
    if( (rc&0xff)!=CAPDB_CORRUPT ) p->nErr++;
  }
}
...
**
** If we get a CAPDB_CORRUPT error, rerun the query after appending
...
static void run_schema_dump_query(
  ...
  va_list ap;
  va_start(ap, zQuery);
  z = capdb_vmprintf(zQuery, ap);
  va_end(ap); 
  capdb_exec(p->db, z, dump_callback, p, &zErr);
  capdb_free(z);
  if( zErr ){
...
int capdb_db_dump(
  ...
  DState x;
  memset(&x, 0, sizeof(x));
  x.rc = capdb_exec(db, "BEGIN", 0, 0, 0);
  if( x.rc ) return x.rc;
  x.db = db;
...
int main(int argc, char **argv){
  ...
  rc = capdb_db_dump(db, zSchema, zTable, 
          (int(*)(const char*,void*))fputs, (void*)stdout);
  if( rc ){
    fprintf(stderr, "Error: capdb_db_dump() returns %d\n", rc);
  }
  capdb_close(db);
  return rc!=CAPDB_OK;  
}
