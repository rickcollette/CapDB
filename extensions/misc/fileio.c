...
static wchar_t *winUtf8To16(const char *zText){
  ...
  zWideText = capdb_malloc64(nChar*sizeof(WCHAR) );
  if( zWideText==0 ){
    return 0;
  }
...
** CAPDB_TOOBIG error.
**
** Throw an CAPDB_IOERR if there are difficulties pulling the file
** off of disk.
*/
static void readFileContents(capdb_context *ctx, const char *zName){
  FILE *in;
  capdb_int64 nIn;
  void *pBuf;
  capdb *db;
  ...
  mxBlob = capdb_limit(db, CAPDB_LIMIT_LENGTH, -1);
  if( nIn>mxBlob ){
    capdb_result_error_code(ctx, CAPDB_TOOBIG);
    fclose(in);
    return;
  }
  pBuf = capdb_malloc64( nIn ? nIn : 1 );
  if( pBuf==0 ){
    capdb_result_error_nomem(ctx);
    fclose(in);
    return;
  }
  if( nIn==(capdb_int64)fread(pBuf, 1, (size_t)nIn, in) ){
    capdb_result_blob64(ctx, pBuf, nIn, capdb_free);
  }else{
    capdb_result_error_code(ctx, CAPDB_IOERR);
    capdb_free(pBuf);
  }
  fclose(in);
}
...
** vprintf(zFmt, ...).
*/
static void ctxErrorMsg(capdb_context *ctx, const char *zFmt, ...){
  char *zMsg = 0;
  va_list ap;
  va_start(ap, zFmt);
...
*/
static capdb_uint64 fileTimeToUnixTime(
...
**
** If an OOM condition is encountered, CAPDB_NOMEM is returned. Otherwise,
** CAPDB_OK is returned if the directory is successfully created, or
** CAPDB_ERROR otherwise.
*/
static int makeDirectory(
  const char *zFile
){
  char *zCopy = capdb_mprintf("%s", zFile);
  int rc = CAPDB_OK;
  ...
  if( zCopy==0 ){
    ...
    while( rc==CAPDB_OK ){
      ...
      if( rc2!=0 ){
        if( mkdir(zCopy, 0777) ) rc = CAPDB_ERROR;
      }else{
        if( !S_ISDIR(sStat.st_mode) ) rc = CAPDB_ERROR;
      }
      zCopy[i] = '/';
      i++;
    }
...
static int writeFile(
  capdb_context *pCtx,          /* Context to return bytes written in */
  const char *zFile,              /* File to write */
  capdb_value *pData,           /* Data to write */
  mode_t mode,                    /* MODE parameter passed to writefile() */
  capdb_int64 mtime             /* MTIME parameter (or -1 to not set time) */
){
  if( zFile==0 ) return 1;
#if !defined(_WIN32) && !defined(WIN32)
  if( S_ISLNK(mode) ){
    const char *zTo = (const char*)capdb_value_text(pData);
    if( zTo==0 ) return 1;
    unlink(zFile);
...
#endif
  {
    if( S_ISDIR(mode) ){
      ...
      capdb_int64 nWrite = 0;
      const char *z;
      int rc = 0;
      FILE *out = capdb_fopen(zFile, "wb");
      if( out==0 ) return 1;
      z = (const char*)capdb_value_blob(pData);
      if( z ){
        capdb_int64 n = fwrite(z, 1, capdb_value_bytes(pData), out);
        nWrite = capdb_value_bytes(pData);
        if( nWrite!=n ){
          rc = 1;
        }
      }
      ...
      if( rc ) return 2;
      capdb_result_int64(pCtx, nWrite);
    }
  }
...
static void writefileFunc(
  ...
  if( zFile==0 ) return;
  if( argc>=3 ){
    mode = (mode_t)capdb_value_int(argv[2]);
  }
  if( argc==4 ){
    mtime = capdb_value_int64(argv[3]);
  }

  res = writeFile(context, zFile, argv[1], mode, mtime);
  if( res==1 && errno==ENOENT ){
    if( makeDirectory(zFile)==CAPDB_OK ){
      res = writeFile(context, zFile, argv[1], mode, mtime);
    }
  }
...
static void lsModeFunc(
  ...
  int i;
  int iMode = capdb_value_int(argv[0]);
  char z[16];
  (void)argc;
  if( S_ISLNK(iMode) ){
    z[0] = 'l';
  ...
  z[10] = '\0';
  capdb_result_text(context, z, -1, CAPDB_TRANSIENT);
}
...
static int fsdirConnect(
  ...
  (void)argv;
  (void)pzErr;
  rc = capdb_declare_vtab(db, "CREATE TABLE x" FSDIR_SCHEMA);
  if( rc==CAPDB_OK ){
    pNew = (fsdir_tab*)capdb_malloc64( sizeof(*pNew) );
...
*/
static int fsdirOpen(capdb_vtab *p, capdb_vtab_cursor **ppCursor){
  fsdir_cursor *pCur;
  (void)p;
...
static void fsdirResetCursor(fsdir_cursor *pCur){
  ...
  for(i=0; i<=pCur->iLvl; i++){
    ...
    capdb_free(pLvl->zDir);
  }
  capdb_free(pCur->zPath);
  capdb_free(pCur->aLvl);
  pCur->aLvl = 0;
  pCur->zPath = 0;
  pCur->zBase = 0;
  pCur->nBase = 0;
...
static void fsdirSetErrmsg(fsdir_cursor *pCur, const char *zFmt, ...){
  va_list ap;
  va_start(ap, zFmt);
  pCur->base.pVtab->zErrMsg = capdb_vmprintf(zFmt, ap);
  va_end(ap);
}
...
static int fsdirNext(capdb_vtab_cursor *cur){
  ...
  if( S_ISDIR(m) && pCur->iLvl+3<pCur->mxLvl ){
    ...
    if( iNew>=pCur->nLvl ){
      int nNew = iNew+1;
      capdb_int64 nByte = nNew*sizeof(FsdirLevel);
      FsdirLevel *aNew = (FsdirLevel*)capdb_realloc64(pCur->aLvl, nByte);
  ...
  while( pCur->iLvl>=0 ){
    ...
    if( pEntry ){
      ...
      capdb_free(pCur->zPath);
      pCur->zPath = capdb_mprintf("%s/%s", pLvl->zDir, pEntry->d_name);
      if( pCur->zPath==0 ) return CAPDB_NOMEM;
...
static int fsdirColumn(
  ...
  switch( i ){
    case FSDIR_COLUMN_NAME: {
      capdb_result_text(ctx, &pCur->zPath[pCur->nBase], -1, CAPDB_TRANSIENT);
      break;
    }

    case FSDIR_COLUMN_MODE:
      capdb_result_int64(ctx, pCur->sStat.st_mode);
      break;

    case FSDIR_COLUMN_MTIME:
      capdb_result_int64(ctx, pCur->sStat.st_mtime);
      break;
    ...
    case FSDIR_COLUMN_DATA: {
      ...
      if( S_ISDIR(m) ){
        ...
        while( 1 ){
          ...
          if( n<nBuf ) break;
          if( aBuf!=aStatic ) capdb_free(aBuf);
          nBuf = nBuf*2;
          aBuf = capdb_malloc64(nBuf);
          if( aBuf==0 ){
            capdb_result_error_nomem(ctx);
            return CAPDB_NOMEM;
          }
        ...
        capdb_result_text(ctx, aBuf, n, CAPDB_TRANSIENT);
        if( aBuf!=aStatic ) capdb_free(aBuf);
#endif
      }else{
    ...
    case FSDIR_COLUMN_LEVEL:
      capdb_result_int(ctx, pCur->iLvl+2);
      break;
...
static int fsdirFilter(
  ...
  if( (idxNum & 0x02)!=0 ){
    assert( argc>i );
    pCur->zBase = (const char*)capdb_value_text(argv[i++]);
  }
  if( (idxNum & 0x0c)!=0 ){
    assert( argc>i );
    pCur->mxLvl = capdb_value_int(argv[i++]);
  ...
  if( pCur->zBase ){
    ...
    pCur->zPath = capdb_mprintf("%s/%s", pCur->zBase, zDir);
  }else{
    pCur->zPath = capdb_mprintf("%s", zDir);
  }
...
static int fsdirBestIndex(
  ...
  for(i=0; i<pIdxInfo->nConstraint; i++, pConstraint++){
    if( pConstraint->op==CAPDB_INDEX_CONSTRAINT_EQ ){
    ...
    if( pConstraint->iColumn==FSDIR_COLUMN_LEVEL
     ...
     && idxLevel<0
      ...
      if( pConstraint->op==CAPDB_INDEX_CONSTRAINT_LE ){
        idxLevel = i;
        idxLevelEQ = 0x08;
        omitLevel = 1;
      }else if( pConstraint->op==CAPDB_INDEX_CONSTRAINT_LT ){
        idxLevel = i;
        idxLevelEQ = 0x04;
        omitLevel = 1;
      }
...
*/
static int fsdirRegister(capdb *db){
  ...
  int rc = capdb_create_module(db, "fsdir", &fsdirModule, 0);
  return rc;
}
#else         /* CAPDB_OMIT_VIRTUALTABLE */
# define fsdirRegister(x) CAPDB_OK
#endif

/*
** This version of realpath() works on any system.  The string
** returned is held in memory allocated using capdb_malloc64().
** The caller is responsible for calling capdb_free().
*/
...
** an CAPDB_NOMEM error if an OOM is encountered.
*/
static void realpathFunc(
  ...
  (void)argc;
  zPath = (const char*)capdb_value_text(argv[0]);
  if( zPath==0 ) return;
  if( zPath[0]==0 ) zPath = ".";
  zCopy = capdb_mprintf("%s",zPath);
  if( zCopy==0 ) return;
  len = strlen(zCopy);
  ...
  while( 1 /*exit-by-break*/ ){
    ...
    if( zOut ){
      if( cSep ){
        zOut = capdb_mprintf("%z%s",zOut,&zCopy[len]);
      }
      break;
    }else{
      size_t i = len-1;
      ...
      if( i<=0 ){
        if( zCopy[0]=='/' ){
          ...
          zOut = capdb_mprintf("%z/%s", zOut, zCopy);
        }          
        break;
      }
      cSep = zCopy[i];
  ...
  if( zOut ){
    ...
    ** Windows will have already done this simplification for us. */
    ...
    /* Return the result */
    capdb_result_text(context, zOut, -1, capdb_free);
  }
}
...
int capdb_fileio_init(
  ...
  rc = capdb_create_function(db, "readfile", 1, 
                               CAPDB_UTF8|CAPDB_DIRECTONLY, 0,
                               readfileFunc, 0, 0);
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "writefile", -1,
                                 CAPDB_UTF8|CAPDB_DIRECTONLY, 0,
                                 writefileFunc, 0, 0);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "lsmode", 1, CAPDB_UTF8, 0,
                                 lsModeFunc, 0, 0);
  }
  if( rc==CAPDB_OK ){
    rc = fsdirRegister(db);
  ...
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "realpath", 1,
                                 CAPDB_UTF8, 0,
                                 realpathFunc, 0, 0);
  }
  return rc;
