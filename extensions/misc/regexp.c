...
** same name.
*/
#define re_match   capdbre_match
#define re_compile capdbre_compile
#define re_free    capdbre_free
...
static int re_match(ReCompiled *pRe, const unsigned char *zIn, int nIn){
  ...
  if( pRe->nState<=(sizeof(aSpace)/(sizeof(aSpace[0])*2)) ){
    ...
    pToFree = capdb_malloc64( sizeof(ReStateNumber)*2*pRe->nState );
...
static int re_resize(ReCompiled *p, unsigned int N){
  ...
  aOp = capdb_realloc64(p->aOp, N*sizeof(p->aOp[0]));
  if( aOp==0 ){ p->zErr = "out of memory"; return 1; }
  p->aOp = aOp;
  aArg = capdb_realloc64(p->aArg, N*sizeof(p->aArg[0]));
...
*/
static int re_maxlen(capdb_context *context){
  capdb *db = capdb_context_db_handle(context);
  return capdb_limit(db, CAPDB_LIMIT_LIKE_PATTERN_LENGTH,-1);
}
...
static void re_sql_func(
  ...
  int setAux = 0;           /* True to invoke capdb_set_auxdata() */

  (void)argc;  /* Unused */
  pRe = capdb_get_auxdata(context, 0);
  if( pRe==0 ){
    int mxLen = re_maxlen(context);
    int nPattern;
    zPattern = (const char*)capdb_value_text(argv[0]);
    if( zPattern==0 ) return;
    nPattern = capdb_value_bytes(argv[0]);
    if( nPattern>mxLen ){
      zErr = "REGEXP pattern too big";
    }else{
      zErr = re_compile(&pRe, zPattern, re_maxnfa(mxLen),
                        capdb_user_data(context)!=0);
    }
    if( zErr ){
      re_free(pRe);
  ...
  zStr = (const unsigned char*)capdb_value_text(argv[1]);
  if( zStr!=0 ){
    capdb_result_int(context, re_match(pRe, zStr, -1));
  }
  if( setAux ){
    capdb_set_auxdata(context, 0, pRe, re_free_voidptr);
  }
}
...
** if the CAPDB_DEBUG compile-time option is used.
**
...
static void re_bytecode_func(
  ...
  pStr = capdb_str_new(0);
  if( pRe->nInit>0 ){
    capdb_str_appendf(pStr, "INIT     ");
    for(i=0; i<pRe->nInit; i++){
      capdb_str_appendf(pStr, "%02x", pRe->zInit[i]);
    }
    capdb_str_appendf(pStr, "\n");
  }
  for(i=0; (unsigned)i<pRe->nState; i++){
    capdb_str_appendf(pStr, "%-8s %4d\n",
         ReOpName[(unsigned char)pRe->aOp[i]], pRe->aArg[i]);
  }
  if( capdb_str_errcode(pStr)==CAPDB_NOMEM ){
    capdb_str_finish(pStr);
    re_free(pRe);
    capdb_result_error_nomem(context);
    return;
  }
  ...
  capdb_result_text(context, z, n-1, capdb_free);
  re_free(pRe);
}
...
int capdb_regexp_init(
  ...
  rc = capdb_create_function(db, "regexp", 2, 
                            CAPDB_UTF8|CAPDB_INNOCUOUS|CAPDB_DETERMINISTIC,
                            0, re_sql_func, 0, 0);
  if( rc==CAPDB_OK ){
    /* The regexpi(PATTERN,STRING) function is a case-insensitive version
    ** of regexp(PATTERN,STRING). */
    rc = capdb_create_function(db, "regexpi", 2,
                            CAPDB_UTF8|CAPDB_INNOCUOUS|CAPDB_DETERMINISTIC,
                            (void*)db, re_sql_func, 0, 0);
#if defined(CAPDB_DEBUG)
    if( rc==CAPDB_OK ){
      rc = capdb_create_function(db, "regexp_bytecode", 1,
                            CAPDB_UTF8|CAPDB_INNOCUOUS|CAPDB_DETERMINISTIC,
