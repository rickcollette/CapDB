...
struct nextCharContext {
  ...
  capdb_stmt *pStmt;              /* Prepared statement used to query */
...
static void nextCharAppend(nextCharContext *p, unsigned c){
  ...
  if( p->nUsed+1 > p->nAlloc ){
    unsigned int *aNew;
    int n = p->nAlloc*2 + 30;
    aNew = capdb_realloc64(p->aResult, n*sizeof(unsigned int));
    if( aNew==0 ){
      p->mallocFailed = 1;
      return;
    }else{
...
static void findNextChars(nextCharContext *p){
  ...
  for(;;){
    capdb_bind_text(p->pStmt, 1, (char*)p->zPrefix, p->nPrefix,
                      CAPDB_STATIC);
    n = writeUtf8(zPrev, cPrev+1);
    capdb_bind_text(p->pStmt, 2, (char*)zPrev, n, CAPDB_STATIC);
    ...
    if( rc==CAPDB_DONE ){
      ...
      const unsigned char *zOut = capdb_column_text(p->pStmt, 0);
      unsigned cNext;
...
static void nextCharFunc(
  ...
  nextCharContext c;
  const unsigned char *zTable = capdb_value_text(argv[1]);
  const unsigned char *zField = capdb_value_text(argv[2]);
  ...
  memset(&c, 0, sizeof(c));
  c.db = capdb_context_db_handle(context);
  c.zPrefix = capdb_value_text(argv[0]);
  c.nPrefix = capdb_value_bytes(argv[0]);
  if( zTable==0 || zField==0 || c.zPrefix==0 ) return;
  if( argc>=4
   && (zWhere = capdb_value_text(argv[3]))!=0
   && zWhere[0]!=0
  ){
    zWhereClause = capdb_mprintf("AND (%s)", zWhere);
    if( zWhereClause==0 ){
  ...
  if( argc>=5
   && (zCollName = capdb_value_text(argv[4]))!=0
   && zCollName[0]!=0 
  ){
    zColl = capdb_mprintf("collate \"%w\"", zCollName);
    if( zColl==0 ){
      capdb_result_error_nomem(context);
      if( zWhereClause[0] ) capdb_free(zWhereClause);
      return;
    }
  }else{
    zColl = "";
  ...
  if( zWhereClause[0] ) capdb_free(zWhereClause);
  if( zColl[0] ) capdb_free(zColl);
  if( zSql==0 ){
    capdb_result_error_nomem(context);
    return;
  }

  rc = capdb_prepare_v2(c.db, zSql, -1, &c.pStmt, 0);
  capdb_free(zSql);
  if( rc ){
    capdb_result_error(context, capdb_errmsg(c.db), -1);
    return;
  }
  findNextChars(&c);
  if( c.mallocFailed ){
    ...
    unsigned char *pRes;
    pRes = capdb_malloc64( c.nUsed*4 + 1 );
    if( pRes==0 ){
      capdb_result_error_nomem(context);
    }else{
      int i;
      ...
      pRes[n] = 0;
      capdb_result_text(context, (const char*)pRes, n, capdb_free);
    }
  }
  capdb_finalize(c.pStmt);
  capdb_free(c.aResult);
}

#ifdef _WIN32
__declspec(dllexport)
#endif
int capdb_nextchar_init(
  capdb *db, 
  char **pzErrMsg, 
  ...
  rc = capdb_create_function(db, "next_char", 3,
                               CAPDB_UTF8|CAPDB_INNOCUOUS, 0,
                               nextCharFunc, 0, 0);
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "next_char", 4,
                                 CAPDB_UTF8|CAPDB_INNOCUOUS, 0,
                                 nextCharFunc, 0, 0);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "next_char", 5,
                                 CAPDB_UTF8|CAPDB_INNOCUOUS, 0,
                                 nextCharFunc, 0, 0);
  }
  return rc;
