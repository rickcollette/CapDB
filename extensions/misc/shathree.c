...
static void sha3Func(
  ...
  if( argc==1 ){
    ...
    iSize = capdb_value_int(argv[1]);
    if( iSize!=224 && iSize!=256 && iSize!=384 && iSize!=512 ){
      capdb_result_error(context, "SHA3 size should be one of: 224 256 "
                                    "384 512", -1);
      return;
    }
  }
  ...
  SHA3Init(&cx, iSize);
  if( eType==CAPDB_BLOB ){
    SHA3Update(&cx, capdb_value_blob(argv[0]), nByte);
  }else{
    SHA3Update(&cx, capdb_value_text(argv[0]), nByte);
  }
  capdb_result_blob(context, SHA3Final(&cx), iSize/8, CAPDB_TRANSIENT);
}

/* Compute a string using capdb_vsnprintf() with a maximum length
...
static void sha3_step_vformat(
  ...
  char zBuf[50];
  va_start(ap, zFormat);
  capdb_vsnprintf(sizeof(zBuf),zBuf,zFormat,ap);
  va_end(ap);
  n = (int)strlen(zBuf);
...
/*
** Update a SHA3Context using a single capdb_value.
*/
static void sha3UpdateFromValue(SHA3Context *p, capdb_value *pVal){
  switch( capdb_value_type(pVal) ){
    ...
    case CAPDB_INTEGER: {
      ...
      int j;
      unsigned char x[9];
      capdb_int64 v = capdb_value_int64(pVal);
      memcpy(&u, &v, 8);
      for(j=8; j>=1; j--){
        x[j] = u & 0xff;
        u >>= 8;
    ...
    case CAPDB_FLOAT: {
      ...
      int j;
      unsigned char x[9];
      double r = capdb_value_double(pVal);
      memcpy(&u, &r, 8);
      for(j=8; j>=1; j--){
        x[j] = u & 0xff;
        u >>= 8;
    ...
    case CAPDB_TEXT: {
      int n2 = capdb_value_bytes(pVal);
      const unsigned char *z2 = capdb_value_text(pVal);
    ...
    case CAPDB_BLOB: {
      int n2 = capdb_value_bytes(pVal);
      const unsigned char *z2 = capdb_value_blob(pVal);
...
static void sha3QueryFunc(
  ...
  while( zSql[0] ){
    ...
    if( rc ){
      char *zMsg = capdb_mprintf("error SQL statement [%s]: %s",
                                   zSql, capdb_errmsg(db));
      capdb_finalize(pStmt);
      capdb_result_error(context, zMsg, -1);
      capdb_free(zMsg);
      return;
    }
    if( !capdb_stmt_readonly(pStmt) ){
      char *zMsg = capdb_mprintf("non-query: [%s]", capdb_sql(pStmt));
      capdb_finalize(pStmt);
      capdb_result_error(context, zMsg, -1);
      capdb_free(zMsg);
      return;
    ...
    while( CAPDB_ROW==capdb_step(pStmt) ){
      ...
      for(i=0; i<nCol; i++){
        sha3UpdateFromValue(&cx, capdb_column_value(pStmt,i));
      }
    }
    capdb_finalize(pStmt);
  }
...
static void sha3AggStep(
  ...
  SHA3Context *p;
  p = (SHA3Context*)capdb_aggregate_context(context, sizeof(*p));
  if( p==0 ) return;
  if( p->nRate==0 ){
    int sz = 256;
    if( argc==2 ){
      sz = capdb_value_int(argv[1]);
...
*/
static void sha3AggFinal(capdb_context *context){
  SHA3Context *p;
  p = (SHA3Context*)capdb_aggregate_context(context, sizeof(*p));
  if( p==0 ) return;
  if( p->iSize ){
    capdb_result_blob(context, SHA3Final(p), p->iSize/8, CAPDB_TRANSIENT);
  }
}
...
int capdb_shathree_init(
  ...
  rc = capdb_create_function(db, "sha3", 1,
                      CAPDB_UTF8 | CAPDB_INNOCUOUS | CAPDB_DETERMINISTIC,
                      0, sha3Func, 0, 0);
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "sha3", 2,
                      CAPDB_UTF8 | CAPDB_INNOCUOUS | CAPDB_DETERMINISTIC,
                      0, sha3Func, 0, 0);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "sha3_agg", 1,
                      CAPDB_UTF8 | CAPDB_INNOCUOUS | CAPDB_DETERMINISTIC,
                      0, 0, sha3AggStep, sha3AggFinal);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "sha3_agg", 2,
                      CAPDB_UTF8 | CAPDB_INNOCUOUS | CAPDB_DETERMINISTIC,
                      0, 0, sha3AggStep, sha3AggFinal);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "sha3_query", 1,
                      CAPDB_UTF8 | CAPDB_DIRECTONLY,
                      0, sha3QueryFunc, 0, 0);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "sha3_query", 2,
                      CAPDB_UTF8 | CAPDB_DIRECTONLY,
                      0, sha3QueryFunc, 0, 0);
  }
  return rc;
