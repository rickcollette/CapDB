...
/* Compute a string using capdb_vsnprintf() and hash it */
...
static void sha1Func(
  ...
  hash_init(&cx);
  if( eType==CAPDB_BLOB ){
    pData = (const unsigned char*)capdb_value_blob(argv[0]);
  }else{
    pData = (const unsigned char*)capdb_value_text(argv[0]);
  }
  if( pData==0 ) return;
  ...
  if( capdb_user_data(context)!=0 ){
    ...
    capdb_result_blob(context, zOut, 20, CAPDB_TRANSIENT);
  }else{
    /* sha1() - hexadecimal text result */
    hash_finish(&cx, zOut, 0);
    capdb_result_text(context, zOut, 40, CAPDB_TRANSIENT);
  }
}
      ...
      for(i=0; i<nCol; i++){
        switch( capdb_column_type(pStmt,i) ){
          ...
          case CAPDB_INTEGER: {
            ...
            int j;
            unsigned char x[9];
            capdb_int64 v = capdb_column_int64(pStmt,i);
            memcpy(&u, &v, 8);
            for(j=8; j>=1; j--){
              x[j] = u & 0xff;
              u >>= 8;
          ...
          case CAPDB_TEXT: {
            int n2 = capdb_column_bytes(pStmt, i);
            const unsigned char *z2 = capdb_column_text(pStmt, i);
          ...
          case CAPDB_BLOB: {
            int n2 = capdb_column_bytes(pStmt, i);
            const unsigned char *z2 = capdb_column_blob(pStmt, i);
...
int capdb_sha_init(
  ...
  rc = capdb_create_function(db, "sha1", 1, 
                       CAPDB_UTF8 | CAPDB_INNOCUOUS | CAPDB_DETERMINISTIC,
                                0, sha1Func, 0, 0);
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "sha1b", 1, 
                       CAPDB_UTF8 | CAPDB_INNOCUOUS | CAPDB_DETERMINISTIC,
                          (void*)&one, sha1Func, 0, 0);
  }
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "sha1_query", 1, 
