...
static void is_base85(capdb_context *context, int na, capdb_value *av[]){
  assert(na==1);
  switch( capdb_value_type(av[0]) ){
  case CAPDB_TEXT:
    {
      int rv = allBase85( (char *)capdb_value_text(av[0]),
                          capdb_value_bytes(av[0]) );
  ...
  case CAPDB_NULL:
    ...
    break;
  default:
    capdb_result_error(context, "is_base85 accepts only text or NULL", -1);
    return;
  }
}
#endif
...
static void base85(capdb_context *context, int na, capdb_value *av[]){
  capdb_int64 nb, nc, nv = capdb_value_bytes(av[0]);
  int nvMax = capdb_limit(capdb_context_db_handle(context),
                            CAPDB_LIMIT_LENGTH, -1);
  char *cBuf;
  u8 *bBuf;
  assert(na==1);
  switch( capdb_value_type(av[0]) ){
  case CAPDB_BLOB:
    ...
    if( nvMax < nc ){
      capdb_result_error(context, "blob expanded to base85 too big", -1);
      return;
    }
    bBuf = (u8*)capdb_value_blob(av[0]);
    if( !bBuf ){
      if( CAPDB_NOMEM==capdb_errcode(capdb_context_db_handle(context)) ){
        goto memFail;
      }
      capdb_result_text(context,"",-1,CAPDB_STATIC);
      break;
    }
    cBuf = capdb_malloc64(nc);
    if( !cBuf ) goto memFail;
    nc = (int)(toBase85(bBuf, nb, cBuf, "\n") - cBuf);
    capdb_result_text(context, cBuf, nc, capdb_free);
    break;
  case CAPDB_TEXT:
    nc = nv;
    nb = 4*(nv/5) + nv%5; /* may overestimate */
    if( nvMax < nb ){
      capdb_result_error(context, "blob from base85 may be too big", -1);
      return;
    }else if( nb<1 ){
      nb = 1;
    }
    cBuf = (char *)capdb_value_text(av[0]);
    if( !cBuf ){
      if( CAPDB_NOMEM==capdb_errcode(capdb_context_db_handle(context)) ){
        goto memFail;
      }
      capdb_result_zeroblob(context, 0);
      break;
    }
    bBuf = capdb_malloc64(nb);
    if( !bBuf ) goto memFail;
    nb = (int)(fromBase85(cBuf, nc, bBuf) - bBuf);
    capdb_result_blob(context, bBuf, nb, capdb_free);
    break;
  default:
    capdb_result_error(context, "base85 accepts only blob or text.", -1);
    return;
  }
  return;
 memFail:
  capdb_result_error(context, "base85 OOM", -1);
}

/*
** Establish linkage to running SQLite library.
*/
#ifndef CAPDB_SHELL_EXTFUNCS
#ifdef _WIN32
__declspec(dllexport)
#endif
int capdb_base85_init
#else
static int capdb_base85_init
#endif
(capdb *db, char **pzErr, const capdb_api_routines *pApi){
...
#ifndef OMIT_BASE85_CHECKER
  {
    int rc = capdb_create_function
      (db, "is_base85", 1,
       CAPDB_DETERMINISTIC|CAPDB_INNOCUOUS|CAPDB_UTF8,
       0, is_base85, 0, 0);
    if( rc!=CAPDB_OK ) return rc;
  }
#endif
  return capdb_create_function
    (db, "base85", 1,
     CAPDB_DETERMINISTIC|CAPDB_INNOCUOUS|CAPDB_DIRECTONLY|CAPDB_UTF8,
     0, base85, 0, 0);
}

/*
** Define some macros to allow this extension to be built into the shell
** conveniently, in conjunction with use of CAPDB_SHELL_EXTFUNCS. This
** allows shell.c, as distributed, to have this extension built in.
*/
# define BASE85_INIT(db) capdb_base85_init(db, 0, 0)
