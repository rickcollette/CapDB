...
** copied from capdbInt.h.
*/
#ifndef LARGEST_INT64
# define LARGEST_INT64  (0xffffffff|(((capdb_int64)0x7fffffff)<<32))
#endif
#ifndef SMALLEST_INT64
# define SMALLEST_INT64 (((capdb_int64)-1) - LARGEST_INT64)
#endif

/*
** The following is also copied from capdbInt.h. To facilitate coverage
** testing.
*/
#ifndef ALWAYS
...
struct UnionSrc {
  ...
  capdb_int64 iMin;             /* Minimum rowid */
  capdb_int64 iMax;             /* Maximum rowid */
...
struct UnionTab {
  ...
  capdb_stmt *pNotFound;        /* UDF to invoke if file not found on open */
  capdb_stmt *pOpenClose;       /* UDF to invoke on open and close */
...
struct UnionCsr {
  ...
  /* Used by swarmvtab only */
  capdb_int64 iMaxRowid;        /* Last rowid to visit */
...
/*
** If *pRc is other than CAPDB_OK when this function is called, it
...
** CAPDB_NOMEM.
*/
static void *unionMalloc(int *pRc, capdb_int64 nByte){
  void *pRet;
  assert( nByte>0 );
  if( *pRc==CAPDB_OK ){
...
static char *unionStrdup(int *pRc, const char *zIn){
  char *zRet = 0;
  if( zIn ){
    capdb_int64 nByte = strlen(zIn) + 1;
...
** statement by calling capdb_finalize(). Alternatively, if statement
...
** capdb_malloc().
*/
static capdb_stmt *unionPrepare(
  ...
  assert( pzErr );
  if( *pRc==CAPDB_OK ){
    int rc = capdb_prepare_v2(db, zSql, -1, &pRet, 0);
    if( rc!=CAPDB_OK ){
      *pzErr = capdb_mprintf("sql error: %s", capdb_errmsg(db));
      *pRc = rc;
    }
  }
  return pRet;
...
*/
static capdb_stmt *unionPreparePrintf(
  int *pRc,                       /* IN/OUT: Error code */
  char **pzErr,                   /* OUT: Error message */
  capdb *db,                    /* Database handle */
...
/*
** Call capdb_reset() on SQL statement pStmt. If *pRc is set to 
** CAPDB_OK when this function is called, then it is set to the
** value returned by capdb_reset() before this function exits.
** In this case, *pzErr may be set to point to an error message
** buffer allocated by capdb_malloc().
*/
#if 0
static void unionReset(int *pRc, capdb_stmt *pStmt, char **pzErr){
  int rc = capdb_reset(pStmt);
  if( *pRc==CAPDB_OK ){
    *pRc = rc;
    if( rc ){
      *pzErr = capdb_mprintf("%s", capdb_errmsg(capdb_db_handle(pStmt)));
    }
  }
}
#endif
...
/*
** Call capdb_finalize() on SQL statement pStmt. If *pRc is set to 
** CAPDB_OK when this function is called, then it is set to the
** value returned by capdb_finalize() before this function exits.
*/
static void unionFinalize(int *pRc, capdb_stmt *pStmt, char **pzErr){
...
**
** If successful, return CAPDB_OK. Otherwise an SQLite error code. In this
...
** any error message using capdb_free().
*/
static int unionInvokeOpenClose(
  ...
  int rc = CAPDB_OK;
  if( pTab->pOpenClose ){
    capdb_bind_text(pTab->pOpenClose, 1, pSrc->zFile, -1, CAPDB_STATIC);
    if( pTab->bHasContext ){
      capdb_bind_text(pTab->pOpenClose, 2, pSrc->zContext, -1, CAPDB_STATIC);
    }
    capdb_bind_int(pTab->pOpenClose, 2+pTab->bHasContext, bClose);
    capdb_step(pTab->pOpenClose);
    if( CAPDB_OK!=(rc = capdb_reset(pTab->pOpenClose)) ){
      if( pzErr ){
        *pzErr = capdb_mprintf("%s", capdb_errmsg(pTab->db));
      }
    }
  }
  return rc;
...
** code is returned if an error occurs, or CAPDB_OK otherwise.
*/
...
** xDisconnect method.
*/
static int unionDisconnect(capdb_vtab *pVtab){
  if( pVtab ){
    ...
    for(i=0; i<pTab->nSrc; i++){
      ...
      capdb_free(pSrc->zDb);
      capdb_free(pSrc->zTab);
      capdb_free(pSrc->zFile);
      capdb_free(pSrc->zContext);
    }
    capdb_finalize(pTab->pNotFound);
    capdb_finalize(pTab->pOpenClose);
    capdb_free(pTab->zSourceStr);
    capdb_free(pTab->aSrc);
    capdb_free(pTab);
  }
  return CAPDB_OK;
}
...
** return CAPDB_ERROR and set (*pzErr) to point to an English language
** error message. If the table is a rowid table and no error occurs,
** return CAPDB_OK and leave (*pzErr) unmodified.
*/
static int unionIsIntkeyTable(
  ...
  rc = capdb_errcode(db);
  if( rc==CAPDB_ERROR 
   || (rc==CAPDB_OK && (!bPk || capdb_stricmp("integer", zType)))
  ){
    rc = CAPDB_ERROR;
    *pzErr = capdb_mprintf("no such rowid table: %s%s%s",
        (pSrc->zDb ? pSrc->zDb : ""),
        (pSrc->zDb ? "." : ""),
        pSrc->zTab
    );
...
** If this is not the case, *pRc is set to CAPDB_ERROR and NULL is
** returned.
**
...
** of the caller to free the returned string using capdb_free() when
...
static char *unionSourceToStr(
  ...
  char *zRet = 0;
  if( *pRc==CAPDB_OK ){
    capdb *db = unionGetDb(pTab, pSrc);
    int rc = unionIsIntkeyTable(db, pSrc, pzErr);
    capdb_stmt *pStmt = unionPrepare(&rc, db, 
        "SELECT group_concat(quote(name) || '.' || quote(type)) "
        "FROM pragma_table_info(?, ?)", pzErr
    );
    if( rc==CAPDB_OK ){
      capdb_bind_text(pStmt, 1, pSrc->zTab, -1, CAPDB_STATIC);
      capdb_bind_text(pStmt, 2, pSrc->zDb, -1, CAPDB_STATIC);
...
** to point to an error message buffer allocated by capdb_mprintf().
** Or, if no problems regarding the source tables are detected and no
** other error occurs, CAPDB_OK is returned.
*/
static int unionSourceCheck(UnionTab *pTab, char **pzErr){
  ...
  for(i=1; i<pTab->nSrc; i++){
    ...
    if( rc==CAPDB_OK && capdb_stricmp(z, z0) ){
      *pzErr = capdb_mprintf("source table schema mismatch");
      rc = CAPDB_ERROR;
    }
    capdb_free(z);
  }
...
static int unionOpenDatabaseInner(UnionTab *pTab, UnionSrc *pSrc, char **pzErr){
  static const int openFlags = CAPDB_OPEN_READONLY | CAPDB_OPEN_URI;
  int rc;

  rc = unionInvokeOpenClose(pTab, pSrc, 0, pzErr);
  if( rc!=CAPDB_OK ) return rc;

  rc = capdb_open_v2(pSrc->zFile, &pSrc->db, openFlags, 0);
  if( rc==CAPDB_OK ) return rc;
  if( pTab->pNotFound ){
    capdb_close(pSrc->db);
    pSrc->db = 0;
    capdb_bind_text(pTab->pNotFound, 1, pSrc->zFile, -1, CAPDB_STATIC);
    if( pTab->bHasContext ){
      capdb_bind_text(pTab->pNotFound, 2, pSrc->zContext, -1, CAPDB_STATIC);
    }
    capdb_step(pTab->pNotFound);
    if( CAPDB_OK!=(rc = capdb_reset(pTab->pNotFound)) ){
      *pzErr = capdb_mprintf("%s", capdb_errmsg(pTab->db));
      return rc;
    }
    rc = capdb_open_v2(pSrc->zFile, &pSrc->db, openFlags, 0);
  ...
  if( rc!=CAPDB_OK ){
    *pzErr = capdb_mprintf("%s", capdb_errmsg(pSrc->db));
  }
  return rc;
}
...
** expected, or if it is already open when this function is called, CAPDB_OK
** is returned.
**
    ...
    if( rc==CAPDB_OK ){
      ...
      if( rc==CAPDB_OK ){
        if( pTab->zSourceStr==0 ){
          ...
          if( capdb_stricmp(z, pTab->zSourceStr) ){
            *pzErr = capdb_mprintf("source table schema mismatch");
            rc = CAPDB_ERROR;
          }
          capdb_free(z);
...
** caller to eventually free the buffer using capdb_free().
*/
static void unionConfigureVtab(
  int *pRc,                       /* IN/OUT: Error code */
  UnionTab *pTab,                 /* Table to configure */
  capdb_stmt *pStmt,            /* SQL statement to find sources */
  ...
  int i;
  if( rc==CAPDB_OK ){
    pTab->bHasContext = (capdb_column_count(pStmt)>4);
  }
  for(i=0; rc==CAPDB_OK && i<nArg; i++){
      ...
      if( *zVal=='=' ){
        ...
        if( zVal ){
          ...
          if( zOpt[0]==':' ){
            ...
            int iParam = capdb_bind_parameter_index(pStmt, zOpt);
            if( iParam==0 ){
              *pzErr = capdb_mprintf(
              ...
              rc = capdb_bind_text(pStmt, iParam, zVal, -1, CAPDB_TRANSIENT);
            }
          }else if( nOpt==7 && 0==capdb_strnicmp(zOpt, "maxopen", 7) ){
            pTab->nMaxOpen = atoi(zVal);
            if( pTab->nMaxOpen<=0 ){
              *pzErr = capdb_mprintf("swarmvtab: illegal maxopen value");
              rc = CAPDB_ERROR;
            }
          }else if( nOpt==7 && 0==capdb_strnicmp(zOpt, "missing", 7) ){
            if( pTab->pNotFound ){
              *pzErr = capdb_mprintf(
          ...
          }else if( nOpt==9 && 0==capdb_strnicmp(zOpt, "openclose", 9) ){
            if( pTab->pOpenClose ){
              *pzErr = capdb_mprintf(
            ...
            *pzErr = capdb_mprintf("swarmvtab: unrecognized option: %s",zOpt);
            rc = CAPDB_ERROR;
          }
          capdb_free(zVal);
        }
        ...
        if( i==0 && nArg==1 ){
          ...
          *pzErr = capdb_mprintf( "swarmvtab: parse error: %s", azArg[i]);
          rc = CAPDB_ERROR;
        }
      }
      capdb_free(zArg);
...
static int unionConnect(
  ...
  if( capdb_stricmp("temp", argv[1]) ){
    /* unionvtab tables may only be created in the temp schema */
    *pzErr = capdb_mprintf("%s tables must be created in TEMP schema", zVtab);
    rc = CAPDB_ERROR;
  }else if( argc<4 || (argc>4 && bSwarm==0) ){
    *pzErr = capdb_mprintf("wrong number of arguments for %s", zVtab);
    rc = CAPDB_ERROR;
  }else{
    int nAlloc = 0;               /* Allocated size of pTab->aSrc[] */
    capdb_stmt *pStmt = 0;      /* Argument statement */
    ...
    ** as an argument to the CREATE VIRTUAL TABLE statement. */
    while( rc==CAPDB_OK && CAPDB_ROW==capdb_step(pStmt) ){
      const char *zDb = (const char*)capdb_column_text(pStmt, 0);
      const char *zTab = (const char*)capdb_column_text(pStmt, 1);
      capdb_int64 iMin = capdb_column_int64(pStmt, 2);
      capdb_int64 iMax = capdb_column_int64(pStmt, 3);
      UnionSrc *pSrc;

      /* Grow the pTab->aSrc[] array if required. */
      if( nAlloc<=pTab->nSrc ){
        int nNew = nAlloc ? nAlloc*2 : 8;
        UnionSrc *aNew = (UnionSrc*)capdb_realloc64(
      ...
      if( iMax<iMin || (pTab->nSrc>0 && iMin<=pTab->aSrc[pTab->nSrc-1].iMax) ){
        *pzErr = capdb_mprintf("rowid range mismatch error");
        rc = CAPDB_ERROR;
      }
      ...
      if( rc==CAPDB_OK ){
        ...
        if( pTab->bHasContext ){
          const char *zContext = (const char*)capdb_column_text(pStmt, 4);
    ...
    ** table in this case.  */
    if( rc==CAPDB_OK && pTab->nSrc==0 ){
      *pzErr = capdb_mprintf("no source tables configured");
      rc = CAPDB_ERROR;
    }
    ...
    ** check that the first table is a rowid table only.  */
    ...
    if( rc==CAPDB_OK ){
      ...
      capdb *tdb = unionGetDb(pTab, pSrc);
    ...
    if( rc==CAPDB_OK && CAPDB_ROW==capdb_step(pStmt) ){
      const char *zDecl = (const char*)capdb_column_text(pStmt, 0);
      rc = capdb_declare_vtab(db, zDecl);
      pTab->iPK = capdb_column_int(pStmt, 1);
    }
  ...
  if( rc!=CAPDB_OK ){
    unionDisconnect((capdb_vtab*)pTab);
    pTab = 0;
  }
...
** xOpen
*/
static int unionOpen(capdb_vtab *p, capdb_vtab_cursor **ppCursor){
  UnionCsr *pCsr;
  int rc = CAPDB_OK;
...
** xClose
*/
static int unionClose(capdb_vtab_cursor *cur){
...
** returns CAPDB_ROW, it should be called again within the same xNext()
** method call. See unionNext() for details.
*/
static int doUnionNext(UnionCsr *pCsr){
  int rc = CAPDB_OK;
  assert( pCsr->pStmt );
  if( capdb_step(pCsr->pStmt)!=CAPDB_ROW ){
    UnionTab *pTab = (UnionTab*)pCsr->base.pVtab;
    rc = unionFinalizeCsrStmt(pCsr);
    if( rc==CAPDB_OK && pTab->bSwarm ){
      pCsr->iTab++;
...
** xNext
*/
static int unionNext(capdb_vtab_cursor *cur){
  int rc;
  do {
...
** xRowid
*/
static int unionRowid(capdb_vtab_cursor *cur, sqlite_int64 *pRowid){
  UnionCsr *pCsr = (UnionCsr*)cur;
...
** xEof
*/
static int unionEof(capdb_vtab_cursor *cur){
  UnionCsr *pCsr = (UnionCsr*)cur;
  return pCsr->pStmt==0;
}
...
static int unionFilter(
  ...
  capdb_int64 iMin = SMALLEST_INT64;
  capdb_int64 iMax = LARGEST_INT64;

  assert( idxNum==0 
       || idxNum==CAPDB_INDEX_CONSTRAINT_EQ
       || idxNum==CAPDB_INDEX_CONSTRAINT_LE
       || idxNum==CAPDB_INDEX_CONSTRAINT_GE
       || idxNum==CAPDB_INDEX_CONSTRAINT_LT
       || idxNum==CAPDB_INDEX_CONSTRAINT_GT
       || idxNum==(CAPDB_INDEX_CONSTRAINT_GE|CAPDB_INDEX_CONSTRAINT_LE)
  );

  (void)idxStr;  /* Suppress harmless warning */
  
  if( idxNum==CAPDB_INDEX_CONSTRAINT_EQ ){
    assert( argc==1 );
    iMin = iMax = capdb_value_int64(argv[0]);
  }else{

    if( idxNum & (CAPDB_INDEX_CONSTRAINT_LE|CAPDB_INDEX_CONSTRAINT_LT) ){
      assert( argc>=1 );
      iMax = capdb_value_int64(argv[0]);
      if( idxNum & CAPDB_INDEX_CONSTRAINT_LT ){
    ...
    if( idxNum & (CAPDB_INDEX_CONSTRAINT_GE|CAPDB_INDEX_CONSTRAINT_GT) ){
      assert( argc>=1 );
      iMin = capdb_value_int64(argv[argc-1]);
      if( idxNum & CAPDB_INDEX_CONSTRAINT_GT ){
  ...
  for(i=0; i<pTab->nSrc; i++){
    ...
    zSql = capdb_mprintf("%z%sSELECT rowid, * FROM %s%q%s%Q"
        , zSql
        , (zSql ? " UNION ALL " : "")
        , (pSrc->zDb ? "'" : "")
        , (pSrc->zDb ? pSrc->zDb : "")
    ...
    if( iMin==iMax ){
      zSql = capdb_mprintf("%z WHERE rowid=%lld", zSql, iMin);
    }else{
      ...
      if( iMax!=LARGEST_INT64 && iMax<pSrc->iMax ){
        zSql = capdb_mprintf("%z %s rowid<=%lld", zSql, zWhere, iMax);
      }
    }
  ...
  if( zSql==0 ){
    ...
    capdb *db = unionGetDb(pTab, &pTab->aSrc[pCsr->iTab]);
...
*/
static int createUnionVtab(capdb *db){
  static capdb_module unionModule = {
    0,                            /* iVersion */
    unionConnect,
    unionConnect,
  ...
  rc = capdb_create_module(db, "unionvtab", &unionModule, 0);
  if( rc==CAPDB_OK ){
    rc = capdb_create_module(db, "swarmvtab", &unionModule, (void*)db);
  }
  return rc;
}
