...
*/
static int totypeAtoi64(const char *zNum, capdb_int64 *pNum, int length){
  capdb_uint64 u = 0;
...
** capdbRealToI64().
*/
static capdb_int64 totypeDoubleToInt(double r){
  if( r<-9223372036854774784.0 ) return 0;
  if( r>+9223372036854774784.0 ) return 0;
  return (capdb_int64)r;
}
...
static void tointegerFunc(
  ...
  switch( capdb_value_type(argv[0]) ){
    case CAPDB_FLOAT: {
      ...
      capdb_int64 iVal = totypeDoubleToInt(rVal);
      if( rVal==(double)iVal ){
    ...
    case CAPDB_BLOB: {
      const unsigned char *zBlob = capdb_value_blob(argv[0]);
      if( zBlob ){
        int nBlob = capdb_value_bytes(argv[0]);
        if( nBlob==sizeof(capdb_int64) ){
          capdb_int64 iVal;
          if( TOTYPE_BIGENDIAN ){
            int i;
            unsigned char zBlobRev[sizeof(capdb_int64)];
            for(i=0; i<sizeof(capdb_int64); i++){
              zBlobRev[i] = zBlob[sizeof(capdb_int64)-1-i];
            }
            memcpy(&iVal, zBlobRev, sizeof(capdb_int64));
          }else{
            memcpy(&iVal, zBlob, sizeof(capdb_int64));
          }
          capdb_result_int64(context, iVal);
        }
      }
    ...
    case CAPDB_TEXT: {
      const unsigned char *zStr = capdb_value_text(argv[0]);
      if( zStr ){
        int nStr = capdb_value_bytes(argv[0]);
    ...
    default: {
      assert( capdb_value_type(argv[0])==CAPDB_NULL );
      break;
    }
  }
}
...
static void torealFunc(
  ...
  switch( capdb_value_type(argv[0]) ){
    case CAPDB_FLOAT: {
      capdb_result_double(context, capdb_value_double(argv[0]));
      break;
    }
    case CAPDB_INTEGER: {
      capdb_int64 iVal = capdb_value_int64(argv[0]);
...
int capdb_totype_init(
  ...
  rc = capdb_create_function(db, "tointeger", 1,
        CAPDB_UTF8 | CAPDB_DETERMINISTIC | CAPDB_INNOCUOUS, 0,
        tointegerFunc, 0, 0);
  if( rc==CAPDB_OK ){
    rc = capdb_create_function(db, "toreal", 1,
        CAPDB_UTF8 | CAPDB_DETERMINISTIC | CAPDB_INNOCUOUS, 0,
        torealFunc, 0, 0);
  }
  return rc;
