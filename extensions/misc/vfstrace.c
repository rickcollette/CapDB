...
** capdb_open_v2() or by making the trace VFS be the default (by setting
...
** and the SQLite amalgamation source files (capdb.c, capdb.h) into
** the working directory.  Then compile using a command like the following:
**
**    gcc -o capdb -Os -I. -DCAPDB_ENABLE_VFSTRACE \
**        -DCAPDB_THREADSAFE=0 -DCAPDB_ENABLE_FTS3 -DCAPDB_ENABLE_RTREE \
**        -DHAVE_READLINE -DHAVE_USLEEP=1 \
**        shell.c test_vfstrace.c capdb.c -ldl -lreadline -lncurses
**
...
** use threads so we added -DCAPDB_THREADSAFE=0 just to make the code
...
**
**    gcc -DCAPDB_ENABLE_VFSTRACE \
**         shell.c test_vfstrace.c capdb.c -ldl -lpthread
**
...
** work:
**
**    /opt/mingw/bin/i386-mingw32msvc-gcc -o capdb.exe -Os -I \
**         -DCAPDB_THREADSAFE=0 -DCAPDB_ENABLE_VFSTRACE \
**         shell.c test_vfstrace.c capdb.c
**
** Similar compiler commands will work on different systems.  The key
** invariants are (1) you must have -DCAPDB_ENABLE_VFSTRACE so that
** the shell.c source file will know to include the -vfstrace command-line
** option and (2) you must compile and link the three source files
** shell,c, test_vfstrace.c, and capdb.c.
**
...
struct vfstrace_info {
  capdb_vfs *pRootVfs;              /* The underlying real VFS */
  int (*xOut)(const char*, void*);    /* Send output here */
  unsigned int mTrace;                /* Mask of interfaces to trace */
  u8 bOn;                             /* Tracing on/off */
  void *pOutArg;                      /* First argument to xOut */
  const char *zVfsName;               /* Name of this trace-VFS */
  capdb_vfs *pTraceVfs;             /* Pointer back to the trace VFS */
};

/*
** The capdb_file object for the trace VFS
*/
...
struct vfstrace_file {
  ...
  capdb_file *pReal;      /* The real underlying file */
};
...
*/
static int vfstraceClose(capdb_file*);
static int vfstraceRead(capdb_file*, void*, int iAmt, capdb_int64 iOfst);
static int vfstraceWrite(capdb_file*,const void*,int iAmt, capdb_int64);
static int vfstraceTruncate(capdb_file*, capdb_int64 size);
static int vfstraceSync(capdb_file*, int flags);
static int vfstraceFileSize(capdb_file*, capdb_int64 *pSize);
static int vfstraceLock(capdb_file*, int);
static int vfstraceUnlock(capdb_file*, int);
static int vfstraceCheckReservedLock(capdb_file*, int *);
static int vfstraceFileControl(capdb_file*, int op, void *pArg);
static int vfstraceSectorSize(capdb_file*);
static int vfstraceDeviceCharacteristics(capdb_file*);
static int vfstraceShmLock(capdb_file*,int,int,int);
static int vfstraceShmMap(capdb_file*,int,int,int, void volatile **);
static void vfstraceShmBarrier(capdb_file*);
static int vfstraceShmUnmap(capdb_file*,int);

/*
** Method declarations for vfstrace_vfs.
*/
static int vfstraceOpen(capdb_vfs*, const char *, capdb_file*, int , int *);
static int vfstraceDelete(capdb_vfs*, const char *zName, int syncDir);
static int vfstraceAccess(capdb_vfs*, const char *zName, int flags, int *);
static int vfstraceFullPathname(capdb_vfs*, const char *zName, int, char *);
static void *vfstraceDlOpen(capdb_vfs*, const char *zFilename);
static void vfstraceDlError(capdb_vfs*, int nByte, char *zErrMsg);
static void (*vfstraceDlSym(capdb_vfs*,void*, const char *zSymbol))(void);
static void vfstraceDlClose(capdb_vfs*, void*);
static int vfstraceRandomness(capdb_vfs*, int nByte, char *zOut);
static int vfstraceSleep(capdb_vfs*, int microseconds);
static int vfstraceCurrentTime(capdb_vfs*, double*);
static int vfstraceGetLastError(capdb_vfs*, int, char*);
static int vfstraceCurrentTimeInt64(capdb_vfs*, capdb_int64*);
static int vfstraceSetSystemCall(capdb_vfs*,const char*, capdb_syscall_ptr);
static capdb_syscall_ptr vfstraceGetSystemCall(capdb_vfs*, const char *);
static const char *vfstraceNextSystemCall(capdb_vfs*, const char *zName);
...
static void vfstrace_printf(
  ...
  if( pInfo->bOn ){
    va_start(ap, zFormat);
    zMsg = capdb_vmprintf(zFormat, ap);
    va_end(ap);
...
static const char *vfstrace_errcode_name(int rc ){
  const char *zVal = 0;
  switch( rc ){
    case CAPDB_OK:                 zVal = "CAPDB_OK";                 break;
    case CAPDB_INTERNAL:           zVal = "CAPDB_INTERNAL";           break;
    case CAPDB_ERROR:              zVal = "CAPDB_ERROR";              break;
    case CAPDB_PERM:               zVal = "CAPDB_PERM";               break;
    case CAPDB_ABORT:              zVal = "CAPDB_ABORT";              break;
    case CAPDB_BUSY:               zVal = "CAPDB_BUSY";               break;
    case CAPDB_LOCKED:             zVal = "CAPDB_LOCKED";             break;
    case CAPDB_NOMEM:              zVal = "CAPDB_NOMEM";              break;
    case CAPDB_READONLY:           zVal = "CAPDB_READONLY";           break;
    case CAPDB_INTERRUPT:          zVal = "CAPDB_INTERRUPT";          break;
    case CAPDB_IOERR:              zVal = "CAPDB_IOERR";              break;
    case CAPDB_CORRUPT:            zVal = "CAPDB_CORRUPT";            break;
    case CAPDB_NOTFOUND:           zVal = "CAPDB_NOTFOUND";           break;
    case CAPDB_FULL:               zVal = "CAPDB_FULL";               break;
    case CAPDB_CANTOPEN:           zVal = "CAPDB_CANTOPEN";           break;
    case CAPDB_PROTOCOL:           zVal = "CAPDB_PROTOCOL";           break;
    case CAPDB_EMPTY:              zVal = "CAPDB_EMPTY";              break;
    case CAPDB_SCHEMA:             zVal = "CAPDB_SCHEMA";             break;
    case CAPDB_TOOBIG:             zVal = "CAPDB_TOOBIG";             break;
    case CAPDB_CONSTRAINT:         zVal = "CAPDB_CONSTRAINT";         break;
    case CAPDB_MISMATCH:           zVal = "CAPDB_MISMATCH";           break;
    case CAPDB_MISUSE:             zVal = "CAPDB_MISUSE";             break;
    case CAPDB_NOLFS:              zVal = "CAPDB_NOLFS";              break;
    case CAPDB_IOERR_READ:         zVal = "CAPDB_IOERR_READ";         break;
    case CAPDB_IOERR_SHORT_READ:   zVal = "CAPDB_IOERR_SHORT_READ";   break;
    case CAPDB_IOERR_WRITE:        zVal = "CAPDB_IOERR_WRITE";        break;
    case CAPDB_IOERR_FSYNC:        zVal = "CAPDB_IOERR_FSYNC";        break;
    case CAPDB_IOERR_DIR_FSYNC:    zVal = "CAPDB_IOERR_DIR_FSYNC";    break;
    case CAPDB_IOERR_TRUNCATE:     zVal = "CAPDB_IOERR_TRUNCATE";     break;
    case CAPDB_IOERR_FSTAT:        zVal = "CAPDB_IOERR_FSTAT";        break;
    case CAPDB_IOERR_UNLOCK:       zVal = "CAPDB_IOERR_UNLOCK";       break;
    case CAPDB_IOERR_RDLOCK:       zVal = "CAPDB_IOERR_RDLOCK";       break;
    case CAPDB_IOERR_DELETE:       zVal = "CAPDB_IOERR_DELETE";       break;
    case CAPDB_IOERR_BLOCKED:      zVal = "CAPDB_IOERR_BLOCKED";      break;
    case CAPDB_IOERR_NOMEM:        zVal = "CAPDB_IOERR_NOMEM";        break;
    case CAPDB_IOERR_ACCESS:       zVal = "CAPDB_IOERR_ACCESS";       break;
    case CAPDB_IOERR_CHECKRESERVEDLOCK:
                               zVal = "CAPDB_IOERR_CHECKRESERVEDLOCK"; break;
    case CAPDB_IOERR_LOCK:         zVal = "CAPDB_IOERR_LOCK";         break;
    case CAPDB_IOERR_CLOSE:        zVal = "CAPDB_IOERR_CLOSE";        break;
    case CAPDB_IOERR_DIR_CLOSE:    zVal = "CAPDB_IOERR_DIR_CLOSE";    break;
    case CAPDB_IOERR_SHMOPEN:      zVal = "CAPDB_IOERR_SHMOPEN";      break;
    case CAPDB_IOERR_SHMSIZE:      zVal = "CAPDB_IOERR_SHMSIZE";      break;
    case CAPDB_IOERR_SHMLOCK:      zVal = "CAPDB_IOERR_SHMLOCK";      break;
    case CAPDB_IOERR_SHMMAP:       zVal = "CAPDB_IOERR_SHMMAP";       break;
    case CAPDB_IOERR_SEEK:         zVal = "CAPDB_IOERR_SEEK";         break;
    case CAPDB_IOERR_GETTEMPPATH:  zVal = "CAPDB_IOERR_GETTEMPPATH";  break;
    case CAPDB_IOERR_CONVPATH:     zVal = "CAPDB_IOERR_CONVPATH";     break;
    case CAPDB_READONLY_DBMOVED:   zVal = "CAPDB_READONLY_DBMOVED";   break;
    case CAPDB_LOCKED_SHAREDCACHE: zVal = "CAPDB_LOCKED_SHAREDCACHE"; break;
    case CAPDB_BUSY_RECOVERY:      zVal = "CAPDB_BUSY_RECOVERY";      break;
    case CAPDB_CANTOPEN_NOTEMPDIR: zVal = "CAPDB_CANTOPEN_NOTEMPDIR"; break;
  }
  return zVal;
}
...
static void vfstrace_print_errcode(
  ...
  if( zVal==0 ){
    ...
    if( zVal ){
      capdb_snprintf(sizeof(zBuf), zBuf, "%s | 0x%x", zVal, rc&0xffff00);
    }else{
      capdb_snprintf(sizeof(zBuf), zBuf, "%d (0x%x)", rc, rc);
    }
    zVal = zBuf;
  }
...
*/
static int vfstraceClose(capdb_file *pFile){
  ...
  if( rc==CAPDB_OK ){
    capdb_free((void*)p->base.pMethods);
    p->base.pMethods = 0;
  }
  return rc;
}
...
*/
static int vfstraceTruncate(capdb_file *pFile, sqlite_int64 size){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
*/
static int vfstraceSync(capdb_file *pFile, int flags){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
  int i;
  ...
  memcpy(zBuf, "|0", 3);
  i = 0;
  if( flags & CAPDB_SYNC_FULL )        strappend(zBuf, &i, "|FULL");
  else if( flags & CAPDB_SYNC_NORMAL ) strappend(zBuf, &i, "|NORMAL");
  if( flags & CAPDB_SYNC_DATAONLY )    strappend(zBuf, &i, "|DATAONLY");
  if( flags & ~(CAPDB_SYNC_FULL|CAPDB_SYNC_DATAONLY) ){
    capdb_snprintf(sizeof(zBuf)-i, &zBuf[i], "|0x%x", flags);
  }
...
*/
static int vfstraceFileSize(capdb_file *pFile, sqlite_int64 *pSize){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
*/
static int vfstraceLock(capdb_file *pFile, int eLock){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
*/
static int vfstraceUnlock(capdb_file *pFile, int eLock){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
*/
static int vfstraceCheckReservedLock(capdb_file *pFile, int *pResOut){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
*/
static int vfstraceFileControl(capdb_file *pFile, int op, void *pArg){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
  char zBuf[100];
  ...
  switch( op ){
    case CAPDB_FCNTL_LOCKSTATE:           zOp = "LOCKSTATE";           break;
    case CAPDB_GET_LOCKPROXYFILE:         zOp = "GET_LOCKPROXYFILE";   break;
    case CAPDB_SET_LOCKPROXYFILE:         zOp = "SET_LOCKPROXYFILE";   break;
    case CAPDB_LAST_ERRNO:                zOp = "LAST_ERRNO";          break;
    case CAPDB_FCNTL_SIZE_HINT: {
      capdb_snprintf(sizeof(zBuf), zBuf, "SIZE_HINT,%lld",
                       *(capdb_int64*)pArg);
      zOp = zBuf;
      break;
    }
    case CAPDB_FCNTL_CHUNK_SIZE: {
      capdb_snprintf(sizeof(zBuf), zBuf, "CHUNK_SIZE,%d", *(int*)pArg);
      zOp = zBuf;
      break;
    }
    case CAPDB_FCNTL_FILE_POINTER:        zOp = "FILE_POINTER";        break;
    case CAPDB_FCNTL_WIN32_AV_RETRY:      zOp = "WIN32_AV_RETRY";      break;
    case CAPDB_FCNTL_PERSIST_WAL: {
       capdb_snprintf(sizeof(zBuf), zBuf, "PERSIST_WAL,%d", *(int*)pArg);
       zOp = zBuf;
       break;
    }
    case CAPDB_FCNTL_OVERWRITE:           zOp = "OVERWRITE";           break;
    case CAPDB_FCNTL_VFSNAME:             zOp = "VFSNAME";             break;
    case CAPDB_FCNTL_POWERSAFE_OVERWRITE: zOp = "POWERSAFE_OVERWRITE"; break;
    case CAPDB_FCNTL_PRAGMA: {
      const char *const* a = (const char*const*)pArg;
      if( a[1] && strcmp(a[1],"vfstrace")==0 && a[2] ){
        const u8 *zArg = (const u8*)a[2];
        if( zArg[0]>='0' && zArg[0]<='9' ){
          pInfo->mTrace = (capdb_uint64)strtoll(a[2], 0, 0);
        }else{
          static const struct {
            const char *z;
            unsigned int m;
          ...
          while( zArg[0] ){
            ...
            for(jj=0; jj<(int)(sizeof(aKw)/sizeof(aKw[0])); jj++){
              if( capdb_strnicmp(aKw[jj].z,(const char*)zArg,n)==0 ){
                if( onOff ){
      ...
      capdb_snprintf(sizeof(zBuf), zBuf, "PRAGMA,[%s,%s]",a[1],a[2]);
      zOp = zBuf;
      break;
    }
    case CAPDB_FCNTL_BUSYHANDLER:         zOp = "BUSYHANDLER";         break;
    case CAPDB_FCNTL_TEMPFILENAME:        zOp = "TEMPFILENAME";        break;
    case CAPDB_FCNTL_MMAP_SIZE: {
      capdb_int64 iMMap = *(capdb_int64*)pArg;
      capdb_snprintf(sizeof(zBuf), zBuf, "MMAP_SIZE,%lld",iMMap);
      zOp = zBuf;
      break;
    }
    case CAPDB_FCNTL_TRACE:               zOp = "TRACE";               break;
    case CAPDB_FCNTL_HAS_MOVED:           zOp = "HAS_MOVED";           break;
    case CAPDB_FCNTL_SYNC:                zOp = "SYNC";                break;
    case CAPDB_FCNTL_COMMIT_PHASETWO:     zOp = "COMMIT_PHASETWO";     break;
    case CAPDB_FCNTL_WIN32_SET_HANDLE:    zOp = "WIN32_SET_HANDLE";    break;
    case CAPDB_FCNTL_WAL_BLOCK:           zOp = "WAL_BLOCK";           break;
    case CAPDB_FCNTL_ZIPVFS:              zOp = "ZIPVFS";              break; 
    case CAPDB_FCNTL_RBU:                 zOp = "RBU";                 break;
    case CAPDB_FCNTL_VFS_POINTER:         zOp = "VFS_POINTER";         break;
    case CAPDB_FCNTL_JOURNAL_POINTER:     zOp = "JOURNAL_POINTER";     break;
    case CAPDB_FCNTL_WIN32_GET_HANDLE:    zOp = "WIN32_GET_HANDLE";    break;
    case CAPDB_FCNTL_PDB:                 zOp = "PDB";                 break;
    case CAPDB_FCNTL_BEGIN_ATOMIC_WRITE:  zOp = "BEGIN_ATOMIC_WRITE";  break;
    case CAPDB_FCNTL_COMMIT_ATOMIC_WRITE: zOp = "COMMIT_ATOMIC_WRITE"; break;
    case CAPDB_FCNTL_ROLLBACK_ATOMIC_WRITE: {
       zOp = "ROLLBACK_ATOMIC_WRITE";
       break;
    }
    case CAPDB_FCNTL_LOCK_TIMEOUT: {
       capdb_snprintf(sizeof(zBuf), zBuf, "LOCK_TIMEOUT,%d", *(int*)pArg);
       zOp = zBuf;
       break;
    }
    case CAPDB_FCNTL_DATA_VERSION:        zOp = "DATA_VERSION";        break;
    case CAPDB_FCNTL_SIZE_LIMIT:          zOp = "SIZE_LIMIT";          break;
    case CAPDB_FCNTL_CKPT_DONE:           zOp = "CKPT_DONE";           break;
    case CAPDB_FCNTL_RESERVE_BYTES:       zOp = "RESERVED_BYTES";      break;
    case CAPDB_FCNTL_CKPT_START:          zOp = "CKPT_START";          break;
    case CAPDB_FCNTL_EXTERNAL_READER:     zOp = "EXTERNAL_READER";     break;
    case CAPDB_FCNTL_CKSM_FILE:           zOp = "CKSM_FILE";           break;
    case CAPDB_FCNTL_RESET_CACHE:         zOp = "RESET_CACHE";         break;
    case 0xca093fa0:                       zOp = "DB_UNCHANGED";        break;
    default: {
      capdb_snprintf(sizeof zBuf, zBuf, "%d", op);
      zOp = zBuf;
      break;
    }
  }
  ...
  if( rc==CAPDB_OK ){
    switch( op ){
      case CAPDB_FCNTL_VFSNAME: {
        *(char**)pArg = capdb_mprintf("vfstrace.%s/%z",
                                    pInfo->zVfsName, *(char**)pArg);
        zRVal = *(char**)pArg;
        break;
      }
      case CAPDB_FCNTL_MMAP_SIZE: {
        capdb_snprintf(sizeof(zBuf2), zBuf2, "%lld", *(capdb_int64*)pArg);
        zRVal = zBuf2;
        break;
      }
      case CAPDB_FCNTL_HAS_MOVED:
      case CAPDB_FCNTL_PERSIST_WAL: {
        capdb_snprintf(sizeof(zBuf2), zBuf2, "%d", *(int*)pArg);
        zRVal = zBuf2;
        break;
      }
      case CAPDB_FCNTL_PRAGMA:
...
*/
static int vfstraceSectorSize(capdb_file *pFile){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
*/
static int vfstraceDeviceCharacteristics(capdb_file *pFile){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
*/
static int vfstraceShmLock(capdb_file *pFile, int ofst, int n, int flags){
  ...
  memcpy(zLck, "|0", 3);
  if( flags & CAPDB_SHM_UNLOCK )    strappend(zLck, &i, "|UNLOCK");
  if( flags & CAPDB_SHM_LOCK )      strappend(zLck, &i, "|LOCK");
  if( flags & CAPDB_SHM_SHARED )    strappend(zLck, &i, "|SHARED");
  if( flags & CAPDB_SHM_EXCLUSIVE ) strappend(zLck, &i, "|EXCLUSIVE");
  if( flags & ~(0xf) ){
     capdb_snprintf(sizeof(zLck)-i, &zLck[i], "|0x%x", flags);
  }
...
static void vfstraceShmBarrier(capdb_file *pFile){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
...
static int vfstraceShmUnmap(capdb_file *pFile, int delFlag){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
static int vfstraceFetch(capdb_file *pFile, i64 iOff, int nAmt, void **pptr){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
...
static int vfstraceUnfetch(capdb_file *pFile, i64 iOff, void *ptr){
  vfstrace_file *p = (vfstrace_file *)pFile;
  vfstrace_info *pInfo = p->pInfo;
  int rc;
  vfstraceOnOff(pInfo, VTR_FETCH);
...
static int vfstraceOpen(
  ...
  capdb_vfs *pRoot = pInfo->pRootVfs;
  p->pInfo = pInfo;
  ...
  if( p->pReal->pMethods ){
    capdb_io_methods *pNew = capdb_malloc64( sizeof(*pNew) );
    const capdb_io_methods *pSub = p->pReal->pMethods;
...
** returning.
*/
static int vfstraceDelete(capdb_vfs *pVfs, const char *zPath, int dirSync){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  capdb_vfs *pRoot = pInfo->pRootVfs;
  int rc;
...
*/
static void *vfstraceDlOpen(capdb_vfs *pVfs, const char *zPath){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  capdb_vfs *pRoot = pInfo->pRootVfs;
...
*/
static void vfstraceDlError(capdb_vfs *pVfs, int nByte, char *zErrMsg){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  capdb_vfs *pRoot = pInfo->pRootVfs;
...
*/
static void (*vfstraceDlSym(capdb_vfs *pVfs,void *p,const char *zSym))(void){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  capdb_vfs *pRoot = pInfo->pRootVfs;
...
*/
static void vfstraceDlClose(capdb_vfs *pVfs, void *pHandle){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  capdb_vfs *pRoot = pInfo->pRootVfs;
...
** random data.
*/
static int vfstraceRandomness(capdb_vfs *pVfs, int nByte, char *zBufOut){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  capdb_vfs *pRoot = pInfo->pRootVfs;
...
** actually slept.
*/
static int vfstraceSleep(capdb_vfs *pVfs, int nMicro){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  capdb_vfs *pRoot = pInfo->pRootVfs;
...
*/
static int vfstraceCurrentTime(capdb_vfs *pVfs, double *pTimeOut){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  capdb_vfs *pRoot = pInfo->pRootVfs;
  int rc;
...
static int vfstraceCurrentTimeInt64(capdb_vfs *pVfs, capdb_int64 *pTimeOut){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  capdb_vfs *pRoot = pInfo->pRootVfs;
  int rc;
  vfstraceOnOff(pInfo, VTR_CURTIME);
...
*/
static int vfstraceGetLastError(capdb_vfs *pVfs, int nErr, char *zErr){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  capdb_vfs *pRoot = pInfo->pRootVfs;
  int rc;
...
static int vfstraceSetSystemCall(
  capdb_vfs *pVfs,
  const char *zName,
  capdb_syscall_ptr pFunc
){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  capdb_vfs *pRoot = pInfo->pRootVfs;
  return pRoot->xSetSystemCall(pRoot, zName, pFunc);
}
static capdb_syscall_ptr vfstraceGetSystemCall(
  capdb_vfs *pVfs,
  const char *zName
){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
...
static const char *vfstraceNextSystemCall(capdb_vfs *pVfs, const char *zName){
  vfstrace_info *pInfo = (vfstrace_info*)pVfs->pAppData;
  capdb_vfs *pRoot = pInfo->pRootVfs;
...
**
** CAPDB_NOMEM is returned in the case of a memory allocation error.
** CAPDB_NOTFOUND is returned if zOldVfsName does not exist.
*/
int vfstrace_register(
   const char *zTraceName,           /* Name of the newly constructed VFS */
   const char *zOldVfsName,          /* Name of the underlying VFS */
  ...
  pRoot = capdb_vfs_find(zOldVfsName);
  if( pRoot==0 ) return CAPDB_NOTFOUND;
  nName = strlen(zTraceName);
  nByte = sizeof(*pNew) + sizeof(*pInfo) + nName + 1;
  pNew = capdb_malloc64( nByte );
  ...
  return capdb_vfs_register(pNew, makeDefault);
}
...
void vfstrace_unregister(const char *zTraceName){
  capdb_vfs *pVfs = capdb_vfs_find(zTraceName);
  if( pVfs==0 ) return;
