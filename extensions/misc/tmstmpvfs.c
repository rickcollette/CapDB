...
** into your application.  If both this file and "capdb.c" are statically
** linked, and if "capdb.c" is compiled with an option like:
**
**       -DCAPDB_EXTRA_INIT=capdb_register_tmstmpvfs
**
...
**     capdb_load_extension(db, "./tmstmpvfs");
**     capdb_close(db);
**
...
** the results from CAPDB_FCNTL_VFSNAME (or the .vfsname command in
...
** source tree.
*/
#if defined(CAPDB_AMALGAMATION) && !defined(CAPDB_TMSTMPVFS_STATIC)
# define CAPDB_TMSTMPVFS_STATIC
#endif
#ifdef CAPDB_TMSTMPVFS_STATIC
# include "capdb.h"
#else
# include "capdbext.h"
...
*/
typedef struct capdb_vfs TmstmpVfs;
...
#define ORIGFILE(p) ((capdb_file*)(((TmstmpFile*)(p))+1))
...
struct TmstmpFile {
  ...
  capdb_int64 iOfst;   /* Offset of last WAL frame header */
  capdb_vfs *pSubVfs;  /* Underlying VFS */
};
...
*/
static int tmstmpClose(capdb_file*);
static int tmstmpRead(capdb_file*, void*, int iAmt, capdb_int64 iOfst);
static int tmstmpWrite(capdb_file*,const void*,int iAmt, capdb_int64 iOfst);
static int tmstmpTruncate(capdb_file*, capdb_int64 size);
static int tmstmpSync(capdb_file*, int flags);
static int tmstmpFileSize(capdb_file*, capdb_int64 *pSize);
static int tmstmpLock(capdb_file*, int);
static int tmstmpUnlock(capdb_file*, int);
static int tmstmpCheckReservedLock(capdb_file*, int *pResOut);
static int tmstmpFileControl(capdb_file*, int op, void *pArg);
static int tmstmpSectorSize(capdb_file*);
static int tmstmpDeviceCharacteristics(capdb_file*);
static int tmstmpShmMap(capdb_file*, int iPg, int pgsz, int, void volatile**);
static int tmstmpShmLock(capdb_file*, int offset, int n, int flags);
static void tmstmpShmBarrier(capdb_file*);
static int tmstmpShmUnmap(capdb_file*, int deleteFlag);
static int tmstmpFetch(capdb_file*, capdb_int64 iOfst, int iAmt, void **pp);
static int tmstmpUnfetch(capdb_file*, capdb_int64 iOfst, void *p);

/*
** Methods for TmstmpVfs
*/
static int tmstmpOpen(capdb_vfs*, const char *, capdb_file*, int , int *);
static int tmstmpDelete(capdb_vfs*, const char *zName, int syncDir);
static int tmstmpAccess(capdb_vfs*, const char *zName, int flags, int *);
static int tmstmpFullPathname(capdb_vfs*, const char *zName, int, char *zOut);
static void *tmstmpDlOpen(capdb_vfs*, const char *zFilename);
static void tmstmpDlError(capdb_vfs*, int nByte, char *zErrMsg);
static void (*tmstmpDlSym(capdb_vfs *pVfs, void *p, const char*zSym))(void);
static void tmstmpDlClose(capdb_vfs*, void*);
static int tmstmpRandomness(capdb_vfs*, int nByte, char *zOut);
static int tmstmpSleep(capdb_vfs*, int microseconds);
static int tmstmpCurrentTime(capdb_vfs*, double*);
static int tmstmpGetLastError(capdb_vfs*, int, char *);
static int tmstmpCurrentTimeInt64(capdb_vfs*, capdb_int64*);
static int tmstmpSetSystemCall(capdb_vfs*, const char*,capdb_syscall_ptr);
static capdb_syscall_ptr tmstmpGetSystemCall(capdb_vfs*, const char *z);
static const char *tmstmpNextSystemCall(capdb_vfs*, const char *zName);

static capdb_vfs tmstmp_vfs = {
...
static const capdb_io_methods tmstmp_io_methods = {
  3,                            /* iVersion */
  tmstmpClose,                  /* xClose */
  tmstmpRead,                   /* xRead */
  tmstmpWrite,                  /* xWrite */
...
static void tmstmpPutTS(TmstmpFile *p, unsigned char *aOut){
  capdb_uint64 tm = 0;
  p->pSubVfs->xCurrentTimeInt64(p->pSubVfs, (capdb_int64*)&tm);
...
** Close a connection
*/
static int tmstmpClose(capdb_file *pFile){
...
static int tmstmpWrite(
  ...
  TmstmpFile *p = (TmstmpFile*)pFile;
  capdb_file *pSub = ORIGFILE(pFile);
  if( !p->hasCorrectReserve ){
...
*/
static int tmstmpTruncate(capdb_file *pFile, sqlite_int64 size){
  pFile = ORIGFILE(pFile);
...
** Sync a tmstmp-file.
*/
static int tmstmpSync(capdb_file *pFile, int flags){
  pFile = ORIGFILE(pFile);
...
*/
static int tmstmpFileSize(capdb_file *pFile, sqlite_int64 *pSize){
  TmstmpFile *p = (TmstmpFile *)pFile;
  pFile = ORIGFILE(p);
...
** Lock a tmstmp-file.
*/
static int tmstmpLock(capdb_file *pFile, int eLock){
  pFile = ORIGFILE(pFile);
...
*/
static int tmstmpUnlock(capdb_file *pFile, int eLock){
  pFile = ORIGFILE(pFile);
...
*/
static int tmstmpCheckReservedLock(capdb_file *pFile, int *pResOut){
  pFile = ORIGFILE(pFile);
...
*/
static int tmstmpFileControl(capdb_file *pFile, int op, void *pArg){
  int rc;
  TmstmpFile *p = (TmstmpFile*)pFile;
  pFile = ORIGFILE(pFile);
  ...
  switch( op ){
    case CAPDB_FCNTL_VFSNAME: {
      if( p->hasCorrectReserve && rc==CAPDB_OK ){
        *(char**)pArg = capdb_mprintf("tmstmp/%z", *(char**)pArg);
      }
      break;
    }
    case CAPDB_FCNTL_CKPT_START: {
      p->inCkpt = 1;
      assert( p->isDb );
      assert( p->pPartner!=0 );
      p->pPartner->inCkpt = 1;
...
*/
static int tmstmpSectorSize(capdb_file *pFile){
  pFile = ORIGFILE(pFile);
...
*/
static int tmstmpDeviceCharacteristics(capdb_file *pFile){
  int devchar = 0;
  pFile = ORIGFILE(pFile);
...
static int tmstmpShmLock(capdb_file *pFile, int offset, int n, int flags){
  pFile = ORIGFILE(pFile);
  return pFile->pMethods->xShmLock(pFile,offset,n,flags);
}

/* Memory barrier operation on shared memory */
static void tmstmpShmBarrier(capdb_file *pFile){
  pFile = ORIGFILE(pFile);
  pFile->pMethods->xShmBarrier(pFile);
}

/* Unmap a shared memory segment */
static int tmstmpShmUnmap(capdb_file *pFile, int deleteFlag){
  pFile = ORIGFILE(pFile);
...
static int tmstmpUnfetch(capdb_file *pFile, capdb_int64 iOfst, void *pPage){
  pFile = ORIGFILE(pFile);
...
static int tmstmpOpen(
  ...
  if( (flags & CAPDB_OPEN_WAL)!=0 ){
    pDb = (TmstmpFile*)capdb_database_file_object(zName);
    if( pDb==0
  ...
  if( pDb!=0 ){
    ...
    u32 pid;
    TmstmpLog *pLog;
    capdb_uint64 r1;         /* Milliseconds since 1970-01-01 */
    capdb_uint64 days;       /* Days since 1970-01-01 */
    capdb_uint64 sod;        /* Start of date specified by r1 */
    capdb_uint64 z;          /* Days since 0000-03-01 */
    capdb_uint64 era;        /* 400-year era */
    int h;                     /* hour */
    int m;                     /* minute */
    int s;                     /* second */
    int f;                     /* millisecond */
    ...
    p->isDb = 1;
    r1 = 0;
    pLog = capdb_malloc64( sizeof(TmstmpLog) );
    if( pLog==0 ){
    ...
    p->pLog = pLog;
    p->pSubVfs->xCurrentTimeInt64(p->pSubVfs, (capdb_int64*)&r1);
    ...
    M = mp + (mp<10 ? 3 : -9);
    Y = y + (M <=2);
    capdb_randomness(sizeof(r2), &r2);
    pid = GETPID;
    pLog->zLogname = capdb_mprintf(
...
*/
static int tmstmpDelete(capdb_vfs *p, const char *zName, int syncDir){
  capdb_vfs *pSub = ORIGVFS(p);
  return pSub->xDelete(pSub,zName,syncDir);
}
static int tmstmpAccess(capdb_vfs *p, const char *zName, int flags, int *pR){
  capdb_vfs *pSub = ORIGVFS(p);
  return pSub->xAccess(pSub,zName,flags,pR);
}
static int tmstmpFullPathname(capdb_vfs*p,const char *zName,int n,char *zOut){
  capdb_vfs *pSub = ORIGVFS(p);
  return pSub->xFullPathname(pSub,zName,n,zOut);
}
static void *tmstmpDlOpen(capdb_vfs *p, const char *zFilename){
  capdb_vfs *pSub = ORIGVFS(p);
  return pSub->xDlOpen(pSub,zFilename);
}
static void tmstmpDlError(capdb_vfs *p, int nByte, char *zErrMsg){
  capdb_vfs *pSub = ORIGVFS(p);
  return pSub->xDlError(pSub,nByte,zErrMsg);
}
static void(*tmstmpDlSym(capdb_vfs *p, void *pDl, const char *zSym))(void){
  capdb_vfs *pSub = ORIGVFS(p);
  return pSub->xDlSym(pSub,pDl,zSym);
}
static void tmstmpDlClose(capdb_vfs *p, void *pDl){
  capdb_vfs *pSub = ORIGVFS(p);
  return pSub->xDlClose(pSub,pDl);
}
static int tmstmpRandomness(capdb_vfs *p, int nByte, char *zOut){
  capdb_vfs *pSub = ORIGVFS(p);
  return pSub->xRandomness(pSub,nByte,zOut);
}
static int tmstmpSleep(capdb_vfs *p, int microseconds){
  capdb_vfs *pSub = ORIGVFS(p);
  return pSub->xSleep(pSub,microseconds);
}
static int tmstmpCurrentTime(capdb_vfs *p, double *prNow){
  capdb_vfs *pSub = ORIGVFS(p);
  return pSub->xCurrentTime(pSub,prNow);
}
static int tmstmpGetLastError(capdb_vfs *p, int a, char *b){
  capdb_vfs *pSub = ORIGVFS(p);
  return pSub->xGetLastError(pSub,a,b);
}
static int tmstmpCurrentTimeInt64(capdb_vfs *p, capdb_int64 *piNow){
  capdb_vfs *pSub = ORIGVFS(p);
  return pSub->xCurrentTimeInt64(pSub,piNow);
}
static int tmstmpSetSystemCall(capdb_vfs *p, const char *zName,
                               capdb_syscall_ptr x){
  capdb_vfs *pSub = ORIGVFS(p);
  return pSub->xSetSystemCall(pSub,zName,x);
}
static capdb_syscall_ptr tmstmpGetSystemCall(capdb_vfs *p, const char *z){
  capdb_vfs *pSub = ORIGVFS(p);
  return pSub->xGetSystemCall(pSub,z);
}
static const char *tmstmpNextSystemCall(capdb_vfs *p, const char *zName){
  capdb_vfs *pSub = ORIGVFS(p);
...
static int tmstmpRegisterVfs(void){
  ...
  if( pOrig==&tmstmp_vfs ) return CAPDB_OK;
  tmstmp_vfs.iVersion = pOrig->iVersion;
  tmstmp_vfs.pAppData = pOrig;
  tmstmp_vfs.szOsFile = pOrig->szOsFile + sizeof(TmstmpFile);
  rc = capdb_vfs_register(&tmstmp_vfs, 1);
  return rc;
}

#if defined(CAPDB_TMSTMPVFS_STATIC)
/* This variant of the initializer runs when the extension is
** statically linked.
*/
int capdb_register_tmstmpvfs(const char *NotUsed){
  (void)NotUsed;
  return tmstmpRegisterVfs();
}
int capdb_unregister_tmstmpvfs(void){
  if( capdb_vfs_find("tmstmpvfs") ){
    capdb_vfs_unregister(&tmstmp_vfs);
  }
  return CAPDB_OK;
}
#endif /* defined(CAPDB_TMSTMPVFS_STATIC */
