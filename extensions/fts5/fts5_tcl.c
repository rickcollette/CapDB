...
#endif
extern int capdbFts5TestRegisterMatchinfo(capdb*);
extern int capdbFts5TestRegisterTok(capdb*, fts5_api*);
...
*/
static int f5tDbPointer(Tcl_Interp *interp, Tcl_Obj *pObj, capdb **ppDb){
  struct SqliteDb *p;
  Tcl_CmdInfo cmdInfo;
...
static int f5tResultToErrorCode(const char *zRes){
  ...
  for(i=0; i<sizeof(aErr)/sizeof(aErr[0]); i++){
    if( 0==capdb_stricmp(zRes, aErr[i].zError) ){
      return aErr[i].rc;
    }
  }
...
static int CAPDB_TCLAPI f5tDbAndApi(
  Tcl_Interp *interp, 
  Tcl_Obj *pObj, 
  capdb **ppDb, 
  fts5_api **ppApi
  ...
  if( rc!=TCL_OK ){
    ...
    rc = capdb_prepare_v2(db, "SELECT fts5(?1)", -1, &pStmt, 0);
    if( rc!=CAPDB_OK ){
      Tcl_AppendResult(interp, "error: ", capdb_errmsg(db), (char*)0);
      return TCL_ERROR;
    }
    capdb_bind_pointer(pStmt, 1, (void*)&pApi, "fts5_api_ptr", 0);
    capdb_step(pStmt);

    if( capdb_finalize(pStmt)!=CAPDB_OK ){
      Tcl_AppendResult(interp, "error: ", capdb_errmsg(db), (char*)0);
      return TCL_ERROR;
    }
...
** tcl object, the code in this file wraps it in an capdb_malloc'd 
...
static int CAPDB_TCLAPI xF5tApi(void*, Tcl_Interp*, int, Tcl_Obj *CONST []);
...
** Description...
*/
static int CAPDB_TCLAPI xF5tApi(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
#define CASE(i,str) case i: assert( strcmp(aSub[i].zName, str)==0 );
  switch( iSub ){
    ...
    CASE(8, "xRowid") {
      capdb_int64 iRowid = p->pApi->xRowid(p->pFts);
    ...
    CASE(12, "xSetAuxdata") {
      F5tAuxData *pData = (F5tAuxData*)capdb_malloc64(sizeof(F5tAuxData));
      if( pData==0 ){
...
static void xF5tFunction(
  ...
  capdb_context *pCtx,          /* Context for returning result/error */
  int nVal,                       /* Number of values in apVal[] array */
  capdb_value **apVal           /* Array of trailing arguments */
){
  ...
  for(i=0; i<nVal; i++){
    Tcl_Obj *pObj = 0;
    switch( capdb_value_type(apVal[i]) ){
      case CAPDB_TEXT:
        pObj = Tcl_NewStringObj((const char*)capdb_value_text(apVal[i]), -1);
        break;
      case CAPDB_BLOB:
        pObj = Tcl_NewByteArrayObj(
            capdb_value_blob(apVal[i]), capdb_value_bytes(apVal[i])
        );
        break;
      case CAPDB_INTEGER:
        pObj = Tcl_NewWideIntObj(capdb_value_int64(apVal[i]));
        break;
      case CAPDB_FLOAT:
        pObj = Tcl_NewDoubleObj(capdb_value_double(apVal[i]));
        break;
      default:
        pObj = Tcl_NewObj();
        break;
  ...
  if( rc!=TCL_OK ){
    capdb_result_error(pCtx, Tcl_GetStringResult(p->interp), -1);
  }else{
    ...
    if( c=='b' && strcmp(zType,"bytearray")==0 && pVar->bytes==0 ){
      ...
      ** has no string representation. */
      ...
      capdb_result_blob(pCtx, data, (int)nn, CAPDB_TRANSIENT);
    }else if( c=='b' && strcmp(zType,"boolean")==0 ){
      int n;
      Tcl_GetIntFromObj(0, pVar, &n);
      capdb_result_int(pCtx, n);
      ...
      capdb_result_text(pCtx, (char*)data, (int)nn, CAPDB_TRANSIENT);
    }
  }
}
...
/*
**      capdb_fts5_create_function DB NAME SCRIPT
**
** Description...
*/
static int CAPDB_TCLAPI f5tCreateFunction(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
/*
**      capdb_fts5_tokenize DB TOKENIZER TEXT
**
** Description...
*/
static int CAPDB_TCLAPI f5tTokenize(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  if( nText>0 ){
    pCopy = capdb_malloc64(nText);
    if( pCopy==0 ){
      tokenizer.xDelete(pTok);
      Tcl_AppendResult(interp, "error in capdb_malloc()", (char*)0);
      return TCL_ERROR;
    }else{
...
**   script using [capdb_fts5_locale].
*/
...
** capdb_fts5_locale 
*/
static int CAPDB_TCLAPI f5tTokenizerLocale(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  if( p->xToken==0 ){
    Tcl_AppendResult(interp, 
        "capdb_fts5_locale may only be used by tokenizer callback", (char*)0
    );
    return TCL_ERROR;
  }
...
/*
** capdb_fts5_token ?-colocated? TEXT START END
*/
static int CAPDB_TCLAPI f5tTokenizerReturn(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  if( p->xToken==0 ){
    Tcl_AppendResult(interp, 
        "capdb_fts5_token may only be used by tokenizer callback", (char*)0
    );
    return TCL_ERROR;
  }
...
/*
**      capdb_fts5_create_tokenizer DB NAME SCRIPT
**
...
** text to tokenize.
**
** SCRIPT2 should invoke the [capdb_fts5_token] command once for each
** token within the tokenized text.
*/
static int CAPDB_TCLAPI f5tCreateTokenizer(
...
static void CAPDB_TCLAPI xF5tFree(ClientData clientData){
  ckfree(clientData);
}

/*
**      capdb_fts5_may_be_corrupt BOOLEAN
**
** Set or clear the global "may-be-corrupt" flag. Return the old value.
*/
static int CAPDB_TCLAPI f5tMayBeCorrupt(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
#ifdef CAPDB_DEBUG
  int bOld = capdb_fts5_may_be_corrupt;
  ...
  if( objc==2 ){
    ...
    capdb_fts5_may_be_corrupt = bNew;
  }
...
static int CAPDB_TCLAPI f5tTokenHash(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
...
static int CAPDB_TCLAPI f5tRegisterMatchinfo(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  rc = capdbFts5TestRegisterMatchinfo(db);
  if( rc!=CAPDB_OK ){
    Tcl_SetResult(interp, (char*)capdbErrName(rc), TCL_VOLATILE);
    return TCL_ERROR;
  }
...
static int CAPDB_TCLAPI f5tRegisterTok(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  rc = capdbFts5TestRegisterTok(db, pApi);
  if( rc!=CAPDB_OK ){
    Tcl_SetResult(interp, (char*)capdbErrName(rc), TCL_VOLATILE);
    return TCL_ERROR;
  }
...
static int f5tOrigintextCreate(
  ...
  pTok = (OriginTextTokenizer*)capdb_malloc64(sizeof(OriginTextTokenizer));
  if( pTok==0 ){
    rc = CAPDB_NOMEM;
  }else if( nArg<1 ){
    rc = CAPDB_ERROR;
...
static int xOriginToken(
  ...
  if( nToken==(iEnd-iStart) && 0==memcmp(pToken, &p->pText[iStart], nToken) ){
    ...
    if( nReq>p->nBuf ){
      capdb_free(p->aBuf);
      p->aBuf = capdb_malloc64(nReq*2);
      if( p->aBuf==0 ) return CAPDB_NOMEM;
      p->nBuf = nReq*2;
    }
...
/*
**      capdb_fts5_register_origintext DB
**
** Description...
*/
static int CAPDB_TCLAPI f5tRegisterOriginText(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  Tcl_ResetResult(interp);
  if( rc!=CAPDB_OK ){
    Tcl_AppendResult(interp, "error: ", capdb_errmsg(db), (void*)0);
    return TCL_ERROR;
  }
  return TCL_OK;
}
...
**
** CAPDB_OK is returned if the table is successfully dropped. Or, if an
** error occurs, an SQLite error code.
*/
static int capdb_fts5_drop_corrupt_table(
  ...
  rc = capdb_db_config(db, CAPDB_DBCONFIG_DEFENSIVE, -1, &bDef);
  if( rc==CAPDB_OK ){
    char *zScript = capdb_mprintf(
    ...
    if( zScript==0 ){
      ...
      if( bDef ) capdb_db_config(db, CAPDB_DBCONFIG_DEFENSIVE, 0, 0);
      rc = capdb_exec(db, zScript, 0, 0, 0);
      if( bDef ) capdb_db_config(db, CAPDB_DBCONFIG_DEFENSIVE, 1, 0);
      capdb_free(zScript);
    }
  }
...
/*
**      capdb_fts5_drop_corrupt_table DB DATABASE TABLE
**
** Description...
*/
static int CAPDB_TCLAPI f5tDropCorruptTable(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  rc = capdb_fts5_drop_corrupt_table(db, zDb, zTab);
  if( rc!=CAPDB_OK ){
    Tcl_AppendResult(interp, "error: ", capdb_errmsg(db), (void*)0);
    return TCL_ERROR;
  }
...
*/
void f5tStrFunc(capdb_context *pCtx, int nArg, capdb_value **apArg){
  const char *zText = 0;
  assert( nArg==1 );

  zText = (const char*)capdb_value_text(apArg[0]);
  if( zText ){
    capdb_int64 nText = strlen(zText);
    ...
    if( zCopy==0 ){
      ...
      capdb_result_text64(pCtx, zCopy, nText, f5tFree, CAPDB_UTF8);
    }
  }
}

/*
**      capdb_fts5_register_str DB
**
...
*/
static int CAPDB_TCLAPI f5tRegisterStr(
  void * clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  capdb_create_function(db, "str", 1, CAPDB_UTF8, 0, f5tStrFunc, 0, 0);
...
int Fts5tcl_Init(Tcl_Interp *interp){
  static struct Cmd {
    ...
    { "capdb_fts5_create_tokenizer",   f5tCreateTokenizer, 1 },
    { "capdb_fts5_token",              f5tTokenizerReturn, 1 },
    { "capdb_fts5_locale",             f5tTokenizerLocale, 1 },
    { "capdb_fts5_tokenize",           f5tTokenize, 0 },
    { "capdb_fts5_create_function",    f5tCreateFunction, 0 },
    { "capdb_fts5_may_be_corrupt",     f5tMayBeCorrupt, 0 },
    { "capdb_fts5_token_hash",         f5tTokenHash, 0 },
    { "capdb_fts5_register_matchinfo", f5tRegisterMatchinfo, 0 },
    { "capdb_fts5_register_fts5tokenize", f5tRegisterTok, 0 },
    { "capdb_fts5_register_origintext",f5tRegisterOriginText, 0 },
    { "capdb_fts5_drop_corrupt_table", f5tDropCorruptTable, 0 },
    { "capdb_fts5_register_str",       f5tRegisterStr, 0 }
  };
  int i;
  F5tTokenizerContext *pContext;
