...
#ifdef CAPDB_DEBUG
int capdbFts5Corrupt() { return CAPDB_CORRUPT_VTAB; }
#endif
...
struct Fts5Index {
  ...
  */
  ...
  capdb_blob *pReader;          /* RO incr-blob open on %_data table */
  capdb_stmt *pWriter;          /* "INSERT ... %_data VALUES(?,?)" */
  capdb_stmt *pDeleter;         /* "DELETE FROM %_data ... id>=? AND id<=?" */
  capdb_stmt *pIdxWriter;       /* "INSERT ... %_idx VALUES(?,?,?,?)" */
  capdb_stmt *pIdxDeleter;      /* "DELETE FROM %_idx WHERE segid=?" */
  capdb_stmt *pIdxSelect;
  capdb_stmt *pIdxNextSelect;
  int nRead;                      /* Total number of blocks read */

  capdb_stmt *pDeleteFromIdx;

  capdb_stmt *pDataVersion;
...
static int fts5IndexCorruptRowid(Fts5Index *pIdx, i64 iRowid){
  ...
  capdbFts5ConfigErrmsg(pIdx->pConfig, 
...
**
** poslist:
**   Used by capdbFts5IterPoslist() when the poslist needs to be buffered.
...
*/
static void *fts5IdxMalloc(Fts5Index *p, capdb_int64 nByte){
  return capdbFts5MallocZero(&p->rc, nByte);
}
...
static void fts5IndexCloseReader(Fts5Index *p){
  if( p->pReader ){
    int rc;
    capdb_blob *pReader = p->pReader;
    p->pReader = 0;
    rc = capdb_blob_close(pReader);
    if( p->rc==CAPDB_OK ) p->rc = rc;
  }
}
...
static Fts5Data *fts5DataRead(Fts5Index *p, i64 iRowid){
  ...
  if( p->rc==CAPDB_OK ){
    ...
    if( p->pReader ){
      /* This call may return CAPDB_ABORT if there has been a savepoint
      ** rollback since it was last used. In this case a new blob handle
      ** is required.  */
      capdb_blob *pBlob = p->pReader;
      p->pReader = 0;
      ...
      if( rc==CAPDB_ABORT ) rc = CAPDB_OK;
    }

    /* If the blob handle is not open at this point, open it and seek 
    ** to the requested entry.  */
    if( p->pReader==0 && rc==CAPDB_OK ){
      Fts5Config *pConfig = p->pConfig;
      rc = capdb_blob_open(pConfig->db, 
          pConfig->zDb, p->zDataTbl, "block", iRowid, 0, &p->pReader
      );
    }

    /* If either of the capdb_blob_open() or capdb_blob_reopen() calls
    ** above returned CAPDB_ERROR, return CAPDB_CORRUPT_VTAB instead.
    ** All the reasons those functions might return CAPDB_ERROR - missing
    ** table, missing row, non-blob/text in block column - indicate 
    ** backing store corruption.  */
    if( rc==CAPDB_ERROR ) rc = FTS5_CORRUPT_ROWID(p, iRowid);

    if( rc==CAPDB_OK ){
      u8 *aOut = 0;               /* Read blob data into this buffer */
      i64 nByte = capdb_blob_bytes(p->pReader);
      i64 szData = (sizeof(Fts5Data) + 7) & ~7;
      i64 nAlloc = szData + nByte + FTS5_DATA_PADDING;
      pRet = (Fts5Data*)capdb_malloc64(nAlloc);
      if( pRet ){
        pRet->nn = nByte;
      ...
      if( rc==CAPDB_OK ){
        rc = capdb_blob_read(p->pReader, aOut, nByte, 0);
      }
      if( rc!=CAPDB_OK ){
        capdb_free(pRet);
        pRet = 0;
...
static int fts5IndexPrepareStmt(
  ...
  if( p->rc==CAPDB_OK ){
    if( zSql ){
      int rc = capdb_prepare_v3(p->pConfig->db, zSql, -1,
          CAPDB_PREPARE_PERSISTENT|CAPDB_PREPARE_NO_VTAB,
          ppStmt, 0);
      /* If this prepare() call fails with CAPDB_ERROR, then one of the
      ** %_idx or %_data tables has been removed or modified. Call this
      ** corruption.  */
      p->rc = (rc==CAPDB_ERROR ? CAPDB_CORRUPT : rc);
    }else{
      p->rc = CAPDB_NOMEM;
    }
  }
...
static void fts5DataWrite(Fts5Index *p, i64 iRowid, const u8 *pData, int nData){
  ...
  if( p->pWriter==0 ){
    Fts5Config *pConfig = p->pConfig;
    fts5IndexPrepareStmt(p, &p->pWriter, capdb_mprintf(
  ...
  capdb_bind_int64(p->pWriter, 1, iRowid);
  capdb_bind_blob(p->pWriter, 2, pData, nData, CAPDB_STATIC);
  capdb_step(p->pWriter);
  p->rc = capdb_reset(p->pWriter);
  capdb_bind_null(p->pWriter, 2);
}
...
static void fts5DataDelete(Fts5Index *p, i64 iFirst, i64 iLast){
  ...
  capdb_bind_int64(p->pDeleter, 1, iFirst);
  capdb_bind_int64(p->pDeleter, 2, iLast);
  capdb_step(p->pDeleter);
  p->rc = capdb_reset(p->pDeleter);
}
...
static void fts5DataRemoveSegment(Fts5Index *p, Fts5StructureSegment *pSeg){
  ...
  if( p->pIdxDeleter==0 ){
    Fts5Config *pConfig = p->pConfig;
    fts5IndexPrepareStmt(p, &p->pIdxDeleter, capdb_mprintf(
          "DELETE FROM '%q'.'%q_idx' WHERE segid=?",
          pConfig->zDb, pConfig->zName
    ));
  }
  if( p->rc==CAPDB_OK ){
    capdb_bind_int(p->pIdxDeleter, 1, iSegid);
    capdb_step(p->pIdxDeleter);
    p->rc = capdb_reset(p->pIdxDeleter);
  }
}
...
static void fts5StructureRelease(Fts5Structure *pStruct){
  if( pStruct && 0>=(--pStruct->nRef) ){
    ...
    for(i=0; i<pStruct->nLevel; i++){
      capdb_free(pStruct->aLevel[i].aSeg);
    }
    capdb_free(pStruct);
  }
}
...
void *capdbFts5StructureRef(Fts5Index *p){
  fts5StructureRef(p->pStruct);
  return (void*)p->pStruct;
}
void capdbFts5StructureRelease(void *p){
  if( p ){
    fts5StructureRelease((Fts5Structure*)p);
  }
}
int capdbFts5StructureTest(Fts5Index *p, void *pStruct){
...
**
** This function is a no-op if (*pRc) is not CAPDB_OK when it is called. If
** an error occurs, (*pRc) is set to an SQLite error code before returning.
*/
static void fts5StructureMakeWritable(int *pRc, Fts5Structure **pp){
  Fts5Structure *p = *pp;
  if( *pRc==CAPDB_OK && p->nRef>1 ){
    i64 nByte = SZ_FTS5STRUCTURE(p->nLevel);
    Fts5Structure *pNew;
    pNew = (Fts5Structure*)capdbFts5MallocZero(pRc, nByte);
    if( pNew ){
      int i;
      memcpy(pNew, p, nByte);
      ...
      for(i=0; i<p->nLevel; i++){
        ...
        pLvl->aSeg = (Fts5StructureSegment*)capdbFts5MallocZero(pRc, nByte);
        if( pLvl->aSeg==0 ){
          for(i=0; i<p->nLevel; i++){
            capdb_free(pNew->aLevel[i].aSeg);
          }
          capdb_free(pNew);
          return;
        }
...
static int fts5StructureDecode(
  ...
  ** structure record.  */
  ...
  pRet = (Fts5Structure*)capdbFts5MallocZero(&rc, nByte);

  if( pRet ){
    pRet->nRef = 1;
    pRet->nLevel = nLevel;
    pRet->nSegment = nSegment;
    i += capdbFts5GetVarint(&pData[i], &pRet->nWriteCounter);

    for(iLvl=0; rc==CAPDB_OK && iLvl<nLevel; iLvl++){
      ...
      if( i>=nData ){
        ...
        pLvl->aSeg = (Fts5StructureSegment*)capdbFts5MallocZero(&rc, 
    ...
    if( nSegment!=0 && rc==CAPDB_OK ) rc = FTS5_CORRUPT;
    if( bStructureV2 ){
...
static void fts5StructureAddLevel(int *pRc, Fts5Structure **ppStruct){
  ...
  assert( (ppStruct!=0 && (*ppStruct)!=0) || (*pRc)!=CAPDB_OK );
  if( *pRc==CAPDB_OK ){
    Fts5Structure *pStruct = *ppStruct;
    int nLevel = pStruct->nLevel;
    capdb_int64 nByte = SZ_FTS5STRUCTURE(nLevel+2);

    pStruct = capdb_realloc64(pStruct, nByte);
    if( pStruct ){
...
static void fts5StructureExtendLevel(
  ...
  if( *pRc==CAPDB_OK ){
    ...
    aNew = capdb_realloc64(pLvl->aSeg, nByte);
    if( aNew ){
      if( bInsert==0 ){
  ...
  if( p->rc==CAPDB_OK ){
    ...
    if( p->rc==CAPDB_OK ){
      if( (pConfig->pgsz==0 || pConfig->iCookie!=iCookie) ){
        p->rc = capdbFts5ConfigLoad(pConfig, iCookie);
      }
    }else if( p->rc==CAPDB_CORRUPT_VTAB ){
      capdbFts5ConfigErrmsg(p->pConfig, 
  ...
  if( p->rc==CAPDB_OK ){
    if( p->pDataVersion==0 ){
      p->rc = fts5IndexPrepareStmt(p, &p->pDataVersion, 
          capdb_mprintf("PRAGMA %Q.data_version", p->pConfig->zDb)
          );
      if( p->rc ) return 0;
    }

    if( CAPDB_ROW==capdb_step(p->pDataVersion) ){
      iVersion = capdb_column_int64(p->pDataVersion, 0);
    }
    p->rc = capdb_reset(p->pDataVersion);
  }
...
#define fts5BufferSafeAppendVarint(pBuf, iVal) {                \
  (pBuf)->n += capdbFts5PutVarint(&(pBuf)->p[(pBuf)->n], (iVal));  \
...
static void fts5StructureWrite(Fts5Index *p, Fts5Structure *pStruct){
  if( p->rc==CAPDB_OK ){
    ...
    if( 0==capdbFts5BufferSize(&p->rc, &buf, nHdr) ){
      capdbFts5Put32(buf.p, iCookie);
      buf.n = 4;
...
static int fts5DlidxIterEof(Fts5Index *p, Fts5DlidxIter *pIter){
  return p->rc!=CAPDB_OK || pIter->aLvl[0].bEof;
}
...
static void fts5DlidxIterLast(Fts5Index *p, Fts5DlidxIter *pIter){
  ...
  for(i=pIter->nLvl-1; p->rc==CAPDB_OK && i>=0; i--){
...
static int fts5DlidxLvlPrev(Fts5DlidxLvl *pLvl){
  ...
  if( iOff<=pLvl->iFirstOff ){
    ...
    while( 1 ){
      ...
      ii += capdbFts5GetVarint(&a[ii], &delta);
...
static Fts5DlidxIter *fts5DlidxIterInit(
  ...
  for(i=0; p->rc==CAPDB_OK && bDone==0; i++){
    capdb_int64 nByte = SZ_FTS5DLIDXITER(i+1);
    Fts5DlidxIter *pNew;

    pNew = (Fts5DlidxIter*)capdb_realloc64(pIter, nByte);
    if( pNew==0 ){
      p->rc = CAPDB_NOMEM;
    }else{
...
static void fts5SegIterLoadRowid(Fts5Index *p, Fts5SegIter *pIter){
  ...
  while( iOff>=pIter->pLeaf->szLeaf ){
    ...
    if( pIter->pLeaf==0 ){
      if( p->rc==CAPDB_OK ) FTS5_CORRUPT_ITER(p, pIter);
      return;
    }
    iOff = 4;
    a = pIter->pLeaf->p;
  }
  iOff += capdbFts5GetVarint(&a[iOff], (u64*)&pIter->iRowid);
  pIter->iLeafOffset = iOff;
}
...
static void fts5SegIterAllocTombstone(Fts5Index *p, Fts5SegIter *pIter){
  ...
  if( nTomb>0 ){
    ...
    pNew = (Fts5TombstoneArray*)capdbFts5MallocZero(&p->rc, nByte);
    if( pNew ){
...
static void fts5SegIterInit(
  ...
  if( p->rc==CAPDB_OK ){
    ...
    do {
      fts5SegIterNextPage(p, pIter);
    }while( p->rc==CAPDB_OK && pIter->pLeaf && pIter->pLeaf->nn==4 );
  }

  if( p->rc==CAPDB_OK && pIter->pLeaf ){
    pIter->iLeafOffset = 4;
    assert( pIter->pLeaf!=0 );
...
static void fts5SegIterReverseInitPage(Fts5Index *p, Fts5SegIter *pIter){
  ...
  while( 1 ){
    ...
    if( iRowidOffset>=pIter->nRowidOffset ){
      ...
      int *aNew = (int*)capdb_realloc64(pIter->aRowidOffset,nNew*sizeof(int));
      if( aNew==0 ){
        p->rc = CAPDB_NOMEM;
        break;
      }
...
static void fts5SegIterReverseNewPage(Fts5Index *p, Fts5SegIter *pIter){
  ...
  fts5DataRelease(pIter->pLeaf);
  pIter->pLeaf = 0;
  while( p->rc==CAPDB_OK && pIter->iLeafPgno>pIter->iTermLeafPgno ){
    Fts5Data *pNew;
    pIter->iLeafPgno--;
...
static int fts5MultiIterIsEmpty(Fts5Index *p, Fts5Iter *pIter){
  ...
  return (p->rc==CAPDB_OK && pSeg->pLeaf && pSeg->nPos==0);
}
...
static void fts5SegIterNext_None(
  ...
  if( iOff<pIter->iEndofDoclist ){
    ...
    u64 iDelta;
    iOff += capdbFts5GetVarint(&pIter->pLeaf->p[iOff], (u64*)&iDelta);
    pIter->iLeafOffset = iOff;
    pIter->iRowid += iDelta;
    ...
    if( pIter->pSeg ){
      ...
      int nTerm = 0;
      int nList;
      capdbFts5HashScanNext(p->pHash);
      capdbFts5HashScanEntry(p->pHash, &zTerm, &nTerm, &pList, &nList);
      if( pList==0 ) goto next_none_eof;
      pIter->pLeaf->p = (u8*)pList;
      pIter->pLeaf->nn = nList;
      pIter->pLeaf->szLeaf = nList;
      pIter->iEndofDoclist = nList;
      capdbFts5BufferSet(&p->rc,&pIter->term, nTerm, (u8*)zTerm);
...
static void fts5SegIterNext(
  ...
  if( iOff<n ){
    ...
    if( iOff>=pIter->iEndofDoclist ){
      ...
      u64 iDelta;
      iOff += capdbFts5GetVarint(&a[iOff], &iDelta);
      pIter->iRowid += iDelta;
      assert_nc( iDelta>0 );
    }
    pIter->iLeafOffset = iOff;
    ...
    while( iOff==0 ){
      ...
      if( (iOff = fts5LeafFirstRowidOff(pLeaf)) && iOff<pLeaf->szLeaf ){
        iOff += capdbFts5GetVarint(&pLeaf->p[iOff], (u64*)&pIter->iRowid);
        pIter->iLeafOffset = iOff;
...
static capdb_stmt *fts5IdxSelectStmt(Fts5Index *p){
  if( p->pIdxSelect==0 ){
    Fts5Config *pConfig = p->pConfig;
    fts5IndexPrepareStmt(p, &p->pIdxSelect, capdb_mprintf(
...
static void fts5SegIterSeekInit(
  ...
  ** contain term (pTerm/nTerm), if it is present in the segment. */
  ...
  if( p->rc ) return;
  capdb_bind_int(pIdxSelect, 1, pSeg->iSegid);
  capdb_bind_blob(pIdxSelect, 2, pTerm, nTerm, CAPDB_STATIC);
  if( CAPDB_ROW==capdb_step(pIdxSelect) ){
    i64 val = capdb_column_int(pIdxSelect, 0);
    iPg = (int)(val>>1);
    bDlidx = (val & 0x0001);
  }
  p->rc = capdb_reset(pIdxSelect);
  capdb_bind_null(pIdxSelect, 2);
  ...
  if( p->rc==CAPDB_OK && (bGe==0 || (flags & FTS5INDEX_QUERY_SCANONETERM)) ){
...
*/
static capdb_stmt *fts5IdxNextStmt(Fts5Index *p){
  if( p->pIdxNextSelect==0 ){
    Fts5Config *pConfig = p->pConfig;
    fts5IndexPrepareStmt(p, &p->pIdxNextSelect, capdb_mprintf(
          "SELECT pgno FROM '%q'.'%q_idx' WHERE "
...
static void fts5SegIterNextInit(
  ...
  int bDlidx = 0;
  capdb_stmt *pSel = 0;         /* SELECT to find iPg */

  pSel = fts5IdxNextStmt(p);
  if( pSel ){
    assert( p->rc==CAPDB_OK );
    capdb_bind_int(pSel, 1, pSeg->iSegid);
    capdb_bind_blob(pSel, 2, pTerm, nTerm, CAPDB_STATIC);

    if( capdb_step(pSel)==CAPDB_ROW ){
      i64 val = capdb_column_int64(pSel, 0);
      iPg = (int)(val>>1);
      bDlidx = (val & 0x0001);
    }
    p->rc = capdb_reset(pSel);
    capdb_bind_null(pSel, 2);
    if( p->rc ) return;
  }
...
static void fts5SegIterHashInit(
  ...
  if( pTerm==0 || (flags & FTS5INDEX_QUERY_SCAN) ){
    ...
    p->rc = capdbFts5HashScanInit(p->pHash, (const char*)pTerm, nTerm);
    capdbFts5HashScanEntry(p->pHash, (const char**)&z, &n, &pList, &nList);
    if( pList ){
    ...
    /* The call to capdbFts5HashScanInit() causes the hash table to
    ...
    ** possibility altogether.  */
    ...
    p->rc = capdbFts5HashQuery(p->pHash, sizeof(Fts5Data), 
  ...
  if( pLeaf ){
    capdbFts5BufferSet(&p->rc, &pIter->term, n, z);
...
** fts5DataRelease(). Then free the array itself using capdb_free().
*/
...
/*
** This function is a no-op unless CAPDB_DEBUG is defined when this module
...
static void fts5SegIterNextFrom(
  ...
  do{
    if( bMove && p->rc==CAPDB_OK ) pIter->xNext(p, pIter, 0);
    if( pIter->pLeaf==0 ) break;
...
static void fts5MultiIterAdvanced(
  ...
  int i;
  for(i=(pIter->nSeg+iChanged)/2; i>=iMinset && p->rc==CAPDB_OK; i=i/2){
    int iEq;
...
** called. If an OOM error is encountered, *pRc is set to CAPDB_NOMEM
** before returning.
*/
static void fts5IndexExtractColset(
  ...
  if( *pRc==CAPDB_OK ){
    ...
    if( pColset->nCol>1 && capdbFts5BufferSize(pRc, &pIter->poslist, nPos) ){
      return;
    }
  ...
  if( *pRc==CAPDB_OK ){
    ...
    if( pConfig->eDetail==FTS5_DETAIL_NONE ){
      ...
      if( pConfig->nCol<=100 ){
        ...
        capdbFts5BufferSize(pRc, &pIter->poslist, pConfig->nCol);
      }else{
...
static void fts5MultiIterFinishSetup(Fts5Index *p, Fts5Iter *pIter){
  ...
  for(iIter=pIter->nSeg-1; iIter>0; iIter--){
    ...
    if( (iEq = fts5MultiIterDoCompare(pIter, iIter)) ){
      Fts5SegIter *pSeg = &pIter->aSeg[iEq];
      if( p->rc==CAPDB_OK ) pSeg->xNext(p, pSeg, 0);
  ...
  if( p->rc==CAPDB_OK ){
    if( pStruct->nSegment>=FTS5_MAX_SEGMENT ){
      ...
      ** array is 63 elements, or 252 bytes, in size.  */
      ...
      {
        capdb_stmt *pIdxSelect = fts5IdxSelectStmt(p);
        if( p->rc==CAPDB_OK ){
          u8 aBlob[2] = {0xff, 0xff};
          capdb_bind_int(pIdxSelect, 1, iSegid);
          capdb_bind_blob(pIdxSelect, 2, aBlob, 2, CAPDB_STATIC);
          assert_nc( capdb_step(pIdxSelect)!=CAPDB_ROW );
          p->rc = capdb_reset(pIdxSelect);
          capdb_bind_null(pIdxSelect, 2);
        }
      }
...
static void fts5WriteDlidxClear(
  ...
  for(i=0; i<pWriter->nDlidx; i++){
    ...
    capdbFts5BufferZero(&pDlidx->buf);
    pDlidx->bPrevValid = 0;
  }
}
...
static int fts5WriteDlidxGrow(
  ...
  if( p->rc==CAPDB_OK && nLvl>=pWriter->nDlidx ){
    Fts5DlidxWriter *aDlidx = (Fts5DlidxWriter*)capdb_realloc64(
...
static void fts5WriteFlushBtree(Fts5Index *p, Fts5SegWriter *pWriter){
  ...
  if( p->rc==CAPDB_OK ){
    ...
    /* capdb_bind_int(p->pIdxWriter, 1, pWriter->iSegid); */
    capdb_bind_blob(p->pIdxWriter, 2, z, pWriter->btterm.n, CAPDB_STATIC);
    capdb_bind_int64(p->pIdxWriter, 3, bFlag + ((i64)pWriter->iBtPage<<1));
    capdb_step(p->pIdxWriter);
    p->rc = capdb_reset(p->pIdxWriter);
    capdb_bind_null(p->pIdxWriter, 2);
  }
  pWriter->iBtPage = 0;
}
...
static void fts5WriteBtreeNoTerm(
  ...
  ** has already been started, append an 0x00 byte to it.  */
  if( pWriter->bFirstRowidInPage && pWriter->aDlidx[0].buf.n>0 ){
    ...
    capdbFts5BufferAppendVarint(&p->rc, &pDlidx->buf, 0);
  }
  ...
  for(i=0; p->rc==CAPDB_OK && bDone==0; i++){
    ...
    if( pDlidx->buf.n>=p->pConfig->pgsz ){
      ...
      ** also push its first rowid upwards. */
      ...
      if( p->rc==CAPDB_OK && pDlidx[1].buf.n==0 ){
        i64 iFirst = fts5DlidxExtractFirstRowid(&pDlidx->buf);

        /* This was the root node. Push its first rowid up to the new root. */
        pDlidx[1].pgno = pDlidx->pgno;
        capdbFts5BufferAppendVarint(&p->rc, &pDlidx[1].buf, 0);
        capdbFts5BufferAppendVarint(&p->rc, &pDlidx[1].buf, pDlidx->pgno);
        capdbFts5BufferAppendVarint(&p->rc, &pDlidx[1].buf, iFirst);
    ...
    if( pDlidx->bPrevValid ){
      ...
      capdbFts5BufferAppendVarint(&p->rc, &pDlidx->buf, !bDone);
      capdbFts5BufferAppendVarint(&p->rc, &pDlidx->buf, iPgno);
      iVal = iRowid;
    }

    capdbFts5BufferAppendVarint(&p->rc, &pDlidx->buf, iVal);
    pDlidx->bPrevValid = 1;
    pDlidx->iPrev = iRowid;
  }
}
...
static void fts5WriteAppendPoslistData(
  ...
  assert( p->pConfig->pgsz>0 || p->rc!=CAPDB_OK );
  while( p->rc==CAPDB_OK 
...
static void fts5WriteFinish(
  ...
  for(i=0; i<pWriter->nDlidx; i++){
    capdbFts5BufferFree(&pWriter->aDlidx[i].buf);
  }
  capdb_free(pWriter->aDlidx);
}
...
static void fts5WriteInit(
  ...
  capdbFts5BufferSize(&p->rc, &pWriter->writer.pgidx, nBuffer);
  capdbFts5BufferSize(&p->rc, &pWriter->writer.buf, nBuffer);

  if( p->pIdxWriter==0 ){
    Fts5Config *pConfig = p->pConfig;
    fts5IndexPrepareStmt(p, &p->pIdxWriter, capdb_mprintf(
...
static void fts5TrimSegments(Fts5Index *p, Fts5Iter *pIter){
  ...
  Fts5Buffer buf;
  memset(&buf, 0, sizeof(Fts5Buffer));
  for(i=0; i<pIter->nSeg && p->rc==CAPDB_OK; i++){
...
static void fts5IndexMergeLevel(
  ...
  for(fts5MultiIterNew(p, pStruct, flags, 0, 0, 0, iLvl, nInput, &pIter);
    ...
    if( p->rc==CAPDB_OK && bTermWritten==0 ){
...
static int fts5IndexMerge(
  ...
  while( nRem>0 && p->rc==CAPDB_OK ){
    ...
    ** there is no level suitable to merge segments from.  */
    ...
    if( p->rc==CAPDB_OK && pStruct->aLevel[iBestLvl].nMerge==0 ){
...
static void fts5IndexAutomerge(
  ...
  if( p->rc==CAPDB_OK && p->pConfig->nAutomerge>0 && ALWAYS((*ppStruct)!=0) ){
    Fts5Structure *pStruct = *ppStruct;
...
static void fts5IndexCrisismerge(
  ...
  if( pStruct && pStruct->nLevel>0 ){
    int iLvl = 0;
    while( p->rc==CAPDB_OK && pStruct->aLevel[iLvl].nSeg>=nCrisis ){
      fts5IndexMergeLevel(p, &pStruct, iLvl, 0);
      assert( p->rc!=CAPDB_OK || pStruct->nLevel>(iLvl+1) );
...
** Close the read-only blob handle, if it is open.
*/
void capdbFts5IndexCloseReader(Fts5Index *p){
  fts5IndexCloseReader(p);
  fts5IndexReturn(p);
}
...
static void fts5SecureDeleteIdxEntry(
  ...
  if( iPgno!=1 ){
    ...
    if( p->pDeleteFromIdx==0 ){
      fts5IndexPrepareStmt(p, &p->pDeleteFromIdx, capdb_mprintf(
          "DELETE FROM '%q'.'%q_idx' WHERE (segid, (pgno/2)) = (?1, ?2)",
          p->pConfig->zDb, p->pConfig->zName
      ));
    }
    if( p->rc==CAPDB_OK ){
      capdb_bind_int(p->pDeleteFromIdx, 1, iSegid);
      capdb_bind_int(p->pDeleteFromIdx, 2, iPgno);
      capdb_step(p->pDeleteFromIdx);
      p->rc = capdb_reset(p->pDeleteFromIdx);
    }
  }
}
...
static void fts5SecureDeleteOverflow(
  ...
  *pbLastInDoclist = 1;
  for(pgno=iPgno; p->rc==CAPDB_OK && pgno<=pSeg->pgnoLast; pgno++){
    ...
    if( iNext==0 ){
      ...
      ** and nIdx accordingly.  */
      if( pLeaf->nn>pLeaf->szLeaf ){
        ...
        aIdx = capdbFts5MallocZero(&p->rc, (pLeaf->nn-pLeaf->szLeaf)+2);
        if( aIdx==0 ) break;
        i2 = capdbFts5PutVarint(aIdx, iFirst-nShift);
        if( i1<pLeaf->nn ){
...
static void fts5DoSecureDelete(
  ...
  nIdx = nPg-iPgIdx;
  aIdx = capdbFts5MallocZero(&p->rc, ((i64)nIdx)+16);
  if( p->rc ) return;
  ...
  ** to be the last rowid in the doclist for its term.  */
  ...
  if( pSeg->bDel ){
    iOff += capdbFts5PutVarint(&aPg[iOff], iDelta);
    aPg[iOff++] = 0x01;
  }else if( bLastInDoclist==0 ){
    if( iNextOff!=iPgIdx ){
      u64 iNextDelta = 0;
      iNextOff += fts5GetVarint(&aPg[iNextOff], &iNextDelta);
      iOff += capdbFts5PutVarint(&aPg[iOff], iDelta + iNextDelta);
    }
  }else if( 
    ...
    if( iNextOff!=iPgIdx ){
      ...
      ** being removed. */
      ...
      if( (iKeyOff+nSuffix)>(u64)iPgIdx || (iNextOff+nSuffix2)>(u64)iPgIdx ){
        ...
        if( iKey!=1 ){
          iOff += capdbFts5PutVarint(&aPg[iOff], nPrefix);
        }
        iOff += capdbFts5PutVarint(&aPg[iOff], nSuffix);
  ...
  if( p->rc==CAPDB_OK ){
    ...
    for(iIdx=0; iIdx<nIdx; /* no-op */){
      ...
      if( iKeyIn!=iDelKeyOff ){
        ...
        nPg += capdbFts5PutVarint(&aPg[nPg], iKeyOut - iPrevKeyOut);
        iPrevKeyOut = iKeyOut;
      }
    }
...
**
** Return CAPDB_OK if successful, or an SQLite error code if an error
...
static int fts5FlushSecureDelete(
  ...
  if( p->pConfig->iVersion!=FTS5_CURRENT_VERSION_SECUREDELETE ){
    Fts5Config *pConfig = p->pConfig;
    capdb_stmt *pStmt = 0;
    fts5IndexPrepareStmt(p, &pStmt, capdb_mprintf(
...
static void fts5FlushOneHash(Fts5Index *p){
  ...
  ** for the new level-0 segment.  */
  ...
  if( capdbFts5HashIsEmpty(pHash)==0 ){
    ...
    if( iSegid ){
      ...
      ** term/doclist currently stored within the hash table. */
      if( p->rc==CAPDB_OK ){
        p->rc = capdbFts5HashScanInit(pHash, 0, 0);
      }
      while( p->rc==CAPDB_OK && 0==capdbFts5HashScanEof(pHash) ){
        ...
        capdbFts5HashScanEntry(pHash, &zTerm, &nTerm, &pDoclist, &nDoclist);
        if( bSecureDelete==0 ){
          fts5WriteAppendTerm(p, &writer, nTerm, (const u8*)zTerm);
          if( p->rc!=CAPDB_OK ) break;
        ...
        if( !bSecureDelete && pgsz>=(pBuf->n + pPgidx->n + nDoclist + 1) ){
          ...
          ** doclist.  */
          while( p->rc==CAPDB_OK && iOff<nDoclist ){
            u64 iDelta = 0;
            ...
            ** using fts5FlushSecureDelete().  */
            if( bSecureDelete ){
              if( eDetail==FTS5_DETAIL_NONE ){
                ...
                if( p->rc!=CAPDB_OK || pDoclist[iOff]==0x01 ){
                  iOff++;
                  continue;
                }
              }
            ...
            if( p->rc==CAPDB_OK && bTermWritten==0 ){
              fts5WriteAppendTerm(p, &writer, nTerm, (const u8*)zTerm);
              bTermWritten = 1;
              assert( p->rc!=CAPDB_OK || writer.bFirstRowidInPage==0 );
            }
            
            if( writer.bFirstRowidInPage ){
              fts5PutU16(&pBuf->p[0], (u16)pBuf->n);   /* first rowid on page */
              pBuf->n += capdbFts5PutVarint(&pBuf->p[pBuf->n], iRowid);
              writer.bFirstRowidInPage = 0;
              fts5WriteDlidxAppend(p, &writer, iRowid);
            }else{
              u64 iRowidDelta = (u64)iRowid - (u64)iPrev;
              pBuf->n += capdbFts5PutVarint(&pBuf->p[pBuf->n], iRowidDelta);
            }
            if( p->rc!=CAPDB_OK ) break;
        ...
        /* pBuf->p[pBuf->n++] = '\0'; */
        assert( pBuf->n<=pBuf->nSpace );
        if( p->rc==CAPDB_OK ) capdbFts5HashScanNext(pHash);
      }
      fts5WriteFinish(p, &writer, &pgnoLast);
  
      assert( p->rc!=CAPDB_OK || bSecureDelete || pgnoLast>0 );
      if( pgnoLast>0 ){
...
static Fts5Structure *fts5IndexOptimizeStruct(
  ...
  capdb_int64 nByte = SZ_FTS5STRUCTURE(1);
  ...
  */
  ...
  pNew = (Fts5Structure*)capdbFts5MallocZero(&p->rc, nByte);
  ...
  if( pNew ){
    ...
    pLvl->aSeg = (Fts5StructureSegment*)capdbFts5MallocZero(&p->rc, nByte);
    if( pLvl->aSeg ){
      int iLvl, iSeg;
      int iSegOut = 0;
...
int capdbFts5IndexOptimize(Fts5Index *p){
  ...
  assert( p->rc==CAPDB_OK );
  fts5IndexFlush(p);
  assert( p->rc!=CAPDB_OK || p->nContentlessDelete==0 );
  pStruct = fts5StructureRead(p);
  assert( p->rc!=CAPDB_OK || pStruct!=0 );
  fts5StructureInvalidate(p);
  ...
  if( pNew ){
    ...
    while( p->rc==CAPDB_OK && pNew->aLevel[iLvl].nSeg>0 ){
...
static void fts5AppendPoslist(
  ...
  assert( nData>0 );
  if( p->rc==CAPDB_OK && 0==fts5BufferGrow(&p->rc, pBuf, nByte) ){
...
static void fts5NextRowid(Fts5Buffer *pBuf, int *piOff, i64 *piRowid){
  ...
  if( i>=pBuf->n ){
    *piOff = -1;
    ...
    u64 iVal;
    *piOff = i + capdbFts5GetVarint(&pBuf->p[i], &iVal);
    *piRowid += iVal;
  }
}
...
static void fts5MergeRowidLists(
  ...
  assert( nBuf==1 );
  capdbFts5BufferSize(&p->rc, &out, p1->n + p2->n);
  if( p->rc ) return;
...
#define fts5PrefixMergerNextPosition(p) \
  capdbFts5PoslistNext64((p)->aPos,(p)->iter.nPoslist,&(p)->iOff,&(p)->iPos)
#define FTS5_MERGE_NLIST 16
  ...
  ** position-list, hence (nBuf+1) 10 byte paddings.  */
  if( capdbFts5BufferSize(&p->rc, &out, nOut) ) return;
  ...
  while( pHead ){
    ...
    if( pHead->pNext && iLastRowid==pHead->pNext->iter.iRowid ){
      ...
      ** at most nMerge*10 bytes of unexpected space. */
      if( capdbFts5BufferSize(&p->rc, &tmp, nTmp+nMerge*10) ){
        break;
      }
      fts5BufferZero(&tmp);
      ...
      pThis = pHead;
      pHead = pThis->pNext;
      capdbFts5PoslistSafeAppend(&tmp, &iPrev, pThis->iPos);
      ...
      if( pHead->iPos!=iPrev ){
        capdbFts5PoslistSafeAppend(&tmp, &iPrev, pHead->iPos);
      }
      ...
      if( tmp.n+nTail>nTmp-FTS5_DATA_ZERO_PADDING ){
        if( p->rc==CAPDB_OK ) FTS5_CORRUPT_IDX(p);
        break;
      }
...
static void fts5TokendataIterAppendMap(
  ...
  if( p->rc==CAPDB_OK ){
    if( pT->nMap==pT->nMapAlloc ){
      ...
      aNew = (Fts5TokenDataMap*)capdb_realloc64(pT->aMap, nAlloc);
      if( aNew==0 ){
        p->rc = CAPDB_NOMEM;
        return;
      }
...
static void fts5TokendataIterSortMap(Fts5Index *p, Fts5TokenDataIter *pT){
  ...
  aTmp = (Fts5TokenDataMap*)capdbFts5MallocZero(&p->rc, nByte);
  if( aTmp ){
...
static void fts5TokendataIterDelete(Fts5TokenDataIter *pSet){
  if( pSet ){
    ...
    capdb_free(pSet->aPoslistReader);
    capdb_free(pSet->aMap);
    capdb_free(pSet);
  }
}
...
static void prefixIterSetupTokendataCb(
  ...
  while( 0==capdbFts5PoslistNext64(
...
static void prefixIterSetupCb(
  ...
  if( p1->base.nData>0 ){
    if( p1->base.iRowid<=pSetup->iLastRowid && pSetup->doclist.n>0 ){
      int i;
      for(i=0; p->rc==CAPDB_OK && pSetup->doclist.n; i++){
        int i1 = i*nMerge;
        int iStore;
...
static void fts5SetupPrefixIter(
  ...
  pStruct = fts5StructureRead(p);
  assert( p->rc!=CAPDB_OK || (s.aBuf && pStruct) );
  ...
  if( p->rc==CAPDB_OK ){
    ...
    ** main term index here.  */
    ...
    assert( (*ppIter)!=0 || p->rc!=CAPDB_OK );
    if( p->rc==CAPDB_OK && s.pTokendata ){
...
/*
** Indicate that all subsequent calls to capdbFts5IndexWrite() pertain
** to the document with rowid iRowid.
*/
int capdbFts5IndexBeginWrite(Fts5Index *p, int bDelete, i64 iRowid){
  assert( p->rc==CAPDB_OK );

  /* Allocate the hash table if it has not already been allocated */
  if( p->pHash==0 ){
    p->rc = capdbFts5HashNew(p->pConfig, &p->pHash, &p->nPendingData);
  }
...
**
** If successful, set *pp to point to the new object and return CAPDB_OK.
** Otherwise, set *pp to NULL and return an SQLite error code.
*/
int capdbFts5IndexOpen(
  Fts5Config *pConfig, 
  int bCreate, 
  Fts5Index **pp,
  char **pzErr
  ...
  *pp = p = (Fts5Index*)capdbFts5MallocZero(&rc, sizeof(Fts5Index));
  if( rc==CAPDB_OK ){
    p->pConfig = pConfig;
    p->nWorkUnit = FTS5_WORK_UNIT;
    p->zDataTbl = capdbFts5Mprintf(&rc, "%s_data", pConfig->zName);
    if( p->zDataTbl && bCreate ){
      rc = capdbFts5CreateTable(
          pConfig, "data", "id INTEGER PRIMARY KEY, block BLOB", 0, pzErr
      );
      if( rc==CAPDB_OK ){
        rc = capdbFts5CreateTable(pConfig, "idx", 
            "segid, term, pgno, PRIMARY KEY(segid, term)", 
            1, pzErr
        );
      }
      if( rc==CAPDB_OK ){
        rc = capdbFts5IndexReinit(p);
      }
    }
  }

  assert( rc!=CAPDB_OK || p->rc==CAPDB_OK );
  if( rc ){
    capdbFts5IndexClose(p);
    *pp = 0;
  }
  return rc;
}
...
/*
** Close a handle opened by an earlier call to capdbFts5IndexOpen().
*/
int capdbFts5IndexClose(Fts5Index *p){
  ...
  if( p ){
    assert( p->pReader==0 );
    fts5StructureInvalidate(p);
    capdb_finalize(p->pWriter);
    capdb_finalize(p->pDeleter);
    capdb_finalize(p->pIdxWriter);
    capdb_finalize(p->pIdxDeleter);
    capdb_finalize(p->pIdxSelect);
    capdb_finalize(p->pIdxNextSelect);
    capdb_finalize(p->pDataVersion);
    capdb_finalize(p->pDeleteFromIdx);
    capdbFts5HashFree(p->pHash);
...
int capdbFts5IndexWrite(
  ...
  rc = capdbFts5HashWrite(
      p->pHash, p->iWriteRowid, iCol, iPos, FTS5_MAIN_PREFIX, pToken, nToken
  );

  for(i=0; i<pConfig->nPrefix && rc==CAPDB_OK; i++){
    const int nChar = pConfig->aPrefix[i];
    int nByte = capdbFts5IndexCharlenToBytelen(pToken, nToken, nChar);
    if( nByte ){
      rc = capdbFts5HashWrite(p->pHash, 
...
static Fts5TokenDataIter *fts5AppendTokendataIter(
  ...
  if( p->rc==CAPDB_OK ){
    if( pIn==0 || pIn->nIter==pIn->nIterAlloc ){
      ...
      pNew = (Fts5TokenDataIter*)capdb_realloc64(pIn, nByte);
  ...
  if( nHit==0 ){
    ...
    if( nHit>1 && eDetail!=FTS5_DETAIL_NONE ){
      ...
      if( pT->aPoslistReader==0 ){
        pT->aPoslistReader = (Fts5PoslistReader*)capdbFts5MallocZero(
            &pIter->pIndex->rc,
      ...
      if( eDetail==FTS5_DETAIL_FULL && pT->nMapAlloc<(pT->nMap + nByte) ){
        ...
        Fts5TokenDataMap *aNew = (Fts5TokenDataMap*)capdb_realloc64(
            pT->aMap, nNew*sizeof(Fts5TokenDataMap)
        );
        if( aNew==0 ){
          pIter->pIndex->rc = CAPDB_NOMEM;
          return;
        }
        pT->aMap = aNew;
        pT->nMapAlloc = nNew;
      ...
      while( 1 ){
        ...
        capdbFts5PoslistSafeAppend(&pIter->poslist, &iPrev, iMinPos);
        capdbFts5PoslistReaderNext(&pT->aPoslistReader[iMin]);
...
int capdbFts5IndexQuery(
  ...
  if( capdbFts5BufferSize(&p->rc, &buf, nToken+1)==0 ){
...
*/
int capdbFts5IterNext(Fts5IndexIter *pIndexIter){
  Fts5Iter *pIter = (Fts5Iter*)pIndexIter;
  assert( pIter->pIndex->rc==CAPDB_OK );
  if( pIter->nSeg==0 ){
...
*/
int capdbFts5IterNextScan(Fts5IndexIter *pIndexIter){
  Fts5Iter *pIter = (Fts5Iter*)pIndexIter;
...
*/
int capdbFts5IterNextFrom(Fts5IndexIter *pIndexIter, i64 iMatch){
  Fts5Iter *pIter = (Fts5Iter*)pIndexIter;
  if( pIter->nSeg==0 ){
    assert( pIter->pTokenDataIter );
...
*/
const char *capdbFts5IterTerm(Fts5IndexIter *pIndexIter, int *pn){
  int n;
...
static int fts5SetupPrefixIterTokendata(
  ...
  assert( token.p!=0 || p->rc!=CAPDB_OK );
  ctx.pT = (Fts5TokenDataIter*)capdbFts5MallocZero(&p->rc,
...
*/
void capdbFts5IndexIterClearTokendata(Fts5IndexIter *pIndexIter){
  Fts5Iter *pIter = (Fts5Iter*)pIndexIter;
...
int capdbFts5IndexIterWriteTokendata(
  ...
  if( pIter->nSeg>0 ){
    ...
    if( pT==0 ){
      pT = (Fts5TokenDataIter*)capdbFts5MallocZero(&p->rc,
                                           SZ_FTS5TOKENDATAITER(1));
...
/*
** Close an iterator opened by an earlier call to capdbFts5IndexQuery().
*/
void capdbFts5IterClose(Fts5IndexIter *pIndexIter){
  if( pIndexIter ){
...
*/
int capdbFts5IndexSetAverages(Fts5Index *p, const u8 *pData, int nData){
  assert( p->rc==CAPDB_OK );
...
** occurs.
*/
int capdbFts5IndexSetCookie(Fts5Index *p, int iNew){
  int rc;                              /* Return code */
  ...
  assert( p->rc==CAPDB_OK );
  capdbFts5Put32(aCookie, iNew);

  rc = capdb_blob_open(pConfig->db, pConfig->zDb, p->zDataTbl, 
      "block", FTS5_STRUCTURE_ROWID, 1, &pBlob
  );
  if( rc==CAPDB_OK ){
    capdb_blob_write(pBlob, aCookie, 4, 0);
    rc = capdb_blob_close(pBlob);
  }
...
** disk. If successful, CAPDB_OK is returned and (*piOrigin) set to
...
static void fts5IndexTombstoneRebuild(
  ...
  */
  ...
  while( 1 ){
    ...
    apOut = (Fts5Data**)capdbFts5MallocZero(&p->rc, sizeof(Fts5Data*) * nOut);
    szPage = 8 + nSlot*szKey;
    for(ii=0; ii<nOut; ii++){
      Fts5Data *pNew = (Fts5Data*)capdbFts5MallocZero(&p->rc, 
...
static void fts5IndexTombstoneAdd(
  ...
  assert( p->rc==CAPDB_OK || (nHash==0 && apHash==0) );
...
** rows from origin iOrigin. Return CAPDB_OK if successful, or an SQLite
...
static void fts5TestDlidxReverse(
  ...
  if( p->rc==CAPDB_OK && cksum1!=cksum2 ) p->rc = FTS5_CORRUPT;
}
...
static int fts5QueryCksum(
  ...
  int rc = capdbFts5IndexQuery(
      p, z, n, (flags | FTS5INDEX_QUERY_NOTOKENDATA), 0, &pIter
  );

  while( rc==CAPDB_OK && ALWAYS(pIter!=0) && 0==capdbFts5IterEof(pIter) ){
    i64 rowid = pIter->iRowid;

    if( eDetail==FTS5_DETAIL_NONE ){
      cksum ^= capdbFts5IndexEntryCksum(rowid, 0, 0, iIdx, z, n);
    }else{
      Fts5PoslistReader sReader;
      for(capdbFts5PoslistReaderInit(pIter->pData, pIter->nData, &sReader);
          sReader.bEof==0;
          capdbFts5PoslistReaderNext(&sReader)
      ){
        int iCol = FTS5_POS2COLUMN(sReader.iPos);
        int iOff = FTS5_POS2OFFSET(sReader.iPos);
        cksum ^= capdbFts5IndexEntryCksum(rowid, iCol, iOff, iIdx, z, n);
      }
    }
    if( rc==CAPDB_OK ){
      rc = capdbFts5IterNext(pIter);
    }
  }
  fts5IterClose(pIter);
...
static void fts5TestTerm(
  ...
  if( *pbFail==0 
    ...
    ** the same. If not, call this corruption.  */
    ...
    if( rc==CAPDB_OK && ck1!=ck2 ) rc = FTS5_CORRUPT;
    ...
    ** test to fail.  */
    ...
    if( rc==CAPDB_OK && cksum3!=expected ){
      *pbFail = 1;
    }
    *pCksum = cksum3;
  }
...
static void fts5IndexIntegrityCheckEmpty(
  ...
  ** (a) exist and (b) contain no terms. */
  for(i=iFirst; p->rc==CAPDB_OK && i<=iLast; i++){
...
static void fts5IntegrityCheckPgidx(Fts5Index *p, i64 iRowid, Fts5Data *pLeaf){
  ...
  ii = pLeaf->szLeaf;
  while( ii<pLeaf->nn && p->rc==CAPDB_OK ){
    int res;
    i64 iOff;
    int nIncr;
...
static void fts5IndexIntegrityCheckSegment(
  ...
  while( p->rc==CAPDB_OK && CAPDB_ROW==capdb_step(pStmt) ){
    ...
    const char *zIdxTerm = (const char*)capdb_column_blob(pStmt, 1);
    int nIdxTerm = capdb_column_bytes(pStmt, 1);
    int iIdxLeaf = capdb_column_int(pStmt, 2);
    int bIdxDlidx = capdb_column_int(pStmt, 3);
...
#if 0
  if( p->rc==CAPDB_OK && iter.iLeaf!=pSeg->pgnoLast ){
    p->rc = FTS5_CORRUPT;
  }
#endif
}
...
** as calculated by capdbFts5IndexEntryCksum() is cksum.
**
** Return CAPDB_CORRUPT if any of the internal checks fail, or if the
** checksum does not match. Return CAPDB_OK if all checks pass without
** error, or some other SQLite error code if another error (e.g. OOM)
** occurs.
*/
int capdbFts5IndexIntegrityCheck(Fts5Index *p, u64 cksum, int bUseCksum){
  int eDetail = p->pConfig->eDetail;
  ...
  */
  for(fts5MultiIterNew(p, pStruct, flags, 0, 0, 0, -1, 0, &pIter);
    ...
    if( eDetail==FTS5_DETAIL_NONE ){
      if( 0==fts5MultiIterIsEmpty(p, pIter) ){
        cksum2 ^= capdbFts5IndexEntryCksum(iRowid, 0, 0, -1, z, n);
      }
    }else{
      poslist.n = 0;
      fts5SegiterPoslist(p, &pIter->aSeg[pIter->aFirst[1].iFirst], 0, &poslist);
      fts5BufferAppendBlob(&p->rc, &poslist, 4, (const u8*)"\0\0\0\0");
      while( 0==capdbFts5PoslistNext64(poslist.p, poslist.n, &iOff, &iPos) ){
        int iCol = FTS5_POS2COLUMN(iPos);
        int iTokOff = FTS5_POS2OFFSET(iPos);
        cksum2 ^= capdbFts5IndexEntryCksum(iRowid, iCol, iTokOff, -1, z, n);
      }
    }
  }
  ...
  fts5MultiIterFree(pIter);
  if( p->rc==CAPDB_OK && bUseCksum && cksum!=cksum2 ){
    p->rc = FTS5_CORRUPT;
    capdbFts5ConfigErrmsg(p->pConfig, 
...
#ifdef CAPDB_DEBUG
  /* In CAPDB_DEBUG builds, expensive extra checks were run as part of
  ** the integrity-check above. If no other errors were detected, but one
  ** of these tests failed, set the result to CAPDB_CORRUPT_VTAB here. */
  if( p->rc==CAPDB_OK && bTestFail ){
    p->rc = FTS5_CORRUPT;
  }
  fts5BufferFree(&term);
#endif
...
#if defined(CAPDB_TEST) || defined(CAPDB_FTS5_DEBUG)
/*
...
#endif /* CAPDB_TEST || CAPDB_FTS5_DEBUG */
...
static void fts5DebugRowid(int *pRc, Fts5Buffer *pBuf, i64 iKey){
  ...
  if( iSegid==0 ){
    if( iKey==FTS5_AVERAGES_ROWID ){
      capdbFts5BufferAppendPrintf(pRc, pBuf, "{averages} ");
    }else{
      capdbFts5BufferAppendPrintf(pRc, pBuf, "{structure}");
    }
  }
  else{
    capdbFts5BufferAppendPrintf(pRc, pBuf, "{%s%ssegid=%d h=%d pgno=%d}",
        bDlidx ? "dlidx " : "", 
        bTomb ? "tombstone " : "", 
...
static void fts5DebugStructure(
  ...
  for(iLvl=0; iLvl<p->nLevel; iLvl++){
    ...
    capdbFts5BufferAppendPrintf(pRc, pBuf, 
        " {lvl=%d nMerge=%d nSeg=%d", iLvl, pLvl->nMerge, pLvl->nSeg
    );
    for(iSeg=0; iSeg<pLvl->nSeg; iSeg++){
      Fts5StructureSegment *pSeg = &pLvl->aSeg[iSeg];
      capdbFts5BufferAppendPrintf(pRc, pBuf, " {id=%d leaves=%d..%d",
          pSeg->iSegid, pSeg->pgnoFirst, pSeg->pgnoLast
      );
      if( pSeg->iOrigin1>0 ){
        capdbFts5BufferAppendPrintf(pRc, pBuf, " origin=%lld..%lld",
...
static void fts5DecodeAverages(
  ...
  while( i<nBlob ){
    u64 iVal;
    i += capdbFts5GetVarint(&pBlob[i], &iVal);
    capdbFts5BufferAppendPrintf(pRc, pBuf, "%s%d", zSpace, (int)iVal);
    zSpace = " ";
  }
}
#endif /* CAPDB_TEST || CAPDB_FTS5_DEBUG */
...
static int fts5DecodePoslist(int *pRc, Fts5Buffer *pBuf, const u8 *a, int n){
  ...
  while( iOff<n ){
    ...
    capdbFts5BufferAppendPrintf(pRc, pBuf, " %d", iVal);
  }
  return iOff;
}
#endif /* CAPDB_TEST || CAPDB_FTS5_DEBUG */
...
static int fts5DecodeDoclist(int *pRc, Fts5Buffer *pBuf, const u8 *a, int n){
  ...
  if( n>0 ){
    iOff = capdbFts5GetVarint(a, (u64*)&iDocid);
    capdbFts5BufferAppendPrintf(pRc, pBuf, " id=%lld", iDocid);
  }
  while( iOff<n ){
    int nPos;
    int bDel;
    iOff += fts5GetPoslistSize(&a[iOff], &nPos, &bDel);
    capdbFts5BufferAppendPrintf(pRc, pBuf, " nPos=%d%s", nPos, bDel?"*":"");
    iOff += fts5DecodePoslist(pRc, pBuf, &a[iOff], MIN(n-iOff, nPos));
    if( iOff<n ){
      i64 iDelta;
      iOff += capdbFts5GetVarint(&a[iOff], (u64*)&iDelta);
      iDocid += iDelta;
      capdbFts5BufferAppendPrintf(pRc, pBuf, " id=%lld", iDocid);
    }
  }
...
** buffer pBuf.
**
** If *pRc is other than CAPDB_OK when this function is called, it is a
...
static void fts5DecodeRowidList(
  ...
  while( i<nData ){
    const char *zApp = "";
    u64 iVal;
    i += capdbFts5GetVarint(&pData[i], &iVal);
    iRowid += iVal;
    ...
    capdbFts5BufferAppendPrintf(pRc, pBuf, " %lld%s", iRowid, zApp);
  }
}
#endif /* CAPDB_TEST || CAPDB_FTS5_DEBUG */
...
static void fts5DecodeFunction(
  ...
  capdb_value **apVal           /* Function arguments */
){
  ...
  int rc = CAPDB_OK;             /* Return code */
  capdb_int64 nSpace = 0;
  int eDetailNone = (capdb_user_data(pCtx)!=0);

  assert( nArg==2 );
  UNUSED_PARAM(nArg);
  memset(&s, 0, sizeof(Fts5Buffer));
  iRowid = capdb_value_int64(apVal[0]);

  /* Make a copy of the second argument (a blob) in aBlob[]. The aBlob[]
  ** copy is followed by FTS5_DATA_ZERO_PADDING 0x00 bytes, which prevents
  ** buffer overreads even if the record is corrupt.  */
  n = capdb_value_bytes(apVal[1]);
  aBlob = capdb_value_blob(apVal[1]);
  nSpace = ((i64)n) + FTS5_DATA_ZERO_PADDING;
  a = (u8*)capdbFts5MallocZero(&rc, nSpace);
  ...
  if( bDlidx ){
    ...
    for(fts5DlidxLvlNext(&lvl); lvl.bEof==0; fts5DlidxLvlNext(&lvl)){
      capdbFts5BufferAppendPrintf(&rc, &s, 
    ...
    int nSlot = (n - 8) / szKey;
    int ii;
    capdbFts5BufferAppendPrintf(&rc, &s, " nElem=%d", (int)nElem);
    if( aBlob[1] ){
      capdbFts5BufferAppendPrintf(&rc, &s, " 0");
    }
    for(ii=0; ii<nSlot; ii++){
      ...
      if( iVal!=0 ){
        capdbFts5BufferAppendPrintf(&rc, &s, " %lld", (i64)iVal);
      }
    }
  }else if( iSegid==0 ){
    ...
    while( iOff<szLeaf && rc==CAPDB_OK ){
      ...
      capdbFts5BufferAppendPrintf(&rc, &s, " term=");
    ...
    if( n<4 ){
      capdbFts5BufferSet(&rc, &s, 7, (const u8*)"corrupt");
      goto decode_out;
    }else{
 ...
 decode_out:
  capdb_free(a);
  if( rc==CAPDB_OK ){
    capdb_result_text(pCtx, (const char*)s.p, s.n, CAPDB_TRANSIENT);
  }else{
...
static void fts5RowidFunction(
  ...
  const char *zArg;
  if( nArg==0 ){
    capdb_result_error(pCtx, "should be: fts5_rowid(subject, ....)", -1);
  }else{
    zArg = (const char*)capdb_value_text(apVal[0]);
    if( 0==capdb_stricmp(zArg, "segment") ){
      i64 iRowid;
      int segid, pgno;
      if( nArg!=3 ){
        ...
        segid = capdb_value_int(apVal[1]);
        pgno = capdb_value_int(apVal[2]);
        iRowid = FTS5_SEGMENT_ROWID(segid, pgno);
        capdb_result_int64(pCtx, iRowid);
      }
    }else{
      capdb_result_error(pCtx, 
...
static int fts5structConnectMethod(
  ...
  if( rc==CAPDB_OK ){
    pNew = capdbFts5MallocZero(&rc, sizeof(*pNew));
  }
...
static int fts5structBestIndexMethod(
  ...
  for(i=0, p=pIdxInfo->aConstraint; i<pIdxInfo->nConstraint; i++, p++){
    ...
    if( p->op==CAPDB_INDEX_CONSTRAINT_EQ && p->iColumn==11 ){
      rc = CAPDB_OK;
...
*/
static int fts5structDisconnectMethod(capdb_vtab *pVtab){
...
*/
static int fts5structOpenMethod(capdb_vtab *p, capdb_vtab_cursor **ppCsr){
  int rc = CAPDB_OK;
  Fts5StructVcsr *pNew = 0;

  pNew = capdbFts5MallocZero(&rc, sizeof(*pNew));
  *ppCsr = (capdb_vtab_cursor*)pNew;
...
*/
static int fts5structCloseMethod(capdb_vtab_cursor *cur){
...
*/
static int fts5structNextMethod(capdb_vtab_cursor *cur){
  Fts5StructVcsr *pCsr = (Fts5StructVcsr*)cur;
...
** row of output.
*/
static int fts5structEofMethod(capdb_vtab_cursor *cur){
  Fts5StructVcsr *pCsr = (Fts5StructVcsr*)cur;
...
static int fts5structColumnMethod(
  ...
  switch( i ){
    case 0: /* level */
      capdb_result_int(ctx, pCsr->iLevel);
      break;
    case 1: /* segment */
      capdb_result_int(ctx, pCsr->iSeg);
      break;
    case 2: /* merge */
      capdb_result_int(ctx, pCsr->iSeg < p->aLevel[pCsr->iLevel].nMerge);
      break;
    case 3: /* segid */
      capdb_result_int(ctx, pSeg->iSegid);
      break;
    case 4: /* leaf1 */
      capdb_result_int(ctx, pSeg->pgnoFirst);
      break;
    case 5: /* leaf2 */
      capdb_result_int(ctx, pSeg->pgnoLast);
      break;
    case 6: /* origin1 */
      capdb_result_int64(ctx, pSeg->iOrigin1);
      break;
    case 7: /* origin2 */
      capdb_result_int64(ctx, pSeg->iOrigin2);
      break;
    case 8: /* npgtombstone */
      capdb_result_int(ctx, pSeg->nPgTombstone);
      break;
    case 9: /* nentrytombstone */
      capdb_result_int64(ctx, pSeg->nEntryTombstone);
      break;
    case 10: /* nentry */
      capdb_result_int64(ctx, pSeg->nEntry);
      break;
  }
  return CAPDB_OK;
}
...
static int fts5structFilterMethod(
  ...
  aBlob = (const u8*)capdb_value_blob(argv[0]);
...
** with FTS5.
**
** If successful, CAPDB_OK is returned. If an error occurs, some other
** SQLite error code is returned instead.
*/
int capdbFts5IndexInit(capdb *db){
#if defined(CAPDB_TEST) || defined(CAPDB_FTS5_DEBUG)
  int rc = capdb_create_function(
      db, "fts5_decode", 2, CAPDB_UTF8, 0, fts5DecodeFunction, 0, 0
  );

  if( rc==CAPDB_OK ){
    rc = capdb_create_function(
        db, "fts5_decode_none", 2, 
        CAPDB_UTF8, (void*)db, fts5DecodeFunction, 0, 0
    );
  }

  if( rc==CAPDB_OK ){
    rc = capdb_create_function(
        db, "fts5_rowid", -1, CAPDB_UTF8, 0, fts5RowidFunction, 0, 0
    );
  }

  if( rc==CAPDB_OK ){
    static const capdb_module fts5structure_module = {
      0,                           /* iVersion      */
      0,                           /* xCreate       */
    ...
    rc = capdb_create_module(db, "fts5_structure", &fts5structure_module, 0);
  }
  return rc;
#else
  return CAPDB_OK;
