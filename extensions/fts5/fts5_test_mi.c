...
**
**   int capdbFts5TestRegisterMatchinfo(capdb *db);
*/
...
** handle (accessible using capdb_errcode()/errmsg()).
*/
static int fts5_api_from_db(capdb *db, fts5_api **ppApi){
  capdb_stmt *pStmt = 0;
  int rc;

  *ppApi = 0;
  rc = capdb_prepare(db, "SELECT fts5(?1)", -1, &pStmt, 0);
  if( rc==CAPDB_OK ){
    capdb_bind_pointer(pStmt, 1, (void*)ppApi, "fts5_api_ptr", 0);
    (void)capdb_step(pStmt);
...
static int fts5MatchinfoGlobalCb(
  ...
  switch( f ){
    ...
    case 'x': {
      int i;
      for(i=0; i<p->nPhrase && rc==CAPDB_OK; i++){
    ...
    case 'a': {
      ...
      if( nRow==0 ){
        ...
        int i;
        for(i=0; rc==CAPDB_OK && i<p->nCol; i++){
          capdb_int64 nToken;
  ...
  switch( f ){
    ...
    case 's': {
      ...
      for(i=0; rc==CAPDB_OK && i<nInst; i++){
        ...
        for(j=i+1; rc==CAPDB_OK && j<nInst; j++){
          int ip, ic, io;
...
static Fts5MatchinfoCtx *fts5MatchinfoNew(
  const Fts5ExtensionApi *pApi,   /* API offered by current FTS version */
  Fts5Context *pFts,              /* First arg to pass to pApi functions */
  capdb_context *pCtx,          /* Context for returning error message */
  ...
  for(i=0; zArg[i]; i++){
    ...
    if( n<0 ){
      char *zErr = capdb_mprintf("unrecognized matchinfo flag: %c", zArg[i]);
  ...
  p = (Fts5MatchinfoCtx*)capdb_malloc64(nByte);
  if( p==0 ){
...
static void fts5MatchinfoFunc(
  ...
  if( p==0 || capdb_stricmp(zArg, p->zArg) ){
  ...
  if( rc!=CAPDB_OK ){
    ...
    capdb_result_blob(pCtx, (void*)p->aRet, nByte, CAPDB_TRANSIENT);
  }
}
...
*/
int capdbFts5TestRegisterMatchinfoAPI(fts5_api *pApi){
  int rc;
...
int capdbFts5TestRegisterMatchinfo(capdb *db){
  ...
  ** FTS5 documentation. Refer there for details. */
  ...
  return capdbFts5TestRegisterMatchinfoAPI(pApi);
}
