...
void test_rbu_delta(capdb_context *pCtx, int nArg, capdb_value **apVal){
  Tcl_Interp *interp = (Tcl_Interp*)capdb_user_data(pCtx);
  Tcl_Obj *pScript;
  int i;
  ...
  for(i=0; i<nArg; i++){
    capdb_value *pIn = apVal[i];
    const char *z = (const char*)capdb_value_text(pIn);
...
static int xRenameCallback(void *pArg, const char *zOld, const char *zNew){
  ...
  return rc ? CAPDB_IOERR : CAPDB_OK;
}

static int CAPDB_TCLAPI test_capdb_rbu_cmd(
  ...
  switch( iCmd ){
    ...
    case 2: /* create_rbu_delta */ {
      ...
      int rc = capdb_create_function(
          db, "rbu_delta", -1, CAPDB_UTF8, (void*)interp, test_rbu_delta, 0, 0
      );
      Tcl_SetObjResult(interp, Tcl_NewStringObj(capdbErrName(rc), -1));
      ret = (rc==CAPDB_OK ? TCL_OK : TCL_ERROR);
      break;
    }

    case 3: /* savestate */ {
      int rc = capdb_rbu_savestate(pRbu);
      Tcl_SetObjResult(interp, Tcl_NewStringObj(capdbErrName(rc), -1));
      ret = (rc==CAPDB_OK ? TCL_OK : TCL_ERROR);
      break;
    }
    ...
    case 12: /* dbRbu_eval */ 
    case 4:  /* dbMain_eval */ {
      capdb *db = capdb_rbu_db(pRbu, (iCmd==12));
      int rc = capdb_exec(db, Tcl_GetString(objv[2]), 0, 0, 0);
      if( rc!=CAPDB_OK ){
        Tcl_SetObjResult(interp, Tcl_NewStringObj(capdb_errmsg(db), -1));
        ret = TCL_ERROR;
      }
      break;
    }
    ...
    case 6: /* db */ {
      ...
      if( Tcl_GetBooleanFromObj(interp, objv[2], &bArg) ){
        ...
        char zBuf[50];
        capdb *db = capdb_rbu_db(pRbu, bArg);
        if( capdbTestMakePointerStr(interp, zBuf, (void*)db) ){
          ret = TCL_ERROR;
        }else{
    ...
    case 7: /* state */ {
      ...
      int eState = capdb_rbu_state(pRbu);
      assert( eState>0 && eState<=5 );
      Tcl_SetResult(interp, (char*)aRes[eState], TCL_STATIC);
      break;
    }
    case 8: /* progress */ {
      capdb_int64 nStep =  capdb_rbu_progress(pRbu);
    ...
    case 10: /* temp_size_limit */ {
      ...
      if( Tcl_GetWideIntFromObj(interp, objv[2], &nLimit) ){
        ...
        nLimit = capdb_rbu_temp_size_limit(pRbu, nLimit);
        Tcl_SetObjResult(interp, Tcl_NewWideIntObj(nLimit));
      }
      break;
    }
    case 11: /* temp_size */ {
      capdb_int64 sz = capdb_rbu_temp_size(pRbu);
    ...
    case 13: /* rename_handler */ {
      ...
      assert( !capdb_stricmp(aCmd[13].zName, "rename_handler") );
      if( Tcl_GetCharLength(pScript)==0 ){
        capdb_rbu_rename_handler(pRbu, 0, 0);
      }else{
        pTest->xRename = Tcl_DuplicateObj(pScript);
        Tcl_IncrRefCount(pTest->xRename);
        capdb_rbu_rename_handler(pRbu, pTest, xRenameCallback);
      }
      break;
    }
...
static void createRbuWrapper(
  ...
  pTest->pRbu = pRbu;
  pTest->interp = interp;
  Tcl_CreateObjCommand(interp, zCmd, test_capdb_rbu_cmd, (ClientData)pTest, 0);
}

/*
** Tclcmd: capdb_rbu CMD <target-db> <rbu-db> ?<state-db>?
*/
static int CAPDB_TCLAPI test_capdb_rbu(
  ClientData clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  pRbu = capdb_rbu_open(zTarget, zRbu, zStateDb);
...
/*
** Tclcmd: capdb_rbu_vacuum CMD <target-db> <state-db>
*/
static int CAPDB_TCLAPI test_capdb_rbu_vacuum(
  ClientData clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  pRbu = capdb_rbu_vacuum(zTarget, zStateDb);
  createRbuWrapper(interp, zCmd, pRbu);
  Tcl_SetObjResult(interp, objv[1]);
  return TCL_OK;
}
...
/*
** Tclcmd: capdb_rbu_create_vfs ?-default? NAME PARENT
*/
static int CAPDB_TCLAPI test_capdb_rbu_create_vfs(
  ClientData clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  rc = capdb_rbu_create_vfs(zName, zParent);
  if( rc!=CAPDB_OK ){
    Tcl_SetObjResult(interp, Tcl_NewStringObj(capdbErrName(rc), -1));
    return TCL_ERROR;
  }else if( objc==4 ){
    capdb_vfs *pVfs = capdb_vfs_find(zName);
    capdb_vfs_register(pVfs, 1);
  }
...
/*
** Tclcmd: capdb_rbu_destroy_vfs NAME
*/
static int CAPDB_TCLAPI test_capdb_rbu_destroy_vfs(
  ClientData clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  capdb_rbu_destroy_vfs(zName);
  return TCL_OK;
}

/*
** Tclcmd: capdb_rbu_internal_test
*/
static int CAPDB_TCLAPI test_capdb_rbu_internal_test(
  ClientData clientData,
  Tcl_Interp *interp,
  int objc,
  Tcl_Obj *CONST objv[]
  ...
  db = capdb_rbu_db(0, 0);
  if( db!=0 ){
    Tcl_AppendResult(interp, "capdb_rbu_db(0, 0)!=0", NULL);
    return TCL_ERROR;
  }
...
int SqliteRbu_Init(Tcl_Interp *interp){ 
  static struct {
    ...
    { "capdb_rbu", test_capdb_rbu },
    { "capdb_rbu_vacuum", test_capdb_rbu_vacuum },
    { "capdb_rbu_create_vfs", test_capdb_rbu_create_vfs },
    { "capdb_rbu_destroy_vfs", test_capdb_rbu_destroy_vfs },
    { "capdb_rbu_internal_test", test_capdb_rbu_internal_test },
  };
  int i;
