...
static void unknown_option_error(
  ...
  for(i=0; aOpt[i].zText; i++){
    ...
    char *zOpt = capdb_mprintf("%s %s", aOpt[i].zText,
        eType==CMDLINE_BARE ? "" :
...
static int get_boolean_option(CmdLineOption *pOpt, const char *zArg){
  if( 0==capdb_stricmp(zArg, "true") ) return 1;
  if( 0==capdb_stricmp(zArg, "1") ) return 1;
  if( 0==capdb_stricmp(zArg, "0") ) return 0;
  if( 0==capdb_stricmp(zArg, "false") ) return 0;
...
static void parse_command_line(
  ...
  for(iArg=iStart; iArg<argc; iArg++){
    ...
    for(iOpt=0; aOpt[iOpt].zText; iOpt++){
      ...
      if( 0==capdb_strnicmp(pOpt->zText, zArg, nArg) ){
        if( nMatch ){
    ...
    if( nMatch==0 && 0==capdb_strnicmp("-cmdline:verbose", zArg, nArg) ){
      bVerbose = 1;
      nMatch = 1;
    }
...
static void execsql(capdb *db, const char *zSql){
  int rc = capdb_exec(db, zSql, 0, 0, 0);
  if( rc!=CAPDB_OK ) abort_due_to_error(rc);
}

static int xConflict(void *pCtx, int eConflict, capdb_changeset_iter *p){
  return CAPDB_CHANGESET_ABORT;
}
...
static void run_test(
  ...
  rc = capdb_session_attach(pSession, 0);
  if( rc!=CAPDB_OK ) abort_due_to_error(rc);
  ...
  rc = capdb_session_changeset(pSession, &nChangeset, &pChangeset);
  if( rc!=CAPDB_OK ) abort_due_to_error(rc);
  execsql(db, "COMMIT");

  /* Apply the changeset to the second db */
  rc = capdb_changeset_apply(db2, nChangeset, pChangeset, 0, xConflict, 0);
  if( rc!=CAPDB_OK ) abort_due_to_error(rc);
  ...
  for(bWithoutRowid=0; bWithoutRowid<2; bWithoutRowid++){
    for(bInteger=0; bInteger<2; bInteger++){
      if( o.bAll || (o.bWithoutRowid==bWithoutRowid && o.bInteger==bInteger) ){
        ...
        unlink(o.zDb);
        rc = capdb_open(o.zDb, &db);
        if( rc!=CAPDB_OK ) abort_due_to_error(rc);
        unlink(zDb2);
        rc = capdb_open(zDb2, &db2);
        if( rc!=CAPDB_OK ) abort_due_to_error(rc);
