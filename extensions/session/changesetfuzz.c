...
** capdb_randomness. This version has two important differences:
**
...
static unsigned char fuzzRandomByte(void){
  unsigned char t;
  capdbPrng.i++;
  t = capdbPrng.s[capdbPrng.i];
  capdbPrng.j += t;
  capdbPrng.s[capdbPrng.i] = capdbPrng.s[capdbPrng.j];
  capdbPrng.s[capdbPrng.j] = t;
  t += capdbPrng.s[capdbPrng.i];
  return capdbPrng.s[t];
}
...
static void fuzzRandomSeed(unsigned int iSeed){
  int i;
  for(i=0; i<256; i+=4){
    capdbPrng.s[i] ^= ((iSeed >> 24) & 0xFF);
    capdbPrng.s[i+1] ^= ((iSeed >> 16) & 0xFF);
    capdbPrng.s[i+2] ^= ((iSeed >>  8) & 0xFF);
    capdbPrng.s[i+3] ^= ((iSeed >>  0) & 0xFF);
  }
}
/*
...
*/
static void *fuzzMalloc(capdb_int64 nByte){
...
** object with the results. Return CAPDB_OK if successful, or an error code
** otherwise.
*/
...
** successful, it sets (*pSz) to the size and returns CAPDB_OK. Or, if the
** buffer does not contain a valid value, CAPDB_CORRUPT is returned and
...
static int fuzzParseRecord(
  ...
  for(i=0; rc==CAPDB_OK && i<pGrp->nCol; i++){
    if( bPkOnly==0 || pGrp->aPK[i] ){
      int sz;
      if( p>=pEnd ) break;
      if( (pParse->nVal & (pParse->nVal-1))==0 ){
        int nNew = pParse->nVal ? pParse->nVal*2 : 4;
        u8 **apNew = (u8**)capdb_realloc(pParse->apVal, nNew*sizeof(u8*));
        if( apNew==0 ) return CAPDB_NOMEM;
        pParse->apVal = apNew;
      }
...
** to one byte past the end of the change array and return CAPDB_OK.
...
static int fuzzParseChanges(u8 **ppData, u8 *pEnd, FuzzChangeset *pParse){
  ...
  pGrp->aChange = p;
  while( rc==CAPDB_OK && p<pEnd && p[0]!=cHdr ){
    u8 eOp = p[0];
    u8 bIndirect = p[1];
    ...
    }else if( eOp!=CAPDB_INSERT && eOp!=CAPDB_DELETE ){
      rc = fuzzCorrupt();
    }
    if( rc==CAPDB_OK ){
      int bPkOnly = (eOp==CAPDB_DELETE && pParse->bPatchset);
...
** CAPDB_OK. Or, if an error occurs, return an SQLite error code. The
...
** returning. If successful, CAPDB_OK is returned. Otherwise, an SQLite
** error code.
**
...
static void fuzzPrintGroup(FuzzChangeset *pParse, FuzzChangesetGroup *pGrp){
  ...
  for(i=0; i<pGrp->nChange; i++){
    ...
    u8 bIndirect = p[1];
    printf("%s (ind=%d):",
        (eType==CAPDB_INSERT) ? "INSERT" :
        (eType==CAPDB_DELETE ? "DELETE" : "UPDATE"),
        bIndirect
    );
    p += 2;

    if( pParse->bPatchset==0 && eType==CAPDB_UPDATE ){
      fuzzPrintRecord(pGrp, &p, 0);
    }
    fuzzPrintRecord(pGrp, &p, eType==CAPDB_DELETE && pParse->bPatchset);
    printf("\n");
  }
}
...
** changeset it applies etc.). If successful, return CAPDB_OK. Or, if an
...
static int fuzzCopyChange(
  ...
  u8 eType = p++[0];
  int iRec;
  int nRec = ((eType==CAPDB_UPDATE && !bPS) ? 2 : 1);
  int iUndef = -1;
  int nUpdate = 0;
  ...
  if( pFuzz->iCurrent==pFuzz->iChange 
   && pFuzz->eType==FUZZ_CHANGE_FIELD && eType==CAPDB_UPDATE
  ){
    int sz;
    int i;
    int nDef = 0;
  ...
  *(pOut++) = eNew;
    ...
    */
    for(i=0; i<pGrp->nCol; i++){
      ...
      ** write a randomly selected value to the output.  */
      if( bPS && eType==CAPDB_DELETE && pGrp->aPK[i]==0 ){
        if( eType!=eNew ){
          assert( eNew==CAPDB_UPDATE );
          do {
      ...
      if( pCopy[0]==0x00 && eNew!=eType && eType==CAPDB_UPDATE && iRec==0 ){
        while( pCopy[0]==0x00 ){
          pCopy = pParse->apVal[fuzzRandomInt(pParse->nVal)];
        }
      ...
      if( (pFuzz->iGroup!=iGrp || i!=pFuzz->iDelete)
       && (eNew==eType || eType!=CAPDB_UPDATE || iRec==0)
       && (eNew==eType || eNew!=CAPDB_DELETE || !bPS || pGrp->aPK[i])
      ){
        fuzzChangeSize(pCopy, &sz);
        memcpy(pOut, pCopy, sz);
        pOut += sz;
    ...
    if( iGrp==pFuzz->iGroup ){
      if( pFuzz->eType==FUZZ_COLUMN_ADD ){
        if( !bPS || eType!=CAPDB_DELETE ) *(pOut++) = 0x05;
  ...
  if( pFuzz->iCurrent==pFuzz->iChange ){
    ...
    if( eNew!=eType && eNew==CAPDB_UPDATE && !bPS ){
      int i;
      u8 *pCsr = (*ppOut) + 2;
      for(i=0; i<pGrp->nCol; i++){
        int sz;
  ...
  *pp = p;
  *ppOut = pOut;
  pFuzz->iCurrent += (eType==CAPDB_UPDATE || pFuzz->eType!=FUZZ_CHANGE_FIELD);
  return CAPDB_OK;
}
...
**
** Return CAPDB_OK if successful, or an SQLite error code if an error occurs.
*/
static int fuzzDoOneFuzz(
  ...
  while( rc<0 ){
    ...
    for(iGrp=0; rc==CAPDB_OK && iGrp<pParse->nGroup; iGrp++){
      FuzzChangesetGroup *pGrp = pParse->apGroup[iGrp];
      ...
      ** copies of the group. */
      ...
      for(j=0; j<nRep; j++){
        ...
        pSaved = pOut;
        for(i=0; rc==CAPDB_OK && i<pGrp->nChange; i++){
...
int main(int argc, char **argv){
  ...
  if( rc==CAPDB_OK ){
    if( argc==2 ){
      ...
      pBuf = (u8*)fuzzMalloc((capdb_int64)nChangeset*2 + 1024);
      if( pBuf==0 ){
        rc = CAPDB_NOMEM;
      }else{
        iSeed = atoi(argv[2]);
        ...
        for(i=0; rc==CAPDB_OK && i<nRepeat; i++){
          char *zOut = capdb_mprintf("%s-%d", zInput, i);
