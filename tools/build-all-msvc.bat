...
IF NOT DEFINED DLL_FILE_NAME (
  SET DLL_FILE_NAME=capdb.dll
)

IF NOT DEFINED DLL_PDB_FILE_NAME (
  SET DLL_PDB_FILE_NAME=capdb.pdb
)

IF NOT DEFINED LIB_FILE_NAME (
  SET LIB_FILE_NAME=capdb.lib
)

IF NOT DEFINED EXE_FILE_NAME (
  SET EXE_FILE_NAME=capdb.exe
)

IF NOT DEFINED EXE_PDB_FILE_NAME (
  SET EXE_PDB_FILE_NAME=capdbsh.pdb
)
  ...
  FOR /F "tokens=2* delims==" %%D IN ('SET PLATFORMNAME') DO (
    ...
    FOR %%B IN (%CONFIGURATIONS%) DO (
      ...
      IF /I "%%B" == "Debug" (
        ...
        REM       NDEBUG define.  Additionally, both CAPDB_ENABLE_API_ARMOR
        REM       and CAPDB_DEBUG defines should be enabled.
        REM
        SET DEBUG=3

        REM
        REM NOTE: Setting this to non-zero should enable the CAPDB_MEMDEBUG
        REM       define.
        REM
      ...
      REM
      REM       3. Build the "capdb.dll" and "capdb.lib" binaries for this
      REM          platform.
      REM
      REM       4. Copy the "capdb.dll" and "capdb.lib" binaries for this
      REM          platform to the platform-specific directory beneath the
      REM          binary directory.
      REM
      REM       5. Unless prevented from doing so, copy the "capdb.pdb"
      ...
      "%ComSpec%" /C (
        ...
        REM
        REM NOTE: Call NMAKE with the MSVC makefile to build the "capdb.dll"
        ...
        REM
        REM NOTE: Copy the "capdb.dll" file to the appropriate directory for
        ...
        REM
        REM NOTE: Copy the "capdb.lib" file to the appropriate directory for
        REM       the build and platform beneath the binary directory.
        REM
        ...
        REM
        REM NOTE: Copy the "capdb.pdb" file to the appropriate directory for
        ...
        IF DEFINED BUILD_ALL_SHELL (
          ...
          REM
          REM NOTE: Call NMAKE with the MSVC makefile to build the "capdb.exe"
          REM       binary.  The x86 compiler will be used to compile the native
          REM       command line tools needed during the build process itself.
          REM       Also, disable looking for and/or linking to the native Tcl
          REM       runtime library.
          ...
          REM
          REM NOTE: Copy the "capdb.exe" file to the appropriate directory
          ...
          REM
          REM NOTE: Copy the "capdbsh.pdb" file to the appropriate directory
          REM       for the build and platform beneath the binary directory
