...
*/
#if defined(CAPDB_OMIT_ALTERTABLE) || defined(CAPDB_OMIT_VIRTUALTABLE)
#  define ALTER      0
#else
...
#endif
#ifdef CAPDB_OMIT_CONFLICT_CLAUSE
#  define CONFLICT   0
#else
...
#  define TRIGGER    0x00002000
    ...
    (defined(CAPDB_OMIT_VACUUM) || defined(CAPDB_OMIT_ATTACH))
#  define VACUUM     0
#else
...
#endif
#ifdef CAPDB_OMIT_RETURNING
#  define RETURNING  0
#else
#  define RETURNING  0x00400000
#endif
#ifndef CAPDB_ENABLE_ORDERED_SET_AGGREGATES
#  define ORDERSET   0
#else
...
int main(int argc, char **argv){
  ...
  ** number of collisions */
  ...
  printf("  return n;\n");
  printf("}\n");
  printf("int capdbKeywordCode(const unsigned char *z, int n){\n");
  printf("  int id = TK_ID;\n");
  printf("  if( n>=2 ) keywordCode((char*)z, n, &id);\n");
  printf("  return id;\n");
  printf("}\n");
  printf("#define CAPDB_N_KEYWORD %d\n", nKeyword);
  printf("int capdb_keyword_name(int i,const char **pzName,int *pnName){\n");
  printf("  if( i<0 || i>=CAPDB_N_KEYWORD ) return CAPDB_ERROR;\n");
  printf("  i++;\n");
  printf("  *pzName = zKWText + aKWOffset[i];\n");
  printf("  *pnName = aKWLen[i];\n");
  printf("  return CAPDB_OK;\n");
  printf("}\n");
  printf("int capdb_keyword_count(void){ return CAPDB_N_KEYWORD; }\n");
  printf("int capdb_keyword_check(const char *zName, int nName){\n");
  printf("  return TK_ID!=capdbKeywordCode((const u8*)zName, nName);\n");
  printf("}\n");
