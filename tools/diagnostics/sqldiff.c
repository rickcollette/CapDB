...
** against ext/misc/capdb_stdio.c.)
**
...
/*
** Clear and free an capdb_str object
*/
static void strFree(capdb_str *pStr){
  capdb_free(capdb_str_finish(pStr));
}
...
static void cmdlineError(const char *zFormat, ...){
  capdb_str *pOut = capdb_str_new(0);
  va_list ap;
  va_start(ap, zFormat);
  capdb_str_vappendf(pOut, zFormat, ap);
  va_end(ap);
  capdb_fprintf(stderr, "%s: %s\n", g.zArgv0, capdb_str_value(pOut));
  strFree(pOut);
  capdb_fprintf(stderr, "\"%s --help\" for more help\n", g.zArgv0);
  exit(1);
}
...
**
** Space to hold the returned string is obtained from capdb_malloc().  The
...
static char *safeId(const char *zId){
  int i, x;
  char c;
  if( zId[0]==0 ) return capdb_mprintf("\"\"");
  for(i=x=0; (c = zId[i])!=0; i++){
    if( !isalpha(c) && c!='_' ){
      if( i>0 && isdigit(c) ){
        x++;
      }else{
        return capdb_mprintf("\"%w\"", zId);
      }
    }
  }
  if( x || !capdb_keyword_check(zId,i) ){
    return capdb_mprintf("%s", zId);
  }
  return capdb_mprintf("\"%w\"", zId);
}
...
** goes wrong.
*/
static capdb_stmt *db_vprepare(const char *zFormat, va_list ap){
  char *zSql;
  int rc;
  capdb_stmt *pStmt;
  ...
  rc = capdb_prepare_v2(g.db, zSql, -1, &pStmt, 0);
  if( rc ){
...
static capdb_stmt *db_prepare(const char *zFormat, ...){
  va_list ap;
  capdb_stmt *pStmt;
  va_start(ap, zFormat);
...
static void namelistFree(char **az){
  if( az ){
    int i;
    for(i=0; az[i]; i++) capdb_free(az[i]);
    capdb_free(az);
  }
}
...
** hold the list is obtained from capdb_malloc() and should released
...
static char **columnNames(
  ...
  capdb_stmt *pStmt;     /* SQL statement being run */
  ...
  if( g.bSchemaPK==0 ){
    ...
    */
    ...
    while( CAPDB_ROW==capdb_step(pStmt) ){
      if( capdb_stricmp((const char*)capdb_column_text(pStmt,3),"pk")==0 ){
        zPkIdxName = capdb_mprintf("%s", capdb_column_text(pStmt, 1));
        break;
      }
    }
    ...
    if( zPkIdxName ){
      ...
      while( CAPDB_ROW==capdb_step(pStmt) ){
        nCol++;
        if( capdb_column_int(pStmt,5) ){ nKey++; continue; }
        if( capdb_column_int(pStmt,1)>=0 ) truePk = 1;
      }
    ...
    */
    ...
    pStmt = db_prepare("PRAGMA %s.table_info=%Q", zDb, zTab);
    while( CAPDB_ROW==capdb_step(pStmt) ){
      if( capdb_column_int(pStmt,5)>0 ) nPK++;
    }
    capdb_reset(pStmt);
    if( nPK==0 ) nPK = 1;
    truePk = 1;
  ...
  if( g.bSchemaCompare ){
    assert( capdb_stricmp(zTab,"sqlite_schema")==0
            || capdb_stricmp(zTab,"sqlite_master")==0 );
  ...
  *pnPKey = nPK;
  naz = nPK;
  az = capdb_malloc64( sizeof(char*)*(nPK+1) );
  if( az==0 ) runtimeError("out of memory");
  memset(az, 0, sizeof(char*)*(nPK+1));
  if( g.bSchemaCompare ){
    az[0] = capdb_mprintf("%s", "type");
    az[1] = capdb_mprintf("%s", "name");
  }
  while( CAPDB_ROW==capdb_step(pStmt) ){
    char * sid = safeId((char*)capdb_column_text(pStmt,1));
    int iPKey;
    if( truePk && (iPKey = capdb_column_int(pStmt,5))>0 ){
      az[iPKey-1] = sid;
    }else{
      if( !g.bSchemaCompare
          || !(strcmp(sid,"rootpage")==0
               ||strcmp(sid,"name")==0
               ||strcmp(sid,"type")==0)){
        az = capdb_realloc64(az, sizeof(char*)*(naz+2) );
        if( az==0 ) runtimeError("out of memory");
        az[naz++] = sid;
      }
    }
  ...
  ** avoids extraneous diffs against the schemas due to rowid variance. */
  if( az[0]==0 ){
    ...
    for(i=0; i<sizeof(azRowid)/sizeof(azRowid[0]); i++){
      for(j=1; j<naz; j++){
        if( capdb_stricmp(az[j], azRowid[i])==0 ) break;
      }
      if( j>=naz ){
        az[0] = capdb_mprintf("%s", azRowid[i]);
        break;
      }
    }
    if( az[0]==0 ){
      for(i=1; i<naz; i++) capdb_free(az[i]);
      capdb_free(az);
      az = 0;
    }
  }
...
/*
** Print the capdb_value X as an SQL literal.
*/
static void printQuoted(FILE *out, capdb_value *X){
  switch( capdb_value_type(X) ){
    case CAPDB_FLOAT: {
      double r1;
      char zBuf[50];
      r1 = capdb_value_double(X);
      capdb_snprintf(sizeof(zBuf), zBuf, "%!.15g", r1);
      capdb_fprintf(out, "%s", zBuf);
      break;
    }
    case CAPDB_INTEGER: {
      capdb_fprintf(out, "%lld", capdb_value_int64(X));
      break;
    }
    case CAPDB_BLOB: {
      const unsigned char *zBlob = capdb_value_blob(X);
      int nBlob = capdb_value_bytes(X);
      if( zBlob ){
        int i;
        capdb_fprintf(out, "x'");
        for(i=0; i<nBlob; i++){
          capdb_fprintf(out, "%02x", zBlob[i]);
        }
        capdb_fprintf(out, "'");
      }else{
    ...
    case CAPDB_TEXT: {
      const unsigned char *zArg = capdb_value_text(X);

      if( zArg==0 ){
        capdb_fprintf(out, "NULL");
      }else{
        int inctl = 0;
        int i, j;
        capdb_fprintf(out, "'");
        for(i=j=0; zArg[i]; i++){
          ...
          if( ctl>inctl ){
            inctl = ctl;
            capdb_fprintf(out, "%.*s'||X'%02x", i-j, &zArg[j], c);
            j = i+1;
          }else if( ctl ){
            capdb_fprintf(out, "%02x", c);
            j = i+1;
          }else{
            if( inctl ){
              inctl = 0;
            ...
            if( c=='\'' ){
              capdb_fprintf(out, "%.*s'", i-j+1, &zArg[j]);
              j = i+1;
            }
          }
        }
        capdb_fprintf(out, "%s'", &zArg[j]);
      }
      break;
    }
    case CAPDB_NULL: {
...
static void dump_table(const char *zTab, FILE *out){
  ...
  int i;                    /* Loop counter */
  capdb_stmt *pStmt;      /* SQL statement */
  const char *zSep;         /* Separator string */
  capdb_str *pIns;        /* Beginning of the INSERT statement */
  ...
  if( !g.bSchemaOnly ){
    ...
    pIns = capdb_str_new(0);
    if( az==0 ){
      pStmt = db_prepare("SELECT * FROM aux.%s", zId);
      capdb_str_appendf(pIns,"INSERT INTO %s VALUES", zId);
    }else{
      capdb_str *pSql = capdb_str_new(0);
      zSep =  "SELECT";
      for(i=0; az[i]; i++){
        capdb_str_appendf(pSql, "%s %s", zSep, az[i]);
        zSep = ",";
      }
      capdb_str_appendf(pSql," FROM aux.%s", zId);
      zSep = " ORDER BY";
      for(i=1; i<=nPk; i++){
        capdb_str_appendf(pSql, "%s %d", zSep, i);
        zSep = ",";
      }
      pStmt = db_prepare("%s", capdb_str_value(pSql));
      strFree(pSql);
      capdb_str_appendf(pIns, "INSERT INTO %s", zId);
      zSep = "(";
      for(i=0; az[i]; i++){
        capdb_str_appendf(pIns, "%s%s", zSep, az[i]);
        zSep = ",";
      }
      capdb_str_appendf(pIns,") VALUES");
      namelistFree(az);
    }
    nCol = capdb_column_count(pStmt);
    while( CAPDB_ROW==capdb_step(pStmt) ){
      capdb_fprintf(out, "%s",capdb_str_value(pIns));
      zSep = "(";
      for(i=0; i<nCol; i++){
        capdb_fprintf(out, "%s",zSep);
        printQuoted(out, capdb_column_value(pStmt,i));
        zSep = ",";
      }
      capdb_fprintf(out, ");\n");
    }
...
static void diff_one_table(const char *zTab, FILE *out){
  ...
  int i;                    /* Loop counter */
  const char *zSep;         /* Separator string */
  capdb_str *pSql;        /* Comparison query */
  capdb_stmt *pStmt;      /* Query statement to do the diff */
  const char *zLead =       /* Becomes line-comment for sqlite_schema */
    (g.bSchemaCompare)? "-- " : "";

  pSql = capdb_str_new(0);
  if( g.fDebug==DEBUG_COLUMN_NAMES ){
    ...
    */
    az = columnNames("aux",zTab, &nPk, 0);
    if( az==0 ){
      capdb_fprintf(stdout, "Rowid not accessible for %s\n", zId);
    }else{
      capdb_fprintf(stdout, "%s:", zId);
      for(i=0; az[i]; i++){
        capdb_fprintf(stdout, " %s", az[i]);
        if( i+1==nPk ) capdb_fprintf(stdout, " *");
      }
      capdb_fprintf(stdout, "\n");
    }
    goto end_diff_one_table;
  }

  if( capdb_table_column_metadata(g.db,"aux",zTab,0,0,0,0,0,0) ){
    if( !capdb_table_column_metadata(g.db,"main",zTab,0,0,0,0,0,0) ){
      /* Table missing from second database. */
      if( g.bSchemaCompare )
        capdb_fprintf(out, "-- 2nd DB has no %s table\n", zTab);
      else
        capdb_fprintf(out, "DROP TABLE %s;\n", zId);
    }
    goto end_diff_one_table;
  }
  ...
  if( capdb_table_column_metadata(g.db,"main",zTab,0,0,0,0,0,0) ){
    ...
    if( g.bSchemaCompare ){
      capdb_fprintf(out, "-- 1st DB has no %s table\n", zTab);
    }else{
      dump_table(zTab, out);
    }
    goto end_diff_one_table;
  ...
  if( az && az2 ){
    for(n=0; az[n] && az2[n]; n++){
      if( capdb_stricmp(az[n],az2[n])!=0 ) break;
    }
  }
  if( az==0
   || az2==0
   ...
   || az[n]
    ...
    /* Schema mismatch */
    capdb_fprintf(out, "%sDROP TABLE %s; -- due to schema mismatch\n", zLead, zId);
    dump_table(zTab, out);
    goto end_diff_one_table;
  }
  ...
  for(n2=n; az2[n2]; n2++){
    ...
    capdb_fprintf(out, "ALTER TABLE %s ADD COLUMN %s;\n", zId, zNTab);
    capdb_free(zNTab);
  }
  nQ = nPk2+1+2*(n2-nPk2);
  if( n2>nPk2 ){
    zSep = "SELECT ";
    for(i=0; i<nPk; i++){
      capdb_str_appendf(pSql, "%sB.%s", zSep, az[i]);
      zSep = ", ";
    }
    capdb_str_appendf(pSql, ", 1 /* changed row */");
    while( az[i] ){
      capdb_str_appendf(pSql, ", A.%s IS NOT B.%s, B.%s",
                az[i], az2[i], az2[i]);
      i++;
    }
    while( az2[i] ){
      capdb_str_appendf(pSql, ", B.%s IS NOT NULL, B.%s",
                az2[i], az2[i]);
      i++;
    }
    ...
    zSep = " WHERE";
    for(i=0; i<nPk; i++){
      capdb_str_appendf(pSql, "%s A.%s=B.%s", zSep, az[i], az[i]);
      zSep = " AND";
    }
    zSep = "\n   AND (";
    while( az[i] ){
      capdb_str_appendf(pSql, "%sA.%s IS NOT B.%s%s\n",
                zSep, az[i], az2[i], az2[i+1]==0 ? ")" : "");
      zSep = "        OR ";
      i++;
    }
    while( az2[i] ){
      capdb_str_appendf(pSql, "%sB.%s IS NOT NULL%s\n",
                zSep, az2[i], az2[i+1]==0 ? ")" : "");
      zSep = "        OR ";
      i++;
    }
    capdb_str_appendf(pSql, " UNION ALL\n");
  }
  zSep = "SELECT ";
  for(i=0; i<nPk; i++){
    capdb_str_appendf(pSql, "%sA.%s", zSep, az[i]);
    zSep = ", ";
  }
  capdb_str_appendf(pSql, ", 2 /* deleted row */");
  while( az2[i] ){
    capdb_str_appendf(pSql, ", NULL, NULL");
    i++;
  }
  capdb_str_appendf(pSql, "\n  FROM main.%s A\n", zId);
  capdb_str_appendf(pSql, " WHERE NOT EXISTS(SELECT 1 FROM aux.%s B\n", zId);
  zSep =          "                   WHERE";
  for(i=0; i<nPk; i++){
    capdb_str_appendf(pSql, "%s A.%s=B.%s", zSep, az[i], az[i]);
    zSep = " AND";
  ...
  capdb_str_appendf(pSql, ", 3 /* inserted row */");
  while( az2[i] ){
    capdb_str_appendf(pSql, ", 1, B.%s", az2[i]);
    i++;
  }
  capdb_str_appendf(pSql, "\n  FROM aux.%s B\n", zId);
  capdb_str_appendf(pSql, " WHERE NOT EXISTS(SELECT 1 FROM main.%s A\n", zId);
  zSep =          "                   WHERE";
  for(i=0; i<nPk; i++){
    capdb_str_appendf(pSql, "%s A.%s=B.%s", zSep, az[i], az[i]);
    zSep = " AND";
  ...
  pStmt = db_prepare(
    ...
    zTab, zTab);
  while( CAPDB_ROW==capdb_step(pStmt) ){
    char *z = safeId((const char*)capdb_column_text(pStmt,0));
    capdb_fprintf(out, "DROP INDEX %s;\n", z);
    capdb_free(z);
  }
  capdb_finalize(pStmt);
  ...
  if( !g.bSchemaOnly ){
    pStmt = db_prepare("%s", capdb_str_value(pSql));
    while( CAPDB_ROW==capdb_step(pStmt) ){
      int iType = capdb_column_int(pStmt, nPk);
      if( iType==1 || iType==2 ){
        if( iType==1 ){       /* Change the content of a row */
          capdb_fprintf(out, "%sUPDATE %s", zLead, zId);
          zSep = " SET";
          for(i=nPk+1; i<nQ; i+=2){
            if( capdb_column_int(pStmt,i)==0 ) continue;
            capdb_fprintf(out, "%s %s=", zSep, az2[(i+nPk-1)/2]);
            zSep = ",";
            printQuoted(out, capdb_column_value(pStmt,i+1));
          }
        }else{                /* Delete a row */
          capdb_fprintf(out, "%sDELETE FROM %s", zLead, zId);
        }
        zSep = " WHERE";
        for(i=0; i<nPk; i++){
          capdb_fprintf(out, "%s %s=", zSep, az2[i]);
          printQuoted(out, capdb_column_value(pStmt,i));
          zSep = " AND";
        }
        capdb_fprintf(out, ";\n");
      }else{                  /* Insert a row */
        capdb_fprintf(out, "%sINSERT INTO %s(%s", zLead, zId, az2[0]);
        for(i=1; az2[i]; i++) capdb_fprintf(out, ",%s", az2[i]);
        capdb_fprintf(out, ") VALUES");
        zSep = "(";
        for(i=0; i<nPk2; i++){
          capdb_fprintf(out, "%s", zSep);
          zSep = ",";
...
static int rbuDeltaCreate(
  ...
  */
  nHash = lenSrc/NHASH;
  collide = capdb_malloc64( nHash*2*sizeof(int) );
...
static void strPrintfArray(
  capdb_str *pStr,              /* String object to append to */
  const char *zSep,               /* Separator string */
  ...
  for(i=0; az[i] && (i<n || n<0); i++){
    if( i!=0 ) capdb_str_appendf(pStr, "%s", zSep);
    capdb_str_appendf(pStr, zFmt, az[i], az[i], az[i]);
  }
}
...
static void getRbudiffQuery(
  ...
  capdb_str_appendf(pSql, "SELECT ");
  strPrintfArray(pSql, ", ", "%s", azCol, -1);
  capdb_str_appendf(pSql, ", 0, ");  /* Set ota_control to 0 for an insert */
  strPrintfArray(pSql, ", ", "NULL", azCol, -1);
  capdb_str_appendf(pSql, " FROM aux.%Q AS n WHERE NOT EXISTS (\n", zTab);
  capdb_str_appendf(pSql, "    SELECT 1 FROM ", zTab);
  capdb_str_appendf(pSql, " main.%Q AS o WHERE ", zTab);
  ...
  capdb_str_appendf(pSql, ", 1, ");   /* Set ota_control to 1 for a delete */
  strPrintfArray(pSql, ", ", "NULL", azCol, -1);
  capdb_str_appendf(pSql, " FROM main.%Q AS n WHERE NOT EXISTS (\n", zTab);
  capdb_str_appendf(pSql, "    SELECT 1 FROM ", zTab);
  capdb_str_appendf(pSql, " aux.%Q AS o WHERE ", zTab);
  strPrintfArray(pSql, " AND ", "(n.%Q = o.%Q)", azCol, nPK);
  capdb_str_appendf(pSql, "\n) AND ");
  strPrintfArray(pSql, " AND ", "(n.%Q IS NOT NULL)", azCol, nPK);
  ...
  ** be omitted altogether. */
  if( azCol[nPK] ){
    ...
    capdb_str_appendf(pSql, "\nFROM main.%Q AS o, aux.%Q AS n\nWHERE ",
                        zTab, zTab);
    strPrintfArray(pSql, " AND ", "(n.%Q = o.%Q)", azCol, nPK);
    capdb_str_appendf(pSql, " AND ota_control LIKE '%%x%%'");
  }

  /* Now add an ORDER BY clause to sort everything by PK. */
  capdb_str_appendf(pSql, "\nORDER BY ");
  for(i=1; i<=nPK; i++) capdb_str_appendf(pSql, "%s%d", ((i>1)?", ":""), i);
}
...
static void rbudiff_one_table(const char *zTab, FILE *out){
  ...
  int i;
  int nCol;
  capdb_str *pCt;               /* The "CREATE TABLE data_xxx" statement */
  capdb_str *pSql;              /* Query to find differences */
  capdb_str *pInsert;           /* First part of output INSERT statement */
  capdb_stmt *pStmt = 0;
  ...
  ** columns are found, bail out early.  */
  ...
  capdb_str_appendf(pCt, "CREATE TABLE IF NOT EXISTS 'data_%q'(", zTab);
  if( bOtaRowid ) capdb_str_appendf(pCt, "rbu_rowid, ");
  strPrintfArray(pCt, ", ", "%s", &azCol[bOtaRowid], -1);
  capdb_str_appendf(pCt, ", rbu_control);");
  ...
  ** in the data_xxx table. */
  capdb_str_appendf(pInsert, "INSERT INTO 'data_%q' (", zTab);
  if( bOtaRowid ) capdb_str_appendf(pInsert, "rbu_rowid, ");
  strPrintfArray(pInsert, ", ", "%s", &azCol[bOtaRowid], -1);
  capdb_str_appendf(pInsert, ", rbu_control) VALUES(");
  ...
  while( capdb_step(pStmt)==CAPDB_ROW ){
    ...
    ** printed again.  */
    if( capdb_str_length(pCt) ){
      capdb_fprintf(out, "%s\n", capdb_str_value(pCt));
      capdb_str_reset(pCt);
    }

    /* Output the first part of the INSERT statement */
    capdb_fprintf(out, "%s", capdb_str_value(pInsert));
    nRow++;

    if( capdb_column_type(pStmt, nCol)==CAPDB_INTEGER ){
      for(i=0; i<=nCol; i++){
        if( i>0 ) capdb_fprintf(out, ", ");
        printQuoted(out, capdb_column_value(pStmt, i));
      }
    }else{
      char *zOtaControl;
      i64 nOtaControl = capdb_column_bytes(pStmt, nCol);

      zOtaControl = (char*)capdb_malloc64(nOtaControl+1);
      memcpy(zOtaControl, capdb_column_text(pStmt, nCol), nOtaControl+1);

      for(i=0; i<nCol; i++){
        int bDone = 0;
        if( i>=nPK 
            && capdb_column_type(pStmt, i)==CAPDB_BLOB
            && capdb_column_type(pStmt, nCol+1+i)==CAPDB_BLOB
        ){
          const char *aSrc = capdb_column_blob(pStmt, nCol+1+i);
          int nSrc = capdb_column_bytes(pStmt, nCol+1+i);
          const char *aFinal = capdb_column_blob(pStmt, i);
          i64 nFinal = capdb_column_bytes(pStmt, i);
          char *aDelta;
          int nDelta;

          aDelta = capdb_malloc64(nFinal + 60);
          nDelta = rbuDeltaCreate(aSrc, nSrc, aFinal, nFinal, aDelta);
          if( nDelta>0 && nDelta<nFinal ){
            int j;
            capdb_fprintf(out, "x'");
            for(j=0; j<nDelta; j++) capdb_fprintf(out, "%02x", (u8)aDelta[j]);
            capdb_fprintf(out, "'");
      ...
      capdb_fprintf(out, "'%s'", zOtaControl);
      capdb_free(zOtaControl);
    }
  ...
  capdb_finalize(pStmt);
  if( nRow>0 ){
    capdb_str *pCnt = capdb_str_new(0);
    capdb_str_appendf(pCnt,
         "INSERT INTO rbu_count VALUES('data_%q', %d);", zTab, nRow);
    capdb_fprintf(out, "%s\n", capdb_str_value(pCnt));
    strFree(pCnt);
  }
...
static void summarize_one_table(const char *zTab, FILE *out){
  ...
  capdb_str *pSql;        /* Comparison query */
  capdb_stmt *pStmt;      /* Query statement to do the diff */
  capdb_int64 nUpdate;    /* Number of updated rows */
  capdb_int64 nUnchanged; /* Number of unmodified rows */
  capdb_int64 nDelete;    /* Number of deleted rows */
  capdb_int64 nInsert;    /* Number of inserted rows */

  pSql = capdb_str_new(0);
  if( capdb_table_column_metadata(g.db,"aux",zTab,0,0,0,0,0,0) ){
    if( !capdb_table_column_metadata(g.db,"main",zTab,0,0,0,0,0,0) ){
      /* Table missing from second database. */
      capdb_fprintf(out, "%s: missing from second database\n", zTab);
    }
  ...
  if( capdb_table_column_metadata(g.db,"main",zTab,0,0,0,0,0,0) ){
    /* Table missing from source */
    capdb_fprintf(out, "%s: missing from first database\n", zTab);
    goto end_summarize_one_table;
  }
  ...
  if( az==0
   ...
   || az[n]
    ...
    /* Schema mismatch */
    capdb_fprintf(out, "%s: incompatible schema\n", zTab);
    goto end_summarize_one_table;
  }

  /* Build the comparison query */
  for(n2=n; az[n2]; n2++){}
  capdb_str_appendf(pSql, "SELECT 1, count(*)");
  if( n2==nPk2 ){
    capdb_str_appendf(pSql, ", 0\n");
  }else{
    zSep = ", sum(";
    for(i=nPk; az[i]; i++){
      capdb_str_appendf(pSql, "%sA.%s IS NOT B.%s", zSep, az[i], az[i]);
      zSep = " OR ";
    }
    capdb_str_appendf(pSql, ")\n");
  }
  capdb_str_appendf(pSql, "  FROM main.%s A, aux.%s B\n", zId, zId);
  zSep = " WHERE";
  for(i=0; i<nPk; i++){
    capdb_str_appendf(pSql, "%s A.%s=B.%s", zSep, az[i], az[i]);
    zSep = " AND";
  ...
  capdb_str_appendf(pSql, " UNION ALL\n");
  capdb_str_appendf(pSql, "SELECT 2, count(*), 0\n");
  capdb_str_appendf(pSql, "  FROM main.%s A\n", zId);
  capdb_str_appendf(pSql, " WHERE NOT EXISTS(SELECT 1 FROM aux.%s B ", zId);
  zSep = "WHERE";
  for(i=0; i<nPk; i++){
    capdb_str_appendf(pSql, "%s A.%s=B.%s", zSep, az[i], az[i]);
    zSep = " AND";
  ...
  capdb_str_appendf(pSql, ")\n");
  capdb_str_appendf(pSql, " UNION ALL\n");
  capdb_str_appendf(pSql, "SELECT 3, count(*), 0\n");
  capdb_str_appendf(pSql, "  FROM aux.%s B\n", zId);
  capdb_str_appendf(pSql, " WHERE NOT EXISTS(SELECT 1 FROM main.%s A ", zId);
  zSep = "WHERE";
  for(i=0; i<nPk; i++){
    capdb_str_appendf(pSql, "%s A.%s=B.%s", zSep, az[i], az[i]);
    zSep = " AND";
  ...
  if( (g.fDebug & DEBUG_DIFF_SQL)!=0 ){ 
    capdb_fprintf(stdout, "SQL for %s:\n%s\n", zId, capdb_str_value(pSql));
    goto end_summarize_one_table;
  }
  ...
  while( CAPDB_ROW==capdb_step(pStmt) ){
    switch( capdb_column_int(pStmt,0) ){
      case 1:
        nUpdate = capdb_column_int64(pStmt,2);
        nUnchanged = capdb_column_int64(pStmt,1) - nUpdate;
        break;
      case 2:
        nDelete = capdb_column_int64(pStmt,1);
        break;
      case 3:
        nInsert = capdb_column_int64(pStmt,1);
        break;
    }
  }
  capdb_finalize(pStmt);
...
*/
static void putsVarint(FILE *out, capdb_uint64 v){
  int i, n;
  unsigned char p[12];
  if( v & (((capdb_uint64)0xff000000)<<32) ){
    p[8] = (unsigned char)v;
    v >>= 8;
    for(i=7; i>=0; i--){
...
*/
static void putValue(FILE *out, capdb_stmt *pStmt, int k){
  int iDType = capdb_column_type(pStmt, k);
  capdb_int64 iX;
  double rX;
  capdb_uint64 uX;
  int j;
  ...
  switch( iDType ){
    case CAPDB_INTEGER:
      iX = capdb_column_int64(pStmt, k);
      memcpy(&uX, &iX, 8);
      for(j=56; j>=0; j-=8) putc((uX>>j)&0xff, out);
      break;
    case CAPDB_FLOAT:
      rX = capdb_column_double(pStmt, k);
      memcpy(&uX, &rX, 8);
      for(j=56; j>=0; j-=8) putc((uX>>j)&0xff, out);
      break;
    case CAPDB_TEXT:
      iX = capdb_column_bytes(pStmt, k);
      putsVarint(out, (capdb_uint64)iX);
      fwrite(capdb_column_text(pStmt, k),1,(size_t)iX,out);
      break;
    case CAPDB_BLOB:
      iX = capdb_column_bytes(pStmt, k);
      putsVarint(out, (capdb_uint64)iX);
      fwrite(capdb_column_blob(pStmt, k),1,(size_t)iX,out);
      break;
    case CAPDB_NULL:
      break;
  }
...
static void changeset_one_table(const char *zTab, FILE *out){
  ...
  i64 nPk = 0;                  /* Number of PRIMARY KEY columns */
  capdb_str *pSql;            /* SQL for the diff query */
  ...
  while( CAPDB_ROW==capdb_step(pStmt) ){
    nCol++;
    azCol = capdb_realloc64(azCol, sizeof(char*)*nCol);
    if( azCol==0 ) runtimeError("out of memory");
    aiFlg = capdb_realloc64(aiFlg, sizeof(int)*nCol);
    if( aiFlg==0 ) runtimeError("out of memory");
    azCol[nCol-1] = safeId((const char*)capdb_column_text(pStmt,1));
    aiFlg[nCol-1] = i = capdb_column_int(pStmt,5);
    if( i>0 ){
      if( i>nPk ){
        nPk = i;
        aiPk = capdb_realloc64(aiPk, sizeof(int)*nPk);
  ...
  if( nCol>nPk ){
    capdb_str_appendf(pSql, "SELECT %d", CAPDB_UPDATE);
    for(i=0; i<nCol; i++){
      if( aiFlg[i] ){
        capdb_str_appendf(pSql, ",\n       A.%s", azCol[i]);
      }else{
    ...
    zSep = " WHERE";
    for(i=0; i<nPk; i++){
      capdb_str_appendf(pSql, "%s A.%s=B.%s",
    ...
    for(i=0; i<nCol; i++){
      if( aiFlg[i] ) continue;
      capdb_str_appendf(pSql, "%sA.%s IS NOT B.%s", zSep, azCol[i], azCol[i]);
      zSep = " OR\n        ";
    }
    capdb_str_appendf(pSql,")\n UNION ALL\n");
  }
  capdb_str_appendf(pSql, "SELECT %d", CAPDB_DELETE);
  for(i=0; i<nCol; i++){
    if( aiFlg[i] ){
      capdb_str_appendf(pSql, ",\n       A.%s", azCol[i]);
    }else{
  ...
  capdb_str_appendf(pSql, ")\n UNION ALL\n");
  capdb_str_appendf(pSql, "SELECT %d", CAPDB_INSERT);
  for(i=0; i<nCol; i++){
    if( aiFlg[i] ){
      capdb_str_appendf(pSql, ",\n       B.%s", azCol[i]);
    }else{
  ...
  capdb_str_appendf(pSql, ")\n");
  capdb_str_appendf(pSql, " ORDER BY");
  zSep = " ";
  for(i=0; i<nPk; i++){
    capdb_str_appendf(pSql, "%s %d", zSep, aiPk[i]+2);
    zSep = ",";
  }
  capdb_str_appendf(pSql, ";\n");
  ...
  putc('T', out);
  putsVarint(out, (capdb_uint64)nCol);
  ...
  pStmt = db_prepare("%s", capdb_str_value(pSql));
  while( CAPDB_ROW==capdb_step(pStmt) ){
    int iType = capdb_column_int(pStmt,0);
    putc(iType, out);
    putc(0, out);
    switch( capdb_column_int(pStmt,0) ){
      case CAPDB_UPDATE: {
        for(k=1, i=0; i<nCol; i++){
          if( aiFlg[i] ){
            ...
            k++;
          }else if( capdb_column_int(pStmt,k) ){
...
end_changeset_one_table:
  while( nCol>0 ) capdb_free(azCol[--nCol]);
  capdb_free(azCol);
  capdb_free(aiPk);
  capdb_free(zId);
  capdb_free(aiFlg);
...
static void module_name_func(
  capdb_context *pCtx, 
  int nVal, capdb_value **apVal
){
  const char *zSql;
  char zToken[32];

  assert( nVal==1 );
  zSql = (const char*)capdb_value_text(apVal[0]);

  zSql = gobble_token(zSql, zToken, sizeof(zToken));
  if( zSql==0 || capdb_stricmp(zToken, "create") ) return;
  zSql = gobble_token(zSql, zToken, sizeof(zToken));
  if( zSql==0 || capdb_stricmp(zToken, "virtual") ) return;
  zSql = gobble_token(zSql, zToken, sizeof(zToken));
  if( zSql==0 || capdb_stricmp(zToken, "table") ) return;
  zSql = gobble_token(zSql, zToken, sizeof(zToken));
  if( zSql==0 ) return;
  zSql = gobble_token(zSql, zToken, sizeof(zToken));
  if( zSql==0 || capdb_stricmp(zToken, "using") ) return;
  zSql = gobble_token(zSql, zToken, sizeof(zToken));
  
  capdb_result_text(pCtx, zToken, -1, CAPDB_TRANSIENT);
}
...
const char *all_tables_sql(){
  if( g.bHandleVtab ){
    ...
    rc = capdb_create_function(
        g.db, "module_name", 1, CAPDB_UTF8, 0, module_name_func, 0, 0
    );
    assert( rc==CAPDB_OK );
...
static void showHelp(void){
  capdb_fprintf(stdout, "Usage: %s [options] DB1 DB2\n", g.zArgv0);
  capdb_fprintf(stdout,
...
int main(int argc, char **argv){
  ...
  g.zArgv0 = argv[0];
  capdb_config(CAPDB_CONFIG_SINGLETHREAD);
  for(i=1; i<argc; i++){
    const char *z = argv[i];
    if( z[0]=='-' ){
      z++;
      ...
      if( strcmp(z,"table")==0 ){
        ...
        zTab = argv[++i];
        g.bSchemaCompare =
          capdb_stricmp(zTab, "sqlite_schema")==0
          || capdb_stricmp(zTab, "sqlite_master")==0;
      }else
  ...
  rc = capdb_open_v2(zDb1, &g.db, CAPDB_OPEN_READONLY, 0);
  if( rc ){
    cmdlineError("cannot open database file \"%s\"", zDb1);
  }
  rc = capdb_exec(g.db, "SELECT * FROM sqlite_schema", 0, 0, &zErrMsg);
  if( rc || zErrMsg ){
    cmdlineError("\"%s\" does not appear to be a valid SQLite database", zDb1);
  }
  {
    capdb *db2 = 0;
    if( capdb_open_v2(zDb2, &db2, CAPDB_OPEN_READONLY, 0) ){
      cmdlineError("cannot open database file \"%s\"", zDb2);
    }
    capdb_close(db2);
  }
#ifndef CAPDB_OMIT_LOAD_EXTENSION
  capdb_enable_load_extension(g.db, 1);
  for(i=0; i<nExt; i++){
    rc = capdb_load_extension(g.db, azExt[i], 0, &zErrMsg);
    if( rc || zErrMsg ){
  ...
  free(azExt);
#endif
  zSql = capdb_mprintf("ATTACH %Q as aux;", zDb2);
  rc = capdb_exec(g.db, zSql, 0, 0, &zErrMsg);
  capdb_free(zSql);
  zSql = 0;
  if( rc || zErrMsg ){
    cmdlineError("cannot attach database \"%s\"", zDb2);
  }
  rc = capdb_exec(g.db, "SELECT * FROM aux.sqlite_schema", 0, 0, &zErrMsg);
  if( rc || zErrMsg ){
    cmdlineError("\"%s\" does not appear to be a valid SQLite database", zDb2);
  }

  if( neverUseTransaction ) useTransaction = 0;
  if( useTransaction ) capdb_fprintf(out, "BEGIN TRANSACTION;\n");
  if( xDiff==rbudiff_one_table ){
    capdb_fprintf(out, "CREATE TABLE IF NOT EXISTS rbu_count"
  ...
  if( zTab ){
    ...
    while( CAPDB_ROW==capdb_step(pStmt) ){
      xDiff((const char*)capdb_column_text(pStmt,0), out);
    }
    capdb_finalize(pStmt);
  }
  if( useTransaction ) capdb_fprintf(stdout,"COMMIT;\n");
