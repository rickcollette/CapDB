...
** just like the "capdb" command-line shell.  Differences from the
** command-line shell:
**
...
**         like ".shell rm -rf ~"
**
**    (2)  The database is opened with the CAPDB_OPEN_MEMORY flag so that
...
struct GlobalVars {
  ...
  capdb_mem_methods sOrigMem;    /* Original memory methods */
  capdb_mem_methods sOomMem;     /* Memory methods with OOM simulator */
...
*/
static void sqlexec(capdb *db, const char *zFormat, ...){
  va_list ap;
  char *zSql;
  char *zErrMsg = 0;
  int rc;
...
/*
** This callback is invoked by capdb_log().
*/
...
/*
** This callback is invoked by capdb_exec() to return query results.
*/
...
/*
** This callback is invoked by capdb_trace() as each SQL statement
** starts.
*/
...
struct Str {
  ...
  capdb_uint64 n;       /* Bytes of input used */
  capdb_uint64 nAlloc;  /* Bytes allocated to z[] */
...
static void StrAppend(Str *p, const char *z){
  capdb_uint64 n = strlen(z);
  if( p->n + n >= p->nAlloc ){
    char *zNew;
    capdb_uint64 nNew;
    if( p->oomErr ) return;
    nNew = p->nAlloc*2 + 100 + n;
    zNew = capdb_realloc(p->z, (int)nNew);
    if( zNew==0 ){
      capdb_free(p->z);
      memset(p, 0, sizeof(*p));
      p->oomErr = 1;
...
static int callback(void *pCtx, int argc, char **argv, char **colnames){
  ...
  for(i=0; i<argc; i++){
    ...
    if( (capdb_int64)sz+p->nUsed+p->szSep+1 > p->nAlloc ){
      ...
      ** of SQLite.  */
      zNew = p->nAlloc<=0x7fffffff ? capdb_realloc(p->z, (int)p->nAlloc) : 0;
      if( zNew==0 ){
        capdb_free(p->z);
        memset(p, 0, sizeof(*p));
        return 1;
...
struct series_cursor {
  ...
  capdb_int64 iValue;      /* Current value ("value") */
  capdb_int64 mnValue;     /* Mimimum value ("start") */
  capdb_int64 mxValue;     /* Maximum value ("stop") */
  capdb_int64 iStep;       /* Increment ("step") */
};
...
*/
static int seriesOpen(capdb_vtab *p, capdb_vtab_cursor **ppCursor){
  series_cursor *pCur;
...
static int seriesFilter(
  ...
  int i = 0;
  if( idxNum & 1 ){
    pCur->mnValue = capdb_value_int64(argv[i++]);
  }else{
    pCur->mnValue = 0;
  }
  if( idxNum & 2 ){
    pCur->mxValue = capdb_value_int64(argv[i++]);
  }else{
    pCur->mxValue = 0xffffffff;
  }
  if( idxNum & 4 ){
    pCur->iStep = capdb_value_int64(argv[i++]);
...
static int seriesBestIndex(
  ...
  if( startIdx>=0 ){
    ...
    pIdxInfo->aConstraintUsage[startIdx].omit= !CAPDB_SERIES_CONSTRAINT_VERIFY;
  }
  if( stopIdx>=0 ){
    pIdxInfo->aConstraintUsage[stopIdx].argvIndex = ++nArg;
    pIdxInfo->aConstraintUsage[stopIdx].omit = !CAPDB_SERIES_CONSTRAINT_VERIFY;
  }
  if( stepIdx>=0 ){
    pIdxInfo->aConstraintUsage[stepIdx].argvIndex = ++nArg;
    pIdxInfo->aConstraintUsage[stepIdx].omit = !CAPDB_SERIES_CONSTRAINT_VERIFY;
  }
  if( (idxNum & 3)==3 ){
...
int main(int argc, char **argv){
  ...
  int rc = CAPDB_OK;           /* Result codes from API functions */
  int i;                        /* Loop counter */
  int iNext;                    /* Next block of SQL */
  capdb *db;                  /* Open database */
  char *zErrMsg = 0;            /* Error message returned from capdb_exec() */
  ...
  char *zToFree = 0;            /* Call capdb_free() on this afte running zSql */
  ...
  capdb *dataDb = 0;          /* Database holding compacted input data */
  capdb_stmt *pStmt = 0;      /* Statement to insert testcase into dataDb */
  ...
  capdb_int64 iBegin;         /* Start time for the whole program */
  capdb_int64 iStart, iEnd;   /* Start and end-times for a test case */
  ...
  capdb_config(CAPDB_CONFIG_LOG, verboseFlag ? shellLog : shellLogNoop, 0);
  if( nHeap>0 ){
    pHeap = malloc( nHeap );
  ...
  if( oomFlag ){
    capdb_config(CAPDB_CONFIG_GETMALLOC, &g.sOrigMem);
    g.sOomMem = g.sOrigMem;
    g.sOomMem.xMalloc = oomMalloc;
    g.sOomMem.xRealloc = oomRealloc;
    capdb_config(CAPDB_CONFIG_MALLOC, &g.sOomMem);
  }
  if( nLook>0 ){
  ...
  if( nScratch>0 && szScratch>0 ){
    pScratch = malloc( nScratch*(capdb_int64)szScratch );
    if( pScratch==0 ) fatalError("cannot allocate %lld-byte scratch",
                                 nScratch*(capdb_int64)szScratch);
    rc = capdb_config(CAPDB_CONFIG_SCRATCH, pScratch, szScratch, nScratch);
  ...
  */
  if( zDataOut ){
    rc = capdb_open(":memory:", &dataDb);
    if( rc ) abendError("cannot open :memory: database");
    rc = capdb_exec(dataDb,
          "CREATE TABLE testcase(sql BLOB PRIMARY KEY, tm) WITHOUT ROWID;",0,0,0);
    if( rc ) abendError("%s", capdb_errmsg(dataDb));
    rc = capdb_prepare_v2(dataDb,
    ...
    */
    for(; i<nIn; i=iNext, nTest++, g.zTestName[0]=0){
      ...
      if( strncmp(&zIn[i], "/****<",6)==0 ){
        ...
        if( z ){
          z += 6;
          capdb_snprintf(sizeof(g.zTestName), g.zTestName, "%.*s", 
      ...
      */
      ...
      do{
        ...
        StrInit(&sql);
        if( zDbName ){
          rc = capdb_open_v2(zDbName, &db, CAPDB_OPEN_READWRITE, 0);
          if( rc!=CAPDB_OK ){
          ...
          rc = capdb_open_v2(
            "main.db", &db,
            CAPDB_OPEN_READWRITE | CAPDB_OPEN_CREATE | CAPDB_OPEN_MEMORY,
            0);
          if( rc!=CAPDB_OK ){
        ...
        if( pLook ){
          rc = capdb_db_config(db, CAPDB_DBCONFIG_LOOKASIDE,pLook,szLook,nLook);
          if( rc!=CAPDB_OK ) abendError("lookaside configuration filed: %d", rc);
        }
    #ifndef CAPDB_OMIT_TRACE
        capdb_trace(db, verboseFlag ? traceCallback : traceNoop, 0);
    #endif
        capdb_create_function(db, "eval", 1, CAPDB_UTF8, 0, sqlEvalFunc, 0, 0);
        capdb_create_function(db, "eval", 2, CAPDB_UTF8, 0, sqlEvalFunc, 0, 0);
        capdb_create_module(db, "generate_series", &seriesModule, 0);
        capdb_limit(db, CAPDB_LIMIT_LENGTH, 1000000);
        ...
        ** fuzzed at the same time. */
        if( capdb_table_column_metadata(db,0,"autoexec","sql",0,0,0,0,0)==0 ){
          capdb_stmt *pStmt2;
          rc = capdb_prepare_v2(db,"SELECT sql FROM autoexec",-1,&pStmt2,0);
          if( rc==CAPDB_OK ){
            while( capdb_step(pStmt2)==CAPDB_ROW ){
              StrAppend(&sql, (const char*)capdb_column_text(pStmt2, 0));
              StrAppend(&sql, "\n");
            }
          }
        ...
        if( verboseFlag ){
          zErrMsg = 0;
          rc = capdb_exec(db, zSql, execCallback, 0, &zErrMsg);
          if( zErrMsg ){
            capdb_snprintf(sizeof(zErrBuf),zErrBuf,"%z", zErrMsg);
            zErrMsg = 0;
          }
        }else {
          rc = capdb_exec(db, zSql, execNoop, 0, 0);
        }
        g.bOomEnable = 0;
        iEnd = timeOfDay();
        StrFree(&sql);
        rc = capdb_close(db);
        if( rc ){
          abendError("capdb_close() failed with rc=%d", rc);
        }
        if( !zDataOut && capdb_memory_used()>0 ){
          abendError("memory in use after close: %lld bytes",capdb_memory_used());
        }
        if( oomFlag ){
      ...
      */
      if( zDataOut ){
        capdb_bind_blob(pStmt, 1, &zIn[i], iNext-i, CAPDB_STATIC);
        capdb_bind_int64(pStmt, 2, iEnd - iStart);
        rc = capdb_step(pStmt);
        if( rc!=CAPDB_DONE ) abendError("%s", capdb_errmsg(dataDb));
        capdb_reset(pStmt);
      }
  ...
  */
  if( nTest>1 && !quietFlag ){
    capdb_int64 iElapse = timeOfDay() - iBegin;
  ...
  */
  if( zDataOut ){
    ...
    rc = capdb_prepare_v2(dataDb, "SELECT sql, tm FROM testcase ORDER BY tm, sql",
                            -1, &pStmt, 0);
    if( rc ) abendError("%s", capdb_errmsg(dataDb));
    while( capdb_step(pStmt)==CAPDB_ROW ){
      fprintf(out,"/****<%d:%dms>****/", ++n, capdb_column_int(pStmt,1));
      fwrite(capdb_column_blob(pStmt,0),capdb_column_bytes(pStmt,0),1,out);
    }
    fclose(out);
    capdb_finalize(pStmt);
    capdb_close(dataDb);
