...
*/
static void vacuumFinalize(capdb_stmt *pStmt){
...
static void execSql(capdb *db, const char *zSql){
  ...
  printf("%s;\n", zSql);
  if( CAPDB_OK!=capdb_prepare(db, zSql, -1, &pStmt, 0) ){
...
** exits.
*/
static void execExecSql(capdb *db, const char *zSql){
  capdb_stmt *pStmt;
  int rc;
  ...
  while( CAPDB_ROW==capdb_step(pStmt) ){
    execSql(db, (char*)capdb_column_text(pStmt, 0));
  }
  vacuumFinalize(pStmt);
}
...
int main(int argc, char **argv){
  capdb *db;                 /* Connection to the database file */
  int rc;                      /* Return code from SQLite interface calls */
  capdb_uint64 r;            /* A random number */
  ...
  */
  ...
  rc = capdb_open(zDbToVacuum, &db);
  if( rc ){
    fprintf(stderr, "%s: %s\n", zDbToVacuum, capdb_errstr(rc));
    return 1;
  }
  ...
  */
  ...
  zTempDb = capdb_mprintf("%s-vacuum-%016llx", zDbToVacuum, r);
  zBackupDb = capdb_mprintf("%s-backup-%016llx", zDbToVacuum, r);

  /* Attach the zTempDb database to the database connection.
  */
  zSql = capdb_mprintf("ATTACH '%q' AS vacuum_db;", zTempDb);
  execSql(db, zSql);
  capdb_free(zSql);
