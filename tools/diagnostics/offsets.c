...
static void ofstError(GState *p, const char *zFormat, ...){
  ...
  capdb_free(p->zErr);
  va_start(ap, zFormat);
  p->zErr = capdb_vmprintf(zFormat, ap);
  va_end(ap);
}
...
static void ofstRootAndColumn(
  ...
  zSql = capdb_mprintf("SELECT rootpage FROM sqlite_schema WHERE name=%Q",
                         zTable);
  rc = capdb_prepare_v2(db, zSql, -1, &pStmt, 0);
  if( rc ) ofstError(p, "%s: [%s]", capdb_errmsg(db), zSql);
  capdb_free(zSql);
  ...
  p->iRoot = capdb_column_int(pStmt , 0);
  capdb_finalize(pStmt);

  p->iCol = -1;
  zSql = capdb_mprintf("PRAGMA table_info(%Q)", zTable);
  rc = capdb_prepare_v2(db, zSql, -1, &pStmt, 0);
  if( rc ) ofstError(p, "%s: [%s}", capdb_errmsg(db), zSql);
  capdb_free(zSql);
  if( p->zErr ) goto rootAndColumn_exit;
  while( capdb_step(pStmt)==CAPDB_ROW ){
    const char *zCol = capdb_column_text(pStmt, 1);
    if( strlen(zCol)==strlen(zColumn)
     && capdb_strnicmp(zCol, zColumn, strlen(zCol))==0
    ){
      p->iCol = capdb_column_int(pStmt, 0);
      break;
    }
  }
  capdb_finalize(pStmt);
  ...
  if( capdb_step(pStmt)!=CAPDB_ROW ){
    ...
    p->szPg = capdb_column_int(pStmt, 0);
  }
  capdb_finalize(pStmt);
...
static void ofstPushPage(GState *p, int pgno){
  ...
  p->aStack[p->nStack] = pPage = capdb_malloc( p->szPg );
  if( pPage==0 ){
...
static capdb_int64 ofstVarint(GState *p, int *pOfst){
  capdb_int64 x = 0;
  u8 *a = &p->aPage[*pOfst];
  int n = 0;
...
static void ofstWalkLeafPage(GState *p){
  ...
  for(i=0; i<nCell; i++){
    ...
    if( nPayload > p->szPg-35 ){
      capdb_snprintf(sizeof(zMsg), zMsg,
