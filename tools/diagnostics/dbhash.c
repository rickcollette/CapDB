...
static void hash_one_query(const char *zFormat, ...){
  va_list ap;
  capdb_stmt *pStmt;        /* The query defined by zFormat and "..." */
  ...
  while( CAPDB_ROW==capdb_step(pStmt) ){
    for(i=0; i<nCol; i++){
      switch( capdb_column_type(pStmt,i) ){
        ...
        case CAPDB_TEXT: {
          int n = capdb_column_bytes(pStmt, i);
          const unsigned char *z = capdb_column_text(pStmt, i);
        ...
        case CAPDB_BLOB: {
          int n = capdb_column_bytes(pStmt, i);
          const unsigned char *z = capdb_column_blob(pStmt, i);
...
int main(int argc, char **argv){
  ...
  capdb_stmt *pStmt;         /* An SQLite query */
  ...
  for(i=1; i<=nFile; i++){
    static const int openFlags = 
       CAPDB_OPEN_READWRITE |     /* Read/write so hot journals can recover */
       CAPDB_OPEN_URI
    ;
    zDb = argv[i];
    rc = capdb_open_v2(zDb, &g.db, openFlags, 0);
    if( rc ){
    ...
    if( !omitContent ){
      ...
      while( CAPDB_ROW==capdb_step(pStmt) ){
        ...
        ** can be safely omitted. */
        hash_one_query("SELECT * FROM \"%w\"", capdb_column_text(pStmt,0));
      }
      capdb_finalize(pStmt);
    }
