...
static void usage(const char *argv0){
  ...
  printf("\nAnalysis will be done by SQLite version %s dated %.20s\n"
         ...
         capdb_libversion(), capdb_sourceid(), capdb_sourceid()+21);
  exit(1);
}

int main(int argc, char **argv){
  capdb *db = 0;          /* The main database */
  capdb_stmt *pStmt = 0;  /* a query */
  char *zSql;
  int nErr = 0;
  int rc;
  int bQuiet = 0;
  ...
  rc = capdb_open_v2(argv[1], &db, CAPDB_OPEN_READONLY, 0);
  if( rc ){
    printf("Cannot open \"%s\" for reading: %s\n", argv[1], capdb_errmsg(db));
    goto errorOut;
  }
  rc = capdb_prepare_v2(db, "SELECT * FROM sqlite_schema", -1, &pStmt, 0);
  if( rc ){
  ...
  while( capdb_step(pStmt)==CAPDB_ROW ){
    const char *zLog = (const char*)capdb_column_text(pStmt, 0);
    capdb_stmt *pS2;
    if( zLog==0 ) continue;
    zSql = capdb_mprintf("EXPLAIN QUERY PLAN %s", zLog);
    rc = capdb_prepare_v2(db, zSql, -1, &pS2, 0);
    capdb_free(zSql);
    if( rc ){
      if( !bQuiet ){
        printf("Cannot compile LOG entry %d (%s): %s\n",
             capdb_column_int(pStmt, 1), zLog, capdb_errmsg(db));
        fflush(stdout);
      }
      nErr++;
    }else{
      ...
      while( capdb_step(pS2)==CAPDB_ROW ){
        const char *zExplain = (const char*)capdb_column_text(pS2,3);
        const char *z1, *z2;
        int n;
        /* printf("EXPLAIN: %s\n", zExplain); */
        ...
        n = z2 - z1;
        if( zUsing && capdb_strnicmp(zUsing, z1, n)==0 ){
          printf("Using %s:\n%s\n", zUsing, zLog);
          fflush(stdout);
        }
        capdb_bind_text(pIncrCnt,1,z1,n,CAPDB_STATIC);
        capdb_step(pIncrCnt);
        capdb_reset(pIncrCnt);
      }
    }
    capdb_finalize(pS2);
  }
  ...
  while( capdb_step(pStmt)==CAPDB_ROW ){
    printf("%10d %s on %s(%s)\n", 
       ...
       capdb_column_text(pStmt, 3));
  }
  capdb_finalize(pStmt);
  pStmt = 0;

errorOut:
  capdb_finalize(pIncrCnt);
  capdb_finalize(pStmt);
  capdb_close(db);
  return nErr;
}
