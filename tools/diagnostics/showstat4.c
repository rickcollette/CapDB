...
typedef capdb_int64 i64;   /* 64-bit signed integer type */
...
int main(int argc, char **argv){
  ...
  rc = capdb_prepare_v2(db,
        ...
        &pStmt, 0);
  if( rc!=CAPDB_OK || pStmt==0 ){
    fprintf(stderr, "%s\n", capdb_errmsg(db));
    capdb_close(db);
    exit(1);
  }
  while( CAPDB_ROW==capdb_step(pStmt) ){
    if( zIdx==0 || strcmp(zIdx, (const char*)capdb_column_text(pStmt,0))!=0 ){
      if( zIdx ) printf("\n**************************************"
                        "**************\n\n");
      capdb_free(zIdx);
      zIdx = capdb_mprintf("%s", capdb_column_text(pStmt,0));
      if( zIdx==0 ){ fprintf(stderr, "out of memory\n"); abort(); }
      iRow = 0;
    }
    printf("%s sample %d ------------------------------------\n", zIdx, ++iRow);
    printf("  nEq    = %s\n", capdb_column_text(pStmt,1));
    printf("  nLt    = %s\n", capdb_column_text(pStmt,2));
    printf("  nDLt   = %s\n", capdb_column_text(pStmt,3));
    printf("  sample = x'");
    aSample = capdb_column_blob(pStmt,4);
    nSample = capdb_column_bytes(pStmt,4);
