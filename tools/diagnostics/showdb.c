...
typedef capdb_int64 i64;        /* signed 64-bit */
typedef capdb_uint64 u64;       /* unsigned 64-bit */
...
static struct GlobalData {
  ...
  capdb_file *pFd;              /* File descriptor for non-raw mode */
  capdb *pDb;                   /* Database handle that owns pFd */
...
*/
static capdb *openDatabase(const char *zPrg, const char *zName){
  capdb *db = 0;
  int flags = CAPDB_OPEN_READWRITE | CAPDB_OPEN_URI;
  int rc = capdb_open_v2(zName, &db, flags, 0);
  if( rc!=CAPDB_OK ){
    const char *zErr = capdb_errmsg(db);
...
static void fileOpen(const char *zPrg, const char *zName){
  ...
  if( g.bRaw==0 ){
    ...
    rc = capdb_file_control(g.pDb, "main", CAPDB_FCNTL_FILE_POINTER, pArg);
    if( rc!=CAPDB_OK ){
      fprintf(stderr, 
...
**
** Space to hold the content is obtained from capdb_malloc() and needs 
** to be freed by the caller.
*/
static unsigned char *fileRead(capdb_int64 ofst, int nByte){
  unsigned char *aData;
  int got;
  int rc;
  aData = capdb_malloc64(32+(i64)nByte);
...
static unsigned char *print_byte_range(
  capdb_int64 ofst,  /* First byte in the range of bytes to print */
  ...
  if( nLocal<nPayload ){
    ...
    while( ovfl && (cnt++)<g.mxPage ){
      page_usage_msg(ovfl, "overflow %d from cell %d of page %u",
                     cnt, cellno, pgno);
      a = fileRead((ovfl-1)*(capdb_int64)g.pagesize, g.pagesize);
      ovfl = decodeInt32(a);
      capdb_free(a);
    }
  }
...
static void page_usage_btree(
  ...
  if( nCell==1 ){
    capdb_snprintf(sizeof(zEntry),zEntry,"1 row");
  }else{
    capdb_snprintf(sizeof(zEntry),zEntry,"%d rows", nCell);
  }
  if( parent>0 ){
...
static void page_usage_report(const char *zPrg, const char *zDbName){
  ...
  ** usages */
  g.zPageUse = capdb_malloc64( sizeof(g.zPageUse[0])*(g.mxPage+1) );
  ...
  if( g.bTmstmp && a[20]==16 ){
    g.aPageTag = capdb_malloc64( sizeof(struct TmstmpTag)*(g.mxPage+1) );
  ...
  capdb_exec(db, "PRAGMA writable_schema=ON", 0, 0, 0);
  for(j=0; j<2; j++){
    capdb_snprintf(sizeof(zQuery), zQuery,
             "SELECT type, name, rootpage FROM CAPDB_MASTER WHERE rootpage"
             " ORDER BY rowid %s", j?"DESC":"");
    rc = capdb_prepare_v2(db, zQuery, -1, &pStmt, 0);
    if( rc==CAPDB_OK ){
      while( capdb_step(pStmt)==CAPDB_ROW ){
        u32 pgno = (u32)capdb_column_int64(pStmt, 2);
        page_usage_btree(pgno, 0, 0, (const char*)capdb_column_text(pStmt,1));
      }
    }else{
  ...
  for(i=1; i<=g.mxPage; i++){
    capdb_free(g.zPageUse[i]);
  }
  capdb_free(g.zPageUse);
  g.zPageUse = 0;
}
