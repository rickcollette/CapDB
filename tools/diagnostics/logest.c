...
LogEst logEstFromInteger(capdb_uint64 x){
...
static capdb_uint64 logEstToInt(LogEst x){
  capdb_uint64 n;
  if( x<10 ) return 1;
  n = x%10;
  x /= 10;
  ...
  if( x>60 ) return (((capdb_uint64)0xffffffff)<<32)+(capdb_uint64)0xffffffff;
...
static LogEst logEstFromDouble(double x){
  ...
  if( x<1024.0 ) return logEstFromInteger((capdb_uint64)(1024.0*x)) - 100;
  if( x<=2000000000.0 ) return logEstFromInteger((capdb_uint64)x);
  memcpy(&a, &x, 8);
  e = (a>>52) - 1022;
  return e*10;
}
...
int main(int argc, char **argv){
  ...
  for(i=n-1; i>=0; i--){
    if( a[i]<-40 ){
      ...
      capdb_uint64 x = logEstToInt(a[i]+100)*100/1024;
      printf("%5d (%lld.%02lld)\n", a[i], x/100, x%100);
    }
  }
  return 0;
