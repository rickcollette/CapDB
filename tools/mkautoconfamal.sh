...
#
#   1. The files "capdb.c", "capdb.h", "capdbext.h", "shell.c",
#      and "capdbrc.h" are available in the current directory.
#
...
cp capdb.c              $TMPSPACE
cp capdb.h              $TMPSPACE
cp capdbext.h           $TMPSPACE
cp capdbrc.h            $TMPSPACE
cp $TOP/capdb.1         $TMPSPACE
cp $TOP/capdb.pc.in     $TMPSPACE
cp shell.c                $TMPSPACE
cp $TOP/src/capdb.rc    $TMPSPACE
...
mkdir -p tea/generic
cat <<EOF > tea/generic/tclcapdb.c
...
#endif
EOF
cat  $TOP/src/tclsqlite.c           >> tea/generic/tclcapdb.c
