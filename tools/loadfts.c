...
static void readfileFunc(
  ...
  nIn = ftell(in);
  rewind(in);
  pBuf = capdb_malloc( nIn );
  if( pBuf && 1==fread(pBuf, nIn, 1, in) ){
    capdb_result_text(context, pBuf, nIn, capdb_free);
  }else{
    capdb_free(pBuf);
  }
  fclose(in);
...
*/
static void sqlite_error_out(const char *zText, capdb *db){
  fprintf(stderr, "%s: %s\n", zText, capdb_errmsg(db));
  exit(-1);
}
...
struct VisitContext {
  ...
  capdb_stmt *pInsert;          /* INSERT INTO fts VALUES(readtext(:1)) */
};
...
void visit_file(void *pCtx, const char *zPath){
  ...
  /* printf("%s\n", zPath); */
  capdb_bind_text(p->pInsert, 1, zPath, -1, CAPDB_STATIC);
  capdb_step(p->pInsert);
  rc = capdb_reset(p->pInsert);
  if( rc!=CAPDB_OK ){
    sqlite_error_out("insert", p->db);
  }else if( p->nRowPerTrans>0 
         && (capdb_last_insert_rowid(p->db) % p->nRowPerTrans)==0 
  ){
    capdb_exec(p->db, "COMMIT ; BEGIN", 0, 0, 0);
  }
}
...
static void traverse(
  ...
  for(e=readdir(d); e; e=readdir(d)){
    ...
    char *zPath = capdb_mprintf("%s/%s", zDir, e->d_name);
...
int main(int argc, char **argv){
  ...
  for(i=1; i<(argc-1); i+=2){
    ...
    if( strcmp(zOpt, "-fts")==0 ){
      ...
      nCmd++;
      aCmd = capdb_realloc(aCmd, sizeof(char*) * nCmd);
      aCmd[nCmd-1] = zArg;
    }
    else{
      showHelp(argv[0]);
  ...
  rc = capdb_open(argv[argc-1], &db);
  if( rc!=CAPDB_OK ) sqlite_error_out("capdb_open()", db);

  rc = capdb_create_function(db, "readtext", 1, CAPDB_UTF8, 0,
                               readfileFunc, 0, 0);
  if( rc!=CAPDB_OK ) sqlite_error_out("capdb_create_function()", db);

  /* Create the FTS table */
  zSql = capdb_mprintf("CREATE VIRTUAL TABLE fts USING fts%d(content)", iFts);
  rc = capdb_exec(db, zSql, 0, 0, 0);
  if( rc!=CAPDB_OK ) sqlite_error_out("capdb_exec(1)", db);
  capdb_free(zSql);

  for(i=0; i<nCmd; i++){
    zSql = capdb_mprintf("INSERT INTO fts(fts) VALUES(%Q)", aCmd[i]);
    rc = capdb_exec(db, zSql, 0, 0, 0);
    if( rc!=CAPDB_OK ) sqlite_error_out("capdb_exec(1)", db);
    capdb_free(zSql);
  }
  ...
  if( rc!=CAPDB_OK ) sqlite_error_out("capdb_prepare_v2(1)", db);

  /* Load all files in the directory hierarchy into the FTS table. */
  if( sCtx.nRowPerTrans>0 ) capdb_exec(db, "BEGIN", 0, 0, 0);
  traverse(zDir, (void*)&sCtx, visit_file);
  if( sCtx.nRowPerTrans>0 ) capdb_exec(db, "COMMIT", 0, 0, 0);

  /* Clean up and exit. */
  capdb_finalize(sCtx.pInsert);
  capdb_close(db);
  capdb_free(aCmd);
  return 0;
}
